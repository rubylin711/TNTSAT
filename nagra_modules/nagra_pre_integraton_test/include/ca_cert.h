...
**
**  $Id: //CAK/components/certapi/TAGS/CERTAPI_1_4_1/src/ca_cert.h#1 $
**  $Change: 326199 $
**
*/
/*
...
#define CERTAPI_VERSION_MEDIUM 4
#define CERTAPI_VERSION_MINOR  1

/** @page p_cert_history Changes history
 *  - <b> 1.4.1 - 26-Nov-2019 </b>
 *    - Added clarifications to TCertAklReset()
 *
 *  - <b> 1.4.0 - 18-Mar-2019 </b>
 *    - Added TCertAklReset()
 *
 *  - <b> 1.3.2 - 27-Jul-2015 </b>
 *    - Internal documentation structure modifications.
 *
 *  - <b> 1.3.1 - 14-Nov-2014 </b>
...
/* Main page moved unchanged to doc/main.dox. */

/** @page p_cert_preface Preface
 *  <h2>Objectives</h2>
 *
 *  This document specifies the API that gives access the Nagravision CERT 
 *  proprietary IP block (Concurrent Embedded Root of Trust).
 *
 *  <hr><h2>Audience</h2>
 ...
 *
 * <hr><h2>References</h2>
 *  - [NSR] Nagravision, CERT IP Block, Nagra Specific Request (NSR)
 *  
 */
 
/** @page p_cert_conventions Used conventions
 *  <h2>Bit indexing</h2>
...
/** @page p_cert_sw_stack Software stack context
  
  The CERT API shall be implemented in a bare mode (i.e. before the 
  operating system is started) and operating system mode. In case of TEE-based
  architecture, the CERT API shall be implemented on both REE and TEE side.  
  
  The SoC vendor is in charge of providing a reference implementation of the 
  CERT driver.
 
  <hr>@section s_cert_sw_stack_os OS-based software stack context
  The diagram below depicts the software stack in a TEE-based architecture. In a
  TEE-less architecture, only the REE part is available.
  
  @image html cert_software_stack_os_dual.png height=400
  @image rtf cert_software_stack_os_dual.png
 
  In this context the CERT API is used directly by Nagra CAS clients (rich and 
  trusted clients) to lock/unlock the CERT block resource and send commands to it.
  Keys processed by the CERT block are then handled through the REE or TEE SEC API
  respectively.
  
  When the TEE privileged mode is enabled, the CERT is accessible from the TEE only.
  
  \n
 
  <hr>@section s_cert_sw_stack_bare Bare software stack context
...
/** @page p_cert_fct_table Function table
 *
 ...
 *
 *  @section s_cert_cmd_principle Principle of operations
 *
 ...
 *     steps above.
 *
 *  <hr>@section s_cert_cmd_error_free Error free scenario
 *  @image html cert_cmd_error_free.png
 *  @image rtf cert_cmd_error_free.png
 *
 *  <hr>@section s_cert_cmd_proc_error Processing error scenario
 *  @image html cert_cmd_proc_error.png
 *  @image rtf cert_cmd_proc_error.png
 *
 *  <hr>@section s_cert_cmd_timeout_error Timeout error scenario
 *  @image html cert_cmd_timeout_error.png
 *  @image rtf cert_cmd_timeout_error.png
 *
 */

/** @page p_cert_use_cases Use cases
 *
 *  @section s_cert_ucase_descrambling Program descrambling
 ...
 *
 *  <hr>@section s_cert_ucase_r2r RAM2RAM
...
/*
  Only weak group declarations without title.
  Stronger definitions with title are moved in doc/main.dox.
*/

/** @addtogroup g_cert_functionTable */
/** @addtogroup g_cert_lockMgt */
/** @addtogroup g_cert_commandHandling */
/** @addtogroup g_cert_reset */
...
/**
 *  @ingroup g_cert_commandHandling
 *
 *  @brief
...
/**
 *  @ingroup g_cert_reset
 *
 *  @brief
 *    This function resets the AKL CERT IP block.
 *
 *    Although this function does not require any CERT resource handle as input 
 *    parameter, it must be called within a @link TCertLock certLock@endlink/@link TCertUnlock certUnlock@endlink critical section.
 *
 *    Note also that it shall be possible to make a @link TCertExchange certExchange @endlink call right
 *    after a @link TCertAklReset certAklReset @endlink call as described in the following example:
 *    @code
  certLock();
  certExchange();
  certAklReset();
 ...
 *    mode is enabled.
*/
typedef TCertStatus (*TCertAklReset)
(
  void
);
...
typedef struct
{
  ...
  TCertUseDecryptionKey certUseDecryptionKey;
   /**< @deprecated
    */
  TCertAklReset certAklReset;
   /**< Function to reset the AKL CERT IP block
    */
} TCertFunctionTable;
