...
static void HDMI_Start_Proc(mt_void *pPrivateData)
{
	...
	DMSG("save current HdmiAttr to Spr.");
...
mt_s32 mt_hdmi_setHdcp(mt_u8 hdcp_level)
{
	...
	printf("in HDCP bEnableHdmi: %d status:%d hdcp_level = %d  MT_GetOPCStatus = 0x%lx ",stAttr.bEnableHdmi,  stAttr.bHDCPEnable, hdcp_level, MT_GetOPCStatus());
	#if 0
	while( hdcp_level == 2 )
	{
	...
	if (ret == MT_SUCCESS) {
		if (g_hdcp_onoff == stAttr.bHDCPEnable) {
			return ret;
		}
	...
	printf(" return  HDCP status:%d hdcp_level = %d  MT_GetOPCStatus = %lx ", stAttr.bHDCPEnable, hdcp_level, MT_GetOPCStatus());
