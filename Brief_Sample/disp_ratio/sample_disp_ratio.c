...
#include "pthread.h"
/***************************** Macro Definition ******************************/
#ifdef  MT_SAMPLE_DISP_RATIO_DEBUG
#define MT_DISP_RATIO_PRINT   printf
#else
#define MT_DISP_RATIO_PRINT
#endif

#define SAMPLE_DISP_RATIO_FUNCTION_ENTER()             MT_DISP_RATIO_PRINT("[%s]: Enter ==>> \n", __FUNCTION__)
#define SAMPLE_DISP_RATIO_FUNCTION_EXIT()              MT_DISP_RATIO_PRINT("[%s]: Exit ==<< \n", __FUNCTION__)

#define SAMPLE_DISP_RATIO_FATAL_PRINT(fmt...)          MT_DISP_RATIO_PRINT(" [FATAL] " fmt)
#define SAMPLE_DISP_RATIO_ERR_PRINT(fmt...)            MT_DISP_RATIO_PRINT(" [ERROR] " fmt)
#define SAMPLE_DISP_RATIO_WARN_PRINT(fmt...)           MT_DISP_RATIO_PRINT(" [WARN] "  fmt)
#define SAMPLE_DISP_RATIO_INFO_PRINT(fmt...)           MT_DISP_RATIO_PRINT(" [INFO] "  fmt)
#define SAMPLE_DISP_RATIO_DBG_PRINT(fmt...)            MT_DISP_RATIO_PRINT(" [DEBUG] " fmt)


#define SAMPLE_DISP_RATIO_PRINT   printf
...
static MT_RATIO_RUN_INFO ratio_run_info;
...
@*/
static MT_S32 MT_DispRatioInjectTsTask(MT_VOID *args)
{
    ...
    if(NULL == pTsFile)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT( "file %s open error!!\n", fileName);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("failed to MT_UNF_DMX_CreateTSBuffer\n");
        g_bTaskQuit = MT_TRUE;
        fclose(pTsFile);
        return s32Ret;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT( "failed to MT_UNF_DMX_ResetTSBuffer\n");
        g_bTaskQuit = MT_TRUE;
    ...
    while(g_bTaskQuit == MT_FALSE)
    {
        ...
        if(Readlen <= 0)
        {
            SAMPLE_DISP_RATIO_INFO_PRINT("Read ts file end and rewind and Reset TS BUFFER and AVPLAYER...............!\n");
        ...
        if(MT_SUCCESS != s32Ret)
        {
           SAMPLE_DISP_RATIO_ERR_PRINT( "failed to MT_UNF_DMX_PutTSBuffer\n");
        }
    }
...
@*/
static MT_S32 MT_DispRatioCheckDvbcParam(mt_input_cab_para_t *p_cab_in)
{
    if(p_cab_in->freq < 45 || p_cab_in->freq > 862)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("The frequency is not in range\n");
        return MT_FAILURE;
    }

    if(p_cab_in->sym_rate < 900 || p_cab_in->sym_rate > 7200)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("The symbol rate is not in range\n");
        return MT_FAILURE;
    }
    ...
    switch(p_cab_in->mod_type)
    {
        ...
        case 256:
            break;
        default:
            SAMPLE_DISP_RATIO_ERR_PRINT("QAM mismatch(16, 32, 64, 128, 256)\n");
            return MT_FAILURE;
    }
...
@return::MT_FAILURE             Failure.
@*/
static mt_s32 MT_DispRatioCheckDvbsParam(mt_input_sat_para_t *p_sat_in)
{
    if((p_sat_in->freq) > 4200 || (p_sat_in->freq) < 3000)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("freq error. freq = %d \n", p_sat_in->freq);
        SAMPLE_DISP_RATIO_ERR_PRINT("freq must be more than 3,000 and less than 4,200.\n");
        return MT_FAILURE;
    }
...
@return::s32Ret                 The return value of the error
@*/
static MT_S32 MT_DispRatioSetAvplayPidAndCodecType(MT_HANDLE hAvplay      , const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("The input address is empty-------<%s> line: %d\n", __FUNCTION__, __LINE__);
        return MT_FAILURE;
    }
    ...
    SAMPLE_DISP_RATIO_INFO_PRINT("VidPid = %#x, AudPid = %#x-------<%s> line: %d\n", VidPid, AudPid, __FUNCTION__, __LINE__);
    ...
    if(VidPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_GetAttr failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("Set video properties or video PID property failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
    }
    ...
    if(AudPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("Setting the decoding mode or audio PID property failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
    }
    ...
    if((VidPid != INVALID_TSPID) || (AudPid != INVALID_TSPID))
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_SetAttr failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
    }
...
@return::s32Ret                 The return value of the error
@*/
static MT_S32 MT_DispRatioStarToPlay(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    MT_DispRatioSetAvplayPidAndCodecType(hAvplay, pProgInfo);
    ...
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("Has no audio stream!, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
    }
    ...
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("Has no video stream!, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
    }
    ...
    if((enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_AUD) && (enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_VID))
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("Set frame to VO is failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("Get avplay sync attr is failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("Set avplay sync attr is failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
            return s32Ret;
        }
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_Start failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        return s32Ret;
    }
...
@return::s32Ret                 The return value of the error.
@*/
static MT_S32 MT_DispRatioStopToPlay(MT_HANDLE hAvplay)
{
    MT_UNF_AVPLAY_STOP_OPT_S option = { 0 };

    if(MT_INVALID_HANDLE == hAvplay)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("The input handle is empty-------<%s> line: %d\n", __FUNCTION__, __LINE__);
        return MT_FAILURE;
    }
    ...
    SAMPLE_DISP_RATIO_INFO_PRINT("stop live play ...\n");
...
@return::s32Ret                 The return value of the error.
@*/
static MT_S32 MT_DispRatioAvplayInit(MT_HANDLE *phAvplay, MT_HANDLE *phWin, MT_HANDLE *phSoundTrack)
{
    MT_S32                   s32Ret = MT_FAILURE;
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MTADP_AVPlay_RegADecLib failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        return s32Ret;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_Init failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        return s32Ret;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_GetDefaultConfig failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR1;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_Create failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR1;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_AVPLAY_ChnOpen failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR2;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_SND_GetDefaultTrackAttr failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR4;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_SND_CreateTrack failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR4;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_SND_Attach failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR5;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MTADP_VO_CreatWin failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR6;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_VO_AttachWindow failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR7;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_VO_SetWindowEnable failed, s32Ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
        goto ERROR8;
    }
...
@return::MT_VOID
@*/
static MT_VOID MT_DispRatioAvplayDeinit(MT_HANDLE hAvplay, MT_HANDLE hWin, MT_HANDLE hSoundTrack)
{
...
@return::s32Ret                 The return value of the error.
@*/
static MT_S32 MT_DispRatioDmxInit(MT_INPUR_SIG_TYPE_T sig_type)
{
    MT_S32           s32Ret = MT_FAILURE;
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_DMX_Init failed, s32Ret = 0x%x\n", s32Ret);
        return s32Ret;
    }
    ...
    if(MT_INPUT_SIG_TYPE_FILE == sig_type)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("MT_UNF_DMX_AttachTSPort failed, s32Ret = 0x%x\n", s32Ret);
    ...
    else
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to mt_sys_get_version\n");
            return MT_FAILURE;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MT_UNF_DMX_AttachTSPort\n");
            return MT_FAILURE;
        }
    }
...
@return::MT_VOID
@*/
static MT_VOID MT_DispRatioDmxDeinit(MT_VOID)
{
...
static MT_U8 *MT_DispRatioMode(MT_UNF_DISP_ASPECT_RATIO_E ratio_mode, MT_UNF_VO_ASPECT_CVRS_E aspect_cvrs)
{
    switch(ratio_mode)
    {
        ...
        case MT_UNF_DISP_ASPECT_RATIO_4TO3:
            if(aspect_cvrs == MT_UNF_VO_ASPECT_CVRS_LETTERBOX)
            {
                return "4:3LB";
            }
            return "4:3PS";
        ...
        case MT_UNF_DISP_ASPECT_RATIO_AUTO:
            return "Auto";
        default:
            SAMPLE_DISP_RATIO_ERR_PRINT("Error mode : %d \n", ratio_mode);
            return MT_NULL;
    }
}
...
@return::void
@*/
static MT_VOID MT_DispRatioSetAspectRatio(MT_HANDLE hWin, MT_U8 s_status)
{
    ...
    MT_UNF_DISP_VidLayerShow(MT_FALSE);
    usleep(200*1000);

    switch(s_status)
    {
        case 0:
            SAMPLE_DISP_RATIO_INFO_PRINT("Set to 16:9 LB\n");
            ...
            break;
        case 1:
            SAMPLE_DISP_RATIO_INFO_PRINT("Set to 16:9 PS\n");
            setDispAspectRatio.enDispAspectRatio = MT_UNF_DISP_ASPECT_RATIO_16TO9;
            MT_UNF_DISP_SetAspectRatio(MT_UNF_DISPLAY1, &setDispAspectRatio);
            MT_UNF_VO_GetWindowAttr(hWin, &getWindowAttr);
            ...
            break;
        case 2:
            SAMPLE_DISP_RATIO_INFO_PRINT("Set 4:3 LB\n");
            ...
            break;
        case 3:
            SAMPLE_DISP_RATIO_INFO_PRINT("Set 4:3 Ps\n");
            setDispAspectRatio.enDispAspectRatio = MT_UNF_DISP_ASPECT_RATIO_4TO3;
            MT_UNF_DISP_SetAspectRatio(MT_UNF_DISPLAY1, &setDispAspectRatio);
            MT_UNF_VO_GetWindowAttr(hWin, &getWindowAttr);
            getWindowAttr.stWinAspectAttr.enAspectCvrs = MT_UNF_VO_ASPECT_CVRS_PAN_SCAN;
            ...
            break;
        case 4:
            SAMPLE_DISP_RATIO_INFO_PRINT("Set to Auto mode\n");
    ...
    usleep(200*1000);
    MT_UNF_DISP_VidLayerShow(MT_TRUE);
    MT_UNF_VO_GetWindowAttr(hWin, &getWindowAttr);
    MT_UNF_DISP_GetAspectRatio(MT_UNF_DISPLAY1, &getDispAspectRatio);
    SAMPLE_DISP_RATIO_INFO_PRINT("Current aspect ratio is %s\n", MT_DispRatioMode(getDispAspectRatio.enDispAspectRatio, getWindowAttr.stWinAspectAttr.enAspectCvrs));
}

static MT_VOID MT_DispRatioPrintMenu(PMT_COMPACT_TBL *pProgTbl)
{
    SAMPLE_DISP_RATIO_PRINT("commond: \n");
    SAMPLE_DISP_RATIO_PRINT("     r: switch the aspect ratio(16:9LB,16:9PS, 4:3LB,4:3PS, adaptive) \n");
    SAMPLE_DISP_RATIO_PRINT("     l: get current aspect ratio \n");
    SAMPLE_DISP_RATIO_PRINT("     q: quit \n");
#ifdef MT_SAMPLE_APP
    SAMPLE_DISP_RATIO_PRINT("     b : background run \n");
#endif
    SAMPLE_DISP_RATIO_PRINT("     h: help \n");
#ifndef MT_SAMPLE_APP
    SAMPLE_DISP_RATIO_PRINT("'1-%d': Select a program \n", pProgTbl->prog_num);
#endif
    SAMPLE_DISP_RATIO_PRINT("Ratio>> ");
}


static MT_VOID MT_DispRatioExit(void)
{
    g_bTaskQuit = MT_TRUE;
#ifndef MT_SAMPLE_APP
    /** Stop playing the show */
    (MT_VOID)MT_DispRatioStopToPlay(ratio_run_info.hAvPlay);

    /** Audio and video player deinitialization */
    (MT_VOID)MT_DispRatioAvplayDeinit(ratio_run_info.hAvPlay, ratio_run_info.hWin, ratio_run_info.hSoundTrack);

    (MT_VOID)MTADP_Search_FreeAllPmt(ratio_run_info.pProgTbl);

    (MT_VOID)MTADP_Search_DeInit();

    if(MT_INPUT_SIG_TYPE_FILE == ratio_run_info.sInputParam.sig_type)
    {
        /** Wait for the thread to end */
        pthread_join(ratio_run_info.stInjectTSThread, NULL);
    }

    (MT_VOID)MT_DispRatioDmxDeinit();
...
#endif
    memset(&ratio_run_info, 0xff, sizeof(ratio_run_info));
}
...
@return::MT_VOID
@*/
static MT_VOID MT_DispRatioCmdTask(MT_HANDLE hAvPlay, MT_HANDLE hWin, PMT_COMPACT_TBL *pProgTbl)
{
#ifndef MT_SAMPLE_APP
        MT_S32                 s32Ret = MT_SUCCESS;
        MT_U32                 u32ProgNum = 0;
    ...
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("The input handle is empty. \n");
        return;
    }
#endif
    while(1)
    {
        (MT_VOID)MT_DispRatioPrintMenu(pProgTbl);

        fgets((char *)(inputCmd), (sizeof(inputCmd) - 1), stdin);

        if(inputCmd[0] == 'q')
        {
            SAMPLE_DISP_RATIO_INFO_PRINT("now exit!\n");
            g_bTaskQuit = MT_TRUE;
            break;
        }
#ifdef MT_SAMPLE_APP
        ...
        {
            SAMPLE_DISP_RATIO_INFO_PRINT("Ratio play in back!\n");
            break;
        }
#else
        else if(inputCmd[0] > '0' && inputCmd[0] <= '9')
        {
            ...
            if(u32ProgNum > 0 && u32ProgNum <= pProgTbl->prog_num)
            {
                ...
                s32Ret = MT_DispRatioStopToPlay(hAvPlay);
                if(MT_SUCCESS != s32Ret)
                {
                    SAMPLE_DISP_RATIO_ERR_PRINT("MT_DispRatioStopToPlay failed, ret = 0x%x \n", s32Ret);
                }
                SAMPLE_DISP_RATIO_INFO_PRINT("===== Start play ProgNum: %d \n", u32ProgNum);

                /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
                s32Ret = MT_DispRatioStarToPlay(hAvPlay, pstCurrentProgInfo);
                if(MT_SUCCESS != s32Ret)
                {
                    SAMPLE_DISP_RATIO_ERR_PRINT("Switching shows failed\n");
                }
            }
            else
            {
                SAMPLE_DISP_RATIO_ERR_PRINT(" prog_num the biggest is %d \n\n", pProgTbl->prog_num);
                continue;
            }
        }
#endif
        ...
        {
            SAMPLE_DISP_RATIO_INFO_PRINT("please input you want to set \n");
            SAMPLE_DISP_RATIO_INFO_PRINT("0:(16:9 LB), 1:(16:9 PS), 2:(4:3 LB), 3:(4:3 PS), 4:(AUTO) \n");
            scanf("%d", &status);
            SAMPLE_DISP_RATIO_INFO_PRINT("Reset disp aspect ratio. \n");
            /** Switch the aspect ratio each time you enter this function */
            (MT_VOID)MT_DispRatioSetAspectRatio(hWin, status);
        }
        ...
        {
            SAMPLE_DISP_RATIO_INFO_PRINT("Print help info \n");
            continue;
        }
    }
}
...
static MT_VOID MT_DispRatioPrint_Help(MT_CHAR *name)
{
#ifndef MT_SAMPLE_APP
    MT_DISP_RATIO_PRINT("Lack of parameters\n");
    MT_DISP_RATIO_PRINT("\nUsage:\n");
    MT_DISP_RATIO_PRINT("%s\n", name);
    MT_DISP_RATIO_PRINT("    -f: path of the stream file\n");
    MT_DISP_RATIO_PRINT("    -c: DVBC locks frequency\n");
    MT_DISP_RATIO_PRINT("    -s: DVBS locks frequency\n");
    MT_DISP_RATIO_PRINT("example:\n");
    MT_DISP_RATIO_PRINT("    %s -f ./ttx.ts\n",name);
    MT_DISP_RATIO_PRINT("    %s -c 314 6875 64\n",name);
    MT_DISP_RATIO_PRINT("    %s -s 3840 27500 1 0 0\n",name);
#else
    MT_DISP_RATIO_PRINT("    -q: Exit the background\n");
#endif
...
/*
 ...
 @return ::MT_SUCCESS
*/
static MT_S32 MT_DispRatioParase_args(MT_S32 argc, MT_CHAR *argv[], mt_input_para_t *pInputParam)
{
    MT_S32 opt = 0;
    ...
    {
        (MT_VOID)MT_DispRatioPrint_Help(argv[0]);
        return MT_FAILURE;
    }
#endif
...
#endif
{
    ...
    s32Ret = MT_DispRatioParase_args(argc, argv, &ratio_run_info.sInputParam);
    if (MT_FAILURE == s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("Parase args err. stop window.\n");

        return MT_FAILURE;
    }
    else if(MT_TASK_EXIT == s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("Recv stop command. stop window.\n");
    ...
    if(g_bTaskQuit == MT_TRUE)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to mt_sys_init\n");
            return s32Ret;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to StartDmx\n");
            goto ERR0;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MTADP_Disp_Init\n");
            goto ERR1;
        }
        ...
        if(MT_INPUT_SIG_TYPE_FILE != ratio_run_info.sInputParam.sig_type)
        {
            ...
            if(MT_SUCCESS != s32Ret)
            {
                SAMPLE_DISP_RATIO_ERR_PRINT("MTADP_Fe_Init failed-------<%s> line: %d\n", __FUNCTION__, __LINE__);
                goto ERR2;
            }

            if (MT_INPUT_SIG_TYPE_CAB == ratio_run_info.sInputParam.sig_type)
            {
                s32Ret = MT_DispRatioCheckDvbcParam(&ratio_run_info.sInputParam.input_param.cab);
                if(MT_SUCCESS != s32Ret)
                {
                    SAMPLE_DISP_RATIO_ERR_PRINT("MT_DispRatioCheckDvbcParam failed-------<%s> line: %d\n", __FUNCTION__, __LINE__);
                    goto ERR3;
                }

                s32Ret = MTADP_Fe_Connect_Dvbc(TUNER_ID_0,
                                            ratio_run_info.sInputParam.input_param.cab.freq,
                                            ratio_run_info.sInputParam.input_param.cab.sym_rate,
                                            ratio_run_info.sInputParam.input_param.cab.mod_type);
            }
            else if(MT_INPUT_SIG_TYPE_SAT == ratio_run_info.sInputParam.sig_type)
            {
                s32Ret = MT_DispRatioCheckDvbsParam(&ratio_run_info.sInputParam.input_param.sat);
                if(MT_SUCCESS != s32Ret)
                {
                    SAMPLE_DISP_RATIO_ERR_PRINT("MT_DispRatioCheckDvbsParam failed-------<%s> line: %d\n", __FUNCTION__, __LINE__);
                    goto ERR3;
                }

                s32Ret = MTADP_Fe_Connect_Dvbs(TUNER_ID_0,
                                            ratio_run_info.sInputParam.input_param.sat.freq,
                                            ratio_run_info.sInputParam.input_param.sat.sym_rate,
                                            ratio_run_info.sInputParam.input_param.sat.onoff_22k,
                                            ratio_run_info.sInputParam.input_param.sat.polarization,
                                            ratio_run_info.sInputParam.input_param.sat.port_type);
            }

            if(MT_SUCCESS != s32Ret)
            {
                SAMPLE_DISP_RATIO_ERR_PRINT("MTADP_Fe_Connect failed-------<%s> line: %d\n", __FUNCTION__, __LINE__);
                goto ERR3;
            }
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MTADP_VO_Init\n");
            goto ERR3;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MTADP_Snd_Init\n");
            goto ERR4;
        }

        /** DMX module initialization */
        s32Ret = MT_DispRatioDmxInit(ratio_run_info.sInputParam.sig_type);
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to StartDmx\n");
            goto ERR5;
        ...
        if(MT_INPUT_SIG_TYPE_FILE == ratio_run_info.sInputParam.sig_type)
        {
            s32Ret = pthread_create(&ratio_run_info.stInjectTSThread, NULL, (MT_VOID * (*)(MT_VOID *))MT_DispRatioInjectTsTask, &ratio_run_info.sInputParam.input_param.file);
            if(MT_SUCCESS != s32Ret)
            {
                SAMPLE_DISP_RATIO_ERR_PRINT("failed to pthread_create thread \n");
                goto ERR6;
            }
            sleep(1);
            if(g_bTaskQuit == MT_TRUE)
        ...
        s32Ret = MTADP_Search_GetAllPmt(DMX_ID_0, &ratio_run_info.pProgTbl);
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MTADP_Search_GetAllPmt\n");

            goto ERR8;
        }

        /** AVPLAY initialization */
        s32Ret = MT_DispRatioAvplayInit(&ratio_run_info.hAvPlay, &ratio_run_info.hWin, &ratio_run_info.hSoundTrack);
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MT_DispRatioAvplayInit\n");
            goto ERR9;
        }

        /** Select the program */
        pstCurrentProgInfo = ratio_run_info.pProgTbl->proginfo;

        /** Start playing the show */
        s32Ret = MT_DispRatioStarToPlay(ratio_run_info.hAvPlay, pstCurrentProgInfo);
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_DISP_RATIO_ERR_PRINT("failed to MT_DispRatioStarToPlay\n");
            goto ERR10;
        }
#else
	ratio_run_info.hWin = avplayHandle.hWin;

    s32Ret = MT_UNF_VO_GetWindowAttr(ratio_run_info.hWin, &getWindowAttr);
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("Window does't created, please check whether to play first\n");
        return MT_FAILURE;
    }
#endif
    }

    (MT_VOID)MT_DispRatioCmdTask(ratio_run_info.hAvPlay, ratio_run_info.hWin, ratio_run_info.pProgTbl);
...
#ifndef MT_SAMPLE_APP
    /** Stop playing the show */
    s32Ret = MT_DispRatioStopToPlay(ratio_run_info.hAvPlay);
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_DISP_RATIO_ERR_PRINT("MT_DispRatioStopToPlay failed, ret = %d-------<%s> line: %d\n", s32Ret,  __FUNCTION__, __LINE__);
    }
