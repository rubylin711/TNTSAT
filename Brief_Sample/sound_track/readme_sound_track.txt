1. 整体介绍
该sample包含多个基本文件
sample_sound_track.c
该文件是关于DVB或者文件播放和切换声道的主要步骤以及涉及函数。
...
3. 使用
拷贝 sample_sound_track到u盘，确认你要播放的源中包含声道模式
执行以下指令：
mount /dev/sda1 /mnt/
将linux/pub目录下的shared_lib_striped文件夹拷贝到U盘
export LD_LIBRARY_PATH=/mnt/shared_lib_striped/:/usr/local/lib
cd /mnt
./sample_sound_track -f ./ttx.ts(文件播放)
./sample_sound_track -c 314  6875  64 -> frequency SymbolRate QAM(DVBC)
./sample_sound_track -s 3840 27500 1  0 0-> frequency SymbolRate 22k(0:off,1:on) (polar) (DVBS)
