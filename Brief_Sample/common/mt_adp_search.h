...
#define STREAM_TYPE_PCM_AUDIO           0x80    // PCM
#define STREAM_TYPE_AC3_AUDIO           0x81    // AC3
#define STREAM_TYPE_EAC3_AUDIO          0x87    // EAC3
...
#define STREAM_TYPE_DTS_ES_XLL_AUDIO    0x86    // DTS es xll
#define STREAM_TYPE_AVS2_VIDEO          0xD2    // DTS es xll
#define STREAM_TYPE_VIVID_AUDIO         0xD5
...
#define AUDIO_PRESELECTION_DESCRIPTOR 0x19
...
#define AC4_REGISTRATION_DESCRIPTOR 0x5
...
#define DDP_VISUALLY_IMPAIRED_DESCRIPTOR 0x90
...
#define AC4_PRESELECTION_MAX  (8)
#define AC4_AUX_COMPONENT_MAX  (8)
#define MESSAGE_DESCRIPTOR_MAX  (16)

// 标志位结构
typedef struct {
    mt_u8 audio_description : 1;
    mt_u8 spoken_subtitles : 1;
    mt_u8 dialogue_enhancement : 1;
    mt_u8 interactivity_enabled : 1;
    mt_u8 language_code_present : 1;
    mt_u8 text_label_present : 1;
    mt_u8 multi_stream_info_present : 1;
    mt_u8 future_extension : 1;
} audio_flags_t;

// 辅助组件信息
typedef struct {
    mt_u8 num_aux_components;
    mt_u8 reserved_zero_future_use;
    mt_u8 component_tags[AC4_AUX_COMPONENT_MAX]; // 最大支持8个辅助组件
} aux_component_info_t;

// 单个音频预选条目
typedef struct {
    mt_u8 preselection_id;
    mt_u8 audio_rendering_indication;
    mt_u8 reserved;
    audio_flags_t flags;
    char language_code[4];     // 3字符 + 结束符
    char text_label[5];        // 4字符 + 结束符
    mt_u8 message_id;        // 消息ID
    mt_u8 has_message_id;    // 表示message_id是否存在
    aux_component_info_t aux_info;
    mt_u8 has_aux_info;
} audio_preselection_t;

// 音频预选描述符
typedef struct {
    mt_u8 descriptor_tag;
    mt_u8 descriptor_length;
    mt_u8 descriptor_tag_extension;
    mt_u8 num_preselections;
    mt_u8 has_aux_components;
    mt_u8 reserved_global;   // 全局保留位
    audio_preselection_t preselections[AC4_PRESELECTION_MAX]; // 最大支持8个预选
    mt_u8 num_parsed_preselections;
} audio_preselection_descriptor_t;
...
typedef struct hiPMT_AUDIO_S
{
    ...
    mt_u32 u32audiotype;
    pcm_info_t pcm;
    mt_s32 has_audio_preselection;
    audio_preselection_descriptor_t audio_preselection_descriptor;
} PMT_AUDIO;
...
typedef struct hiPMT_TB_S
{
    ...
    mt_u16 u16SCTESubtNum;
    PMT_SCTE_SUBTITLE_S stSCTESubtInfo[SCTE_SUBTITLE_MAX];
...
typedef struct hiMESSAGE_DESCRIPTOR
{
    mt_s32 message_id;
    mt_s8 iso639LanguageCode[4];      // 3字符 + 结束符
    mt_s8 message[32];
} MESSAGE_DESCRIPTOR;
...
typedef struct hiSDT_INFO_S
{
    ...
    mt_u8 message_descriptor_flag;
    MESSAGE_DESCRIPTOR message_descriptor[MESSAGE_DESCRIPTOR_MAX];
    mt_u8 num_message_descriptor;
} SDT_INFO;
...
typedef struct hiNIT_TB_S
{
    mt_u16 u16TsId;
    mt_u16 u16NetID;
    NIT_INFO NitInfo[CHANNEL_MAX_PROG];
} NIT_TB;


typedef struct hiCAT_INFO_S
{
    mt_u16          u16ServiceID;
    mt_u8       u8EitFlag;
    mt_u8       u8EitFlag_PF;
...
typedef struct hiCAT_TB_S
{
    mt_u32 u32ProgNum;
    mt_u16 u16TsId;
    mt_u16 u16NetID;
    CAT_INFO CatInfo[CHANNEL_MAX_PROG];
} CAT_TB;
