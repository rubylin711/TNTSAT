...
#define MT_SAMPLE_DSSPLAY_DEBUG
...
//        printf("\n##%s_%d MT_UNF_DMX_AcquireEs fail[%x]!\n", __FUNCTION__, __LINE__, ret);
    ...
    if(esinfo.u32BufLen > 0)
    {
        ...
        printf("buffer len: %d", esinfo.u32BufLen);
        printf("write_len : %d", write_len );
...
static int _parse_es(void)
{
    ...
    while(loopi < dss_packet_cnt)
    {
        ...
        printf("scid: %d, vidpid: %d\n", dss_header.scid, g_stDssPlayRunInfo.vidpid);
        if((dss_header.scid == g_stDssPlayRunInfo.vidpid) || (dss_header.scid == g_stDssPlayRunInfo.audpid))
        {
            ...
            if(dss_header.scid == g_stDssPlayRunInfo.vidpid)
            {
...
//                if ((g_video_cc != 0xFF) && (((g_video_cc + 1)&0xF) != dss_header.cc))
//                {
//                    //OS_PRINTF("\n##warning, dss video ts packet not continue[%x->%x]!\n", g_video_cc, dss_header.cc);
//                    g_video_lost_cnt += abs(dss_header.cc - g_video_cc);
//                }
//                g_video_cc = dss_header.cc;
                if (dss_header.cf == 0)
                {
...
//                if ((g_audio_cc != 0xFF) && (((g_audio_cc + 1)&0xF) != dss_header.cc))
//                {
//                    //OS_PRINTF("\n##warning, dss audio ts packet not continue[%x->%x]!\n", g_audio_cc, dss_header.cc);
//                    g_audio_lost_cnt += abs(dss_header.cc - g_audio_cc);
//                }
//                g_audio_cc = dss_header.cc;
                if (dss_header.cf == 0)
                {
...
static mt_s32 MT_DssPlayInjectTsTask(mt_void *args)
{
    ...
    {
        printf("Please enter the video pid: ");
        scanf("%d", &vpid);
        getchar();

        printf("Please enter the audio pid: ");
        scanf("%d", &apid);
        getchar();
        g_stDssPlayRunInfo.vidpid = vpid;
        g_stDssPlayRunInfo.audpid = apid;
    }
    ...
    if(g_stDssPlayRunInfo.sInputParam.data_source == 1)
    {
...
static void MT_DssPlayPrint_Help(char *name)
{
    ...
    MT_DSSPLAY_PRINT("    -c: DVBC locks frequency\n");
    MT_DSSPLAY_PRINT("    -s: DVBS locks frequency\n");
    MT_DSSPLAY_PRINT("example:\n");
    MT_DSSPLAY_PRINT("    %s -f ./test.trp\n",name);
    MT_DSSPLAY_PRINT("    %s -c 314 6875 64\n",name);
    MT_DSSPLAY_PRINT("    %s -s 3840 27500 1 0 0\n",name);
