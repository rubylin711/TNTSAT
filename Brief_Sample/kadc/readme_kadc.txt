1. 整体介绍
该sample包含两个基本文件
sample_kadc.c
Makefile
该文件为编译所用到的makefile

2. 编译
在文件所在目录下执行make进行编译
make

3. 使用
拷贝库/linux/pub/shared_lib_striped到u盘
拷贝/linux/pub/bin/sample_kadc到u盘，u盘接上单板， 
执行以下指令：
mount /dev/sda1 /mnt/
export LD_LIBRARY_PATH=/mnt/shared_lib_striped/:/usr/local/lib
cd /mnt/
./sample_kadc -t 0  //0: 3 keys, 1: 5 keys, 2: 7 keys

4.流程介绍
  1） mt_sys_init()                 //系统初始化
  2） MT_UNF_KEYLED_Init()          //初始化KEYLED
  3） MT_UNF_KEYLED_SelectType_V2() //key类型
  4） MT_UNF_KEY_setKeyadcType()    //设置kadc类型
  5） MT_UNF_KEYLED_Hw_Init()       //初始化硬件
  6） MT_UNF_KEY_RepKeyTimeoutVal() //超时时间
  7） MT_UNF_KEY_IsRepKey()         //设置是否使能重复按键
  8） MT_UNF_KEY_IsKeyUp()          //设置是否上报按键抬起
  9） MT_Receive()                  //监视按键的状态
  10）MT_UNF_KEYLED_DeInit()        //去初始化
  11）mt_sys_deinit()               //系统去初始化
END