1. 整体介绍
该sample包含两个基本文件
sample_dolby.c
该文件主要实现Dolby声音模式的切换
Makefile
该文件为编译所用到的makefile

2. 编译
在文件所在目录下执行make进行编译
make

3. 使用
拷贝库/linux/pub/shared_lib_striped到u盘
拷贝/linux/pub/bin/sample_dolby到u盘，u盘接上单板
拷贝6ch_voices_id_7_dd_DVB_h264_25fps.trp文件到u盘中
执行以下指令：
mount /dev/sda1 /mnt/
export LD_LIBRARY_PATH=/mnt/shared_lib_striped/:/usr/local/lib
cd /mnt/
./sample_dolby -f ./6ch_voices_id_7_dd_DVB_h264_25fps.trp

4.流程介绍
  1)  mt_sys_init()             //系统初始化
  2)  MTADP_HDMI_Init()         //HDMI初始化
  3)  MTADP_Disp_Init()         //显示初始化
  4)  MTADP_VO_Init()           //vo设备的初始化
  5)  MTADP_Snd_Init()          //声音设备初始化
  6)  DOLBY_DmxInit()           //Demux初始化
  7)  InjectTsTask()            //接收文件流数据的线程
  8)  MTADP_Search_GetAllPmt()  //获取PMT表
  9)  DOLBY_AvplayInit()        //音视频播器放初始化
  10) DOLBY_StarToPlay()        //开始播放
  11) DOLBY_StopToPlay()        //停止播放
  12) DOLBY_SNDMode()           //改变声音播放模式
  13) DOLBY_AvplayDeInit()      //音视频播放器去初始化
  14) MTADP_Search_FreeAllPmt() //释放PMT表
  15) DOLBY_DmxDeInit()         //Demux去初始化
  16) MTADP_Snd_DeInit()        //音响设备去初始化
  17) MTADP_VO_DeInit()         //vo设备的去初始化
  18) MTADP_Disp_DeInit()       //显示去初始化
  19) MTADP_HDMI_DeInit()       //HDMI去初始化
  20) mt_sys_deinit()           //系统去初始化
