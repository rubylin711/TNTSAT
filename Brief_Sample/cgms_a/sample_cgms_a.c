...
#include "pthread.h"
/***************************** Macro Definition ******************************/
#ifdef  MT_SAMPLE_CGMS_DEBUG

#define MT_CGMS_PRINT   printf
#else

#define MT_CGMS_PRINT

#endif

#define SAMPLE_CGMS_FUNCTION_ENTER()    MT_CGMS_PRINT("[%s]: Enter ==>> \n", __FUNCTION__)
#define SAMPLE_CGMS_FUNCTION_EXIT()     MT_CGMS_PRINT("[%s]: Exit ==<< \n", __FUNCTION__)



#define SAMPLE_CGMS_FATAL_PRINT(fmt...)         MT_CGMS_PRINT(" [FATAL] " fmt)
#define SAMPLE_CGMS_ERR_PRINT(fmt...)           MT_CGMS_PRINT(" [ERROR] " fmt)
#define SAMPLE_CGMS_WARN_PRINT(fmt...)          MT_CGMS_PRINT(" [WARN] "  fmt)
#define SAMPLE_CGMS_INFO_PRINT(fmt...)          MT_CGMS_PRINT(" [INFO] "  fmt)
#define SAMPLE_CGMS_DBG_PRINT(fmt...)           MT_CGMS_PRINT(" [DEBUG] " fmt)
...
static MT_CGMS_RUN_INFO cgms_run_info;
/******************************* API declaration *****************************/
#ifdef MT_SAMPLE_APP
...
static mt_s32 InjectTsTask(mt_void *args)
{
    ...
    SAMPLE_CGMS_INFO_PRINT(">>>open file : %s  >>>> \n", pstParam->file_name);

    pTsFile = fopen((char*)pstParam->file_name, "rb");
    if(pTsFile == NULL)
    {
        SAMPLE_CGMS_ERR_PRINT("file %s open error!!\n", pstParam->file_name);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    {
        SAMPLE_CGMS_ERR_PRINT("failed to MT_UNF_DMX_CreateTSBuffer\n");
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_CGMS_ERR_PRINT( "failed to MT_UNF_DMX_ResetTSBuffer ret=0x%x \n", s32Ret);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    while (g_bTaskQuit == MT_FALSE)
    {
        ...
        if(Readlen <= 0)
        {
            ...
            SAMPLE_CGMS_INFO_PRINT("Read ts file end and rewind and Reset TS BUFFER and AVPLAYER...............!\n");
        ...
        if (s32Ret != MT_SUCCESS)
        {
            SAMPLE_CGMS_ERR_PRINT("failed to MT_UNF_DMX_PutTSBuffer ret=0x%x \n", s32Ret);
        }
    }
...
static mt_s32 MT_CGMSCheckDvbcParam(mt_input_cab_para_t *p_cab_in)
{
    if(p_cab_in->freq < 45 || p_cab_in->freq > 862)
    {
        SAMPLE_CGMS_ERR_PRINT("The frequency is not in range\n");
        return MT_FAILURE;
    }

    if(p_cab_in->sym_rate < 900 || p_cab_in->sym_rate > 7200)
    {
        SAMPLE_CGMS_ERR_PRINT("The symbol rate is not in range\n");
        return MT_FAILURE;
    }
    ...
    switch(p_cab_in->mod_type)
    {
        ...
        case 256:
            break;
        default:
            SAMPLE_CGMS_ERR_PRINT("QAM mismatch(16, 32, 64, 128, 256)\n");
            return MT_FAILURE;
    }
...
static mt_s32 MT_CGMSCheckDvbsParam(mt_input_sat_para_t *p_sat_in)
{

    if((p_sat_in->freq) > 4200 || (p_sat_in->freq) < 3000)
    {
        SAMPLE_CGMS_ERR_PRINT("freq error. freq = %d \n", p_sat_in->freq);
        SAMPLE_CGMS_ERR_PRINT("freq must be more than 3,000 and less than 4,200.\n");
        return MT_FAILURE;
    }
...
@*/
static MT_S32 MT_CGMSDmxInit(MT_INPUR_SIG_TYPE_T sig_type)
{
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_DMX_Init failed, s32Ret = 0x%x\n", s32Ret);
        return s32Ret;
    }
    ...
    if(MT_INPUT_SIG_TYPE_FILE == sig_type)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MT_UNF_DMX_AttachTSPort failed, s32Ret = 0x%x\n", s32Ret);
    ...
    else
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_CGMS_ERR_PRINT("failed to mt_sys_get_version\n");
            return MT_FAILURE;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_CGMS_ERR_PRINT("failed to MT_UNF_DMX_AttachTSPort\n");
            return MT_FAILURE;
        }
    }
...
@return::MT_VOID
@*/
static MT_VOID MT_CGMSDmxDeInit(MT_VOID)
{
...
@*/
static MT_S32 MT_CGMSAvplayInit(MT_HANDLE *phAvplay, MT_HANDLE *phWin, MT_HANDLE* phSoundTrack)
{
    ...
    if(NULL == phAvplay)
    {
        SAMPLE_CGMS_ERR_PRINT("phAvplay is null.\n");
        return MT_FAILURE;
    ...
    if(NULL == phWin)
    {
        SAMPLE_CGMS_ERR_PRINT("phWin is null.\n");
        return MT_FAILURE;
    ...
    {
        SAMPLE_CGMS_ERR_PRINT("phSoundTrack is null.\n");
        return MT_FAILURE;
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MTADP_AVPlay_RegADecLib failed ret=0x%x \n", ret);
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_Init failed ret=0x%x \n", ret);
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_GetDefaultConfig failed ret=0x%x \n", ret);
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_Create failed ret=0x%x \n", ret);
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_ChnOpen failed ret=0x%x \n", ret);
        goto ERR2 ;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_SND_GetDefaultTrackAttr failed ret=0x%x \n", ret);
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_SND_CreateTrack failed ret=0x%x \n", ret);
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_SND_Attach failed ret=0x%x \n", ret);
        goto ERR5;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MTADP_VO_CreatWin failed ret=0x%x \n", ret);
        goto ERR6;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_VO_AttachWindow failed ret=0x%x \n", ret);
        goto ERR7;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_VO_SetWindowEnable failed ret=0x%x \n", ret);
        goto ERR8;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_VOID MT_CGMSAvplayDeInit(MT_HANDLE hAvplay, MT_HANDLE hWin, MT_HANDLE hSoundTrack)
{
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_CGMSSetAvplayPidAndCodecType(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    MT_S32                           ret = MT_FAILURE;
    MT_U32                           u32AudType = 0;
    MT_U32                           VidPid = 0;
    ...
    if(NULL == pProgInfo)
    {
        SAMPLE_CGMS_ERR_PRINT("The input address is empty\n");
        return MT_FAILURE;
    }
    ...
    SAMPLE_CGMS_INFO_PRINT("VidPid = %#x, AudPid = %#x\n", VidPid, AudPid);
    ...
    if(VidPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_GetAttr failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("Set video properties or video PID property failed\n");
            return ret;
        }
    }
    ...
    if(AudPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("Setting the decoding mode or audio PID property failed\n");
            return ret;
        }
    }
    ...
    if((VidPid != INVALID_TSPID) || (AudPid != INVALID_TSPID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_SetAttr failed ret=0x%x \n", ret);
            return ret;
        }
    }
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_CGMSStarToPlay(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    if (MT_INVALID_HANDLE == hAvplay)
    {
        SAMPLE_CGMS_ERR_PRINT("=====phAvplay == NULL=====\n");
        return MT_FAILURE;
    }

    if (NULL == pProgInfo)
    {
        SAMPLE_CGMS_ERR_PRINT("=====pProgInfo == NULL=====\n");
        return MT_FAILURE;
    }

    /** Set the PID of the AV player and set the encoder type */
    ret=MT_CGMSSetAvplayPidAndCodecType(hAvplay, pProgInfo);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_CGMSSetAvplayPidAndCodecType fail! \n");
        return ret;
    }
    ...
    {
        SAMPLE_CGMS_ERR_PRINT("Has no audio stream! ret=0x%x \n", ret);
    }
    ...
    {
        SAMPLE_CGMS_ERR_PRINT("Has no video stream! ret=0x%x \n", ret);
    }
    ...
    if((enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_AUD) && (enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_VID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("Set frame to VO is failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("Get avplay sync attr is failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("Set avplay sync attr is failed ret=0x%x \n", ret);
            return ret;
        }
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_UNF_AVPLAY_Start failed ret=0x%x \n", ret);
        return ret;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_S32 MT_CGMSStopToPlay(MT_HANDLE hAvplay)
{
    ...
    SAMPLE_CGMS_INFO_PRINT("stop live play ...\n");
    return MT_UNF_AVPLAY_Stop(hAvplay, MT_UNF_AVPLAY_MEDIA_CHAN_VID | MT_UNF_AVPLAY_MEDIA_CHAN_AUD, &option);
}


static MT_VOID MT_CGMSExit(void)
{
    g_bTaskQuit = MT_TRUE;

    /** Stop playing the show */
    (MT_VOID)MT_CGMSStopToPlay(cgms_run_info.hAvPlay);

    /** Audio and video player deinitialization */
    (MT_VOID)MT_CGMSAvplayDeInit(cgms_run_info.hAvPlay, cgms_run_info.hWin, cgms_run_info.hSoundTrack);

    (MT_VOID)MTADP_Search_FreeAllPmt(cgms_run_info.pProgTbl);

    (MT_VOID)MTADP_Search_DeInit();

    if(MT_INPUT_SIG_TYPE_FILE == cgms_run_info.sInputParam.sig_type)
    {
        /** Wait for the thread to end */
        pthread_join(cgms_run_info.stInjectTSThread, NULL);
    }

    (MT_VOID)MT_CGMSDmxDeInit();
    ...
    memset(&cgms_run_info, 0xff, sizeof(cgms_run_info));
}
...
@return::void
@*/
static void MT_CGMSPrint_Help(MT_CHAR *name)
{
    MT_CGMS_PRINT("Lack of parameters\n");
    MT_CGMS_PRINT("\nUsage:\n");
    MT_CGMS_PRINT("%s\n", name);
    MT_CGMS_PRINT("    -f: path of the stream file\n");
    MT_CGMS_PRINT("    -c: DVBC locks frequency\n");
    MT_CGMS_PRINT("    -s: DVBS locks frequency\n");
#ifdef MT_SAMPLE_APP
    MT_CGMS_PRINT("    -q: Exit the background\n");
#endif
    MT_CGMS_PRINT("example:\n");
    MT_CGMS_PRINT("    %s -f ./677-CC-12.ts\n", name);
    MT_CGMS_PRINT("    %s -c 314 6875 64\n", name);
    MT_CGMS_PRINT("    %s -s 3840 27500 1 0 0\n", name);
}
...
/*
 ...
 @return ::MT_SUCCESS
*/
static MT_S32 MT_CGMSParase_args(MT_S32 argc, MT_CHAR *argv[], mt_input_para_t *pInputParam)
{
    MT_S32 opt = 0;

    if(argc < 2 && g_bTaskQuit == MT_TRUE)
    {
        (MT_VOID)MT_CGMSPrint_Help(argv[0]);
        return MT_FAILURE;
    }
...
static MT_VOID MT_CGMSPrintMenu(PMT_COMPACT_TBL *pProgTbl)
{
    MT_CGMS_PRINT("commond: \n");
    MT_CGMS_PRINT("     s: switch the four modes of CGMS-A\n");
#ifdef MT_SAMPLE_APP
    MT_CGMS_PRINT("     b : background run \n");
#endif
    MT_CGMS_PRINT("     q: quit \n");
    MT_CGMS_PRINT("     h: help \n");
    MT_CGMS_PRINT("'1-%d': Select a program \n", pProgTbl->prog_num);
    MT_CGMS_PRINT("CGMS_A>> ");
}
...
@return ::MT_FAILURE
*/
static MT_VOID MT_CGMSCmdTask(MT_HANDLE hAvPlay, PMT_COMPACT_TBL *pProgTbl)
{
    MT_U32                 ix = 0;
    MT_S32                 ret = MT_FAILURE;
    MT_CHAR                inPutCmd[32] = { 0 };
    MT_CHAR                *CGMS_A_Mode[] = {"COPY FREELY", "CopyNoMore", "CopyOnce", "CopyNever"};
    MT_U32                 u32ProgNum = 0;
    PMT_COMPACT_PROG       *pstCurrentProgInfo = NULL;
    MT_UNF_DISP_CGMS_CFG_S cgms = { 0 };

    if(MT_INVALID_HANDLE == hAvPlay)
    {
        SAMPLE_CGMS_ERR_PRINT("The input handle is empty-------<%s> line: %d\n", __FUNCTION__, __LINE__);
        return;
    }

    while(1)
    {
        (MT_VOID)MT_CGMSPrintMenu(pProgTbl);
        fgets((char *)(inPutCmd), (sizeof(inPutCmd) - 1), stdin);

        if('q' == inPutCmd[0])
        {
            SAMPLE_CGMS_INFO_PRINT("now exit!\n");
            g_bTaskQuit = MT_TRUE;
            break;
        }
    #ifdef MT_SAMPLE_APP
        ...
        {
            SAMPLE_CGMS_INFO_PRINT("Cgms play in back!\n");
            break;
        }
    #endif
        ...
        if(u32ProgNum > 0 && u32ProgNum <= pProgTbl->prog_num)
        {
            ...
            ret = MT_CGMSStopToPlay(hAvPlay);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_CGMS_ERR_PRINT("MT_CGMSStopToPlay failed ret=0x%x \n", ret);
            }
            MT_CGMS_PRINT("===== Start play ProgNum: %d \n", u32ProgNum);

            /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
            ret = MT_CGMSStarToPlay(hAvPlay, pstCurrentProgInfo);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_CGMS_ERR_PRINT("Switching shows failed\n");
            }
        }
        else if ('s' == inPutCmd[0])
        {
            MT_CGMS_PRINT("open CGMS-A!!! mode::%s\n", CGMS_A_Mode[ix]);

            cgms.bEnable = 1;
            cgms.enType = MT_UNF_DISP_CGMS_TYPE_A;
            cgms.enMode = ix;

            MT_CGMS_PRINT("Current CGMS mode: %d\n", cgms.enMode);
            ret = MT_UNF_DISP_SetCgms(MT_UNF_DISPLAY0, &cgms);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_CGMS_ERR_PRINT("MT_UNF_DISP_SetCgms failed ret=0x%x \n", ret);
                break;
            }
        ...
        {
            SAMPLE_CGMS_INFO_PRINT("Print help info\n");
        }
    }
...
#endif
{
    ...
    /** Get the parameters */
    ret = MT_CGMSParase_args(argc, argv, &cgms_run_info.sInputParam);
    if (MT_FAILURE == ret)
    {
        SAMPLE_CGMS_ERR_PRINT("Parase args err. stop window.\n");

        return MT_FAILURE;
    }
    else if(MT_TASK_EXIT == ret)
    {
        SAMPLE_CGMS_ERR_PRINT("Recv stop command. stop window.\n");
    ...
    if(g_bTaskQuit == MT_TRUE)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("failed to mt_sys_init\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("failed to StartDmx\n");
            goto ERR1;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("failed to MTADP_Disp_Init\n");
            goto ERR2;
        }
    #endif
        ...
        if(MT_INPUT_SIG_TYPE_FILE != cgms_run_info.sInputParam.sig_type)
        {
            ...
            if(MT_SUCCESS != ret)
            {
                SAMPLE_CGMS_ERR_PRINT("MTADP_Fe_Init failed ret=0x%x \n", ret);
                goto ERR3;
            }

            if(MT_INPUT_SIG_TYPE_CAB == cgms_run_info.sInputParam.sig_type)
            {     //dvbc
                ret = MT_CGMSCheckDvbcParam(&cgms_run_info.sInputParam.input_param.cab);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_CGMS_ERR_PRINT("Input sat parameter error!\n");
                    goto ERR4;
                }

                ret = MTADP_Fe_Connect_Dvbc(TUNER_ID_0,
                                            cgms_run_info.sInputParam.input_param.cab.freq,
                                            cgms_run_info.sInputParam.input_param.cab.sym_rate,
                                            cgms_run_info.sInputParam.input_param.cab.mod_type);
            }
            else if(MT_INPUT_SIG_TYPE_SAT == cgms_run_info.sInputParam.sig_type)
            {
                ret = MT_CGMSCheckDvbsParam(&cgms_run_info.sInputParam.input_param.sat);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_CGMS_ERR_PRINT("Input sat parameter error!\n");
                    goto ERR4;
                ...
                ret = MTADP_Fe_Connect_Dvbs(TUNER_ID_0,
                                            cgms_run_info.sInputParam.input_param.sat.freq,
                                            cgms_run_info.sInputParam.input_param.sat.sym_rate,
                                            cgms_run_info.sInputParam.input_param.sat.onoff_22k,
                                            cgms_run_info.sInputParam.input_param.sat.polarization,
                                            cgms_run_info.sInputParam.input_param.sat.port_type);
            }

            if(MT_SUCCESS != ret)
            {
                SAMPLE_CGMS_ERR_PRINT("MTADP_Fe_Connect failed ret=0x%x \n", ret);
                goto ERR4;
            }
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MTADP_VO_Init failed ret=0x%x \n", ret);
            goto ERR4;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MTADP_Snd_Init failed ret=0x%x \n", ret);
            goto ERR5;
        }

        ret = MT_CGMSDmxInit(cgms_run_info.sInputParam.sig_type);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MT_CGMSDmxInit failed ret=0x%x \n", ret);
            goto ERR6;
        }

        if(MT_INPUT_SIG_TYPE_FILE == cgms_run_info.sInputParam.sig_type)
        {
            ret = pthread_create(&cgms_run_info.stInjectTSThread, NULL, (void * (*)(void *))InjectTsTask, &cgms_run_info.sInputParam.input_param.file);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_CGMS_ERR_PRINT("failed to pthread_create\n");
                goto ERR7;
            }
            sleep(1);
            if(g_bTaskQuit == MT_TRUE)
        ...
        /** Get the PMT table */
        while(MT_SUCCESS != MTADP_Search_GetAllPmt(DMX_ID_0, &cgms_run_info.pProgTbl))
        {
            count++;
            SAMPLE_CGMS_ERR_PRINT("failed to MTADP_Search_GetAllPmt\n");
            MT_USLEEP(100000);
            if(20 == count)
            {
                goto ERR9;
        ...
        ret = MT_CGMSAvplayInit(&cgms_run_info.hAvPlay, &cgms_run_info.hWin, &cgms_run_info.hSoundTrack);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MT_CGMSAvplayInit failed ret=0x%x \n", ret);
            goto ERR10;
        }

        MT_UNF_DISP_CGMS_CFG_S cgms;
        cgms.bEnable = 1;
        cgms.enType = MT_UNF_DISP_CGMS_TYPE_A;
        cgms.enMode = 0;
        ...
        pstCurrentProgInfo = cgms_run_info.pProgTbl->proginfo;
        /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
        ret = MT_CGMSStarToPlay(cgms_run_info.hAvPlay, pstCurrentProgInfo);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_CGMS_ERR_PRINT("MT_CGMSStarToPlay failed ret=0x%x \n", ret);
            goto ERR11;
        }
    }

    (MT_VOID)MT_CGMSCmdTask(cgms_run_info.hAvPlay, cgms_run_info.pProgTbl);
    ...
    ret = MT_CGMSStopToPlay(cgms_run_info.hAvPlay);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_CGMS_ERR_PRINT("MT_CGMSStopToPlay failed ret=0x%x \n", ret);
    }
