...
/***************************** Macro Definition ******************************/
#ifdef  MT_SAMPLE_HEAAC_DEBUG

#define MT_HEAAC_PRINT   printf
#else

#define MT_HEAAC_PRINT

#endif

#define SAMPLE_HEAAC_FUNCTION_ENTER()   MT_HEAAC_PRINT("[%s]: Enter ==>> \n", __FUNCTION__)
#define SAMPLE_HEAAC_FUNCTION_EXIT()        MT_HEAAC_PRINT("[%s]: Exit ==<< \n", __FUNCTION__)



#define SAMPLE_HEAAC_FATAL_PRINT(fmt...)       MT_HEAAC_PRINT(" [FATAL] " fmt)
#define SAMPLE_HEAAC_ERR_PRINT(fmt...)         MT_HEAAC_PRINT(" [ERROR] " fmt)
#define SAMPLE_HEAAC_WARN_PRINT(fmt...)        MT_HEAAC_PRINT(" [WARN] "  fmt)
#define SAMPLE_HEAAC_INFO_PRINT(fmt...)        MT_HEAAC_PRINT(" [INFO] "  fmt)
#define SAMPLE_HEAAC_DBG_PRINT(fmt...)         MT_HEAAC_PRINT(" [DEBUG] " fmt)
...
static MT_Heaac_RUN_INFO g_heaac_run_info;
#ifdef MT_SAMPLE_APP
...
static mt_s32 InjectTsTask(mt_void *args)
{
    ...
    SAMPLE_HEAAC_INFO_PRINT(">>>open file : %s  >>>> \n", pstParam->file_name);

    pTsFile = fopen((char*)pstParam->file_name, "rb");
    if(pTsFile == NULL)
    {
        SAMPLE_HEAAC_ERR_PRINT("file %s open error!!\n", pstParam->file_name);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    {
        SAMPLE_HEAAC_ERR_PRINT("failed to MT_UNF_DMX_CreateTSBuffer\n");
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_HEAAC_ERR_PRINT( "failed to MT_UNF_DMX_ResetTSBuffer ret=0x%x \n", s32Ret);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    while (g_bTaskQuit == MT_FALSE)
    {
        ...
        if(Readlen <= 0)
        {
            ...
            SAMPLE_HEAAC_INFO_PRINT("Read ts file end and rewind and Reset TS BUFFER and AVPLAYER...............!\n");
        ...
        if (s32Ret != MT_SUCCESS)
        {
            SAMPLE_HEAAC_ERR_PRINT("failed to MT_UNF_DMX_PutTSBuffer ret=0x%x \n", s32Ret);
        }
    }
...
static mt_s32 MT_HeaacCheckDvbcParam(mt_input_cab_para_t *p_cab_in)
{
    if(p_cab_in->freq < 45 || p_cab_in->freq > 862)
    {
        SAMPLE_HEAAC_ERR_PRINT("The frequency is not in range\n");
        return MT_FAILURE;
    }

    if(p_cab_in->sym_rate < 900 || p_cab_in->sym_rate > 7200)
    {
        SAMPLE_HEAAC_ERR_PRINT("The symbol rate is not in range\n");
        return MT_FAILURE;
    }
    ...
    switch(p_cab_in->mod_type)
    {
        ...
        case 256:
            break;
        default:
            SAMPLE_HEAAC_ERR_PRINT("QAM mismatch(16, 32, 64, 128, 256)\n");
            return MT_FAILURE;
    }
...
static mt_s32 MT_HeaacCheckDvbsParam(mt_input_sat_para_t *p_sat_in)
{

    if((p_sat_in->freq) > 4200 || (p_sat_in->freq) < 3000)
    {
        SAMPLE_HEAAC_ERR_PRINT("freq error. freq = %d \n", p_sat_in->freq);
        SAMPLE_HEAAC_ERR_PRINT("freq must be more than 3,000 and less than 4,200.\n");
        return MT_FAILURE;
    }
...
@*/
static MT_S32 MT_HeaacDmxInit(MT_INPUR_SIG_TYPE_T sig_type)
{
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_DMX_Init failed, s32Ret = 0x%x\n", s32Ret);
        return s32Ret;
    }
    ...
    if(MT_INPUT_SIG_TYPE_FILE == sig_type)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MT_UNF_DMX_AttachTSPort failed, s32Ret = 0x%x\n", s32Ret);
    ...
    else
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("failed to mt_sys_get_version\n");
            return MT_FAILURE;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("failed to MT_UNF_DMX_AttachTSPort\n");
            return MT_FAILURE;
        }
    }
...
@return::MT_VOID
@*/
static MT_VOID MT_HeaacDmxDeInit(MT_VOID)
{
...
@*/
static MT_S32 MT_HeaacAvplayInit(MT_HANDLE *phAvplay, MT_HANDLE *phWin, MT_HANDLE* phSoundTrack)
{
    ...
    if(NULL == phAvplay)
    {
        SAMPLE_HEAAC_ERR_PRINT("phAvplay is null.\n");
        return MT_FAILURE;
    ...
    if(NULL == phWin)
    {
        SAMPLE_HEAAC_ERR_PRINT("phWin is null.\n");
        return MT_FAILURE;
    ...
    {
        SAMPLE_HEAAC_ERR_PRINT("phSoundTrack is null.\n");
        return MT_FAILURE;
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MTADP_AVPlay_RegADecLib failed ret=0x%x \n", ret);
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_Init failed ret=0x%x \n", ret);
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_GetDefaultConfig failed ret=0x%x \n", ret);
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_Create failed ret=0x%x \n", ret);
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_ChnOpen failed ret=0x%x \n", ret);
        goto ERR2 ;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_SND_GetDefaultTrackAttr failed ret=0x%x \n", ret);
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_SND_CreateTrack failed ret=0x%x \n", ret);
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_SND_Attach failed ret=0x%x \n", ret);
        goto ERR5;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MTADP_VO_CreatWin failed ret=0x%x \n", ret);
        goto ERR6;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_VO_AttachWindow failed ret=0x%x \n", ret);
        goto ERR7;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_VO_SetWindowEnable failed ret=0x%x \n", ret);
        goto ERR8;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_S32 MT_HeaacStopToPlay(MT_HANDLE hAvplay)
{
    MT_UNF_AVPLAY_STOP_OPT_S option = { 0 };

    if (MT_INVALID_HANDLE == hAvplay)
    {
        SAMPLE_HEAAC_ERR_PRINT("=====phAvplay == NULL=====\n");
        return MT_FAILURE;
    }
    ...
    SAMPLE_HEAAC_INFO_PRINT("stop live play ...\n");
...
@return::ret                    The return value of the error.
@*/
static MT_VOID MT_HeaacAvplayDeInit(MT_HANDLE hAvplay, MT_HANDLE hWin, MT_HANDLE hSoundTrack)
{
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_HeaacSetAvplayPidAndCodecType(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    MT_S32                           ret = MT_FAILURE;
    MT_U32                           u32AudType = 0;
    MT_U32                           VidPid = 0;
    ...
    if(NULL == pProgInfo)
    {
        SAMPLE_HEAAC_ERR_PRINT("The input address is empty\n");
        return MT_FAILURE;
    }
    ...
    SAMPLE_HEAAC_INFO_PRINT("VidPid = %#x, AudPid = %#x\n", VidPid, AudPid);
    ...
    if(VidPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_GetAttr failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("Set video properties or video PID property failed\n");
            return ret;
        }
    }
    ...
    if(AudPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("Setting the decoding mode or audio PID property failed\n");
            return ret;
        }
    }
    ...
    if((VidPid != INVALID_TSPID) || (AudPid != INVALID_TSPID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_SetAttr failed ret=0x%x \n", ret);
            return ret;
        }
    }
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_HeaacStarToPlay(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    if (NULL == pProgInfo)
    {
        SAMPLE_HEAAC_ERR_PRINT("=====pProgInfo == NULL=====\n");
        return MT_FAILURE;
    }

    /** Set the PID of the AV player and set the encoder type */
    ret=MT_HeaacSetAvplayPidAndCodecType(hAvplay, pProgInfo);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_HeaacSetAvplayPidAndCodecType fail! \n");
        return ret;
    }
    ...
    {
        SAMPLE_HEAAC_ERR_PRINT("Has no audio stream! ret=0x%x \n", ret);
    }
    ...
    {
        SAMPLE_HEAAC_ERR_PRINT("Has no video stream! ret=0x%x \n", ret);
    }
    ...
    if((enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_AUD) && (enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_VID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("Set frame to VO is failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("Get avplay sync attr is failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("Set avplay sync attr is failed ret=0x%x \n", ret);
            return ret;
        }
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_UNF_AVPLAY_Start failed ret=0x%x \n", ret);
        return ret;
    }
...
static MT_VOID MT_HeaacExit(void)
{
    g_bTaskQuit = MT_TRUE;

#ifndef MT_SAMPLE_APP
    /** Stop playing the show */
    (MT_VOID)MT_HeaacStopToPlay(g_heaac_run_info.hAvPlay);

    /** Audio and video player deinitialization */
    (MT_VOID)MT_HeaacAvplayDeInit(g_heaac_run_info.hAvPlay, g_heaac_run_info.hWin, g_heaac_run_info.hSoundTrack);

    (MT_VOID)MTADP_Search_FreeAllPmt(g_heaac_run_info.pProgTbl);

    (MT_VOID)MTADP_Search_DeInit();

    if(MT_INPUT_SIG_TYPE_FILE == g_heaac_run_info.sInputParam.sig_type)
    {
        /** Wait for the thread to end */
        pthread_join(g_heaac_run_info.stInjectTSThread, NULL);
    }

    (MT_VOID)MT_HeaacDmxDeInit();
...
#endif
    memset(&g_heaac_run_info, 0xff, sizeof(g_heaac_run_info));
}
...
@return::void
@*/
static void MT_HeaacPrint_Help(MT_CHAR *name)
{
#ifndef MT_SAMPLE_APP
    MT_HEAAC_PRINT("Lack of parameters\n");
    MT_HEAAC_PRINT("\nUsage:\n");
    MT_HEAAC_PRINT("%s\n", name);
    MT_HEAAC_PRINT("    -f: path of the stream file\n");
    MT_HEAAC_PRINT("    -c: DVBC locks frequency\n");
    MT_HEAAC_PRINT("    -s: DVBS locks frequency\n");
    MT_HEAAC_PRINT("example:\n");
    MT_HEAAC_PRINT("    %s -f ./677-CC-12.ts\n", name);
    MT_HEAAC_PRINT("    %s -c 314 6875 64\n", name);
    MT_HEAAC_PRINT("    %s -s 3840 27500 1 0 0\n", name);
#else
    MT_HEAAC_PRINT("    -q: Exit the background\n");
#endif
}
...
/*
 ...
 @return ::MT_SUCCESS
*/
static MT_S32 MT_HeaacParase_args(MT_S32 argc, MT_CHAR *argv[], mt_input_para_t *pInputParam)
{
    MT_S32 opt = 0;
    ...
    {
        (MT_VOID)MT_HeaacPrint_Help(argv[0]);
        return MT_FAILURE;
    }
#endif
...
static MT_VOID MT_HeaacPrintMenu(mt_s32 prognum)
{
    MT_HEAAC_PRINT("commond: \n");
#ifndef MT_SAMPLE_APP
    MT_HEAAC_PRINT("'1-%d': Select a program \n", prognum);
#endif
    MT_HEAAC_PRINT("     e: Open heaac\n");
    MT_HEAAC_PRINT("     d: close heaac(Take effect after restart) \n");

#ifdef MT_SAMPLE_APP
    MT_HEAAC_PRINT("     b: background run \n");
#endif
    MT_HEAAC_PRINT("     q: quit \n");
    MT_HEAAC_PRINT("     h: help \n");
    MT_HEAAC_PRINT("Heaac>> ");
}
...
@return ::MT_FAILURE
*/
static MT_VOID MT_HeaacCmdTask(MT_HANDLE hAvPlay, PMT_COMPACT_TBL *pProgTbl)
{
    ...
    if(MT_INVALID_HANDLE == hAvPlay)
    {
        SAMPLE_HEAAC_ERR_PRINT("The input handle is empty-------<%s> line: %d\n", __FUNCTION__, __LINE__);
        return;
    }
#endif
    ...
    while(1)
    {
        (MT_VOID)MT_HeaacPrintMenu(pProgTbl->prog_num);
        fgets((char *)(inPutCmd), (sizeof(inPutCmd) - 1), stdin);

        if('q' == inPutCmd[0])
        {
            SAMPLE_HEAAC_INFO_PRINT("now exit!\n");
            g_bTaskQuit = MT_TRUE;
            break;
        }
    #ifdef MT_SAMPLE_APP
        ...
        {
            SAMPLE_HEAAC_INFO_PRINT("heaac play in back!\n");
            break;
        }
    #endif
#ifndef MT_SAMPLE_APP
        ...
        if(u32ProgNum > 0 && u32ProgNum <= pProgTbl->prog_num)
        {
            ...
            ret = MT_HeaacStopToPlay(hAvPlay);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_HEAAC_ERR_PRINT("MT_HeaacStopToPlay failed ret=0x%x \n", ret);
            }
            MT_HEAAC_PRINT("===== Start play ProgNum: %d \n", u32ProgNum);

            /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
            ret = MT_HeaacStarToPlay(hAvPlay, pstCurrentProgInfo);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_HEAAC_ERR_PRINT("Switching shows failed\n");
            }
            clock_gettime(CLOCK_MONOTONIC, &end);
            elapsed_seconds = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
            MT_HEAAC_PRINT("Switching time = %lf s\n", elapsed_seconds);
        }
#endif
        else if ('e' == inPutCmd[0])
        {
            MT_HEAAC_PRINT("Enable HEAAC\n");
            MT_UNF_AVPLAY_Enable_AudioHEAAC(hAvPlay);
            MTADP_Set_Heaac_Enable(MT_TRUE);
        }
        else if ('d' == inPutCmd[0])
        {
            MT_HEAAC_PRINT("Disable HEAAC\n");
            MTADP_Set_Heaac_Enable(MT_FALSE);
        }
        else if('h' == inPutCmd[0])
        {
            SAMPLE_HEAAC_INFO_PRINT("Print help info\n");
        }
    }
...
#endif
{
    ...
    /** Get the parameters */
    ret = MT_HeaacParase_args(argc, argv, &g_heaac_run_info.sInputParam);
    if (MT_FAILURE == ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("Parase args err. stop window.\n");

        return MT_FAILURE;
    }
    else if(MT_TASK_EXIT == ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("Recv stop command. stop window.\n");
    ...
    if(g_bTaskQuit == MT_TRUE)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("failed to mt_sys_init\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("failed to StartDmx\n");
            goto ERR1;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("failed to MTADP_Disp_Init\n");
            goto ERR2;
        }
        ...
        if(MT_INPUT_SIG_TYPE_FILE != g_heaac_run_info.sInputParam.sig_type)
        {
            ...
            if(MT_SUCCESS != ret)
            {
                SAMPLE_HEAAC_ERR_PRINT("MTADP_Fe_Init failed ret=0x%x \n", ret);
                goto ERR3;
            }

            if(MT_INPUT_SIG_TYPE_CAB == g_heaac_run_info.sInputParam.sig_type)
            {     //dvbc
                ret = MT_HeaacCheckDvbcParam(&g_heaac_run_info.sInputParam.input_param.cab);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_HEAAC_ERR_PRINT("Input sat parameter error!\n");
                    goto ERR4;
                }

                ret = MTADP_Fe_Connect_Dvbc(TUNER_ID_0,
                                            g_heaac_run_info.sInputParam.input_param.cab.freq,
                                            g_heaac_run_info.sInputParam.input_param.cab.sym_rate,
                                            g_heaac_run_info.sInputParam.input_param.cab.mod_type);
            }
            else if(MT_INPUT_SIG_TYPE_SAT == g_heaac_run_info.sInputParam.sig_type)
            {
                ret = MT_HeaacCheckDvbsParam(&g_heaac_run_info.sInputParam.input_param.sat);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_HEAAC_ERR_PRINT("Input sat parameter error!\n");
                    goto ERR4;
                ...
                ret = MTADP_Fe_Connect_Dvbs(TUNER_ID_0,
                                            g_heaac_run_info.sInputParam.input_param.sat.freq,
                                            g_heaac_run_info.sInputParam.input_param.sat.sym_rate,
                                            g_heaac_run_info.sInputParam.input_param.sat.onoff_22k,
                                            g_heaac_run_info.sInputParam.input_param.sat.polarization,
                                            g_heaac_run_info.sInputParam.input_param.sat.port_type);
            }

            if(MT_SUCCESS != ret)
            {
                SAMPLE_HEAAC_ERR_PRINT("MTADP_Fe_Connect failed ret=0x%x \n", ret);
                goto ERR4;
            }
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MTADP_VO_Init failed ret=0x%x \n", ret);
            goto ERR4;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MTADP_Snd_Init failed ret=0x%x \n", ret);
            goto ERR5;
        }

        ret = MT_HeaacDmxInit(g_heaac_run_info.sInputParam.sig_type);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MT_HeaacDmxInit failed ret=0x%x \n", ret);
            goto ERR6;
        }

        if(MT_INPUT_SIG_TYPE_FILE == g_heaac_run_info.sInputParam.sig_type)
        {
            ret = pthread_create(&g_heaac_run_info.stInjectTSThread, NULL, (void * (*)(void *))InjectTsTask, &g_heaac_run_info.sInputParam.input_param.file);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_HEAAC_ERR_PRINT("failed to pthread_create\n");
                goto ERR7;
            }
            sleep(1);
            if(g_bTaskQuit == MT_TRUE)
        ...
        /** Get the PMT table */
        while(MT_SUCCESS != MTADP_Search_GetAllPmt(DMX_ID_0, &g_heaac_run_info.pProgTbl))
        {
            count++;
            SAMPLE_HEAAC_ERR_PRINT("failed to MTADP_Search_GetAllPmt\n");
            MT_USLEEP(100000);
            if(20 == count)
            {
                goto ERR9;
        ...
        ret = MT_HeaacAvplayInit(&g_heaac_run_info.hAvPlay, &g_heaac_run_info.hWin, &g_heaac_run_info.hSoundTrack);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MT_HeaacAvplayInit failed ret=0x%x \n", ret);
            goto ERR10;
        }

        pstCurrentProgInfo = g_heaac_run_info.pProgTbl->proginfo;
        /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
        ret = MT_HeaacStarToPlay(g_heaac_run_info.hAvPlay, pstCurrentProgInfo);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_HEAAC_ERR_PRINT("MT_HeaacStarToPlay failed ret=0x%x \n", ret);
            goto ERR11;
        }
        clock_gettime(CLOCK_MONOTONIC, &end);
        elapsed_seconds = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
        MT_HEAAC_PRINT("Search time = %lf s\n", elapsed_seconds);
    #endif
        g_bTaskQuit = MT_FALSE;
    }

#ifdef MT_SAMPLE_APP
    g_heaac_run_info.hAvPlay = avplayHandle.hAvPlay;
#endif

    (MT_VOID)MT_HeaacCmdTask(g_heaac_run_info.hAvPlay, g_heaac_run_info.pProgTbl);
    ...
    ret = MT_HeaacStopToPlay(g_heaac_run_info.hAvPlay);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_HEAAC_ERR_PRINT("MT_HeaacStopToPlay failed ret=0x%x \n", ret);
    }
