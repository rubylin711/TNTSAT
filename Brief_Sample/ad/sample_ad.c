...
/***************************** Macro Definition ******************************/
#ifdef  MT_SAMPLE_AD_DEBUG

#define MT_AD_PRINT   printf
#else
...
#define SAMPLE_AD_FUNCTION_ENTER()  MT_AD_PRINT("[%s]: Enter ==>> \n", __FUNCTION__)
#define SAMPLE_AD_FUNCTION_EXIT()       MT_AD_PRINT("[%s]: Exit ==<< \n", __FUNCTION__)



#define SAMPLE_AD_FATAL_PRINT(fmt...)       MT_AD_PRINT(" [FATAL] " fmt)
#define SAMPLE_AD_ERR_PRINT(fmt...)         MT_AD_PRINT(" [ERROR] " fmt)
#define SAMPLE_AD_WARN_PRINT(fmt...)        MT_AD_PRINT(" [WARN] "  fmt)
#define SAMPLE_AD_INFO_PRINT(fmt...)        MT_AD_PRINT(" [INFO] "  fmt)
#define SAMPLE_AD_DBG_PRINT(fmt...)         MT_AD_PRINT(" [DEBUG] " fmt)
...
static MT_AD_RUN_INFO ad_run_info;
#ifdef MT_SAMPLE_APP
...
static mt_s32 InjectTsTask(mt_void *args)
{
    ...
    SAMPLE_AD_INFO_PRINT(">>>open file : %s  >>>> \n", pstParam->file_name);

    pTsFile = fopen((char*)pstParam->file_name, "rb");
    if(pTsFile == NULL)
    {
        SAMPLE_AD_ERR_PRINT("file %s open error!!\n", pstParam->file_name);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    {
        SAMPLE_AD_ERR_PRINT("failed to MT_UNF_DMX_CreateTSBuffer\n");
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_AD_ERR_PRINT( "failed to MT_UNF_DMX_ResetTSBuffer ret=0x%x \n", s32Ret);
        g_bTaskQuit = MT_TRUE;
        return MT_FAILURE;
    }
    ...
    while (g_bTaskQuit == MT_FALSE)
    {
        ...
        if(Readlen <= 0)
        {
            ...
            SAMPLE_AD_INFO_PRINT("Read ts file end and rewind and Reset TS BUFFER and AVPLAYER...............!\n");
        ...
        if (s32Ret != MT_SUCCESS)
        {
            SAMPLE_AD_ERR_PRINT("failed to MT_UNF_DMX_PutTSBuffer ret=0x%x \n", s32Ret);
        }
    }
...
static mt_s32 MT_ADCheckDvbcParam(mt_input_cab_para_t *p_cab_in)
{
    if(p_cab_in->freq < 45 || p_cab_in->freq > 862)
    {
        SAMPLE_AD_ERR_PRINT("The frequency is not in range\n");
        return MT_FAILURE;
    }

    if(p_cab_in->sym_rate < 900 || p_cab_in->sym_rate > 7200)
    {
        SAMPLE_AD_ERR_PRINT("The symbol rate is not in range\n");
        return MT_FAILURE;
    }
    ...
    switch(p_cab_in->mod_type)
    {
        ...
        case 256:
            break;
        default:
            SAMPLE_AD_ERR_PRINT("QAM mismatch(16, 32, 64, 128, 256)\n");
            return MT_FAILURE;
    }
...
static mt_s32 MT_ADCheckDvbsParam(mt_input_sat_para_t *p_sat_in)
{

    if((p_sat_in->freq) > 4200 || (p_sat_in->freq) < 3000)
    {
        SAMPLE_AD_ERR_PRINT("freq error. freq = %d \n", p_sat_in->freq);
        SAMPLE_AD_ERR_PRINT("freq must be more than 3,000 and less than 4,200.\n");
        return MT_FAILURE;
    }
...
@*/
static MT_S32 MT_ADDmxInit(MT_INPUR_SIG_TYPE_T sig_type)
{
    ...
    if(MT_SUCCESS != s32Ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_DMX_Init failed, s32Ret = 0x%x\n", s32Ret);
        return s32Ret;
    }
    ...
    if(MT_INPUT_SIG_TYPE_FILE == sig_type)
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_AD_ERR_PRINT("MT_UNF_DMX_AttachTSPort failed, s32Ret = 0x%x\n", s32Ret);
    ...
    else
    {
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_AD_ERR_PRINT("failed to mt_sys_get_version\n");
            return MT_FAILURE;
        }
        ...
        if(MT_SUCCESS != s32Ret)
        {
            SAMPLE_AD_ERR_PRINT("failed to MT_UNF_DMX_AttachTSPort\n");
            return MT_FAILURE;
        }
    }
...
@return::MT_VOID
@*/
static MT_VOID MT_ADDmxDeInit(MT_VOID)
{
...
@*/
static MT_S32 MT_ADAvplayInit(MT_HANDLE *phAvplay, MT_HANDLE *phWin, MT_HANDLE* phSoundTrack)
{
    ...
    if(NULL == phAvplay)
    {
        SAMPLE_AD_ERR_PRINT("phAvplay is null.\n");
        return MT_FAILURE;
    ...
    if(NULL == phWin)
    {
        SAMPLE_AD_ERR_PRINT("phWin is null.\n");
        return MT_FAILURE;
    ...
    {
        SAMPLE_AD_ERR_PRINT("phSoundTrack is null.\n");
        return MT_FAILURE;
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MTADP_AVPlay_RegADecLib failed ret=0x%x \n", ret);
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_Init failed ret=0x%x \n", ret);
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_GetDefaultConfig failed ret=0x%x \n", ret);
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_Create failed ret=0x%x \n", ret);
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_ChnOpen failed ret=0x%x \n", ret);
        goto ERR2 ;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_SND_GetDefaultTrackAttr failed ret=0x%x \n", ret);
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_SND_CreateTrack failed ret=0x%x \n", ret);
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_SND_Attach failed ret=0x%x \n", ret);
        goto ERR5;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MTADP_VO_CreatWin failed ret=0x%x \n", ret);
        goto ERR6;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_VO_AttachWindow failed ret=0x%x \n", ret);
        goto ERR7;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_VO_SetWindowEnable failed ret=0x%x \n", ret);
        goto ERR8;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_VOID MT_ADAvplayDeInit(MT_HANDLE hAvplay, MT_HANDLE hWin, MT_HANDLE hSoundTrack)
{
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_ADSetAvplayPidAndCodecType(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    MT_S32                           ret = MT_FAILURE;
    MT_U32                           u32AudType = 0;
    MT_U32                           VidPid = 0;
    ...
    if(NULL == pProgInfo)
    {
        SAMPLE_AD_ERR_PRINT("The input address is empty\n");
        return MT_FAILURE;
    }
    ...
    SAMPLE_AD_INFO_PRINT("VidPid = %#x, AudPid = %#x\n", VidPid, AudPid);
    ...
    if(VidPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_GetAttr failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("Set video properties or video PID property failed\n");
            return ret;
        }
    }
    ...
    if(AudPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("Setting the decoding mode or audio PID property failed\n");
            return ret;
        }
    }
    ...
    if((VidPid != INVALID_TSPID) || (AudPid != INVALID_TSPID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_SetAttr failed ret=0x%x \n", ret);
            return ret;
        }
    }
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_ADStarToPlay(MT_HANDLE hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    if (MT_INVALID_HANDLE == hAvplay)
    {
        SAMPLE_AD_ERR_PRINT("=====phAvplay == NULL=====\n");
        return MT_FAILURE;
    }

    if (NULL == pProgInfo)
    {
        SAMPLE_AD_ERR_PRINT("=====pProgInfo == NULL=====\n");
        return MT_FAILURE;
    }

    /** Set the PID of the AV player and set the encoder type */
    ret=MT_ADSetAvplayPidAndCodecType(hAvplay, pProgInfo);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_ADSetAvplayPidAndCodecType fail! \n");
        return ret;
    }
    ...
    {
        SAMPLE_AD_ERR_PRINT("Has no audio stream! ret=0x%x \n", ret);
    }
    ...
    {
        SAMPLE_AD_ERR_PRINT("Has no video stream! ret=0x%x \n", ret);
    }
    ...
    if((enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_AUD) && (enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_VID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("Set frame to VO is failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("Get avplay sync attr is failed ret=0x%x \n", ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("Set avplay sync attr is failed ret=0x%x \n", ret);
            return ret;
        }
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_Start failed ret=0x%x \n", ret);
        return ret;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_S32 MT_ADStopToPlay(MT_HANDLE hAvplay)
{
    ...
    SAMPLE_AD_INFO_PRINT("stop live play ...\n");
...
@return::ret                    The return value of the error
@*/
static MT_S32 MT_ADStarAd(MT_HANDLE hAvplay, MT_UNF_AVPLAY_AD_ATTR_S audio_ad_attr)
{
    MT_U32  ret = MT_FAILURE;
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_AudioTrack failed ret=0x%x \n", ret);
        return ret;
    }

    ret = MT_UNF_AVPLAY_SetAttr(hAvplay, MT_UNF_AVPLAY_ATTR_ID_AD, &audio_ad_attr);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_SetAttr ad failed ret=0x%x \n", ret);
        return ret;
    }

    /** Start the AV player into the start state, param[enMediaType] Simultaneous playback of audio and video */
    ret = MT_UNF_AVPLAY_Start(hAvplay, MT_UNF_AVPLAY_MEDIA_CHAN_AUD, NULL);
...
static MT_S32 MT_ADStopToAd(MT_HANDLE hAvplay)
{
    MT_UNF_AVPLAY_STOP_OPT_S option = { 0 };
    ...
    option.enMode = MT_UNF_AVPLAY_STOP_MODE_BLACK;
    option.u32TimeoutMs = 0;
    SAMPLE_AD_INFO_PRINT("stop track ...\n");
    return MT_UNF_AVPLAY_Stop(hAvplay, MT_UNF_AVPLAY_MEDIA_CHAN_AUD, &option);
}

static MT_VOID MT_ADExit(void)
{
    g_bTaskQuit = MT_TRUE;

    /** Stop playing the show */
    (MT_VOID)MT_ADStopToPlay(ad_run_info.hAvPlay);

    /** Audio and video player deinitialization */
    (MT_VOID)MT_ADAvplayDeInit(ad_run_info.hAvPlay, ad_run_info.hWin, ad_run_info.hSoundTrack);

    (MT_VOID)MTADP_Search_FreeAllPmt(ad_run_info.pProgTbl);

    (MT_VOID)MTADP_Search_DeInit();

    if(MT_INPUT_SIG_TYPE_FILE == ad_run_info.sInputParam.sig_type)
    {
        /** Wait for the thread to end */
        pthread_join(ad_run_info.stInjectTSThread, NULL);
    }

    (MT_VOID)MT_ADDmxDeInit();
    ...
    memset(&ad_run_info, 0xff, sizeof(ad_run_info));
}
...
@return::void
@*/
static void MT_ADPrint_Help(MT_CHAR *name)
{
#ifndef MT_SAMPLE_APP
    MT_AD_PRINT("Lack of parameters\n");
    MT_AD_PRINT("\nUsage:\n");
    MT_AD_PRINT("%s\n", name);
    MT_AD_PRINT("    -f: path of the stream file\n");
    MT_AD_PRINT("    -c: DVBC locks frequency\n");
    MT_AD_PRINT("    -s: DVBS locks frequency\n");
    MT_AD_PRINT("example:\n");
    MT_AD_PRINT("    %s -f ./677-CC-12.ts\n", name);
    MT_AD_PRINT("    %s -c 314 6875 64\n", name);
    MT_AD_PRINT("    %s -s 3840 27500 1 0 0\n", name);
#endif
#ifdef MT_SAMPLE_APP
    MT_AD_PRINT("    -q: Exit the background\n");
#endif
}
...
/*
 ...
 @return ::MT_SUCCESS
*/
static MT_S32 MT_ADParase_args(MT_S32 argc, MT_CHAR *argv[], mt_input_para_t *pInputParam)
{
    MT_S32 opt = 0;
    ...
    {
        (MT_VOID)MT_ADPrint_Help(argv[0]);
        return MT_FAILURE;
    }
#endif
...
static MT_VOID MT_ADPrintMenu(PMT_COMPACT_TBL *pProgTbl)
{
    MT_AD_PRINT("commond: \n");
    MT_AD_PRINT("     s: Open AD\n");
    MT_AD_PRINT("     t: Close AD\n");
    MT_AD_PRINT("     u: turn up AD weight value(0 ~ 7)\n");
    MT_AD_PRINT("     d: turn down AD weight value(0 ~ 7)\n");
    MT_AD_PRINT("     a: adjust AD weight value(0 ~ 7)\n");
    MT_AD_PRINT("     g: obtain current AD weight value\n");
#ifdef MT_SAMPLE_APP
    MT_AD_PRINT("     b: background run \n");
#endif
    MT_AD_PRINT("     q: quit \n");
    MT_AD_PRINT("     h: help \n");
#ifndef MT_SAMPLE_APP
    MT_AD_PRINT("'1-%d': Select a program \n", pProgTbl->prog_num);
#endif
    MT_AD_PRINT("AD>> ");
}
...
@return ::MT_FAILURE
*/
static MT_VOID MT_ADCmdTask(MT_HANDLE hAvPlay, PMT_COMPACT_TBL *pProgTbl, PMT_COMPACT_PROG *pstCurrentProgInfo)
{
    MT_S32                 ret = MT_FAILURE;
    MT_U32                 adinfo[4] = {0};
    MT_U32                 adtype[4] = {0};
    ...
    MT_UNF_AVPLAY_AD_ATTR_S audio_ad_attr = { 0 };
    mt_s32 ad_vol_weight = 0;
    mt_s32 i = 0;
    mt_s32 j = 0;
    ...
    if(MT_INVALID_HANDLE == hAvPlay)
    {
        SAMPLE_AD_ERR_PRINT("The input handle is empty-------<%s> line: %d\n", __FUNCTION__, __LINE__);
        return;
    }
#endif
    while(1)
    {
        (MT_VOID)MT_ADPrintMenu(pProgTbl);
        fgets((char *)(inPutCmd), (sizeof(inPutCmd) - 1), stdin);

        if ('q' == inPutCmd[0])
        {
            SAMPLE_AD_INFO_PRINT("now exit!\n");
            g_bTaskQuit = MT_TRUE;
            break;
        }
    #ifdef MT_SAMPLE_APP
        ...
        {
            SAMPLE_AD_INFO_PRINT("AD play in back!\n");
            break;
        }
    #endif
    #ifndef MT_SAMPLE_APP
        ...
        if (u32ProgNum > 0 && u32ProgNum <= pProgTbl->prog_num)
        {
            ...
            ret = MT_ADStopToPlay(hAvPlay);
            if (MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MT_ADStopToPlay failed ret=0x%x \n", ret);
            }

            MT_AD_PRINT("===== Start play ProgNum: %d \n", u32ProgNum);
            /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
            ret = MT_ADStarToPlay(hAvPlay, pstCurrentProgInfo);
            if (MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("Switching shows failed\n");
            }

            clock_gettime(CLOCK_MONOTONIC, &end);
            elapsed_seconds = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
            MT_AD_PRINT("Switching time = %lf s\n", elapsed_seconds);
        }
    #endif
        else if ('s' == inPutCmd[0])
        {
            adnum = 0;
            j = 0;
            if(pstCurrentProgInfo->AElementNum > 1)
            {
                for(i = 0; i < pstCurrentProgInfo->AElementNum; i++)
                {
                    if(pstCurrentProgInfo->Audioinfo[i].u32audiotype == 0x03)
                    {
                        adinfo[j] = pstCurrentProgInfo->Audioinfo[i].u16AudioPid;
                        adtype[j] = pstCurrentProgInfo->Audioinfo[i].u32AudioEncType;
                        j++;
                        SAMPLE_AD_INFO_PRINT("adPid = %d \n", pstCurrentProgInfo->Audioinfo[i].u16AudioPid);
                        adnum = j;
                        break;
                    }
                }
            ...
            if(adnum == 0)
            {
                SAMPLE_AD_ERR_PRINT("there no ad pid \n");
                continue;
            }
            else if(adnum  == 1)
            {
                audio_ad_attr.u32AdPid = adinfo[0];
                audio_ad_attr.adectype = adtype[0];
                SAMPLE_AD_INFO_PRINT("adPid = %d \n", audio_ad_attr.u32AdPid);
            }
            else
            {
                for(i = 0; i < adnum; i++)
                {
                    SAMPLE_AD_INFO_PRINT("adinfo[%d] = %d \n", i, adinfo[i]);
                }
                SAMPLE_AD_INFO_PRINT("input numer (0 - %d) \n", adnum-1);
                scanf("%d",&i);
                getchar();
                audio_ad_attr.u32AdPid = adinfo[i];
                audio_ad_attr.adectype = adtype[i];
                SAMPLE_AD_INFO_PRINT("adPid = %d \n", audio_ad_attr.u32AdPid);

            }
            audio_ad_attr.bAdEnble = 1;
            ret = MT_ADStopToAd(hAvPlay);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MT_ADStopToAd failed ret=0x%x \n", ret);
            }
            ...
            /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
            ret = MT_ADStarAd(hAvPlay, audio_ad_attr);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MT_ADStarAd  failed\n");
            }
            MT_AD_PRINT("Open AD\n");
        }
        else if ('t' == inPutCmd[0])
        {
            audio_ad_attr.bAdEnble = 0;
            ret = MT_ADStopToAd(hAvPlay);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MT_ADStopToAd failed ret=0x%x \n", ret);
            ...
            MT_AD_PRINT("Close AD\n");
        }
        else if ('u' == inPutCmd[0])
        {
            ret = MT_UNF_AVPLAY_GetAttr(hAvPlay, MT_UNF_AVPLAY_ATTR_ID_AD_VOL_WEIGHT, &ad_vol_weight);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_GetAttr ad vol weight failed ret=0x%x \n", ret);
                return;
            }

            ad_vol_weight += 1;
            if (ad_vol_weight > MT_UNF_AVPLAY_AD_VOL_WEIGHT_LEVEL_3)
            {
                SAMPLE_AD_WARN_PRINT("The volume weight has been increased to the maximum \n");
                ad_vol_weight = MT_UNF_AVPLAY_AD_VOL_WEIGHT_LEVEL_3;
            }

            ret = MT_UNF_AVPLAY_SetAttr(hAvPlay, MT_UNF_AVPLAY_ATTR_ID_AD_VOL_WEIGHT, &ad_vol_weight);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MT_UNF_AVPLAY_SetAttr ad vol weight failed ret=0x%x \n", ret);
                return;
            }

            SAMPLE_AD_INFO_PRINT("AD volume weight = %d \n", ad_vol_weight);
        }
        else if ('d' == inPutCmd[0])
        {
            ...
            ad_vol_weight -= 1;
            if (ad_vol_weight <= MT_UNF_AVPLAY_AD_VOL_WEIGHT_DEFAULT)
            {
                SAMPLE_AD_WARN_PRINT("The volume weight has been resumed default \n");
                ad_vol_weight = MT_UNF_AVPLAY_AD_VOL_WEIGHT_DEFAULT;
            } 
            else if (ad_vol_weight == MT_UNF_AVPLAY_AD_VOL_WEIGHT_LEVEL_NEG_3)
            {
                SAMPLE_AD_WARN_PRINT("The volume weight has been decreased to the minimum \n");
            }
        ...
        else if ('a' == inPutCmd[0])
        {
            ...
            SAMPLE_AD_INFO_PRINT("Please enter the set volume weight(0 - 7):");
            scanf("%d",&ad_vol_weight);
            getchar();
            if (ad_vol_weight < MT_UNF_AVPLAY_AD_VOL_WEIGHT_DEFAULT || ad_vol_weight > MT_UNF_AVPLAY_AD_VOL_WEIGHT_LEVEL_3)
            {
                SAMPLE_AD_WARN_PRINT("Input error, please re-enter(0 - 7) !!! \n");
                continue;
            }
        ...
        {
            SAMPLE_AD_INFO_PRINT("Print help info\n");
        }
    }
}
...
#endif
{
    ...
    /** Get the parameters */
    ret = MT_ADParase_args(argc, argv, &ad_run_info.sInputParam);
    if (MT_FAILURE == ret)
    {
        SAMPLE_AD_ERR_PRINT("Parase args err. stop window.\n");

        return MT_FAILURE;
    }
    else if(MT_TASK_EXIT == ret)
    {
        SAMPLE_AD_ERR_PRINT("Recv stop command. stop window.\n");
    ...
    if(g_bTaskQuit == MT_TRUE)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("failed to mt_sys_init\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("failed to StartDmx\n");
            goto ERR1;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("failed to MTADP_Disp_Init\n");
            goto ERR2;
        }
        ...
        if(MT_INPUT_SIG_TYPE_FILE != ad_run_info.sInputParam.sig_type)
        {
            ...
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MTADP_Fe_Init failed ret=0x%x \n", ret);
                goto ERR3;
            }

            if(MT_INPUT_SIG_TYPE_CAB == ad_run_info.sInputParam.sig_type)
            {     //dvbc
                ret = MT_ADCheckDvbcParam(&ad_run_info.sInputParam.input_param.cab);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_AD_ERR_PRINT("Input sat parameter error!\n");
                    goto ERR4;
                }

                ret = MTADP_Fe_Connect_Dvbc(TUNER_ID_0,
                                            ad_run_info.sInputParam.input_param.cab.freq,
                                            ad_run_info.sInputParam.input_param.cab.sym_rate,
                                            ad_run_info.sInputParam.input_param.cab.mod_type);
            }
            else if(MT_INPUT_SIG_TYPE_SAT == ad_run_info.sInputParam.sig_type)
            {
                ret = MT_ADCheckDvbsParam(&ad_run_info.sInputParam.input_param.sat);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_AD_ERR_PRINT("Input sat parameter error!\n");
                    goto ERR4;
                ...
                ret = MTADP_Fe_Connect_Dvbs(TUNER_ID_0,
                                            ad_run_info.sInputParam.input_param.sat.freq,
                                            ad_run_info.sInputParam.input_param.sat.sym_rate,
                                            ad_run_info.sInputParam.input_param.sat.onoff_22k,
                                            ad_run_info.sInputParam.input_param.sat.polarization,
                                            ad_run_info.sInputParam.input_param.sat.port_type);
            }

            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("MTADP_Fe_Connect failed ret=0x%x \n", ret);
                goto ERR4;
            }
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MTADP_VO_Init failed ret=0x%x \n", ret);
            goto ERR4;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MTADP_Snd_Init failed ret=0x%x \n", ret);
            goto ERR5;
        }

        ret = MT_ADDmxInit(ad_run_info.sInputParam.sig_type);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MT_ADDmxInit failed ret=0x%x \n", ret);
            goto ERR6;
        }

        if(MT_INPUT_SIG_TYPE_FILE == ad_run_info.sInputParam.sig_type)
        {
            ret = pthread_create(&ad_run_info.stInjectTSThread, NULL, (void * (*)(void *))InjectTsTask, &ad_run_info.sInputParam.input_param.file);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_AD_ERR_PRINT("failed to pthread_create\n");
                goto ERR7;
            }
            sleep(1);
            if(g_bTaskQuit == MT_TRUE)
        ...
        /** Get the PMT table */
        while(MT_SUCCESS != MTADP_Search_GetAllPmt(DMX_ID_0, &ad_run_info.pProgTbl))
        {
            count++;
            SAMPLE_AD_ERR_PRINT("failed to MTADP_Search_GetAllPmt\n");
            MT_USLEEP(100000);
            if(20 == count)
            {
                goto ERR9;
        ...
        ret = MT_ADAvplayInit(&ad_run_info.hAvPlay, &ad_run_info.hWin, &ad_run_info.hSoundTrack);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MT_ADAvplayInit failed ret=0x%x \n", ret);
            goto ERR10;
        }

        pstCurrentProgInfo = ad_run_info.pProgTbl->proginfo;
        /** Start the AV playback into the start state, play according to pstCurrentProgInfo */
        ret = MT_ADStarToPlay(ad_run_info.hAvPlay, pstCurrentProgInfo);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_AD_ERR_PRINT("MT_ADStarToPlay failed ret=0x%x \n", ret);
            goto ERR11;
        }
        clock_gettime(CLOCK_MONOTONIC, &end);
        elapsed_seconds = (end.tv_sec - start.tv_sec) + (end.tv_nsec - start.tv_nsec) / 1e9;
        MT_AD_PRINT("Search time = %lf s\n", elapsed_seconds);
    #endif
        g_bTaskQuit = MT_FALSE;
    }

#ifdef MT_SAMPLE_APP
    ad_run_info.hAvPlay = avplayHandle.hAvPlay;

    ret = MTADP_Get_Current_Info(&pstCurrentProgInfo);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MTADP_Get_Current_Info ERR !!\n");
        return MT_FAILURE;
    }
#endif
    (MT_VOID)MT_ADCmdTask(ad_run_info.hAvPlay, ad_run_info.pProgTbl, pstCurrentProgInfo);
    ...
    ret = MT_ADStopToPlay(ad_run_info.hAvPlay);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_AD_ERR_PRINT("MT_ADStopToPlay failed ret=0x%x \n", ret);
    }
