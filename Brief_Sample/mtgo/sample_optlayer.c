...
static MT_VOID MT_MtgoOptlayerExit(SAMPLE_MTGO_SURFACE_INFO_S *surface)
{
...
static MT_VOID MT_OptlayerPrintMenu(MT_VOID)
{
    MTGO_OPTLAYER_PRINT("commond: \n");
    MTGO_OPTLAYER_PRINT("     1: show layer\n");
    MTGO_OPTLAYER_PRINT("     0: hide layer\n");
#ifdef MT_SAMPLE_APP
...
static void MT_MtgoOptlayerPrint_usage(MT_CHAR *name)
{
...
static MT_S32 MT_MtgoOptlayerParase_args(MT_S32 argc, MT_CHAR *argv[], SAMPLE_MTGO_SURFACE_INFO_S *pSurface)
{
    MT_S32 opt = 0;
    ...
    while((opt = MTADP_Getopt(argc, argv, ":?hHqt:")) != -1)
    {
        switch(opt)
        {
            ...
            case 'H':
                ...
                (MT_VOID)MT_MtgoOptlayerPrint_usage(argv[0]);
                return MT_FAILURE;
            case 'q':
                if(g_bTaskQuit == MT_FALSE)
                {
                    (MT_VOID)MT_MtgoOptlayerExit(pSurface);
                }
                return MT_TASK_EXIT;
            default:
                return MT_SUCCESS;
...
static MT_VOID MT_OptlayerModeCmdTask(SAMPLE_MTGO_SURFACE_INFO_S surface)
{
    MT_S8   inputCmd[32];
    ...
    while(1)
    {
        ...
        {
            /** This API is used to hide and show layers, which are used after layer refresh, with 0 for hidden and 1 for show */
            s32Ret = MT_GO_ShowLayer(surface.hLayer, 1);
            if (MT_SUCCESS != s32Ret)
            {
                SAMPLE_MTGO_ERR_PRINT("<%s>line [%d]: Show Layer failed!\n", __FUNCTION__, __LINE__);
                break;
            }
        }
        else if('0' == inputCmd[0])
        {
            s32Ret = MT_GO_ShowLayer(surface.hLayer, 0);
            if (MT_SUCCESS != s32Ret)
            {
                SAMPLE_MTGO_ERR_PRINT("<%s>line [%d]: Show Layer failed!\n", __FUNCTION__, __LINE__);
                break;
...
#endif
{
    ...
    ret = MT_MtgoOptlayerParase_args(argc, argv, &surface);
    if (MT_FAILURE == ret)
    {
    ...
    //  ret = MT_GO_RefreshLayerSync(hLayer, NULL);
        if (MT_SUCCESS != ret)
        {
    ...
    (MT_VOID)MT_OptlayerModeCmdTask(surface);
