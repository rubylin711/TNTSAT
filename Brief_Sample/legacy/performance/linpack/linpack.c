...
static REAL linpack(long nreps,int arsize)
    ...
    p = &ops;
    if (0x4124f49555555555ULL != *((unsigned long long *)(p))) {
        printf("ops calculate result wrong\n");
        exit(-1);
    }
    a=(REAL *)mempool;
    b=a+arsize2d;
    ...
    for (i=0;i<nreps;i++)
        ...
        p = &norma;
        if (0x3ffffa4000000000ULL != *((unsigned long long *)(p))) {
            printf("norma calculate result wrong\n");
            exit(-1);
        }
        t1 = second();
    ...
    for (i=0;i<nreps;i++)
        {
        matgen(a,lda,n,b,&norma);
        if (0x3ffffa4000000000ULL != *((unsigned long long *)((void *)(&norma)))) {
            printf("norma calculate result wrong\n");
            exit(-1);
        }
        t1 = second();
