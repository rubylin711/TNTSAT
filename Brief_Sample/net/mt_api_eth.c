...
/******************* global variable   ******************/
const MT_CHAR eth_name_array[2][ETH_NAME_LEN] =
{
    "eth0",
    "eth1"
};

static MT_S32 g_s32ETHFd[2] = {0, 0};

/******************* fuction declaration ****************/
static mt_u8 MT_SYS_WriteRegister(mt_u32 regs, mt_u32 value)
{
   printf("regs=%x, value=%x\n",regs,value);
   return 0;
}
static mt_u32 MT_SYS_ReadRegister(mt_u32 regs, mt_u32 *value)
{
    return 0;
}

static MT_CHAR* skip_space(MT_CHAR * line)
{
    MT_CHAR * p = line;
...
static MT_CHAR* skip_word(MT_CHAR* line)
{
    MT_CHAR * p = line;
...
static MT_S32 is_ip(MT_CHAR *ipaddr)
{
    MT_CHAR *pnum, *pdot = ipaddr;
    MT_S32 i = 0;
...
static MT_S32 parse_mac(MT_CHAR *dst, const MT_CHAR *src)
{
    const MT_CHAR *p = src;
    MT_CHAR q[3];
    MT_S32 colon_cnt = 0;
    ...
    while ('\0' != *p)
    {
        ...
        if (':' == *p)
        {
            ...
            q[i] = '\0';
            dst[colon_cnt++] = strtoul(q, NULL, 16);
            i = 0;
        }
        else
        {
...
/* sprintf mac addr */
static MT_CHAR *pr_ether(MT_CHAR *ptr)
{
    static MT_CHAR buff[32];
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_Open(Eth_Port_E ePort)
...
MT_S32 MT_ETH_Open(Eth_Port_E ePort)
{
    ...
    if (ETH_PORT_MAX <= ePort)
    {
        ...
        return MT_ERR_ETH_INVALID_PARA;
    }
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_Close(Eth_Port_E ePort)
Description     :switch to powerdown mode
Calls           :
Data Accessed   :
Data Updated    :
...
/*-----------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_IPAddressSet(Eth_Port_E ePort, MT_U8* ipAdd)
...
MT_S32 MT_ETH_IPAddressSet (Eth_Port_E ePort, MT_CHAR* ipAdd)
{
    ...
    if (NULL == ipAdd)
    {
        ...
        return MT_ERR_ETH_INVALID_POINT;
    }
...
/*-----------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_IPAddressGet (Eth_Port_E ePort, MT_S8* ipAdd)
...
/*--------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_SubNetmaskSet(Eth_Port_E ePort, MT_S8* subNetmask)
...
Return          :MT_SUCCESS
                :MT_FAILURE
                :MT_ERR_ETH_INVALID_POINT
                :MT_ERR_ETH_INVALID_PARA
Others          :
*--------------------------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_SubNetmaskGet(Eth_Port_E ePort, MT_U8* subNetmask)
...
/*--------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_GatewaySet(Eth_Port_E ePort, MT_U8* gateway)
...
/*--------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_GatewayGet(Eth_Port_E ePort, MT_U8* gateway)
Description     :set gateway for ePort
Calls           :
Data Accessed   :
Data Updated    :
...
/*----------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_DNSSet (Eth_Port_E ePort, MT_BOOL enable, MT_S8* dns)
...
/*--------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_DNSGet (Eth_Port_E ePort, MT_U8* dns)
Description     :set dns for ePort
Calls           :
Data Accessed   :
Data Updated    :
...
/*-----------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_SetMac(Eth_Port_E ePort, MT_U8 *mac)
...
Output          :NA
Return          :MT_SUCCESS/MT_FAILURE
                :MT_ERR_ETH_INVALID_POINT/MT_ERR_ETH_INVALID_PARA
Others          :NA
 *------------------------------------------------------------------------------*/
MT_S32 MT_ETH_SetMac(Eth_Port_E ePort, MT_CHAR *mac)
{
    ...
    if(MT_SUCCESS != parse_mac(mac_real, mac))
    {
...
/*-----------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_GetMac(Eth_Port_E ePort, MT_U8 *mac)
...
/*------------------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_GetLinkStatus(Eth_Port_E ePort, PTR_Eth_LinkStatus_E ptrLinkStatus)
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_LinkCfg(Eth_Port_E ePort, Eth_LinkCfg_E eLinkForce)
...
MT_S32 MT_ETH_LinkCfg(Eth_Port_E ePort, Eth_LinkCfg_E eLinkCfg)
{
#if 0
    MT_CHAR *buf = NULL;
    MT_U32 MacPortSetArray[5] = {0x2, 0x3, 0x6, 0x7, 0x7};
    MT_U32 phySetArray[5] = {0x0, 0x100, 0x2000, 0x2100, 0x9000};
    MT_U32 phyAddress[2] = {0x1, 0x3};
    ...
    buf = (MT_CHAR *)malloc(WRITE_CMD_BUF_LEN);
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_FwCtrlSet(Eth_Forward_E eItem, MT_U32 u32Fw)
...
MT_S32 MT_ETH_FwCtrlSet(Eth_Forward_E eItem, MT_U32 u32Fw)
{
    ...
    {
        MT_SYS_WriteRegister(REG_SF_GLB_FWCTRL,u32Fw);
    }
    else if(u32Fw <= 1)
    {
        MT_SYS_ReadRegister(REG_SF_GLB_FWCTRL, &u32RegVal);
        //sf_write_bit(u32RegVal,u8BitShift[eItem],u32Fw);
        MT_SYS_WriteRegister(REG_SF_GLB_FWCTRL, u32RegVal);
    }
    else
    {
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_MactCtrlSet(Eth_Mactctrl_E eItem, MT_U32 u32Macctrl)
...
MT_S32 MT_ETH_MactCtrlSet(Eth_Mactctrl_E eItem, MT_U32 u32Macctrl)
{
    ...
    {
        MT_SYS_WriteRegister(REG_SF_GLB_MACTCTRL,u32Macctrl);
    }
    else if(u32Macctrl <= 1)
    {
        MT_SYS_ReadRegister(REG_SF_GLB_MACTCTRL, &u32RegVal);
        //sf_write_bit(u32RegVal,u8MactBitShift[eItem],u32Macctrl);
        MT_SYS_WriteRegister(REG_SF_GLB_MACTCTRL, u32RegVal);
    }
    else
    {
        API_DEBUG("invalid parameter:%d,%d\n", eItem, u32Macctrl);
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_MacTableEnable(Eth_Port_E ePort)
...
MT_S32 MT_ETH_MacTableEnable(Eth_Port_E ePort)
{
    ...
    MT_SYS_ReadRegister(REG_SF_GLB_MACTCTRL,&u32RegValue);
    //sf_write_bit(u32RegValue, BIT_MACT_ENA_UP_SHIFT, 1)
    MT_SYS_WriteRegister(REG_SF_GLB_MACTCTRL,u32RegValue);
    return MT_SUCCESS;
}

/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_MacTableDisable(Eth_Port_E ePort)
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_MacTableSet(Eth_MacTblIdx_E eMacTblIndex, MT_U8 *u8Mac)
...
{
    MT_U32 u32RegAddrH16,u32RegAddrL32;
    MT_U32 u32RegValue;
    ...
    /* L32 */
    MT_SYS_WriteRegister(u32RegAddrL32,u32MacLo32);
    /* CTRL_BITS */
    MT_SYS_ReadRegister(u32RegAddrH16,&u32RegValue);
...
//    sf_write_bit(u32RegValue, BIT_FW2CPU_UP_SHIFT, (u32MacHi16 >> BIT_FW2CPU_UP_SHIFT) & 0x1);
    ...
    MT_SYS_WriteRegister(u32RegAddrH16,u32RegValue);

    return MT_SUCCESS;
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_VlanEnable(MT_VOID)
...
{
    MT_U32 u32RegValue = 0;
    MT_SYS_ReadRegister(REG_SF_GLB_FWCTRL,&u32RegValue);
    //sf_write_bit(u32RegValue,BIT_VLAN_ENA_SHIFT,1);
    MT_SYS_WriteRegister(REG_SF_GLB_FWCTRL,u32RegValue);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_VlanDisable(MT_VOID)
...
/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_VlanTableSet(Eth_VlanTblIdx_E eVlanTblIndex, MT_U32 u32Vlan)
...
MT_S32 MT_ETH_VlanTableSet(Eth_VlanTblIdx_E eVlanTblIndex, MT_U32 u32Vlan)
{
    ...
    return MT_SYS_WriteRegister(u32RegAddr,u32Vlan);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_CpuVlanCtrlSet(MT_U32 u32CpuVlanCtrl)
...
{
    MT_SYS_WriteRegister(REG_SF_GLB_CPU_VLANCTRL,u32CpuVlanCtrl);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_NoVlanCtrlSet(MT_U32 u32NoVlanCtrl)
...
{
    MT_SYS_WriteRegister(REG_SF_GLB_NOVLANCTRL,u32NoVlanCtrl);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_SpecVlanCtrlSet(MT_U32 u32SpecVlanCtrl)
...
{
    MT_SYS_WriteRegister(REG_SF_GLB_SPEVLANCTRL,u32SpecVlanCtrl);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_UnkownVlanSet(MT_U32 u32UnkownVlanCtrl)
...
{
    MT_SYS_WriteRegister(REG_SF_GLB_UNKNOWN_VLAN,u32UnkownVlanCtrl);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_VOID MT_ETH_PortVlanIdSet(MT_U32 u32PortVlanId)
...
{
    MT_SYS_WriteRegister(REG_SF_GLB_PORT_VLANID,u32PortVlanId);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_FcTimeCtrl(Eth_Port_E ePort, MT_U32 u32FcTimeCtrl)
...
MT_S32 MT_ETH_FcTimeCtrl(Eth_Port_E ePort, MT_U32 u32FcTimeCtrl)
{
    ...
    u32RegAddr = (ETH_PORT_ETH0 == ePort)?(REG_SF_U_GLB_FC_TIMECTRL):(REG_SF_D_GLB_FC_TIMECTRL);
    return MT_SYS_WriteRegister(u32RegAddr,u32FcTimeCtrl);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_FcRxLimit(Eth_Port_E ePort, MT_U32 u32FcRxLimit)
...
MT_S32 MT_ETH_FcRxLimit(Eth_Port_E ePort, MT_U32 u32FcRxLimit)
{
    ...
    u32RegAddr = (ETH_PORT_ETH0 == ePort)?(REG_SF_U_GLB_FC_RXLIMIT):(REG_SF_D_GLB_FC_RXLIMIT);
    return MT_SYS_WriteRegister(u32RegAddr,u32FcRxLimit);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_FcDropCtrl(Eth_Port_E ePort, MT_U32 u32FcDropCtrl)
...
MT_S32 MT_ETH_FcDropCtrl(Eth_Port_E ePort, MT_U32 u32FcDropCtrl)
{
    ...
    u32RegAddr = (ETH_PORT_ETH0 == ePort)?(REG_SF_U_GLB_FC_DROPCTRL):(REG_SF_D_GLB_FC_DROPCTRL);
    return MT_SYS_WriteRegister(u32RegAddr,u32FcDropCtrl);
}

/*----------------------------------------------------------------------------------------*
Function        :MT_S32 MT_ETH_FcLevel(Eth_Port_E ePort, MT_U32 u32FcLevel)
Description     :set flow ctrl drop ctrl
Calls           :
Data Accessed   :
Data Updated    :
...
MT_S32 MT_ETH_FcLevel(Eth_Port_E ePort, MT_U32 u32FcLevel)
{
    ...
    u32RegAddr = (ETH_PORT_ETH0 == ePort)?(REG_SF_U_GLB_FC_LEVEL):(REG_SF_D_GLB_FC_LEVEL);
    return MT_SYS_WriteRegister(u32RegAddr,u32FcLevel);
}
