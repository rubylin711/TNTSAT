...
/***************************** Macro Definition ******************************/
#ifdef MT_SAMPLE_SPLIT_SEARCH_DEBUG

#define MT_SPLIT_SEARCH_PRINT   printf
#else

#define MT_SPLIT_SEARCH_PRINT

#endif

#define SAMPLE_SPLIT_SEARCH_FUNCTION_ENTER()    MT_SPLIT_SEARCH_PRINT("[%s]: Enter ==>> \n", __FUNCTION__)
#define SAMPLE_SPLIT_SEARCH_FUNCTION_EXIT()     MT_SPLIT_SEARCH_PRINT("[%s]: Exit ==<< \n", __FUNCTION__)



#define SAMPLE_SPLIT_SEARCH_FATAL_PRINT(fmt...)         MT_SPLIT_SEARCH_PRINT(" [FATAL] " fmt)
#define SAMPLE_SPLIT_SEARCH_ERR_PRINT(fmt...)           MT_SPLIT_SEARCH_PRINT(" [ERROR] " fmt)
#define SAMPLE_SPLIT_SEARCH_WARN_PRINT(fmt...)          MT_SPLIT_SEARCH_PRINT(" [WARN] "  fmt)
#define SAMPLE_SPLIT_SEARCH_INFO_PRINT(fmt...)          MT_SPLIT_SEARCH_PRINT(" [INFO] "  fmt)
#define SAMPLE_SPLIT_SEARCH_DBG_PRINT(fmt...)           MT_SPLIT_SEARCH_PRINT(" [DEBUG] " fmt)


#define  SAMPLE_SPLIT_SEARCH_PRINT  printf
...
typedef struct
{
    ...
    mt_save_info_t save_info[MAX_TP_NUM];
    mt_input_Blindscan_para_t sInputParam;
}mt_TP_info_para_t;

typedef struct
{
    mt_tpinfo_para_t tppara;
    mt_s32 tuner_id;
    mt_u32 onoff_22k;
} mt_split_search_tp_info_s;
...
static mt_s32 MT_Split_SearchCheckParam(mt_input_Blindscan_para_t *p_BlindScan_in)
{
    if(p_BlindScan_in->startfreq >= p_BlindScan_in->endfreq)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("Frequency setting error\n");
        return MT_FAILURE;
    }
    return MT_SUCCESS;
}
...
@*/
static MT_S32 MT_Split_SearchDmxInit(mt_u32 tuner_id)
{
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_UNF_DMX_AttachTSPort failed-------<%s> line: %d\n", __FUNCTION__, __LINE__);
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_DMX_Init failed.\n");
        return ret;
    }
    ...
    if(MT_CHIP_SYMPHONY2_A0  <= stSysChipInfo.enChipVersion)
    {
        ...
        if(tuner_id == 0)
        {
            ...
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("Connect port 0!\n");
        }
        else if(tuner_id == 1)
        {
            ...
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("Connect port 3!\n");
#else
            /** Bind Demux to tuner port 1 */
            ret = MT_UNF_DMX_AttachTSPort(DMX_ID_0, MT_UNF_DMX_PORT_TSI_1);
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("Connect port 1!\n");
#endif
        }
        else
        {
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_DMX_AttachTSPort failed.\n");
        MT_UNF_DMX_DeInit();
        return ret;
    }
...
/*
 ...
 @return void
*/
static void MT_Split_SearchDmxDeInit(MT_VOID)
{
...
static MT_S32 MT_Split_Search_Notify(MT_U32 tuner_id, mt_unf_fe_blindscan_evt_t enEVT, mt_unf_fe_blindscan_notify_t *punNotify)
{
  mt_s32 i = 0;
  mt_s32 num = 0;
  ...
  switch (enEVT)
  {
    case MT_UNF_FE_BLINDSCAN_EVT_STATUS:
        ...
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("Scan fail.\n");
            return MT_FAILURE;
        }
        else if ((MT_UNF_FE_BLINDSCAN_STATUS_FINISH == *(punNotify->status)) || (MT_UNF_FE_BLINDSCAN_STATUS_QUIT == *(punNotify->status)))
        {
            SAMPLE_SPLIT_SEARCH_PRINT("100%%");
            SAMPLE_SPLIT_SEARCH_PRINT("done\n");
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("Scan over, find %d TP.\n", g_sTPinfo.s32TPNum);
...
@return MT_FAILURE
*/
static mt_s32 MT_Split_Search_Set_Parameter(MT_U32 tuner_id, MT_U32 sig_type, MT_U32 tuner_dev_type, MT_U32 tuner_addr,
             MT_U32 demod_dev_type, MT_U32 demod_addr, MT_U32 out_put_mode, MT_U32 I2c_channel)
{
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("mt_sys_get_version err!\n");
        return ret;
    }

    SAMPLE_SPLIT_SEARCH_INFO_PRINT("MTCommand_Tuner_Operation:   chipVersion = 0x%x\n", stSysChipInfo.enChipVersion);

    ret = mt_unf_fe_get_default_attr(tuner_id,&mtTunerAttr);
    if(SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("mt_unf_fe_get_default_attr err!\n");
        return ret;
    }
    ...
    {
        SAMPLE_SPLIT_SEARCH_INFO_PRINT("[%s %d]tuner_dev_type = %d, tuner_addr = 0x%x\n", __FUNCTION__, __LINE__, tuner_dev_type, tuner_addr);
        mtTunerAttr.tuner_type = tuner_dev_type;
        mtTunerAttr.tuner_addr = tuner_addr;
    }
#ifdef CONFIG_MT_CHIP_SYMPHONY6
    ...
    if (MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("mt_unf_fe_set_attr failed.ret is %d\n",ret);
        return ret;
    }

    SAMPLE_SPLIT_SEARCH_INFO_PRINT("output_mode = %d  I2c_channel=%d \n", out_put_mode, I2c_channel);

    return MT_SUCCESS;
}


static MT_S32 MT_Split_Search_Blindscan(mt_u32 tuner_id, MT_U8 bs_mode, MT_U32 u32StartFreq, MT_U32 u32StopFreq, MT_U8 polar, MT_U8 onoff_22k)
{
    MT_S32 ret = 0;
...
#ifdef MT_SYM4_DSS
    ret = MT_Split_Search_Set_Parameter(tuner_id, MT_UNF_FE_SIG_TYPE_DVBS_AUTO, MT_UNF_TUNER_TYPE_M88TS6011, 0x58, MT_UNF_DEMOD_DEV_TYPE_M88DS6113, 0xD2, 1, 1);
#else
    ret = MT_Split_Search_Set_Parameter(tuner_id, 2048, 34, 88, 288, 24, 1, 0);
#endif
#elif defined CONFIG_MT_CHIP_SYMPHONY6
    if(tuner_id == 1)
    {
        ret = MT_Split_Search_Set_Parameter(tuner_id, 2048, 80, 90, 336, 210, 4, 1);
    }
    else
    {
        ret = MT_Split_Search_Set_Parameter(tuner_id, 2048, 34, 88, 288, 24, 4, 0);
    }
#endif
    ...
    if (0 == bs_mode)
    {
        ...
        stBlindScanPara.scan_para.sat.scan_notify = (mt_void(*)(mt_u32, mt_unf_fe_blindscan_evt_t, void *))MT_Split_Search_Notify;
    }
    else
    {
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call mt_unf_fe_blindscan_start failed.\n");
        return MT_FAILURE;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_S32 MT_Split_SearchAvplayInit(mt_handle *phAvplay, mt_handle *phWin, mt_handle *phSoundTrack)
{
    MT_S32                   ret = MT_FAILURE;
    mt_handle                hAvplay = 0;
    mt_handle                hWin = 0;
    ...
    SAMPLE_SPLIT_SEARCH_FUNCTION_ENTER();

    if(NULL == phAvplay)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("phAvplay is null.\n");
        return MT_FAILURE;

    }
    if(NULL == phWin)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("phWin is null.\n");
        return MT_FAILURE;

    }
    if(NULL == phSoundTrack)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("phSoundTrack is null.\n");
        return MT_FAILURE;
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MTADP_AVPlay_RegADecLib failed.\n");
        return ret;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_AVPLAY_Init failed.\n");
        return ret;
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_AVPLAY_GetDefaultConfig failed.\n");
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_AVPLAY_Create failed.\n");
        goto ERR1;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_AVPLAY_ChnOpen failed.\n");
        goto ERR2 ;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_SND_GetDefaultTrackAttr failed.\n");
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_SND_CreateTrack failed.\n");
        goto ERR4;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_SND_Attach failed.\n");
        goto ERR5;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_VO_CreatWin error\n");
        goto ERR6;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_VO_AttachWindow failed.\n");
        goto ERR7;
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call MT_UNF_VO_SetWindowEnable failed.\n");
        goto ERR8;
    }
    ...
    *phSoundTrack = hSoundTrack;
    ...
    SAMPLE_SPLIT_SEARCH_FUNCTION_EXIT();
...
*/
static void  MT_Split_SearchAvplayDeInit(mt_handle hAvplay, mt_handle hWin, mt_handle hSoundTrack)
{
...
/*
 ...
 @return ::MT_FAILURE
*/
static MT_S32 MT_Split_SearchSetAvplayPidAndCodecType(mt_handle hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    MT_S32                           ret = MT_FAILURE;
    MT_U32                           u32AudType = 0;
    MT_U32                           VidPid = 0;
    ...
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("=====hAvplay is INVALID_HANDLE ======\n");
        return ret;
    }
    if(NULL == pProgInfo)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("=====pProgInfo == NULL=====\n");
        return ret;
    }
    ...
    SAMPLE_SPLIT_SEARCH_INFO_PRINT("VidPid=%x, Vidtype=0x%x, AudPid=%x, AudType=0x%x \n", VidPid, enVidType, AudPid, u32AudType);
    ...
    if(VidPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT(" MT_UNF_AVPLAY_GetAttr failed.\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT(" MT_UNF_AVPLAY_SetAttr failed.\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("Set video properties or video PID property failed.\n");
            return ret;
        }
    }
    ...
    if(AudPid != INVALID_TSPID)
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_AVPlay_SetAdecAttr failed:%#x\n",ret);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("Setting the decoding mode or audio PID property failed:%#x\n",ret);
            return ret;
        }
    }
    ...
    if((VidPid != INVALID_TSPID) || (AudPid != INVALID_TSPID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_UNF_AVPLAY_SetAttr failed:%#x\n", ret);
            return ret;
        }
    }
...
@return::ret                    The return value of the error.
@*/
static MT_S32 MT_Split_SearchStarToPlay(mt_handle hAvplay, const PMT_COMPACT_PROG *pProgInfo)
{
    ...
    if(NULL == pProgInfo)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("p_ProgInfo is NULL!\n");
        return MT_FAILURE;
    }
    if(MT_INVALID_HANDLE == hAvplay)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("hAvplay is not exist\n");
        return ret;
    }

    /** Set the PID of the AV player and set the encoder type */
    ret = MT_Split_SearchSetAvplayPidAndCodecType(hAvplay, pProgInfo);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchSetAvplayPidAndCodecType fail! \n");
        return ret;
    }
    ...
    {
        SAMPLE_SPLIT_SEARCH_INFO_PRINT("Has no audio stream!\n");
    }
    ...
    {
        SAMPLE_SPLIT_SEARCH_INFO_PRINT("Has no video stream!\n");
    }
    ...
    if((enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_AUD) && (enMediaType & MT_UNF_AVPLAY_MEDIA_CHAN_VID))
    {
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("Set frame to VO fail.\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("Get avplay sync attr fail!\n");
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("Set avplay sync attr fail!\n");
            return ret;
        }
    }
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_UNF_AVPLAY_Start fail!  ret=0x%x \n", ret);
        return ret;
    }
...
@return::ret                    The return value of the error.
@*/
static MT_S32 MT_Split_SearchStopToPlay(mt_handle hAvplay)
{
    ...
    SAMPLE_SPLIT_SEARCH_INFO_PRINT("stop live play ...\n");
...
static MT_VOID MT_Split_SearchPrintMenu(MT_U32 prog_num)
{

    SAMPLE_SPLIT_SEARCH_PRINT("\n 1 - %d : select the program \n", prog_num);
    SAMPLE_SPLIT_SEARCH_PRINT("     p : print all tp program info \n");
    SAMPLE_SPLIT_SEARCH_PRINT("     e : print tp list \n");
    SAMPLE_SPLIT_SEARCH_PRINT("     f : full scan \n");
    SAMPLE_SPLIT_SEARCH_PRINT("     w : turn on/off lnb power \n");
    SAMPLE_SPLIT_SEARCH_PRINT("     k : set unblank screen mode\n");
    SAMPLE_SPLIT_SEARCH_PRINT("     g : get first video frame show and avsync done cost time \n");
#ifdef MT_SAMPLE_APP
    SAMPLE_SPLIT_SEARCH_PRINT("     b : background run \n");
#endif
    SAMPLE_SPLIT_SEARCH_PRINT("     h : help \n");
    SAMPLE_SPLIT_SEARCH_PRINT("     q : quit \n");
    SAMPLE_SPLIT_SEARCH_PRINT("Split_search>> ");

}

static MT_VOID MT_Split_SearchPrintTpList(MT_VOID)
{
    MT_S32 i = 0;
    MT_CHAR dvb_type[g_sTPinfo.tpnum][10];
    for(i = 0;i < g_sTPinfo.tpnum; i++)
    {
        switch(g_sTPinfo.save_info[i].dvb_type)
        {
            case 0:
    ...
    SAMPLE_SPLIT_SEARCH_PRINT("<Index>          <Freq>           <Sym Rate>         <Type> \n");
    for(i = 0;i < g_sTPinfo.tpnum; i++)
    {
        SAMPLE_SPLIT_SEARCH_PRINT(" %3d         %10dKHz     %8dKSs           %5s\n", i, g_sTPinfo.save_info[i].freq, g_sTPinfo.save_info[i].symbol_rate, dvb_type[i]);
    }
}


static MT_VOID MT_Split_SearchPrintProgramInfo( mt_tpinfo_para_t tppara[32])
{
    MT_S32  i = 0;
    MT_S32  j = 0;
    MT_S32  k = 0;
    ...
    {
        SAMPLE_SPLIT_SEARCH_PRINT("<Index>          <Freq>           <Sym Rate>         <Type>    <polar>\n");
        SAMPLE_SPLIT_SEARCH_PRINT(" %3d         %10dKHz     %8dKSs           %5s    %4d\n", k+1, tppara[k].freq, tppara[k].symbol_rate, dvb_type[k], tppara[k].polar);
    for(i =0; i<tppara[k].program_num;i++)
    {
        SAMPLE_SPLIT_SEARCH_PRINT("Channel Num = %d, Program ServiceID = %d PMT PID = %x\n", i+1, tppara[k].proginfo[i].ProgID,tppara[k].proginfo[i].PmtPid);

         for (j = 0; j < tppara[k].proginfo[i].VElementNum;j++)
         {
             SAMPLE_SPLIT_SEARCH_PRINT("\tVideo Stream PID   = 0x%x\n",tppara[k].proginfo[i].VElementPid);
             switch (tppara[k].proginfo[i].VideoType)
             {
                case MT_UNF_VCODEC_TYPE_H264:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tVideo Stream Type H264\n");
                    break;
                case MT_UNF_VCODEC_TYPE_MPEG2:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tVideo Stream Type MP2\n");
                    break;
                case MT_UNF_VCODEC_TYPE_MPEG4:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tVideo Stream Type MP4\n");
                    break;
                case MT_UNF_VCODEC_TYPE_HEVC:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tVideo Stream Type HEVC\n");
                    break;
                default:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tVideo Stream Type error\n");
             }
         }
         for (j = 0; j < tppara[k].proginfo[i].AElementNum; j++)
         {
            SAMPLE_SPLIT_SEARCH_PRINT("\tAudio Stream PID   = 0x%x\n", tppara[k].proginfo[i].Audioinfo[j].u16AudioPid);

            switch (tppara[k].proginfo[i].Audioinfo[j].u32AudioEncType)
            {
                case HA_AUDIO_ID_MP3:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tAudio Stream Type MP3\n");
                    break;
                case HA_AUDIO_ID_AAC:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tAudio Stream Type AAC\n");
                    break;
                case HA_AUDIO_ID_DOLBY_PLUS:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tAudio Stream Type AC3\n");
                    break;
                case HA_AUDIO_ID_DTSHD:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tAudio Stream Type DTS\n");
                    break;
                default:
                    SAMPLE_SPLIT_SEARCH_PRINT("\tAudio Stream Type error\n");
             }
         }
         SAMPLE_SPLIT_SEARCH_PRINT("++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++\n");
    }
    ...
    for(i = 0; i < g_sTPinfo.s32TPNum; i++)
    {
        SAMPLE_SPLIT_SEARCH_PRINT("TP:%2d( %2d - %2d) freq:%4dKHZ symbol_rate:%5dKSs \n", i+1, n, tppara[i].program_num + n-1, tppara[i].freq, tppara[i].symbol_rate);
...
static MT_VOID MT_Split_Search_Set_Lnb(mt_u32 tuner_id, MT_U32 low_lo, MT_U32 high_lo)
{
    ...
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("call mt_unf_fe_set_lnb_config failed.\n");
    }
}


static MT_S32 MT_Split_Search_Connect_Dvbs(MT_U32 tuner_id, MT_U32 freq, MT_U32 sym_rate, MT_U32 onoff_22k, MT_U32 polar, MT_U32 u32LoopTimes)
{
      MT_S32 ret = MT_FAILURE;
      MT_U32 u32Loop = 0;
      MT_U32 u32Freq = 0;
    ...
    if (MT_SUCCESS == ret)
    {
        ...
        for (u32Loop = 0; u32Loop < u32LoopTimes; u32Loop++)
        {
            ...
            {
                SAMPLE_SPLIT_SEARCH_INFO_PRINT("Tuner Lock freq %d symb %d polar%d Success!\n", u32Freq, u32SymbolRate, polar);
                SAMPLE_SPLIT_SEARCH_INFO_PRINT("SUCCESS end\n");
                return MT_SUCCESS;
            }
            else
            {
    ...
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("Tuner Lock freq %d symb %d polar%d mt_unf_fe_connect Fail!, ret = 0x%x\n", u32Freq, u32SymbolRate, polar, ret);
    }

    if (u32Loop == u32LoopTimes)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("Tuner Lock freq %d symb %d  polar%d Fail!\n", u32Freq, u32SymbolRate, polar);
    }

    SAMPLE_SPLIT_SEARCH_ERR_PRINT("FAIL end\n");

    return MT_FAILURE;
}


static MT_S32 MT_Split_SearchPlay(MT_HANDLE     hAvplay, MT_U32 onoff_22k)
{
    MT_S32 ret = 0;
    MT_S32 j = 0;
    MT_S32 i = 0;
    ...
    (MT_VOID)MT_Split_SearchPrintTpList();
    printf("Please enter the number of TP you want to add 1 - %d \n", g_sTPinfo.tpnum);
    scanf("%d", &tp_num);
    (MT_VOID)MTADP_Search_Init();
    for(n = 0; n < tp_num; n++)
    {
        (MT_VOID)MT_Split_SearchPrintTpList();
        printf("Please enter the TP serial number you want to add \n");
        scanf("%d", &i);

        ret = MT_Split_Search_Connect_Dvbs(g_sTPinfo.sInputParam.tuner_id, (g_sTPinfo.save_info[i].freq), (g_sTPinfo.save_info[i].symbol_rate), onoff_22k, (g_sTPinfo.save_info[i].polar), 1000);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Fe_Connect_Dvbs failed.\n");
            continue;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Search_GetAllPmt failed.\n");
            continue;
        }
        ...
        for(j = 0; j<progTbl->prog_num; j++)
        {
            ...
            g_sTPinfo.tppara[k].freq = (g_sTPinfo.save_info[i].freq);
            g_sTPinfo.tppara[k].symbol_rate = g_sTPinfo.save_info[i].symbol_rate;
            g_sTPinfo.tppara[k].dvb_type = g_sTPinfo.save_info[i].dvb_type;
            g_sTPinfo.tppara[k].polar = g_sTPinfo.save_info[i].polar;
            ...
            for(idx = 0;idx < stCurrentProgInfo->AElementNum; idx++)
            {
                g_sTPinfo.tppara[k].proginfo[j].Audioinfo[idx].u16AudioPid = stCurrentProgInfo->Audioinfo[idx].u16AudioPid;
                g_sTPinfo.tppara[k].proginfo[j].Audioinfo[idx].u32AudioEncType = stCurrentProgInfo->Audioinfo[idx].u32AudioEncType;
            }
        }
    ...
    ret = MT_Split_Search_Connect_Dvbs(g_sTPinfo.sInputParam.tuner_id, g_sTPinfo.tppara[0].freq, (g_sTPinfo.tppara[0].symbol_rate), onoff_22k, g_sTPinfo.tppara[0].polar, 1000);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Fe_Connect_Dvbs failed.\n");
        return MT_FAILURE;
    }
    ret = MT_Split_SearchStarToPlay(hAvplay, (g_sTPinfo.tppara[0].proginfo));
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchStarToPlay failed.\n");
        return MT_FAILURE;
    }
...
@return::void
@*/
static void MT_Split_SearchPrint_help(char *name)
{
    SAMPLE_SPLIT_SEARCH_PRINT("Options:\n");
    SAMPLE_SPLIT_SEARCH_PRINT(" %s -t tuner_id -b mode -l low_lo -g high_lo -s start_freq -e stop_freq -k 22k -p polar\n", name);
    SAMPLE_SPLIT_SEARCH_PRINT(" %s -t 0 -b 1 -l 5150 -g 5150 -s 950 -e 2150 -k 1 -p 0 \n", name);
    SAMPLE_SPLIT_SEARCH_PRINT(" %s -t 0 -b 1 -l 5150 -g 5750 -s 950 -e 2150 -k 1 -p 0 \n", name);
    SAMPLE_SPLIT_SEARCH_PRINT(" %s -t 0 -b 1 -l 9750 -g 10600 -s 950 -e 2150 -k 1 -p 0 \n", name);
    SAMPLE_SPLIT_SEARCH_PRINT(" %s -t 0 -b 1 -l 11300 -g 11300 -s 950 -e 2150 -k 1 -p 0 \n", name);
    SAMPLE_SPLIT_SEARCH_PRINT(" 9750/10600 and 5150/5750 is Double local oscillator, 5150 and 11300 is Single local vibration \n");
#ifdef MT_SAMPLE_APP
    SAMPLE_SPLIT_SEARCH_PRINT(" %s -q  <exit> \n", name);
#endif

}

static MT_VOID MT_Split_searchExit(void)
{
    MT_UNF_VCODEC_UNBLANK_E unblank;

    (MT_VOID)MT_Split_SearchStopToPlay(g_sTPinfo.stSplitRunInfo.hAvPlay);

    (MT_VOID)MT_Split_SearchAvplayDeInit(g_sTPinfo.stSplitRunInfo.hAvPlay, g_sTPinfo.stSplitRunInfo.hWin, g_sTPinfo.stSplitRunInfo.hSoundTrack);

    (MT_VOID)MTADP_Search_DeInit();

    (MT_VOID)MT_Split_SearchDmxDeInit();
...
static void* MT_Split_ConnectTunerThread(void* arg)
{
    mt_s32 ret = 0;
    mt_split_search_tp_info_s *tp_info = (mt_split_search_tp_info_s*)arg;

    ret = MT_Split_Search_Connect_Dvbs(tp_info->tuner_id, tp_info->tppara.freq, tp_info->tppara.symbol_rate, tp_info->onoff_22k, tp_info->tppara.polar, 1000);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Fe_Connect_SPLIT_SEARCH failed.\n");
    }
...
static void MT_Split_SearchCmdTask(MT_HANDLE        hAvplay, mt_input_Blindscan_para_t *pInutParam)
{
    MT_S32 ret = 0;
    MT_S32 i = 0;
    MT_S32 progNum = 0;
    ...
    pthread_t thread;
    mt_split_search_tp_info_s *p_tpinfo;

    while(1)
    {
        (MT_VOID)MT_Split_SearchPrintMenu(g_sTPinfo.total_num);
        /* get inputCmd*/
        fgets((char *)(inputCmd), (sizeof(inputCmd) - 1), stdin);

        if('q' == inputCmd[0])
        {
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("prepare to exit!\n");
            g_bTaskQuit = MT_TRUE;
            if (MT_UNF_VCODEC_UNBLANK_STABLE != unblank)
            {
                MTADP_Set_VcodeUnblank(MT_UNF_VCODEC_UNBLANK_STABLE);
        ...
        {
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("split_search play in back!\n");
            break;
        }
#endif
        else if(inputCmd[0] > '0' && inputCmd[0] <= '9')
        {
            ...
            {
                SAMPLE_SPLIT_SEARCH_INFO_PRINT("The biggest num is %d \n", g_sTPinfo.total_num);
                continue;
            }

            if(g_sTPinfo.stSplitRunInfo.tpIndex != i)
            {
				/*fix issue32582 Using stack memory for thread data (unsafe),p_tpinfo will free in MT_Split_ConnectTunerThread*/
				p_tpinfo = malloc(sizeof(mt_split_search_tp_info_s));
				if (p_tpinfo == NULL)
    		    {
        			SAMPLE_SPLIT_SEARCH_ERR_PRINT("%s, err: no buffer \n", __FUNCTION__);
					break;
   				}
                memset(p_tpinfo, 0, sizeof(mt_split_search_tp_info_s));
                p_tpinfo->tppara =  g_sTPinfo.tppara[i];
                p_tpinfo->tuner_id = g_sTPinfo.sInputParam.tuner_id;
                p_tpinfo->onoff_22k = pInutParam->onoff_22k;
                ret = pthread_create(&thread, NULL, MT_Split_ConnectTunerThread, (void*)p_tpinfo);
                if(MT_SUCCESS != ret)
                {
                    SAMPLE_SPLIT_SEARCH_ERR_PRINT("call pthread_creates failed\n");
                }

                g_sTPinfo.stSplitRunInfo.tpIndex = i;
            }
            ret = MT_Split_SearchStopToPlay(hAvplay);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchStopToPlay failed.\n");
            }

            // restore ac4    attr info.
            MTADP_AUD_RestoreAc4PlayAttrInfo(hAvplay);
            ret = MT_Split_SearchStarToPlay(hAvplay, g_sTPinfo.tppara[i].proginfo + progNum - 1);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchStarToPlay failed.\n");
            }
#ifdef MT_SAMPLE_APP
            stCurrentProgInfo = g_sTPinfo.tppara[i].proginfo + progNum - 1;

            ret = MTADP_Set_Current_Info(stCurrentProgInfo);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_SPLIT_SEARCH_ERR_PRINT(" MTADP_Set_Current_Info failed.\n");
            }
#endif
        ...
        {
            (MT_VOID)MT_Split_SearchStopToPlay(hAvplay);
            memset(g_sTPinfo.tppara, 0, sizeof(mt_tpinfo_para_t) * MAX_TP_NUM);
            g_sTPinfo.total_num = 0;
            g_sTPinfo.s32TPNum = 0;
            (MT_VOID)MTADP_Search_DeInit();
            ret = MT_Split_SearchPlay(hAvplay, pInutParam->onoff_22k);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchPlay failed.\n");
            }

        }
        else if('p' == inputCmd[0])
        {
            (MT_VOID)MT_Split_SearchPrintProgramInfo(g_sTPinfo.tppara);
        }
        else if('w' == inputCmd[0])
        {
            SAMPLE_SPLIT_SEARCH_PRINT("Choice lnb power(0: power off 1: power open 2: power enhanced):\n");
            scanf("%d", &lnb_power);
            getchar();
            ...
            if(MT_SUCCESS != ret)
            {
                SAMPLE_SPLIT_SEARCH_ERR_PRINT("mt_unf_fe_set_lnb_power failed.\n");
            }
        }
        else if ('k' == inputCmd[0])
        {
            SAMPLE_SPLIT_SEARCH_PRINT("input unblank mode(0:fast 1:stable 2:sync):");
            scanf("%d", &unblank);
            getchar();
            SAMPLE_SPLIT_SEARCH_PRINT("unblank: %d \n", unblank);
        ...
        else if('g' == inputCmd[0])
        {
            ...
            SAMPLE_SPLIT_SEARCH_PRINT("[time] first video frame showed cost time: %lldms \n", first_vid_frm_show_time);
            SAMPLE_SPLIT_SEARCH_PRINT("[time] avsync done cost time: %lldms \n", avsync_done_time);
        }
        ...
        {
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("Print Help info \n");
        }
    }
}
...
@return::void
@*/
    static mt_s32 MT_Split_SearchParase_args(int argc, char *argv[], mt_input_Blindscan_para_t *pInutParam)
    {
        int opt = 0;
        ...
        while((opt = MTADP_Getopt(argc, argv, ":?hHb:l:g:t:e:p:k:s:q")) != -1)
        {
            switch(opt)
            {
                ...
                case '?':
                case 'H':
                    (void)MT_Split_SearchPrint_help(argv[0]);
                    return MT_FAILURE;
                case 'q':
                if(g_bTaskQuit == MT_FALSE)
                {
                    (MT_VOID)MT_Split_searchExit();
                }
                return MT_TASK_EXIT;
                case 't':
...

{
    ...
    {
        (MT_VOID)MT_Split_SearchPrint_help(argv[0]);
        return MT_SUCCESS;
    }
    ret = MT_Split_SearchParase_args(argc, argv, &g_sTPinfo.sInputParam);
    if (MT_FAILURE == ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("Parase args err. stop window.\n");

        return MT_FAILURE;
    }
    else if(MT_TASK_EXIT == ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("Recv stop command. stop window.\n");
    ...
    {
        ret = MT_Split_SearchCheckParam(&g_sTPinfo.sInputParam);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_SampleCheckSPLIT_SEARCHParam failed.\n");
            (MT_VOID)MT_Split_SearchPrint_help(argv[0]);
            return ret;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_SYS_Init failed.\n");
            return ret;
        }
#endif
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Fe_Init failed.\n");
            goto ERR1;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_HDMI_Init failed.\n");
            goto ERR2;
        }
        ...
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Disp_Init failed.\n");
            goto ERR3;
        ...
        if(MT_SUCCESS != ret)
        {
            ...
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_VO_Init failed.\n");
            goto ERR4;
        }
        ret = MTADP_Snd_Init();
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Snd_Init failed.\n");
            goto ERR5;
        }
        //11300/9750/10600
        (void)MT_Split_Search_Set_Lnb(g_sTPinfo.sInputParam.tuner_id, g_sTPinfo.sInputParam.low_lo, g_sTPinfo.sInputParam.high_lo);


        ret = MT_Split_Search_Blindscan(g_sTPinfo.sInputParam.tuner_id, g_sTPinfo.sInputParam.bs_mode, g_sTPinfo.sInputParam.startfreq, g_sTPinfo.sInputParam.endfreq, g_sTPinfo.sInputParam.polar, g_sTPinfo.sInputParam.onoff_22k);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_Search_Blindscan failed.\n");
            goto ERR6;
        }
        ...
        ret = MT_Split_SearchDmxInit(g_sTPinfo.sInputParam.tuner_id);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchDmxInit failed.\n");
            goto ERR6;
        }
        ...
        for(i = 0; i<g_sTPinfo.s32TPNum; i++)
        {
            ret = MT_Split_Search_Connect_Dvbs(g_sTPinfo.sInputParam.tuner_id,
                  (g_sTPinfo.satTPinfo[i].freq/1000) + ((g_sTPinfo.satTPinfo[i].freq%1000)>500?1:0),
                  (g_sTPinfo.satTPinfo[i].symbol_rate),
                   g_sTPinfo.sInputParam.onoff_22k,
                   g_sTPinfo.satTPinfo[i].polar, 1000);
            if(MT_SUCCESS != ret)
            {
                SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Fe_Connect_SPLIT_SEARCH failed.\n");
                continue;
            ...
            k++;
            SAMPLE_SPLIT_SEARCH_INFO_PRINT("k = %d \n", k);

            for(j = 0; j<progTbl->prog_num; j++)
            {
                stCurrentProgInfo = progTbl->proginfo + j;
                g_sTPinfo.tppara[k].freq = (g_sTPinfo.satTPinfo[i].freq/1000) + ((g_sTPinfo.satTPinfo[i].freq%1000)>500?1:0);
                g_sTPinfo.save_info[k].freq = (g_sTPinfo.satTPinfo[i].freq/1000) + ((g_sTPinfo.satTPinfo[i].freq%1000)>500?1:0);
                g_sTPinfo.tppara[k].symbol_rate = g_sTPinfo.satTPinfo[i].symbol_rate;
                g_sTPinfo.save_info[k].symbol_rate = g_sTPinfo.satTPinfo[i].symbol_rate;
                g_sTPinfo.tppara[k].dvb_type = g_sTPinfo.satTPinfo[i].dvb_type;
                g_sTPinfo.tppara[k].polar = g_sTPinfo.satTPinfo[i].polar;
                g_sTPinfo.save_info[k].polar = g_sTPinfo.satTPinfo[i].polar;
                g_sTPinfo.tppara[k].program_num = progTbl->prog_num;
                g_sTPinfo.tppara[k].proginfo[j].VElementNum = stCurrentProgInfo->VElementNum;
                g_sTPinfo.tppara[k].proginfo[j].AElementNum = stCurrentProgInfo->AElementNum;
                g_sTPinfo.tppara[k].proginfo[j].PcrPid = stCurrentProgInfo->PcrPid;
                ...
                g_sTPinfo.tppara[k].proginfo[j].PmtPid = stCurrentProgInfo->PmtPid;
                g_sTPinfo.tppara[k].proginfo[j].ProgID = stCurrentProgInfo->ProgID;
                for(int n = 0;n < stCurrentProgInfo->AElementNum; n++)
                {
        ...
        g_sTPinfo.s32TPNum = k + 1;
        g_sTPinfo.tpnum = g_sTPinfo.s32TPNum;
        ret = MT_Split_Search_Connect_Dvbs(g_sTPinfo.sInputParam.tuner_id, g_sTPinfo.tppara[0].freq, g_sTPinfo.tppara[0].symbol_rate, g_sTPinfo.sInputParam.onoff_22k, g_sTPinfo.tppara[0].polar, 1000);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MTADP_Fe_Connect_SPLIT_SEARCH failed.\n");
            goto ERR8;
        ...
        ret = MT_Split_SearchAvplayInit(&g_sTPinfo.stSplitRunInfo.hAvPlay, &g_sTPinfo.stSplitRunInfo.hWin, &g_sTPinfo.stSplitRunInfo.hSoundTrack);
        if(MT_SUCCESS != ret)
        {
            SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchAvplayInit failed.\n");
            goto ERR8;
        }
#ifdef MT_SAMPLE_APP
        avplayHandle.hAvPlay = g_sTPinfo.stSplitRunInfo.hAvPlay;
#endif

        g_sTPinfo.stSplitRunInfo.tpIndex = 0;
        ret = MT_Split_SearchStarToPlay(g_sTPinfo.stSplitRunInfo.hAvPlay, (g_sTPinfo.tppara[0].proginfo));
        if(MT_SUCCESS != ret)
        {
    ...
    (void)MT_Split_SearchCmdTask(g_sTPinfo.stSplitRunInfo.hAvPlay, &g_sTPinfo.sInputParam);
    if(g_bTaskQuit != MT_TRUE)
    {
        return MT_TASK_RUN;
    }
    ...
    ret = MT_Split_SearchStopToPlay(g_sTPinfo.stSplitRunInfo.hAvPlay);
    if(MT_SUCCESS != ret)
    {
        SAMPLE_SPLIT_SEARCH_ERR_PRINT("MT_Split_SearchStopToPlay failed, ret = %d\n", ret);
    }
