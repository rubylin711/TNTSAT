...
#else
	if((phoneme_tab_data = (unsigned char *)espeak_data_phontab) == NULL)
		return(-1);
	if((phoneme_index = (USHORT *)espeak_data_phonindex) == NULL)
		return(-1);
	if((phondata_ptr = espeak_data_phondata) == NULL)
		return(-1);
	if((tunes = (TUNE *)espeak_data_intonations) == NULL)
		return(-1);
    wavefile_data = (unsigned char *)phondata_ptr;
#endif
...
{//====================================================
	OS_PRINTF("Invalid instruction %.4x for phoneme '%s'\n", instn, WordToString(ph->mnemonic));
}


static unsigned char StressCondition(Translator *tr, PHONEME_LIST *plist, int condition, int control)
{//========================================================================================
// condition:
//	0	if diminished, 1 if unstressed, 2 if not stressed, 3 if stressed, 4 if max stress
	...
	else
	{
		...
		if(phoneme_tab[plist[1].phcode]->type == phVOWEL)
		{
			...
			return(0);  // no stress elevel for this consonant
	}
...
static unsigned char InterpretCondition(Translator *tr, int control, PHONEME_LIST *plist, USHORT *p_prog, WORD_PH_DATA *worddata)
{//========================================================================================================================
	int which;
	int ix;
	unsigned int data;
	...
	if(instn2 < 14)
	{
		...
		if(instn2 < 7)
		{
			...
			if((phoneme_tab[data]->mnemonic == ph->mnemonic) == 1)
				return(1);
		...
		case 0x80:
			...
			case 11:  // isFinalVowel
				for(;;)
				{
					...
					plist->ph = phoneme_tab[plist->phcode];
					if(plist->sourceix != 0)
						return(1);   // start of next word, without finding another vowel
					if(plist->ph->type == phVOWEL)
						return(0);
				}
				break;
			...
			case 15:  // isSeqFlag1
				...
				if(plist->sourceix != 0)
					return(0);   // this is the first phoneme in the word, so no.
...
//bit 8:  change phonemes
	...
	for(prog = &phoneme_index[ph->program]; end_flag != 1; prog++)
	{
		...
		case 1:
			...
			{
				// ChangeIf
				if(StressCondition(tr, plist, instn2 & 7, 1) == 1)
				{
		...
		case 3:
			...
			{
				// process a sequence of conditions, using  unsigned charean accumulator
