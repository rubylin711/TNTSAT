...
{//===============================================================
	...
	if(strstr(name , "en"))  //jace
	{
		tr->data_dictlist = espeak_data_en_dict;
		size = espeak_data_en_dict_len;
	}
#if 0
	if(strstr(name , "de"))
	{
		tr->data_dictlist = espeak_data_de_dict;
		size = espeak_data_de_dict_len;
	}
	if(strstr(name , "fr"))
	{
		tr->data_dictlist = espeak_data_fr_dict;
		size = espeak_data_fr_dict_len;
	}
#endif
	...
	{
		OS_PRINTF( "Full dictionary is not installed for '%s'\n", name);
	}
...
{//=====================================================================================================================================
	...
	if((option_phonemes == 2) && ((word_flags & FLAG_NO_TRACE)==0))
	{
		...
		for(ix=0;  ((ix < (sizeof(wordbuf)-1) && (c = p_start[ix]) != ' ') && (c != 0)  ); ix++)
		{
			wordbuf[ix] = c;
		}
		wordbuf[ix] = 0;
...
*/
{
	...
	{
		lookup_symbol = FLAG_LOOKUP_SYMBOL;
	}
	word1 = word;
...
{//===================================================
	...
	if(flags[0] & FLAG_TEXTMODE)
	{
		...
		text[0] = 0;
		strncpy0(&text[1], word1, sizeof(text)-1);//fix: maybe overflow here
