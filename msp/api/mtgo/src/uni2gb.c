...
static WORD *page_charset2uni[256] = {
...
static BYTE u2c_DC[512] = {
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, /* 0x00-0x03 */
};

static BYTE u2c_F9[512] = {
...
static BYTE *page_uni2charset[256] = {
...
WORD FT2_UnicodeToChar(WORD uniCode)
{
    WORD charCode;
    BYTE *uni2charset;
    BYTE cl = uniCode & 0xFF;
    BYTE ch = (uniCode >> 8) & 0xFF;

    uni2charset = page_uni2charset[ch];
    if (uni2charset) {
	charCode = (((WORD)uni2charset[cl * 2]) << 8) | uni2charset[cl * 2 + 1];
    } else {
	charCode = cl;
    }
...
{
    WORD uniCode = charCode;
    BYTE ch, cl;
    WORD *charset2uni;

    if (charCode & 0xFF00) {
	ch = (BYTE)(charCode >> 8);
	cl = (BYTE)(charCode & 0xFF);

	charset2uni = page_charset2uni[ch];

	if (charset2uni && cl) {
	    uniCode = charset2uni[cl];
	} else {
	    uniCode = ch;
	}
    }
