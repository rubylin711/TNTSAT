...
#ifndef _MT_API_TDE2_H_
#define _MT_API_TDE2_H_
...
#define SUPPORT_CMDFIFO  //dean debug

#define MT_TDE_Open MT_TDE2_Open
#define MT_TDE_Close MT_TDE2_Close
#define MT_TDE_BeginJob MT_TDE2_BeginJob
...
\retval ::MT_ERR_TDE_DEV_OPEN_FAILED Starting a TDE device fails. CNcomment:TDE设备打开失败CNend
\see \n
N/A
*/
...
\attention \n
MT_TDE2_Open must work with MT_TDE2_Close.\n
CNcomment:调用MT_TDE2_Open与MT_TDE2_Close的次数需要对应\n  CNend
\param  N/A
\retval N/A
\see \n
N/A
...
\retval ::MT_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败CNend
\retval ::MT_ERR_TDE_INVALID_HANDLE The handle of a TDE task is invalid. CNcomment:非法的TDE任务句柄CNend
\see \n
N/A
*/
...
\attention \n
Before calling this API, you must call MT_TDE2_Open to start a TDE device and call MT_TDE2_BeginJob to obtain a valid task handle.\n
...
After a task is submitted, its handle becomes invalid. If the task is submitted again, the error code MT_ERR_TDE_INVALID_HANDLE is returned. \n.
CNcomment:在调用此接口前应保证调用MT_TDE2_Open打开TDE设备，并且调用MT_TDE2_BeginJob获得了有效的任务句柄\n
若设置为阻塞操作，函数超时返回或被中断返回时应该注意：此时TDE操作的API函数提前返回，但执行的操作仍会完成\n
暂不支持同步模式\n
提交任务后，此任务对应的handle会变为无效，再次提交会出现错误码MT_ERR_TDE_INVALID_HANDLE \n  CNend
...
\retval ::MT_SUCCESS  A task is submitted successfully in non-block mode. CNcomment:非阻塞任务提交成功CNend
\retval ::MT_ERR_TDE_INVALID_HANDLE  The input task handle is invalid. CNcomment:输入的Job handler无效CNend
\retval ::MT_ERR_TDE_JOB_TIMEOUT  A task is not completed due to timeout. CNcomment:任务超时未完成CNend
\see \n
N/A
*/
...
\retval ::MT_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败CNend
\retval ::MT_FAILURE  The specified task cannot be cancelled because it has been submitted. CNcomment:指定的任务已经提交无法取消CNend
...
\retval ::MT_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败CNend
\retval ::MT_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄CNend
\retval ::MT_ERR_TDE_QUERY_TIMEOUT  The specified task is not completed due to timeout. CNcomment:指定的任务超时未完成CNend
\retval ::MT_ERR_TDE_UNSUPPORTED_OPERATION  The operation is not supported. CNcomment:不支持的操作CNend
\retval ::MT_ERR_TDE_INTERRUPT Blocked job was interrupted.CNcomment:阻塞的TDE任务被打断CNend
\see \n
N/A
*/
...
N/A
*/
mt_s32 MT_TDE2_WaitAllDone(mt_void);
...
\attention \n
MT_TDE2_QuickCopy adopts the direct memory access (DMA) transfer technology; therefore, MT_TDE2_QuickCopy is superior to MT_TDE2_Bitblit in transfer.
...
CNcomment:此接口使用的是直接DMA搬移，因此性能优于MT_TDE2_Bitblit搬移\n
...
\retval 0 success. CNcomment:成功CNend
\retval ::MT_ERR_TDE_DEV_NOT_OPEN  Calling this API fails because no TDE device is started. CNcomment:TDE设备未打开，API调用失败CNend
\retval ::MT_ERR_TDE_NULL_PTR  The parameter contains a null pointer. CNcomment:参数中有空指针错误CNend
\retval ::MT_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄CNend
\retval ::MT_ERR_TDE_INVALID_PARA  The parameter is invalid. CNcomment:无效的参数设置CNend
\retval ::MT_ERR_TDE_NO_MEM  Adding an operation fails due to insufficient memory. CNcomment:内存不足，无法添加操作CNend
\retval ::MT_FAILURE  A system error or an unknown error occurs.CNcomment:系统错误或未知错误CNend
\see \n
N/A
*/
...
\retval ::MT_ERR_TDE_INVALID_PARA  The parameter is invalid. CNcomment:无效的参数设置CNend
\retval ::MT_ERR_TDE_NO_MEM  Adding an operation fails due to insufficient memory. CNcomment:内存不足，无法添加操作CNend
\retval ::MT_ERR_TDE_NOT_ALIGNED  The start address or stride of the bitmap is not aligned based on pixel. CNcomment:位图起始地址或Stride没有按照像素对齐CNend
\retval ::MT_ERR_TDE_MINIFICATION  The start address or stride of the bitmap is not aligned based on pixel. CNcomment:缩小倍数过大CNend
\retval ::MT_ERR_TDE_NOT_ALIGNED  The start address of the color look-up table (CLUT) is not 4-byte aligned. CNcomment:Clut表的起始地址没有按照4byte对齐CNend
\retval ::MT_ERR_TDE_UNSUPPORTED_OPERATION  The operation is not supported. CNcomment:不支持的操作配置CNend
\retval ::MT_FAILURE  A system error or an unknown error occurs. CNcomment:系统错误或未知错误CNend
\see \n
N/A
*/
...
\retval ::MT_ERR_TDE_NOT_ALIGNED  The start address or stride of the bitmap is not aligned based on pixel. CNcomment:位图起始地址或Stride没有按照像素对齐CNend
\retval ::MT_ERR_TDE_UNSUPPORTED_OPERATION  The operation is not supported. CNcomment:不支持的操作配置CNend
\retval ::MT_ERR_TDE_MINIFICATION  The minification is too large. CNcomment:缩小倍数过大CNend
\retval ::MT_FAILURE  A system error or an unknown error occurs. 系统错误或未知错误CNend
\see \n
N/A
*/
...
\retval ::MT_ERR_TDE_INVALID_PARA  The parameter is invalid. CNcomment:无效的参数设置CNend
\retval ::MT_ERR_TDE_NO_MEM  Adding an operation fails due to insufficient memory. CNcomment:内存不足，无法添加操作CNend
\retval ::MT_ERR_TDE_NOT_ALIGNED  The start address of the CLUT is not 4-byte aligned. CNcomment:Clut表的起始地址没有按照4byte对齐CNend
\retval ::MT_ERR_TDE_MINIFICATION  The minification is too large. CNcomment:缩小倍数过大CNend
\retval ::MT_ERR_TDE_UNSUPPORTED_OPERATION  The bitmap does not support this operation. CNcomment:位图的格式操作不支持此操作CNend
\retval ::MT_ERR_TDE_CLIP_AREA  The operating area does not overlap the clipping area, and the displayed information is not updated. CNcomment:操作区域与Clip区域没有交集，显示不会有更新CNend
\retval ::MT_FAILURE  A system error or an unknown error occurs. CNcomment:系统错误或未知错误
\see \n
N/A
*/
...
\attention \n
Before calling this API, you must call MT_TDE2_Open to start a TDE device and call MT_TDE2_BeginJob to obtain a valid task handle.\n
When the foreground bitmap is null, the functions of this API are the same as those of MT_TDE2_QuickFil.\n
During solid drawing, the foreground bitmap supports the colorkey operation only.\n
To draw a rectangle, a vertical line, or a horizontal line by calling MT_TDE2_SolidDraw, you need to set the width and height of the filled rectangle. For example, drawing a vertical line is to draw a rectangle with the width of one pixel.\n
CNcomment:在调用此接口前应保证调用MT_TDE2_Open打开TDE设备，并且调用MT_TDE2_BeginJob获得了有效的任务句柄\n
当前景位图为NULL时，该接口和MT_TDE2_QuickFill实现的功能一样\n
...
\retval ::MT_ERR_TDE_INVALID_PARA  The parameter is invalid. CNcomment:无效的参数设置CNend
\retval ::MT_ERR_TDE_NO_MEM  Adding an operation fails due to insufficient memory. CNcomment:内存不足，无法添加操作CNend
\retval ::MT_ERR_TDE_MINIFICATION  The minification is too large because its maximum value is 255. CNcomment:缩小倍数超出限制（最大为缩小255倍）CNend
\retval ::MT_ERR_TDE_UNSUPPORTED_OPERATION  The bitmap does not support this operation. CNcomment:位图的格式操作不支持此操作CNend
\retval ::MT_ERR_TDE_CLIP_AREA  The operating area does not overlap the clipping area, and the displayed information is not updated. CNcomment:操作区域与Clip区域没有交集，显示不会有更新CNend
\retval ::MT_FAILURE  A system error or an unknown error occurs. CNcomment:系统错误或未知错误CNend
\see \n
N/A
*/
mt_s32      MT_TDE2_MbBlit(TDE_HANDLE s32Handle, TDE2_MB_S* pstMB, TDE2_RECT_S  *pstMbRect, TDE2_SURFACE_S* pstDst, TDE2_RECT_S  *pstDstRect,
...
\retval ::MT_ERR_TDE_NULL_PTR  The parameter contains a null pointer. CNcomment:参数中有空指针错误CNend
\retval ::MT_ERR_TDE_INVALID_HANDLE  The handle of a TDE task is invalid. CNcomment:非法的任务句柄CNend
\retval ::MT_ERR_TDE_INVALID_PARA  The parameter is invalid. CNcomment:无效的参数设置
\retval ::MT_ERR_TDE_NO_MEM  Adding an operation fails due to insufficient memory. CNcomment:内存不足，无法添加操作CNend
\retval ::MT_FAILURE  A system error or an unknown error occurs. CNcomment:系统错误或未知错误CNend
\see \n
N/A
...
N/A
*/
mt_s32 MT_TDE2_Rotate(TDE_HANDLE s32Handle, 
                    TDE2_SURFACE_S *pstSrc, TDE2_RECT_S *pstSrcRect, 
                    TDE2_SURFACE_S *pstDst, mt_s32 s32DstPointX,  mt_s32 s32DstPointY,
...
N/A
*/
mt_s32 MT_TDE2_PatternFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround, 
...
/**
\brief: tmp buffer used in gpe scale will get memory from the Mid_Mem created by api MT_TDE_Create_Mid_Mem.
if Mid_Mem is not created or not enough, tmp buffer will malloc by api  mt_mmz_malloc
如果使用了cmdfifo功能，且cmdfifo中含缩放的节点，那么该接口必须调用；
\param[in]: size of Mid_Mem 
\retval: 0 success
**/
mt_s32 MT_TDE_Create_Mid_Mem(mt_u32 mem_size);
/*!
释放MT_TDE_Create_Mid_Mem创建的连续内存块。
*/
mt_s32 MT_TDE_Destroy_Mid_Mem(mt_void);
#ifdef SUPPORT_CMDFIFO
/*!
标志着cmdfifo链表开始创建，后续的tde操作都会被加入到
cmdfifo链表中直到MT_TDE2_CmdFifo_CreateEnd被调用，返回创建的cmdfifo链表的句柄。 
*/
...
/*!
cmdfifo链表开始运行，运行结束后返回，硬件挂死时会超时退出。 
*/
...
/*!
在创建cmdfifo链表的过程中，获取当前已有的节点数，目前一个cmdfifo
最大支持MAX_NODE_NUM个节点，如果内存充足，可以加大该值。
*/
mt_u32 MT_TDE2_CmdFifo_getNodeCnt(mt_void);

/** 
\brief batch process of filling rec with specified value. CNcomment:批量处理矩形填充，指定范围，指定颜色CNend
\attention N/A
\param[in] s32Handle  Handle of a TDE task. CNcomment:任务句柄CNend
\param[in] pstDst  Target bitmap. CNcomment:目标位图信息CNend
\param[in] pstDstRect array of operating rectangle of the target bitmap. CNcomment:目标操作矩形数组CNend
\param[in] pFillData  value for filling rects. CNcomment:填充颜色数组CNend
\param[in] num number of  rects to be filled. CNcomment:待填充区域数量，pstDstRect 数组元素个数CNend
\param[out] ret_num number of  rects acture be filled. CNcomment:实际完成填充区域数量CNend
\retval 0 success. CNcomment:成功CNend
\retval ::
...
/** 
\brief batch process of blit operating. CNcomment:批量处理blit 操作CNend
\attention N/A
\param[in] s32Handle  Handle of a TDE task. CNcomment:任务句柄CNend
\param[in] pstBlitList  blit operating list. CNcomment:blit 操作列表CNend
\param[out] ret_num number of blit operation have been successfully finished. CNcomment:实际完成blit 操作个数CNend
\retval 0 success. CNcomment:成功CNend
\retval ::
