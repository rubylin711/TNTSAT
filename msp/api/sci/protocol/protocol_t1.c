...
static int32_t T1_Block_SendIBlock(struct s_reader *reader, uint8_t *block_data, unsigned char len, unsigned char *inf, unsigned char ns, int32_t more,                    uint32_t timeout)
{
	...
	block_data[len + 3] = T1_Block_LRC(block_data, (uint32_t)(len + 3));

	return ICC_Async_Transmit(reader, (uint32_t)length, 0, block_data, 0, timeout);
}
...
static int32_t T1_Block_SendRBlock(struct s_reader *reader, uint8_t *block_data, unsigned char type, unsigned char nr, uint32_t timeout)
{
	...
	block_data[0] = T1_BLOCK_NAD;
	block_data[1] = (uint8_t)(type | ((nr << 4) & 0x10));
	block_data[2] = 0x00;
...
static int32_t Protocol_T1_ReceiveBlock(struct s_reader *reader, uint8_t *block_data, uint32_t *block_length, uint8_t *rsp_type, uint32_t timeout)
{
	...
	else
	{
		...
		if(length != 0x00)
		{
			*block_length = (length + 4 > T1_BLOCK_MAX_SIZE) ? T1_BLOCK_MAX_SIZE : (uint32_t)(length + 4);
...
int32_t Protocol_T1_Command(struct s_reader *reader, unsigned char *command, uint16_t command_len, unsigned char *rsp, uint16_t *lr)
{
	...
	if(command[1] == T1_BLOCK_S_IFS_REQ)
	{
		...
		ret = T1_Block_SendSBlock(reader, block_data, T1_BLOCK_S_IFS_REQ, 1, &inf, (uint32_t)timeout);
		rdr_debug_mask(reader, D_IFD, "Protocol: Sending block S(IFS request, %d)\n", inf);
		...
		ret = Protocol_T1_ReceiveBlock(reader, block_data, &block_length, &rsp_type, (uint32_t)timeout);
		...
		if(ret == OK)
		{
			...
			{
				rdr_debug_mask(reader, D_IFD, "Protocol: Received block S(IFS response, %d)\n", block_data[3]);
			}
		}
	...
	else if(command[1] == T1_BLOCK_S_RESYNCH_REQ)
	{
		...
		timeout = ICC_Async_GetTimings(reader, reader->CWT);  // we are going to send: CWT timeout
		//cs_sleepus(reader->block_delay); // we were receiving, now sending so wait BGT time
		ret = T1_Block_SendSBlock(reader, block_data, T1_BLOCK_S_RESYNCH_REQ, 0, NULL, (uint32_t)timeout);
	...
	counter = 0;
	bytes = (uint8_t)MIN(command_len, reader->ifsc);
	...
	timeout = ICC_Async_GetTimings(reader, reader->CWT);  // we are going to send: CWT timeout
	//cs_sleepus(reader->block_delay); // we were receiving, now sending so wait BGT time
	ret = T1_Block_SendIBlock(reader, block_data, bytes, command, reader->ns, more, (uint32_t)timeout);
	rdr_debug_mask(reader, D_IFD, "Sending block I(%d,%d)\n", reader->ns, more);
	...
	while((ret == OK) && more)
	{
		...
		if(ret == OK)
		{
			...
			{
				rdr_debug_mask(reader, D_IFD, "Protocol: Received block R(%d)\n", T1_Block_GetNR(block_data));

				/* Increment ns  */
				reader->ns = (reader->ns == 1) ? 0 : 1; //toggle from 0 to 1 and back

				/* Calculate the number of bytes to send */
				counter += (uint16_t)bytes;
				bytes = (uint8_t)MIN(command_len - counter, reader->ifsc);
				...
				timeout = ICC_Async_GetTimings(reader, reader->CWT);  // we are going to send: CWT timeout
				//cs_sleepus(reader->block_delay); // we were receiving, now sending so wait BGT time
				ret = T1_Block_SendIBlock(reader, block_data, bytes, command + counter, reader->ns, more, (uint32_t)timeout);
				rdr_debug_mask(reader, D_IFD, "Protocol: Sending block I(%d,%d)\n", reader->ns, more);

			}
			else
			{
				rdr_debug_mask(reader, D_TRACE, "ERROR: T1 Command %02X not implemented\n", rsp_type);
				return ERROR;
			}
		}
		else
	...
	while((ret == OK) && more)
	{
		...
		if(ret == OK)
		{
			...
			{
				rdr_debug_mask(reader, D_IFD, "Protocol: Received block I(%d,%d)\n", T1_Block_GetNS(block_data), T1_Block_GetMore(block_data));

				bytes = T1_Block_GetLen(block_data);

				/* Calculate nr */
				nr = (uint8_t)((T1_Block_GetNS(block_data) + 1) % 2);
				...
				if(more)
				{
					...
					timeout = ICC_Async_GetTimings(reader, reader->CWT);  // we are going to send: CWT timeout
					//cs_sleepus(reader->block_delay); // we were receiving, now sending so wait BGT time
					ret = T1_Block_SendRBlock(reader, block_data, T1_BLOCK_R_OK, nr, (uint32_t)timeout);
					rdr_debug_mask(reader, D_IFD, "Protocol: Sending block R(%d)\n", nr);
				}
			}
			else if(rsp_type == T1_BLOCK_S_WTX_REQ)      /* WTX Request S-Block received */
			{
				/* Get wtx multiplier */
				wtx = block_data[3];
				rdr_debug_mask(reader, D_IFD, "Protocol: Received block S(WTX request, %d)\n", wtx);

				/* Send an WTX response S-Block */
				timeout = ICC_Async_GetTimings(reader, reader->CWT);  // we are going to send: CWT timeout
				//cs_sleepus(reader->block_delay); // we were receiving, now sending so wait BGT time
				ret = T1_Block_SendSBlock(reader, block_data, T1_BLOCK_S_WTX_RES, 1, &wtx, (uint32_t)timeout);
				rdr_debug_mask(reader, D_IFD, "Protocol: Sending block S(WTX response, %d)\n", wtx);
			}
			else
			{
				rdr_debug_mask(reader, D_TRACE, "ERROR: T1 Command %02X not implemented in Receive Block\n", rsp_type);
