...
int32_t ATR_InitFromArray(ATR *atr, const unsigned char atr_buffer[ATR_MAX_SIZE], uint32_t length)
{
	...
	if(length < 2)
	{
		cs_debug_mask(D_ATR, "ERROR: this ATR length is %d and minimum length is 2\n", length);
		return (ERROR);
	}
	...
	while(pointer < length)
	{
		...
		{
			cs_debug_mask(D_ATR, "ERROR: this ATR the %d interface bytes for protocol %d are missing\n", pointer + atr_num_ib_table[(0xF0 & TDi) >> 4], pn + 1);
			return (ERROR);
		}
		...
		if((TDi | 0x7F) == 0xFF)
		{
			...
			{
				cs_debug_mask(D_ATR, "ERROR: this ATR reports %d protocols but the maximum value is %d\n", pn + 1, ATR_MAX_PROTOCOLS + 1);
				return (ERROR);
			}
			pn++;
		}
	...
	{
		cs_debug_mask(D_ATR, "ERROR: this ATR reports %i historical bytes but there are only %i\n", atr->hbn, length - pointer - 2);
		if(length - pointer >= 2)
			{ atr->hbn = length - pointer - 2; }
		else
		{
	...
	{
		uchar FI;
		cs_debug_mask(D_ATR, "TA1 = %02x\n", atr->ib[pn][ATR_INTERFACE_BYTE_TA].value);
		FI = (atr->ib[pn][ATR_INTERFACE_BYTE_TA].value & 0xF0) >> 4;
		cs_debug_mask(D_ATR, "FI = %02x\n", FI);
		if(atr_fs_table[FI] == 0)
		{
			cs_debug_mask(D_ATR, "ERROR: this ATR FI for protocol #%d is not returning a valid cardfrequency value\n", pn + 1);
			return (ERROR);
		}
	}
	...
	if(atr->pn == 1 && atr->ib[pn][ATR_INTERFACE_BYTE_TB].present == 1)
	{
		...
		{
			cs_debug_mask(D_ATR, "ERROR: this ATR TB1 for protocol #%d has an invalid value\n", pn + 1);
			return (ERROR);
		}
	}
	return (ATR_OK);
...
int32_t ATR_GetConvention(ATR *atr, int32_t *convention)
{
	...
	{
		cs_debug_mask(D_ATR, "ERROR: this ATR TS byte is %02X and that should be 3B for direct or 3F for inverse convention!\n", atr->TS);
		return (ERROR);
	}
