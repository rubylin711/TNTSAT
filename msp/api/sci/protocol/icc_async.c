...
{
	rdr_debug_mask(reader, D_IFD, "Opening device %s\n", reader->device);
	//reader->written = 0;
	int32_t ret = reader->crdr.reader_init(reader);
	if(ret == OK)
		{ rdr_debug_mask(reader, D_IFD, "Device %s succesfully opened\n", reader->device); }
	else
	{
		//if(reader->typ != R_SC8in1) { NULLFREE(reader->crdr_data); }
		rdr_debug_mask(reader, D_IFD, "ERROR: Can't open %s device\n", reader->device);
	}
	return ret;
}
...
int32_t ICC_Async_Activate(struct s_reader *reader, ATR *atr, uint16_t deprecated)
{
	...
	rdr_log(reader, "ATR: %s\n", cs_hexdump(1, atrarr, atr_size, tmp, sizeof(tmp)));
#endif	
...
int32_t ICC_Async_CardWrite(struct s_reader *reader, unsigned char *command, uint16_t command_len, unsigned char *rsp, uint16_t *lr)
{
	...
	*lr = 0; //will be returned in case of error
	...
	do
	{
		if(try > 1)
		{
			rdr_log(reader, "Warning: needed try nr %i, next ECM has some delay\n", try);
		}
		...
		case ATR_PROTOCOL_TYPE_T14:
			...
			break;
		default:
			rdr_log(reader, "ERROR: Unknown protocol type %i\n", reader->protocol_type);
...
//		{ reader->crdr.unlock(reader); }
	...
	if(ret)
	{
		rdr_debug_mask(reader, D_TRACE, "ERROR: Protocol_T%d_Command returns error\n", type);
		return ERROR;
	}
...
int32_t ICC_Async_GetTimings(struct s_reader *reader, uint32_t wait_etu)
{
	...
	//rdr_debug_mask(reader, D_IFD, "Setting timeout to %i ETU (%d us)\n", wait_etu, timeout);
	return timeout;
}
...
int32_t ICC_Async_Transmit(struct s_reader *reader, uint32_t size, uint32_t expectedlen, unsigned char *data, uint32_t delay, uint32_t timeout)
{
	...
	if(expectedlen)  //expectedlen = 0 means expected len is unknown
		{ rdr_debug_mask(reader, D_IFD, "Transmit size %d bytes, expected len %d bytes, delay %d us, timeout=%d us\n", size, expectedlen, delay, timeout); }
	else
		{ rdr_debug_mask(reader, D_IFD, "Transmit size %d bytes, delay %d us, timeout=%d us\n", size, delay, timeout); }
...
{
	//rdr_debug_mask(reader, D_IFD, "Receive size %d bytes, delay %d us, timeout=%d us\n", size, delay, timeout);
...
{
	rdr_debug_mask(reader, D_IFD, "Closing device %s\n", reader->device);
	call(reader->crdr.close(reader));
//	if(reader->typ != R_SC8in1) { NULLFREE(reader->crdr_data); }
	rdr_debug_mask(reader, D_IFD, "Device %s succesfully closed\n", reader->device);
	return OK;
}
/*
...
static int32_t Parse_ATR(struct s_reader *reader, ATR *atr, uint16_t deprecated)
{
	...
	{
		FI = ATR_DEFAULT_FI;
		rdr_log(reader, "Warning: F=0 is invalid, forcing FI=%d\n", FI);
	}
	if(!D)
	{
		D = ATR_DEFAULT_D;
		rdr_log(reader, "Warning: D=0 is invalid, forcing D=%d\n", D);
	}
	rdr_log(reader, "Init card protocol T%i, FI=%d, F=%d, D=%d, N=%d\n", reader->protocol_type, FI, F, D, N);
	if(deprecated == 0)
...
//	}
	...
	*/
	...
	rdr_log(reader, "Calculated work ETU is %.2f us reader mhz = %u\n", reader->worketu, reader->mhz);
...
//		reader->worketu *= 2.1; else // increase the worketu for v2 and tripple zigo try out
	...
	if(reader->crdr.write_settings)
	{
		...
		call(reader->crdr.write_settings(reader, ETU, N));
	}
    /*
