	...
	*lr = tpdu_lr;
	...
	if(tpdu_rsp[tpdu_lr - 2] == 0x61)   /* Command processed, Lx indicated */
	{
		...
		while(Lm > 0)
		{
			...
			{
				rdr_log(reader, "TPDU Append error, new length %i exceeds max length %i\n", *lr + tpdu_lr, CTA_RES_LEN);
				return ERROR;
			}
...
static int32_t Protocol_T0_ExchangeTPDU(struct s_reader *reader, unsigned char *command, uint16_t command_len, unsigned char *rsp, uint16_t *lr)
{
	...
	*lr = 0; //in case of error this will be returned
	...
	case APDU_CASE_3S:
		...
		break;
	default:
		rdr_debug_mask(reader, D_TRACE, "ERROR: invalid cmd_case = %i in Protocol_T0_ExchangeTPDU\n", cmd_case);
		return ERROR;
	}
	...
	*/
	...
	while(recved < PROTOCOL_T0_MAX_SHORT_RESPONSE)
	{
		...
		if(buffer[recved] == 0x60)
		{
			...
			{
				rdr_debug_mask(reader, D_TRACE, "ERROR: %s: Maximum number of nulls reached: %d\n", __func__, nulls);
				return ERROR;
			}
		}
		else if((buffer[recved] & 0xF0) == 0x60 || (buffer[recved] & 0xF0) == 0x90)  /* SW1 byte received */
		{
			rdr_debug_mask(reader, D_TRACE, "SW1: %02X\n", buffer[recved] & 0xf0);
			recved++;
			if(recved >= PROTOCOL_T0_MAX_SHORT_RESPONSE)
			{
				rdr_debug_mask(reader, D_TRACE, "ERROR: %s: Maximum short response exceeded: %d\n", __func__, recved);
				return ERROR;
			}
			timeout = ICC_Async_GetTimings(reader, reader->read_timeout);  // we are going to receive: WWT timeout
			if(ICC_Async_Receive(reader, 1, buffer + recved, 0, timeout) != OK) { return ERROR; }   //Read SW2 byte
			rdr_debug_mask(reader, D_TRACE, "SW2: %02X\n", buffer[recved] & 0xf0);
			recved++;
			break;
		}
		else if((buffer[recved] & 0x0E) == (command[1] & 0x0E))  /* ACK byte received */
		{
			...
			if(cmd_case == APDU_CASE_2S)
			{
				if(sent >= Lc)
				{
					rdr_debug_mask(reader, D_TRACE, "ERROR: %s: ACK byte: sent=%d exceeds Lc=%d\n", __func__, sent, Lc);
					return ERROR;
				}
				timeout = ICC_Async_GetTimings(reader, reader->char_delay);  // we are going to send: char delay timeout
			...
			else /* Case 3 command: receive data */
			{
				if(recved > PROTOCOL_T0_MAX_SHORT_RESPONSE)
				{
					rdr_debug_mask(reader, D_TRACE, "ERROR: %s: Case 3 ACK - maximum short response exceeded: %d\n", __func__, recved);
					return ERROR;
				}
		...
		{
			rdr_debug_mask(reader, D_TRACE, "ERROR: %s: Received unexpected character: %02X\n", __func__, buffer[recved]);
			return ERROR;
		}
	}//while
...
int32_t Protocol_T14_ExchangeTPDU(struct s_reader *reader, unsigned char *cmd_raw, uint16_t command_len, unsigned char *rsp, uint16_t *lr)
{
	...
	*lr = 0; //in case of error this is returned
	...
	{
		rdr_debug_mask(reader, D_TRACE, "ERROR: invalid cmd_case = %i in Protocol_T14_ExchangeTPDU\n", cmd_case);
		return ERROR;
	}
...
//	if ((reader->typ == R_SMART) && (reader->smart_type >= 2)) timeout *= 1.1;
	...
	if(ixor1 != ixor)
	{
		rdr_debug_mask(reader, D_TRACE, "ERROR: invalid checksum = %02X expected %02X\n", ixor1, ixor);
		return ERROR;
	}
