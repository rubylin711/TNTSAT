...
2015/12/04		     		    Created file      	
******************************************************************************/

#ifndef __MT_JPEG_API_H__
#define __MT_JPEG_API_H__
   ...
   extern "C" 
{
	...
	typedef enum mtMT_JPEG_MEMCHECK_E
	{
	...
	typedef struct mtJPEG_SURFACE_S
	{
		 phys_addr_t pOutPhy[MAX_PIXEL_COMPONENT_NUM];        /**< output physic address,0 is luminance or rgb buf, 1 is chroma buf    *//**<CNcomment:输出物理地址，0分量输出y或者rgb地址，1分量为色度地址 CNend     */
		 mt_char* pOutVir[MAX_PIXEL_COMPONENT_NUM];        /**< output virtual address,0 is luminance or rgb buf, 1 is chroma buf   *//**<CNcomment:输出虚拟地址，0分量输出y或者rgb地址，1分量为色度地址 CNend     */
		 mt_u32   u32OutStride[MAX_PIXEL_COMPONENT_NUM];   /**< output stride                     *//**<CNcomment:输出行间距 CNend      */
         MT_BOOL  bUserPhyMem;                             /**< if use user physic memeory         *//**<CNcomment:是否使用用户的物理内存 CNend  */
	...
	typedef struct mtJPEG_SURFACE_DESCRIPTION_S
	{
		 MT_JPEG_SURFACE_S	 stOutSurface; /**< output  surface   *//**<CNcomment:输出信息 CNend       */
		 MT_JPEG_RECT_S      stCropRect;   /**< output crop size  *//**<CNcomment:输出裁剪区域 CNend   */
		 MT_BOOL bCrop;                    /**<  if crop           *//**<CNcomment:是否裁剪 CNend      */
	}MT_JPEG_SURFACE_DESCRIPTION_S;


	/** Structure of output argb sampler and filte message */
	/** CNcomment:设置输出argb采样和滤波相关信息 CNend */
    typedef struct mtJPEG_DEC_COEF_S
    {
        MT_BOOL bEnHorMedian;                                  /**< if set hor median filter           *//**<CNcomment:是否水平中值滤波 CNend        */
		MT_BOOL bEnVerMedian;                                  /**< if set ver median filter           *//**<CNcomment:是否垂直中值滤波 CNend        */
        MT_BOOL bSetHorSampleCoef;                             /**< if set hor sample coef            *//**<CNcomment:是否设置水平采样参数 CNend    */
		MT_BOOL bSetVerSampleCoef;                             /**< if set ver sample coef            *//**<CNcomment:是否设置垂直采样参数 CNend    */
		MT_BOOL bSetCSCCoef;                                   /**< if set csc coef                    *//**<CNcomment:是否设置CSC参数 CNend         */
		mt_s16  s16HorCoef[MAX_HORCOEF_ROW][MAX_HORCOEF_COL];  /**< set chrome hor sampler coefficient  *//**<CNcomment:色度上采样水平滤波系数 CNend */
		mt_s16  s16VerCoef[MAX_VERCOEF_ROW][MAX_VERCOEF_COL];  /**< set chrome ver sampler coefficient  *//**<CNcomment:色度上采样垂直滤波系数 CNend */
		mt_s16  s16CSCCoef[MAX_CSCCOEF_ROW][MAX_CSCCOEF_COL];  /**< set csc coefficient                 *//**<CNcomment:颜色空间转换系数 CNend       */
    }MT_JPEG_DEC_COEF_S;


	/** Structure of the input and output jpeg width and height,input jpeg format */
	/** CNcomment:jpeg文件的原始大小以及解码输出大小，原始图片格式 CNend */
	typedef struct mtJPEG_INFO_S
	{
	     mt_u32  u32Width[MAX_PIXEL_COMPONENT_NUM];     /**< 0 input and output rgb width and lu width,1 is ch width    *//**<CNcomment:0输入和解码输出rgb图片宽度或者解码yuvsp亮度宽度，1为色度宽度 CNend */
		 mt_u32  u32Height[MAX_PIXEL_COMPONENT_NUM];    /**< 0 input and output rgb height and lu width,1 is ch height  *//**<CNcomment:0输入和解码输出rgb图片高度或者解码yuvsp亮度高度，1为色度高度 CNend */
		 mt_u32  u32OutStride[MAX_PIXEL_COMPONENT_NUM]; /**< 0 output rgb stride and lu stride,1 is ch stride           *//**<CNcomment:0解码输出rgb或yubsp的亮度行间距，1为色度行间距离 CNend             */
		 mt_u32  u32OutSize[MAX_PIXEL_COMPONENT_NUM];   /**< 0 output rgb size and lu size,1 is ch size		         *//**<CNcomment:0解码输出rgb或yubsp的大小，1为色度大小 CNend            			  */
		 MT_JPEG_FMT_E enFmt;  /**< the input format  *//**<CNcomment:输入的图片格式 CNend      */
		 MT_BOOL bOutInfo;     /**< false is get input information and the other get output information   *//**<CNcomment:FALSE获取输入信息，TRUE获取输出信息 CNend */
	}MT_JPEG_INFO_S;


	/** Structure of the leave memsize that jpeg decode can used */
	/** CNcomment:jpeg解码可以使用的内存大小信息 CNend */
	typedef struct mtJPEG_MEMSIZE_INFO_S
	{
	     mt_u32  u32MemSize;      /**< the memsize  *//**<CNcomment:剩余内存大小 CNend */
		 mt_u32  u32Width;		   /**< limit width  *//**<CNcomment:限制宽 CNend */
		 mt_u32  u32Height;		   /**< limit height *//**<CNcomment:限制高 CNend */
		 MT_JPEG_MEMCHECK_E eCheckType; /**< limit type *//**<CNcomment:通过哪种类型来限制 CNend */
	}MT_JPEG_MEMSIZE_INFO_S;
	...
	::MT_JPEG_SetOutDesc
	*/
	mt_s32	MT_JPEG_SetOutDesc(const struct jpeg_decompress_struct *cinfo, const MT_JPEG_SURFACE_DESCRIPTION_S *pstSurfaceDesc);
