	...
	while (--num_rows >= 0)
	{
		  ...
		  for (col = 0; col < (mt_s32)num_cols; col++) 
		  {
			  ...
			  if(  ((col >= pJpegHandle->stOutDesc.stCropRect.x)
				 &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))|| (cinfo->p_jpeg_dbg_info->sw_dec))
#else
			  if(  (col >= pJpegHandle->stOutDesc.stCropRect.x)
			  ...
			  for (col = 0; col < (mt_s32)num_cols; col++) 
			  {
				   ...
				   if(	(col >= pJpegHandle->stOutDesc.stCropRect.x)
					   &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
					 {
							...
							*(INT16*)outptr = (INT16)PACK_SHORT_565(r,g,b);
						    outptr += 2;
         ...
         JSAMPARRAY output_buf, int num_rows)
{
	  ...
	  while (--num_rows >= 0)
	  {
			...
			if (PACK_NEED_ALIGNMENT(outptr)) 
			{
					...
					rgb = (INT32)PACK_SHORT_565(r,g,b);
					*(INT16*)outptr = rgb;
	 ...
	 while (--num_rows >= 0) 
	 {
		   ...
		   for (col = 0; col < (mt_s32)num_cols; col++)  
		   {
				   ...
				   &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				 {
						 ...
						 (*(INT16*)(outptr)) = (INT16)PACK_SHORT_1555(a,r,g,b);
	  ...
	  while (--num_rows >= 0) 
	  {
		    ...
		    for (col = 0; col < (mt_s32)num_cols; col++)  
			{
				   ...
				   &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
				 {
					  ...
					  (*(INT16*)(outptr)) = (INT16)PACK_SHORT_1555(a,r,r,r);
         ...
         JSAMPARRAY output_buf, int num_rows)
{
	  ...
	  while (--num_rows >= 0)
	  {
		    ...
		    for (col = 0; col < (mt_s32)(num_cols>>1); col++)
			{
		    ...
		    if (num_cols&1) 
			{
			      ...
			      g = GETJSAMPLE(*inptr1);
			      b = GETJSAMPLE(*inptr2);
			      *(INT16*)outptr = (INT32)PACK_SHORT_565(r,g,b);
          ...
          JSAMPARRAY output_buf, int num_rows)
{
	  ...
	  while (--num_rows >= 0) 
	  {
		    ...
		    unsigned int g = 0;
		    inptr = input_buf[0][input_row++];
		    g = (unsigned int)rgb;  //clean warning
			#if !defined(CONFIG_JPEG_ADD_GOOGLEFUNCTION) && !defined(CONFIG_JPEG_SUBSET_HARD_DEC_ENABLE)
			   /**局部解码，硬件不支持情况下，这些参数为0**/
			    if(  ((cinfo->output_scanline) < (mt_u32)pJpegHandle->stOutDesc.stCropRect.y) 
				   ||((cinfo->output_scanline+1) > (mt_u32)(pJpegHandle->stOutDesc.stCropRect.h + pJpegHandle->stOutDesc.stCropRect.y)))
			...
			for (col = 0; col < (mt_s32)num_cols; col++) 
			{
				...
				{
						 g = *inptr++;
			             rgb = (INT32)PACK_SHORT_565(g, g, g);
						 *(INT16*)outptr = (INT16)PACK_SHORT_565(g, g, g);
		                 outptr += 2;
				 }
	...
	while (--num_rows >= 0) 
	{
		  ...
		  for (col = 0; col < (mt_s32)num_cols; col++)	
		  {
				 ...
				 &&(col < (pJpegHandle->stOutDesc.stCropRect.w + pJpegHandle->stOutDesc.stCropRect.x)))
			   {
					...
					Red    =  MulDiv255Round((mt_u16)r,a);
				    Green  =  MulDiv255Round((mt_u16)g,a);
				    Blue   =  MulDiv255Round((mt_u16)b,a);
				  	Alpha  = 0XFF;

					if(JCS_ABGR_1555 == cinfo->out_color_space)
					{
					  (*(INT16*)(outptr)) = (INT16)PACK_SHORT_1555(Alpha,Red,Green,Blue);
					}
					else
					{
					  (*(INT16*)(outptr)) = (INT16)PACK_SHORT_1555(Alpha,Blue,Green,Red);
					}
					outptr += 2;
	 ...
	 while (--num_rows >= 0) 
	 {
		   ...
		   for (col = 0; col < (mt_s32)num_cols; col++)  
		   {
				...
				#endif
				{
					 ...
					 {
					 	*(INT16*)outptr = (INT16)PACK_SHORT_565(Red,Green,Blue);
					 }
					 else
					 {
                        *(INT16*)outptr = (INT16)PACK_SHORT_565(Blue,Green,Red);
					 }
					 outptr += 2;
				}
		   }
	 ...
	 while (--num_rows >= 0) 
	 {
		   ...
		   for (col = 0; col < (mt_s32)num_cols; col++)  
		   {
				...
				#endif
				{
					 ...
					 outptr[r]  =   MulDiv255Round((mt_u16)Red,Alpha);
					 outptr[g]  =   MulDiv255Round((mt_u16)Green,Alpha);
					 outptr[b]  =   MulDiv255Round((mt_u16)Blue,Alpha);
					 outptr += 3;
				}
		   }
	  }
		...
		while (--num_rows >= 0)
		{
			...
			for (col = 0; col < (mt_s32)num_cols; col++) 
			{
				...
				#endif
				{
					...
					outptr[r]  = (mt_u8)((u8YCCK[K]*u8YCCK[C])/u8MaxVal);
					outptr[g]  = (mt_u8)((u8YCCK[K]*u8YCCK[M])/u8MaxVal); 
					outptr[b]  = (mt_u8)((u8YCCK[K]*u8YCCK[Y])/u8MaxVal);
					outptr[RGB_ALPHA]  = 0xFF;
					outptr += 4;
				}
		   ...
		   JSAMPARRAY output_buf, int num_rows)
{
		...
		while (--num_rows >= 0)
		{
			...
			for (col = 0; col < (mt_s32)num_cols; col++) 
			{
				...
				{
				  (*(INT16*)(outptr)) = (INT16)PACK_SHORT_1555(a,b,g,r);
				}
				outptr += 2;
		...
		while (--num_rows >= 0)
		{
			...
			for (col = 0; col < (mt_s32)num_cols; col++) 
			{
				...
				#endif
				{
					...
					{
	                    *(INT16*)outptr = (INT16)PACK_SHORT_565(b,g,r);
					}
					outptr += 2;
