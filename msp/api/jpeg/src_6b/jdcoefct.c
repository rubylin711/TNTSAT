...
decompress_onepass (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  if (cinfo->comps_in_scan > 1)
    ...
    MCU_rows_per_iMCU_row = (mt_u32)(cinfo->cur_comp_info[0]->v_samp_factor);
#endif	
       ...
       yoffset++) 
  {
	...
	{
        p_idct = (unsigned short *)(pJpegHandle->swHuff.idct_buf_addr_vir[ci]);
	}		
	...
	start_col = MCU_col_num * (mt_u32)compptr->MCU_sample_width;
  MCU_rows_per_iMCU_row = cinfo->cur_comp_info[ci]->v_samp_factor;
	...
	for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
	  if (cinfo->input_iMCU_row < last_iMCU_row ||
	    ...
	    for (xindex = 0; xindex < useful_width; xindex++) {
	        ...
	        {			
#if 0	        
	            pos = ((cinfo->input_iMCU_row * compptr->MCU_height + yindex) * cinfo->MCUs_per_row * useful_width + 
					MCU_col_num * useful_width  + xindex) * pJpegHandle->swHuff.blk_size;

						pos = (((cinfo->input_iMCU_row * MCU_rows_per_iMCU_row + yoffset) * cinfo->MCUs_per_row)+ MCU_col_num) * pJpegHandle->swHuff.blk_size;		
#endif
              //for bug 116083 and 117168
              pos = (((cinfo->input_iMCU_row * MCU_rows_per_iMCU_row + yindex+yoffset) * cinfo->MCUs_per_row) + MCU_col_num) * pJpegHandle->swHuff.blk_size;

	            p_ptr = (short *)(coef->MCU_buffer[blkn+xindex]);
	            for(i = 0; i < pJpegHandle->swHuff.blk_size; i++)
	            {   
				#ifdef CONFIG_MT_FPGA_GPE
                        if(cinfo->p_jpeg_dbg_info->sw_dec)
                        {   
                            p_idct[pos + i] = p_ptr[jpeg_natural_order[i]];
                        }
                        else
				#endif
                        {
                            p_idct[pos + i] = (unsigned short)(p_ptr[i]);
                        }
					//p_idct[pos + i] = (unsigned short)(p_ptr[i]);
	            }
	        }			
  ...
  if(cinfo->p_jpeg_dbg_info->sw_dec)
  {
      ...
      int col,row;
      JSAMPROW inptr;
      jpeg_component_info *compptr2 = cinfo->comp_info;

      for(i = 0; i < 4; i++)
      {
        if(cinfo->p_jpeg_dbg_info->g_img[i] != NULL)
        {
          for (row=0;row< ((8  * compptr2[i].v_samp_factor)/(cinfo->scale_denom));row++)
          {
          	inptr = output_buf[i][row];            
               for (col = cinfo->p_jpeg_dbg_info->g_output_width[i]; col > 0; col--) {
                 if (cinfo->p_jpeg_dbg_info->g_idx[i] < (cinfo->p_jpeg_dbg_info->g_output_width[i]*cinfo->p_jpeg_dbg_info->g_output_height[i]))
          		*(cinfo->p_jpeg_dbg_info->g_img[i]+cinfo->p_jpeg_dbg_info->g_idx[i])  = GETJSAMPLE(*inptr++);

          		 cinfo->p_jpeg_dbg_info->g_idx[i]++; 	
            }
          }        
        }
      }
...
consume_data (j_decompress_ptr cinfo)
{
  ...
  for (yoffset = coef->MCU_vert_offset; yoffset < coef->MCU_rows_per_iMCU_row;
	...
	#endif /** CONFIG_JPEG_ADD_GOOGLEFUNCTION **/
	{
      ...
      for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	compptr = cinfo->cur_comp_info[ci];
	start_col = MCU_col_num * (mt_u32)compptr->MCU_width;
	for (yindex = 0; yindex < compptr->MCU_height; yindex++) {
...
decompress_data (j_decompress_ptr cinfo, JSAMPIMAGE output_buf)
{
  ...
  unsigned short *p_idct = NULL;
  short *p_ptr = NULL;
  unsigned int w_mcu_aligned[4] = {0};

  compptr = cinfo->comp_info;
  for(ci = 0; ci < cinfo->num_components; ci++)
  {
	  w_mcu_aligned[ci] = jround_up((long) compptr[ci].width_in_blocks,
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
       ci++, compptr++) {
	if((pJpegHandle->bSoftHuffDec) && (pJpegHandle->swHuff.if_need_buf == TRUE))
	{
        p_idct = (unsigned short *)(pJpegHandle->swHuff.idct_buf_addr_vir[ci]);
	}	   	
    /* Don't bother to IDCT an uninteresting component. */
	...
	if(pJpegHandle->swHuff.if_need_buf == FALSE)
	{
		if(cinfo->output_iMCU_row == 0)
		{			
			pJpegHandle->swHuff.idct_buf_addr_phy[ci] = (*cinfo->mem->barry_virt2phy)
				(coef->whole_image[ci], (ulong)buffer[0]);
			pJpegHandle->swHuff.idct_buf_addr_vir[ci] = (ulong)buffer[0];
			printf("\r\n buffer:0x%lx", (ulong)buffer[0]);
		}
	}
    ...
    for (block_row = 0; block_row < block_rows; block_row++) {
	  ...
	  #endif
	  {
	    if(p_idct)
	    {			
//	        pos = ((cinfo->output_iMCU_row * block_rows + block_row) * compptr->width_in_blocks + block_num) * pJpegHandle->swHuff.blk_size;
			pos = ((cinfo->output_iMCU_row * block_rows + block_row) * w_mcu_aligned[ci] / 8 + block_num) * pJpegHandle->swHuff.blk_size;

	        p_ptr = (short *)buffer_ptr;
	        for(i = 0; i < pJpegHandle->swHuff.blk_size; i++)
	        {        
				p_idct[pos + i] = (unsigned short)(p_ptr[i]);
	        }
...
#ifdef CONFIG_MT_FPGA_GPE
	if(cinfo->p_jpeg_dbg_info->sw_dec == 1)
         {
                 /*output yuv*/  
             int col,row;
             JSAMPROW inptr;
...
jinit_d_coef_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  if (need_full_buffer) {
    ...
    coef->pub.decompress_data = decompress_data;
    coef->pub.coef_arrays = coef->whole_image; /* link to virtual arrays */
	pJpegHandle->swHuff.if_need_buf = FALSE;
//	pJpegHandle->swHuff.if_need_buf = TRUE;
