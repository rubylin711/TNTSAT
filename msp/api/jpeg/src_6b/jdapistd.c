...
extern MT_S32 JPEG_HDEC_SofthuffStart(j_decompress_ptr cinfo);
...
static MT_BOOL jpeg_seek_input_data(j_decompress_ptr cinfo, long byte_offset)
{
...
static MT_BOOL alloc_softhuff_coeffbuf(j_decompress_ptr cinfo)
{
	JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
	MT_S32 i,j;
	jpeg_component_info *compptr = cinfo->comp_info;
	MT_U32 idct_buf_size = 0;
//	MT_U32 compnont_size[4] = {0};
	MT_U32 w_mcu_aligned[4] = {0};
	MT_U32 h_mcu_aligned[4] = {0}; 

	MT_INFO_JPEG("\r\n use soft huffman decode@@@@@@@@@@@@@@@@@@@@");
	...
	for(i = 0; i < cinfo->num_components; i++)
	{
  	...
  	if(pJpegHandle->swHuff.if_need_buf == MT_TRUE)
  	{
  		if(pJpegHandle->u32ScalRation == 0)
  		  pJpegHandle->swHuff.blk_size = 64;
  		else if(pJpegHandle->u32ScalRation == 1)
  		  pJpegHandle->swHuff.blk_size = 25;
  		else if(pJpegHandle->u32ScalRation == 2)
  		  pJpegHandle->swHuff.blk_size = 5;
  		else
  		  pJpegHandle->swHuff.blk_size = 1;
  	}
  	else
  	  pJpegHandle->swHuff.blk_size = 64;
    ...
    {
  	//idct coeff buf
  	pJpegHandle->swHuff.idct_buf_size[0] = w_mcu_aligned[0] * h_mcu_aligned[0] / 64 * pJpegHandle->swHuff.blk_size * 2;
  	pJpegHandle->swHuff.idct_buf_size[1] = w_mcu_aligned[1] * h_mcu_aligned[1] / 64 * pJpegHandle->swHuff.blk_size * 2;
  	pJpegHandle->swHuff.idct_buf_size[2] = w_mcu_aligned[2] * h_mcu_aligned[2] / 64 * pJpegHandle->swHuff.blk_size * 2;
  	pJpegHandle->swHuff.idct_buf_size[3] = 0;
    ...
    {
  	  pJpegHandle->swHuff.idct_buf_size[0] = (MT_U32)(w_mcu_aligned[0] * h_mcu_aligned[0] / 64 * pJpegHandle->swHuff.blk_size * 2
  		  * (MT_U32)compptr[0].h_samp_factor * (MT_U32)compptr[0].v_samp_factor
  		  / (MT_U32)cinfo->max_h_samp_factor / (MT_U32)cinfo->max_v_samp_factor);
  	  pJpegHandle->swHuff.idct_buf_size[1] = (MT_U32)(w_mcu_aligned[1] * h_mcu_aligned[1] / 64 * pJpegHandle->swHuff.blk_size * 2
  		  * (MT_U32)compptr[1].h_samp_factor * (MT_U32)compptr[1].v_samp_factor
  		  / (MT_U32)cinfo->max_h_samp_factor / (MT_U32)cinfo->max_h_samp_factor);
  	  pJpegHandle->swHuff.idct_buf_size[2] = (MT_U32)(w_mcu_aligned[2] * h_mcu_aligned[2] / 64 * pJpegHandle->swHuff.blk_size * 2
  		  * (MT_U32)compptr[2].h_samp_factor * (MT_U32)compptr[2].v_samp_factor
  		  / (MT_U32)cinfo->max_h_samp_factor / (MT_U32)cinfo->max_v_samp_factor);
  	  pJpegHandle->swHuff.idct_buf_size[3] = (MT_U32)(w_mcu_aligned[3] * h_mcu_aligned[3] / 64 * pJpegHandle->swHuff.blk_size * 2
  		  * (MT_U32)compptr[3].h_samp_factor * (MT_U32)compptr[3].v_samp_factor
  		  / (MT_U32)cinfo->max_h_samp_factor / (MT_U32)cinfo->max_v_samp_factor);		
    }
	  else if(cinfo->jpeg_color_space == JCS_GRAYSCALE)
	  {   
			pJpegHandle->swHuff.idct_buf_size[0] = w_mcu_aligned[0] * h_mcu_aligned[0] / 64 * pJpegHandle->swHuff.blk_size * 2;
			pJpegHandle->swHuff.idct_buf_size[1] = 0;
			pJpegHandle->swHuff.idct_buf_size[2] = 0;
			pJpegHandle->swHuff.idct_buf_size[3] = 0;
	  }  		
    ...
    if(pJpegHandle->swHuff.if_need_buf == MT_TRUE)
    {
		for(i = 0; i < 4; i++)
		{
			idct_buf_size = pJpegHandle->swHuff.idct_buf_size[i];

			if(idct_buf_size != 0)
			{
			 pJpegHandle->swHuff.idct_buf_addr_phy[i] = MT_GFX_AllocMem(idct_buf_size, 256, (mt_char*)"jpeg", (mt_char*)"JPEG");
			 if(0 == pJpegHandle->swHuff.idct_buf_addr_phy[i])
			 {
			  for(j = 0; j < i - 1; j++)
			  {
				  if(pJpegHandle->swHuff.idct_buf_addr_phy[j] != 0)
				  {
					  MT_GFX_Unmap((void*)pJpegHandle->swHuff.idct_buf_addr_vir[j]);
					  MT_GFX_FreeMem(pJpegHandle->swHuff.idct_buf_addr_phy[j]);
					  pJpegHandle->swHuff.idct_buf_addr_phy[j] = 0;
					  pJpegHandle->swHuff.idct_buf_addr_vir[j] = 0;
							   
				  }
				  MT_ERR_JPEG("\r\n alloc idct buf failed");  
			  }				
				return MT_FALSE;
			 }
			 pJpegHandle->swHuff.idct_buf_addr_vir[i] = (ulong)MT_GFX_MapCached(pJpegHandle->swHuff.idct_buf_addr_phy[i]); 
			 memset((void *)(pJpegHandle->swHuff.idct_buf_addr_vir[i]),0,idct_buf_size);
			 MT_GFX_Flush((void*)pJpegHandle->swHuff.idct_buf_addr_vir[i], 0, 0);		
			}		
		}   
    }
	return MT_TRUE;
...
static void free_softhuff_coeffbuf(j_decompress_ptr cinfo)
{
	JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
	MT_S32 i;
	...
	if(pJpegHandle->swHuff.if_need_buf == MT_TRUE)
	{
		for(i = 0; i < 4; i++)
		{
			if(pJpegHandle->swHuff.idct_buf_addr_phy[i] != 0)
			{
				MT_GFX_Unmap((void*)pJpegHandle->swHuff.idct_buf_addr_vir[i]);
				MT_GFX_FreeMem(pJpegHandle->swHuff.idct_buf_addr_phy[i]);
				pJpegHandle->swHuff.idct_buf_addr_phy[i] = 0;
				pJpegHandle->swHuff.idct_buf_addr_vir[i] = 0;
...
static void flush_softhuff_coeffbuf(j_decompress_ptr cinfo)
{
	JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
	MT_S32 i;
	...
	for(i = 0; i < 4; i++)
	{
		if(pJpegHandle->swHuff.if_need_buf && pJpegHandle->swHuff.idct_buf_addr_phy[i] != 0)
		{
			MT_GFX_Flush((void*)pJpegHandle->swHuff.idct_buf_addr_vir[i], 0, 0);						 
		}			
	} 	
	#if 0
{
	mt_u16 *ptr = (mt_u16 *)pJpegHandle->swHuff.idct_buf_addr_vir[0];
	mt_u32 i;
	for(i = 0; i < pJpegHandle->swHuff.idct_buf_size[0] / 2; i++)
		printf("\r\n %04x", ptr[i]);
}
#endif
}
static MT_BOOL jpeg_start_hard_decompress(j_decompress_ptr cinfo)
{
...
static MT_BOOL jpeg_start_soft_decompress(j_decompress_ptr cinfo)
{
...
GLOBAL(boolean)
jpeg_start_soft_huff_decompress(j_decompress_ptr cinfo)
{

		mt_s32 s32RetVal = MT_SUCCESS;
        JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
 //               pJpegHandle->swHuff.mcu_id = 0;
		...
		**/
		...
		flush_softhuff_coeffbuf(cinfo);
		JPEG_HDEC_SofthuffStart(cinfo);

		free_softhuff_coeffbuf(cinfo);			
...
GLOBAL(boolean)
jpeg_if_softhuff (j_decompress_ptr cinfo)
{
	JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
	return pJpegHandle->bSoftHuffDec;
}
#if 0
GLOBAL(void)
jpeg_use_softhuffx (j_decompress_ptr cinfo)
{
	JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
	pJpegHandle->bSoftHuffDec = MT_TRUE;
}
#endif
...
jpeg_start_decompress (j_decompress_ptr cinfo)
{
		...
		mt_s32 s32BeforeStartHardDecState = 0;
    ...
    if(cinfo->global_state == DSTATE_RUNERR)
      return FALSE;
        ...
        if(MT_TRUE == pJpegHandle->bFirstDec)
        {
			if(DSTATE_READY == s32BeforeStartHardDecState)
			{
              if(pJpegHandle->bSoftHuffDec == MT_FALSE)
				...
				if(bRet == MT_FALSE)
				{
					...
					pJpegHandle->bSoftHuffDec = MT_FALSE;
  					MT_ERR_JPEG("\r\n soft huffman decode failed, use soft decode");
				}
				else
				{
				
	  		    	bRet = alloc_softhuff_coeffbuf(cinfo);	  	
	  				if(bRet == MT_FALSE)
	  				{
	  					pJpegHandle->bSoftHuffDec = MT_FALSE;
	  					MT_ERR_JPEG("\r\n soft huffman decode failed, use soft decode");
        ...
        return FALSE;
	        ...
	        **/
	        cinfo->global_state = s32BeforeStartHardDecState;
				...
				if( (MT_TRUE == pJpegHandle->bInHardDec) && (NULL != cinfo->src->seek_input_data) )
				{/** 如果是进入硬件解码，且硬件硬件解码失败，则码流回退重新进入软件解码 **/
				   	cinfo->src->seek_input_data(cinfo, pJpegHandle->u32CurrentOffset);
				}
				else if((MT_FALSE == pJpegHandle->stHDecDataBuf.bUseInsideData)&&(MT_TRUE == pJpegHandle->bInHardDec))
				{
					 pJpegHandle->bFirstDec = MT_FALSE;
					 return MT_FALSE;
				}
				else if((MT_TRUE == pJpegHandle->stHDecDataBuf.bUseFileData) && (MT_TRUE == pJpegHandle->bInHardDec))
				{/** 只有读文件才需要码流回退 **/
			...
			#else
				if((MT_FALSE == pJpegHandle->stHDecDataBuf.bUseInsideData)&&(MT_TRUE == pJpegHandle->bInHardDec))
				{/**
		          ...
		          **/
					...
					if((MT_TRUE == pJpegHandle->stHDecDataBuf.bUseFileData) && (NULL != cinfo->src->seek_input_data) && (MT_TRUE == pJpegHandle->bInHardDec))
					{/** 只有读文件才需要码流回退 **/
					 	cinfo->src->seek_input_data(cinfo, pJpegHandle->u32CurrentOffset);
					}
				#else
                    if((MT_TRUE == pJpegHandle->stHDecDataBuf.bUseFileData) && (MT_TRUE == pJpegHandle->bInHardDec))
					{/** 只有读文件才需要码流回退 **/
					 	jpeg_seek_input_data(cinfo, (long int)pJpegHandle->u32CurrentOffset);
					}
				#endif
			#endif
		...
		if(MT_FALSE == pJpegHandle->bHdecEnd)
		{
			JPEG_TRACE("===============================================================\n");
 ...
 */
 ...
 static MT_BOOL jpeg_start_tile_hard_decompress(j_decompress_ptr cinfo)
{
		mt_s32 s32RetVal = MT_SUCCESS;
		JPEG_HDEC_HANDLE_S_PTR	pJpegHandle = (JPEG_HDEC_HANDLE_S_PTR)(cinfo->client_data);
...
static MT_BOOL jpeg_start_tile_soft_decompress(j_decompress_ptr cinfo)
{
 ...
 */
static mt_u32 jpeg_soft_read_scanlines(j_decompress_ptr cinfo, 
...
static mt_u32 jpeg_hard_read_scanlines(j_decompress_ptr cinfo, 
                                                   JSAMPARRAY scanlines,
		                                           JDIMENSION max_lines)
{
       mt_s32 row_ctr = 0;
	   row_ctr = JPEG_HDEC_HardCSC(cinfo);
	   if(MT_SUCCESS != row_ctr)
	   {/**
       ...
       if(NULL != scanlines)
       {
	      row_ctr = JPEG_HDEC_OutUserBuf(cinfo, max_lines,(mt_char*)scanlines[0]);
       }
       else
       {
          row_ctr = JPEG_HDEC_OutUserBuf(cinfo, max_lines,NULL);
       }
	   return (mt_u32)row_ctr;
...
#ifdef CONFIG_JPEG_SUBSET_HARD_DEC_ENABLE
static mt_u32 jpeg_hard_read_tile_scanlines(j_decompress_ptr cinfo, huffman_index *index,JSAMPARRAY scanlines)
{
       ...
       if(NULL != scanlines)
       {
	      row_ctr = JPEG_HDEC_OutUserBuf(cinfo, 1,(mt_char*)scanlines[0]);
       }
       else
       {
...
static mt_u32  jpeg_soft_read_tile_scanlines(j_decompress_ptr cinfo, huffman_index *index,JSAMPARRAY scanlines)
{
