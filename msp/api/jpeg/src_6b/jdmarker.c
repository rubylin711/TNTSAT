  ...
  else if ((cinfo->num_components==3) && (cinfo->comp_info[1].h_samp_factor==cinfo->comp_info[2].h_samp_factor) && (cinfo->comp_info[1].v_samp_factor==cinfo->comp_info[2].v_samp_factor))
  {
		if (cinfo->comp_info[0].h_samp_factor==((cinfo->comp_info[1].h_samp_factor)<<1))
		{
		 ...
		 else if (cinfo->comp_info[0].h_samp_factor==cinfo->comp_info[1].h_samp_factor)
		 {
		  ...
		  pJpegHandle->u8Fac[0][0] = (mt_u8)(cinfo->comp_info[0].h_samp_factor);
		  pJpegHandle->u8Fac[0][1] = (mt_u8)(cinfo->comp_info[0].v_samp_factor);
		  pJpegHandle->u8Fac[1][0] = (mt_u8)(cinfo->comp_info[1].h_samp_factor);
		  pJpegHandle->u8Fac[1][1] = (mt_u8)(cinfo->comp_info[1].v_samp_factor);
		  pJpegHandle->u8Fac[2][0] = (mt_u8)(cinfo->comp_info[2].h_samp_factor);
		  pJpegHandle->u8Fac[2][1] = (mt_u8)(cinfo->comp_info[2].v_samp_factor);
  }
  else
  {
...
/* Save an APPn or COM marker into the marker list */
{
  ...
  if (cur_marker != NULL) {	/* will be NULL if bogus length word */
    ...
    length = (INT32)(cur_marker->original_length - data_length);
  }
