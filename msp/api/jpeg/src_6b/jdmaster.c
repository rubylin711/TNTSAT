...
/** revise by y00181162,android no use this,because the free mem is wrong,would be comdump**/
		...
		while( ((cinfo->output_width > 4095) || ( cinfo->output_height > 4095 )) && (MT_FALSE == bDecARGB) && (MT_FALSE == pJpegHandle->bOutYCbCrSP))
	#else
		while( ((cinfo->output_width > 4095) || ( cinfo->output_height > 4095 )) && (MT_FALSE == pJpegHandle->bOutYCbCrSP))
	#endif
		{
			u32Scale++;
			if(u32Scale > 3)
