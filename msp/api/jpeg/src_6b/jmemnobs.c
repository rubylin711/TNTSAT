...
jpeg_get_large (j_common_ptr cinfo, size_t sizeofobject)
{
		...
		**/
		 ...
		 **/
        PhysAddr = (phys_addr_t)MT_GFX_AllocMem((mt_u32)tSize, 64, (mt_char*)"jpeg", "JPEG");
	    if(0 == PhysAddr)
		{  
	          return NULL;
	    }
		 ...
		 **/
//         pVirtAddr = (void FAR *)MT_GFX_MapCached(PhysAddr);
		 pVirtAddr = (void FAR *)MT_GFX_Map(PhysAddr);
...
#else
		pVirtAddr = (void FAR *)HI_GFX_Map((mt_u32)PhysAddr);
		 if (NULL == pVirtAddr)
		 {
	          return NULL;
	     }
...
//		 printf("\r\n ~~~~~~~~jpeg_get_large:%d, 0x%08x", tSize,pVirtAddr);
	     return pVirtAddr;
#endif
}
...
jpeg_free_large (j_common_ptr cinfo, void FAR * object, size_t sizeofobject)
{
		...
		if (0 !=PhysAddr)
		{
		    s32Ret = MT_GFX_Unmap(hdr_ptr);
			if(MT_SUCCESS != s32Ret)
			{
			   return;
			}
		    s32Ret = MT_GFX_FreeMem(PhysAddr);
			if(MT_SUCCESS != s32Ret)
			{
			   return;
			}
