...
fill_input_buffer (j_decompress_ptr cinfo)
{
	 ...
	 if(MT_TRUE == pJpegHandle->stHDecDataBuf.bReadToDataBuf)
	 {   /**
...
fill_mem_input_buffer (j_decompress_ptr cinfo)
{
    ...
    if((NULL == src->stBufStream.img_buffer) || (src->stBufStream.pos >= (mt_u32)src->stBufStream.buffer_size))
    {
        nbytes = 0; /** the nbytes is unsigned type **/    
    }
    else
    {
        nbytes = (src->stBufStream.pos + INPUT_BUF_SIZE > (mt_u32)src->stBufStream.buffer_size ? \
...
fill_retrun_mem_input_buffer (j_decompress_ptr cinfo)
{
	  ...
	  if(pJpegHandle->stHDecDataBuf.u32ConsumeDataSize == (mt_u32)src->stBufStream.buffer_size)
	  {
			src->stBufStream.pos = 0; /** 第二块码流buffer的偏移 **/
	  }
	  if(pJpegHandle->stHDecDataBuf.u32ConsumeDataSize >= (mt_u32)src->stBufStream.buffer_size)
	  {/** 第一块码流已经用完，请使用第二块 **/
		 nbytes = (src->stBufStream.pos + INPUT_BUF_SIZE > (mt_u32)pJpegHandle->stHDecDataBuf.s32StreamReturnLen ? \
...
jpeg_stdio_src (j_decompress_ptr cinfo, FILE * infile)
{
	   ...
	   **/
	   pJpegHandle->stHDecDataBuf.bUseInsideData = MT_TRUE;
	   pJpegHandle->stHDecDataBuf.bUseFileData   = MT_TRUE;
	      ...
	      unsigned char * inbuffer, unsigned long insize)
{
	  ...
	  pJpegHandle->stHDecDataBuf.pDataVirBuf = (mt_char*)inbuffer;
	  ...
	  if(MT_FALSE == pJpegHandle->stHDecDataBuf.bNeedStreamReturn)
	  {
