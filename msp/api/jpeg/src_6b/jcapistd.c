...
{	
	...
	if(HI_FALSE == pJpgeHandle->bEncSupport)
	{
		return HI_FAILURE;
	}
    /** yuv444package的需要填充一位,TDE转AYCbCr8888排布YUAV,编码需求 **/
	pTmpSrcBuf = (HI_CHAR*)(pJpgeHandle->pVir + cinfo->next_scanline * pJpgeHandle->EncIn.ViYStride);
	for(s32Cnt = 0; s32Cnt < (cinfo->image_width * cinfo->input_components);s32Cnt+=3)
	{
		...
		s32Offset+=4;
		/** memcpy(pTmpSrcBuf,scanlines[0],EncIn.ViYStride); **/
	}
	if(cinfo->next_scanline == (cinfo->image_height - 1))
	{
		JPEG_HENC_EncodeImg(cinfo);
	}
		      ...
		      JDIMENSION num_lines)
{
		...
		if (NULL != pJpgeHandle)
		{
			...
			if(HI_TRUE == pJpgeHandle->bEncSupport && HI_FAILURE == s32Ret)
			{
				ERREXIT1(cinfo, JERR_BAD_STATE, cinfo->global_state);
			}
			if(HI_TRUE == pJpgeHandle->bEncSupport && HI_SUCCESS == s32Ret)
			{
				return num_lines;
			}
		}
