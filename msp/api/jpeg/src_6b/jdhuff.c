			 ...
			 d_derived_tbl ** pdtbl)
{
  ...
  for (l = 1; l <= 16; l++) {
    if (htbl->bits[l]) {
      ...
      p += htbl->bits[l];
      dtbl->maxcode[l] = (INT32)huffcode[p-1]; /* maximum code of length l */
      dtbl->maxaddr[l] = p - 1;
    } else {
  ...
  dtbl->maxaddr[17] = 0xFFFFFL; /* ensures jpeg_huff_decode terminates */
...
if(pJpegHandle->bSoftHuffDec)
{
       ...
       {
        p_idct = (unsigned short *)(pJpegHandle->swHuff.idct_buf_addr_vir[0]);
       }
       else
       {
        p_idct = (unsigned short *)(pJpegHandle->swHuff.idct_buf_addr_vir[ci]);
       }
//       printf("\r\n %s, %d,0x%08x", __FUNCTION__, __LINE__, pJpegHandle->swHuff.idct_buf_addr_vir[0]);
       if(p_idct)
      {      
        if(cinfo->jpeg_color_space == JCS_YCbCr)
            {
          p_idct[pJpegHandle->swHuff.mcu_id* cinfo->blocks_in_MCU * pJpegHandle->swHuff.blk_size + blkn * pJpegHandle->swHuff.blk_size] = s & 0xFFFF;
//          printf("\r\n %d, 0x%04x", pJpegHandle->swHuff.mcu_id* cinfo->blocks_in_MCU * pJpegHandle->swHuff.blk_size + blkn * pJpegHandle->swHuff.blk_size, s & 0xFFFF);
            }
        else
        {
printf("\r\n rgb and cmyk will be added later");
        }
//        printf("\r\n g_mcu_id:%d, pos:%d, mcu_row:%d, mcu_col:%d, mcu_width:%d", g_mcu_id, pos / g_blk_size,mcu_row,mcu_col, mcu_width);
      }
}
#endif
      }
        ...
        if(pJpegHandle->bSoftHuffDec)
            {
                   if(p_idct)   
        {
            if(k < pJpegHandle->swHuff.blk_size)        
            {
                if(cinfo->jpeg_color_space == JCS_YCbCr)
                {
                    p_idct[pJpegHandle->swHuff.mcu_id * cinfo->blocks_in_MCU * pJpegHandle->swHuff.blk_size + blkn * pJpegHandle->swHuff.blk_size + k] = s & 0xFFFF;
//                    printf("\r\n %d, 0x%04x", pJpegHandle->swHuff.mcu_id * cinfo->blocks_in_MCU * pJpegHandle->swHuff.blk_size + blkn * pJpegHandle->swHuff.blk_size + k, s & 0xFFFF);
                }
                else
                {
    printf("\r\n rgb and cmyk will be added later");             
