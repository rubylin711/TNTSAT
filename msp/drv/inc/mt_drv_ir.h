...
/*
#define MT_FATAL_IR(fmt ...) \
    MT_FATAL_PRINT(MT_ID_IR, fmt)

#define MT_ERR_IR(fmt ...) \
    MT_ERR_PRINT(MT_ID_IR, fmt)

#define MT_WARN_IR(fmt ...) \
    MT_WARN_PRINT(MT_ID_IR, fmt)

#define MT_INFO_IR(fmt ...) \
    MT_INFO_PRINT(MT_ID_IR, fmt)

#define MT_DEBUG_IR(fmt ...) \
    MT_DBG_PRINT(MT_ID_IR, fmt)
*/
...
/* irq define */
#if 1
#define IR_IRQ_NO   (68+32)
#define IR_IO_BASE  (0xfff44000)
#else
...
typedef struct
{
    mt_u32               IrKeyDataH;
    mt_u32               IrKeyDataL;
    irda_protocol_t IrProtocol;
    MT_UNF_KEY_STATUS_E  IrKeyState;
}IR_KEY_S;
...
mt_s32 MT_DRV_IR_Init(mt_void);
mt_s32 MT_DRV_IR_DeInit(mt_void);
mt_s32 MT_DRV_IR_Enable(MT_BOOL bEnable);
mt_s32 MT_DRV_IR_GetValueWithProtocol(enum KEY_STATUS *penPressStatus, mt_u64 *pu64KeyId,
                                      mt_char *pszProtocolName, mt_s32 s32NameSize, mt_u32 u32TimeoutMs);
mt_s32 MT_DRV_IR_EnableKeyUp(MT_BOOL bEnable);
mt_s32 MT_DRV_IR_EnableRepKey(MT_BOOL bEnable);
mt_s32 MT_DRV_IR_SetRepKeyTimeoutAttr(mt_u32 u32TimeoutMs);
mt_s32 MT_DRV_IR_SetFetchMode(mt_s32 s32Mode);
