...
static avcpu_decrypt_param_t g_avcpu_decrypt_param = {0};
...
static void mt_avcpu_set_encrypt_status(MT_BOOL status)
{
	is_encrypt = status;
}
...
static int mt_avcpu_decrypt_gatparam(char *p)
{
    ...
    DECRYPT_PRINTK("avcpu_decrypt_gatparam c1:%s,c2:%s,c3:%s,c4:%u\n",g_avcpu_decrypt_param.rsapss,g_avcpu_decrypt_param.rsakey,g_avcpu_decrypt_param.keyslot_type,g_avcpu_decrypt_param.keyslot_num);
	if(g_avcpu_decrypt_param.rsapss[0] != '\0' && g_avcpu_decrypt_param.rsakey[0] != '\0' && g_avcpu_decrypt_param.keyslot_type[0] != '\0')
		mt_avcpu_set_encrypt_status(TRUE);
...
static int on_cmd_aes_cbc_decrypt(ulong store_addr,u32 run_addr,const char *key_type, u32 key_slot)
{
    int ret = RET_SUCCESS;
	...
	const char *kltype;
	kltype=key_type;
 	DECRYPT_PRINTK("on_cmd_aes_cbc_decrypt start,kltype:%s,key_slot:%u\n",key_type,key_slot);

    img_addr = (unsigned char *)(ulong)(store_addr+SIGNATURE_LEN);
    dst_addr = (unsigned char *)(ulong)(run_addr-STUFF_LEN);
...
static int on_cmd_rsa_verify(ulong store_addr,u32 run_addr,const char *rsa_pss, const char *rsa_key)
{
    int ret = RET_SUCCESS;
    int algo;
    const char *param;
...
static int on_cmd_rsa_verify_raw(ulong store_addr,u32 run_addr,const char *rsa_pss, const char *rsa_key)
{
    int ret = RET_SUCCESS;
    int algo = 0;
    const char *param;
    ...
    param = rsa_pss;
	DECRYPT_PRINTK("algo=%d,saddr=0x%lx,raddr=0x%x,pss:%s,key:%s,p:%s\n",algo,store_addr,run_addr,rsa_pss,rsa_key,param);
   ...
   ret = get_rsa_public_key(rsa_key, &pub_key, &key_len);
    ...
    msg_addr = (unsigned char *)(ulong)(store_addr+SIGNATURE_LEN);
    msg_len  = UBO_HEADER_LEN;
    sig_addr = (unsigned char *)(ulong)(store_addr);
	...
	DECRYPT_PRINTK("%s: RSA_verify %d (%lx %u) (%lx %u) (%lx %u), ret %d.\n",__FUNCTION__,
	     algo, (ulong)msg_addr, msg_len, (ulong)sig_addr, sig_len,
	     (ulong)pub_key, key_len,
	     ret);
...
u32 mt_avcpu_do_decrypt(u32 ddr_addr,avcpu_decrypt_param_t decry_param,struct mtd_info *mtd)
{
	...
	ktmp_size = AVCPU_SIZE;
	tmp_addr = kmalloc(ktmp_size , GFP_KERNEL);
	DECRYPT_PRINTK("avcpu_decrypt addr:0x%lx,ktmp_size:0x%x,mtd:%p\n",(ulong)tmp_addr,ktmp_size,mtd);
	ktmp_addr=(ulong)tmp_addr;
	ret=mtd_read(mtd, 0, ktmp_size, (size_t*)&retlen, (u_char*)ktmp_addr);

	DECRYPT_PRINTK("retlen:0x%lx\n",retlen);
	if(ret ||(retlen != ktmp_size))
	return RET_FAILURE;

	DECRYPT_PRINTK("param rsapss:%s,rsakey:%s,type:%s,num:%u\n",decry_param.rsapss,decry_param.rsakey,decry_param.keyslot_type,decry_param.keyslot_num);
