...
#define kl_readl(addr)         HAL_GET_U32((volatile u32 *)addr)
#define kl_writel(v, addr)     HAL_PUT_U32((volatile u32 *)addr, v)
...
int bare_kl_store_tdc(void *bhandle, unsigned char *p_input)
{
    ...
    if (kl_readl(KEYLADDER_REG_STATE_CHECK) & KL_CMDST_FLAG0_ERR) {
        return ERR_FAILURE;
    }

    bare_kl_wait_complete(bhandle, &err_code);
