...
void SI_SetBpssVidVal(uint8_t c0, uint8_t c1, uint8_t c2);
...
void SI_HW_RecoveryDTSCfg(void)
{
	//Recover the dts config
	...
	HdmiPrivDrvPutBit32((ulong)NULL, reg_addr, BIT_MSK__REG_INT_MUX, 2);
	reg_addr = REG_ADDR__EMP_CTRL1;
	switch (params->emp_mode) {
...
void SI_HW_ResetHDMITX(void)
{
	...
	//udelay(100);
	SI_HW_ResetCtrl(1);
	// temp SiiLibTimeMilliDelay(1);
	SI_HW_ResetPhy(1);
	// temp SiiLibTimeMilliDelay(10);
	...
	SI_HW_ResetCtrl(0);
	mt_clk_reset(MT_CLK_HDMI);
	SI_HW_RecoveryDTSCfg();
	// temp SiiLibTimeMilliDelay(100);
...
MT_BOOL SI_HDMI_Setup_INBoot(MT_U32 *VIC)
{
	...
	r8 = SiiDrvCraRdReg8((SiiInst_t)0, REG_ADDR__INTR1_MASK);
	//HDMI HPD Flag
	if (!SI_HPD_Status() || (r8==0)) {
		return MT_FALSE;
	}
...
void SI_SW_ResetHDMITX(void)
{
	...
	while ( !siiIsTClockStable() && --TimeOut ) {
		SiiLibTimeMilliDelay(1);         // wait for input pixel clock to stabilze
	}
	...
	if (TimeOut) {
		...
		SiiLibTimeMilliDelay(1);          // allow TCLK (sent to Rx across the HDMS link) to stabilize
	} else {
...
{
	uint8_t u8Value = 0;
	u8Value = SiiDrvCraRdReg8((ulong)NULL, REG_ADDR__TPI_SC) & BIT_MSK__TPI_SC__REG_TPI_OUTPUT_MODE_B0;
	return u8Value;
}
//--------------------------------------------------------------------------
void SI_TX_SetHDMIMode(MT_U8 Enabled)
{
	uint8_t u8Value = 0;
	COM_INFO("Set HDMI mode to %d.\n", Enabled);
	u8Value = SiiDrvCraRdReg8((ulong)NULL, REG_ADDR__TPI_SC);
	if (Enabled) {
		SiiDrvTxTmdsModeSet(DRV_HDMI_Get_TxInst(), SII_TMDS_MODE__AUTO);
	} else {
		SiiDrvTxTmdsModeSet(DRV_HDMI_Get_TxInst(), SII_TMDS_MODE__DVI);
	}

	COM_INFO("AUDP_TXCTRL_ADDR:0x%x\n", SiiDrvCraRdReg8((ulong)NULL, REG_ADDR__TPI_SC));
}
...
void SI_SetBpssVidVal(uint8_t c0, uint8_t c1, uint8_t c2)
{
	...
	return;
	cms0 = (uint16_t)SiiDrvCraRdReg8((SiiInst_t)NULL, REG_ADDR__VP__CMS__CSC0__MULTI_CSC_CONFIG);
	cms0 |= ((uint16_t)SiiDrvCraRdReg8((SiiInst_t)NULL, REG_ADDR__VP__CMS__CSC0__MULTI_CSC_CONFIG + 1)) << 8;

	cms1 = (uint16_t)SiiDrvCraRdReg8((SiiInst_t)NULL, REG_ADDR__VP__CMS__CSC1__MULTI_CSC_CONFIG);
	cms1 |= ((uint16_t)SiiDrvCraRdReg8((SiiInst_t)NULL, REG_ADDR__VP__CMS__CSC1__MULTI_CSC_CONFIG + 1)) << 8;
	...
	c1_12 = ((uint16_t)c1) << 4;
	c2_12 = ((uint16_t)c2) << 4;
	//COM_INFO("%s_%d Force CS:[%x %x %x]\n", __func__, __LINE__, (uint32_t)c0, (uint32_t)c1, (uint32_t)c2);
	SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_ACTIVE_Y, c0_12 & 0xFF);
	SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_ACTIVE_Y + 1, (c0_12 >> 8) & 0x0F);
	SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_ACTIVE_CB, c1_12 & 0xFF);
	SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_ACTIVE_CB + 1, (c1_12 >> 8) & 0x0F);
	SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_ACTIVE_CR, c2_12 & 0xFF);
	SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_ACTIVE_CR + 1, (c2_12 >> 8) & 0x0F);
}
...
void SI_SetHdmiVideo(MT_U8 Enabled)
{
	...
	if (MT_FALSE == Enabled) {
		...
		SiiDrvCraSetBit8((SiiInst_t)NULL, REG_ADDR__VP__OUTPUT_BLANK_CONFIG, BIT_MSK__VP__OUTPUT_BLANK_CONFIG__ENABLE_ACTIVE_OVERRIDE);
	} else {
	...
	//COM_INFO("%s_%d Force Vid:%d CS:[%x %x %x]\n", __func__, __LINE__, (uint32_t)Enabled, (uint32_t)BlankValue[0], (uint32_t)BlankValue[1], (uint32_t)BlankValue[2]);
}

void SI_SetHdmiAudio(MT_U8 Enabled)
{
#define		AUD_MUTE_MODE		(0)
	uint8_t regc = 0;
	return ;
	regc = SiiDrvCraRdReg8((SiiInst_t)NULL, REG_ADDR__SYS_CTRL1);
	COM_INFO("SI_SetHdmiAudio to %d.\n", Enabled);
	if (MT_FALSE == Enabled) {
		if ( AUD_MUTE_MODE == 0 ) {
			regc &= ~(1 << 0);
		...
		SiiDrvCraWrReg8((SiiInst_t)NULL, REG_ADDR__SYS_CTRL1, regc);
...
{
	COM_INFO("SI_SetHdmiAudioPkg to %d.\n", Enabled);
	if (MT_FALSE == Enabled) {
		SiiDrvCraSetBit8((SiiInst_t)NULL, REG_ADDR__AUDP_TXCTRL, BIT_MSK__AUDP_TXCTRL__REG_AUD_MUTE_EN);
	} else {
		SiiDrvCraClrBit8((SiiInst_t)NULL, REG_ADDR__AUDP_TXCTRL, BIT_MSK__AUDP_TXCTRL__REG_AUD_MUTE_EN);
...
void SI_Start_HDMITX(void)
{
	...
	#if defined (DVI_SUPPORT)
	if (bVideoMode >= 100) {
		MT_PRINT("index is out of bounds.\n");
		return ;
	}
	#else
	if (bVideoMode >= 60) {
...
MT_S32 SI_TX_IsHDMImode()
{
	uint8_t u8Value = 0;

	u8Value = SiiDrvCraRdReg8((ulong)NULL, REG_ADDR__TPI_SC);
	if ( u8Value & BIT_MSK__TPI_SC__REG_TPI_OUTPUT_MODE_B0) {
		return MT_TRUE;
	}
	return MT_FALSE;
}
...
//     section below.
//
//  2.5 InitSystem() calls ReadChipInfo(): Read 3 MT_U8s from registers {0x72:0x02..0x04} into ChipInf[] (Device ID + Rev. #).
...
//
// 2.6.12   SI_BlockReadEEPROM (4, EE_TX_VIDEOPATH_ADDR, abData) reads 4 MT_U8s of video path from the EEPROM to local array
...
//
// 2.6.14 SI_Init_HDMITX() reads 4 MT_U8s from EEPROM[0x98] into local array abData[]. These 4 MT_U8s carry the following
//         information:
//
...
//
//  2.6.14.1 SI_Set_AudioInfoFramePacket() prepares data MT_U8s 1-4 of the audio InfoFrame, per EIA861B, section 6.3,
//       that include information about the number of audio channels, sampling frequency and speaker mapping. It then
//       calls SI_TX_SendInfoFrame() with the Type (AUDIO) and the frame data MT_U8s as parameters.
//
...
//
//  2.6.14.1.2  Calls GetRestOfInfoFrameHeader() to prepare the first 3 MT_U8s of the Audio InfoFrame header.
//
...
//
//  2.6.17.1    SI_SendAVIInfoFrame() reads 13 MT_U8s from EEPROM location 0xD0. The first
//      4 MT_U8s contain the follwoing data:
//
