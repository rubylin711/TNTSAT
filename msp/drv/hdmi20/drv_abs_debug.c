 ...
 */
mt_void ABS_PtkToMem(ABS_DBG_S_PTR pABSdbg, MT_U8 u8DbgLevel, mt_u32 u32FuncNum,  MT_U8 *u8Buf)
{
	mt_u32 len, left, right;
	MT_U8  temp[ABS_DBG_MEM_IN_BUF_SIZE  + ABS_DBG_INF_HEAD_SIZE] = {0};
	...
	mt_osal_snprintf(temp, ABS_BUF_DEBUG_LEN, "%s%04d%d%04d%s%s",
...
{
	MT_U8 *pos = MT_NULL, *pmem = MT_NULL;
	MT_U8  u8FuncNumBuf[ABS_DBG_INF_DATA_WIDTH + 2] = {0}, u8StrLenBuf[ABS_DBG_INF_DATA_WIDTH + 1] = {0};
	mt_u32 u32FuncNum, u32StrLen = 0;
	MT_U8  u8DbgLevel;
	mt_s32 right, left, ret = 0;
	...
	while (left >= 0) {
		...
		if (right > 0) {
			...
			mt_osal_strncpy(u8FuncNumBuf, pos - ABS_DBG_INF_DATA_LEN, ABS_DBG_INF_DATA_WIDTH);
			u8DbgLevel = *(pos - ABS_DBG_INF_DATA_LEN + ABS_DBG_INF_DATA_WIDTH);
			mt_osal_strncpy(u8StrLenBuf, pos - ABS_DBG_INF_DATA_LEN + ABS_DBG_INF_DATA_WIDTH + 1,
			...
			if (!pos) {
				printk("Find Over.\n");
				ABS_DBG_LOCK(pABSdbg->dbg_lock);
				pABSdbg->dbg_rflag = MT_FALSE;
				ABS_DBG_UNLOCK(pABSdbg->dbg_lock);

				return -EPERM;
			}

			if (pos - pABSdbg->pdbg_addr < ABS_DBG_INF_DATA_LEN) {
				MT_U8 temp[ABS_DBG_INF_DATA_LEN + 1] = {0};

				mt_osal_strncpy(temp, pABSdbg->pdbg_addr + pABSdbg->dbg_mem_size -
								(ABS_DBG_INF_DATA_LEN - (pos - pABSdbg->pdbg_addr)),
								ABS_DBG_INF_DATA_LEN - (pos - pABSdbg->pdbg_addr));

				mt_osal_strncat(temp, pABSdbg->pdbg_addr, pos - pABSdbg->pdbg_addr);

				mt_osal_strncpy(u8FuncNumBuf, temp, ABS_DBG_INF_DATA_WIDTH);
				u8DbgLevel = *(temp + ABS_DBG_INF_DATA_WIDTH);
				mt_osal_strncpy(u8StrLenBuf, temp + ABS_DBG_INF_DATA_WIDTH + 1, ABS_DBG_INF_DATA_WIDTH);
				//printk("temp :%s\n", temp);
			} else {
				mt_osal_strncpy(u8FuncNumBuf, pos - ABS_DBG_INF_DATA_LEN, ABS_DBG_INF_DATA_WIDTH);
				u8DbgLevel = *(pos - ABS_DBG_INF_DATA_LEN + ABS_DBG_INF_DATA_WIDTH);
		...
		if ((u32FuncNum >= 0 && u32FuncNum <= ABS_FUNC_15)
				...
				(u8DbgLevel >= ABS_DBG_INFO && u8DbgLevel < ABS_DBG_END)) {
			if ( ((1 << u32FuncNum) & pABSdbg->dbg_mask) && (u8DbgLevel >= pABSdbg->dbg_level)) {
				MT_U8 buf_out[ABS_DBG_MEM_IN_BUF_SIZE + ABS_DBG_INF_HEAD_SIZE + 8] = {0};

				switch (u8DbgLevel) {
					case ABS_DBG_INFO:
						mt_osal_strncpy(buf_out, "INFO : ", 7);
						break;
					case ABS_DBG_WARN:
						mt_osal_strncpy(buf_out, "WARN : ", 7);
						break;
					case ABS_DBG_ERROR:
						mt_osal_strncpy(buf_out, "ERR  : ", 7);
						break;
					case ABS_DBG_FATAL:
						mt_osal_strncpy(buf_out, "FATAL: ", 7);
						break;
					default :
				...
				mt_osal_strncat(buf_out, pFuncName[u32FuncNum], ABS_DBG_INF_DATA_LEN);

				if (pmem + ABS_DBG_INF_FUN_NO >= pABSdbg->pdbg_addr + pABSdbg->dbg_mem_size) {
					mt_osal_strncat(buf_out, pmem + ABS_DBG_INF_FUN_NO - pABSdbg->dbg_mem_size, u32StrLen);
				} else if (pmem + ABS_DBG_INF_FUN_NO + u32StrLen >= pABSdbg->pdbg_addr + pABSdbg->dbg_mem_size) {
					mt_osal_strncat(buf_out, pmem + ABS_DBG_INF_FUN_NO, pABSdbg->pdbg_addr +
									pABSdbg->dbg_mem_size - (pmem + ABS_DBG_INF_FUN_NO));

					mt_osal_strncat(buf_out, pABSdbg->pdbg_addr, u32StrLen + pmem + ABS_DBG_INF_FUN_NO -
									(pABSdbg->pdbg_addr + pABSdbg->dbg_mem_size));
				} else {
					mt_osal_strncat(buf_out, pmem + ABS_DBG_INF_FUN_NO, u32StrLen);
				}
				...
				switch (pABSdbg->dbg_type) {
					...
					case DBG_ALL:
						ABS_PTK("%s", buf_out);
						if (pABSdbg->pdbg_fp) {
							//ret = MT_DRV_FILE_Write(pABSdbg->pdbg_fp, buf_out, strlen(buf_out));
						}
						break;
	...
	while (pos < temp + ABS_BUF_DEBUG_LEN) {
		MT_OSAL_Strncat(u8FuncNumBuf, pos - 9, 4);
		MT_OSAL_Strncat(u8StrLenBuf, pos - 4, 4);
		u32FuncNum = simple_strtoul(u8FuncNumBuf, NULL, 10);
		u32StrLen = simple_strtoul(u8StrLenBuf, NULL, 10);
		...
		if (u32FuncNum >= 0 && u32FuncNum <= ABS_FUNC_15) {
			...
			if ( (1 << u32FuncNum) & pABSdbg->dbg_mask) {
				MT_U8 buf_out[ABS_DBG_MEM_IN_BUF_SIZE + ABS_DBG_INF_HEAD_SIZE] = {0};
...
mt_s32 ABS_DbgInit(ABS_DBG_S_PTR pABSdbg, mt_mod_id_e module_id)
{
	...
	pABSdbg->pdbg_addr = (MT_U8 *)ABS_KMALLOC((mt_u32)module_id, pABSdbg->dbg_mem_size + 1);
	if (!pABSdbg->pdbg_addr) {
		ABS_ERR(pABSdbg->module_id, "ABS malloc memory space failed! addr(0x%p) is error\n", pABSdbg->pdbg_addr);
		pABSdbg->pdbg_addr = MT_NULL;
		return MT_FAILURE;
	}
	ABS_MEMSET(pABSdbg->pdbg_addr, 0, pABSdbg->dbg_mem_size);
