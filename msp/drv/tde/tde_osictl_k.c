...
//#define TDE_DEBUG_DISABLE_3
#if defined(TDE_DEBUG_DISABLE) || defined(TDE_DEBUG_DISABLE_3)
#define DUMP_LOG \
    do {         \
    } while (0)
...
mt_void TdeOsiPrintDrvSurface(TDE_DRV_SURFACE_S *p_surface)
{
    if (p_surface == NULL) {
  return;
    }

    TDE_LOG("DrvSurface: [%x][%x][%x][%x] [%x][%x][%x][%x] [%x][%x][%x]\n", p_surface->u32PhyAddr, p_surface->enColorFmt, p_surface->u32Pitch, p_surface->u32Width, p_surface->u32Height, p_surface->enHScan, p_surface->enVScan, p_surface->u32CbCrPhyAddr, p_surface->u32CbCrPitch, p_surface->enRgbOrder, p_surface->bAlphaMax255);
}

mt_void TdeOsiPrintSurface(TDE2_SURFACE_S *p_surface)
{
    if (p_surface == NULL) {
  return;
    }

    TDE_LOG("Surface: [%lx][%x][%x][%x][%x] [%lx][%x][%lx][%x] [%x][%x][%x][%x]\n", p_surface->u32PhyAddr, p_surface->enColorFmt, p_surface->u32Stride, p_surface->u32Width, p_surface->u32Height, p_surface->pu8ClutPhyAddr, p_surface->bYCbCrClut, p_surface->u32CbCrPhyAddr, p_surface->u32CbCrStride, p_surface->u8Alpha0, p_surface->u8Alpha1, p_surface->bAlphaExt1555, p_surface->bAlphaMax255);
}

mt_void TdeOsiPrintMBSurface(TDE2_MB_S *p_surface)
{
    if (p_surface == NULL) {
  return;
    }

    TDE_LOG("MBSurface: [%x][%x][%x][%x][%x][%x][%x]\n", p_surface->enMbFmt, p_surface->u32YPhyAddr, p_surface->u32YStride, p_surface->u32YWidth, p_surface->u32YHeight, p_surface->u32CbCrPhyAddr, p_surface->u32CbCrStride);
}

mt_void TdeOsiPrintRect(TDE2_RECT_S *p_rect)
{
    if (p_rect == NULL) {
  return;
    }

    TDE_LOG("Rect: [%x][%x][%x][%x]\n", p_rect->s32Xpos, p_rect->s32Ypos, p_rect->u32Width, p_rect->u32Height);
}

mt_void TdeOsiPrintOption(TDE2_OPT_S *p_opt)
{
    if (p_opt == NULL) {
  return;
    }

    TDE_LOG("Opt: [%x][%x][%x][%x] [%x][%x][%x][%x] [%x][%x][%x][%x] [%x][%x]\n", p_opt->enAluCmd, p_opt->enRopCode_Color, p_opt->enRopCode_Alpha, p_opt->enColorKeyMode, p_opt->enClipMode, p_opt->enDeflickerMode, p_opt->bResize, p_opt->enFilterMode, p_opt->enMirror, p_opt->bClutReload, p_opt->u8GlobalAlpha, p_opt->enOutAlphaFrom, p_opt->u32Colorize, p_opt->enGsBlur);
    TDE_LOG("Blend: [%x][%x][%x][%x] [%x][%x][%x]\n", p_opt->stBlendOpt.eBlendCmd, p_opt->stBlendOpt.eSrc1BlendMode, p_opt->stBlendOpt.eSrc2BlendMode, p_opt->stBlendOpt.bSrc1AlphaPremulti, p_opt->stBlendOpt.bSrc2AlphaPremulti, p_opt->stBlendOpt.bPixelAlphaEnable, p_opt->stBlendOpt.bGlobalAlphaEnable);
}
...
#define TDE_CALC_MAX_SLICE_NUM(bFF, vstep, in_w) \
    ((MT_FALSE == (bFF) && TDE_NO_SCALE_STEP == (vstep)) ? (1) : ((in_w) / 118 + 5)) /*AI7D02711*/

#define TDE_FILLUP_RECT_BY_DRVSURFACE(rect, drvSur) \
    do {                                            \
  (rect).s32Xpos = (mt_s32)(drvSur).u32Xpos;  \
  (rect).s32Ypos = (mt_s32)(drvSur).u32Ypos;  \
...
#define TDE_SET_CLIP(phwNode, pstOpt)                                                                    \
  ...
  if (TDE2_CLIPMODE_NONE != (pstOpt)->enClipMode) {                                                \
      ...
      stClipCmd.u16ClipStartX = (mt_u16)(pstOpt)->stClipRect.s32Xpos;                              \
      stClipCmd.u16ClipStartY = (mt_u16)(pstOpt)->stClipRect.s32Ypos;                              \
      stClipCmd.u16ClipEndX = ((pstOpt)->stClipRect.s32Xpos + (pstOpt)->stClipRect.u32Width - 1);  \
      stClipCmd.u16ClipEndY = (pstOpt)->stClipRect.s32Ypos + (pstOpt)->stClipRect.u32Height - 1;   \
      stClipCmd.bInsideClip = (TDE2_CLIPMODE_INSIDE == (pstOpt)->enClipMode) ? MT_TRUE : MT_FALSE; \
      if (TdeHalNodeSetClipping(phwNode, &stClipCmd) < 0)                                          \
...
STATIC MT_BOOL s_bRegionDeflicker = MT_FALSE;
/****************************************************************************/
...
/* AI7D02649 beg */
STATIC INLINE mt_s32 TdeOsiSetClutOpt(TDE2_SURFACE_S *pClutSur, TDE2_SURFACE_S *pOutSur, TDE_CLUT_USAGE_E *penClutUsage,
                                      MT_BOOL bClutReload, TDE_HWNode_S *pstHWNode);
STATIC INLINE mt_s32 TdeOsiSetClutOpt2(TDE2_SURFACE_S *pClutSur, TDE2_SURFACE_S *pOutSur, TDE_CLUT_USAGE_E *penClutUsage,
                                      MT_BOOL bClutReload, TDE_HWNode_S *pstHWNode);

STATIC INLINE TDE_CLUT_USAGE_E TdeOsiGetClutUsage(TDE2_COLOR_FMT_E enSrcFmt, TDE2_COLOR_FMT_E enDstFmt);
/* AI7D02649 end */

STATIC mt_s32 TdeOsiGetScanInfo(TDE2_SURFACE_S *pSrc, TDE2_RECT_S *pstSrcRect,
...
STATIC mt_s32 TdeOsiGetInterRect(TDE2_RECT_S *pRect1, TDE2_RECT_S *pRect2, TDE2_RECT_S *pInterRect);

STATIC INLINE mt_s32 TdeOsiSetMbPara(TDE_HANDLE s32Handle,
                                     TDE2_MB_S *pstMB, TDE2_RECT_S *pstMbRect,
                                     TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect,
                                     TDE2_MBOPT_S *pMbOpt);

STATIC INLINE mt_s32 TdeOsiAdjQuality(TDE_DRV_SURFACE_S *pS1Tmp, TDE_DRV_SURFACE_S *pS2Tmp,
                                      TDE2_MB_S *pstMB, TDE2_RECT_S *pstMbRect,
                                      TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect,
                                      TDE_PIC_MODE_E enPicMode, TDE2_MBOPT_S *pMbOpt);

STATIC INLINE mt_u32 TdeOsiMbCalcHStep(mt_u32 u32Wi, mt_u32 u32Wo,
                                       TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt, MT_BOOL bCbCr);

STATIC INLINE mt_u32 TdeOsiMbCalcVStep(mt_u32 u32Hi, mt_u32 u32Ho,
                                       TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt, MT_BOOL bCbCr);

STATIC INLINE mt_s32 TdeOsiMbCalcHOffset(mt_u32 u32Xi, TDE2_MB_COLOR_FMT_E enFmt, MT_BOOL bCbCr);

STATIC INLINE mt_s32 TdeOsiMbCalcVOffset(mt_u32 u32Yi, TDE2_MB_COLOR_FMT_E enFmt,
                                         TDE_PIC_MODE_E enPicMode, MT_BOOL bCbCr);

STATIC INLINE mt_u32 TdeOsiCalcNoLapSlice(TDE_SLICE_INFO *pSliceInfo, mt_u32 u32MaxNum, mt_u32 u32Xi,
                                          mt_u32 u32Xo, mt_u32 u32BmpW, /*AI7D02711*/
                                          TDE_FILTER_OPT *pstFilterOpt);

STATIC INLINE mt_u32 TdeOsiCalcOverLapSlice(TDE_SLICE_INFO *pSliceInfo, mt_u32 u32MaxNum, mt_u32 u32BmpXi, mt_u32 u32BmpWi,
                                            mt_u32 u32BmpXo, mt_u32 u32BmpWo, /*AI7D02711*/
                                            TDE_FILTER_OPT *pstFilterOpt);

STATIC mt_s32 TdeOsiSetFilterChildNode(TDE_HANDLE s32Handle, TDE_HWNode_S *pNode,
                                       TDE2_RECT_S *pInRect,
                                       TDE2_RECT_S *pOutRect,
                                       TDE2_DEFLICKER_MODE_E enDeflickerMode,
                                       TDE_FILTER_OPT *pstFilterOpt);

STATIC mt_s32 TdeOsiSetFilterNode(TDE_HANDLE s32Handle, TDE_HWNode_S *pNode,
...
STATIC mt_s32 TdeOsi1SourceFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstDst,
                                TDE2_RECT_S *pstDstRect, TDE2_FILLCOLOR_S *pstFillColor,
                                TDE2_OPT_S *pstOpt);
#ifndef TDE_BOOT
STATIC mt_s32 TdeOsi2SourceFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc,
                                TDE2_RECT_S *pstSrcRect,
                                TDE2_SURFACE_S *pstDst,
                                TDE2_RECT_S *pstDstRect, TDE2_FILLCOLOR_S *pstFillColor,
                                TDE2_OPT_S *pstOpt);
#endif
STATIC INLINE mt_s32 TdeOsiCheckSurface(TDE2_SURFACE_S *pstSurface, TDE2_RECT_S *pstRect);
STATIC INLINE mt_s32 TdeOsiCheckMbSurface(TDE2_MB_S *pstMbSurface, TDE2_RECT_S *pstRect);
STATIC INLINE mt_s32 TdeOsiCheckYc422RPara(TDE2_SURFACE_S *pstSrcSurface, TDE2_RECT_S *pstSrcRect,
                                           TDE2_SURFACE_S *pstDstSurface, TDE2_RECT_S *pstDstRect,
                                           MT_BOOL bDeflicker, MT_BOOL bResize);
...
STATIC INLINE mt_void TdeOsiConvertSurface(TDE2_SURFACE_S *pstSur, TDE2_RECT_S *pstRect,
                                           TDE_SCANDIRECTION_S *pstScanInfo,
                                           TDE_DRV_SURFACE_S *pstDrvSur,
                                           TDE2_RECT_S *pstOperationArea);

STATIC mt_s32 TdeOsiConvertMbSurface(TDE2_MB_S *pstMB, TDE2_RECT_S *pstMbRect,
                                     TDE_DRV_SURFACE_S *pstDrvY,
                                     TDE_DRV_SURFACE_S *pstDrvCbCr,
                                     TDE_PIC_MODE_E enPicMode, MT_BOOL bCbCr2Opt);

STATIC INLINE mt_void TdeOsiCalcMbFilterOpt(TDE_FILTER_OPT *pstFilterOpt, TDE2_MB_COLOR_FMT_E enInFmt,
                                            TDE2_MB_COLOR_FMT_E enOutFmt,
                                            TDE2_RECT_S *pInRect, TDE2_RECT_S *pOutRect, MT_BOOL bCbCr,
                                            MT_BOOL bCus, TDE_PIC_MODE_E enPicMode);

STATIC mt_s32 TdeOsiSetClipPara(TDE2_SURFACE_S *pstBackGround, TDE2_RECT_S *pstBGRect,
...
STATIC mt_s32 TdeOsiMbSetClipPara(TDE2_RECT_S *pstDstRect, TDE2_MBOPT_S *pstMbOpt,
                                  TDE_HWNode_S *pstHwNode);

STATIC mt_s32 TdeOsiSetBaseOptParaForBlit(TDE2_OPT_S *pstOpt, TDE2_SURFACE_S *pstSrc1, TDE2_SURFACE_S *pstSrc2,
                                          TDE_OPERATION_CATEGORY_E enOptCategory, TDE_HWNode_S *pstHwNode);

STATIC mt_s32 TdeOsiAdjClipPara(TDE_HWNode_S *pHWNode);
STATIC INLINE mt_s32 TdeOsiSetDeflickerPara(TDE_HWNode_S *pstHwNode, TDE2_DEFLICKER_MODE_E enDeflickerMode, TDE_FILTER_OPT *pstFilterOpt);

STATIC mt_s32 TdeOsiSetNodeFinish(TDE_HANDLE s32Handle, TDE_HWNode_S *pHWNode,
                                  mt_u32 u32WorkBufNum, TDE_NODE_SUBM_TYPE_E enSubmType);

STATIC INLINE mt_s32 TdeOsiCheckResizePara(mt_u32 u32InWidth, mt_u32 u32InHeight,
                                           mt_u32 u32OutWidth, mt_u32 u32OutHeight);
STATIC INLINE MT_BOOL TdeOsiWhetherContainAlpha(TDE2_COLOR_FMT_E enColorFmt);
STATIC INLINE mt_void TdeOsiSetFilterOptAdjInfo(TDE_DRV_SURFACE_S *pInSur, TDE_DRV_SURFACE_S *pOutSur,
                                                TDE_FILTER_OPT *pstFilterOpt, TDE_CHILD_SCALE_MODE_E enScaleMode);
STATIC INLINE mt_void TdeOsiSetExtAlpha(TDE2_SURFACE_S *pstBackGround, TDE2_SURFACE_S *pstForeGround,
                                        TDE_HWNode_S *pstHwNode);
...
STATIC mt_s32 rgb2ycc(mt_u8 r, mt_u8 g, mt_u8 b, mt_u8 *y, mt_u8 *cb, mt_u8 *cr)
{
...
STATIC mt_s32 ycc2rgb(mt_u8 y, mt_u8 cb, mt_u8 cr, mt_u8 *r, mt_u8 *g, mt_u8 *b)
{
...
STATIC TDE2_FMT_BITOFFSET_S s_u8FmtBitAndOffsetArray[TDE2_COLOR_FMT_AYCbCr8888 + 1] =
    {
        { 0, 2, 3, 3, 8, 5, 2, 0 },   /* TDE2_COLOR_FMT_RGB233 */
...
*****************************************************************************/
STATIC mt_s32 TdeOsiColorConvert(TDE2_FILLCOLOR_S *pstFillColor, TDE2_SURFACE_S *pstSur, mt_u32 *pu32OutColor)
{
    mt_u8 a, r, g, b, y, cb, cr;
    ...
    if (((TDE2_COLOR_FMT_CLUT1 <= pstFillColor->enColorFmt) && (pstFillColor->enColorFmt <= TDE2_COLOR_FMT_A8)) || (pstFillColor->enColorFmt >= TDE2_COLOR_FMT_YCbCr422))
    {
    ...
    a = (pstFillColor->u32FillColor >> s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8AlphaOffset) & (0xff >> (8 - s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8AlphaBits));
    r = (pstFillColor->u32FillColor >> s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8RedOffset) & (0xff >> (8 - s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8RedBits));
    g = (pstFillColor->u32FillColor >> s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8GreenOffset) & (0xff >> (8 - s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8GreenBits));
    b = (pstFillColor->u32FillColor >> s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8BlueOffset) & (0xff >> (8 - s_u8FmtBitAndOffsetArray[pstFillColor->enColorFmt].u8BlueBits));

    if ((TDE2_COLOR_FMT_ARGB1555 <= pstFillColor->enColorFmt) && (pstFillColor->enColorFmt <= TDE2_COLOR_FMT_BGRA1555))
    {
        if (a)
        {
            a = pstSur->u8Alpha1;
...
STATIC INLINE mt_s32 TdeOsiCheckResizePara(mt_u32 u32InWidth, mt_u32 u32InHeight,
                                           mt_u32 u32OutWidth, mt_u32 u32OutHeight)
{
    TDE_LOG("%s IN:%d [%x][%x][%x][%x]\n", __FUNCTION__, __LINE__, u32InWidth, u32InHeight, u32OutWidth, u32OutHeight);

    if ((u32InWidth != u32OutWidth) && (u32InHeight != u32OutHeight)) // in aria , only support the H / V scaler
    {
        TDE_FUN_OUT;
        return MT_ERR_TDE_INVALID_PARA;
    }

    if ((TDE_MAX_RECT_WIDTH < u32InWidth) || (TDE_MAX_RECT_HEIGHT < u32InHeight) || (TDE_MAX_RECT_WIDTH < u32OutWidth) || (TDE_MAX_RECT_HEIGHT < u32OutHeight))
    {
...
STATIC TDE_COLORFMT_TRANSFORM_E TdeOsiGetFmtTransType(TDE2_COLOR_FMT_E enSrc2Fmt, TDE2_COLOR_FMT_E enDstFmt)
{
    ...
    TDE_LOG("%s:%d [%x][%x][%x][%x]\n", __FUNCTION__, __LINE__, enSrc2Fmt, enDstFmt, enSrcCategory, enDstCategory);
...
* Output:        none
* Return:        MT_TRUE: single ROP;MT_FALSE: non single ROP
* Others:        none
*****************************************************************************/
STATIC INLINE MT_BOOL TdeOsiIsSingleSrc2Rop(TDE2_ROP_CODE_E enRop)
{
    TDE_LOG("%s:%d [%x]\n", __FUNCTION__, __LINE__, enRop);
    switch (enRop)
    {
        case TDE2_ROP_BLACK:
...
STATIC INLINE TDE_CLUT_USAGE_E TdeOsiGetClutUsage(TDE2_COLOR_FMT_E enSrcFmt, TDE2_COLOR_FMT_E enDstFmt)
{
    ...
    TDE_LOG("%s:%d [%x][%x][%x]\n", __FUNCTION__, __LINE__, enSrcFmt, enDstFmt, enColorTransType);
                                      ...
                                      MT_BOOL bClutReload, TDE_HWNode_S *pstHWNode)
{
    ...
    if ((-1 != pClutSur->pu8ClutPhyAddr) && (0 != pClutSur->pu8ClutPhyAddr))
    {
        ...
        if ((!pClutSur->bYCbCrClut && TDE_COLORFMT_CATEGORY_YCbCr == enFmtCate) || (pClutSur->bYCbCrClut && TDE_COLORFMT_CATEGORY_ARGB == enFmtCate))
        {
        ...
        *penClutUsage = TdeOsiGetClutUsage(pClutSur->enColorFmt, pOutSur->enColorFmt); /*AI7D02757*/
        if (TDE_CLUT_CLUT_BYPASS > *penClutUsage)
        {
            ...
            if ((pClutSur->pu8ClutPhyAddr) % 4)
            {
                TDE_TRACE(TDE_KERN_INFO, "pClutSur->pu8ClutPhyAddr=%lx\n", pClutSur->pu8ClutPhyAddr);
                TDE_FUN_OUT;
                return MT_ERR_TDE_NOT_ALIGNED;
            }
    ...
    if ((-1 != pClutSur->pu8ClutPhyAddr) && (0 != pClutSur->pu8ClutPhyAddr))
    {
        ...
        *penClutUsage = TdeOsiGetClutUsage(pClutSur->enColorFmt, pOutSur->enColorFmt); /*AI7D02757*/
        if (TDE_CLUT_CLUT_BYPASS > *penClutUsage)
        {
            ...
            stClutCmd.pu8PhyClutAddr = pClutSur->pu8ClutPhyAddr;
            if (TdeHalNodeSetClutOpt2(pstHWNode, &stClutCmd, bClutReload) < 0)
            {
                TDE_FUN_OUT;
                return MT_ERR_TDE_UNSUPPORTED_OPERATION;
            }
                                                ...
                                                TDE_FILTER_OPT *pstFilterOpt, TDE_CHILD_SCALE_MODE_E enScaleMode)
{
    ...
    if ((TDE_CHILD_SCALE_MB_CONCA_L == enScaleMode) && (TDE_DRV_COLOR_FMT_YCbCr420MB == pInSur->enColorFmt || TDE_DRV_COLOR_FMT_YCbCr422MBH == pInSur->enColorFmt))
    {
        pstFilterOpt->bEvenStartInX = MT_TRUE;
        pstFilterOpt->bBadLastPix = MT_TRUE;
    }

    if (TDE_DRV_COLOR_FMT_YCbCr422 == pOutSur->enColorFmt)
    {
        pstFilterOpt->bEvenStartOutX = MT_TRUE;
    }
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiCheckSingleSrc2Opt(TDE2_COLOR_FMT_E enSrc2Fmt, TDE2_COLOR_FMT_E enDstFmt,
                                       TDE2_OPT_S *pstOpt)
{
    TDE_COLORFMT_TRANSFORM_E enColorTransType;
    ...
    if (pstOpt->enAluCmd & TDE2_ALUCMD_ROP)
    {
        ...
        if ((!TdeOsiIsSingleSrc2Rop(pstOpt->enRopCode_Alpha)) || (!TdeOsiIsSingleSrc2Rop(pstOpt->enRopCode_Color)))
        {
    ...
    if (TDE_COLORFMT_TRANSFORM_CLUT_CLUT == enColorTransType)
    {
        ...
        if ((pstOpt->enDeflickerMode != TDE2_DEFLICKER_MODE_NONE) || (pstOpt->bResize) || (pstOpt->enAluCmd & TDE2_ALUCMD_ROP) || (pstOpt->enAluCmd & TDE2_ALUCMD_COLORIZE)
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiCheckDoubleSrcOpt(TDE2_COLOR_FMT_E enSrc1Fmt, TDE2_COLOR_FMT_E enSrc2Fmt,
                                      TDE2_COLOR_FMT_E enDstFmt, TDE2_OPT_S *pstOpt)
{
    ...
    if ((TDE_COLORFMT_CATEGORY_BYTE <= enSrc1Category) || (TDE_COLORFMT_CATEGORY_BYTE <= enSrc2Category) || (TDE_COLORFMT_CATEGORY_BYTE <= enDstCategory))
    {
    ...
    if (TDE_COLORFMT_CATEGORY_ARGB == enSrc1Category || TDE_COLORFMT_CATEGORY_YCbCr == enSrc1Category)
    {
    ...
    {
        if ((TDE_COLORFMT_CATEGORY_CLUT != enSrc2Category) || (TDE_COLORFMT_CATEGORY_CLUT != enDstCategory))
        {
        ...
        if ((pstOpt->enDeflickerMode != TDE2_DEFLICKER_MODE_NONE) || (pstOpt->bResize) || (TDE2_ALUCMD_NONE != pstOpt->enAluCmd)
    ...
    else if ((TDE_COLORFMT_CATEGORY_An == enSrc1Category) && (TDE_COLORFMT_CATEGORY_An == enSrc2Category) && (TDE_COLORFMT_CATEGORY_An == enDstCategory))
    {
...
* Others:        none
*****************************************************************************/
STATIC mt_u16 TdeOsiDoubleSrcGetOptCode(TDE2_COLOR_FMT_E enSrc1Fmt, TDE2_COLOR_FMT_E enSrc2Fmt,
...
* Others:       none
*****************************************************************************/
STATIC mt_u16 TdeOsiSingleSrc2GetOptCode(TDE2_COLOR_FMT_E enSrc2Fmt, TDE2_COLOR_FMT_E enDstFmt)
{
    mt_u16 u16Code = 0;
    TDE_COLORFMT_TRANSFORM_E enColorTransType;
...
* Others:        none
*****************************************************************************/
STATIC mt_void TdeOsiGetConvbyCode(mt_u16 u16Code, TDE_DRV_CONV_MODE_CMD_S *pstConv)
{
    TDE_FUN_IN;
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiGetbppByFmt(TDE2_COLOR_FMT_E enFmt)
{
    TDE_LOG("%s:%d [%x]\n", __FUNCTION__, __LINE__, enFmt);
    switch (enFmt) {
...
*****************************************************************************/
STATIC mt_s32 TdeOsiGetScanInfo(TDE2_SURFACE_S *pSrc, TDE2_RECT_S *pstSrcRect, TDE2_SURFACE_S *pstDst,
                                TDE2_RECT_S *pstDstRect, TDE2_OPT_S *pstOpt,
                                TDE_SCANDIRECTION_S *pstSrcDirection,
                                TDE_SCANDIRECTION_S *pstDstDirection)
{
    mt_u32 u32SrcAddr;
    mt_u32 u32DstAddr;
    MT_BOOL bFilter = MT_FALSE;
    TDE2_MIRROR_E enMirror = TDE2_MIRROR_NONE;
    mt_s32 s32SrcdBpp, s32DstBpp;
    ...
    else
    {
        ...
        u32SrcAddr = pSrc->u32PhyAddr + pstSrcRect->s32Ypos * pSrc->u32Stride + ((pstSrcRect->s32Xpos * s32SrcdBpp) >> 3);
        u32DstAddr = pstDst->u32PhyAddr + pstDstRect->s32Ypos * pstDst->u32Stride + ((pstDstRect->s32Xpos * s32DstBpp) >> 3);

        /* source is above of target or on the left of the same direction */
        // if (u32SrcAddr <= u32DstAddr)
        if (0) //in aria , no this function
        {
            TDE_LINE;
...
STATIC mt_s32 TdeOsiGetInterRect(TDE2_RECT_S *pRect1, TDE2_RECT_S *pRect2, TDE2_RECT_S *pInterRect)
{
    mt_s32 s32Left, s32Top, s32Right, s32Bottom;
    mt_s32 s32Right1, s32Bottom1, s32Right2, s32Bottom2;
...
* Others:        none
*****************************************************************************/
STATIC INLINE MT_BOOL TdeOsiIsRect1InRect2(TDE2_RECT_S *pRect1, TDE2_RECT_S *pRect2)
{
    mt_s32 s32Right1, s32Bottom1, s32Right2, s32Bottom2;
    ...
    if ((pRect1->s32Xpos >= pRect2->s32Xpos) && (pRect1->s32Ypos >= pRect2->s32Ypos) && (s32Right1 <= s32Right2) && (s32Bottom1 <= s32Bottom2))
    {
        return MT_TRUE;
    }
                                     ...
                                     TDE2_MBOPT_S *pMbOpt)
{
    ...
    mt_u32 u32WorkBufNum = 0;
    TDE2_OUTALPHA_FROM_E enAlphaFrom = TDE2_OUTALPHA_FROM_NORM;
    TDE2_CSC_OPT_S stCscOpt = { 0 };

    TDE_FUN_IN;

    TdeOsiPrintMBSurface(pstMB);
    TdeOsiPrintRect(pstMbRect);

    TdeOsiPrintSurface(pstDst);
    TdeOsiPrintRect(pstDstRect);
    ...
    if ((TDE2_MB_COLOR_FMT_MP1_YCbCr420MBP == pstMB->enMbFmt) || (TDE2_MB_COLOR_FMT_MP2_YCbCr420MBP == pstMB->enMbFmt) || (TDE2_MB_COLOR_FMT_MP2_YCbCr420MBI == pstMB->enMbFmt))
    {
    ...
    stConv.dst_fmt = TdeOsiGetFmtCategory(pstDst->enColorFmt);
    stConv.fmt_mask |= 0x1 << 4;
    ...
    if (TDE2_MB_COLOR_FMT_JPG_YCbCr400MBP == pstMB->enMbFmt)
    {
        ...
        if (TdeOsiConvertMbSurface(pstMB, pstMbRect, &stDrvS1, MT_NULL, TDE_FRAME_PIC_MODE, MT_FALSE) < 0)
        {
        ...
        stConv.src1_fmt = TDE_COLORFMT_CATEGORY_MB;
        stConv.fmt_mask |= 0x1 << 1;
        ...
        TdeHalNodeSetGlobalAlpha(pstHWNode, pMbOpt->u8OutAlpha, MT_TRUE);
        ...
        if ((TDE2_MBRESIZE_NONE == pMbOpt->enResize) && (MT_FALSE == pMbOpt->bDeflicker))
        {
        ...
        else
        {
            ...
            TdeOsiCalcMbFilterOpt(&stFilterOpt, pstMB->enMbFmt, TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP,
                                  pstMbRect, pstDstRect, MT_FALSE, MT_FALSE, TDE_FRAME_PIC_MODE);
    ...
    if (TdeOsiConvertMbSurface(pstMB, pstMbRect, &stDrvS1, &stDrvS2, enPicMode, MT_FALSE) < 0)
    {
    ...
    switch (pMbOpt->enResize)
    {
        case TDE2_MBRESIZE_QUALITY_HIGH:
        {
            ...
            TdeHalNodeSetMbMode(pstHWNode, &stMbCmd);
            TdeOsiCalcMbFilterOpt(&stFilterOpt, pstMB->enMbFmt, TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP,
                                  &stAdjFieldRect, pstDstRect, MT_FALSE, MT_FALSE, enPicMode);
            TDE_FILLUP_RECT_BY_DRVSURFACE(stInRect, stDrvS1);
            ...
            TdeOsiCalcMbFilterOpt(&stFilterOpt, pstMB->enMbFmt, TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP,
                                  &stAdjFieldRect, pstDstRect, MT_TRUE, MT_FALSE, enPicMode);
            ...
            if ((s32Ret = TdeOsiSetFilterChildNode(s32Handle, pstHWNodePass2, &stInRect, &stOutRect, MT_FALSE /*pMbOpt->bDeflicker*/,
                                                   &stFilterOpt)) < 0)
            {
                TdeOsiListPutPhyBuff(u32WorkBufNum);
            ...
            TdeHalNodeSetGlobalAlpha(pstHWNodePass3, pMbOpt->u8OutAlpha, MT_TRUE);
        ...
        case TDE2_MBRESIZE_QUALITY_MIDDLE:
        {
            ...
            TdeOsiCalcMbFilterOpt(&stFilterOpt, pstMB->enMbFmt, TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP,
                                  &stAdjFieldRect, &stAdjFieldRect, MT_TRUE, MT_TRUE, enPicMode);
            TDE_FILLUP_RECT_BY_DRVSURFACE(stInRect, stDrvS2);
            TDE_FILLUP_RECT_BY_DRVSURFACE(stOutRect, stDrvS1Tmp);
            ...
            if ((s32Ret = TdeOsiSetFilterChildNode(s32Handle, pstHWNode, &stInRect, &stOutRect, MT_FALSE,
                                                   &stFilterOpt)) < 0)
            {
                TdeOsiListPutPhyBuff(u32WorkBufNum);
            ...
            TdeHalNodeSetGlobalAlpha(pstHWNodePass2, pMbOpt->u8OutAlpha, MT_TRUE);
            ...
            TdeOsiCalcMbFilterOpt(&stFilterOpt, pstMB->enMbFmt, TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP,
                                  &stAdjFieldRect, pstDstRect, MT_FALSE, MT_FALSE, TDE_FRAME_PIC_MODE);
            TDE_FILLUP_RECT_BY_DRVSURFACE(stInRect, stDrvS1);
...
STATIC INLINE mt_s32 TdeOsiAdjQuality(TDE_DRV_SURFACE_S *pS1Tmp, TDE_DRV_SURFACE_S *pS2Tmp,
                                      ...
                                      TDE_PIC_MODE_E enPicMode, TDE2_MBOPT_S *pMbOpt)
{
    mt_u32 u32WorkBufferNum = 0;

    TDE_FUN_IN;
    /* needn't zoom */
    if ((TDE2_MBRESIZE_NONE == pMbOpt->enResize) || ((pstMbRect->u32Height == pstDstRect->u32Height) && (pstMbRect->u32Width == pstDstRect->u32Width)))
    {
    ...
    else /* need zoom */
    {
        ...
        if (TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP == pstMB->enMbFmt && TDE2_COLOR_FMT_YCbCr422 != pstDst->enColorFmt)
        {
            pMbOpt->enResize = TDE2_MBRESIZE_QUALITY_LOW;
            TDE_FUN_OUT;
            return 0;
    ...
    if (TDE2_MBRESIZE_QUALITY_HIGH == pMbOpt->enResize)
    {
        ...
        else
        {
            ...
            TDE_ASSERT(MT_NULL != pS2Tmp->u32PhyAddr);
        }
...
STATIC INLINE mt_u32 TdeOsiMbCalcHStep(mt_u32 u32Wi, mt_u32 u32Wo,
                                       TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt, MT_BOOL bCbCr)
{
    TDE_LOG("%s:%d:  [%x][%x][%x][%x][%x]\n", __FUNCTION__, __LINE__, u32Wi, u32Wo, enInFmt, enOutFmt, bCbCr);
    if (1 >= u32Wo)
    {
        return 0;
    }
...
STATIC INLINE mt_u32 TdeOsiMbCalcVStep(mt_u32 u32Hi, mt_u32 u32Ho,
                                       TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt, MT_BOOL bCbCr)
{
    TDE_LOG("%s:%d:  [%x][%x][%x][%x][%x]\n", __FUNCTION__, __LINE__, u32Hi, u32Ho, enInFmt, enOutFmt, bCbCr);
    if (1 >= u32Ho)
    {
        return 0;
    }
...
STATIC INLINE mt_s32 TdeOsiMbCalcHOffset(mt_u32 u32Xi, TDE2_MB_COLOR_FMT_E enFmt, MT_BOOL bCbCr)
{
    TDE_LOG("%s:%d:  [%x][%x][%x]\n", __FUNCTION__, __LINE__, u32Xi, enFmt, bCbCr);
    if (!bCbCr) {
  return 0;
    }

    if ((TDE2_MB_COLOR_FMT_MP1_YCbCr420MBP == enFmt) || (TDE2_MB_COLOR_FMT_JPG_YCbCr420MBP == enFmt))
    {
        if (1 == (u32Xi & 0x1))
        {
            return (mt_s32)(1 << (TDE_FLOAT_BITLEN - 2)); /* 0.25 * 2^12 */
        }

        return (mt_s32)((-1) << (TDE_FLOAT_BITLEN - 2)); /* -0.25 * 2^12 */
    }

    if ((TDE2_MB_COLOR_FMT_MP2_YCbCr420MBP == enFmt) || (TDE2_MB_COLOR_FMT_MP2_YCbCr420MBI == enFmt) || (TDE2_MB_COLOR_FMT_JPG_YCbCr422MBHP == enFmt))
    {
        if (1 == (u32Xi & 0x1))
        {
            return (mt_s32)(1 << (TDE_FLOAT_BITLEN - 1)); /* 0.5 * 2^12 */
        }
    }
...
STATIC INLINE mt_s32 TdeOsiMbCalcVOffset(mt_u32 u32Yi, TDE2_MB_COLOR_FMT_E enFmt,
                                         TDE_PIC_MODE_E enPicMode, MT_BOOL bCbCr)
{
    TDE_LOG("%s:%d:  [%x][%x][%x][%x]\n", __FUNCTION__, __LINE__, u32Yi, enFmt, enPicMode, bCbCr);
    if (bCbCr)
    {
        ...
        else if ((TDE2_MB_COLOR_FMT_MP2_YCbCr420MBI == enFmt) || (TDE2_MB_COLOR_FMT_MP1_YCbCr420MBP == enFmt) || (TDE2_MB_COLOR_FMT_MP2_YCbCr420MBP == enFmt))
        {
            switch (u32Yi & 0x3)
            {
                case 0:
                {
                    return (mt_s32)((-1) << (TDE_FLOAT_BITLEN - 3)); /* -0.125 * 2^12 */
                }
                case 1:
                {
                    return (mt_s32)(1 << (TDE_FLOAT_BITLEN - 3)); /* 0.125 * 2^12 */
                ...
                case 2:
                {
                    return (mt_s32)(3 << (TDE_FLOAT_BITLEN - 3)); /* 0.375 * 2^12 */
                }
                case 3:
                {
                    return (mt_s32)(5 << (TDE_FLOAT_BITLEN - 3)); /* 0.625 * 2^12 */
                }
                default:
                    return 0;
            }
    ...
    {
        if ((TDE2_MB_COLOR_FMT_MP2_YCbCr420MBI == enFmt) || (TDE2_MB_COLOR_FMT_MP1_YCbCr420MBP == enFmt) || (TDE2_MB_COLOR_FMT_MP2_YCbCr420MBP == enFmt))
        {
            switch (u32Yi & 0x3)
            {
                case 0:
...
*****************************************************************************/
STATIC INLINE mt_u32 TdeOsiCalcNoLapSlice(TDE_SLICE_INFO *pSliceInfo, mt_u32 u32MaxNum, mt_u32 u32Xi, mt_u32 u32Xo,
                                          mt_u32 u32BmpW, TDE_FILTER_OPT *pstFilterOpt)
{
#define TDE_GET_STARTX_BY_SCANINFO(hScan, sliceW, x) \
    ((hScan) ? (mt_u32)((x) + (sliceW) - 1) : ((mt_u32)(x)))

    /* mt_u32 u32SliceNum; */
    mt_u32 u32BmpXo = u32Xo;
    mt_u32 u32OverLapPix = (pstFilterOpt->bEvenStartInX && pstFilterOpt->bEvenStartOutX) ? 4 : 2;
    mt_u32 n = 1;
    ...
    else
    {
        mt_u32 u32OfstWord;
        /* mt_u32 u32WFirst; */
        /* mt_u32 u32W64SliceNum;*/ /* not include first block */
        ...
        {
            if ((mt_s32)(u32BmpW - u32Xo + u32BmpXo) <= TDE_MAX_SLICE_WIDTH)
            {
                break;
            }
        ...
        if ((mt_s32)(u32BmpW - u32Xo + u32BmpXo) > 0) {
...
*****************************************************************************/
STATIC INLINE mt_u32 TdeOsiCalcOverLapSlice(TDE_SLICE_INFO *pSliceInfo, mt_u32 u32MaxNum, mt_u32 u32BmpXi, mt_u32 u32BmpWi,
                                            mt_u32 u32BmpXo, mt_u32 u32BmpWo, TDE_FILTER_OPT *pstFilterOpt)
{
    mt_u32 u32Xi = u32BmpXi;
    mt_u32 u32Wi;
    mt_u32 s32HOfst = pstFilterOpt->s32HOffset;
    mt_u32 u32Xo = u32BmpXo;
    mt_u32 u32Wo;
    mt_u32 u32OfstWord = 0;
    mt_u32 u32LeftWi = u32BmpWi;
    mt_u32 u32LeftWo = u32BmpWo;
    mt_u32 u32CalcWi = 0;
    mt_u32 u32BackPix = (pstFilterOpt->bEvenStartInX) ? 2 : 0; /* u32BackPix is 2 at h2 version */
    mt_u32 n;
    ...
    for (n = 0; n < u32MaxNum; n++) /*AI7D02711*/
    {
        ...
        if (0 > (mt_s32)((u32Wi - 5 - u32BackPix) * TDE_NO_SCALE_STEP - s32HOfst))
        {
            u32Wo = u32LeftWo;
        }
        else
        ...
        if (((u32BmpXi + u32BmpWi) <= (u32Xi + pSliceInfo[n].u32SliceWi)) || ((u32BmpXo + u32BmpWo) <= (u32Xo + u32Wo)))
        {
            n++;
            break;
        }
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiSetFilterChildNode(TDE_HANDLE s32Handle, TDE_HWNode_S *pNode, TDE2_RECT_S *pInRect,
                                       TDE2_RECT_S *pOutRect,
                                       TDE2_DEFLICKER_MODE_E enDeflickerMode,
                                       TDE_FILTER_OPT *pstFilterOpt)
{
...
#if 1 // for aria tde, aria tde donot support the
    TdeOsiSetNodeFinish(s32Handle, pNode, pstFilterOpt->u32WorkBufNum, TDE_NODE_SUBM_ALONE);
#else
    mt_void *pBuf;
    TDE_HWNode_S *pChildNode;
    TDE_SLICE_INFO *sliceInfo = MT_NULL;
    TDE_CHILD_INFO stChildInfo = { 0 };
    mt_u32 u32SliceNum = 1;
    MT_BOOL bDeflicker = (TDE2_DEFLICKER_MODE_NONE == enDeflickerMode) ? MT_FALSE : MT_TRUE;
    mt_u32 u32MaxSliceNum =
        TDE_CALC_MAX_SLICE_NUM(bDeflicker, pstFilterOpt->u32VStep, pInRect->u32Width);
    mt_u32 u32FirstNum;
    mt_s32 s32Ret;
    mt_s32 i;
    mt_u32 u32TDE_CLIP_START = 0; //pNode->u32TDE_CLIP_START;
    mt_u32 u32TDE_CLIP_STOP = 0;  //pNode->u32TDE_CLIP_STOP;
    ...
    if ((MT_FALSE == bDeflicker) && (TDE_NO_SCALE_STEP == pstFilterOpt->u32VStep))
    {
    ...
    else if ((TDE_NO_SCALE_STEP == pstFilterOpt->u32HStep) && ((bDeflicker) || (TDE_NO_SCALE_STEP != pstFilterOpt->u32VStep)))
    {
        TDE_TRACE(TDE_KERN_DEBUG, "NoLap slice block for filter!\n");
        u32SliceNum = TdeOsiCalcNoLapSlice(sliceInfo, u32MaxSliceNum, (mt_u32)(pInRect->s32Xpos),
                                           (mt_u32)(pOutRect->s32Xpos),
    ...
    else if ((TDE_NO_SCALE_STEP != pstFilterOpt->u32HStep) && ((bDeflicker) || (TDE_NO_SCALE_STEP != pstFilterOpt->u32VStep)))
    {
        ...
        u32SliceNum = TdeOsiCalcOverLapSlice(sliceInfo, u32MaxSliceNum, pInRect->s32Xpos,
                                             pInRect->u32Width, (mt_u32)pOutRect->s32Xpos,
    ...
    stChildInfo.u32Yi = (mt_u32)pInRect->s32Ypos;
    ...
    stChildInfo.u32Yo = (mt_u32)pOutRect->s32Ypos;
    ...
    {
        for (i = u32FirstNum + 1; i < (mt_s32)u32SliceNum; i++)
        {
...
{
    mt_s32 ori_in_width;   //original image width
    mt_s32 ori_in_height;  //original image height
    mt_s32 zme_out_width;  //output full image width
    mt_s32 zme_out_height; //output full image height

    mt_s32 update_instart_w; //the start_x of update area in original image
    mt_s32 update_instart_h; //the start_y of update area in original image
    mt_s32 update_in_width;  //the width of update area in original image
    mt_s32 update_in_height; //the height of update area in original image
} UpdateConfig;

typedef struct tagUpdateInfo
{
    mt_s32 zme_instart_w; //the start_x of needed readin area in original image
    mt_s32 zme_instart_h; //the start_y of needed readin area in original image
    mt_s32 zme_in_width;  //the width of needed readin area in original image
    mt_s32 zme_in_height; //the height of needed readin area in original image

    mt_s32 zme_outstart_w; //the start_x of needed update area in output image
    mt_s32 zme_outstart_h; //the start_y of needed update area in output image
    mt_s32 zme_out_width;  //the width of needed update area in output image
    mt_s32 zme_out_height; //the height of needed update area in output image

    mt_s32 zme_hphase;     //the start phase of horizontal scale
    mt_s32 zme_vphase;     //the start phase of vertical scale
    mt_s32 def_offsetup;   //the up offset of deflicker
    mt_s32 def_offsetdown; //the down offset of deflicker
} UpdateInfo;

STATIC void TdeOsiGetHUpdateInfo(UpdateConfig *reg, UpdateInfo *info, int scaler_en)
{
    mt_s32 zme_hinstart = 0, zme_hinstop = 0;
    mt_s32 zme_houtstart = 0, zme_houtstop = 0;
    mt_s32 update_hstart = reg->update_instart_w;
    mt_s32 update_hstop = update_hstart + reg->update_in_width - 1;
    mt_s32 zme_hphase = 0;
    mt_s32 ratio = (mt_s32)(4096 * (reg->ori_in_width - 1) / (reg->zme_out_width - 1) + 1 / 2);
    mt_s32 dratio = 4096 * (reg->zme_out_width - 1) / (reg->ori_in_width - 1);
...
{
    mt_s32 zme_vinstart = 0, zme_vinstop = 0;
    mt_s32 zme_voutstart = 0, zme_voutstop = 0;
    mt_s32 update_vstart = reg->update_instart_h;
    mt_s32 update_vstop = update_vstart + reg->update_in_height - 1;
    mt_s32 zme_vphase = 0;
    mt_s32 ratio = (mt_s32)(4096 * (reg->ori_in_height - 1) / (reg->zme_out_height - 1) + 1 / 2);
    mt_s32 dratio = 4096 * (reg->zme_out_height - 1) / (reg->ori_in_height - 1);
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiSetFilterNode(TDE_HANDLE s32Handle, TDE_HWNode_S *pNode, TDE2_SURFACE_S *pstForeGround, TDE2_SURFACE_S *pstDst,
                                  TDE2_RECT_S *pInRect, TDE2_RECT_S *pOutRect, TDE2_DEFLICKER_MODE_E enDeflickerMode, TDE_FILTER_OPT *pstFilterOpt)
{
    ...
    MT_BOOL bDeflicker;
    MT_BOOL bScale = MT_FALSE;

    TDE_FUN_IN;
    enInFmt = pstForeGround->enColorFmt;
    enOutFmt = pstDst->enColorFmt;
    bDeflicker = (TDE2_DEFLICKER_MODE_NONE == enDeflickerMode) ? MT_FALSE : MT_TRUE;
    ...
    {
        pstFilterOpt->bEvenStartInX = MT_TRUE; /* YCbCr422 ask for start width is even of each slice */
        pstFilterOpt->bBadLastPix = MT_TRUE;   /* YCbCr422 ask for conver 2 noneffective pixel points for each slice when blocking */
    }
    ...
    {
        pstFilterOpt->bVRing = MT_TRUE;
    }

    if (TDE_NO_SCALE_STEP > pstFilterOpt->u32HStep)
    {
        pstFilterOpt->bHRing = MT_TRUE;
    }

    pstFilterOpt->bCoefSym = MT_TRUE; /* arithmetic suply field, keeping open asymmetry */
    ...
    /* local deflicker */
    if (((pstForeGround->u32Width != pInRect->u32Width) || (pstForeGround->u32Height != pInRect->u32Height)) && s_bRegionDeflicker)
    {
        ...
        ret = TdeOsiSetFilterChildNode(s32Handle, pNode, &stUpdateInRect, &stUpdateOutRect, enDeflickerMode, pstFilterOpt);
        TDE_FUN_OUT;
        return ret;
    }
    else
    {
        ret = TdeOsiSetFilterChildNode(s32Handle, pNode, pInRect, pOutRect, enDeflickerMode, pstFilterOpt);
        TDE_FUN_OUT;
        return ret;
    }
...
* Others:        none
*****************************************************************************/
STATIC INLINE mt_void TdeOsiCalcMbFilterOpt(TDE_FILTER_OPT *pstFilterOpt, TDE2_MB_COLOR_FMT_E enInFmt, TDE2_MB_COLOR_FMT_E enOutFmt,
                                            TDE2_RECT_S *pInRect, TDE2_RECT_S *pOutRect, MT_BOOL bCbCr,
                                            MT_BOOL bCus, TDE_PIC_MODE_E enPicMode)
{
    TDE_FUN_IN;
    ...
    else
    {
        ...
        {
            pstFilterOpt->u32HStep = TdeOsiMbCalcHStep(pInRect->u32Width, pOutRect->u32Width, enInFmt, enOutFmt, MT_FALSE);
            pstFilterOpt->u32VStep = TdeOsiMbCalcVStep(pInRect->u32Height, pOutRect->u32Height, enInFmt, enOutFmt, MT_FALSE);
        }
        else
        {
...
//    else if (TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP != enOutFmt && TDE2_MB_COLOR_FMT_JPG_YCbCr444MBP != enInFmt && enOutFmt == enInFmt)
//    {
//        pstFilterOpt->s32HOffset = 0;
//        pstFilterOpt->s32VOffset = 0;
//    }
...
*****************************************************************************/
STATIC INLINE mt_void TdeOsiAdjPara4YCbCr422R(TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect, TDE2_FILLCOLOR_S *pstFillColor)
{
                                ...
                                TDE2_RECT_S *pstDstRect, TDE2_FILLCOLOR_S *pstFillColor, TDE2_OPT_S *pstOpt)
{
    ...
    TDE_FUN_IN;
    if ((MT_NULL == pstDst) || (MT_NULL == pstDstRect) || (MT_NULL == pstFillColor))
    {
...
*****************************************************************************/
#ifndef TDE_BOOT
STATIC mt_s32 TdeOsi2SourceFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc, TDE2_RECT_S *pstSrcRect,
                                TDE2_SURFACE_S *pstDst,
                                TDE2_RECT_S *pstDstRect, TDE2_FILLCOLOR_S *pstFillColor,
                                TDE2_OPT_S *pstOpt)
{
    ...
    TDE_FUN_IN;
    if ((NULL == pstDst) || (NULL == pstDstRect) || (NULL == pstFillColor) || (NULL == pstOpt) || (NULL == pstSrc) || (NULL == pstSrcRect))
    {
        TDE_FUN_OUT;
        return MT_ERR_TDE_NULL_PTR;
    }
    ...
    if (TDE2_COLORKEY_MODE_FOREGROUND == pstOpt->enColorKeyMode)
    {
        ...
        if (TDE_CLUT_COLOREXPENDING != enClutUsage && TDE_CLUT_CLUT_BYPASS != enClutUsage)
        {
...
*****************************************************************************/
STATIC INLINE mt_s32 TdeOsiCheckYc422RPara(TDE2_SURFACE_S *pstSrcSurface, TDE2_RECT_S *pstSrcRect,
                                           TDE2_SURFACE_S *pstDstSurface, TDE2_RECT_S *pstDstRect, MT_BOOL bDeflicker, MT_BOOL bResize)
{
    TDE_FUN_IN;
    if (TDE2_COLOR_FMT_YCbCr422 == pstSrcSurface->enColorFmt && TDE2_COLOR_FMT_YCbCr422 == pstDstSurface->enColorFmt)
    {
    ...
    else if (TDE2_COLOR_FMT_YCbCr422 != pstSrcSurface->enColorFmt && TDE2_COLOR_FMT_YCbCr422 == pstDstSurface->enColorFmt)
    {
        if ((bDeflicker && bResize && pstSrcRect->u32Width != pstDstRect->u32Width) || (bResize && pstSrcRect->u32Width != pstDstRect->u32Width && pstSrcRect->u32Height != pstDstRect->u32Height))
        {
            TDE_FUN_OUT;
            return -1;
        }
...
* Others:        none
*****************************************************************************/
INLINE mt_s32 TdeOsiCheckSurfaceEX(TDE2_SURFACE_S *pstSurface, TDE2_RECT_S *pstRect)
{
    mt_s32 s32Bpp;
    mt_u32 u32BytePerPixel;

    TDE_FUN_IN;
    TDE_CHECK_COLORFMT(pstSurface->enColorFmt);

    if ((TDE_MAX_SURFACE_PITCH < pstSurface->u32Stride) || (0 == pstSurface->u32Stride) || (0 == pstRect->u32Height) || (0 == pstRect->u32Width) || (pstRect->s32Xpos < 0) || ((mt_u32)pstRect->s32Xpos >= pstSurface->u32Width) || (pstRect->s32Ypos < 0) || ((mt_u32)pstRect->s32Ypos >= pstSurface->u32Height))
    {
    ...
    if ((TDE2_COLOR_FMT_YCbCr422 == pstSurface->enColorFmt) && ((pstRect->s32Xpos & 0x1) || (pstRect->u32Width & 0x1)))
    {
    ...
    if (pstSurface->enColorFmt <= TDE2_COLOR_FMT_halfword)
    {
        ...
        {
            u64 n = pstSurface->u32PhyAddr;
            u32BytePerPixel = (s32Bpp >> 3);

            if (do_div(n, u32BytePerPixel))
            {
    ...
    {
        if (((pstSurface->u32CbCrStride > TDE_MAX_SURFACE_PITCH) || (pstSurface->u32CbCrStride == 0)) && (pstSurface->enColorFmt != TDE2_COLOR_FMT_JPG_YCbCr400MBP))
        {
    ...
    if ((TDE_MAX_RECT_WIDTH_EX < pstRect->u32Width) || (TDE_MAX_RECT_HEIGHT_EX < pstRect->u32Height))
    {
...
STATIC INLINE mt_s32 TdeOsiCheckSurface(TDE2_SURFACE_S *pstSurface, TDE2_RECT_S *pstRect)
{
    ...
    if ((TDE_MAX_RECT_WIDTH < pstRect->u32Width) || (TDE_MAX_RECT_HEIGHT < pstRect->u32Height))
    {
        TDE_TRACE(TDE_KERN_INFO, "invalid operation area!");
        TDE_FUN_OUT;
        return -1;
...
* Others:        none
*****************************************************************************/
STATIC INLINE mt_s32 TdeOsiCheckMbSurfaceEX(TDE2_MB_S *pstMbSurface, TDE2_RECT_S *pstRect)
{
    TDE_FUN_IN;
    TDE_CHECK_MBCOLORFMT(pstMbSurface->enMbFmt);

    if ((pstRect->s32Xpos < 0) || (TDE_MAX_RECT_WIDTH_EX < pstRect->u32Width) || (TDE_MAX_RECT_HEIGHT_EX < pstRect->u32Height) || (0 == pstRect->u32Height) || (0 == pstRect->u32Width) || (TDE_MAX_SURFACE_PITCH < pstMbSurface->u32YStride) || (TDE_MAX_SURFACE_PITCH < pstMbSurface->u32CbCrStride) || ((mt_u32)pstRect->s32Xpos >= pstMbSurface->u32YWidth) || (pstRect->s32Ypos < 0) || ((mt_u32)pstRect->s32Ypos >= pstMbSurface->u32YHeight))
    {
...
STATIC INLINE mt_s32 TdeOsiCheckMbSurface(TDE2_MB_S *pstMbSurface, TDE2_RECT_S *pstRect)
{
    ...
    if ((pstRect->s32Xpos < 0) || (TDE_MAX_RECT_WIDTH < pstRect->u32Width) || (TDE_MAX_RECT_HEIGHT < pstRect->u32Height) || (0 == pstRect->u32Height) || (0 == pstRect->u32Width) || (TDE_MAX_SURFACE_PITCH < pstMbSurface->u32YStride) || (TDE_MAX_SURFACE_PITCH < pstMbSurface->u32CbCrStride) || ((mt_u32)pstRect->s32Xpos >= pstMbSurface->u32YWidth) || (pstRect->s32Ypos < 0) || ((mt_u32)pstRect->s32Ypos >= pstMbSurface->u32YHeight))
    {
        TDE_TRACE(TDE_KERN_INFO, "invalid parameter in operation rect!\n");
        TDE_FUN_OUT;
        return -1;
...
STATIC TDE_OPERATION_CATEGORY_E TdeOsiGetOptCategory(TDE2_SURFACE_S *pstBackGround,
                                                     ...
                                                     TDE2_OPT_S *pstOpt)
{
    TDE2_SURFACE_S *pTmpSrc2 = MT_NULL;
    TDE2_RECT_S *pTmpSrc2Rect = MT_NULL;
    TDE_FUN_IN;
    if ((MT_NULL == pstDst) || (MT_NULL == pstDstRect))
    {
    ...
    if ((MT_NULL != pstBackGround) && (MT_NULL != pstForeGround))
    {
        if ((MT_NULL == pstBackGroundRect) || (MT_NULL == pstForeGroundRect) || (MT_NULL == pstOpt))
        {
            TDE_TRACE(TDE_KERN_INFO, "\n");
            TDE_FUN_OUT;
            return TDE_OPERATION_BUTT;
        ...
        if ((pstBackGroundRect->u32Height != pstDstRect->u32Height) || (pstBackGroundRect->u32Width != pstDstRect->u32Width))
        {
            TDE_TRACE(TDE_KERN_INFO, "\n");
            TDE_FUN_OUT;
            return TDE_OPERATION_BUTT;
    ...
    else if ((MT_NULL == pstBackGround) && (MT_NULL != pstForeGround))
    {
    ...
    if ((MT_NULL == pstOpt) || (!pstOpt->bResize))
    {
        TDE_UNIFY_RECT(pTmpSrc2Rect, pstDstRect);
    }

    if ((MT_NULL == pstOpt) && (pTmpSrc2->enColorFmt == pstDst->enColorFmt))
    {
        TDE_FUN_OUT;
                                           ...
                                           TDE2_RECT_S *pstOperationArea)
{
    ...
    //pstDrvSur->u32Width = pstSur->u32Width;
    //pstDrvSur->u32Height = pstSur->u32Height;
...
//    if (TDE_SCAN_RIGHT_LEFT == pstScanInfo->enHScan)
    {
        //pstDrvSur->u32Xpos = (mt_u32)pstRect->s32Xpos + pstRect->u32Width - 1;
        pstDrvSur->u32Xpos = (mt_u32)pstRect->s32Xpos;
    }
//    else
//    {
//        pstDrvSur->u32Xpos = (mt_u32)pstRect->s32Xpos;
...
//    if (TDE_SCAN_DOWN_UP == pstScanInfo->enVScan)
    {
        //pstDrvSur->u32Ypos = (mt_u32)pstRect->s32Ypos + pstRect->u32Height - 1;
        pstDrvSur->u32Ypos = (mt_u32)pstRect->s32Ypos;
    }
//    else
//    {
//        pstDrvSur->u32Ypos = (mt_u32)pstRect->s32Ypos;
//    }
    ...
    if (NULL != pstOperationArea)
    {
        ...
        pstOperationArea->s32Ypos = (mt_s32)pstDrvSur->u32Ypos;
    ...
    TdeOsiPrintDrvSurface(pstDrvSur);
    TdeOsiPrintRect(pstOperationArea);
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiConvertMbSurface(TDE2_MB_S *pstMB, TDE2_RECT_S *pstMbRect, TDE_DRV_SURFACE_S *pstDrvY,
                                     TDE_DRV_SURFACE_S *pstDrvCbCr, TDE_PIC_MODE_E enPicMode, MT_BOOL bCbCr2Opt)
{
    TDE_DRV_COLOR_FMT_E enAdjColorFmt;
    TDE_FUN_IN;
    if ((MT_NULL == pstMB) || (MT_NULL == pstMbRect))
    {
    ...
    if (MT_NULL != pstDrvY)
    {
        ...
        pstDrvY->u32Xpos = (mt_u32)pstMbRect->s32Xpos;
    ...
    switch (enAdjColorFmt)
    {
        ...
        {
            pstDrvCbCr->u32Xpos = (mt_u32)pstMbRect->s32Xpos / 2;
            pstDrvCbCr->u32Ypos = (mt_u32)pstMbRect->s32Ypos;
            pstDrvCbCr->u32Width =
        ...
        {
            pstDrvCbCr->u32Xpos = (mt_u32)pstMbRect->s32Xpos;
            pstDrvCbCr->u32Ypos = (mt_u32)pstMbRect->s32Ypos / 2;
    ...
    switch (enAdjColorFmt)
    {
        ...
        case TDE_DRV_COLOR_FMT_YCbCr422MBV: {
            pstDrvCbCr->u32Xpos = (mt_u32)pstMbRect->s32Xpos;
            pstDrvCbCr->u32Ypos = ((mt_u32)pstMbRect->s32Ypos / 4);
            pstDrvCbCr->u32Width = pstMbRect->u32Width;
...
#ifndef TDE_BOOT
STATIC mt_s32 TdeOsiSetPatternClipPara(TDE2_SURFACE_S *pstBackGround, TDE2_RECT_S *pstBGRect,
                                       TDE2_SURFACE_S *pstForeGround,
                                       TDE2_RECT_S *pstFGRect, TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect,
                                        ...
                                        TDE_HWNode_S *pstHwNode)
{
    ...
    TDE_FUN_IN;
    if ((NULL != pstForeGround) && (pstForeGround->bAlphaExt1555) && (TDE2_COLOR_FMT_ARGB1555 <= pstForeGround->enColorFmt) && (pstForeGround->enColorFmt <= TDE2_COLOR_FMT_BGRA1555))
    {
        enSrc |= TDE_DRV_SRC_S2;
    }

    if ((NULL != pstBackGround) && (pstBackGround->bAlphaExt1555) && (TDE2_COLOR_FMT_ARGB1555 <= pstBackGround->enColorFmt) && (pstBackGround->enColorFmt <= TDE2_COLOR_FMT_BGRA1555))
    {
                                          ...
                                          TDE_OPERATION_CATEGORY_E enOptCategory, TDE_HWNode_S *pstHwNode)
{
    ...
    TDE_FUN_IN;
    if ((MT_NULL == pstHwNode) || (MT_NULL == pstOpt))
    {
        TDE_FUN_OUT;
        return MT_SUCCESS;
    }

    if ((MT_NULL != pstSrc1) && (MT_NULL != pstSrc2))
    {
        if ((TDE_COLORFMT_CATEGORY_An == TdeOsiGetFmtCategory(pstSrc2->enColorFmt)) && (TDE_COLORFMT_CATEGORY_ARGB == TdeOsiGetFmtCategory(pstSrc1->enColorFmt) || TDE_COLORFMT_CATEGORY_YCbCr == TdeOsiGetFmtCategory(pstSrc1->enColorFmt))) {
...
STATIC INLINE mt_s32 TdeOsiSetDeflickerPara(TDE_HWNode_S *pstHwNode, TDE2_DEFLICKER_MODE_E enDeflickerMode, TDE_FILTER_OPT *pstFilterOpt)
{
    ...
    mt_u8 u8DefTable[] = { 0, 64, 0, 12, 40, 12, 15, 37, 15, 17, 34, 17,
                     2, 4, 7 };
#else
    mt_u8 u8DefTable[] = { 0, 64, 0, 0, 64, 0, 0, 64, 0, 0, 64, 0,
                     0, 64, 0 };
#endif

    TDE_FUN_IN;

    if (MT_TRUE == bDeflicker)
    {
        stFlickerCmd.enDfeMode = TDE_DRV_AUTO_FILTER;
    }

    memcpy((mt_void *)&stFlickerCmd.u8Coef0LastLine, (const mt_void *)u8DefTable, sizeof(u8DefTable));
...
STATIC mt_s32 TdeOsiAdjClipPara(TDE_HWNode_S *pHWNode)
{
  ...
  {
    mt_u32 u32ClipStartX, u32ClipStartY, u32ClipEndX, u32ClipEndY;
    mt_u32 u32OutStartX, u32OutStartY, u32OutEndX, u32OutEndY;
    mt_u32 u32StartX, u32StartY, u32EndX, u32EndY;
                                  ...
                                  mt_u32 u32WorkBufNum, TDE_NODE_SUBM_TYPE_E enSubmType)
{
...
#endif
#endif
    pstCmd->stNodeBuf.pBuf = (mt_void *)pHWNode - TDE_NODE_HEAD_BYTE;
    pstCmd->stNodeBuf.u32NodeSz = sizeof(TDE_HWNode_S);
    pstCmd->stNodeBuf.u64Update = (0xffffffff) | ((mt_u64)0x000003ff << 32);
    ...
    if (MT_NULL != pstJob->pstTailNode)
    {
        mt_u32 u32NextNodeAddr = 0;

        //mt_u32 *pNextNodeAddr = (mt_u32 *)pstJob->pstTailNode->stNodeBuf.pBuf +(TDE_NODE_HEAD_BYTE >> 2)+((pstJob->pstTailNode->stNodeBuf.u32NodeSz) >> 2);
        //mt_u64 *pNextNodeUpdate = (mt_u64 *)(pNextNodeAddr + 1);
        TDE_HWNode_S *lastNode = NULL;
        //TDE_HWNode_S *nextNode = NULL;

        //lastNode = (TDE_HWNode_S *)pstJob->pstTailNode->stNodeBuf.pBuf + TDE_NODE_HEAD_BYTE;
        lastNode = (TDE_HWNode_S *)wgetvrt(pstJob->pstTailNode->stNodeBuf.u32PhyAddr);

        u32NextNodeAddr = pstCmd->stNodeBuf.u32PhyAddr;
        //*pNextNodeUpdate = pstCmd->stNodeBuf.u64Update;
        //nextNode = (TDE_HWNode_S *)wgetvrt(u32NextNodeAddr);

		if (lastNode != NULL)
	        TdeHalHwNodeSetNext(lastNode, u32NextNodeAddr); // set the  cmd fifo addr to the pre cmd fifo
                                                        //TDE_LOG("Jack: [%x][%x] [%x][%x]\n", (mt_u32)lastNode, pstJob->pstTailNode->stNodeBuf.u32PhyAddr, (mt_u32)nextNode, u32NextNodeAddr);
    }
    ...
    *(((mt_u32 *)pstCmd->stNodeBuf.pBuf) + 1) = s32Handle;
    ...
    {
        pstJob->bAqUseBuff = MT_TRUE;
    }

    TdeHalHwNodeSetID(pHWNode, (mt_u32)pstCmd->stNodeBuf.u32PhyAddr, (mt_u32)s32Handle);

//   when finished the hw node config, print it's value
#ifndef TDE_DEBUG_DISABLE
    TdeHalHwNodePrint(pHWNode, MT_FALSE);
#endif
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiBeginJob(TDE_HANDLE *ps32Handle)
{
    mt_s32 ret = 0;
    TDE_FUN_IN;
    if (NULL == ps32Handle)
    ...
    ret = TdeOsiListBeginJob(ps32Handle);
    TDE_FUN_OUT;
    return ret;
}
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiEndJob(TDE_HANDLE s32Handle, MT_BOOL bBlock, mt_u32 u32TimeOut,
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiCancelJob(TDE_HANDLE s32Handle)
{
    mt_s32 ret = 0;
    TDE_FUN_IN;
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiWaitForDone(TDE_HANDLE s32Handle, mt_u32 u32TimeOut)
{
    mt_s32 ret = 0;
    TDE_FUN_IN;
    if (in_interrupt())
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiWaitAllDone(MT_BOOL bSync)
{
    mt_s32 ret = 0;
    TDE_FUN_IN;
    if (in_interrupt())
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiQuickCopy(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc, TDE2_RECT_S *pstSrcRect,
                       TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect)
{
    mt_s32 ret = 0;
    TDE_FUN_IN;
...
static mt_s32 TdeOsiDuplicateTempSurface(TDE2_SURFACE_S *pstInSurface, TDE2_SURFACE_S *pstOutSurface)
{
    mt_u32 size1 = 0;
    mt_u32 size2 = 0;
    mt_s32 ret = 0;
    ...
    if (pstInSurface->u32PhyAddr != 0)
    {
        size1 = pstInSurface->u32Stride * pstOutSurface->u32Height;
        ret = mt_drv_mmz_alloc("tde", NULL, size1, 16, &mmzBuf);
        //pBuf = (mt_u32 *)TDE_MALLOC(pstOutSurface->u32Stride * pstOutSurface->u32Height);
        // if(NULL == pBuf)
        if (ret != MT_SUCCESS)
        {
            TDE_LOG("TdeOsiDuplicateSurface err: %d [%x][%x][%x]\n", __LINE__, pstInSurface->u32Stride, pstOutSurface->u32Height, size1);
            TDE_FUN_OUT;
            return -1;
        }

        //pstOutSurface->u32PhyAddr = (mt_u32)wgetphy(pBuf);
        pstOutSurface->u32PhyAddr = (ulong)mmzBuf.startPhyAddr;
    }

    if (pstInSurface->u32CbCrPhyAddr != 0)
    {
        size2 = pstOutSurface->u32CbCrStride * pstOutSurface->u32Height;
        ret = mt_drv_mmz_alloc("tde", NULL, size2, 16, &mmzBuf);
        //pBuf = (mt_u32 *)TDE_MALLOC(pstOutSurface->u32CbCrStride * pstOutSurface->u32Height);
        // if(NULL == pBuf)
        if (ret != MT_SUCCESS)
        {
            TDE_LOG("TdeOsiDuplicateSurface err: %d [%x][%x][%x]\n", __LINE__, pstInSurface->u32CbCrStride, pstOutSurface->u32Height, size2);
            TDE_FUN_OUT;
            return -1;
        }

        //pstOutSurface->u32CbCrPhyAddr = (mt_u32)wgetphy(pBuf);
        pstOutSurface->u32CbCrPhyAddr = (ulong)mmzBuf.startPhyAddr;
    }

    TDE_LOG("TdeOsiDuplicateTempSurface: [%x] [%x] [%x][%x]\n", pstOutSurface->u32PhyAddr, size1, pstOutSurface->u32CbCrPhyAddr, size2);
    TDE_FUN_OUT;
    return 0;
}

static mt_void TdeOsiFreeTempSurface(TDE2_SURFACE_S *pstTmpSurface)
{
    //mt_u32 *pBuf = NULL;
    mmz_buffer_s mmzBuf;
    TDE_FUN_IN;

    if (0 != pstTmpSurface->u32PhyAddr)
    {
        mmzBuf.startPhyAddr = pstTmpSurface->u32PhyAddr;
        mt_drv_mmz_release(&mmzBuf);
        //pBuf = wgetvrt(pstTmpSurface->u32PhyAddr);
        //TDE_FREE(pBuf);
    }

    if (0 != pstTmpSurface->u32CbCrPhyAddr)
    {
        mmzBuf.startPhyAddr = pstTmpSurface->u32CbCrPhyAddr;
        mt_drv_mmz_release(&mmzBuf);
        //pBuf = wgetvrt(pstTmpSurface->u32CbCrPhyAddr);
        //TDE_FREE(pBuf);
    }
...
*****************************************************************************/
mt_s32 TdeOsiQuickResize(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc, TDE2_RECT_S *pstSrcRect,
                         TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect)
{
    mt_s32 s32Ret = MT_FAILURE;
    TDE2_OPT_S stOpt = { 0 };
    TDE2_SURFACE_S stTempSurface;
    TDE2_RECT_S stTempRect;

    TDE_FUN_IN;
    stOpt.bResize = MT_TRUE;

    MT_ASSERT(pstSrcRect != NULL);
    MT_ASSERT(pstDstRect != NULL);

    if (pstSrcRect->u32Width != pstDstRect->u32Width && pstSrcRect->u32Height != pstDstRect->u32Height)
    {
        // V scale frist, src to tmp surface
        memcpy(&stTempRect, pstDstRect, sizeof(TDE2_RECT_S));
        stTempRect.u32Width = pstSrcRect->u32Width;
        stTempRect.u32Height = pstDstRect->u32Height;

        s32Ret = TdeOsiDuplicateTempSurface(pstDst, &stTempSurface);
        if (s32Ret < 0)
        {
            TDE_FUN_OUT;
            return s32Ret;
        ...
        if ((s32Ret = TdeOsiBlit(s32Handle, NULL, NULL, pstSrc, pstSrcRect,
                                 &stTempSurface, &stTempRect, &stOpt)) < 0)
        {
            TdeOsiFreeTempSurface(&stTempSurface);
            TDE_FUN_OUT;
            return s32Ret;
        }

        // h scale , tmp surface to dst
        if ((s32Ret = TdeOsiBlit(s32Handle, NULL, NULL, &stTempSurface, &stTempRect,
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiQuickFlicker(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc, TDE2_RECT_S *pstSrcRect,
                          TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect)
{
    mt_s32 s32Ret = MT_FAILURE;
    TDE2_OPT_S stOpt = { 0 };
    ...
    stOpt.bResize = MT_TRUE;
    bDeflicker = (stOpt.enDeflickerMode == TDE2_DEFLICKER_MODE_NONE) ? MT_FALSE : MT_TRUE;
    if ((s32Ret = TdeOsiBlit(s32Handle, NULL, NULL, pstSrc, pstSrcRect,
                             pstDst, pstDstRect, &stOpt)) < 0)
    {
        TDE_FUN_OUT;
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiQuickFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect,
                       mt_u32 u32FillData)
{
    mt_s32 ret = 0;
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiSingleSrc1Blit(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround, TDE2_RECT_S *pstBackGroundRect,
                                   TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect)
{
    mt_s32 s32Ret;
    TDE_HWNode_S *pstHWNode = NULL;
    ...
    TDE_FUN_IN;
    if ((NULL == pstBackGround) || (NULL == pstBackGroundRect) || (NULL == pstDst) || (NULL == pstDstRect))
    {
        TDE_FUN_OUT;
        return MT_ERR_TDE_NULL_PTR;
    }
    ...
    if (TdeOsiGetScanInfo(pstBackGround, pstBackGroundRect, pstDst, pstDstRect, MT_NULL,
                          &stSrcScanInfo,
                          &stDstScanInfo) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
    ...
    TdeOsiSetExtAlpha(pstBackGround, MT_NULL, pstHWNode); /* AI7D02681 */
...
static TDE2_ROTATOR_TYPE_S rotate_mirror_tab[TDE2_MIRROR_BUTT][TDE2_ROTATE_BUTT] = {
    {ROTATOR_COPY_OP, ROTATOR_VERT_MIRROR_TRANS, ROTATOR_HORI_VERT_MIRROR, ROTATOR_HORI_MIRROR_TRANS},
    {ROTATOR_HORI_MIRROR, ROTATOR_HORI_VERT_MIRROR_TRANS, ROTATOR_VERT_MIRROR, ROTATOR_TRANS},
    {ROTATOR_VERT_MIRROR, ROTATOR_TRANS, ROTATOR_HORI_MIRROR, ROTATOR_HORI_VERT_MIRROR_TRANS},
    {ROTATOR_HORI_VERT_MIRROR, ROTATOR_HORI_MIRROR_TRANS, ROTATOR_COPY_OP, ROTATOR_VERT_MIRROR_TRANS}
};
...
* Others:        none
*****************************************************************************/
STATIC mt_s32 TdeOsiSingleSrc2Blit(TDE_HANDLE s32Handle,
                                   TDE2_SURFACE_S *pstForeGround,
                                   TDE2_RECT_S *pstForeGroundRect,
                                   TDE2_SURFACE_S *pstDst,
                                   TDE2_RECT_S *pstDstRect,
                                   TDE2_OPT_S *pstOpt)
{
    ...
    TdeOsiPrintSurface(pstForeGround);
    TdeOsiPrintRect(pstForeGroundRect);
    TdeOsiPrintSurface(pstDst);
    TdeOsiPrintRect(pstDstRect);
    TdeOsiPrintOption(pstOpt);

    if ((NULL == pstDst) || (NULL == pstDstRect) || (NULL == pstOpt) || (NULL == pstForeGround) || (NULL == pstForeGroundRect))
    {
        TDE_FUN_OUT;
        return MT_ERR_TDE_NULL_PTR;
    }
    ...
    if (pstOpt->bResize)
    {
        ...
        {
            TDE_FUN_OUT;
            return MT_ERR_TDE_MINIFICATION;
        }
    }
    ...
    /// all bitblt operation to src1
    TdeHalNodeSetSrc1(pstHWNode, &stSrcDrvSurface);
    stConv.src1_fmt = TdeOsiGetFmtCategory(pstForeGround->enColorFmt);
    stConv.fmt_mask |= 0x1 << 1;
    ...
    TdeOsiSetBaseOptParaForBlit(pstOpt, MT_NULL, pstForeGround, TDE_OPERATION_SINGLE_SRC2, pstHWNode);

    TdeOsiSetExtAlpha(MT_NULL, pstForeGround, pstHWNode); /* AI7D02681 */

    if ((pstOpt->bResize) || (pstOpt->enDeflickerMode != TDE2_DEFLICKER_MODE_NONE))
    {

        TdeHalNodeSetScaler(pstHWNode, &stSrcDrvSurface, &stDstDrvSurface);
        ...
        if (pstForeGround->enColorFmt == TDE2_COLOR_FMT_JPG_YCbCr422MBHP ||
            (pstForeGround->enColorFmt >= TDE2_COLOR_FMT_MP1_YCbCr420MBP && pstForeGround->enColorFmt <= TDE2_COLOR_FMT_JPG_YCbCr420MBP) || (pstForeGround->enColorFmt == TDE2_COLOR_FMT_JPG_YCbCr400MBP))
        {
            stTempForeGround.enColorFmt = TDE2_COLOR_FMT_YCbCr422;
        }
        else if (pstForeGround->enColorFmt == TDE2_COLOR_FMT_JPG_YCbCr422MBVP || pstForeGround->enColorFmt == TDE2_COLOR_FMT_JPG_YCbCr444MBP)
        {
    ...
    else
    {
        if (pstOpt->enGsBlur)
        {
            TdeHalNodeSetBlurInfo(pstHWNode, &pstOpt->stBlurOpt);
        }


        if (pstOpt->enRotator)
        {
            TdeHalNodeSetRegularRotatorOpt(pstHWNode, rotate_mirror_tab[pstOpt->enMirror][pstOpt->enRotator]);
        }
...
* Others:        AI7D02579 add support for YCbCr422
*****************************************************************************/
STATIC mt_s32 TdeOsiDoubleSrcBlit(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround, TDE2_RECT_S *pstBackGroundRect,
                                  TDE2_SURFACE_S *pstForeGround, TDE2_RECT_S *pstForeGroundRect,
                                  TDE2_SURFACE_S *pstDst,
                                  TDE2_RECT_S *pstDstRect,
                                  TDE2_OPT_S *pstOpt)
{
    ...
    TDE_FUN_IN;
    if ((NULL == pstBackGround) || (NULL == pstBackGroundRect) || (NULL == pstForeGround) || (NULL == pstForeGroundRect) || (NULL == pstDst) || (NULL == pstDstRect) || (NULL == pstOpt))
    {
        TDE_FUN_OUT;
        return MT_ERR_TDE_NULL_PTR;
    }
    ...
    TdeHalNodeSetSrc2(pstHWNode, &stSrcDrvSurface); //  src1 => src2

    TdeOsiConvertSurface(pstForeGround, &stForeRect, &stSrcScanInfo, &stSrcDrvSurface, &stFGOptArea);

    TdeHalNodeSetSrc1(pstHWNode, &stSrcDrvSurface); // src2 => src1
    ...
    if ((pstOpt->bResize) || (TDE2_DEFLICKER_MODE_NONE != pstOpt->enDeflickerMode))
    {
        ...
        stDSAdjInfo.bDoubleSource = MT_TRUE;
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiBlit(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround, TDE2_RECT_S *pstBackGroundRect,
                  ...
                  TDE2_RECT_S *pstDstRect, TDE2_OPT_S *pstOpt)
{
    ...
    switch (enOptCategory)
    {
        case TDE_OPERATION_SINGLE_SRC1:
        {
            if (NULL == pstBackGround)
            {
                s32Ret = TdeOsiSingleSrc1Blit(s32Handle, pstForeGround, pstForeGroundRect,
                                              pstDst, pstDstRect);
                TDE_FUN_OUT;
                return s32Ret;
            }
            ...
            {
                s32Ret = TdeOsiSingleSrc1Blit(s32Handle, pstBackGround, pstBackGroundRect,
                                              pstDst, pstDstRect);
                TDE_FUN_OUT;
                return s32Ret;
            }
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiMbBlit(TDE_HANDLE s32Handle, TDE2_MB_S *pstMB, TDE2_RECT_S *pstMbRect,
                    TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect, TDE2_MBOPT_S *pstMbOpt)
{
    mt_u32 phy, u32YPhyAddr, u32CbCrPhyAddr, u32BytePerPixel, u32CbCrBytePerPixel;
    mt_u32 u32Height, u32Width, i, j, m, n;
    mt_s32 sRet, s32Bpp;
    mt_u32 u32CbCrHeight = 0;

    TDE_FUN_IN;
    if ((MT_NULL == pstMB) || (MT_NULL == pstMbRect) || (MT_NULL == pstDst) || (MT_NULL == pstDstRect) || (MT_NULL == pstMbOpt))
    {
    ...
    if ((pstDstRect->u32Height == pstMbRect->u32Height) && (pstDstRect->u32Width == pstMbRect->u32Width))
    {
    ...
    if ((pstMB->enMbFmt == TDE2_MB_COLOR_FMT_MP1_YCbCr420MBP) || (pstMB->enMbFmt == TDE2_MB_COLOR_FMT_MP2_YCbCr420MBP) || (pstMB->enMbFmt == TDE2_MB_COLOR_FMT_MP2_YCbCr420MBI) || (pstMB->enMbFmt == TDE2_MB_COLOR_FMT_JPG_YCbCr420MBP) || (pstMB->enMbFmt == TDE2_MB_COLOR_FMT_JPG_YCbCr422MBVP))
    {
...
* Others:        none
*****************************************************************************/
STATIC INLINE mt_s32 TdeOsiBitmapMaskCheckPara(TDE2_SURFACE_S *pstBackGround, TDE2_RECT_S *pstBackGroundRect,
                                               TDE2_SURFACE_S *pstForeGround, TDE2_RECT_S *pstForeGroundRect,
                                               ...
                                               TDE2_RECT_S *pstDstRect)
{
    ...
    if ((pstBackGroundRect->u32Width != pstForeGroundRect->u32Width) || (pstForeGroundRect->u32Width != pstMaskRect->u32Width) || (pstMaskRect->u32Width != pstDstRect->u32Width) || (pstBackGroundRect->u32Height != pstForeGroundRect->u32Height) || (pstForeGroundRect->u32Height != pstMaskRect->u32Height) || (pstMaskRect->u32Height != pstDstRect->u32Height))
    {
        TDE_TRACE(TDE_KERN_INFO, "Don't support scale!\n");
        TDE_LOG("rect_wh[%d][%d] [%d][%d] [%d][%d] [%d][%d]\n", pstBackGroundRect->u32Width, pstBackGroundRect->u32Height, pstForeGroundRect->u32Width, pstForeGroundRect->u32Height, pstMaskRect->u32Width, pstMaskRect->u32Height, pstDstRect->u32Width, pstDstRect->u32Height);
        TDE_LOG("rect_xy[%d][%d] [%d][%d] [%d][%d] [%d][%d]\n", pstBackGroundRect->s32Xpos, pstBackGroundRect->s32Ypos, pstForeGroundRect->s32Xpos, pstForeGroundRect->s32Ypos, pstMaskRect->s32Xpos, pstMaskRect->s32Ypos, pstDstRect->s32Xpos, pstDstRect->s32Ypos);
        TDE_FUN_OUT;
        return MT_ERR_TDE_INVALID_PARA;
    }
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiBitmapMaskRop(TDE_HANDLE s32Handle,
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiBitmapMaskBlend(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround,
                             TDE2_RECT_S *pstBackGroundRect,
                             TDE2_SURFACE_S *pstForeGround, TDE2_RECT_S *pstForeGroundRect,
                             TDE2_SURFACE_S *pstMask,
...
{
    TDE_HWNode_S *pstHWNode = NULL;
    TDE_DRV_SURFACE_S stSrc1DrvSurface = { 0 };
    TDE_DRV_SURFACE_S stSrc2DrvSurface = { 0 };
    TDE_DRV_SURFACE_S stSrc3DrvSurface = { 0 };
    TDE_DRV_SURFACE_S stDstDrvSurface = { 0 };
    TDE_SCANDIRECTION_S stScanInfo = { 0 };
    ...
    if (TDE2_COLOR_FMT_A8 != pstMask->enColorFmt)
    {
        TDE_TRACE(TDE_KERN_INFO, "Maskbitmap's colorformat can only be A8 in TdeOsiBitmapMaskBlend!\n");
        TDE_FUN_OUT;
        return MT_ERR_TDE_INVALID_PARA;
    }
    ...
    TdeOsiConvertSurface(pstForeGround, pstForeGroundRect, &stScanInfo, &stSrc1DrvSurface, NULL);

    TdeHalNodeSetSrc1(pstHWNode, &stSrc1DrvSurface);

    TdeOsiConvertSurface(pstBackGround, pstBackGroundRect, &stScanInfo, &stSrc2DrvSurface, NULL);

    TdeHalNodeSetSrc2(pstHWNode, &stSrc2DrvSurface);

    TdeOsiConvertSurface(pstMask, pstMaskRect, &stScanInfo, &stSrc3DrvSurface, NULL);

    TdeHalNodeSetSrc3(pstHWNode, &stSrc3DrvSurface);

    TdeOsiConvertSurface(pstDst, pstDstRect, &stScanInfo, &stDstDrvSurface, NULL);

    TdeHalNodeSetTgt(pstHWNode, &stDstDrvSurface, TDE2_OUTALPHA_FROM_NORM);

    TdeHalNodeSetGlobalAlpha(pstHWNode, u8Alpha, MT_TRUE);

    if (TdeHalNodeSetBaseOperate(pstHWNode, TDE_TREE_SRC_OPT, TDE_ALU_MASK_BLEND, 0) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FUN_OUT;
        return MT_ERR_TDE_UNSUPPORTED_OPERATION;
    ...
    stBlendOpt.bGlobalAlphaEnable = MT_TRUE;
    stBlendOpt.bPixelAlphaEnable = MT_TRUE;
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiImageMultiply(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround,
                           TDE2_RECT_S *pstBackGroundRect,
                           TDE2_SURFACE_S *pstForeGround, TDE2_RECT_S *pstForeGroundRect,
                           ...
                           TDE2_MULTIPLY_OPT_S *pMultiplyOpt)
{
    ...
    //TDE_DRV_CONV_MODE_CMD_S stConv = {0};
    //TDE2_BLEND_OPT_S stBlendOpt = {0};
    TDE_DRV_BASEOPT_MODE_E enMode;
    TDE_DRV_ALU_MODE_E enAluMode;
    TDE2_IMAGE_MULTIPLY_TYPE_E type;
    mt_s32 ret = 0;
    TDE_CLUT_USAGE_E enClutUsage = TDE_CLUT_USAGE_BUTT;
    mt_s32 s32Ret = MT_FAILURE;
    ...
    MT_ASSERT(pMultiplyOpt != MT_NULL);

    type = pMultiplyOpt->multiplyType;

    if ((ret = TdeOsiBitmapMaskCheckPara(pstBackGround, pstBackGroundRect,
                                         pstForeGround, pstForeGroundRect, pstPattern,
                                         pstPatternRect, pstDst, pstDstRect)) < 0)
    {
        TDE_FUN_OUT;
        return ret;
    }
    ...
    TdeOsiConvertSurface(pstPattern, pstPatternRect, &stScanInfo, &stSrc3DrvSurface, NULL);
    ...
    if (pMultiplyOpt->enAluCmd & TDE2_ALUCMD_ROP)
    {
        if ((TDE2_ROP_BUTT <= pMultiplyOpt->enRopCode_Alpha) || (TDE2_ROP_BUTT <= pMultiplyOpt->enRopCode_Color))
        {
            TDE_TRACE(TDE_KERN_INFO, "enRopCode error!\n");
            TdeHalFreeNodeBuf(pstHWNode);
            TDE_FUN_OUT;
        ...
        if ((!TdeOsiIsSingleSrc2Rop(pMultiplyOpt->enRopCode_Alpha)) || (!TdeOsiIsSingleSrc2Rop(pMultiplyOpt->enRopCode_Color)))
        {
            TDE_TRACE(TDE_KERN_INFO, "Only support single s2 rop!\n");
            TdeHalFreeNodeBuf(pstHWNode);
            TDE_FUN_OUT;
        ...
        if (TdeHalNodeSetRop(pstHWNode, pMultiplyOpt->enRopCode_Color, pMultiplyOpt->enRopCode_Alpha) < 0)
        {
            TdeHalFreeNodeBuf(pstHWNode);
            TDE_FUN_OUT;
            return MT_ERR_TDE_UNSUPPORTED_OPERATION;
    ...
    if (pMultiplyOpt->enAluCmd & TDE2_ALUCMD_BLEND)
    {
        enAluMode = TDE_ALU_BLEND;
        if (TdeHalNodeSetBlend(pstHWNode, &pMultiplyOpt->stBlendOpt) < 0)
        {
            TdeHalFreeNodeBuf(pstHWNode);
            TDE_FUN_OUT;
            return MT_ERR_TDE_UNSUPPORTED_OPERATION;
    ...
    TdeHalNodeSetGlobalAlpha(pstHWNode, pMultiplyOpt->u8GlobalAlpha, pMultiplyOpt->stBlendOpt.bGlobalAlphaEnable);

    enMode = TDE_TREE_SRC_OPT;
    if (type == TDE_IMAGE_MULTIPLY)
    {
        enAluMode = TDE_ALU_MULTIPLY;
    }
    else if (type == TDE_IMAGE_STENCIL)
    {
        enAluMode = TDE_ALU_STENCIL;
    }
    else
    {
        // error
    ...
    if ((s32Ret = TdeOsiSetClutOpt(pstForeGround, pstDst, &enClutUsage, pMultiplyOpt->bClutReload, pstHWNode)) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FUN_OUT;
        return s32Ret;
    ...
    if ((s32Ret = TdeOsiSetClutOpt2(pstPattern, pstDst, &enClutUsage, pMultiplyOpt->bClutReload, pstHWNode)) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FUN_OUT;
        return s32Ret;
    ...
    if (TDE2_COLORKEY_MODE_NONE != pMultiplyOpt->enColorKeyMode)
    {
        TDE_DRV_COLORKEY_CMD_S stColorkey;
        TDE_COLORFMT_CATEGORY_E enFmtCategory;
        stColorkey.unColorKeyValue = pMultiplyOpt->unColorKeyValue;

        switch (pMultiplyOpt->enColorKeyMode)
        {
            case TDE2_COLORKEY_MODE_BACKGROUND:
                stColorkey.enColorKeyMode = TDE_DRV_COLORKEY_BACKGROUND;
    ...
    if (TdeHalNodeSetBaseOperate(pstHWNode, enMode, enAluMode, 0) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FUN_OUT;
        return MT_ERR_TDE_UNSUPPORTED_OPERATION;
    ...
    //TdeHalNodeSetGlobalAlpha(pstHWNode, 0xff, MT_TRUE);
    if ((ret = TdeOsiSetNodeFinish(s32Handle, pstHWNode, 0, TDE_NODE_SUBM_ALONE)) < 0)
    {
        TdeOsiListPutPhyBuff(1);
        TdeHalFreeNodeBuf(pstHWNode);
...
EXPORT_SYMBOL(TdeOsiImageMultiply);
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiSolidDraw(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc, TDE2_RECT_S *pstSrcRect,
                       TDE2_SURFACE_S *pstDst,
                       TDE2_RECT_S *pstDstRect, TDE2_FILLCOLOR_S *pstFillColor,
                       TDE2_OPT_S *pstOpt)
{
...
mt_s32 TdeOsiSetDeflickerLevel(TDE_DEFLICKER_LEVEL_E eDeflickerLevel)
{
    return TdeHalSetDeflicerLevel(eDeflickerLevel);
}
EXPORT_SYMBOL(TdeOsiSetDeflickerLevel);

mt_s32 TdeOsiGetDeflickerLevel(TDE_DEFLICKER_LEVEL_E *pDeflickerLevel)
{
...
mt_s32 TdeOsiSetAlphaThresholdValue(mt_u8 u8ThresholdValue)
{
...
mt_s32 TdeOsiSetAlphaThresholdState(MT_BOOL bEnAlphaThreshold)
{
...
mt_s32 TdeOsiGetAlphaThresholdValue(mt_u8 *pu8ThresholdValue)
{
...
mt_s32 TdeOsiGetAlphaThresholdState(MT_BOOL *pbEnAlphaThreshold)
{
    if (MT_NULL == pbEnAlphaThreshold)
    {
        return MT_FAILURE;
    }
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiCheckSingleSrcPatternOpt(TDE2_COLOR_FMT_E enSrcFmt,
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiCheckDoubleSrcPatternOpt(TDE2_COLOR_FMT_E enBackGroundFmt, TDE2_COLOR_FMT_E enForeGroundFmt,
                                      TDE2_COLOR_FMT_E enDstFmt, TDE2_PATTERN_FILL_OPT_S *pstOpt)
{
    ...
    if ((TDE_COLORFMT_CATEGORY_BYTE <= enBackGroundCategory) || (TDE_COLORFMT_CATEGORY_BYTE <= enForeGroundCategory) || (TDE_COLORFMT_CATEGORY_BYTE <= enDstCategory))
    {
        TDE_TRACE(TDE_KERN_INFO, "Unkown color format!\n");
        TDE_FUN_OUT;
        return -1;
    ...
    else if ((TDE2_COLOR_FMT_YCbCr888 == enBackGroundFmt) || (TDE2_COLOR_FMT_AYCbCr8888 == enForeGroundFmt))
    {
        if (TDE_COLORFMT_CATEGORY_An == enForeGroundCategory)
        {
            if (!TdeOsiWhetherContainAlpha(enDstFmt))
    ...
    {
        if ((TDE_COLORFMT_CATEGORY_CLUT != enForeGroundCategory) || (TDE_COLORFMT_CATEGORY_CLUT != enDstCategory))
        {
            TDE_TRACE(TDE_KERN_INFO, "Unsupported operation!\n");
            TDE_FUN_OUT;
            return -1;
    ...
    else if ((TDE_COLORFMT_CATEGORY_An == enBackGroundCategory) && (TDE_COLORFMT_CATEGORY_An == enForeGroundCategory) && (TDE_COLORFMT_CATEGORY_An == enDstCategory))
    {
    ...
    {
        if ((TDE_COLORFMT_CATEGORY_CLUT != enForeGroundCategory) || (TDE_COLORFMT_CATEGORY_CLUT != enBackGroundCategory)) {
            TDE_TRACE(TDE_KERN_INFO, "Unsupported operation!\n");
            TDE_FUN_OUT;
            return -1;
        }
...
MT_BOOL TdeOsiCheckOverlap(TDE2_SURFACE_S *pstSur1, TDE2_RECT_S *pstRect1, TDE2_SURFACE_S *pstSur2, TDE2_RECT_S *pstRect2)
{
    mt_u32 u32Rect1StartPhy;
    mt_u32 u32Rect1EndPhy;
    mt_u32 u32Rect2StartPhy;
    mt_u32 u32Rect2EndPhy;
    mt_u32 u32Bpp1;
    mt_u32 u32Bpp2;
    ...
    u32Rect1StartPhy = pstSur1->u32PhyAddr + (pstRect1->s32Ypos * pstSur1->u32Stride) + pstRect1->s32Xpos * u32Bpp1;
    u32Rect1EndPhy = u32Rect1StartPhy + (pstRect1->u32Height - 1) * pstSur1->u32Stride + (pstRect1->u32Width - 1) * u32Bpp1;

    u32Rect2StartPhy = pstSur2->u32PhyAddr + (pstRect2->s32Ypos * pstSur2->u32Stride) + pstRect2->s32Xpos * u32Bpp2;
    u32Rect2EndPhy = u32Rect2StartPhy + (pstRect2->u32Height - 1) * pstSur2->u32Stride + (pstRect2->u32Width - 1) * u32Bpp2;
    ...
    if (((u32Rect1StartPhy >= u32Rect2StartPhy) && (u32Rect1StartPhy <= u32Rect2EndPhy)) || ((u32Rect1EndPhy >= u32Rect2StartPhy) && (u32Rect1EndPhy <= u32Rect2EndPhy)))
    {
        TDE_FUN_OUT;
        return MT_TRUE;
    }
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiSingleSrcPatternFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstSrc,
                                  ...
                                  TDE2_PATTERN_FILL_OPT_S *pstOpt)
{
    ...
    if (pstOpt != MT_NULL)
    {
        ...
        if (TdeOsiSetPatternClipPara(MT_NULL, MT_NULL, pstSrc, pstSrcRect, pstDst, pstDstRect, pstOpt, pstHWNode) < 0)
        {
            TdeHalFreeNodeBuf(pstHWNode);
            TDE_FUN_OUT;
            return MT_ERR_TDE_CLIP_AREA;
    ...
    if (TdeHalNodeSetBaseOperate(pstHWNode, enBaseMode, enAluMode, MT_NULL) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FUN_OUT;
        return MT_ERR_TDE_UNSUPPORTED_OPERATION;
    ...
    TdeOsiSetExtAlpha(MT_NULL, pstSrc, pstHWNode);
    ...
    {
        s32Ret = TdeOsiSetClutOpt(pstSrc, pstDst, &enClutUsage, MT_TRUE, pstHWNode);
    }
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiDoubleSrcPatternFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround,
                                  TDE2_RECT_S *pstBackGroundRect, TDE2_SURFACE_S *pstForeGround, TDE2_RECT_S *pstForeGroundRect,
                                  TDE2_SURFACE_S *pstDst, TDE2_RECT_S *pstDstRect, TDE2_PATTERN_FILL_OPT_S *pstOpt)
{
    TDE_HWNode_S *pstHWNode = NULL;
    ...
    TDE_FUN_IN;
    if ((MT_NULL == pstBackGround) || (MT_NULL == pstBackGroundRect) || (MT_NULL == pstForeGround) || (MT_NULL == pstForeGroundRect) || (MT_NULL == pstDst) || (MT_NULL == pstDstRect) || (MT_NULL == pstOpt))
    {
        TDE_FUN_OUT;
        return MT_ERR_TDE_INVALID_PARA;
    }
    ...
    if (TdeOsiCheckOverlap(pstForeGround, pstForeGroundRect, pstBackGround, pstBackGroundRect) || TdeOsiCheckOverlap(pstForeGround, pstForeGroundRect, pstDst, pstDstRect))
    {
        TDE_TRACE(TDE_KERN_INFO, "Surface overlap!\n");
        TDE_FUN_OUT;
        return MT_ERR_TDE_INVALID_PARA;
    ...
    if (TdeHalNodeSetBaseOperate(pstHWNode, TDE_DOUBLE_SRC_PATTERN_FILL_OPT, enAluMode, MT_NULL) < 0)
    {
        TdeHalFreeNodeBuf(pstHWNode);
        TDE_FUN_OUT;
        return MT_ERR_TDE_UNSUPPORTED_OPERATION;
...
* Others:        none
*****************************************************************************/
mt_s32 TdeOsiPatternFill(TDE_HANDLE s32Handle, TDE2_SURFACE_S *pstBackGround,
...
* Others:        none
*****************************************************************************/
mt_s32 TdeCalScaleRect(const TDE2_RECT_S *pstSrcRect, const TDE2_RECT_S *pstDstRect,
                       TDE2_RECT_S *pstRectInSrc, TDE2_RECT_S *pstRectInDst)
{
    ...
    TdeOsiGetHUpdateInfo(&reg, &info, MT_TRUE);
    TdeOsiGetVUpdateInfo(&reg, &info, MT_TRUE, MT_TRUE);
...
mt_s32 TdeOsiEnableRegionDeflicker(MT_BOOL bRegionDeflicker)
{
