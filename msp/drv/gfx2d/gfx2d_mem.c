...
static HI_U32 g_u32MemPoolVrtAddr = 0;/*内存池虚拟地址*/
static HI_U32 g_u32MemPoolSize = 0;/*内存池大小*/
static HI_U32  g_u32MemryDisablecount = 0; /*自旋锁操作数*/
...
void GFX2D_Memory_Disableirq (HI_U16 i, unsigned long*lockflags)
{	
    spin_lock_irqsave(&g_struMemBlock[i].lock, *lockflags);
    g_u32MemryDisablecount++;
}   
/*释放自旋锁*/
void GFX2D_Memory_Enableirq (HI_U16 i, unsigned long*lockflags)
{	
    spin_unlock_irqrestore(&g_struMemBlock[i].lock, *lockflags);	
    g_u32MemryDisablecount--;  	
}
...
{
    /*申请MMZ内存*/
    g_u32MemPoolPhyAddr = HI_GFX_AllocMem("GFX2D_MemPool",NULL,u32Size);
    /*申请失败*/
    if(0 == g_u32MemPoolPhyAddr)
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "malloc mempool buffer failed!MemPoolSize = 0x%x\n",u32Size);
        HI_GFX_FreeMem(0);
        return HI_FAILURE;
    }
    /*映射物理地址为虚拟地址*/
    ...
    /*映射失败*/
    if(0 == g_u32MemPoolVrtAddr)
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "mempoolphyaddr map failed!\n");
...
HI_S32 GFX2D_MEM_Deinit(HI_VOID)
{
    ...
    for(i=0;i< g_uRegisterCount;i++)
    {
        if(g_struMemBlock[i].nFree!=g_struMemBlock[i].nUnitNum)
            HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "some buffer in use\n");    
    }
    /*解除内存池映射*/
...
HI_S32 GFX2D_MEM_Register(const HI_U32 u32MemSize, const HI_U32 u32BlockSize)
{
    ...
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "The memory pool is not initial!\n");
        return HI_FAILURE;
    }
    /*内存块大小范围不在内存单元与内存池之间*/
    if((u32MemSize < u32BlockSize)|| (u32MemSize > g_u32MemPoolSize))
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "u32MemSize too large or too small!u32MemSize = %d,u32BlockSize=%d\n",u32MemSize,u32BlockSize);
        return HI_FAILURE;
    }
    /*判断注册次数是否超过规定次数*/
    if(g_uRegisterCount >= RegisterMaxTimes)
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "the times of register is more than the max times \n");
        return HI_FAILURE;
    }
    /*判断是否存在相同内存单元的内存块*/
    for(i=0;i< g_uRegisterCount;i++)
    {
        ...
        {
            HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "already  register memory block of the same size \n");    
            return HI_FAILURE;
        }
    }
    /*判断内存块总大小是否超出内存池大小*/
    for(i=0;i< g_uRegisterCount;i++)
    {
        ...
        {
            HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "the registered memory size is larger than the memory pool size\n");    
            return HI_FAILURE;
        }
    }
    ...
    g_struMemBlock [g_uRegisterCount].nUnitNum = u32MemSize/ u32BlockSize;/*内存单元个数*/
    g_struMemBlock [g_uRegisterCount].pStartAddr = (HI_U8*)( g_u32MemPoolVrtAddr+ u32Size);/*  内存块起始地址*/
...
HI_S32 GFX2D_Mem_UnRegister(const HI_U32 u32BlockSize)
{
    ...
    if(pBlock==NULL) 
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "There is no such memory block!\n"); 
        return HI_FAILURE;
    }
    /*存在未释放的内存*/
    if(g_struMemBlock[i].nFree!= g_struMemBlock[i].nUnitNum)
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "some memory is not free!\n");   
        return HI_FAILURE;
    }
...
HI_VOID *GFX2D_MEM_Alloc(const HI_U32 u32Size)
{
    ...
    {
	/*打印warning信息*/
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "the alloc memory size can't be 0!\n");    
	 return NULL;
    }	
    ...
    {
 	/*打印warning信息*/
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "the memory block is not registered!\n");    
        return NULL;    
    }
    
    /*获取自旋锁*/
    GFX2D_Memory_Disableirq(i,&lockflags);

    /*无空闲内存单元*/
    if(!pBlock->nFree)
    {
        GFX2D_Memory_Enableirq(i,&lockflags);
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "There is no free unit !\n"); 
        return NULL;    
    }
...
HI_S32 GFX2D_MEM_Unit_Free(HI_U32 i,HI_VOID *pBuf)
{
    MemoryBlock *pBlock = NULL;
    unsigned long lockflags;
    if(((HI_U32)pBuf < (HI_U32)g_struMemBlock[i].pStartAddr) 
        || ((HI_U32)pBuf >= ((HI_U32)g_struMemBlock[i].pStartAddr + g_struMemBlock[i].nSize)))
    {
        return HI_FAILURE;
    }
    pBlock = &g_struMemBlock[i];
    ...
    *(HI_U16*) pBuf = pBlock->nFirst;
    ...
    pBlock->nFirst = ((HI_U32) pBuf - (HI_U32)pBlock->pStartAddr)/pBlock->nUnitSize;
     /*释放自旋锁*/
...
HI_S32 GFX2D_MEM_Free(HI_VOID *pBuf)
{
    ...
    if(((HI_U32)pBuf < g_u32MemPoolVrtAddr)
        || ((HI_U32)pBuf>=(g_u32MemPoolVrtAddr + g_u32MemPoolSize)))
    {
        HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "the addr is out of the range of the memory pool !\n");    
        return HI_FAILURE;
    }
    ...
    HI_GFX_COMM_LOG_ERROR(HIGFX_TDE_ID, "the addr is not the right addr of memory !\n");    
    return HI_FAILURE;
...
{
    HI_U16 i;
    SEQ_Printf(p, "--------- Montage TDE Memory Pool Info ---------\n");
