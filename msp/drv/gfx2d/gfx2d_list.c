...
static inline HI_VOID DeleteTask(GFX2D_LIST_TASK_MGR_S *pstTaskMgr,
                                 GFX2D_LIST_TASK_S *pstTask);
static inline HI_VOID SubTask(GFX2D_LIST_TASK_MGR_S *pstTaskMgr);
...
HI_S32 GFX2D_LIST_Deinit(HI_VOID)
{
    ...
    u32IntNum = GFX2D_HAL_GetIsrNum(0, 0);
    free_irq(u32IntNum, NULL);
                          ...
                          const HI_U32 u32Timeout)
{
    ...
    pstTask = CreateTask(enDevId, pNode, enNodeType,bSync,u32Timeout);
    if (HI_NULL == pstTask)
    {
                                            ...
                                            const HI_U32 u32Timeout)
{
    ...
    pstTask = (GFX2D_LIST_TASK_S *)GFX2D_MEM_Alloc(sizeof(GFX2D_LIST_TASK_S));
    if (HI_NULL == pstTask)
    {
...
static HI_S32 WaitForDone(GFX2D_LIST_TASK_S *pstTask, HI_U32 u32Timeout)
{
    ...
    {
        kt = ms_to_ktime(u32Timeout);
    }
    
    if (ktime_to_us(kt) < 1000)
        kt = ms_to_ktime(1);
    ...
    {        
        ...
        s32Ret = wait_event_interruptible_hrtimeout(pstTask->stWaitQueueHead,
                 (GFX2D_LIST_TASKSTATE_FINISH == pstTask->enState), kt);
        spin_lock_irqsave(&(pstTask->lock),lockflag);
        pstTask->bWait = HI_FALSE;
        if (GFX2D_LIST_TASKSTATE_FINISH != pstTask->enState)
        {
            if (u32Timeout != 0)
            {
                if (s32Ret < 0)
        ...
        spin_unlock_irqrestore(&(pstTask->lock),lockflag);
        if ((u32Timeout != 0) || (HI_SUCCESS == s32Ret))
        {
            break;
        }
    }
...
#ifndef GFX2D_PROC_UNSUPPORT
HI_S32 GFX2D_LIST_ReadProc(struct seq_file *p, HI_VOID *v)
{
    GFX2D_LIST_TASK_MGR_S *pstTaskMgr = &gs_astLISTTaskMgr[HI_GFX2D_DEV_ID_0][GFX2D_HAL_DEV_TYPE_HWC];
