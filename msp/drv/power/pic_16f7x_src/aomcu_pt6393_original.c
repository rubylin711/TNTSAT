...
static void spi_wr_port(U16 reg_addr,U32 data)
{
	U8 i = 0;
	U8 tmp = 0;
	U32 val = 0;
	...
	{
		tmp = (val >> (i*8)) & 0xff;
		wr_port(reg_addr + i,tmp);
	}
}

static void spi_rd_port(U16 reg_addr,U32* data)
{
	U8 i = 0;
	U8 tmp = 0;
	U32 val = 0;
...
static U8 spi_fp_is_trans_complete(void)
{
	U8 tmp = 0;
	rd_port(FP_SPI_STA+2,tmp);
...
static U8 spi_fp_is_txd_fifo_full(void)
{
	U8 tmp = 0;
	rd_port(FP_SPI_STA+1,tmp);
...
static U8 spi_fp_is_rxd_fifo_empty(void)
{
	U8 tmp = 0;
	rd_port(FP_SPI_STA,tmp);
...
static U8 fp_is_trans_complete(void)
{
	U8 tmp = 0;
	rd_port(FP_SPI_STA+2,tmp);
...
static S8 spi_fp_write(U8 *p_cmd_buf, U8 cmd_len, U8 *p_data_buf, U8 data_len, U8 secure)
{
    U16 timeout = FP_SPI_TIME_OUT;
    U8 i  = 0;
    U8 r = 0;
    U8 j = 0;
    U8 dtmp = 0;
    ...
    if((p_cmd_buf == 0) && (cmd_len != 0))
    {
        return -1;
    }

    if((p_data_buf == 0) && (data_len != 0))
    {
        return -1;
    }
    ...
    {
        p_buf = p_cmd_buf;
        wr_port(FP_SPI_TC, data_len);

		//HAL_PUT_U32((volatile u32 *)FP_SPI_CTRL, (cmd_dtmp | 0x203));
		//spi_wr_port(FP_SPI_CTRL, (cmd_dtmp | 0x203));
		cmd_0 = 0x03;
		cmd_1 = 0x02;
		cmd_2 = cmd_len & 0x3f;

		wr_port(FP_SPI_CTRL+2, cmd_2);
		wr_port(FP_SPI_CTRL+1, cmd_1);
		wr_port(FP_SPI_CTRL, cmd_0);
        ...
        for(i = 0; i < cmd_len; i ++)
        {
            ...
            wr_port(FP_SPI_CMD_FIFO, p_buf[i]);
        }
    }
    ...
    if(data_len > 0)
    {
        ...
        while(i)
        {
			...
			//spi_wr_port(FP_SPI_TXD, dtmp);
			wr_port(FP_SPI_TXD, p_buf[3]);
			wr_port(FP_SPI_TXD+1, p_buf[2]);
			wr_port(FP_SPI_TXD+2, p_buf[1]);
			wr_port(FP_SPI_TXD+3, p_buf[0]);
            i --;
            p_buf += 4;
        }
 ...
 /*
			...
			//HAL_PUT_U32((volatile u32 *)FP_SPI_TXD, dtmp);
			//spi_wr_port(FP_SPI_TXD, dtmp);
			wr_port(FP_SPI_TXD, tmp[0]);
			wr_port(FP_SPI_TXD+1, tmp[1]);
			wr_port(FP_SPI_TXD+2, tmp[2]);
			wr_port(FP_SPI_TXD+3, tmp[3]);
*/
			wr_port(FP_SPI_TXD+3, p_buf[0]);
        }
    }
...
static S8 spi_fp_read(U8 *p_wbuf, U8 wlen, U8 *p_rbuf, U8 rlen)
{
    ...
    if(wlen > 0)
    {
		...
		wr_port(FP_SPI_TC, rlen);
		cmd = (wlen & 0x3f);
		//spi_wr_port(FP_SPI_CTRL, cmd_dtmp);
		wr_port(FP_SPI_CTRL+2, cmd);
    ...
    if(rlen > 0)
    {
        ...
        while(i)
        {
			...
			rd_port(FP_SPI_RXD, p_buf[3]);
			rd_port(FP_SPI_RXD+1, p_buf[2]);
			rd_port(FP_SPI_RXD+2, p_buf[1]);
			rd_port(FP_SPI_RXD+3, p_buf[0]);

            //dtmp = HAL_GET_U32((volatile u32 *)(FP_SPI_RXD));
			//spi_rd_port(FP_SPI_RXD, &dtmp);
/*
...
*/
        ...
        if(r > 0)
        {
            ...
            for(j = 1; j <= r; j++)
            {
                ...
                rd_port(FP_SPI_RXD+(j-1),p_buf[0]);
                p_buf ++;
            }
        }
    }
...
static void PT6393_write(U8 *p_wbuf, U8 wlen)
{
	...
	//len+0x2+0x03	//FP_SPI_CTRL: 0x10203
	//wr_port(FP_SPI_CTRL, 0x03);
	wr_port(FP_SPI_CTRL+2, cmd_2);
	wr_port(FP_SPI_CTRL+1, cmd_1);
	wr_port(FP_SPI_CTRL, 0x03);
	for(i = 0; i < wlen; i ++)
	{
		...
		//HAL_PUT_U32((volatile u32 *)FP_SPI_CMD_FIFO, p_buf[i]);
		wr_port(FP_SPI_CMD_FIFO, p_wbuf[i]);
	}
...
static void PT6393_read(U8 *p_rbuf, U8 rlen)
{
	...
	rd_port(FP_SPI_RXD, p_rbuf[3]);
	rd_port(FP_SPI_RXD+1, p_rbuf[2]);
	rd_port(FP_SPI_RXD+2, p_rbuf[1]);
	rd_port(FP_SPI_RXD+3, p_rbuf[0]);
...
/*
		wr_port(LPM_AOMCU_MESSAGE, p_buf[3] );
		wr_port(LPM_AOMCU_MESSAGE +1 ,p_buf[2] );
		wr_port(LPM_AOMCU_MESSAGE +2 , p_buf[1]);
		wr_port(LPM_AOMCU_MESSAGE +3 , p_buf[0]);
*/
	}
...
void led_test()
{
	...
	wr_port(FP_SPI_TC, 0x1);
	wr_port(FP_SPI_TC+1, 0);
	wr_port(FP_SPI_TC+2, 0);
	wr_port(FP_SPI_TC+3, 0);

	wr_port(FP_SPI_CTRL, 0x3);
	wr_port(FP_SPI_CTRL+1, 0x2);
	wr_port(FP_SPI_CTRL+2, 0x1);
	wr_port(FP_SPI_CTRL+3, 0);

	wr_port(FP_SPI_CMD_FIFO, cmd);
	wr_port(FP_SPI_CMD_FIFO+1, 0);
	wr_port(FP_SPI_CMD_FIFO+2, 0);
	wr_port(FP_SPI_CMD_FIFO+3, 0);

	while(spi_fp_is_txd_fifo_full());
	wr_port(FP_SPI_TXD, 0);//0
	wr_port(FP_SPI_TXD+1, 0);
	wr_port(FP_SPI_TXD+2, 0);
	wr_port(FP_SPI_TXD+3, 0x02);//0x2
...
U8 read_key(void)
{
	...
	wr_port(FP_SPI_CTRL+2, 0x1);
	wr_port(FP_SPI_CTRL+1, 0x2);
	wr_port(FP_SPI_CTRL, 0x5);

	//step3
	wr_port(FP_SPI_CMD_FIFO, 0x46);//cmd
	wr_port(FP_SPI_CMD_FIFO+1, 0);
	wr_port(FP_SPI_CMD_FIFO+2, 0);
	wr_port(FP_SPI_CMD_FIFO+3, 0);
	...
	while(1)
	{
		...
		rd_port(FP_SPI_RXD, data[3]);
		rd_port(FP_SPI_RXD+1, data[2]);
		rd_port(FP_SPI_RXD+2, data[1]);
		rd_port(FP_SPI_RXD+3, data[0]);
		break;
	}
...
void pt6393_init()
{
	...
	pt6393_cmd_display_ctrl(PT6393_DISPLAY_ON, PT6393_PULSE_WIDTH_6);
#else
	//U32 val = 0;
	//1.set pinmux
	wr_port(0xb400, 0x2);
...
*/
	//3.data cfg 0x10000;
	wr_port(FP_SPI_DATA_CFG,0x0);
	wr_port(FP_SPI_DATA_CFG+1,0x0);
	wr_port(FP_SPI_DATA_CFG+2,0x1);
	wr_port(FP_SPI_DATA_CFG+3,0x0);
	//0xf0000040;
	wr_port(FP_SPI_CH_BAUD,0xf0);//0xf0
	wr_port(FP_SPI_CH_BAUD+1,0x0);
	wr_port(FP_SPI_CH_BAUD+2,0x0);
	wr_port(FP_SPI_CH_BAUD+3,0xf0);
	//0x67f57f14;
	wr_port(FP_SPI_CH_MODE_CFG,0x14);
	wr_port(FP_SPI_CH_MODE_CFG+1,0x7f);
	wr_port(FP_SPI_CH_MODE_CFG+2,0xf5);
	wr_port(FP_SPI_CH_MODE_CFG+3,0x67);
	//0
	wr_port(FP_SPI_INT_CFG,0x0);
	wr_port(FP_SPI_INT_CFG+1,0x0);
	wr_port(FP_SPI_INT_CFG+2,0x0);
	wr_port(FP_SPI_INT_CFG+3,0x0);
	//0x0000005f;
	wr_port(FP_SPI_PIN_MODE,0x5f);
	wr_port(FP_SPI_PIN_MODE+1,0);
	wr_port(FP_SPI_PIN_MODE+2,0);
	wr_port(FP_SPI_PIN_MODE+3,0);
	//0x000100a0;
	wr_port(FP_SPI_PIN_CTRL,0xa0);
	wr_port(FP_SPI_PIN_CTRL+1,0x0);
	wr_port(FP_SPI_PIN_CTRL+2,0x01);
	wr_port(FP_SPI_PIN_CTRL+3,0x0);
...
void main(void )
{
	...
	while(1)
	{
		...
		if(cnt == 10)  // polling status
		{
			...
			if(0 != u8KeyValue)
			{
				wr_port(LPM_AOMCU_MESSAGE, u8KeyValue);
				wr_port(LPM_AOMCU_MESSAGE+1, WAKE_UP_KEY);
			}
