...
#define ES7210_CHANNELS_MAX	MIC_CHN_8 //MIC_CHN_8  //modify for
...
#define I2C_DEFAULT_ADDR_0 0x41
#if ES7210_CHANNELS_MAX > MIC_CHN_4
#define I2C_DEFAULT_ADDR_1 0x40 //0x42
#endif
#if ES7210_CHANNELS_MAX > MIC_CHN_8
# define I2C_DEFAULT_ADDR_2 0x42
#endif
#if ES7210_CHANNELS_MAX > MIC_CHN_12
# define I2C_DEFAULT_ADDR_3 0x41
#endif

static unsigned short default_i2c_addr[ADC_DEV_MAXNUM] = {
    I2C_DEFAULT_ADDR_0,
#if ES7210_CHANNELS_MAX > MIC_CHN_4
    I2C_DEFAULT_ADDR_1,
#endif
...
#define ES7210_WL_16BIT 3
#define ES7210_WL_18BIT 2
#define ES7210_WL_20BIT 1
#define ES7210_WL_24BIT 0
#define ES7210_WL_32BIT 4
...
#define ES7210_WORK_MODE    ES7210_TDM_1LRCK_LJ

#if defined(CONFIG_PANTHER_SND_RX_I2C_BUS)
#define ES7210_I2C_BUS_NUM		CONFIG_PANTHER_SND_RX_I2C_BUS
#else
#error ES7210_I2C_BUS_NUM not defined
#endif
...
extern unsigned short mic_i2c_addr[4];
...
static int es7210_multi_chips_write(u8 reg, unsigned char value)
{
	...
	for(i=0; i< ADC_DEV_MAXNUM; i++){
		//pr_info("es7210_multi_chips_write i:%d,ctrl:0x%x,addr:0x%x\n",i,i2c_clt1[i],i2c_clt1[i]->addr);
				...
				struct snd_soc_dai *dai)
{
	...
	switch (params_format(params))
	{
		case SNDRV_PCM_FORMAT_S16_LE:
			bit_depth = ES7210_WL_16BIT;
			break;
		case SNDRV_PCM_FORMAT_S24_3LE:
			bit_depth = ES7210_WL_24BIT;
			break;
		case SNDRV_PCM_FORMAT_S32_LE:
			bit_depth = ES7210_WL_32BIT;
			break;
		default:
			return -EINVAL;
	}

	es7210_multi_chips_update_bits(ES7210_SDP_CFG1_REG11, 0xe0, bit_depth<<5);
	return 0;
}

#define es7210_RATES SNDRV_PCM_RATE_8000_96000

#define es7210_FORMATS (SNDRV_PCM_FMTBIT_S16_LE | SNDRV_PCM_FMTBIT_S24_3LE |\
...
#define SYM4_DAI_RX_PINMUX_REG 0xbf13c09C
#define SYM4_DAI_RXI2C_PINMUX_REG 0xbf13c070
static inline u32 ES7210_READ32(u32 x)
{
    u32 val = (*(volatile u32*)(SYMPHONY_I2C1_SW_BASE+(x)));
    return val;
}

static inline void ES7210_WRITE32(u32 x, u32 val)
{
    (*(volatile u32*)(SYMPHONY_I2C1_SW_BASE+(x)) = (u32)(val));
    wmb();
}

static void sym4_dai_rx_pinmux_cfg(void)
{
	void* reg = NULL;
    u32 idx = 0, val = 0;

    /* pinmux set */
	reg = ioremap(SYM4_DAI_RX_PINMUX_REG, 0x40);
	for(idx = 0; idx < 4; idx++) {
        val = readl(reg);
        val &= ~(0xf);
        val |= 0x4;
	...
	reg = ioremap(SYM4_DAI_RXI2C_PINMUX_REG, 0x20);
	for(idx = 0; idx < 2; idx++) {
        val = readl(reg);
        val &= ~(0x7);
        val |= 0x3;
	...
	ES7210_WRITE32(0x18,0x6);
    ES7210_WRITE32(0x1c,0x53);		
}
			...
			const struct i2c_device_id *i2c_id)
{
	...
	if (i2c_id->driver_data < ADC_DEV_MAXNUM) {
		...
		i2c_clt1[i2c_id->driver_data]->addr=(mic_i2c_addr[i2c_id->driver_data]<<1);
...
*/
#if 1
struct i2c_board_info es7210_i2c_board_info[] = {
#if ES7210_CHANNELS_MAX > 0
...
// return 0;
    ...
    sym4_dai_rx_pinmux_cfg();

	if (i2c_dev_num == 0) {
		i2c_dev_num = ADC_DEV_MAXNUM;
		for(i = 0; i < i2c_dev_num; ++i) {
			mic_i2c_addr[i] = default_i2c_addr[i];
		}
	}
#endif
	...
	else printk("register es7210 i2c ok!!!!");
	return ret;
}
