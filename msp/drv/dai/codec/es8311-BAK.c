			...
			struct snd_soc_dai *dai)
{
	...
	*/
	if(coeff >= 0) {
		...
		es8311_write(ES8311_CLK_MANAGER_REG02, 0x60);//wei debug
		...
		//es8311_write(ES8311_CLK_MANAGER_REG05, 0x7);//wei debug
		...
		//es8311_write(ES8311_CLK_MANAGER_REG06, regv);//wei debug
		es8311_write(ES8311_CLK_MANAGER_REG06, 0x38);
	}
...
static int es8311_suspend(struct snd_soc_codec *codec)
{
	...
	es8311_write(ES8311_SYSTEM_REG0E, 0xFF);
	es8311_write(ES8311_SYSTEM_REG12, 0x02);
	es8311_write(ES8311_SYSTEM_REG14, 0x00);
	es8311_write(ES8311_SYSTEM_REG0D, 0xFA);
	es8311_write(ES8311_ADC_REG15, 0x00);
	es8311_write(ES8311_DAC_REG37, 0x08);
	es8311_write(ES8311_RESET_REG00, 0x00);
...
static int es8311_probe(struct snd_soc_codec *codec)
{
...

	{
		...
		/*
		read_cmd[0] = ES8311_CHD1_REGFD;
		cmd_len = 1;
		
		if (i2c_clt1[0] == NULL)
			pr_err("es7210_read client->adapter==NULL\n");
		
		ret = i2c_master_send(i2c_clt1[0], read_cmd, cmd_len);
		if (ret != cmd_len) {
			pr_err("es7210_read error1\n");
			return -1;
		}
		printk("wei debug i2c read ret = %d\n",ret);
		ret = i2c_master_recv(i2c_clt1[0], &rt_value, 1);
		if (ret != 1) {
			pr_err("es7210_read error2, ret = %d.\n", ret);
			return -1;
		}
		*/
		ret = es8311_read(ES8311_CHD1_REGFD, &rt_value);
		printk("wei debug i2c recv ret = %d, rt_value=%d\n",ret,rt_value);
	}
	...
	//es8311_write(ES8311_SYSTEM_REG14, 0x1A);
	es8311_write(ES8311_SYSTEM_REG14, 0x12);
	if(es8311->dmic_enable == true) {
		es8311_update_bits(ES8311_SYSTEM_REG14,
				0x40, 0x40);
	}
			...
			struct device_attribute *attr, char *_buf)
{
	int ret;
	u8 value = 0;
	ret = es8311_read(cur_reg, &value);
	sprintf(_buf, "%s(): get 0x%04x=0x%04x\n",
			__FUNCTION__, cur_reg,value);
	return ret;
}
					...
					const struct i2c_device_id *id)
{
	 ...
	 for(idx = 0; idx < 2; idx++)
	{
		...
		val |= 0x3;
		writel(val, (void *)reg);
		printk("%s %d reg:0x%x val:0x%x \n", __func__, __LINE__, reg, readl((void *)reg));
		reg += 4;
	}
	iounmap(reg);
	...
	es8311->dmic_enable = true;     // dmic interface disabled
...
static int es8311_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	struct i2c_adapter *adapter = client->adapter;
	printk("wei debug %s!\n",__func__);

	if (adapter->nr == ES8311_I2C_BUS_NUM) {
		if (client->addr == 0x30) {
			printk("wei debug %s!\n",__func__);
			return 0;
		}
...
static struct i2c_driver es8311_i2c_driver = {
	...
	.address_list = es8311_i2c_addr,
	.detect = es8311_i2c_detect,
};
#endif
