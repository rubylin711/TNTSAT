...
unsigned short es8311_i2c_addr[1] = {ES8311_I2C_ADDR};
struct i2c_client *i2c_es8311[1] = {NULL};

#if defined(CONFIG_PANTHER_SND_RX_I2C_BUS)
#define ES8311_I2C_BUS_NUM		CONFIG_PANTHER_SND_RX_I2C_BUS
#else
#error ES8311_I2C_BUS_NUM not defined
#endif
...
static int es8311_read(u8 reg, u8 *rt_value)
{
	int ret;
	u8 read_cmd[3] = {0};
	u8 cmd_len = 0;

	if(NULL == i2c_es8311[0]) {
		pr_err("es8311_read client->adapter==NULL\n");
		return -1;
	}
	...
	ret = i2c_master_send(i2c_es8311[0], read_cmd, cmd_len);
	if (ret != cmd_len) {
		pr_err("es8311_read error1\n");
		return -1;
	}

	ret = i2c_master_recv(i2c_es8311[0], rt_value, 1);
	if (ret != 1) {
		pr_err("es8311_read error2, ret = %d.\n", ret);
		return -1;
	}

	return 0;
}
static int es8311_write(u8 reg, unsigned char value)
{
	int ret = 0;
	u8 write_cmd[2] = {0};

	if(NULL == i2c_es8311[0]){
		pr_err("es8311_write client->adapter==NULL\n");
		return -1;
	}

	write_cmd[0] = reg;
	write_cmd[1] = value;
	ret = i2c_master_send(i2c_es8311[0], write_cmd, 2);
	if (ret != 2) {
		pr_err("es8311_write error->[REG-0x%02x,val-0x%02x]\n",reg,value);
		return -1;
	}

	return 0;
}
static int es8311_update_bits(u8 reg, u8 mask, u8 value)
{
	u8 val_old,val_new;

	es8311_read(reg, &val_old);
	val_new = (val_old & ~mask) | (value & mask);
	if (val_new != val_old) {
		es8311_write(reg, val_new);
	}
...
static const struct _coeff_div coeff_div[] = {
	...
	{8192000 , 16000, 0x01, 0x01, 0x01, 0x01, 0x00, 0x01, 0xff, 0x10, 0x20, 0x40, 0x00},//wei debug
	...
	{11289600, 22050, 0x02, 0x01, 0x01, 0x01, 0x00, 0x01, 0xff, 0x10, 0x10, 0x10, 0x04},//wei debug
	...
	{11289600, 44100, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0xff, 0x08, 0x10, 0x10, 0x04},//wei debug
	...
	{12288000, 48000, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00, 0xff, 0x08, 0x10, 0x10, 0x04},//wei debug
	...
	{98304000, 48000, 0x08, 0x01, 0x08, 0x08, 0x00, 0x08, 0x00, 0x04, 0x10, 0x10, 0x04},//wei debug
	/* 64k */                                                                             	
	...
	{12288000, 96000, 0x01, 0x02, 0x01, 0x01, 0x00, 0x00, 0x7f, 0x04, 0x10, 0x10, 0x04},//wei debug
	...
	{98304000, 96000, 0x08, 0x02, 0x01, 0x01, 0x00, 0x03, 0xff, 0x04, 0x10, 0x10, 0x04},//wei debug
	//mclk	   rate  prediv mult adcdiv dacdiv fsmode lrch lrcl bckdiv osr dacosr adcscale
};
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	switch (freq) {
	case 98304000://wei add
	case 8192000://wei add
	case 11289600:
	case 22579200:
		es8311->mclk = freq;
		return 0;
...
static int es8311_set_dai_fmt(struct snd_soc_dai *codec_dai, unsigned int fmt)
{
	...
	u8 adciface = 0;
	u8 daciface = 0;
	es8311_read(ES8311_RESET_REG00, &iface);
	es8311_read(ES8311_SDPOUT_REG0A, &adciface);
	es8311_read(ES8311_SDPIN_REG09, &daciface);
	...
	es8311_write(ES8311_RESET_REG00, iface);
	...
	es8311_read(ES8311_CLK_MANAGER_REG06, &iface);
	/* clock inversion */
	...
	es8311_write(ES8311_CLK_MANAGER_REG06, iface);
	es8311_write(ES8311_SDPOUT_REG0A, adciface);
	es8311_write(ES8311_SDPIN_REG09, daciface);
	return 0;
}
			...
			struct snd_soc_dai *dai)
{
	...
	int coeff;
	u8 regv, datmp;
	printk("sample rate %dHz with %dHz MCLK\n",
	...
	*/
	if(coeff >= 0) {
		es8311_read(ES8311_CLK_MANAGER_REG02, &regv);
		regv &= 0x07;
		...
		es8311_write(ES8311_CLK_MANAGER_REG06, regv);
	  	es8311_read(ES8311_ADC_REG16, &regv);
		regv &= 0x38;
	  	regv |= (coeff_div[coeff].adcscale ) << 0;          
	  	es8311_write(ES8311_ADC_REG16, regv);
	}
	
	if(substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		es8311_read(ES8311_SDPIN_REG09, &iface);
		iface &= 0xE3;
		/* bit size */
		...
		/* set iface */
		es8311_write(ES8311_SDPIN_REG09, iface);
	} else {
		es8311_read(ES8311_SDPOUT_REG0A, &iface);
		iface &= 0xE3;
		/* bit size */
		switch (params_format(params)) {
			case SNDRV_PCM_FORMAT_S16_LE:
		...
		/* set iface */
		es8311_write(ES8311_SDPOUT_REG0A, iface);
	}
	return 0;
}
			...
			enum snd_soc_bias_level level)
{
	...
	case SND_SOC_BIAS_ON:
		...
		es8311_write(ES8311_ADC_REG16, 0x4);
		...
		if(es8311->mclkinv == true) {
			es8311_update_bits(ES8311_CLK_MANAGER_REG01,
					0x40, 0x40);
		}
		else {
			es8311_update_bits(ES8311_CLK_MANAGER_REG01,
		...
		if(es8311->sclkinv == true) {
			es8311_update_bits(ES8311_CLK_MANAGER_REG06,
					0x20, 0x20);
		}
		else {
			es8311_update_bits(ES8311_CLK_MANAGER_REG06,
		...
		es8311_read(ES8311_SYSTEM_REG14, &regv);
		regv &= 0xCF;
		//regv |= 0x1A;
		regv |= 0x12;
		es8311_write(ES8311_SYSTEM_REG14, regv);
		if(es8311->dmic_enable == true) {
			es8311_update_bits(ES8311_SYSTEM_REG14,
					0x40, 0x40);
		}
		else {
			es8311_update_bits(ES8311_SYSTEM_REG14,
		...
		es8311_write(ES8311_ADC_REG15, 0x31);
		...
		es8311_write(ES8311_GPIO_REG44, 0x68);
	...
	case SND_SOC_BIAS_STANDBY:
		es8311_write(ES8311_SYSTEM_REG12, 0x02);
		es8311_write(ES8311_DAC_REG32, 0x00);
		es8311_write(ES8311_ADC_REG17, 0x00);
		es8311_write(ES8311_SYSTEM_REG0E, 0xFF);
		es8311_write(ES8311_SYSTEM_REG0D, 0xFA);
		es8311_write(ES8311_ADC_REG15, 0x00);
		...
		es8311_write(ES8311_CLK_MANAGER_REG01, 0x00);
		es8311_write(ES8311_GP_REG45, 0x01);
		break;
...
{
	if(tristate) {
		es8311_update_bits(ES8311_CLK_MANAGER_REG07,
			0x30, 0x30);
	} else {
		es8311_update_bits(ES8311_CLK_MANAGER_REG07,
			0x30, 0x00);
...
static int es8311_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if (mute) {
		es8311_write(ES8311_SYSTEM_REG12, 0x02);
		es8311_update_bits(ES8311_DAC_REG31, 0x60, 0x60);
		es8311_write(ES8311_DAC_REG32, 0x00);
		es8311_write(ES8311_DAC_REG37, 0x08);
	} else {
		es8311_write(ES8311_DAC_REG32, 0x8F);
		es8311_update_bits(ES8311_DAC_REG31, 0x60, 0x00);
		es8311_write(ES8311_SYSTEM_REG12, 0x00);
		//printk("es8311 reg:\n");
		//for(i=0; i<=0x4f; i++){
			//es8311_read(i, &val);
			//printk("off:0x%02x, val:0x%02x  ",i,val);
			//if(i%16==0)printk("\n");
		//}
	}
	return 0;
...
static int es8311_probe(struct snd_soc_codec *codec)
{
	...
	es8311_write(ES8311_SYSTEM_REG12, 0x00);
	es8311_write(ES8311_SYSTEM_REG13, 0x10);
	es8311_write(ES8311_SDPIN_REG09, 0x00);
	es8311_write(ES8311_SDPOUT_REG0A, 0x00);
	es8311_write(ES8311_SYSTEM_REG0E, 0x02);
	es8311_write(ES8311_ADC_REG15, 0x40);
	es8311_write(ES8311_ADC_REG1B, 0x0A);
	es8311_write(ES8311_ADC_REG1C, 0x6A);
...
#define SYM4_DAI_TXI2C_PINMUX_REG 0xbf13c070
#define SYM4_DAI_TX_PINMUX_REG 0xbf13c08c
#define SYM4_DAI_TX_CLK_REG1 0xbf5d0098
#define SYM4_DAI_TX_CLK_REG2 0xbf5d0120
#define SYM4_DAI_TX_CLK_REG3 0xbf5d0128
#define SYM4_DAI_TX_CONFIG_REG 0xbf1cc200
					...
					const struct i2c_device_id *id)
{
	...
	//config pinmux for i2c
	reg = ioremap(SYM4_DAI_TXI2C_PINMUX_REG, 0x10);
	for(idx = 0; idx < 2; idx++) {
		val = readl(reg);
		val &= ~(0x7);
		val |= 0x3;
...
#if 0
//set tx clk for es8311 master mode
	reg = ioremap(SYM4_DAI_TX_CLK_REG1, 0x10);
	writel(0xf000281f, (void *)reg);
	iounmap(reg);

	reg = ioremap(SYM4_DAI_TX_CLK_REG2, 0x10);
	writel(0x32204081, (void *)reg);
	iounmap(reg);

	reg = ioremap(SYM4_DAI_TX_CLK_REG3, 0x10);
	writel(0, (void *)reg);
	iounmap(reg);

	reg = ioremap(SYM4_DAI_TX_CONFIG_REG, 0x10);
	writel(0x90401100, (void *)reg);
	iounmap(reg);
//set tx clk for es8311 master mode
#endif
...
static int __init es8311_init(void)
{
	...
	if (ret != 0)
		pr_err("Failed to register es8311 i2c driver : %d \n", ret);
	return ret;
