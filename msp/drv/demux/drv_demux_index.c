...
typedef struct mt_SC_INFO_S
{
    SC_TYPE_E     eScType;
    mt_s32        s32ScID;           /*! for H.264, assign PPS or SPS ID; for non-H.264, assign the byte after 00 00 01 */
    mt_s32        s32SupScID;        /*! for H.264 only, record the SPS ID for the current used PPS */
    mt_s64        s64GlobalOffset;   /*! the offset of the start code, in the global(whole) stream data space */
    mt_s32        s32OffsetInPacket; /*! the offset of the start code, in the stream data packet */
    mt_s32        s32PacketCount;    /*! the stream data packet number where the start code was found */
} SC_INFO_S;

/*! state of the instance */
typedef enum mt_CTX_STATE_E
{
    CTX_STATE_DISABLE = 0,
    CTX_STATE_ENABLE,
    CTX_STATE_BUTT
...
/*! context */
typedef struct mt_FIDX_CTX_S
{
    CTX_STATE_E   eCtxState;
    VIDSTD_E      eVideoStandard;  /*! video standard type */
    STRM_TYPE_E   eStrmType;       /*! stream type, ES or PES */
    mt_u32        u32PTS;          /*! current PTS, usually equals to the pts of the latest stream packet */
    SC_INFO_S     stSPS[32];       /*! start code of the sequence level parameters.
                                          H264  - sps
                                          MPEG2 - sequence header
                                          AVS   - sequence header
                                          MPEG4 - VOL or higher  */
    mt_u8         u8SpsFresh[32];  /*! one SPS can be used by one I frame only, an I frame without SPS will be treated as P or B frame */
    SC_INFO_S     stPPS[256];      /*! picture level parameter
                                          H264  - pps
                                          MPEG2 - picture header
                                          AVS   - picture header
                                          MPEG4 - VOP header  */
    mt_u8         s32SpsIdForPps[256];

    //previous 2 bytes, for start code detection, to prevent the 00 or 00 00 lost
    mt_u8         u8Prev2Bytes[2]; /*! store the latest 2 byte  */
    mt_u32        u32PrevPts;            /*! the pts of the packet where the last 2nd byte in */
    mt_s64        s64PrevGlobalOffset;   /*! the global offset of the last 2nd byte */
    mt_s32        s32PrevPacketOffset;   /*! the offset in the packet of the last 2nd byte */
    mt_s32        s32PrevPacketCount;    /*! the packet count of the packet where the last 2nd byte in */
    mt_s64        s64PrevGlobalOffset_1; /*! the global offset of the last 1st byte */
    mt_s32        s32PrevPacketOffset_1; /*! the offset in the packet of the last 1st byte */
    mt_s32        s32PrevPacketCount_1;  /*! the packet count of the packet where the last 1st byte in */

    //this SC store the latest tetected start code
    mt_s32        s32ThisSCValid;  /*! indicate the support data of this start code is ready for use */
    SC_INFO_S     stThisSC;                           /*! when a start code was found, the support data is probably not enough. If so, this start */
    mt_u8         u8ThisSCData[SC_SUPPORT_DATA_SIZE]; /*! has to be stored temporarily to wait more data */
    mt_s32        s32ThisSCDataLen;  /*! actual support data size, usually equals to SC_SUPPORT_DATA_SIZE */
    ...
    mt_s32        s32WaitFrameSize;  /*! indicate if most info of stNewFramePos has been ready, except the frame size */
    mt_s32        s32SpsID;          /*! H264: SPS ID */
    mt_s32        s32PpsID;          /*! H264: PPS ID */
    mt_u32       *Param;
}FIDX_CTX_S;
...
static mt_void (*s_OutputFramePosition)(mt_u32 *Param, FRAME_POS_S *pstScInfo);

/*=======================================================================
                           function declaration
  =======================================================================*/
static mt_s32 ProcessThisSC(mt_s32 InstIdx);
static mt_s32 ProcessSC_Mpeg2(mt_s32 InstIdx);
static mt_s32 ProcessSC_Avs(mt_s32 InstIdx);
static mt_s32 ProcessSC_Mpeg4(mt_s32 InstIdx);
static mt_s32 ProcessSC_H264(mt_s32 InstIdx);
...
do{                                                                     \
    ...
    {                                                                   \
        (mt_void)s_OutputFramePosition(pstCtx->Param, &pstCtx->stNewFramePos );       \
    }                                                                   \
...
static mt_s32 IsPesSC(mt_u8 Code, VIDSTD_E eVidStd )
{
    mt_s32 ret = 0;
...
************************************************************************/
mt_s32 ProcessThisSC(mt_s32 InstIdx)
{
    FIDX_CTX_S *pstCtx = &s_FidxIIS[InstIdx];
    mt_s32 s32Ret = FIDX_ERR;
...
@return
  if success return FIDX_OK, otherwise return FIDX_ERR
************************************************************************/
mt_s32 ProcessSC_Mpeg2(mt_s32 InstIdx)
{
    mt_u8 Code;
    mt_s32 s32Ret = FIDX_ERR;
    FIDX_CTX_S *pstCtx = &s_FidxIIS[InstIdx];
    ...
    if( pstFrmPos->eFrameType != FIDX_FRAME_TYPE_UNKNOWN )
    {
        ...
        {
            pstFrmPos->s32FrameSize = (mt_s32)(pstThisSC->s64GlobalOffset - pstFrmPos->s64GlobalOffset);
...
************************************************************************/
mt_s32 ProcessSC_Avs(mt_s32 InstIdx)
{
    mt_u8 Code;
    mt_s32 s32Ret = FIDX_ERR;
    FIDX_CTX_S *pstCtx = &s_FidxIIS[InstIdx];
...
@return
  ...
  return FIDX_OK, otherwise return FIDX_ERR
************************************************************************/
mt_s32 ProcessSC_Mpeg4(mt_s32 InstIdx)
{
    mt_u8 Code;
    mt_s32 s32Ret = FIDX_ERR;
    FIDX_CTX_S *pstCtx = &s_FidxIIS[InstIdx];
...
static mt_u32 simple_ue_v( mt_u32 Code, mt_u32 *BitNum )
{
    mt_u32 Val;
    mt_u32 zeros;
    zeros = (mt_s32)PvrZerosMS_32(Code);
    if (zeros < 16)
    {
        *BitNum = (zeros<<1) + 1;
...
static mt_s32  ParseSPS( mt_u8 *pu8Data, mt_s32 s32DataLen, mt_s32 *p32SpsID )
{
    mt_u32  u32Code, u32Val, u32BitNum;

    u32Code = ((mt_u32)pu8Data[4]<<24) | ((mt_u32)pu8Data[5]<<16) |
        ((mt_u32)pu8Data[6]<<8) | ((mt_u32)pu8Data[7]);

    u32Val = simple_ue_v( u32Code, &u32BitNum );
    FIDX_ASSERT_RET( u32Val<=31, "SPS ID out of range!" );
    FIDX_ASSERT_RET( ((u32BitNum+7)>>3)<=(mt_u32)(s32DataLen-4), "data NOT enough for SPS" );

    *p32SpsID = (mt_s32)u32Val;
    return FIDX_OK;
}

static mt_s32  ParsePPS( mt_u8 *pu8Data, mt_s32 s32DataLen, mt_s32 *ps32SPSID, mt_s32 *ps32PPSID )
{
    mt_s32 s32Code;
    mt_u32 u32Val, u32BitNum;
    PVR_BS Bs;
    ...
    u32Val = simple_ue_v((mt_u32)s32Code,&u32BitNum );
    FIDX_ASSERT_RET( u32Val<=255, "PPS ID out of range!" );
    FIDX_ASSERT_RET((mt_s32)BITS_2_BYTES((mt_u32)PvrBsPos(&Bs))<= s32DataLen, "data NOT enough for PPS" );
    *ps32PPSID = (mt_s32)u32Val;
    PvrBsSkip(&Bs, (mt_s32)u32BitNum );

    /* seq_parameter_set_id */
    s32Code = PvrBsShow(&Bs, 32);
    u32Val = simple_ue_v( (mt_u32)s32Code, &u32BitNum );
    FIDX_ASSERT_RET( u32Val<=31, "SPS ID out of range!" );
    FIDX_ASSERT_RET( (mt_s32)BITS_2_BYTES((mt_u32)PvrBsPos(&Bs))<=s32DataLen, "data NOT enough for SPS" );
    *ps32SPSID = (mt_s32)u32Val;

    return FIDX_OK;
}

static mt_s32  ParseSliceHeader( mt_u8 *pu8Data, mt_s32 s32DataLen, mt_s32 *ps32FirstMBInSlice,
    mt_s32 *ps32SliceType, mt_s32 *ps32PPSId )
{
    mt_s32  s32Code;
    mt_u32  u32Val, u32BitNum;
    PVR_BS Bs;
    ...
    FIDX_ASSERT_RET( (mt_s32)BITS_2_BYTES((mt_u32)PvrBsPos(&Bs))<=s32DataLen, "data NOT enough for 'first_mb_in_slice'" );
    *ps32FirstMBInSlice = (mt_s32)u32Val;
    PvrBsSkip(&Bs, (mt_s32)u32BitNum );
    ...
    FIDX_ASSERT_RET( (mt_s32)BITS_2_BYTES((mt_u32)PvrBsPos(&Bs))<=s32DataLen, "data NOT enough for 'slice_type'" );
    *ps32SliceType = (mt_s32)u32Val;
    PvrBsSkip(&Bs, (mt_s32)u32BitNum );
    ...
    FIDX_ASSERT_RET( (mt_s32)BITS_2_BYTES((mt_u32)PvrBsPos(&Bs))<=s32DataLen, "data NOT enough for 'pic_parameter_set_id'" );
    *ps32PPSId = (mt_s32)u32Val;
...
static mt_void H264MakeFrame( FIDX_CTX_S *pstCtx )
{
    SC_INFO_S   *pstSpsSC = &pstCtx->stSPS[0];
    ...
    if( FIDX_FRAME_TYPE_I == pstFrmPos->eFrameType && 1 == pstCtx->u8SpsFresh[pstCtx->s32SpsID] )
    {
        ...
        {
            pstFrmPos->s32FrameSize = (mt_s32)(pstThisSC->s64GlobalOffset -
                            pstCtx->stSPS[pstCtx->s32SpsID].s64GlobalOffset);
        }
        else
        {
            pstFrmPos->s32FrameSize = (mt_s32)(pstEndSC->s64GlobalOffset -
                            pstCtx->stSPS[pstCtx->s32SpsID].s64GlobalOffset);
        }
    }
    else
    {
        ...
        {
            pstFrmPos->s32FrameSize = (mt_s32)(pstEndSC->s64GlobalOffset - pstFrmPos->s64GlobalOffset);
        }
    }
...
if success(find the start of a frame, and can output one index record)
  return FIDX_OK, otherwise return FIDX_ERR
************************************************************************/
mt_s32 ProcessSC_H264(mt_s32 InstIdx)
{
    mt_s32 s32Ret = FIDX_ERR;
    mt_s32 NalUnitType;
    mt_s32 s32SpsID, s32PpsID, s32FirstMb, s32SliceType;
    FIDX_CTX_S *pstCtx = &s_FidxIIS[InstIdx];
    SC_INFO_S   *pstSpsSC = &pstCtx->stSPS[0];
    ...
    SC_INFO_S   *pstThisSC = &pstCtx->stThisSC;
    FRAME_POS_S *pstFrmPos = &pstCtx->stNewFramePos;
    mt_u8       *pu8Data = &pstCtx->u8ThisSCData[0];
    static  mt_s32  SlcType2FrmType[10] = {2,3,1,0,0,2,3,1,0,0};
    ...
    NalUnitType = (mt_s32)(pu8Data[0] & 31);
        ...
        if( FIDX_OK == (s32Ret=ParseSliceHeader(pu8Data, pstCtx->s32ThisSCDataLen, &s32FirstMb, &s32SliceType, &s32PpsID)) )
        {
            ...
            if( FIDX_FRAME_TYPE_UNKNOWN != pstFrmPos->eFrameType )
            {
                ...
                else
                {
                    ...
                    mt_s32 s32FrmType = SlcType2FrmType[s32SliceType];
 ...
 ************************************************************************/
mt_void  FIDX_FeedStartCode(mt_s32 InstIdx, const FINDEX_SCD_S *pstSC)
{
    FIDX_CTX_S *pstCtx;
    ...
    if( 1 == IsPesSC(pstSC->u8StartCode, pstCtx->eVideoStandard) )
    {
        ...
        if( NULL != s_OutputFramePosition )
        {
            (mt_void)s_OutputFramePosition( pstCtx->Param, &PesFrame );
        }
