...
typedef struct
{
    ...
    mt_u8           u8Payload[184]; //include adaption and payload
} TS_PACKET;
...
typedef struct
{
    ...
    mt_u32       u32WritePtr;
    mt_u32       u32ProcesPtr;
    mt_u32       u32ProcesOffset;
    mt_u32       u32WriteOffset;
    mt_u8        u8TmpDataArry[2];
    mt_u8        u8TmpArryLen;
} st_SW_DataBuf;
...
typedef struct
{
    mt_u32        u32UsedFlag;
    mt_u32        u32RunFlag;
    mt_u32        u32CC; //continuity_counter in ts header
    st_SW_DataBuf stDataBuf;
    struct semaphore stSwMutex;
} ST_SW_ChnInfor;

static mt_u32 gu32SWThreadRunFlag = 0;

static ST_SW_ChnInfor stSwChn[DMX_CHANNEL_CNT];
static struct task_struct *pstDemuxSwThread = MT_NULL;
...
static mt_s32  DMX_SwCheckTsPacket(mt_u32 u32ChId, TS_PACKET* ts)
{
    mt_u32 u32CC, u32CCNeed;
    mt_u8 u8Sync = 0;
    mt_u8 u8Pusi;
    mt_u8* p;
...
static mt_s32  DMX_SwInsertData(mt_u32 u32ChId, mt_u8* pu8Data, mt_u32 u32DataLen)
{
    return MT_SUCCESS;
}
...
none
 ...
 \li ::
 */
static mt_s32  DMX_SwGetDataStatus(mt_u32 u32ChId, st_SW_DataBuf* pstBufStatus)
{
...
/**
 ...
 \attention:return MT_FAILURE when the channel have no filter
none
 \param[in] Sec
 \param[in] ChnID

 \retval none
 \return MT_SUCCESS pass
        MT_FAILURE unpass

 \see
 \li ::
 */
mt_s32 DMX_SwSectionFilterCheck(mt_u32 ChanId, mt_u8 *Sect, mt_u32 SectLen)
{
...
mt_s32 DMX_SwSectionCRC(mt_u32 ChnID, mt_u8 *Sec, mt_u32 u32SecLen)
{
    DMX_DEV_OSI_S *pDmxDevOsi = g_pDmxDevOsi;
    MT_UNF_DMX_CHAN_CRC_MODE_E CrcMode;
    mt_u32 u32Crc;

    CrcMode = pDmxDevOsi->DmxChanInfo[ChnID].ChanCrcMode;
    if (    (MT_UNF_DMX_CHAN_CRC_MODE_FORCE_AND_DISCARD == CrcMode)
        || ((MT_UNF_DMX_CHAN_CRC_MODE_BY_SYNTAX_AND_DISCARD == CrcMode) && (Sec[1] & 0x80)) )
    {
        if (u32SecLen < 7) //crc error len < 7
        {
            MT_WARN_DEMUX("crc error:len < 7,len:%d!\n",u32SecLen);
            return MT_FAILURE;
        }
 ...
 */
static mt_u32 DMX_SwCheckIfChnOvfl(mt_u32 u32ChId)
{
    st_SW_DataBuf* pstBufStatus = MT_NULL;
    mt_u32 u32UsedSize;
...
static mt_s32 DMX_SwGetNewBlock(mt_u32 u32ChId, mt_u32 *pu32NewBlk)
{
...
static mt_s32 DMX_SwParseTmpData(mt_u32 u32ChId)
{
    st_SW_DESC*    pstWriteDesc;
    st_SW_DataBuf* pstDataBuf;
    mt_u8*         pu8Data;
    mt_u32 u32WriteBlk;
    mt_u32 u32TempDataLen;
    mt_u32 u32SecLen;
    ...
    do
    {
        ...
        pu8Data   = (mt_u8*)(u32BufVirAddr + pstWriteDesc->u16DataLen);
        u32SecLen = ((pu8Data[1] & 0x0f) << 8) + pu8Data[2] + 3;
        if (u32TempDataLen >= u32SecLen)
        {
            if (DMX_SwSectionFilterCheck(u32ChId, pu8Data, u32SecLen) != MT_SUCCESS
                || DMX_SwSectionCRC(u32ChId, pu8Data, u32SecLen) != MT_SUCCESS)
            {
                //drop this section
                if (u32TempDataLen > u32SecLen)
                {
                ...
                {
                    MT_ERR_DEMUX("DMX_SwParseTmpData error:offset:%x,len:%x!\n", pstDataBuf->u32WriteOffset,
...
static mt_s32 DMX_SwClrTmpData(mt_u32 u32ChId)
{
    st_SW_DESC*    pstWriteDesc;
    st_SW_DataBuf* pstDataBuf;
    mt_u32 u32WriteBlk;
...
static mt_s32  DMX_SwInsertTmpData(mt_u32 u32ChId, mt_u8* pu8Data, mt_u32 u32DataLen, mt_u32 u32Pusi)
{
    st_SW_DataBuf* pstDataBuf;
    st_SW_DESC*    pstWriteDesc;
    mt_u32 u32TempDataLen, u32ValidBufLen, u32NeedBufLen;
    mt_u32 u32TempDataStartAddr;
    mt_u32 u32WriteBlk;
    mt_s32 ret;
    mt_u32 u32BufVirAddr;
    mt_u32 u32CurSecLen;
        ...
        if (u32DataLen >= 3)
        {
            do
            {
                ...
                if (u32ValidBufLen < u32NeedBufLen)
                {
                    ...
                    if (MT_SUCCESS != ret)
                    {
                        MT_WARN_DEMUX("DMX_SwGetNewBlock error:%x!\n", ret);
                        DMX_SwClrTmpData(u32ChId);
                        return MT_FAILURE;
                    }
                ...
                {
                    memcpy((mt_void*)(u32BufVirAddr + pstDataBuf->u32WriteOffset), pu8Data, u32NeedBufLen);
                ...
                {
                    memcpy((mt_void*)(u32BufVirAddr + pstDataBuf->u32WriteOffset), pu8Data, u32DataLen);
    ...
    else
    {
        if (!u32TempDataLen)
        {
            ...
            {
                return MT_SUCCESS; //no temp data and no pusi,drop it
            }
            else
            {
                if ((pstDataBuf->u8TmpArryLen + u32DataLen) >= 3)
                {
                    ...
                    memcpy((mt_void*)(u32BufVirAddr + pstDataBuf->u32WriteOffset), pstDataBuf->u8TmpDataArry,
        ...
        else //u32TempDataLen is >= 3
        {
            ...
            u32CurSecLen = ((*(mt_u8*)(u32TempDataStartAddr + 1) & 0x0f) << 8)
                            + *(mt_u8*)(u32TempDataStartAddr + 2) + 3;
        }
        ...
        {
            MT_ERR_DEMUX("error:tmp bufferlen wrong,check it:%x,seclen:%x!\n", u32TempDataLen,
                         u32CurSecLen);
            return MT_FAILURE;
        }
        ...
        if (u32ValidBufLen < u32NeedBufLen)
        {
            MT_ERR_DEMUX("error:buffer status wrong,check it:%d,%d,seclen:%d,phyaddr:%x,datalen:%x!\n", u32ValidBufLen,
...
static mt_s32  DMX_SwResetChannel(mt_u32 u32ChId)
{
    st_SW_DataBuf* pstDataBuf;
    st_SW_DESC*    pstWriteDesc;
...
static mt_s32 DMX_SwGetTsHeaderLen(TS_PACKET* ts, mt_u32 *pu32HeaderLen)
{
    /*
...
static mt_s32  DMX_SwParseTsPacket(mt_u32 u32ChId, TS_PACKET* ts)
{
    mt_u8 u8Pointer;
    mt_u32 u32HeaderLen;
    mt_s32 s32Ret;
    ...
    {
        MT_WARN_DEMUX("ts header syntax error!\n");
        return MT_FAILURE;
    }
    ...
    if (ts->ts_header.bits.head_byte1 & 0x40) //payload_unit_start_indicator
    {
        ...
        {
            MT_WARN_DEMUX("pointer invalid:%d,header:%d!\n", u8Pointer,u32HeaderLen);
            return MT_FAILURE;
        }
        if (u8Pointer)
        {
            ...
            {
                MT_WARN_DEMUX("insert tmp data error:%x!\n", s32Ret);
            }
        }
...
static mt_u32 DMX_SwGetRunChannelNum(mt_void)
{
    mt_u32 i, u32Count = 0;
...
//#define DMX_SW_DEBUG
#ifdef DMX_SW_DEBUG
static struct file * g_stTsHandle = MT_NULL;
#endif
static mt_s32 MT_DMX_SWExtractthread(mt_void *arg)
{
...
#ifdef DMX_SW_DEBUG
    g_stTsHandle = mt_drv_file_open("/hi/rv.ts", 1);
#endif
    ...
    while (1)
    {
        for (i = 0; i < DMX_CHANNEL_CNT; i++)
        {
            ...
            //read ts packet
            ret = DMX_OsiReadDataRequest(i, DMX_ACQUIRE_BUF_NUM, &u32AcquiredNum, stReqBufTmp, 0);
            if (MT_SUCCESS != ret)
            {
                UNLOCKSWCHN(i);
                MT_WARN_DEMUX("read data error:%x!\n", ret);
                continue;
            }
...
#ifdef DMX_SW_DEBUG
                mt_drv_file_write(g_stTsHandle, u32BufVirAddr, 188);
#endif
                if (MT_SUCCESS == DMX_SwCheckTsPacket(i, (TS_PACKET*)u32BufVirAddr) && !(DMX_SwCheckIfChnOvfl(i)))
                {
                    if (MT_SUCCESS != DMX_SwParseTsPacket(i, (TS_PACKET*)u32BufVirAddr))
                    {
                        //pare ts packet error
                        MT_WARN_DEMUX("channel:%d parse ts packet error!\n", i);
                    }
                }
            }
            ...
            if (MT_SUCCESS != ret)
            {
                MT_ERR_DEMUX("release data error:%x\n", ret);
            }
...
#ifdef DMX_SW_DEBUG
    mt_drv_file_close(g_stTsHandle);
#endif
    return MT_SUCCESS;
}

mt_s32 MT_DMX_SwInit()
{
    mt_u32 i;
    ...
    for (i = 0; i < DMX_CHANNEL_CNT; i++)
    {
        MT_INIT_MUTEX(&stSwChn[i].stSwMutex);
    }
...
mt_s32 MT_DMX_SwNewChannel(mt_u32 u32ChannelId)
{
    ...
    if ((MT_UNF_DMX_CHAN_TYPE_SEC != ChanInfo->ChanType) && (MT_UNF_DMX_CHAN_TYPE_ECM_EMM != ChanInfo->ChanType))
    {
        MT_ERR_DEMUX("not support channel type:%d\n", u32ChannelId);
        return MT_FAILURE;
    }
    ...
    s32Ret = MT_DRV_MMZ_AllocAndMap(BufName, MT_NULL, ChanInfo->ChanBufSize, 4,
                                       &(stSwChn[u32ChannelId].stDataBuf.dataBuf));
    if (MT_SUCCESS != s32Ret)
    {
        MT_FATAL_DEMUX("Malloc %s desc failed\n", BufName);
        return MT_ERR_DMX_ALLOC_MEM_FAILED;
    }

    snprintf(BufName, 16,"DMX_SWDescriptor[%u]", u32ChannelId);
    s32Ret = MT_DRV_MMZ_AllocAndMap(BufName, MT_NULL, u32BlockNum * sizeof(st_SW_DESC), 4,
                                       &(stSwChn[u32ChannelId].stDataBuf.dscBuf));
    if (MT_SUCCESS != s32Ret)
    {
        MT_FATAL_DEMUX("Malloc %s desc failed\n", BufName);
        MT_DRV_MMZ_UnmapAndRelease(&stSwChn[u32ChannelId].stDataBuf.dataBuf);
        return MT_ERR_DMX_ALLOC_MEM_FAILED;
    }
...
mt_s32 MT_DMX_SwDestoryChannel(mt_u32 u32ChannelId)
{
    ...
    {
        MT_DMX_SwCloseChannel(u32ChannelId);
    }
    ...
    MT_DRV_MMZ_UnmapAndRelease(&stSwChn[u32ChannelId].stDataBuf.dataBuf);
    MT_DRV_MMZ_UnmapAndRelease(&stSwChn[u32ChannelId].stDataBuf.dscBuf);
...
mt_s32 MT_DMX_SwOpenChannel(mt_u32 u32ChannelId)
{
    ...
    {
        MT_ERR_DEMUX("channel not in use:%d\n", u32ChannelId);
        return MT_FAILURE;
    }
    ...
    {
        pstDemuxSwThread = kthread_create(MT_DMX_SWExtractthread, NULL, "DmxSw"); //create thread
...
mt_s32 MT_DMX_SwCloseChannel(mt_u32 u32ChannelId)
{
    ...
    if ((DMX_SwGetRunChannelNum() == 0) && (pstDemuxSwThread != MT_NULL) && gu32SWThreadRunFlag) //no running sw channel,stop thread
    {
        kthread_stop(pstDemuxSwThread);
        gu32SWThreadRunFlag = 0;
        pstDemuxSwThread = MT_NULL;
    }
...
{
    mt_s32 s32Ret;
    st_SW_DataBuf* pstBufStatus = NULL;
...
extern mt_u32 GetSectionLength(ulong u32BufStartAddr, mt_u32 u32BufLen, mt_u32 u32Offset);
extern mt_u32 s_u32DMXDropCnt[];
static mt_s32 DMXSwParseSection(mt_u32 *pu32Parsed, mt_u32 u32AcqNum, mt_u32 *pu32Offset,
                                phys_addr_t u32BufPhyAddr, mt_u32 u32BufLen, mt_u32 u32ChnID,
                                DMX_UserMsg_S* psMsgList)
{
    ...
    u32BufVirAddr = u32BufPhyAddr
                    - pstDataBuf->dataBuf.startPhyAddr
                    + pstDataBuf->dataBuf.startVirAddr;
    ...
    for (i = *pu32Parsed; i < u32AcqNum; i++)
    {
        ...
        if ((0 == u32Seclen) || (u32Seclen > DMX_MAX_SEC_LEN))
        {
            ...
            MT_ERR_DEMUX("  Section Parser  Error seclen:%x,phaddr:%x,u32BufLen:%x,offset:%x!\n", u32Seclen,
...
static mt_s32 DMX_SwReadDataRequset(mt_u32         u32ChId,
                                    mt_u32         u32AcqNum,
                                    mt_u32 *       pu32AcqedNum,
                                    DMX_UserMsg_S* psMsgList)
{
    ...
    mt_u32 u32DataLen, u32Offset;
    mt_u32 u32WriteBlk, u32ProcsBlk;
    mt_s32 s32Ret;
    ...
    *pu32AcqedNum = 0;
    ...
    {
        pstCurDesc = (st_SW_DESC*)(pstDataBuf->dscBuf.startVirAddr + u32WriteBlk * DMX_SW_DESC_SIZE);
        u32DataLen = pstCurDesc->u16DataLen;
                                ...
                                mt_u32         u32TimeOutMs)
{
    ...
    MT_DRV_SYS_GetTimeStampMs(&TimeMs);
    ...
    UNLOCKSWCHN(u32ChId);
    if ((s32Ret == MT_SUCCESS) && pu32AcqedNum)
    {
        pChInfo->u32HitAcq++;
        return MT_SUCCESS;
    }
...
mt_s32 MT_DMX_SwReleaseReadData(mt_u32 u32ChId, mt_u32 u32RelNum, DMX_UserMsg_S* psMsgList)
{
    ...
    DMX_DEV_OSI_S  *pDmxDevOsi = g_pDmxDevOsi;
    DMX_ChanInfo_S   *pChInfo;
    mt_u32 u32RealseFlag, u32NextBlkValidFlag;
    mt_u32 u32WriteBlk, u32ReadBlk, u32ReadBlkNext;
    mt_u32 i = 0;
    mt_u32          TimeMs;
    ...
    while (u32ReadBlk != u32WriteBlk)
    {
        ...
        for (; i < u32RelNum; i++)
        {
            ...
            else if (u32NextBlkValidFlag)
            {
                ...
                {
                    MT_ERR_DEMUX("psMsgList[%d].u32BufStartAddr:%x,len:%x,bufferAddr:%x,len:%x,nextptr:%x,nexlen:%x,desc phy:%x\n",
 ...
 */
mt_s32 MT_DMX_SwPeekDataRequest(mt_u32         u32ChId,
                                mt_u32         u32PeekLen,
                                DMX_UserMsg_S* psMsgList)
{
    mt_s32 s32Ret;
    DMX_DEV_OSI_S  *pDmxDevOsi = g_pDmxDevOsi;
    DMX_ChanInfo_S *pChInfo;
    mt_u32 u32WriteBlk, u32ProcsBlk,u32Offset;
    mt_u32 u32AcqedNum;
    st_SW_DataBuf* pstDataBuf;
