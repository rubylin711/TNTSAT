...
//#define AO_MEM_TRACE_DEBUG            /* MT_ID_AO   memory debug trace  */
//#define AIAO_MEM_TRACE_DEBUG          /* MT_ID_AIAO memory debug trace  */  
...
#ifdef __KERNEL__
    pMemAddr = MT_KMALLOC(u32ModuleID, u32Size, flag);
#else
    pMemAddr = malloc(u32Size);
#endif

#ifdef AIAO_MEM_TRACE_DEBUG
    if (MT_ID_AIAO != u32ModuleID)
    {
        MT_ERR_AO("u32ModuleID(0x%x) should be(0x%x) \n", u32ModuleID, MT_ID_AIAO);
    }

    MT_ERR_AO("malloc u32ModuleID(0x%.4x), size(0x%.6x),pMemAddr(%p), aiao_cnt(%d) \n", u32ModuleID, u32Size, pMemAddr,g_u32MallocCount_AIAO);
...
mt_void AUTIL_AIAO_FREE(mt_u32 u32ModuleID, mt_void* pMemAddr)
{
    if (NULL != pMemAddr)
    {
        ...
        MT_ERR_AO("free u32ModuleID(0x%x), pMemAddr(%p), aiao_cnt(%d) \n", u32ModuleID, pMemAddr,g_u32MallocCount_AIAO);
#endif
#ifdef __KERNEL__
        MT_KFREE(u32ModuleID, pMemAddr);
#else
        free(pMemAddr);
#endif
    }
...
mt_void* AUTIL_AO_MALLOC(mt_u32 u32ModuleID, mt_u32 u32Size, mt_s32 flag)
{
    ...
    if (MT_ID_AO != u32ModuleID)
    {
        MT_ERR_AO("u32ModuleID(0x%x) should be(0x%x) \n", u32ModuleID, MT_ID_AO);
    }

    MT_ERR_AO("malloc u32ModuleID(0x%.4x), size(0x%.6x),pMemAddr(%p), ao_cnt(%d) \n", u32ModuleID, u32Size, pMemAddr,g_u32MallocCount_AO);
    g_u32MallocCount_AO++;    
#endif
...
mt_void AUTIL_AO_FREE(mt_u32 u32ModuleID, mt_void* pMemAddr)
{
    if (NULL != pMemAddr)
    {
        ...
        MT_ERR_AO("free u32ModuleID(0x%x), pMemAddr(%p), ao_cnt(%d) \n", u32ModuleID, pMemAddr,g_u32MallocCount_AO);
#endif
#ifdef __KERNEL__
        MT_KFREE(u32ModuleID, pMemAddr);
#else
...
static int AOE_VOL2IDX(mt_u16 vol_reg)
{
    int idx;
 ...
 */
static mt_u32 g_u32IEC61937HbrDataType[] =
{
...
mt_s32  AUTIL_isIEC61937Hbr(mt_u32 u32IEC61937DataType, mt_u32 uSourceRate)
{
    ...
    for (n = 0; n < (mt_s32)(sizeof(g_u32IEC61937HbrDataType) / sizeof(mt_s32)); n++)
    {
...
mt_u32 AUTIL_CalcFrameSize(mt_u32 uCh, mt_u32 uBitDepth)
{
    mt_u32 uFrameSize = 0;
...
****************************************************************************/
{
    mt_s16 e, eindex;                // Exponent and index into table
...
mt_u32 AUTIL_VolumeLinear2RegdB(mt_u32 u32Linear)
{
    ...
    else
    {
        ...
        RegdB = (((mt_u32) ((20 * FHLL_log10(u32Linear << 16)) + 0x8000) >> 16) - 40) + VOLUME_0dB;
    }
...
{
    return (mt_u32)(dBVol + VOLUME_0dB);
}
...
{
    const mt_char *apcName[MT_UNF_AI_BUTT] =
    {
        "I2S0",
        "I2S1",
        "ADC0",
...
{
    const mt_char *apcName[MT_UNF_TRACK_MODE_BUTT] =
    {
        "STEREO",
        "DOULBE MONO",
        "DOULBE LEFT",
    ...
    if (enMode < MT_UNF_TRACK_MODE_BUTT)
    {
...
const mt_char *AUTIL_ScmsMode2Name(MT_UNF_SND_SPDIF_SCMSMODE_E enScms)
{
    switch(enScms)
    {
        case MT_UNF_SND_SPDIF_SCMSMODE_COPYALLOW:
            return "CopyAllow";
        case MT_UNF_SND_SPDIF_SCMSMODE_COPYONCE:
            return "CopyOnce";
        case MT_UNF_SND_SPDIF_SCMSMODE_COPYNOMORE:
            return "CopyNoMore";
        case MT_UNF_SND_SPDIF_SCMSMODE_COPYPROHIBITED:
            return "CopyDefy";
        default:
            return "UnknownScms";
    }
...
{
    const mt_char *apcName[MT_UNF_SND_HDMI_MODE_BUTT] =
    {
        "PCM",
        "RAW",
        "HBR2LBR",
    ...
    if (enMode < MT_UNF_SND_HDMI_MODE_BUTT)
    {
        return apcName[enMode];
    }
...
{
    const mt_char *apcName[MT_UNF_SND_SPDIF_MODE_BUTT] =
    {
        "PCM",
        "RAW",
    };

    if (enMode < MT_UNF_SND_SPDIF_MODE_BUTT)
    {
        return apcName[enMode];
    }
...
{
    const mt_char *apcName[SND_ENGINE_TYPE_BUTT] =
    {
        "PCM",
        "SPDIF RAW",
        "HDMI RAW",
...
mt_void AUTIL_OS_GetTime(mt_u32 *t_ms)
{
	...
	*t_ms = (tv.tv_nsec/1000000) + tv.tv_sec * 1000;
#endif
	return;
...
{
    AUTIL_CMTP_PLATFORM_E enChipPlatform;
    
#if    defined (CMTP_TYPE_hi3751v100) \
            || defined (CMTP_TYPE_hi3751v200) \
            || defined(CMTP_TYPE_hi3796cv100_a) \
            || defined(CMTP_TYPE_hi3796cv100) \
            || defined(CMTP_TYPE_hi3798cv100_a) \
            || defined(CMTP_TYPE_hi3798cv100)
            enChipPlatform = AUTIL_CMTP_TYPE_PLATFORM_S5;    
#else
            enChipPlatform = AUTIL_CMTP_PLATFORM_S40;
#endif
