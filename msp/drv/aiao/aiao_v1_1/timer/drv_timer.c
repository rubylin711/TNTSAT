...
static HI_U32 Timer_AllocHandle(HI_HANDLE *phHandle)
{
    HI_U32 i;
    ...
    {
        HI_ERR_AI("Too many Timer channel!\n");
        return HI_FAILURE;
    }
...
static HI_U32 Timer_Create(AIAO_Timer_Create_S *pstParam,HI_HANDLE handle)
{
    HI_S32 Ret,i;
    ...
    for(i = 0;i < TIMER_MAX_TOTAL_NUM;i++)
    {
        if(g_pstGlobalTIMERRS.pstTIMER_ATTR_S[i])
        {
            ...
            {
                HI_ERR_AI("This Timer has been occupied!\n");
                return HI_FAILURE;
            }
...
HI_U32 Timer_Drv_Create(HI_HANDLE *phHandle,AIAO_TimerIsrFunc **pFunc)
{
    HI_HANDLE hHandle;
    HI_S32 Ret;
...
HI_U32 Timer_Drv_SetAttr(HI_HANDLE handle,HI_U32 u32Config)
{
    AIAO_TIMER_ID_E enTimerID;
    TIMER_CHANNEL_STATE_S *state = HI_NULL;
    ...
    stParam.stIfAttr.enRate = HI_UNF_SAMPLE_RATE_48K;
...
HI_U32 Timer_Drv_SetEnable(HI_HANDLE handle,HI_BOOL bEnalbe)
{
    TIMER_CHANNEL_STATE_S *state = HI_NULL; 
    AIAO_TIMER_ID_E enTimerID;
    HI_S32 Ret;
...
HI_S32 HI_DRV_Timer_Create(HI_HANDLE *phHandle,AIAO_TimerIsrFunc **pFunc)
{
    HI_S32 Ret;
    
    Ret = Timer_Drv_Create(phHandle,pFunc);
...
HI_S32 HI_DRV_Timer_SetAttr(HI_HANDLE handle,HI_U32 u32Config)
{
    HI_S32 Ret;
    
    Ret = Timer_Drv_SetAttr(handle,u32Config);
