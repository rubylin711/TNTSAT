...
struct hiaudio_data {
    struct hisi_aiao_hwparams sthwparam;    //ao hw params
    struct mutex mutex;
...
#ifdef HI_ALSA_AI_SUPPORT
    struct hisi_aiao_hwparams sthwparam_ai; //ai hw params
    int ai_handle;
    struct file cfile;
...
#ifdef CONFIG_PM
	unsigned int pm_aoe_readpos;	//save aoe readpos when suspend
	unsigned int pm_aoe_writepos;
	unsigned int suspend_state;
	unsigned int suspend_isradjust; //save soft irq when suspend
#endif	
};
