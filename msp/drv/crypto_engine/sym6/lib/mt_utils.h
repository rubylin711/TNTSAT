...
#ifndef REG32
#define REG32(addr) (*(volatile unsigned int *)((ulong)addr))
#endif
...
/*
 * @brief According to Sym6 SoC bus mapping, the HW supports up to 4GB of memory addressing,
 *        and the map addres is [0x0 ~ 0x7fffffff](low 2GB), [0x80000000 ~ 0xffffffff](high 2GB).
 */
static inline mt_u32 virt_to_io_phys_addr(ulong virt_addr)
{
    phys_addr_t cpu_p_addr = virt_to_phys((void *)virt_addr);
    mt_u32 ip_p_addr = (mt_u32)cpu_p_addr;

    if ((cpu_p_addr >> 32) & 1UL)
       ip_p_addr = cpu_p_addr - 0x80000000;

    return ip_p_addr;
}

static inline mt_u32 core_phys_to_io_phys_addr(phys_addr_t core_phys_addr)
{
    mt_u32 ip_p_addr = (mt_u32)core_phys_addr;

    if ((core_phys_addr >> 32) & 1UL)
       ip_p_addr = core_phys_addr - 0x80000000;
...
static inline mt_u32 virt_to_io_phys_addr(ulong virt_addr)
{
    return (mt_u32)virt_addr;
}

static inline mt_u32 core_phys_to_io_phys_addr(phys_addr_t core_phys_addr)
{
    return (mt_u32)core_phys_addr;
}
#endif
