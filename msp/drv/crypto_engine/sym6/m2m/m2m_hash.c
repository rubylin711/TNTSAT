...
static mt_s32 m2m_cmd0_hash_operation(m2m_hash_ctx_t *ctx, const phys_addr_t data,
                                      mt_u32 len, bool terminate)
{
    ...
    if (((mt_u64)data >= AHB_BSRAM_ADDR)
        && ((mt_u64)data < AHB_BSRAM_ADDR + AHB_BSRAM_SIZE)) {
        cmd0.AT = 1;
    }
...
mt_s32 m2m_cmd0_hash_update(m2m_hash_ctx_t *ctx, const phys_addr_t data, mt_u32 len)
{
    mt_s32 res = HW_CE_ERROR_GENERIC;
    ...
    if ((ctx->block_size > 0) && (len > free_size)) {
        ...
        flush_dcache_range((ulong)ctx->block, max_block_size);

        res = m2m_cmd0_hash_operation(ctx, (phys_addr_t)HW_M2M_PHYS_ADDR(ctx->block), max_block_size, false);
        if (res != HW_CE_SUCCESS)
            return res;
    ...
    if (n_blocks > 0)
    {
        res = m2m_cmd0_hash_operation(ctx, data + free_size, n_blocks * max_block_size, false);
        if (res != HW_CE_SUCCESS)
            return res;
...
mt_s32 m2m_cmd0_hash_final(m2m_hash_ctx_t *ctx, mt_u8 *out)
{
    ...
    if (ctx->block_size)
        flush_dcache_range((ulong)ctx->block, ctx->block_size);

    res = m2m_cmd0_hash_operation(ctx, (phys_addr_t)HW_M2M_PHYS_ADDR(ctx->block), ctx->block_size, true);
#endif
    if (res != HW_CE_SUCCESS)
        return res;
