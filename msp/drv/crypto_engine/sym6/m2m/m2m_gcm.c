...
mt_s32 m2m_gcm_request_auth(m2m_gcm_req_t *req, const mt_u8 *aad, mt_u32 len)
{
    ...
    flush_dcache_range((mt_u64)aad, len);
#endif
    req->aad_len += len;
...
mt_s32 m2m_gcm_start_operation(m2m_gcm_req_t *req)
{
	...
	req->final_req.DesMagNum = M2M_DES_MAGIC_NUM;
    ...
    flush_dcache_range((ulong)&req->aad_len,
    ...
    flush_dcache_range((ulong)&req->final_req, sizeof(req->final_req));
