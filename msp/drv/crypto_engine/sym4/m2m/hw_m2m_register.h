...
/* 31:0  CMD_CNT   RO  cmd queue fifo data counter, max value is 48, before write, make sure fifo not overflow
 * Default: 0x00000000
 */
#define M2M_CHn_CMD_QUEUE_CNT_REG(n) (M2M_BASE+0x0004+n*0x1000)

#define M2M_CHn_CMD_QUEUE_CNT_MAX (48)
 ...
 * Default: 0x xxxxxxxx
 * hex address from 0x0130~0x0184(n=0~1)
 * for debug usage
 */
#define M2M_CHn_DESC_EXTR_REG(n) (M2M_BASE+0x0130+n*0x1000)

/* IV中间结果
 * 31:0  IV_CONT    RO IV content, if CMD done or Descriptor done, this register data is the result of AES/TDES IV
 * Default: 0x xxxxxxxx
 * hex address from 0x0188~0x0194(n=0~1)
 */
#define M2M_CHn_IV_CONT_REG(n) (M2M_BASE+0x0188+n*0x1000)

/* HASH中间结果
 * 31:0  GHASH_CONT   RO GHASH content, if CMD done or descriptor done, this register data is the result of HASH/GHASH/CMAC
 * Default: 0x xxxxxxxx
 * hex address from 0x0198~0x01d4(n=0~1)
 */
#define M2M_CHn_GHASH_CONT_REG(n) (M2M_BASE+0x0198+n*0x1000)

/* 最终IV结果
 * 31:0  TEMPOUT    RO IV CONT, if CMD done, this register data is the result of AES/TDES IV
 * Default: 0x xxxxxxxx
 * hex address from 0x0200~0x020c(n=0~1, RING_NUM=0~4)
 */
#define M2M_CHn_TEMPOUT0_REG(n, RING_NUM) (M2M_BASE+0x0200+n*0x1000+RING_NUM*0x50)

/* 最终HASH结果
 * 31:0  TEMPOUT    RO HASH/HMAC:output
 *         Others:GhashCount
 * Default: 0x xxxxxxxx
 * hex address from 0x0210~0x024c(n=0~1, RING_NUM=0~4)
 */
#define M2M_CHn_TEMPOUT1_REG(n, RING_NUM) (M2M_BASE+0x0210+n*0x1000+RING_NUM*0x50)
...
#define M2M_BGC_SETn_REG(n) (M2M_BGC_SET0_REG + n * 0x30)
