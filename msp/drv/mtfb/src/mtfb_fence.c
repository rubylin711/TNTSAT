...
int mtfb_create_fence(struct sw_sync_timeline *timeline, const char *fence_name, unsigned value)
{
	...
	if (timeline == NULL) 
	{
	MTFB_ERROR("\rmtfb_create_fence%d\n", __LINE__);
		return -EINVAL;
	}
	/**
	 **获取可用的文件节点
	...
	fd = get_unused_fd();
	if (fd < 0) {
		MTFB_ERROR("get_unused_fd failed!\n");
		return fd;
	}
	/**
	 **创建同步节点
...
int mtfb_fence_wait(int fence_fd, long timeout)
{
	...
	if (fence == NULL)
	{
		MTFB_ERROR("sync_fence_fdget failed!\n");
		return -EINVAL;
	}
	...
	if (err < 0)
	{
		MTFB_WARNING("error waiting on fence: 0x%x\n", err);
	}

	/**
	 **谁使用谁释放，mtfb使用acquire fence，则由mtfb来释放，
	 ** mtfb创建的release fence给GPU使用，则由GPU释放realease fence。
	 **/
