...
static S32 _mt_fe_tn_get_reg_rda5815s(MT_FE_CS8800_Device_Handle handle, U8 reg_addr, U8 *reg_data)
{
	return _mt_fe_tn_get_reg_ss2_cs8800(handle, reg_addr, reg_data);
}

static S32 _mt_fe_tn_set_reg_rda5815s(MT_FE_CS8800_Device_Handle handle, U8 reg_addr, U8 reg_data)
{
	return _mt_fe_tn_set_reg_ss2_cs8800(handle, reg_addr, reg_data);
}
...
#if 0
static MT_FE_RET RDA5815_tn_get_offset(MT_FE_CS8800_Device_Handle handle, S32 *p_offset)
{
	*p_offset = 0;
...
MT_FE_RET mt_fe_tn_init_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle)
{
	S32 ret = 0;

	//RDA5815S_Address = handle->m_device_ss2.tuner_cfg.tuner_dev_addr;

	_mt_sleep_cs8800(1);		//Wait 1ms.
...
MT_FE_RET mt_fe_tn_set_freq_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle, U32 freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)//unsigned long fPLL, unsigned long fSym )
{
	...
	_mt_sleep_cs8800(5);		//Wait 5ms;

	return 0;
}

MT_FE_RET mt_fe_tn_get_gain_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle, U32 *p_gain)
{
...
MT_FE_RET mt_fe_tn_get_strength_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle, S8 *p_strength)
{
//	U8  ii = 0;
	U32 gain_val, strength;
	MT_FE_LOCK_STATE lock_state;

	mt_fe_tn_get_gain_RDA5815S_cs8800(handle, &gain_val);
	...
	mt_fe_dmd_get_lock_state_cs8800_ss2(handle, &lock_state);
...
MT_FE_RET mt_fe_tn_sleep_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle)
{
#if 0
	SleepTuner(handle);
	mt_fe_WaitTime(50);
...
MT_FE_RET mt_fe_tn_wake_up_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle)
{
#if 0
	U8 val;
...
MT_FE_RET mt_fe_tn_get_tuner_freq_offset_RDA5815S_cs8800(MT_FE_CS8800_Device_Handle handle, S32 *p_offset)
{
	*p_offset = 0;
	return MtFeErr_Ok;
}
