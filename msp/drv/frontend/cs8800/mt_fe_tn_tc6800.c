...
/* Montage Technology (Shanghai) Co., Ltd                                    */
/* MONTAGE PROPRIETARY AND CONFIDENTIAL                                      */
/* Copyright (c) 2022 Montage Technology Group Limited. All Rights Reserved. */
/*****************************************************************************/
/*
...
static MT_FE_TN_DEVICE_SETTINGS_TC6800 TC6800_tuner_cfg_cs8800;
static U32 tc6800_cs8800_dbg_level = 0;


static S32 _mt_fe_tn_get_reg_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr, U8 *reg_data)
{
	mt_fe_tn_read_cs8800(handle->tuner_dev_addr, &reg_addr, 1, reg_data, 1);
...
static S32 _mt_fe_tn_set_reg_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr, U8 reg_data)
{
	...
	mt_fe_tn_write_cs8800(handle->tuner_dev_addr, p_buf, 2);
...
static S32 _mt_fe_tn_write_fw_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr, U8 *reg_data, U8 data_len)
{
	...
	mt_fe_tn_write_cs8800(handle->tuner_dev_addr, p_buf, (U8)(data_len + 1));
...
{
	_mt_sleep_cs8800(ticks_ms);
}


static void _mt_delay_tc6800(U32 ticks_ms)
{
	_mt_delay_cs8800(ticks_ms);
}
...
static S32 _mt_fe_tn_set_reg_bit_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr, U8 data, U8 high_bit, U8 low_bit)
{
	U8 tmp = 0, value = 0;
...
static S32 _mt_fe_tn_set_reg_bit_expand_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr_expand, U8 reg_addr, U8 reg_data, U8 high_bit, U8 low_bit)
{
	U8 tmp = 0, value = 0;
	U8 data = 0;
...
static S32 mt_fe_tn_download_fw_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	...
	U8 data0 = 0, data1 = 0;
	S32 ret = 0;
	const U8 *p_fw = m88tc6800_fm_cs8800;
	firm_size = sizeof(m88tc6800_fm_cs8800) / 64;
	...
	if ((handle->tuner_mcu_status != 1) || (data0 != 0))
	{
		...
		ret = _mt_fe_tn_write_fw_tc6800(handle, 0xc0, (U8 *)&(p_fw[64 * i]), (sizeof(m88tc6800_fm_cs8800) % 64));
...
int mt_fe_tn_global_reset_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 user_app)
{
	U8 buf = 0;
	U8 i = 0;
	U8 reset_flag = 0;
...
#if TC6800_ENABLE_PRINT
	tc6800_dbg_print(("mt_fe_tn_global_reset_tc6800 >>> user_app = %d\n", user_app));
#endif		
	for(i = 0; i < 24; i++)
	{
...
static void mt_fe_tn_set_freq_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 freq_KHz)
{
	...
	if ((handle->tuner_application == 3) || (handle->tuner_application == 4))
	{
		...
		//tc6800_dbg_print(("get 0x9a = 0x%02x  0x9d = 0x%02x 0xa7 = 0x%02x 0xb8 = 0x%02x 0x23 = 0x%02x 0xc9 = 0x%02x 0xab = 0x%02x\n", buf1, buf2, buf3, buf4, buf5, buf6, buf7)); //2022-6-13
		...
		if ((buf1 == 12) && (buf2 == 63))
		{
			if ((buf3 < 7) || (buf7 < 20) || (buf4 < 6))
			{
				...
				buf4 &= 0x1f;
				buf7 &= 0x3f;
				//tc6800_dbg_print(("get 0x9a = 0x%02x  0x9d = 0x%02x 0xa7 = 0x%02x 0xb8 = 0x%02x 0x23 = 0x%02x 0x26 = 0x%02x 0xab = 0x%02x\n", buf1, buf2, buf3, buf4, buf5, buf6, buf7));
			}
		}
...
__attribute__((unused))static void mt_fe_tn_internal_fef_enable_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
#if INTERNAL_FEF_ENABLE
...
__attribute__((unused))static void mt_fe_tn_internal_fef_disable_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
#if INTERNAL_FEF_ENABLE
...
static void mt_fe_tn_get_diagnose_info_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 info_mode)
{
...
{
	TC6800_tuner_cfg_cs8800.tuner_dev_addr = handle->m_device_c_b.tuner_cfg.tuner_dev_addr;

	if (handle->m_device_c_b.demod_type == MtFeType_DVBC)
	{
		TC6800_tuner_cfg_cs8800.tuner_init_OK = handle->m_device_c_b.tuner_cfg.tuner_init_ok;
		TC6800_tuner_cfg_cs8800.tuner_application = 0;
		if (handle->m_device_c_b.input_params.demod_bandwidth == MtFeBandwidth_8M)
			TC6800_tuner_cfg_cs8800.tuner_bandwidth = 8000;
		else if (handle->m_device_c_b.input_params.demod_bandwidth == MtFeBandwidth_7M)
			TC6800_tuner_cfg_cs8800.tuner_bandwidth = 7000;
		else if (handle->m_device_c_b.input_params.demod_bandwidth == MtFeBandwidth_6M)
			TC6800_tuner_cfg_cs8800.tuner_bandwidth = 6000;

		TC6800_tuner_cfg_cs8800.tuner_dac = 7200;

		// fix 105076 start
		if (handle->m_device_c_b.board_cfg.iVppSelC == 1)
			TC6800_tuner_cfg_cs8800.tuner_dac_gain = 2;
		else
			TC6800_tuner_cfg_cs8800.tuner_dac_gain = 4;
		// fix 105076 end
	}
	...
	mt_fe_tn_init_tc6800(&TC6800_tuner_cfg_cs8800);

	// custom_config_sel bit[7:4] for B/C/T/T2 module
	TC6800_tuner_cfg_cs8800.tuner_custom_cfg = (handle->custom_config_sel >> 4) & 0x0F;
	handle->m_device_c_b.tuner_cfg.tuner_init_ok = TC6800_tuner_cfg_cs8800.tuner_init_OK;
	handle->m_device_c_b.tuner_cfg.tuner_open = TC6800_tuner_cfg_cs8800.tuner_init_OK;
...
MT_FE_RET mt_fe_tn_set_freq_tc6800_c_b(MT_FE_CS8800_Device_Handle handle, U32 Freq_KHz, U32 sym_rate, S16 lpf_offset)
{
	...
	{
		mt_fe_tn_init_tc6800_c_b(handle);
	}
	...
	else
	{
		if (handle->m_device_c_b.input_params.demod_bandwidth == MtFeBandwidth_8M)
			...
			TC6800_tuner_cfg_cs8800.tuner_bandwidth = 6000;
		else if (handle->m_device_c_b.input_params.demod_bandwidth == MtFeBandwidth_10M)
			TC6800_tuner_cfg_cs8800.tuner_bandwidth = 5000;
		else
			TC6800_tuner_cfg_cs8800.tuner_bandwidth = 8000;
	}

	mt_fe_tn_set_freq_tc6800(&TC6800_tuner_cfg_cs8800, Freq_KHz);
...
#if 0
	{
		...
		for (reg_addr = 0x00; reg_addr <= 0xff; reg_addr ++)
		{
			_mt_fe_tn_get_reg_tc6800(&TC6800_tuner_cfg_cs8800, (unsigned char)reg_addr, &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x100; reg_addr <= 0x105; reg_addr ++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_cs8800, 0x1a, (unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x200; reg_addr <= 0x27c; reg_addr ++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_cs8800, 0x39,(unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x300; reg_addr <= 0x332; reg_addr ++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_cs8800, 0x4e, (unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x400; reg_addr <= 0x418; reg_addr ++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_cs8800, 0x53, (unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}
	}
#endif
...
MT_FE_RET mt_fe_tn_get_strength_tc6800_c_b(MT_FE_CS8800_Device_Handle handle, S8 *p_strength)
{
	...
	if (handle->m_device_c_b.tuner_cfg.tuner_lna_type == 1)
	{
		if (handle->m_device_c_b.input_params.input_freq_kHz < 474000)
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_cs8800)) - 9);
		else if (handle->m_device_c_b.input_params.input_freq_kHz <= 666000)
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_cs8800)) - 14);
		else if (handle->m_device_c_b.input_params.input_freq_kHz <= 794000)
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_cs8800)) - 12);
		else
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_cs8800)) - 9);
	}
	else
		strength = (S8)(mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_cs8800));
...
MT_FE_RET mt_fe_tn_get_gain_tc6800_c_b_cs8800(MT_FE_CS8800_Device_Handle handle, U32 *p_gain)
{
	...
	strength = mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_cs8800);

	//mt_fe_i2c_repeat_disable_cs8800(handle);

	//printk("%s[%d]strength = %d, strength = 0x%x\n", __FUNCTION__, __LINE__, strength, strength);

	*p_gain = strength * (-100); //the unit is -0.01dbm

	//printk("%s[%d]*p_gain = %d, *p_gain = 0x%x\n", __FUNCTION__, __LINE__, *p_gain, *p_gain);
...
{
	mt_fe_tn_sleep_tc6800(&TC6800_tuner_cfg_cs8800);
...
MT_FE_RET mt_fe_tn_wake_up_tc6800_c_b(MT_FE_CS8800_Device_Handle handle)
{
	...
	mt_fe_tn_set_freq_tc6800_c_b(handle, handle->m_device_c_b.input_params.input_freq_kHz, 0, 0);
...
{
	TC6800_tuner_cfg_cs8800.tuner_int_im = tc6800_im;
...
{
	TC6800_tuner_cfg_cs8800.tuner_clock_out = tc6800_clock;
...
MT_FE_RET mt_fe_tn_get_diagnose_info_tc6800_c_b_cs8800(MT_FE_CS8800_Device_Handle handle, U32* data1, U32* data2)
{
	...
	if(*data1 > 2)
	{
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_cs8800, 1);
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_cs8800, 2);
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_cs8800, 3);
	}
	else
	{
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_cs8800, 1);
...
void mt_fe_tn_set_tc6800_dbg_level_cs8800(U32 tc6800_dbg_level)
{
	tc6800_cs8800_dbg_level = tc6800_dbg_level;
}

void mt_fe_tn_get_tc6800_dbg_level_cs8800(U32 *tc6800_dbg_level)
{
	*tc6800_dbg_level = tc6800_cs8800_dbg_level;
}

void mt_fe_tn_reset_tc6800_cs8800(MT_FE_CS8800_Device_Handle handle)
{
	mt_fe_dmd_close_c_b_cs8800(handle);

	_mt_fe_tn_set_reg_tc6800(&TC6800_tuner_cfg_cs8800, 0x06, 0x01);
	_mt_delay_tc6800(1);
	_mt_fe_tn_set_reg_tc6800(&TC6800_tuner_cfg_cs8800, 0x06, 0x00);
...
void mt_fe_cs8800_tn_attach_tc6800(void)
{
	...
	handle->m_device_c_b.tuner_cfg.tuner_set_application = (MT_FE_RET(*)(void *, U8))mt_fe_tn_global_reset_tc6800;
