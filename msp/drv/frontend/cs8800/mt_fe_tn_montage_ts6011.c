...
S32 mt_fe_tn_ts6011_get_signal_strength(MT_FE_Tuner_Handle_TS6011 handle, S32 Vagc, MT_BOOL bLocked)
{
	...
	gain = mt_fe_tn_ts6011_get_gain(handle, Vagc);  //Get the total tuner gain in 0.01dB
	...
	//mt_fe_dmd_get_pure_lock_cs8800_ss2(handle, &lockstate);		// Get the lock status of the demodulator M88DS3002;
...
MT_FE_RET mt_fe_tn_init_ts6011_cs8800(MT_FE_CS8800_Device_Handle handle)
{
	...
	handle->m_device_ss2.tuner_cfg.tuner_handle = (mt_handle)ts6011_handle;

	ts6011_handle->tuner_dev_addr = handle->m_device_ss2.tuner_cfg.tuner_dev_addr;

	mt_fe_tn_ts6011_init(ts6011_handle);
...
/*************************************************************************
** Function: mt_fe_tn_get_tuner_freq_offset_ts6011
**
**
...
MT_FE_RET mt_fe_tn_get_tuner_freq_offset_ts6011_cs8800(MT_FE_CS8800_Device_Handle handle, S32 *p_offset)
{
	...
	*p_offset = mt_fe_tn_ts6011_get_freq_offset(ts6011_handle);
...
MT_FE_RET mt_fe_tn_set_freq_ts6011_cs8800(MT_FE_CS8800_Device_Handle handle, U32 freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)
{
	...
	//printk("%s[%d] ---- freq[%d KHz], sym[%d], offset[%d]\n", __FUNCTION__, __LINE__, freq_KHz, sym_rate_KSs, lpf_offset_KHz);

	handle->m_device_ss2.tuner_cfg.tuner_handle = (mt_handle)ts6011_handle;

	if (!handle->m_device_ss2.tuner_cfg.tuner_init_ok)
	{
		mt_fe_tn_init_ts6011_cs8800(handle);
	}

	mt_fe_tn_ts6011_set_freq(ts6011_handle, freq_KHz, sym_rate_KSs, lpf_offset_KHz);
...
MT_FE_RET mt_fe_tn_get_gain_ts6011_cs8800(MT_FE_CS8800_Device_Handle handle, U32 *p_gain)
{
	...
	_mt_fe_dmd_get_reg_ss2_cs8800(demod_handle, 0x3f, &AgcPWM);

	Vagc = AgcPWM * 33 - 1024;

	if (Vagc < 0)  Vagc = 0;

	*p_gain = mt_fe_tn_ts6011_get_gain(ts6011_handle, Vagc);
...
MT_FE_RET mt_fe_tn_get_strength_ts6011_cs8800(MT_FE_CS8800_Device_Handle handle, S8 *p_strength)
{
	...
	gain = mt_fe_tn_ts6011_get_gain(ts6011_handle, Vagc);

#if (MT_FE_CFG_CUSTOMER_SELECT == 1)
	if (gain > 9000)		level =  0;
	else if (gain > 8500)	level = (U8)( 0 + (9000 - gain) * 20 / 1000);
	else if (gain > 8000)	level = (U8)(10 + (8500 - gain) * 10 / 1000);
	else if (gain > 7000)	level = (U8)(15 + (8000 - gain) * 20 / 1000);
	else if (gain > 6000)	level = (U8)(35 + (7000 - gain) * 20 / 1000);
	...
	mt_fe_dmd_get_pure_lock_cs8800_ss2(demod_handle, &lockstate);
...
void mt_fe_cs8800_tn_attach_ts6011(void)
{
