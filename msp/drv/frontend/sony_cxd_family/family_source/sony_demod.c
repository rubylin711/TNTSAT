...
#include "dvb_cable/sony_demod_dvbc.h"
#endif
...
#include "dvb_cable/sony_demod_dvbc2.h"
#endif

#ifdef SONY_DEMOD_SUPPORT_DVBS_S2
#include "dvb_sat/sony_demod_dvbs_s2.h"
#endif
...
#include "isdb_cable/sony_demod_isdbc.h"
#endif

#ifdef SONY_DEMOD_SUPPORT_ISDBS
#include "isdb_sat/sony_demod_isdbs.h"
#endif

#ifdef SONY_DEMOD_SUPPORT_ISDBS3
#include "isdb_sat/sony_demod_isdbs3.h"
#endif

#ifdef SONY_DEMOD_SUPPORT_J83B
#include "j83b/sony_demod_j83b.h"
#endif
...
sony_result_t sony_demod_Initialize (sony_demod_t * pDemod)
{
	...
	pr_err("[%s %d] ChipId:0x%x \n",__FUNCTION__,__LINE__,pDemod->chipId);
    ...
    case SONY_DEMOD_CHIP_ID_CXD6802:
                                    ...
                                    int32_t value)
{
    ...
    case SONY_DEMOD_CONFIG_TADC_DC_BIAS:
        /* slave    Bank    Addr    Bit    default    Name
         * --------------------------------------------------------
         * <SLV-X>  00h     41h     [0]    1'b00      OREG_ADC_TC_PD_DCGEN
         */
        if (sony_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVX, 0x00, 0x41, (uint8_t) (value ? 0x00 : 0x01), 0x01) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
        break;
    ...
    case SONY_DEMOD_CONFIG_ATSC3_KOREAN_MODE:
        /* CXD2878A family only */
        if (!SONY_DEMOD_CHIP_ID_2878A_FAMILY (pDemod->chipId)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_NOSUPPORT);
        }

        /* slave    Bank    Addr    Bit    default    Name
         * ---------------------------------------------------
         * <SLV-T>  9Eh     E9h     [0]    1'b0       OREG_ALP_KMODE_ON
         */
        result = sony_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT,
    ...
    case SONY_DEMOD_CONFIG_ATSC3_PLPID_PACKET_SPLP:
        /* CXD2878 family only */
        if (!SONY_DEMOD_CHIP_ID_2878_FAMILY (pDemod->chipId)) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_NOSUPPORT);
        }
        ...
        /* slave    Bank    Addr    Bit    default    Name
         * ---------------------------------------------------
         * <SLV-T>  95h     23h     [1:0]    2'd3     OREG_OIF_PLPID_CONVMODE_S
         */
        result = sony_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT,
    ...
    case SONY_DEMOD_CONFIG_ATSC3_AUTO_PLP_ID_SPLP:
    ...
    case SONY_DEMOD_CONFIG_ISDBS3_LOWCN_HOLD:
        ...
        /*  slave     Bank    Addr    Bit     Default    Setting     Signal name
         * -----------------------------------------------------------------------------
         *  <SLV-T>   A1h     5Bh    [4:0]    8'h00      8'h11       OCPM_FINE_CNTH_HOLD_H,3'b000,OCPM_FINE_CNTH_HOLD_L
         */
    ...
    case SONY_DEMOD_CONFIG_ALP_SYNC_LENGTH:
        /* This register can change only in Sleep state */
        if (pDemod->state != SONY_DEMOD_STATE_SLEEP) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_SW_STATE);
        }
        ...
        /* slave    Bank    Addr    Bit    default    Name
         * ---------------------------------------------------
         * <SLV-T>  02h     C1h     [6]    1'b0       OALP_WFMT_SERSTDUR
         */
        result = sony_demod_SetAndSaveRegisterBits (pDemod, pDemod->i2cAddressSLVT, 0x02, 0xC1, (uint8_t) (value ? 0x40 : 0x00), 0x40);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
        break;
...
static sony_result_t XtoSL(sony_demod_t * pDemod)
{
    ...
    if (SONY_DEMOD_CHIP_ID_2878_FAMILY (pDemod->chipId)) {
        ...
        if (pDemod->chipId == SONY_DEMOD_CHIP_ID_CXD6802) {
            ...
            if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x10, data, sizeof (data)) != SONY_RESULT_OK) {
                SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
            }
        } else {
            /* Set SLV-T Bank : 0x95 */
