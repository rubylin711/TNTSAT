                                    ...
                                    sony_j83b_tune_param_t * pTuneParam)
{
    ...
    if ((pInteg->pTuner) && (pInteg->pTuner->TerrCableTune)) {
        ...
        /* Tune the RF part */
        result = pInteg->pTuner->TerrCableTune (pInteg->pTuner, pTuneParam->centerFreqKHz, SONY_DTV_SYSTEM_J83B, pTuneParam->bandwidth);
        if (result != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (result);
        }
                                    ...
                                    sony_integ_j83b_scan_callback_t callBack)
{
    ...
    while (scanResult.centerFreqKHz <= pScanParam->endFrequencyKHz) {
        ...
        scanResult.tuneResult = sony_integ_j83b_Tune (pInteg, &scanResult.tuneParam);
