...
/*------------------------------------------------------------------------------
  Copyright 2016-2020 Sony Semiconductor Solutions Corporation

  Last Updated    : 2020/10/23
  Modification ID : b18046a6528522fe8de419252a316663e87b216c
------------------------------------------------------------------------------*/
...
/*
 @brief Confirms VQ lock state for demodulator.
*/
static sony_result_t IsVqLocked (sony_demod_t * pDemod);
                                                ...
                                                uint8_t * pUnlockDetected)
{
    ...
    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_SyncStat");
    ...
    if (pDemod->system != SONY_DTV_SYSTEM_J83B) {
        /* Not J.83B */
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_SW_STATE);
    }
     ...
     * <SLV-T>   40h     88h     [1]          IREG_AR_TIMEOUT     0:Checking, 1:J.83B signal does not exist
...
sony_result_t sony_demod_j83b_monitor_IFAGCOut (sony_demod_t * pDemod, uint32_t * pIFAGCOut)
{
    uint8_t rdata[2];

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_IFAGCOut");
...
sony_result_t sony_demod_j83b_monitor_QAM (sony_demod_t * pDemod, sony_j83b_constellation_t * pQAM)
{
    uint8_t rdata = 0x00;

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_QAM");
    ...
    *pQAM = (sony_j83b_constellation_t) (rdata & 0x07);

    SONY_TRACE_RETURN (SONY_RESULT_OK);
}

sony_result_t sony_demod_j83b_monitor_SymbolRate (sony_demod_t * pDemod, uint32_t * pSymRate)
{
    uint8_t rdata1[2];
    uint8_t rdata2[5];
    uint32_t trialsrate;
    ...
    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_SymbolRate");
...
sony_result_t sony_demod_j83b_monitor_SamplingOffset (sony_demod_t * pDemod,
                                                      int32_t * pPPM)
{
    sony_j83b_constellation_t qam;
    uint32_t symRate = 0;
    uint32_t standardSymRate = 0;
    sony_result_t result = SONY_RESULT_OK;

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_SamplingOffset");
    ...
    result = sony_demod_j83b_monitor_QAM (pDemod, &qam);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }

    result = sony_demod_j83b_monitor_SymbolRate (pDemod, &symRate);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }

    if (pDemod->bandwidth == SONY_DTV_BW_J83B_5_06_5_36_MSPS && qam == SONY_J83B_CONSTELLATION_64QAM) {
        standardSymRate = 5056941;
    } else if (pDemod->bandwidth == SONY_DTV_BW_J83B_5_06_5_36_MSPS && qam == SONY_J83B_CONSTELLATION_256QAM) {
        standardSymRate = 5360537;
    } else if (pDemod->bandwidth == SONY_DTV_BW_J83B_5_60_MSPS && qam == SONY_J83B_CONSTELLATION_256QAM) {
    ...
    /*
     * Clock Frequency Offset[ppm] = {(Standard Sr - Sr) / Standard Sr} * 10^8
     *                             = {(1 - (Sr / Standard Sr)} * 10^8
     *                             = 10^8 - (Sr /Standard  Sr) * 10^8
     *                             = 10^8 - {(Sr * (2^8 * 5^8) / Standard Sr)}
     * Sr * (2^8 * 5^8) / Standard Sr = Sr * 2^8 * 5^4 * 5^4 / Standard Sr
     */
...
sony_result_t sony_demod_j83b_monitor_CarrierOffset (sony_demod_t * pDemod, int32_t * pOffset)
{
    uint8_t rdata[2];

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_CarrierOffset");
...
sony_result_t sony_demod_j83b_monitor_SpectrumSense (sony_demod_t * pDemod, sony_demod_terr_cable_spectrum_sense_t * pSense)
{
    uint8_t rdata = 0x00;

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_SpectrumSense");
    ...
    /* VQ lock check */
    {
        sony_result_t result = IsVqLocked (pDemod);
        if (result != SONY_RESULT_OK) {
            SLVT_UnFreezeReg (pDemod);
            SONY_TRACE_RETURN (result);
        }
    ...
    /* Set SLV-T Bank : 0x80 */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x00, 0x80) != SONY_RESULT_OK) {
        SLVT_UnFreezeReg (pDemod);
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
     ...
     * ---------------------------------------------------------------------------------
     * <SLV-T>   80h     55h     [0]            DFRMJ_IQREV     0:Normal,   1:inverted
     */
    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x55, &rdata, 1) != SONY_RESULT_OK) {
        SLVT_UnFreezeReg (pDemod);
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    ...
    if (pDemod->confSense == SONY_DEMOD_TERR_CABLE_SPECTRUM_INV) {
        *pSense = (rdata & 0x01) ? SONY_DEMOD_TERR_CABLE_SPECTRUM_NORMAL : SONY_DEMOD_TERR_CABLE_SPECTRUM_INV;
    } else {
        *pSense = (rdata & 0x01) ? SONY_DEMOD_TERR_CABLE_SPECTRUM_INV : SONY_DEMOD_TERR_CABLE_SPECTRUM_NORMAL;
    }

    SONY_TRACE_RETURN (SONY_RESULT_OK);
}

sony_result_t sony_demod_j83b_monitor_SNR (sony_demod_t * pDemod, int32_t * pSNR)
{
    sony_j83b_constellation_t qam = SONY_J83B_CONSTELLATION_64QAM;
    uint16_t reg = 0x00;
    uint8_t rdata[2];
    sony_result_t result = SONY_RESULT_OK;

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_SNR");
    ...
    qam = (sony_j83b_constellation_t) (rdata[0] & 0x07);
    ...
    switch (qam) {
    case SONY_J83B_CONSTELLATION_64QAM:
    case SONY_J83B_CONSTELLATION_256QAM:
...
sony_result_t sony_demod_j83b_monitor_PreRSBER (sony_demod_t * pDemod, uint32_t * pBER)
{
    uint8_t rdata[3];
    uint32_t bitError = 0;
    uint32_t periodExp = 0;

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_PreRSBER");
    ...
    if ((periodExp <= 11) && (bitError > (1U << periodExp) * 128 * 7)) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_HW_STATE);
    }

    /*
      BER = bitError * 10000000 / (2^N * 128 * 7)
          = bitError * 78125 / (2^N * 7)
          = bitError * 125 * 625 / (2^N * 7)
          (Divide in 2 steps to prevent overflow.)
    */
    {
        uint32_t div = 0;
...
sony_result_t sony_demod_j83b_monitor_PacketErrorNumber (sony_demod_t * pDemod, uint32_t * pPEN)
{
    uint8_t rdata[3];

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_PacketErrorNumber");
...
sony_result_t sony_demod_j83b_monitor_PER (sony_demod_t * pDemod, uint32_t * pPER)
{
    sony_result_t result = SONY_RESULT_OK;
    uint32_t packetError = 0;
    uint32_t periodExp = 0;

    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_PER");
                                                  ...
                                                  sony_j83b_interleave_t * pInterleave)
{
    ...
    SONY_TRACE_ENTER ("sony_demod_j83b_monitor_Interleave");
    ...
    /* slave    Bank    Addr    Bit              Name                meaning
     * ---------------------------------------------------------------------------------
     * <SLV-T>   80h     53h     [0]          DFRMJ_ICWLOCK      0:UNLOCK, 1:LOCK
     * <SLV-T>   80h     54h    [3:0]       IREG_DFRMJ_ICW[3:0]
     */

    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x53, rdata, 2) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
    ...
    *pInterleave = (sony_j83b_interleave_t) (rdata[1] & 0x0F);
...
static sony_result_t IsARLocked (sony_demod_t * pDemod)
{
    ...
    result = sony_demod_j83b_monitor_SyncStat (pDemod, &ar, &tslock,&earlyUnlock);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
...
static sony_result_t IsVqLocked (sony_demod_t * pDemod)
{
    ...
    /* slave    Bank    Addr    Bit              Name
     * ------------------------------------------------------------
     * <SLV-T>   80h     50h    [0]             VQLOCL
     */
    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x50, &rdata, 1) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
