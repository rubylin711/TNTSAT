...
sony_result_t sony_demod_dvbt_monitor_TSRate (sony_demod_t * pDemod,
                                              uint32_t * pTSRateKbps)
{
    ...
    /*
     * TS Rate (packet/s) = (1000 * 63000 * BW * MOD * CR) / (121856 * (1 + GI))
     * TS Rate (kbps)     = (188 * 8 * 63000 * BW * MOD * CR) / (121856 * (1 + GI))
     *                    = (123375 * BW * MOD * (24 * CR)) / (119 * (32 + (32 * GI)))
     *
     * BW = 5 or 6 or 7 or 8
     * CR = 1/2 or 2/3 or 3/4 or 5/6 or 7/8
     * GI = 1/32 or 1/16 or 1/8 or 1/4
     *
     * MOD (non-hierarchy)
     *    MOD      Modulation
     *  ------------------------
    ...
    const uint8_t CRx24[5] = {12, 16, 18, 20, 21}; /* CR x 24 (1/2, 2/3, 3/4, 5/6, 7/8) */
    const uint8_t GIx32[4] = {1, 2, 4, 8};         /* GI x 32 (1/32, 1/16, 1/8, 1/4) */
    const uint8_t MOD_NonH[3] = {2, 4, 6};         /* MOD for non-hierarchy (QPSK, 16QAM, 64QAM) */
    const uint8_t MOD_HierHp[3] = {2, 2, 2};       /* MOD for hierarchy (HP) (QPSK, 16QAM, 64QAM) */
    const uint8_t MOD_HierLp[3] = {0, 2, 4};       /* MOD for hierarchy (LP) (QPSK, 16QAM, 64QAM) */

    uint8_t MOD_val = 0;
    uint8_t CRx24_val = 0;
    uint32_t div = 0;

    SONY_TRACE_ENTER ("sony_demod_dvbt_monitor_TSRate");
    ...
    if (tps.hierarchy == SONY_DVBT_HIERARCHY_NON) {
        ...
        /* Profile for hierarchical signal */
        uint8_t data = 0;
    ...
    *pTSRateKbps = ((123375 * (uint32_t)pDemod->bandwidth * MOD_val * CRx24_val) + div / 2) / div; /* round */
                                                   ...
                                                   int32_t * pSNR)
{
    ...
    if (SONY_DEMOD_CHIP_ID_2856_FAMILY (pDemod->chipId)) {
        /* SNR history register for CXD2856 family
         *
         *  slave    Bank    Addr    Bit     Name
         * ---------------------------------------------------------------------------------
        ...
        /* SNR history register for CXD2878 family
         *
         *  slave    Bank    Addr    Bit     Name
         * ---------------------------------------------------------------------------------
         * <SLV-T>   02h     30h    [0]      IREG_SNR_VALID0
