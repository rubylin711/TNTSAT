/*------------------------------------------------------------------------------
  ...
  Modification ID : f08108eb7355363ac9e3ef592cba6f75731a51b6
------------------------------------------------------------------------------*/
                                                         ...
                                                         sony_atsc3_l1detail_plp_t * pL1DetailPlp)
{
    ...
    sony_memset (pL1DetailSubframe, 0, sizeof (sony_atsc3_l1detail_subframe_t));
    sony_memset (pL1DetailPlp, 0, sizeof (sony_atsc3_l1detail_plp_t));
...
sony_result_t sony_demod_atsc3_monitor_ALPHeaderLengthError (sony_demod_t * pDemod,
                                                             ...
                                                             uint8_t errorKorean[4])
{
    ...
    SONY_TRACE_ENTER ("sony_demod_atsc3_monitor_ALPHeaderLengthError");
    ...
    result = sony_demod_atsc3_monitor_PostBCHFER (pDemod, fer);
    if (result != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (result);
    }
    ...
    for (i = 0; i < 4; i++) {
        if (fer[i] > 0) {
            errorStandard[i] = SONY_DEMOD_ATSC3_MONITOR_ALPHEADERLENGTHERROR_INVALID;
            errorKorean[i] = SONY_DEMOD_ATSC3_MONITOR_ALPHEADERLENGTHERROR_INVALID;
        } else {
            errorStandard[i] = 0;
            errorKorean[i] = 0;
        }
    ...
    /* SLV-T bank 0x9E */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x00, 0x9E) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
     ...
     *  slave     Bank      Addr     Bit      Signal name
     * --------------------------------------------------------------
     *  <SLV-T>   9Eh       E0h      [7:0]    IREG_B2A0_ALPMON_HLD[7:0]
     *  <SLV-T>   9Eh       E1h      [7:0]    IREG_B2A0_ALPMON_K_HLD[7:0]
     *  <SLV-T>   9Eh       E2h      [7:0]    IREG_B2A1_ALPMON_HLD[7:0]
     *  <SLV-T>   9Eh       E3h      [7:0]    IREG_B2A1_ALPMON_K_HLD[7:0]
     *  <SLV-T>   9Eh       E4h      [7:0]    IREG_B2A2_ALPMON_HLD[7:0]
     *  <SLV-T>   9Eh       E5h      [7:0]    IREG_B2A2_ALPMON_K_HLD[7:0]
     *  <SLV-T>   9Eh       E6h      [7:0]    IREG_B2A3_ALPMON_HLD[7:0]
     *  <SLV-T>   9Eh       E7h      [7:0]    IREG_B2A3_ALPMON_K_HLD[7:0]
     */

    if (pDemod->pI2c->ReadRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE0, data, 8) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
     ...
     *  slave     Bank      Addr     Bit      Signal name
     * --------------------------------------------------------------
     *  <SLV-T>   9Eh       E8h      [0]      OREGD_B2A_ALPMON_HLD_CLR
     */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE8, 0x01) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
