                                             ...
                                             uint8_t plpID[4])
{
     ...
     *  <SLV-T>   93h     85h    [0]    8'h01     8'h0X          OREG_PLP_ID_AUTO
     */
    for (i = 0; i < 4; i++) {
        if (i < plpIDNum) {
            data[i] = plpID[i] | 0x80;
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x85,
        pDemod->atsc3AutoPLPID_SPLP ? 0x01 : 0x00) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
...
sony_result_t sony_demod_atsc3_AutoDetectSeq_SetCWTracking (sony_demod_t * pDemod, uint8_t * pCompleted)
{
    ...
    */
    ...
    switch (rdata[0] >> 5) {
    case 1:/* 8K */
        carrierDuration = 192; /* 843.75 * 2^8 / 1125 */
        break;
    case 4:/* 16K */
        carrierDuration = 96; /* 421.875 * 2^8 / 1125 */
        break;
    case 5:/* 32K */
        carrierDuration = 48; /* 210.9375 * 2^8 / 1125 */
        break;
    default:
    ...
    case SONY_DTV_BW_8_MHZ:
        samplingFreq = 8; /* 2^10 * 1125 / 9216000 = 1/8 */
        break;
    case SONY_DTV_BW_7_MHZ:
        samplingFreq = 7; /* 2^10 * 1125 / 8064000 = 1/7 */
        break;
    case SONY_DTV_BW_6_MHZ:
        samplingFreq = 6; /* 2^10 * 1125 / 6912000 = 1/6 */
        break;
    default:
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_HW_STATE);
    }
    ...
    /*  slave     Bank    Addr   Bit      Default     Value      Name
     * ----------------------------------------------------------------------------
     *  <SLV-T>   9Ah     38h    [3:0]    8'h04       8'h0C      OREG_CAS_CCIFLT_EN_CW
     */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x38, 0x0C) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
...
static sony_result_t SLtoAA3(sony_demod_t * pDemod)
{
     ...
     *
     * Note: This setting is moved to XtoSL and SDtoSL.
     */

    /* ATSC 3.0 initial setting
     *  slave     Bank    Addr    Bit      Default    Value      Name
     * -----------------------------------------------------------------------------------------
     *  <SLV-T>   95h     79h     [7:0]    8'h20      8'h10      OREG_GOODIDX_Q[7:0]
     *  <SLV-T>   95h     7Bh     [7:0]    8'h40      8'h10      OREG_GOODIDX_16[7:0]
     */
    /* Set SLV-T Bank : 0x95 */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x00, 0x95) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    ...
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x79, 0x10) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x7B, 0x10) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    /* Set SLV-T Bank : 0x9C */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x00, 0x9C) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }

    if (pDemod->atsc3EasState == SONY_DEMOD_ATSC3_EAS_STATE_EAS) {
        /* ATSC3.0 initial setting (for EAS mode)
         *  slave     Bank    Addr    Bit      Default    Value      Name
         * -----------------------------------------------------------------------------------------
         *  <SLV-T>   9Ch     50h     [7:0]    8'h93      8'h93      OREG_L1SEQ_TR_12[39:32]
         *  <SLV-T>   9Ch     51h     [7:0]    8'h40      8'h40      OREG_L1SEQ_TR_12[31:24]
         *  <SLV-T>   9Ch     52h     [7:0]    8'h09      8'h09      OREG_L1SEQ_TR_12[23:16]
         *  <SLV-T>   9Ch     53h     [7:0]    8'hB4      8'hB4      OREG_L1SEQ_TR_12[15:8]
         *  <SLV-T>   9Ch     54h     [7:0]    8'h01      8'h01      OREG_L1SEQ_TR_12[7:0]
         *  <SLV-T>   9Ch     65h     [7:0]    8'hFC      8'hFC      OREG_L1SEQ_TR_16[39:32]
         *  <SLV-T>   9Ch     66h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[31:24]
         *  <SLV-T>   9Ch     67h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[23:16]
         *  <SLV-T>   9Ch     68h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[15:8]
         *  <SLV-T>   9Ch     69h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[7:0]
         *  <SLV-T>   9Ch     D4h     [7:0]    8'h01      8'h01      OREG_L1SEQ_SOUT_12[19:16]
         *  <SLV-T>   9Ch     D5h     [7:0]    8'hD8      8'hD8      OREG_L1SEQ_SOUT_12[15:8]
         *  <SLV-T>   9Ch     D6h     [7:0]    8'h1E      8'h1E      OREG_L1SEQ_SOUT_12[7:0]
         *  <SLV-T>   9Ch     E0h     [7:0]    8'h01      8'h01      OREG_L1SEQ_SOUT_16[19:16]
         *  <SLV-T>   9Ch     E1h     [7:0]    8'h00      8'h00      OREG_L1SEQ_SOUT_16[15:8]
         *  <SLV-T>   9Ch     E2h     [7:0]    8'h04      8'h04      OREG_L1SEQ_SOUT_16[7:0]
         *  <SLV-T>   9Ch     FCh     [7:0]    8'h12      8'h12      OREG_L1SEQ_PARAM_16[4:0]
         */
        ...
        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x50, data50, 5) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x65, data65, 5) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xD4, dataD4, 3) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        if (pDemod->pI2c->WriteRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xE0, dataE0, 3) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }

        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xFC, 0x12) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    } else {
        /* ATSC3.0 initial setting (for Active-ATSC3)
         *  slave     Bank    Addr    Bit      Default    Value      Name
         * -----------------------------------------------------------------------------------------
         *  <SLV-T>   9Ch     50h     [7:0]    8'h93      8'h93      OREG_L1SEQ_TR_12[39:32]
         *  <SLV-T>   9Ch     51h     [7:0]    8'h40      8'h40      OREG_L1SEQ_TR_12[31:24]
         *  <SLV-T>   9Ch     52h     [7:0]    8'h09      8'h09      OREG_L1SEQ_TR_12[23:16]
         *  <SLV-T>   9Ch     53h     [7:0]    8'hB4      8'hC0      OREG_L1SEQ_TR_12[15:8]
         *  <SLV-T>   9Ch     54h     [7:0]    8'h01      8'h01      OREG_L1SEQ_TR_12[7:0]
         *  <SLV-T>   9Ch     65h     [7:0]    8'hFC      8'hD3      OREG_L1SEQ_TR_16[39:32]
         *  <SLV-T>   9Ch     66h     [7:0]    8'h00      8'h40      OREG_L1SEQ_TR_16[31:24]
         *  <SLV-T>   9Ch     67h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[23:16]
         *  <SLV-T>   9Ch     68h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[15:8]
         *  <SLV-T>   9Ch     69h     [7:0]    8'h00      8'h00      OREG_L1SEQ_TR_16[7:0]
         *  <SLV-T>   9Ch     D4h     [7:0]    8'h01      8'h01      OREG_L1SEQ_SOUT_12[19:16]
         *  <SLV-T>   9Ch     D5h     [7:0]    8'hD8      8'hD8      OREG_L1SEQ_SOUT_12[15:8]
         *  <SLV-T>   9Ch     D6h     [7:0]    8'h1E      8'h1C      OREG_L1SEQ_SOUT_12[7:0]
         *  <SLV-T>   9Ch     E0h     [7:0]    8'h01      8'h01      OREG_L1SEQ_SOUT_16[19:16]
         *  <SLV-T>   9Ch     E1h     [7:0]    8'h00      8'hD8      OREG_L1SEQ_SOUT_16[15:8]
         *  <SLV-T>   9Ch     E2h     [7:0]    8'h04      8'h1D      OREG_L1SEQ_SOUT_16[7:0]
         *  <SLV-T>   9Ch     FCh     [7:0]    8'h12      8'h14      OREG_L1SEQ_PARAM_16[4:0]
         */
        ...
        if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0xFC, 0x14) != SONY_RESULT_OK) {
            SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
        }
    }
...
static sony_result_t detectCW (sony_demod_t * pDemod)
{
     ...
     *  slave     Bank    Addr   Bit       Default     Value      Name
     * ----------------------------------------------------------------------------------------
     *  <SLV-T>   9Ah     38h    [3:0]     8'h04       8'h05      OREG_CAS_CCIFLT_EN_CW[3:0]
     */
    if (pDemod->pI2c->WriteOneRegister (pDemod->pI2c, pDemod->i2cAddressSLVT, 0x38, 0x05) != SONY_RESULT_OK) {
        SONY_TRACE_RETURN (SONY_RESULT_ERROR_I2C);
    }
...
static sony_result_t initCWDetection (sony_demod_t * pDemod)
{
    ...
    /*  slave     Bank    Addr    Bit      Value    Name
     * ----------------------------------------------------------------------------
     *  <SLV-T>   9Ah     38h     [3:0]    8'h04    OREG_CAS_CCIFLT_EN_CW
     */
