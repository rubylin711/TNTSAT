...
#define SONY_DEMOD_ATSC3_MONITOR_ALPHEADERLENGTHERROR_INVALID 0xFF  /**< ALP header length error value is invalid. */
...
/**
 @brief Monitor the ALP header length format error for each PLPs.

        This provides the ALP header length error status for each (max 4) PLPs.
        If plpID[N] is not specified in tune API,
        errorStandard[N] and errorKorean[N] will become
        SONY_DEMOD_ATSC3_MONITOR_ALPHEADERLENGTHERROR_INVALID (0xFF).

        If FER > 0 for the PLP, ALP header length error monitor cannot return correct value.
        Then errorStadard[N] and errorKorean[N] become
        SONY_DEMOD_ATSC3_MONITOR_ALPHEADERLENGTHERROR_INVALID (0xFF).

 @param pDemod The demodulator instance.
 @param errorStandard Pointer of 4 length array to store each PLP ALP header length error status in standard mode.
 @param errorKorean Pointer of 4 length array to store each PLP ALP header length error status in Korean mode.

 @return SONY_RESULT_OK if errorStadard and  errorKorean are valid.
*/
