 ...
 *  0.01.11     2020.03.05  Daniel          Modify
 *****************************************************************************/
...
mt_fe_hd2502_priv_handle hd2502_handle;
...
static S32 _mt_fe_tn_set_reg_ts6011(MT_FE_Tuner_Handle_TS6011 handle, UINT8 reg_addr, UINT8 reg_data)
{
	UINT8 buf[2];
	...
	return Write_I2C(TS6011_I2C_ADDRESS, 2, buf);
}

static S32 _mt_fe_tn_get_reg_ts6011(MT_FE_Tuner_Handle_TS6011 handle,  UINT8 reg_addr, UINT8 *buffer)
{
	UINT8 ret = 0;
	UINT8 tuner_addr = TS6011_I2C_ADDRESS;
		
	ret = Write_I2C(tuner_addr, 1, &reg_addr);

	ret |= Read_I2C(tuner_addr, 1, buffer);
	//printk("_mt_fe_tn_get_reg_ts6011 >>>> reg_addr=0x%02x, buf[0] = 0x%02x\n",reg_addr, buffer[0]);
	return ret;
}
...
S32 mt_fe_tn_ts6011_wakeup_hd2502(MT_FE_Tuner_Handle_TS6011 handle)
{
...
S32 mt_fe_tn_ts6011_sleep_hd2502(MT_FE_Tuner_Handle_TS6011 handle)
{
...
/************************************************************************/
static S32 mt_fe_tn_ts6011_get_gain_hd2502(MT_FE_Tuner_Handle_TS6011 handle, U32 Vagc)
{
...
void mt_fe_tn_ts6011_init_hd2502(MT_FE_Tuner_Handle_TS6011 handle)
{
	...
	if(handle->tuner_init_OK == 0)
	{
		...
		handle->tuner_clock_out			 = 1;     // "0" CLK-output disable, "1" CLK-output enable.
     	...
     	//printk("[ %s : %d ]======  \n",__FUNCTION__,__LINE__);
	mt_fe_tn_ts6011_wakeup_hd2502(handle);
...
void mt_fe_tn_set_tuner_ts6011_hd2502(MT_FE_Tuner_Handle_TS6011 handle, U32 freq_MHz, U32 symbol_rate_KSs)
{
#if MT_FE_TS6011_DEBUG_PRNT
...
S32 mt_fe_tn_ts6011_set_freq_hd2502(MT_FE_Tuner_Handle_TS6011 handle, U32 Freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)
{
	...
	//printk("[ %s : %d ]======freq_MHz=%d lpf_offset_KHz=%d,handle->tuner_lpf_offset_KHz =%d\n",__FUNCTION__,__LINE__,freq_MHz,lpf_offset_KHz,ts6011_handle->tuner_lpf_offset_KHz);
	
	ts6011_handle->tuner_lpf_offset_KHz = (U32)lpf_offset_KHz;
	//printk("[ %s : %d ]\n",__FUNCTION__,__LINE__);
	mt_fe_tn_set_tuner_ts6011_hd2502(ts6011_handle, freq_MHz, sym_rate_KSs);
	//printk("[ %s : %d ]\n",__FUNCTION__,__LINE__);
#ifdef DUMP_REG
	if(1)
	{
		...
		for(i=0; i<0xff; i++)
		{
			_mt_fe_tn_get_reg_ts6011(handle, i, &ReadData1);
			//printk("%02x,",ReadData1);
			//if(i%16 == 0)  printk("\n");
		}
		printk("\n==========demod reg : ============\n");
	}
...
{
	HDIC2501_OpenTunerI2C(TS6011_I2C_ADDRESS);
	int i=0;
	u8 ReadData1=0;
	printk("\n==========tuner reg : 0x00 -0xff ============\n");
	for(i=0; i<0xff; i++)
	{
		_mt_fe_tn_get_reg_ts6011(ts6011_handle, i, &ReadData1);
...
S32 mt_fe_tn_ts6011_get_signal_strength_hd2502(MT_FE_Tuner_Handle_TS6011 handle, S32 Vagc, MT_BOOL bLocked)
{
	...
	gain = mt_fe_tn_ts6011_get_gain_hd2502(handle, Vagc);  //Get the total tuner gain in 0.01dB

	// Step 4: Calculate the signal strength based on the total gain of the tuner

#if (MT_FE_CFG_CUSTOMER_SELECT == 1)
	if (gain > 9000)		strength = 0;
	else if (gain > 8500)	strength = (U8)( 0 + (9000 - gain) * 20 / 1000);
	else if (gain > 8000)	strength = (U8)(10 + (8500 - gain) * 10 / 1000);
	else if (gain > 7000)	strength = (U8)(15 + (8000 - gain) * 20 / 1000);
	else if (gain > 6000)	strength = (U8)(35 + (7000 - gain) * 20 / 1000);
	...
	//mt_fe_dmd_hd2502_get_pure_lock(handle, &lockstate);		// Get the lock status of the demodulator;
...
int mt_fe_tn_init_ts6011_hd2502(void *handle)
{
	hd2502_handle = handle;
	hd2502_handle->tuner_handle = (U32)ts6011_handle;

	mt_fe_tn_ts6011_init_hd2502(ts6011_handle);
	return 0;
}

MT_FE_RET mt_fe_tn_sleep_ts6011_hd2502(mt_fe_hd2502_priv_handle handle)
{
	hd2502_handle = handle;
	hd2502_handle->tuner_handle = (U32)ts6011_handle;

	mt_fe_tn_ts6011_sleep_hd2502(ts6011_handle);

	return MtFeErr_Ok;
}

MT_FE_RET mt_fe_tn_wake_up_ts6011_hd2502(mt_fe_hd2502_priv_handle handle)
{
	hd2502_handle = handle;
	hd2502_handle->tuner_handle = (U32)ts6011_handle;

	mt_fe_tn_ts6011_wakeup_hd2502(ts6011_handle);

	return MtFeErr_Ok;
}

MT_FE_RET mt_fe_tn_get_strength_ts6011_hd2502(mt_fe_hd2502_priv_handle handle, U32 *p_gain, S32 *p_strength)
{
	S32 level = -8000;
	S32 gain = 0;
	...
	hd2502_handle = handle;
	handle->tuner_handle = (U32)ts6011_handle;

	// Step 1: read the AGC PWM rate from the demodulator register

	//handle->dmd_get_reg(handle, 0x3f, &AgcPWM);
	HDIC2501_ReadRegister(0x3f, &AgcPWM);
	// Step 2: Calculate the AGC voltage based on the AGC PWM rate, unit: mV
	...
	gain = mt_fe_tn_ts6011_get_gain_hd2502(ts6011_handle, Vagc);
	...
	//mt_fe_dmd_hd2502_get_pure_lock(handle, &lockstate);
	HDIC2501_IsDemodLocked(&locked, &Status);
	bLocked = (locked == 1) ? TRUE : FALSE;
...
MT_FE_RET mt_fe_tn_set_freq_ts6011_hd2502(mt_fe_hd2502_priv_handle handle, U32 freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)
{
	...
	mt_fe_tn_ts6011_set_freq_hd2502(ts6011_handle, freq_KHz, sym_rate_KSs, lpf_offset_KHz);

	return MtFeErr_Ok;
}

S32 mt_fe_tn_get_tuner_freq_offset_ts6011_hd2502(mt_fe_hd2502_priv_handle handle)
{
	hd2502_handle = handle;
	handle->tuner_handle = (U32)ts6011_handle;

	return mt_fe_tn_ts6011_get_freq_offset_hd2502(ts6011_handle);
}

MT_FE_RET mt_fe_tn_get_gain_ts6011_hd2502(mt_fe_hd2502_priv_handle handle, U32 *p_gain)
{
	S32 Vagc;
	U8  AgcPWM;
	...
	*p_gain = mt_fe_tn_ts6011_get_gain_hd2502(ts6011_handle, Vagc);
