...
#include "port_sony_cxd2856.h"
#include "drv_frontend_ioctl.h"

#include "drv_gpio_ioctl.h"

#include "sony_cxd2856/dvb_cable/sony_demod_dvbc_monitor.h"
#include "sony_cxd2856/dvb_cable/sony_demod_dvbc2_monitor.h"
#include "sony_cxd2856/dvb_terr/sony_demod_dvbt_monitor.h"
#include "sony_cxd2856/dvb_terr/sony_demod_dvbt2_monitor.h"
#include "sony_cxd2856/dvb_sat/sony_demod_dvbs_s2_monitor.h"
#include "sony_cxd2856/j83b/sony_demod_j83b_monitor.h"

#include "sony_cxd2856/tuner/terr_cable_MxL608/sony_tuner_MxL608.h"
#include "sony_cxd2856/tuner/R850/sony_tuner_r850.h"
#include "sony_cxd2856/tuner/R858C/sony_tuner_r858.h"
#include "sony_cxd2856/tuner/R836/sony_tuner_r836.h"
#include "sony_cxd2856/tuner/tc6800/sony_tuner_tc6800.h"
...
//#define FOR_PORT_SONY_CXD2856_CONNECT_SYCHRONOUS
...
#define MT_FE_SONY_CXD2856_PIN_LEVEL_LOW 0
#define MT_FE_SONY_CXD2856_PIN_LEVEL_HIGH 1
//#define MT_FE_SONY_CXD2856_DISEQC_COMMAND_START_DELAY 30
#define MT_FE_SONY_CXD2856_DISEQC_COMMAND_START_DELAY 50
#define MT_FE_SONY_CXD2856_DISEQC_COMMAND_END_DELAY 50

#define MAX_TP_ONE_SCAN MAX_BS_TP_NUM_PER_SAT

#define SONY_CXD2856_NL_NAME "sat_bs_nl"

/* cmd0 can match data0 and data1, cmd1 also can match data0 and data1 */
enum
{
  SONY_CXD2856_NL_OPS_CMD0,
  //SONY_CXD2856_NL_OPS_CMD1,
  __SONY_CXD2856_NL_OPS_MAX,
};

#define SONY_CXD2856_NL_OPS_AMOUNT (__SONY_CXD2856_NL_OPS_MAX)

enum
{
  SONY_CXD2856_NL_ATTR_UNSPEC,
  SONY_CXD2856_NL_ATTR_DATA0,
  //SONY_CXD2856_NL_ATTR_DATA1,
  __SONY_CXD2856_NL_ATTR_MAX,
};

#define SONY_CXD2856_NL_FAMILY_ATTR_MAX (__SONY_CXD2856_NL_ATTR_MAX - 1)
#define SONY_CXD2856_NL_FAMILY_ATTR_AMOUNT (__SONY_CXD2856_NL_ATTR_MAX)

struct sony_cxd2856_netlink_data
{
  pid_t pid;
  u32 seq;
  u32 freq_khz;
...
struct sony_cxd2856_netlink_pid_list
{
  struct list_head list;
  struct sony_cxd2856_netlink_data sony_cxd2856_nl_data;
  struct net *net;
};

static sony_example_driver_handle g_sony_cxd2856_handle[MAX_SONY_CXD2856_DEVICES] = {NULL, NULL};
static mt_fe_sony_cxd2856_priv_handle p_priv[MAX_SONY_CXD2856_DEVICES] = {NULL, NULL};
static sony_demod_create_param_t createParam[MAX_SONY_CXD2856_DEVICES];


static uint8_t gFeIndex = 0;
static uint8_t gFeCount[MAX_SONY_CXD2856_DEVICES] = {0,0};
...
static sony_example_driver_instance_t *dev_handle[MAX_SONY_CXD2856_DEVICES] = {NULL, NULL};

static int bInitialized_SONY_CXD2856[MAX_SONY_CXD2856_DEVICES] = {0, 0};

//extern sony_result_t mt_fe_dmd_register_notify_sony_cxd2856_ss2(void (*callback)(MT_FE_MSG msg, void *p_tp_info));

/* The crypto netlink socket */
//static struct sock *sony_cxd2856_nlsk;

mt_fe_sony_cxd2856_priv_handle g_sony_cxd2856_priv[MAX_SONY_CXD2856_DEVICES] = {NULL, NULL};
int g_i2c_sony_cxd2856 = 0;
static uint8_t sony_cxd2856_notify_scan_status[MAX_SONY_CXD2856_DEVICES] = {0, 0};
mt_unf_fe_channel_info_t *sony_cxd2856_pg_channel_info[MAX_SONY_CXD2856_DEVICES] = {NULL, NULL};


static int port_sony_cxd2856_get_ber(void *handle, MT_U32 *p_ber);

//mmz_buffer_s sMBuf;

sony_i2c_t cxd2856_i2c;

void sony_cxd2856_get_sony_index_by_addr(uint8_t demod_addr);

extern R840_ErrCode R840_AGC_Slow(void); //Set AGC clock to 60Hz

extern sony_result_t sony_tuner_r858_get_tuner_num(sony_tuner_terr_cable_t *pTuner, R858_ExtTunerNum_Type *pExtTunerNum, R858_IntTunerNum_Type *pIntTunerNum);
extern void i2c_isr_onoff(int i2_id, int on);

#if 0
static int port_sony_cxd2856_blind_scan_cancel(void *handle);
static int port_sony_cxd2856_blind_scan_start(void *pArg);
...
static LIST_HEAD(sony_cxd2856_nl_pid_list);

static DEFINE_MUTEX(sony_cxd2856_nl_mutex);
#if 0
static struct genl_ops sony_cxd2856_nl_ops[SONY_CXD2856_NL_OPS_AMOUNT];


static struct genl_family sony_cxd2856_nl_family = {
    .name = SONY_CXD2856_NL_NAME,
    .version = 0x1,
    .maxattr = SONY_CXD2856_NL_FAMILY_ATTR_MAX,
    .netnsok = true,
    .module = THIS_MODULE,
    .ops = sony_cxd2856_nl_ops,
    .n_ops = ARRAY_SIZE(sony_cxd2856_nl_ops),
};
#endif
extern struct genl_family dvbs_nl_family;

/*static struct sony_cxd2856_netlink_data g_sony_cxd2856_nl_data_snd = {0}; clean warning*/

sony_example_driver_handle mt_fe_sony_cxd2856_get_handle(void)
{
  return g_sony_cxd2856_handle[gSonyIndex];
}

#if 0 /*clean warning  no used*/
static int sony_cxd2856_nl_sendmsg(mt_unf_fe_channel_info_t *p_channel_info)
{
  //mt_unf_fe_channel_info_t *p_channel_info = NULL;
  struct genl_info info;
  struct sony_cxd2856_netlink_data sony_cxd2856_nl_data_snd;
  struct sk_buff *skb;
  struct sony_cxd2856_netlink_data *sony_cxd2856_nl_data;
  ...
  {
    sony_cxd2856_nl_data = &(sony_cxd2856_nl_pid->sony_cxd2856_nl_data);
    pid = sony_cxd2856_nl_data->pid;
    ...
    hdr = genlmsg_put(skb, 0 /* from kernel */,
                      sony_cxd2856_nl_data->seq, &sony_cxd2856_nl_family, 0, sony_cxd2856_nl_data->cmd);
    if (!hdr)
      goto out_nlmsg_free;
    ...
    genl_info_net_set(&info, sony_cxd2856_nl_pid->net);

    NETLINK_CB(skb).portid = 0; /* from kernel */

    snprintf(sony_cxd2856_nl_data_snd.name, sizeof(sony_cxd2856_nl_data_snd.name), "kernel nl");
    sony_cxd2856_nl_data_snd.freq_khz = p_channel_info->frequency; //0xaabbccdd;
    sony_cxd2856_nl_data_snd.symbol_rate = p_channel_info->symbol_rate;
    sony_cxd2856_nl_data_snd.port_type = p_channel_info->port_type;
    sony_cxd2856_nl_data_snd.pid = pid;
    sony_cxd2856_nl_data_snd.seq = sony_cxd2856_nl_data->seq + 1;
    sony_cxd2856_nl_data_snd.cmd = sony_cxd2856_nl_data->cmd;
    sony_cxd2856_nl_data_snd.data01 = sony_cxd2856_nl_data->data01;

    //NLA_PUT_TYPE(skb, typeof(sony_cxd2856_nl_data_snd), sony_cxd2856_nl_data_snd.data01, sony_cxd2856_nl_data_snd);
    if (nla_put(skb, sony_cxd2856_nl_data_snd.data01, sizeof(sony_cxd2856_nl_data_snd), &sony_cxd2856_nl_data_snd))
...
sony_result_t _mt_i2c_write_sony(uint8_t dev_addr, uint8_t *w_buf, U16 w_byte)
{
  ...
  i2c = i2c_get_adapter(g_i2c_sony_cxd2856);
  msg->addr = dev_addr;
...
mt_unf_fe_fec_type_t port_sony_cxd2856_deparse_dvb_type(sony_dtv_system_t dtv_type)
{
...
static int port_sony_cxd2856_channel_set(void *handle, mt_unf_fe_connect_para_t *para)
{
  //MT_U32 for_scan = 0;
  sony_result_t ret = SONY_RESULT_OK, result = SONY_RESULT_OK;
  sony_dtv_system_t dtv_type = SONY_DTV_SYSTEM_ANY;
  mt_unf_fe_channel_info_t *p_channel_info = NULL;
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;

  sony_cxd2856_get_sony_index_by_addr(sony_cxd2856_handle->demod.i2cAddressSLVT);

  //p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  //sony_cxd2856_handle = p_priv->sony_cxd2856_handle;

  //printk("%s[%d][%d] ---- handle = 0x%08x, sony_handle = 0x%08x\n", __FUNCTION__, __LINE__, gSonyIndex, handle, sony_cxd2856_handle);
      ...
      (para->sig_type == (MT_UNF_FE_SIG_TYPE_SAT | MT_UNF_FE_SIG_TYPE_SAT_2)))
  {
    ...
    printk("sony_cxd2856_channel set ss2: freq = %d, sym = %d, type = %d, use_uc = %d\n",
    ...
    ret = sony_integ_dvbs_s2_Tune(&(sony_cxd2856_handle->integ), &sony_ss2_param);
      ...
      (para->sig_type == (MT_UNF_FE_SIG_TYPE_DVB_T | MT_UNF_FE_SIG_TYPE_DVB_T2)))
  {
    ...
    if (MT_UNF_FE_SIG_TYPE_DVB_T == para->sig_type)
    {
      ...
      ret = sony_integ_dvbt_BlindTune(&(sony_cxd2856_handle->integ),
      ...
      sony_cxd2856_handle->demod.system = dtv_type;
    }
    else if (MT_UNF_FE_SIG_TYPE_DVB_T2 == para->sig_type)
    {
      ...
      ret = sony_integ_dvbt2_BlindTune(&(sony_cxd2856_handle->integ),
                                       para->connect_param.ter.freq,
                                       sony_tt2_bw,
                                       profile,
                                       &sony_tuned_profile);
      ...
      if (ret == SONY_RESULT_OK)
      {
        ...
        tmpResult = sony_demod_dvbt2_monitor_DataPLPs(&(sony_cxd2856_handle->demod), 
      ...
      else
      {
        ...
        sony_demod_dvbt2_CheckTSLock(&(sony_cxd2856_handle->demod), &stat);
      ...
      sony_integ_dvbt2_Tune(&(sony_cxd2856_handle->integ), &t2_tune_param);
      if (ret == SONY_RESULT_OK)
      {
        sony_result_t tmpResult;
    ...
    else
    {
      ...
      ret = sony_integ_dvbt_t2_BlindTune(&(sony_cxd2856_handle->integ),
                                         para->connect_param.ter.freq,
                                         sony_tt2_bw,
                                         dtv_type,
                                         profile,
      ...
      if (sony_tuned_system == SONY_DTV_SYSTEM_DVBT)
      {
        ...
        sony_cxd2856_handle->demod.system = SONY_DTV_SYSTEM_DVBT;
      }
      else if (sony_tuned_system == SONY_DTV_SYSTEM_DVBT2)
      {
        ...
        {
          result = sony_integ_dvbt2_Scan_SwitchDataPLP(&(p_priv->sony_cxd2856_handle)->integ, 1, (uint8_t)para->connect_param.ter.plp_id, sony_tuned_profile);
        ...
        sony_cxd2856_handle->demod.system = SONY_DTV_SYSTEM_DVBT2;
		/*fix issue30643*/
      ...
      else
      {
        ...
        sony_cxd2856_handle->demod.system = SONY_DTV_SYSTEM_ANY;
      }
    }

    printk("sony_cxd2856_channel_set() tt2: freq = %d, bandwidth = %d, type = %d, dtv_type = %d, ret = %d\n",
           para->connect_param.ter.freq,
  ...
  else if (para->sig_type == MT_UNF_FE_SIG_TYPE_CAB)
  {
    ...
    printk("sony_cxd2856_channel set c: freq = %d, symbol rate = %d, bandwidth = %d, type = %d\n",
    ...
    ret = sony_integ_dvbc_Tune(&(sony_cxd2856_handle->integ), &sony_c_param);
  ...
  else if (para->sig_type == MT_UNF_FE_SIG_TYPE_J83B)
  {
    ...
    printk("sony_cxd2856_channel set b: freq = %d, symbol rate = %d, type = %d\n",
           para->connect_param.cab.freq,
           para->connect_param.cab.sym_rate,
           dtv_type);
    ...
    ret = sony_integ_j83b_Tune(&(sony_cxd2856_handle->integ), &sony_b_param);
...
static void port_sony_cxd2856_set_22k_onoff(void *handle, uint8_t onoff_22k, uint8_t diseqc_out_when_lnb_off)
{
  ...
  //mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_instance_t * sony_cxd2856_handle = p_priv->sony_cxd2856_handle;


  printk("port_sony_cxd2856_set_22k_onoff %d - %s\n", onoff_22k, (onoff_22k == 1) ? "22K On" : "22K Off");
}

static void port_sony_cxd2856_set_lnb_voltage(void *handle, MT_FE_LNB_VOLTAGE voltage, uint8_t vsel_pin_13v)
{

}

static void port_sony_cxd2856_set_lnb_onoff(void *handle, uint8_t lnb_enable, mt_unf_fe_pin_config_para_t *pin_config)
{
}

static int port_sony_cxd2856_diseqc_sendmsg(void *handle, mt_unf_fe_diseqc_sendmsg_t *p_diseqc_sendmsg)
{
...
static int port_sony_cxd2856_diseqc_send_tone_burst(void *handle, uint8_t mode)
{
  sony_result_t ret = SONY_RESULT_OK;
...
static int port_sony_cxd2856_diseqc_recvmsg(void *handle, mt_unf_fe_diseqc_recvmsg_t *p_diseqc_recvmsg)
{
  sony_result_t ret = SONY_RESULT_OK;
...
static int port_sony_cxd2856_get_status(void *handle, mt_unf_fe_status_t *p_status)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  ...
  switch ((&sony_cxd2856_handle->demod)->system)
  {
  case SONY_DTV_SYSTEM_DVBC:
    sony_demod_dvbc_CheckTSLock(&(sony_cxd2856_handle->demod), &stat);
  ...
  case SONY_DTV_SYSTEM_DVBC2:
    sony_demod_dvbc2_CheckTSLock(&(sony_cxd2856_handle->demod), &stat);
    break;
  ...
  case SONY_DTV_SYSTEM_DVBT:
	...
	*/
	sony_demod_dvbt_CheckDemodLock(&(sony_cxd2856_handle->demod), &stat);
  ...
  case SONY_DTV_SYSTEM_DVBT2:
	...
	*/
	sony_demod_dvbt2_CheckDemodLock(&(sony_cxd2856_handle->demod), &stat);
  ...
  case SONY_DTV_SYSTEM_J83B:
    sony_demod_j83b_CheckTSLock(&(sony_cxd2856_handle->demod), &stat);
    break;

#ifdef SONY_DEMOD_SUPPORT_DVBS_S2
  case SONY_DTV_SYSTEM_DVBS:
  case SONY_DTV_SYSTEM_DVBS2:
    sony_demod_dvbs_s2_CheckTSLock(&(sony_cxd2856_handle->demod), &stat);
    break;
#endif
    ...
    switch ((&sony_cxd2856_handle->demod)->system)
    {
      ...
      case SONY_DTV_SYSTEM_DVBT2:
        if ((p_priv->cfg.tun2_type == MT_UNF_TUNER_TYPE_R836) || 
            (p_priv->cfg.tun2_type == MT_UNF_TUNER_TYPE_RAFAEL836))
        {
          result = sony_demod_I2cRepeaterEnable(&sony_cxd2856_handle->demod, 0x01);

          R840_AGC_Slow();

          //printk("%s[%d] ---- R840_AGC_Slow()\n", __FUNCTION__, __LINE__);

          result = sony_demod_I2cRepeaterEnable(&sony_cxd2856_handle->demod, 0x00);
        }
        break;
...
static int port_sony_cxd2856_get_signal_quality(void *handle, MT_U32 *p_quality)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  ...
  {
  case SONY_DTV_SYSTEM_DVBC:
    sony_demod_dvbc_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
    percent = (uint8_t)(snr * 2);
    break;

  case SONY_DTV_SYSTEM_DVBC2:
    sony_demod_dvbc2_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
    percent = (uint8_t)(snr * 2);
    break;

  case SONY_DTV_SYSTEM_DVBT:
    sony_demod_dvbt_monitor_Quality(&(sony_cxd2856_handle->demod), &percent);
    break;

  case SONY_DTV_SYSTEM_DVBT2:
    sony_demod_dvbt2_monitor_Quality(&(sony_cxd2856_handle->demod), &percent);
    break;

  case SONY_DTV_SYSTEM_J83B:
    sony_demod_j83b_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
    percent = (uint8_t)(snr * 2);
    break;
  ...
  case SONY_DTV_SYSTEM_DVBS:
  case SONY_DTV_SYSTEM_DVBS2:
    sony_demod_dvbs_s2_monitor_CNR(&(sony_cxd2856_handle->demod), &snr);
...
static int port_sony_cxd2856_get_ber(void *handle, MT_U32 *p_ber)
{
  ...
  {
  case SONY_DTV_SYSTEM_DVBC:
    sony_demod_dvbc_monitor_PER(&sony_cxd2856_handle->demod, &ber);
    break;

  case SONY_DTV_SYSTEM_DVBC2:
    sony_demod_dvbc2_monitor_PER(&sony_cxd2856_handle->demod, &ber);
    break;

  case SONY_DTV_SYSTEM_DVBT:
    sony_demod_dvbt_monitor_PER(&(sony_cxd2856_handle->demod), &ber);
    break;

  case SONY_DTV_SYSTEM_DVBT2:
    sony_demod_dvbt2_monitor_PER(&(sony_cxd2856_handle->demod), &ber);
    break;

  case SONY_DTV_SYSTEM_J83B:
    sony_demod_j83b_monitor_PER(&(sony_cxd2856_handle->demod), &ber);
    break;

#ifdef SONY_DEMOD_SUPPORT_DVBS_S2
  case SONY_DTV_SYSTEM_DVBS:
  case SONY_DTV_SYSTEM_DVBS2:
    sony_demod_dvbs_s2_monitor_PER(&(sony_cxd2856_handle->demod), &ber);
    break;
#endif
...
static int port_sony_cxd2856_get_accurate_snr(void *handle, MT_S32 *p_snr)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  ...
  case SONY_DTV_SYSTEM_DVBT:
    sony_demod_dvbt_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
    break;

  case SONY_DTV_SYSTEM_DVBT2:
    sony_demod_dvbt2_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
    break;
...
static int port_sony_cxd2856_get_snr(void *handle, MT_U32 *p_snr)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
...
static int port_sony_cxd2856_get_signal_strength(void *handle, MT_U32 *p_strength)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  int32_t _strength = 0;
  ...
  case SONY_DTV_SYSTEM_J83B:
    ...
    (&sony_cxd2856_handle->tunerTerrCable)->ReadRFLevel(&sony_cxd2856_handle->tunerTerrCable, &_strength);
...
static void port_sony_cxd2856_get_signal_info(void *handle, mt_unf_fe_signal_info_t *p_sig_info)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  ...
  if((sony_cxd2856_handle->demod.system == SONY_DTV_SYSTEM_DVBT)||(sony_cxd2856_handle->demod.system == SONY_DTV_SYSTEM_DVBT2))
  {
  ...
  {
    pr_err("ERR:%s[%d] ---- UNSUPPORT sony_cxd2856_handle->demod.system = %d\n", __FUNCTION__, __LINE__, sony_cxd2856_handle->demod.system);
  }

  //printk("%s[%d] ---- sony_cxd2856_handle->demod.system = %d\n", __FUNCTION__, __LINE__, sony_cxd2856_handle->demod.system);
  ...
  switch (sony_cxd2856_handle->demod.system)
  {
    case SONY_DTV_SYSTEM_DVBT:
      ...
      result = sony_demod_dvbt_monitor_TPSInfo(&(sony_cxd2856_handle->demod), 
                                      &dvbt_tpsinfo);
    ...
    case SONY_DTV_SYSTEM_DVBT2:
      ...
      result = sony_demod_dvbt2_monitor_L1Pre(&(sony_cxd2856_handle->demod), 
                                              &dvbt2_l1pre);
      ...
      sony_demod_dvbt2_monitor_ActivePLP(&(sony_cxd2856_handle->demod),
                                         SONY_DVBT2_PLP_DATA,
                                         &dvbt2_plpinfo);
...
static int port_sony_cxd2856_get_signal_agc(void *handle, mt_u32 *p_agc)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  int32_t _strength = 0;
...
static int port_sony_cxd2856_get_default_timeout(void *handle, mt_u32 *timeout)
{
  *timeout = 2000;
...
static int port_sony_cxd2856_set_io(void *handle, MT_BOOL onoff)
{
  return MT_SUCCESS;
}

static int port_sony_cxd2856_channel_connect(void *handle, mt_unf_fe_connect_para_t *para)
{
  int ret = 0;
  ...
  ret = port_sony_cxd2856_channel_set(handle, para);
...
static int port_sony_cxd2856_standby(void *handle)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  ...
  //sony_integ_Shutdown(&(sony_cxd2856_handle->integ));
  sony_integ_Sleep(&(sony_cxd2856_handle->integ));

  return MT_SUCCESS;
}

static int port_sony_cxd2856_wakeup(void *handle)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = p_priv->sony_cxd2856_handle;
  ...
  //sony_integ_Sleep(&(sony_cxd2856_handle->integ));
  port_sony_cxd2856_channel_connect(handle, &(p_priv->param));

  return MT_SUCCESS;
}

static int port_sony_cxd2856_get_dbg_info(void *handle)
{
  mt_fe_sony_cxd2856_priv_handle p_priv = (mt_fe_sony_cxd2856_priv_handle)handle;
  ...
  case SONY_DTV_SYSTEM_DVBT:
    sony_demod_dvbt_CheckTSLock(&(sony_cxd2856_handle->demod), &stat);
...

    {
      ...
      result = sony_demod_dvbt_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
...

    {
      ...
      result = sony_demod_dvbt_monitor_PreViterbiBER(&(sony_cxd2856_handle->demod), &ber);
...

    {
      ...
      result = sony_demod_dvbt_monitor_PreRSBER(&(sony_cxd2856_handle->demod), &ber);
...

    {
      ...
      result = sony_demod_dvbt_monitor_PER(&(sony_cxd2856_handle->demod), &per);
...

    {
      ...
      result = sony_demod_dvbt_monitor_PacketErrorNumber(&(sony_cxd2856_handle->demod), &pen);
  ...
  case SONY_DTV_SYSTEM_DVBT2:
...

    {
      ...
      result = sony_demod_dvbt2_monitor_SNR(&(sony_cxd2856_handle->demod), &snr);
...

    {
      ...
      result = sony_demod_dvbt2_monitor_PreLDPCBER(&(sony_cxd2856_handle->demod), &ber);
...

    {
      ...
      result = sony_demod_dvbt2_monitor_PreBCHBER(&(sony_cxd2856_handle->demod), &ber);
...

    {
      ...
      result = sony_demod_dvbt2_monitor_PER(&(sony_cxd2856_handle->demod), &per);
...

    {
      ...
      result = sony_demod_dvbt2_monitor_PacketErrorNumber(&(sony_cxd2856_handle->demod), &pen);
  ...
  if (p_priv->cfg.tun2_type == MT_UNF_TUNER_TYPE_R858C)
  {
    ...
    result = sony_tuner_r858_get_tuner_num(&dev_handle[gSonyIndex]->tunerTerrCable, &ExtTunerNum, &IntTunerNum);
...
static int port_sony_cxd2856_ioctl(void *handle, mt_u32 cmd, ulong param)
{
  ...
  case NIM_IOCTRL_GET_SIGNAL_INFO:
    if (param != 0)
      port_sony_cxd2856_get_signal_info(handle, (mt_unf_fe_signal_info_t *)param);
    break;
  ...
  case NIM_IOCTRL_GET_SCAN_STATUS:
    ...
    if (param != 0)
      *((uint8_t *)param) = sony_cxd2856_notify_scan_status[gSonyIndex];
  ...
  case NIM_IOCTRL_DISEQC_SENDMSG:
    ...
    port_sony_cxd2856_diseqc_sendmsg(handle, p_sendmsg);
    break;

  case NIM_IOCTRL_DISEQC_SEND_TONEBURST:
    //p_diseqc_cmd = (mt_unf_fe_diseqc_sendmsg_t*)param;
    port_sony_cxd2856_diseqc_send_tone_burst(handle, param); // 'param' is the target tone_burst
    break;

  case NIM_IOCTRL_DISEQC_RECVMSG:
    p_recvmsg = (mt_unf_fe_diseqc_recvmsg_t *)param;
    port_sony_cxd2856_diseqc_recvmsg(handle, p_recvmsg);
    break;
    //#endif
#endif
  ...
  case NIM_IOCTRL_T2_SET_PLP_NO:
    ...
    result = sony_integ_dvbt2_Scan_SwitchDataPLP(&(p_priv->sony_cxd2856_handle)->integ, 1, (uint8_t)param, sony_tuned_profile);
  ...
  case NIM_IOCTRL_T_T2_GET_CELL_ID:
    ...
    switch ((&sony_cxd2856_handle->demod)->system)
    {
      ...
      case SONY_DTV_SYSTEM_DVBT2:
        result = sony_demod_dvbt2_monitor_L1Pre(&(sony_cxd2856_handle->demod), &dvbt2_l1pre);
  ...
  case NIM_IOCTRL_GET_TN_DBG_INFO:
    port_sony_cxd2856_get_dbg_info(p_priv);
    break;

  case NIM_IOCTRL_SUSPEND:
    sony_cxd2856_suspend(p_priv);
    break;

  case NIM_IOCTRL_RESUME:
    sony_cxd2856_resume(p_priv);
    break;
  case NIM_IOCTRL_GET_ACCURATE_SNR:
    port_sony_cxd2856_get_accurate_snr(p_priv,(MT_S32 *)param);
    break;
  default:
    break;
  }
...
static int port_sony_cxd2856_blind_scan_cancel(void *handle)
{
  ...
  printk("port_sony_cxd2856_blind_scan_cancel\n");
  mt_fe_dmd_blindscan_abort_ss2_sony_cxd2856(sony_cxd2856_handle, 1);
#endif
...
static const struct nla_policy sony_cxd2856_policy[SONY_CXD2856_NL_FAMILY_ATTR_AMOUNT] = {
    [SONY_CXD2856_NL_ATTR_DATA0] = {.type = NLA_BINARY, .len = sizeof(struct sony_cxd2856_netlink_data)},
};

__attribute__((unused))static int sony_cxd2856_nl_recvmsg0(struct sk_buff *skb, struct genl_info *info)
{
  struct sony_cxd2856_netlink_data *sony_cxd2856_nl_data;
  struct sony_cxd2856_netlink_pid_list *sony_cxd2856_nl_pid;
  int err = 0;
  ...
  if (info->attrs[SONY_CXD2856_NL_ATTR_DATA0])
  {
    sony_cxd2856_nl_data = nla_data(info->attrs[SONY_CXD2856_NL_ATTR_DATA0]);
    sony_cxd2856_nl_pid = kzalloc(sizeof(struct sony_cxd2856_netlink_pid_list), GFP_KERNEL);
    if (sony_cxd2856_nl_pid == NULL)
      return -1; // No memory, SONY_RESULT_ERROR_OVERFLOW
    sony_cxd2856_nl_pid->net = sock_net(skb->sk);
    /* copy from sk_buff, because we will use it in other thread, and sk_buff will be released soon */
    sony_cxd2856_nl_pid->sony_cxd2856_nl_data = *sony_cxd2856_nl_data;
    //printk("netlink : app_pid = %d\n", NETLINK_CB(skb).portid);
    //printk("netlink : pid = %d\n", sony_cxd2856_nl_data->pid);
    //printk("netlink : seq = 0x%x\n", sony_cxd2856_nl_data->seq);
    //printk("netlink : cmd = %d\n", sony_cxd2856_nl_data->cmd);
    //printk("netlink : data01 = %d\n", sony_cxd2856_nl_data->data01);
    //printk("netlink : name : %s, freq = %u, sym = %u\n", sony_cxd2856_nl_data->name, sony_cxd2856_nl_data->freq_khz, sony_cxd2856_nl_data->symbol_rate);
    ...
    list_add_tail(&(sony_cxd2856_nl_pid->list), &sony_cxd2856_nl_pid_list);
    mutex_unlock(&sony_cxd2856_nl_mutex);
  }
...
static struct genl_ops sony_cxd2856_nl_ops[SONY_CXD2856_NL_OPS_AMOUNT] = {
    [SONY_CXD2856_NL_OPS_CMD0] = {
        .cmd = SONY_CXD2856_NL_OPS_CMD0,
        .doit = sony_cxd2856_nl_recvmsg0,
        .policy = sony_cxd2856_policy,
    },
};
...
uint8_t sony_cxd2856_get_sony_index(mt_unf_fe_attr_t *attr)
{
  uint8_t index = 0, tmp = 0;

  for (index = 0; index < MAX_SONY_CXD2856_DEVICES; index++)
  {
  ...
  if (index == MAX_SONY_CXD2856_DEVICES)
  {
    for (tmp = 0; tmp < MAX_SONY_CXD2856_DEVICES; tmp++)
    {
      if (bInitialized_SONY_CXD2856[tmp] == 0)
      {
        index = tmp;
        break;
      }
...
int sony_cxd2856_attach(frontend_info_s *info, mt_unf_fe_attr_t *attr)
{
  //uint8_t reg_val = 0;
  ...
  uint8_t enableSatTuner = 0;
  uint8_t enableLNBC = 0;
  uint8_t tuner_addr = 0xC0;

  uint8_t cur_sony_index = sony_cxd2856_get_sony_index(attr);

  if (cur_sony_index == MAX_SONY_CXD2856_DEVICES)
  {
    printk("%s[%d] ---- Too many (> %d) Sony CXD2856 devices.\n", __FUNCTION__, __LINE__, MAX_SONY_CXD2856_DEVICES);
  ...
  if (bInitialized_SONY_CXD2856[gSonyIndex])
  {
    return MT_SUCCESS;
  }

  cxd2856_i2c.Read = Sony_I2C_Read;
  cxd2856_i2c.Write = Sony_I2C_Write;
  cxd2856_i2c.ReadRegister = Sony_I2C_ReadRegister;
  cxd2856_i2c.WriteRegister = Sony_I2C_WriteRegister;
  cxd2856_i2c.WriteOneRegister = Sony_I2C_WriteOneRegister;

  info->ops.connect = port_sony_cxd2856_channel_connect;
  info->ops.get_status = port_sony_cxd2856_get_status;
  info->ops.get_ber = port_sony_cxd2856_get_ber;
  info->ops.get_snr = port_sony_cxd2856_get_snr;
  info->ops.get_signal_strength = port_sony_cxd2856_get_signal_strength;
  info->ops.get_signal_quality = port_sony_cxd2856_get_signal_quality;
  info->ops.get_signal_agc = port_sony_cxd2856_get_signal_agc;

  info->ops.standby = port_sony_cxd2856_standby;
  info->ops.wakeup = port_sony_cxd2856_wakeup;
  info->ops.get_default_timeout = port_sony_cxd2856_get_default_timeout;
  info->ops.set_io = port_sony_cxd2856_set_io;
  info->ops.port_ioctl = port_sony_cxd2856_ioctl;
  //info->ops.blind_scan = port_sony_cxd2856_blind_scan;
	...
	HAL_PUT_U32(SYMPHONY_IO_VA(0xbf200020), 0);
	HAL_PUT_U32(SYMPHONY_IO_VA(0xbf200020), 0xc7eb11ff);
#elif 1 //defined(CONFIG_MT_CHIP_SYMPHONY6) // Sym6 + SONYCXD2856 + TC6800 pinmux TS3 4线
  ...
  {
    //p_priv[gSonyIndex] = kzalloc(sizeof(mt_fe_sony_cxd2856_priv_t), GFP_KERNEL);
    p_priv[gSonyIndex] = vzalloc(sizeof(mt_fe_sony_cxd2856_priv_t));
    if (p_priv[gSonyIndex] == NULL)
      return -ENOMEM;
  }
  ...
  p_priv[gSonyIndex]->sony_cxd2856_handle = dev_handle[gSonyIndex];
  info->handle = (void *)p_priv[gSonyIndex];
  g_sony_cxd2856_priv[gSonyIndex] = p_priv[gSonyIndex];

  g_sony_cxd2856_handle[gSonyIndex] = dev_handle[gSonyIndex];

  g_i2c_sony_cxd2856 = attr->demod_i2c_id;

  if (g_i2c_sony_cxd2856 == 1)
  {
  ...
  if (NULL == sony_cxd2856_pg_channel_info[gSonyIndex])
  {
    //sony_cxd2856_pg_channel_info[gSonyIndex] = kzalloc(sizeof(mt_unf_fe_channel_info_t) * MAX_BS_TP_NUM_PER_SAT, GFP_KERNEL);
    sony_cxd2856_pg_channel_info[gSonyIndex] = vzalloc(sizeof(mt_unf_fe_channel_info_t) * MAX_BS_TP_NUM_PER_SAT);
    if (sony_cxd2856_pg_channel_info[gSonyIndex] == NULL)
    {
  ...
  p_priv[gSonyIndex]->scan_info.p_channel_info = sony_cxd2856_pg_channel_info[gSonyIndex];

  if (bInitialized_SONY_CXD2856[gSonyIndex] == 0)
  {
#if 0 // tuner @ 20210106
        ...
        (attr->fe_config.tun2_type == MT_UNF_TUNER_TYPE_MXL_608)) 
    {
      ...
      result = sony_tuner_MxL608_Create(&dev_handle[gSonyIndex]->tunerTerrCable, MXL608_XTAL_16MHz, tuner_addr, &cxd2856_i2c, 0, &MxL608Tuner);
      if (result != SONY_RESULT_OK)
      {
    ...
    else if (attr->fe_config.tun2_type == MT_UNF_TUNER_TYPE_R850)
    {
      ...
      result = sony_tuner_r850_Create(&dev_handle[gSonyIndex]->tunerTerrCable, tuner_addr, &cxd2856_i2c, 0, 0, &R850Tuner);
      if (result != SONY_RESULT_OK)
      {
    ...
    else if (attr->fe_config.tun2_type == MT_UNF_TUNER_TYPE_R858C)
    {
      ...
      result = sony_tuner_r858_Create(&dev_handle[gSonyIndex]->tunerTerrCable, tuner_addr, &cxd2856_i2c, 0, gSonyIndex, &R858Tuner);
      if (result != SONY_RESULT_OK)
      {
             ...
             (attr->fe_config.tun2_type == MT_UNF_TUNER_TYPE_RAFAEL836))
    {
      ...
      //printk("%s[%d] -- Select tuner R836, addr[%02x][%02x], dev_handle[%d]->tunerTerrCable = 0x%08x\n", __FUNCTION__, __LINE__, tuner_addr, attr->fe_config.tun2_addr, gSonyIndex, (ulong)&(dev_handle[gSonyIndex]->tunerTerrCable));

      result = sony_tuner_r840_Create(&dev_handle[gSonyIndex]->tunerTerrCable, tuner_addr, &cxd2856_i2c, 0, 0, &R840Tuner);
      if (result != SONY_RESULT_OK)
      {
             ...
             (attr->tuner_type == MT_UNF_TUNER_TYPE_RAFAEL836))
    {
      ...
      //printk("%s[%d] -- Select tuner R836, addr[%02x][%02x], dev_handle[%d]->tunerTerrCable = 0x%08x\n", __FUNCTION__, __LINE__, tuner_addr, attr->tuner_addr, gSonyIndex, (ulong)&(dev_handle[gSonyIndex]->tunerTerrCable));
    ...
    else if (attr->fe_config.tun2_type == MT_UNF_TUNER_TYPE_M88TC6800)
    {
      ...
      //printk("%s[%d] -- Select tuner TC6800, addr[%02x][%02x], dev_handle[%d]->tunerTerrCable = 0x%08x\n", __FUNCTION__, __LINE__, tuner_addr, attr->tuner_addr, gSonyIndex, &(dev_handle[gSonyIndex]->tunerTerrCable));

      result = sony_tuner_tc6800_Create(&dev_handle[gSonyIndex]->tunerTerrCable, tuner_addr, &cxd2856_i2c, 0, 0, &TC6800Tuner);
      if (result != SONY_RESULT_OK)
      {
    ...
    result = sony_integ_Create(&dev_handle[gSonyIndex]->integ, &dev_handle[gSonyIndex]->demod, &createParam[gSonyIndex], &cxd2856_i2c, &dev_handle[gSonyIndex]->tunerTerrCable,
                               (enableSatTuner ? &dev_handle[gSonyIndex]->tunerSat : NULL), /* If the driver need not to control tuner, NULL can be used. */
  ...
  bInitialized_SONY_CXD2856[gSonyIndex] = 1;
...
int sony_cxd2856_detach(frontend_info_s *info)
{
  //mt_fe_sony_cxd2856_priv_handle p_priv[gSonyIndex] = NULL;
  //sony_example_driver_instance_t * dev_handle[gSonyIndex] = NULL;
	mt_fe_sony_cxd2856_priv_handle priv = NULL;
    sony_example_driver_handle sony_cxd2856_handle = NULL;
	if (NULL == info)
	{
	...
	priv = (mt_fe_sony_cxd2856_priv_handle)(info->handle);
	if (NULL == priv)
	{
		printk(KERN_ERR "[%s %d]error, p_priv is NULL\n", __FUNCTION__, __LINE__);
		return -EINVAL;
	}
    sony_cxd2856_handle = priv->sony_cxd2856_handle;
    if (NULL == sony_cxd2856_handle)
	{
    ...
    if ((info->is_attach) &&(gFeCount[gSonyIndex] == 0))
    {
        ...
        vfree(sony_cxd2856_handle);
        sony_cxd2856_handle = NULL;
        ...
        {
          kfree(sony_cxd2856_pg_channel_info[gSonyIndex]);
          sony_cxd2856_pg_channel_info = NULL;
        }
#endif

        info->is_attach = 0;
        bInitialized_SONY_CXD2856[gSonyIndex] = 0;
    }
...
//int sony_cxd2856_resume(frontend_info_s *info, mt_unf_fe_attr_t *attr)
//int sony_cxd2856_resume(frontend_info_s *info)
int sony_cxd2856_resume(void *handle)
{
  //uint8_t reg_val = 0;
  ...
  //mt_fe_sony_cxd2856_priv_handle priv_handle = (mt_fe_sony_cxd2856_priv_handle)info->handle;
  mt_fe_sony_cxd2856_priv_handle priv_handle = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = priv_handle->sony_cxd2856_handle;
  ...
  /*fix issue30833*/
  i2c_isr_onoff(g_i2c_sony_cxd2856, 0);
...
#if 0
    printk("%s[%d] -- dev_handle[%d]->integ = [0x%08x], dev_handle[%d]->demod = [0x%08x], createParam[%d] = [0x%08x], dev_handle[%d]->tunerTerrCable = [0x%08x]\n", 
            __FUNCTION__, __LINE__, 
            ...
            gSonyIndex, (unsigned int)&(dev_handle[gSonyIndex]->tunerTerrCable));
#endif
  ...
  ret = MT_SUCCESS;
err:
  i2c_isr_onoff(g_i2c_sony_cxd2856, 1);
  return ret;
}

//int sony_cxd2856_suspend(frontend_info_s *info)
int sony_cxd2856_suspend(void *handle)
{
  //mt_fe_sony_cxd2856_priv_handle priv_handle = (mt_fe_sony_cxd2856_priv_handle)info->handle;
  mt_fe_sony_cxd2856_priv_handle priv_handle = (mt_fe_sony_cxd2856_priv_handle)handle;
  sony_example_driver_handle sony_cxd2856_handle = priv_handle->sony_cxd2856_handle;
