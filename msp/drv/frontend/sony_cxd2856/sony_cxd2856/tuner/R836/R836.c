...
/*Clean warning redefined with R850.h R858.h
* so move the following define to R836.c
*/
#define VERSION "R836_v2.9J5"
#define VER_NUM 9
...
static int R836_Convert(int InvertNum)
{
	int ReturnNum = 0;
	int AddNum = 0x80;
	int BitNum = 0x01;
...
static int I2C_Read_Len(I2C_LEN_TYPE *I2C_Info)
{
	...
	{
		I2C_Info->Data[i] = R836_Convert(I2C_Info->Data[i]);
...
static int I2C_Write(I2C_TYPE *I2C_Info)
{
	...
	//printk("%s[%d] ---- [%02x] -- Write [%02x] - [%02x]\n", __FUNCTION__, __LINE__, I2C_Info->I2cAddr, I2C_Info->RegAddr, I2C_Info->Data);
...
UINT8 R840_XtalDiv = R840_XTAL_DIV2;
...
UINT8 R840_Xtal_Pwr = R840_XTAL_GM3V_STRONG;
UINT8 R840_Xtal_Pwr_tmp = R840_XTAL_GM3V_STRONG;
...
static R840_RF_Gain_TYPE R840_PreGainMode = R840_RF_AUTO; //for test use
...
SysFreq_Info_Type R840_SysFreq_Sel(R840_Standard_Type R840_Standard, UINT32 RF_freq)
{
	...
	case R840_ATSC_IF_5M:
		...
		R840_SysFreq_Info.LNA_DISCHARGE = 0x0A; // LNA disch=10                   (R5[4:0]=5'b01010)
		R840_SysFreq_Info.NRB_TOP_ADDER = 2;	// Nrb Adder: +6
												// R840_SysFreq_Info.RF_DISCHARGE=0x20;
												//RF disch = 1                        (R31[7:5]=3'b001)
		break;
...
R840_ErrCode R840_Init(void)
{
	...
	if (R840_Initial_done_flag == FALSE)
	{
		...
		R840_Xtal_Pwr = R840_XTAL_GM3V_STRONG;
#else
		if (R840_InitReg() != RT_Success)
			return RT_Fail;
		...
		if (R840_Xtal_Pwr_tmp == R840_XTAL_GM3V_STRONG)
			R840_Xtal_Pwr = R840_XTAL_GM3V_STRONG;
		else
...
R840_ErrCode R840_Xtal_Check(void)
{
	...
	for (i = 0; i < R840_XTAL_CHECK_SIZE; i++) //from weak to strong
	{
		if (i == R840_XTAL_GM3V_STRONG) //
		{
		...
		{
			R840_Array[15] = (R840_Array[15] & 0x9F) | ((UINT8)(R840_XTAL_NOR_HIGHEST - i) << 5);
...
R840_ErrCode R840_PLL(UINT32 LO_Freq, R840_Standard_Type R840_Standard)
{
	...
	if (R840_Initial_done_flag == TRUE)
	{
		...
		if (R840_Xtal_Pwr < R840_XTAL_NOR_HIGH)
		{
			XTAL_POW1 = 0x00; //high,       R16[4]=0
			XTAL_POW0 = 0x20; //high,       R15[6:5]=01
		}
		else if (R840_Xtal_Pwr <= R840_XTAL_NOR_HIGHEST)
		{
			XTAL_POW1 = 0x00; //high,       R16[4]=0
			XTAL_POW0 = ((UINT8)(R840_XTAL_NOR_HIGHEST - R840_Xtal_Pwr) << 5);
		}
		else
		{
			XTAL_POW1 = 0x10; //low,       R16[4]=1
	...
	if (R840_XtalDiv == R840_XTAL_DIV2)
		R840_Delay_MS(20);
	else
		R840_Delay_MS(10);
...
R840_ErrCode R840_SetTF(UINT32 u4FreqKHz, UINT8 u1TfType)
{
	...
	if ((u4FreqKHz > 0) && (u4FreqKHz < R840_LNA_LOW_LOWEST[R840_SetTfType])) //Ultra Low
	{
		...
		if (u4FreqKHz < R840_TF_Freq_Lowest[u1TfType][R840_TF_LOWEST_NUM - 1])
		{
			u1FreqCount = R840_TF_LOWEST_NUM;
		}
		else
		{
	...
	else if ((u4FreqKHz >= R840_LNA_LOW_LOWEST[R840_SetTfType]) && (u4FreqKHz < R840_LNA_MID_LOW[R840_SetTfType])) //Low
	{
		...
		if (u4FreqKHz < R840_TF_Freq_Low[u1TfType][R840_TF_LOW_NUM - 1])
		{
			u1FreqCount = R840_TF_LOW_NUM;
		}
		else
		{
	...
	else if ((u4FreqKHz >= R840_LNA_MID_LOW[R840_SetTfType]) && (u4FreqKHz < R840_LNA_HIGH_MID[R840_SetTfType])) //Mid
	{
		...
		if ((u4FreqKHz < R840_TF_Freq_Mid[u1TfType][R840_TF_MID_NUM - 1]))
		{
			u1FreqCount = R840_TF_MID_NUM;
		}
		else
		{
	...
	else //HIGH
	{
		...
		if (u4FreqKHz < R840_TF_Freq_High[u1TfType][R840_TF_HIGH_NUM - 1])
		{
			u1FreqCount = R840_TF_HIGH_NUM;
		}
		else
		{
...
R840_ErrCode R840_GPIO(R840_GPIO_Type R840_GPIO_Conrl)
{
	...
	//R15[0]
	if (R840_GPIO_Conrl == R840_HI_SIG)
...
R840_ErrCode R840_SetStandard(R840_Standard_Type RT_Standard)
{
	...
	// Record init flag
	R840_I2C.RegAddr = 0x1A;
	R840_Array[26] = ((R840_Array[26] & 0xF0) | (R840_Initial_done_flag << 1) | (R840_Fil_Cal_flag[RT_Standard] << 2) | R840_IMR_done_flag | (R840_DetectTfType << 3));
	R840_I2C.Data = R840_Array[26];
	if (I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;
...
R840_ErrCode R840_SetFrequency(R840_Set_Info R840_INFO)
{
	...
	//Set LT
	if (R840_INFO.R840_LT == R840_LT_ON)
	{
	...
	//Set Clk Out
	if (R840_INFO.R840_ClkOutMode == R840_CLK_OUT_OFF)
	{
	...
	if (R840_INFO.R840_Standard < R840_ATV_SIZE) //ATV
	{
		...
		if (R840_XtalDiv == R840_XTAL_DIV1)
		{
			//AGC CLK to 500hz
...
{
	UINT16 acc_lna_gain[25] = {
		0, 12, 23, 46, 66, 86, 103, 120, 136, 154,		  //0~9
...
R840_ErrCode R840_RfGainMode(R840_RF_Gain_TYPE R840_RfGainType)
{
	...
	if (R840_RfGainType == R840_RF_MANUAL)
	{
		if (R840_PreGainMode == R840_RF_AUTO)
		{
			R840_I2C_Len.RegAddr = 0x00;
			R840_I2C_Len.Len = 5;
			if (I2C_Read_Len(&R840_I2C_Len) != RT_Success)
...
R840_ErrCode R840_VgaCtrl(R840_Vga_Mode_TYPE R840_VgaModeType, UINT8 u1ManualCode)
{
	...
	if (R840_VgaModeType == R840_VGA_MANUAL)	
	{
		//VGA auto off
		R840_I2C.RegAddr = 0x0C;
...
R840_ErrCode R840_GetIfRssi(INT32 *VgaGain)
{
	...
	Dlg_I2C.I2cAddr = R840_I2C.I2cAddr;
	Dlg_I2C_Len.I2cAddr = R840_I2C_Len.I2cAddr;
...
R840_ErrCode R840_PulseGain(UINT8 FEC_Lock)
{
#if (R840_PULSE_GAIN == TRUE)
