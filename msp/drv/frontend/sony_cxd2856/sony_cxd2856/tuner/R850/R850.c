...
/*Clean warning redefined with R836.h R858.h
* so move the following define to R850.c
*/
#define VERSION   "R850_GUI_v1.4C"
#define VER_NUM   0
#define CHIP_ID       0x98

static int R850_Convert(int InvertNum)
{
	int ReturnNum = 0;
	int AddNum    = 0x80;
	int BitNum    = 0x01;
...
static bool I2C_Write_Len(R850_TUNER_NUM R850_Tuner_Num, I2C_LEN_TYPE *I2C_Info)
{
	...
	return (result == SONY_RESULT_OK) ? RT_Success : RT_Fail;
}

static bool I2C_Read_Len(R850_TUNER_NUM R850_Tuner_Num, I2C_LEN_TYPE *I2C_Info)
{
	//R850_ADDRESS = R850_ADDRESS_MULTI[R850_Tuner_Num]; //Device address
	...
	{
		I2C_Info->Data[i] = R850_Convert(I2C_Info->Data[i]);
...
static bool I2C_Write(R850_TUNER_NUM R850_Tuner_Num, I2C_TYPE *I2C_Info)
{
	//R850_ADDRESS = R850_ADDRESS_MULTI[R850_Tuner_Num]; //Device address
	sony_result_t result = SONY_RESULT_OK;
	uint8_t buf[2];
...
//==========Device address for single tuner============//
...
#endif
//===========Xtal Frequency===========//
UINT16 R850_Xtal = 24000;//16000;					 //Xtal clock rate.
...
//LPF Cal Array 72MHz
UINT8 R850_LPF_CAL_Array[3][R850_REG_NUM] = 
{   //24M  //16M  //27MHz
	{
...
UINT8 R850_IMR_CAL_Array[3][R850_REG_NUM] = 
{   //24M  //16M  //27MHz
	{
		0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,  //read only
...
static INT8 Lna_Acc_Gain_offset[86] = 
{
...
static INT8 Lna_Acc_Gain_offset[86] = 
{
	 -7,  -9,  -9, -10,  -9,  -9, -10, -11, -11, -12,	// 45~145
...
R850_Freq_Info_Type R850_Freq_Sel(UINT32 LO_freq, UINT32 RF_freq, R850_Standard_Type R850_Standard)
{
	R850_Freq_Info_Type R850_Freq_Info = {0};
...
{
	R850_SysFreq_Info_Type R850_SysFreq_Info = {0};
	...
	case R850_DVB_C_6M_IF_5M:
		...
		R850_SysFreq_Info.MIXER_VTH=0x0A;				//R41[3:0] 1.34 (A)
		R850_SysFreq_Info.MIXER_VTL=0x03;				//R43[3:0] 0.64 (4)
...
R850_ErrCode R850_IIC_CHECK(R850_TUNER_NUM R850_Tuner_Num)
{
	...
	for(write_cnt=8; write_cnt<R850_REG_NUM ; write_cnt++)
	{
		...
		{
			//This means "I2C_Read_Len" or "I2C_Write" function is not working properly.
			return RT_Fail;
		}
	}
	...
	for(write_cnt=8; write_cnt<R850_REG_NUM ; write_cnt++)
	{
		...
		{
			//This means "I2C_Read_Len" or "I2C_Write_Len" function is not working properly.
			return RT_Fail;
		}
	}
...
//Xtal cap
	...
	case 3:
		...
		case R850_TUNER_1: //address = 0xf8, other:0xb8,0x78,0x38
			oth_adrs[0]=R850_ADDRESS_MULTI[1];
#if 0//(R850_TOTAL_TUNER_NUMBER > 2)
	...
	case 4:
		...
		case R850_TUNER_1: //address = 0xf8, other:0xb8,0x78,0x38
			...
			oth_adrs[1]=R850_ADDRESS_MULTI[2];
#endif
#if 0//(R850_TOTAL_TUNER_NUMBER > 3)
...
R850_ErrCode R850_PLL(R850_TUNER_NUM R850_Tuner_Num, UINT32 LO_Freq, R850_Standard_Type R850_Standard)//ok
{
	...
	else if(R850_Xtal == 27000)
	{
		...
		u2XalDivJudge = (UINT16)((LO_Freq + R850_IF_GOLOBAL) / 500 / 27);
		//  48, 120 ,264 ,288, 336
		if((u2XalDivJudge==4)||(u2XalDivJudge==10)||(u2XalDivJudge==22)||(u2XalDivJudge==24)||(u2XalDivJudge==28))
		{
			CP_OFFSET = 0x02;  //30u,     R37[1]=1
...
R850_ErrCode R850_SetFrequency(R850_TUNER_NUM R850_Tuner_Num, R850_Set_Info R850_INFO) //Write Multi byte
{
	...
	switch(R850_TOTAL_TUNER_NUMBER)
	{
		...
		case 3:
			...
			case R850_TUNER_1: //address = 0xf8, other:0xb8,0x78,0x38
				oth_adrs[0]=R850_ADDRESS_MULTI[1];
#if 0//((R850_TOTAL_TUNER_NUMBER==3) || ((R850_TOTAL_TUNER_NUMBER==4)))
				oth_adrs[1]=R850_ADDRESS_MULTI[2];
#endif
				break;
			case R850_TUNER_2://address = 0xb8, other:0xf8,0x78,0x38
		...
		case 4:
			...
			case R850_TUNER_1: //address = 0xf8, other:0xb8,0x78,0x38
				...
				oth_adrs[1]=R850_ADDRESS_MULTI[2];
#endif
#if 0//(R850_TOTAL_TUNER_NUMBER==4)
				oth_adrs[2]=R850_ADDRESS_MULTI[3];
#endif
				break;
			case R850_TUNER_2://address = 0xb8, other:0xf8,0x78,0x38
...
R850_ErrCode R850_AGC_Slow(R850_TUNER_NUM R850_Tuner_Num, R850_Standard_Type R850_Standard)
{
	...
	//R850_Delay_MS(R850_Tuner_Num, 200);
...
//Method 2 : All frequencies are finely adjusted..
	...
	rf_total_gain = acc_lna_gain + acc_rfbuf_gain + acc_mixer_gain + rf_gain_info.RF_gain4 * 15 + rf_gain_info.RF_gain4 * 10 / 3;
...
//Optimize value
	...
	Dlg_I2C.I2cAddr = R850_I2C.I2cAddr;
	Dlg_I2C_Len.I2cAddr = R850_I2C_Len.I2cAddr;
...
R850_ErrCode R850_Dump_Data(R850_TUNER_NUM R850_Tuner_Num)
{
	...
	printk("IMG_flag = %d, Pulse_flag = %d\n", R850_IMGFlag, R850_PulseFlag);
