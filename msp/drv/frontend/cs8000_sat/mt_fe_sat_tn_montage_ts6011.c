...
/********************************************************************************************/
/* Filename:      mt_fe_sat_tn_m88ts6011.c
 *
 ...
 *  0.01.08     2018.03.29  Daniel          Modify
 *****************************************************************************/
...
MT_FE_TN_DEVICE_SETTINGS_TS6011 ts6011_config;
MT_FE_Tuner_Handle_TS6011 ts6011_handle = &ts6011_config;

MT_FE_CS8000_SAT_Device_Handle cs8k_sat_handle;


#if 1//(MT_FE_TN_SUPPORT_TS6011 > 0)


void _mt_fe_sat_tn_set_pll_freq_ts6011(MT_FE_Tuner_Handle_TS6011 handle)
{
	U32 fcry_KHz;
	U8  refDiv1;
	U8  refDiv2;
	...
	//  20160704 add by Kenny

	if(handle->tuner_crystal_KHz == 27000)
	{
		div1m = 19;
		div1p5m = 10;
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x41, 0x82);
	}
	else if(handle->tuner_crystal_KHz == 24000)
	{
		div1m = 16;
		div1p5m = 8;
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x41, 0x8a);
	}
	else
	{
		div1m = 19;
	...
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x27, reg27);
	/*_mt_fe_sat_tn_set_reg_ts6011(handle, 0x27, 0x87);*/
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x28, (U8)(ulNDiv1 & 0xFF));
	reg29 = (((ulNDiv2 >> 8) & 0x0F) + ucLomod2) & 0x7f;
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x29, reg29);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2a, (U8)(ulNDiv2 & 0xFF));
	refDiv1= refDiv1 & 0x1F;
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x36, refDiv1);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x39, refDiv2);

	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2f, 0xf5);
	/*_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2F, 0x75);*/
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x30, 0x05);

	if(refDiv1 == 19)
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2c, 0x02);
	}
	else
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2c, 0x00);
	}
...
void _mt_fe_sat_tn_set_bb_ts6011(MT_FE_Tuner_Handle_TS6011 handle)
{
	U32 f3dB;
	U8  reg40, tmp;
	...
	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x40, &tmp);
	tmp &= 0xC0;
	tmp |= (reg40 & 0x3F);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x40, tmp);
}

S32 mt_fe_sat_tn_ts6011_wakeup(MT_FE_Tuner_Handle_TS6011 handle)
{
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x07, 0x7d);
	_mt_cs8k_sat_sleep(10);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x6d, 0x1a);

	return 0;
}

S32 mt_fe_sat_tn_ts6011_sleep(MT_FE_Tuner_Handle_TS6011 handle)
{
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x07, 0x6d);
	_mt_cs8k_sat_sleep(1);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x6d, 0x18);

	return 0;
}

// fix bug 101382 start
/*************************************************************************/
...
/************************************************************************/
S32 mt_fe_sat_tn_ts6011_get_gain(MT_FE_Tuner_Handle_TS6011 handle, U32 Vagc)
{
	...
	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x5A, &reg5a);
	RF_GC = reg5a & 0x0f;
	if(RF_GC > 12)
	{
		RF_GC = 12;
	...
	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x5F, &reg5f);
	IF_GC = reg5f & 0x0f;
	if(IF_GC > 10)
	{
		IF_GC = 10;
	...
	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x70, &reg70);
	BB_GC = reg70 & 0x0f;
	if(BB_GC > 12)
	{
		BB_GC = 12;
	...
	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x72, &reg72);
	VGA_GC = reg72 & 0x03;
	if(VGA_GC > 2)
	{
		VGA_GC = 2;
	...
	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x3F, &reg3F);
	TIA_GC = (reg3F >> 4) &  0x07;
	if(TIA_GC > 5)
	{
		TIA_GC = 5;
...
void mt_fe_sat_tn_ts6011_init(MT_FE_Tuner_Handle_TS6011 handle)
{
	if(handle->tuner_init_OK == 0)
	{
		...
		handle->tuner_version			 = 108;
		handle->tuner_time				 = 18032918;
	...
	mt_fe_sat_tn_ts6011_wakeup(handle);

	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2B, 0x1e);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x2F, 0xf5);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x30, 0x05);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x82, 0x80);

	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x57, 0x2e);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x58, 0x02);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x46, 0x14);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x47, 0xec);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x4a, 0x0a);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x4b, 0xf6);

	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x91, 0x19);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x92, 0x45);

	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x6b, 0x10);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x77, 0x25);

	if(handle->tuner_input_mode == 1)
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x67, 0x2a);
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x68, 0x05);
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x6a, 0x06);
	}
	else
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x67, 0x08);
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x68, 0x15);
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x6a, 0x02);
	}


	if(handle->tuner_clock_out == 1)
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x0a, 0x00);
	}
	else
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x0a, 0x24);
	}
...
void mt_fe_sat_tn_set_tuner_ts6011(MT_FE_Tuner_Handle_TS6011 handle, U32 freq_MHz, U32 symbol_rate_KSs)
{
#if MT_FE_TS6011_DEBUG_PRNT
	...
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x5a, 0x0c);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x5b, 0x4c);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x5c, 0x54);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x5f, 0x01);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x60, 0x8b);
	// fix bug 101393 end

	_mt_fe_sat_tn_set_pll_freq_ts6011(handle);
	_mt_fe_sat_tn_set_bb_ts6011(handle);

	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x6d, 0x1a);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x82, 0x80);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x00, 0x01);
	_mt_fe_sat_tn_set_reg_ts6011(handle, 0x00, 0x00);

	return;
}

// fix bug 101393 start
void mt_fe_sat_tn_ts6011_adjust_AGC(MT_FE_Tuner_Handle_TS6011 handle)
{
	int i;
	U8 val, tmp;

	_mt_cs8k_sat_sleep(50);//fix bug 101416

	_mt_fe_sat_tn_get_reg_ts6011(handle, 0x5a, &val);

	for(i = 0; i < 5; i ++)
	{
		_mt_cs8k_sat_sleep(5);//fix bug 101416
		_mt_fe_sat_tn_get_reg_ts6011(handle, 0x5a, &tmp);
	...
	if(tmp != val)
	{
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x5b, 0xcc);
		_mt_fe_sat_tn_set_reg_ts6011(handle, 0x5c, 0xf4);
	}
...
S32 mt_fe_sat_tn_ts6011_set_freq(MT_FE_Tuner_Handle_TS6011 handle, U32 Freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)
{
	...
	mt_fe_sat_tn_set_tuner_ts6011(handle, freq_MHz, sym_rate_KSs);
...
/*************************************************************************/
/*  This is a demo function to calculate the signal strength use mt_fe_sat_tn_ts6011_get_gain*/
...
/************************************************************************/
S32 mt_fe_sat_tn_ts6011_get_signal_strength(MT_FE_Tuner_Handle_TS6011 handle, S32 Vagc, MT_BOOL bLocked)
{
	U8  strength = 0;
	S32 gain = 0;
	...
	gain = mt_fe_sat_tn_ts6011_get_gain(handle, Vagc);  //Get the total tuner gain in 0.01dB
	...
	//mt_fe_dmd_ds6103_get_pure_lock(handle, &lockstate);		// Get the lock status of the demodulator M88DS3002;
...
{
	cs8k_sat_handle = handle;
	handle->tuner_cfg.tuner_handle = (U32)ts6011_handle;

	mt_fe_sat_tn_ts6011_init(ts6011_handle);
...
MT_FE_RET mt_fe_sat_tn_sleep_ts6011(MT_FE_CS8000_SAT_Device_Handle handle)
{
	...
	mt_fe_sat_tn_ts6011_sleep(ts6011_handle);
...
MT_FE_RET mt_fe_sat_tn_wake_up_ts6011(MT_FE_CS8000_SAT_Device_Handle handle)
{
	...
	mt_fe_sat_tn_ts6011_wakeup(ts6011_handle);
...
MT_FE_RET mt_fe_sat_tn_get_strength_ts6011(MT_FE_CS8000_SAT_Device_Handle handle, U32 *p_gain, S32 *p_strength)
{
	...
	Vagc = AgcPWM * 33 + 630 - 1024;

	if(Vagc < 0)  Vagc = 0;


	gain = mt_fe_sat_tn_ts6011_get_gain(ts6011_handle, Vagc);
	...
	mt_fe_dmd_cs8k_sat_get_pure_lock(handle, &lockstate);
...
MT_FE_RET mt_fe_sat_tn_set_freq_ts6011(MT_FE_CS8000_SAT_Device_Handle handle, U32 freq_KHz, U32 sym_rate_KSs, S16 lpf_offset_KHz)
{
	...
	mt_fe_sat_tn_ts6011_set_freq(ts6011_handle, freq_KHz, sym_rate_KSs, lpf_offset_KHz);
...
MT_FE_RET mt_fe_sat_tn_get_tuner_freq_offset_ts6011(MT_FE_CS8000_SAT_Device_Handle handle, S32 *p_freq_offset_KHz)
{
	...
	return mt_fe_sat_tn_ts6011_get_freq_offset(ts6011_handle);
}

// fix bug 101382 start
MT_FE_RET mt_fe_sat_tn_get_gain_ts6011(MT_FE_CS8000_SAT_Device_Handle handle, U32 *p_gain)
{
	S32 Vagc;
	U8  AgcPWM;
	...
	*p_gain = mt_fe_sat_tn_ts6011_get_gain(ts6011_handle, Vagc);
...
MT_FE_RET mt_fe_sat_tn_get_version_ts6011(MT_FE_CS8000_SAT_Device_Handle handle, U32 *p_version_number, U32 *p_version_time)
{
	...
	*p_version_number	 = ts6011_handle->tuner_version;
	*p_version_time		 = ts6011_handle->tuner_time;
...
MT_FE_RET mt_fe_sat_tn_adjust_agc_ts6011(MT_FE_CS8000_SAT_Device_Handle handle)
{
	...
	mt_fe_sat_tn_ts6011_adjust_AGC(ts6011_handle);
