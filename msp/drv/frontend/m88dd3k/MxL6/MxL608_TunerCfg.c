...
MXL_STATUS MxLWare608_OEM_ReadRegister_dd3k(UINT8 devId, UINT8 regAddr, UINT8 *regDataPtr);
MXL_STATUS MxLWare608_OEM_WriteRegister_dd3k(UINT8 devId, UINT8 regAddr, UINT8 regData);

MXL608_REG_CTRL_INFO_T MxL608_OverwriteDefaults_dd3k[] = 
{
  {0x14, 0xFF, 0x13},
  {0x6D, 0xFF, 0x8A},
  {0x6D, 0xFF, 0x0A},
...
MXL608_REG_CTRL_INFO_T MxL608_DigitalDvbc_dd3k[] = 
{
  {0x0C, 0xFF, 0x00},
  {0x13, 0xFF, 0x04},
  {0x53, 0xFF, 0x7E},
...
MXL608_REG_CTRL_INFO_T MxL608_DigitalIsdbtAtsc_dd3k[] = 
{
  {0x0C, 0xFF, 0x00},
  {0x13, 0xFF, 0x04},
  {0x53, 0xFF, 0xFE},
...
MXL608_REG_CTRL_INFO_T MxL608_DigitalDvbt_dd3k[] = 
{
  {0x0C, 0xFF, 0x00},
  {0x13, 0xFF, 0x04},
  {0x53, 0xFF, 0xFE},
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxL608_Ctrl_ProgramRegisters_dd3k
--|
--| AUTHOR        : Dong Liu
--|
...
MXL_STATUS MxL608_Ctrl_ProgramRegisters_dd3k(UINT8 devId, PMXL608_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  ...
  while (status == MXL_TRUE)
  {
    ...
    {
      status = MxLWare608_OEM_ReadRegister_dd3k(devId,ctrlRegInfoPtr[i].regAddr, &tmp);
    ...
    status = MxLWare608_OEM_WriteRegister_dd3k(devId,ctrlRegInfoPtr[i].regAddr, tmp);
    if (status != MXL_TRUE) break;
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxL608_Ctrl_WriteRegField_dd3k
--|
--| AUTHOR        : Dong Liu
--|
...
MXL_STATUS MxL608_Ctrl_WriteRegField_dd3k(UINT8 devId, PMXL608_REG_CTRL_INFO_T ctrlRegInfoPtr)
{
  MXL_STATUS status = MXL_TRUE;
  UINT8 tmp = 0;
  ...
  {
      status = MxLWare608_OEM_ReadRegister_dd3k(devId, ctrlRegInfoPtr->regAddr, &tmp);
  ...
  status = MxLWare608_OEM_WriteRegister_dd3k(devId, ctrlRegInfoPtr->regAddr, tmp);

  return status;
}

/*------------------------------------------------------------------------------
--| FUNCTION NAME : Ctrl_SetRfFreqLutTblReg_MXL608_dd3k
--|
--| AUTHOR        : Dong Liu
--|
...
MXL_STATUS Ctrl_SetRfFreqLutTblReg_MXL608_dd3k(UINT8 devId, UINT32 FreqInHz, PMXL608_CHAN_DEPENDENT_FREQ_TABLE_T freqLutPtr)
{
  ...
  // Program registers
  for (idx = 0; idx < MxL608_SPUR_REGISTER_dd3k.SpurRegNum; idx++)
    status |= MxLWare608_OEM_WriteRegister_dd3k(devId, MxL608_SPUR_REGISTER_dd3k.SpurRegAddr[idx], regSetData[idx]);
