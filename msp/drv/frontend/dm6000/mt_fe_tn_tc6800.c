...
static S32 _mt_fe_tn_write_fw_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr, U8 *reg_data, U8 data_len)
{
	...
	mt_fe_tn_write_dm6k(handle->tuner_dev_addr, p_buf, (U8)(data_len + 1));
...
static S32 mt_fe_tn_download_fw_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	...
	U8	data0 = 0, data1 = 0;
	S32	ret = 0;
	const U8* p_fw = m88tc6800_fm_dm6k;
	firm_size = sizeof(m88tc6800_fm_dm6k) /64;
	...
	if ((handle->tuner_mcu_status != 1) || (data0 != 0))
	{
		...
		ret = _mt_fe_tn_write_fw_tc6800(handle, 0xc0, (U8*)&(p_fw[64 * i]), (sizeof(m88tc6800_fm_dm6k) % 64));
...
static void mt_fe_tn_get_diagnose_info_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32* data1, U32* data2)
{
	#if TC6800_ENABLE_PRINT
...
{
	TC6800_tuner_cfg.tuner_dev_addr = handle->m_device_ctt2.tuner_cfg.tuner_dev_addr;

	if(handle->m_device_ctt2.demod_type == MtFeType_DVBC)
	{
		TC6800_tuner_cfg.tuner_init_OK = handle->m_device_ctt2.tuner_cfg.tuner_init_ok;
	...
	else
	{
		...
		{
			TC6800_tuner_cfg.tuner_application = 4;
			TC6800_tuner_cfg.tuner_dac_gain = 1;
		}
		else if(handle->m_device_ctt2.demod_type == MtFeType_DVBT2)
		{
			TC6800_tuner_cfg.tuner_application = 3;
			TC6800_tuner_cfg.tuner_dac_gain = 2;
		}
		...
		if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_8M)
			...
			TC6800_tuner_cfg.tuner_bandwidth = 10000;
		TC6800_tuner_cfg.tuner_dac = 5000;
	}

	mt_fe_tn_init_tc6800(&TC6800_tuner_cfg);

	handle->m_device_ctt2.tuner_cfg.tuner_init_ok = TC6800_tuner_cfg.tuner_init_OK;
	handle->m_device_ctt2.tuner_cfg.tuner_open = TC6800_tuner_cfg.tuner_init_OK;
...
MT_FE_RET mt_fe_tn_set_freq_tc6800_tc(MT_FE_DM6K_Device_Handle  handle, U32 Freq_KHz, U32 sym_rate, S16 lpf_offset)
{
	...
	mt_fe_tn_set_freq_tc6800(&TC6800_tuner_cfg, Freq_KHz);
...
MT_FE_RET mt_fe_tn_wake_up_tc6800_tc(MT_FE_DM6K_Device_Handle  handle)
{
	...
	mt_fe_tn_set_freq_tc6800_tc(handle, handle->m_device_ctt2.input_params.input_freq_kHz, 0, 0);
	return MtFeErr_Ok;
}

MT_FE_RET mt_fe_tn_xtal_tc6800_tc(MT_FE_DM6K_Device_Handle  handle,U32 xtal_khz)
{
