...
*/
  ...
  if ((mt_fe_tn_write_dm6k(MXL603_Address,buf,2)) != MtFeErr_Ok)
     {
     	status = MXL_FALSE;
     }
  else
...
*/
  ...
  /* If OEM data is not required, customer should treat devId as I2C slave Address */
...
#if 1
	if ((mt_fe_tn_write_dm6k(MXL603_Address, tmp, 2)) != MtFeErr_Ok)
	{
		status = MXL_FALSE;
	}
	else
	...
	if ((mt_fe_tn_read_dm6k(MXL603_Address, tmp, 0, DataPtr, 1)) != MtFeErr_Ok)
	{
		status = MXL_FALSE;
	}
	else
...
#else
	if ((mt_fe_tn_read_dm6k(MXL603_Address, tmp, 2, DataPtr, 1)) != MtFeErr_Ok)
	{
		status = MXL_FALSE;
	}
	else
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  switch(powerMode)
  {
    ...
    case MXL603_PWR_MODE_STANDBY:
      ...
      {
        status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare603_OEM_WriteRegister(devId, DFE_SEQ_TUNE_RF1_BO_REG, (UINT8)(standbyLt & 0x3F));
        status |= MxLWare603_OEM_WriteRegister(devId, PAGE_CHANGE_REG, 0x00);
      }
      break;
...
*/
  ...
  if (chanTuneCfg.startTune == MXL_ENABLE)
  {
    if (chanTuneCfg.signalMode <= MXL603_DIG_J83B)
    {
      ...
      if (freqLutPtr)
        status |= Ctrl_SetRfFreqLutTblReg_dm6k(devId, chanTuneCfg.freqInHz, freqLutPtr);
...
MT_FE_RET mt_fe_tn_Init_MxL603(MT_FE_DM6K_Device_Handle  handle)
{
	...
	*/
	  ...
	  MXL603_Address = handle->m_device_ctt2.tuner_cfg.tuner_dev_addr;
	  ...
	  if((handle->m_device_ctt2.demod_type==MtFeType_DVBT)||(handle->m_device_ctt2.demod_type==MtFeType_DVBT2))
	  {
...
MT_FE_RET mt_fe_tn_sleep_MxL603(MT_FE_DM6K_Device_Handle  handle)
{
	...
	mt_fe_i2c_repeat_enable_dm6k(handle);
	status = MxLWare603_API_CfgDevPowerMode(devId,MXL603_PWR_MODE_STANDBY,MXL_DISABLE,0);
	mt_fe_i2c_repeat_disable_dm6k(handle);
...
MT_FE_RET mt_fe_tn_wake_up_MxL603(MT_FE_DM6K_Device_Handle  handle)
{
	...
	mt_fe_i2c_repeat_enable_dm6k(handle);
	status = MxLWare603_API_CfgDevPowerMode(devId,MXL603_PWR_MODE_ACTIVE,MXL_DISABLE,0);
	mt_fe_i2c_repeat_disable_dm6k(handle);
