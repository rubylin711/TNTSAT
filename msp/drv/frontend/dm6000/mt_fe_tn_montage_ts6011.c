...
void mt_fe_tn_ts6011_init(MT_FE_Tuner_Handle_TS6011 handle)
{
	if(handle->tuner_init_OK == 0)
	{
		...
		handle->tuner_clock_out			 = 0;     // "0" CLK-output disable,"1" CLK-output enable.
	}
...
MT_FE_RET mt_fe_tn_get_gain_ts6011(MT_FE_DM6K_Device_Handle handle, U32 *p_gain)
{
	...
	_mt_fe_dmd_get_reg_ss2(demod_handle, 0x3f, &AgcPWM);
...
MT_FE_RET mt_fe_tn_get_strength_ts6011(MT_FE_DM6K_Device_Handle handle, S8 *p_strength)
{
	...
	mt_fe_dmd_get_pure_lock_dm6k_ss2(demod_handle, &lockstate);
