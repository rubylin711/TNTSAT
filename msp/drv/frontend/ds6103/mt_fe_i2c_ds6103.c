...
static MT_FE_RET _mt_fe_i2c_write(U8 dev_addr, U8 *w_buf, U16 w_byte)
{
	...
	i2c = i2c_get_adapter(g_i2c_ds6103);
	msg->addr = dev_addr;
...
void _mt_delayus_ds6103(U32 us)
{
	usleep_range(us / 100 * 100 + 50, us / 100 * 100 + 100);
}

MT_FE_RET _mt_fe_write32_ds6103(U32 reg_addr, U32 reg_data)
{
...
MT_FE_RET _mt_fe_read32_ds6103(U32 reg_addr, U32 *p_data)
{
...
**
*****************************************************************/
MT_FE_RET _mt_fe_dmd_set_reg_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 data)
{
	/*
		TODO:
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:  _mt_fe_dmd_set_reg_ds6103() dev_addr = 0x%02x, reg 0x%02x - data 0x%02x FAILED!\n", handle->demod_dev_addr, reg_index, data);
		return MtFeErr_I2cErr;
	}
...
**
*****************************************************************/
MT_FE_RET _mt_fe_dmd_get_reg_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 *p_buf)
{
	/*
		TODO:
			Obtain the i2c mutex
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:	_mt_fe_dmd_get_reg_ds6103() dev_addr = 0x%02x, reg 0x%02x FAILED!\n", handle->demod_dev_addr, reg_index);
		return MtFeErr_I2cErr;
	}
...
**
*****************************************************************/
MT_FE_RET _mt_fe_it_set_reg_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 data)
{
	U8 tmp;
	MT_FE_RET ret;
	U8 buf[2];
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:  _mt_fe_it_set_reg_ds6103() dev_addr = 0x%02x, reg 0x%02x - data 0x%02x FAILED!\n", handle->it_dev_addr, reg_index, data);
		return MtFeErr_I2cErr;
	}
...
**
*****************************************************************/
MT_FE_RET _mt_fe_it_get_reg_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 *p_buf)
{
	U8 tmp;
	MT_FE_RET ret;
	U8 temp[2] = {0, 0};
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:	_mt_fe_it_get_reg_ds6103() dev_addr = 0x%02x, reg 0x%02x FAILED!\n", handle->it_dev_addr, reg_index);
		return MtFeErr_I2cErr;
	}
...
**
*****************************************************************/
MT_FE_RET _mt_fe_tn_set_reg_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 data)
{
	...
	*/
	...
	/*Do not care to close the I2C repeater, it will close by itself*/
	val = 0x11;
	ret = _mt_fe_dmd_set_reg_ds6103(handle, 0x03, val);
	if (ret != MtFeErr_Ok)
		return ret;
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:  _mt_fe_tn_set_reg_ds6103() dev_addr = 0x%02x, reg 0x%02x - data 0x%02x FAILED!\n", handle->tuner_cfg.tuner_dev_addr, reg_index, data);
		return MtFeErr_I2cErr;
	}
...
**
*****************************************************************/
MT_FE_RET _mt_fe_tn_get_reg_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 *p_buf)
{
	MT_FE_RET ret;
	U8 val;
	U8 temp[2] = {0, 0};
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:	_mt_fe_tn_get_reg_ds6103() dev_addr = 0x%02x, reg 0x%02x FAILED!\n", handle->tuner_cfg.tuner_dev_addr, reg_index);
		return MtFeErr_I2cErr;
	}
...
MT_FE_RET _mt_fe_tn_read_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 *p_buf, U16 n_byte)
{
	U8	val, ret;
...
MT_FE_RET _mt_fe_tn_write_ds6103(MT_FE_DS6103_Device_Handle handle, U8 *p_buf, U16 n_byte)
{
	U8	val, ret;
...
**
*****************************************************************/
MT_FE_RET _mt_fe_write_fw_ds6103(MT_FE_DS6103_Device_Handle handle, U8 reg_index, U8 *p_buf, U16 n_byte)
{
	...
	*/
	...
	if (ret != MtFeErr_Ok)
	{
		printk("MT:  _mt_fe_write_fw_ds6103() dev_addr = 0x%02x, reg 0x%02x FAILED!\n", handle->demod_dev_addr, reg_index);
		return MtFeErr_I2cErr;
	}
...
S32 _mt_fe_tn_get_reg_ts6011(MT_FE_Tuner_Handle_TS6011 handle, U8 reg_addr, U8 *reg_data)
{
	MT_FE_RET ret = MtFeErr_Ok;

	ds6103_handle->tn_get_reg(ds6103_handle, reg_addr, reg_data);

	return (ret == MtFeErr_Ok) ? 0 : -1;
}

S32 _mt_fe_tn_set_reg_ts6011(MT_FE_Tuner_Handle_TS6011 handle, U8 reg_addr, U8 reg_data)
{
	MT_FE_RET ret = MtFeErr_Ok;

	ds6103_handle->tn_set_reg(ds6103_handle, reg_addr, reg_data);

	return (ret == MtFeErr_Ok) ? 0 : -1;
}

S32 _mt_fe_tn_set_reg_bit_ts6011(MT_FE_Tuner_Handle_TS6011 handle, U8 reg_addr, U8 data, U8 high_bit, U8 low_bit)
{
	U8 tmp = 0, value = 0;
