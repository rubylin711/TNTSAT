...
#include "mt_fe_fw_cab_tn_tc6800.h"
...
void _mt_fe_tn_set_RF_front_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 freq_KHz)
{
	...
	if (freq_KHz <= 602000) {
	    buf2 = 0x55;
	    //_mt_fe_tn_set_reg_tc6800(handle, 0x8b, 0x55);
...
void _mt_fe_tn_set_LO_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 freq_KHz, U8 mixer_sel)
{
	...
	buf0 = buf0 + 48;
    } else if (handle->tuner_int_fvco_tg < 7500000) {
	buf0 = buf0 + 32;
    } else if (handle->tuner_int_fvco_tg < 8000000) {
	buf0 = buf0 + 16;
    } else {
	buf0 = buf0 + 0;
    }
...
void _mt_fe_tn_set_PLL_freq_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 Delta_sign, U8 Num_step)
{
	...
	if (((Fvco_in_KHz > (harmK * handle->tuner_crystal + 10)) && (Fvco_in_KHz < (harmK * handle->tuner_crystal + 1010))) || ((Fvco_in_KHz < (harmK * handle->tuner_crystal - 10)) && (Fvco_in_KHz > (harmK * handle->tuner_crystal - 1010)))) {
	    pll_ref = 2 * handle->tuner_crystal / 3;
	    Icp_scale_en = 2;
	} else if (((Fvco_in_KHz > (harmK * handle->tuner_crystal + 1020)) && (Fvco_in_KHz < (harmK * handle->tuner_crystal + 2020))) || ((Fvco_in_KHz < (harmK * handle->tuner_crystal - 1020)) && (Fvco_in_KHz > (harmK * handle->tuner_crystal - 2020)))) {
	    pll_ref = 2 * handle->tuner_crystal / 3;
	    Icp_scale_en = 1;
	} else {
	    pll_ref = 2 * handle->tuner_crystal / 1;
	...
	if ((((FDIV_N & 0x03) == 0x01) && (FDIV_F == 0x01)) ||
	    (((FDIV_N & 0x03) == 0x03) && (FDIV_F == 0x0f))) {
	    pll_sdm_en = 1;
	    FDIV_FIN = 0x03;
	}
    }
    ...
    handle->tuner_Fvco_val = Fvco_KHz / 1000; //output,MHz
...
void _mt_fe_tn_set_DAC_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 freq_KHz)
{
    ...
    f1f2number = ((DACFreq_KHz * 1000 * 512) / FreqTrue108_Hz) * 128 + (((DACFreq_KHz * 1000 * 512) % FreqTrue108_Hz) * 32 / FreqTrue108_Hz) * 4;
	...
	f2d = handle->tuner_bandwidth / 2 - 0;
    } else {
	f2d = handle->tuner_bandwidth / 2 - 100;
    }
...
void _mt_fe_tn_poweron_set_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
    ...
    MT_INFO_FRONTEND("TC6800 power on.\n");
    //#endif

    //if(handle->tuner_application == 0)
    //{
    //	_mt_fe_tn_set_reg_bit_tc6800(handle, 0x0f, 0x02, 4, 2);	// Will degrade DVB-T2 ACI Performance, will be removed in B2
    //}
    ...
    if (buf != 0x18) {
	//_mt_fe_tn_set_reg_tc6800(handle, 0x15, 0xf3);		//will removed for B2
	...
	_mt_fe_tn_set_reg_expand_tc6800(handle, 0x39, 0x1a, 0x06);
	_mt_fe_tn_set_reg_expand_tc6800(handle, 0x39, 0x1b, 0x0b);
	...
	_mt_fe_tn_set_reg_bit_tc6800(handle, 0x35, 0x0d, 7, 4);
    } else {
	_mt_fe_tn_set_reg_tc6800(handle, 0x05, 0x04);
    ...
    if (handle->tuner_application == 3) {
	_mt_fe_tn_set_reg_bit_tc6800(handle, 0x37, 0x03, 2, 1);
    } else {
	if (handle->tuner_gpio_out == 0) {
	    _mt_fe_tn_set_reg_bit_tc6800(handle, 0x37, 0x00, 2, 0);
	}
    }
    ...
    _mt_fe_tn_set_reg_tc6800(handle, 0xb8, 0x01);
    _mt_fe_tn_set_reg_tc6800(handle, 0xbe, 0x62);
    _mt_fe_tn_set_reg_tc6800(handle, 0xb8, 0x03); //	0x01
    _mt_fe_tn_set_reg_tc6800(handle, 0xbe, 0x62); //	0xa2
    _mt_fe_tn_set_reg_tc6800(handle, 0xb8, 0x05); //	0x03
    _mt_fe_tn_set_reg_tc6800(handle, 0xbe, 0x62); //	0xa2
    ...
    _mt_fe_tn_set_reg_tc6800(handle, 0x14, 0x4b);
	...
	_mt_fe_tn_get_reg_tc6800(handle, 0x50, &data3);
              ...
              ((data3 != 0xc6) && (data3 != 0xc7) && (data3 != 0xc8))) &&
             (i < 4));
...
void _mt_fe_tn_set_mch_app_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	...
	if (freq_ch >= 954000) {
	    ...
	    idxh = 0;
	    idxl = 0;
	} else if (freq_ch >= 858000) {
	    freq_h = 954000;
	    freq_l = 858000;
	    idxh = 0;
	    idxl = 1;
	} else if (freq_ch >= 810000) {
	    freq_h = 858000;
	    freq_l = 810000;
	    idxh = 1;
	    idxl = 2;
	} else if (freq_ch >= 738000) {
	    freq_h = 810000;
	    freq_l = 738000;
	    idxh = 2;
	    idxl = 3;
	} else if (freq_ch >= 658000) {
	    freq_h = 738000;
	    freq_l = 658000;
	    idxh = 3;
	    idxl = 4;
	} else if (freq_ch >= 586000) {
	    freq_h = 658000;
	    freq_l = 586000;
	    idxh = 4;
	    idxl = 5;
	} else if (freq_ch >= 538000) {
	    freq_h = 586000;
	    freq_l = 538000;
	    idxh = 5;
	    idxl = 6;
	} else if (freq_ch >= 498000) {
	    freq_h = 538000;
	    freq_l = 498000;
	    idxh = 6;
	    idxl = 7;
	} else if (freq_ch >= 474000) {
	    freq_h = 498000;
	    freq_l = 474000;
	    idxh = 7;
	    idxl = 8;
	} else if (freq_ch >= 443000) {
	    freq_h = 474000;
	    freq_l = 443000;
	    idxh = 8;
	    idxl = 9;
	} else if (freq_ch >= 419000) {
	    freq_h = 443000;
	    freq_l = 419000;
	    idxh = 9;
	    idxl = 10;
	} else if (freq_ch >= 395000) {
	    freq_h = 419000;
	    freq_l = 395000;
	    idxh = 10;
	    idxl = 11;
	} else if (freq_ch >= 371000) {
	    freq_h = 395000;
	    freq_l = 371000;
	    idxh = 11;
	    idxl = 12;
	} else if (freq_ch >= 355000) {
	    freq_h = 371000;
	    freq_l = 355000;
	    idxh = 12;
	    idxl = 13;
	} else if (freq_ch >= 339000) {
	    freq_h = 355000;
	    freq_l = 339000;
	    idxh = 13;
	    idxl = 14;
	} else if (freq_ch > 334000) {
	    freq_h = 339000;
	    freq_l = 334000;
	    idxh = 14;
	    idxl = 14;
	} else if (freq_ch >= 243000) {
	    freq_h = 334000;
	    freq_l = 243000;
	    idxh = 15;
	    idxl = 16;
	} else if (freq_ch >= 227000) {
	    freq_h = 243000;
	    freq_l = 227000;
	    idxh = 16;
	    idxl = 17;
	} else if (freq_ch >= 211000) {
	    freq_h = 227000;
	    freq_l = 211000;
	    idxh = 17;
	    idxl = 18;
	} else if (freq_ch >= 195000) {
	    freq_h = 211000;
	    freq_l = 195000;
	    idxh = 18;
	    idxl = 19;
	} else if (freq_ch >= 179000) {
	    freq_h = 195000;
	    freq_l = 179000;
	    idxh = 19;
	    idxl = 20;
	} else if (freq_ch >= 163000) {
	    freq_h = 179000;
	    freq_l = 163000;
	    idxh = 20;
	    idxl = 21;
	} else if (freq_ch >= 147000) {
	    freq_h = 163000;
	    freq_l = 147000;
	    idxh = 21;
	    idxl = 22;
	} else if (freq_ch >= 131000) {
	    freq_h = 147000;
	    freq_l = 131000;
	    idxh = 22;
	    idxl = 23;
	} else if (freq_ch > 112000) {
	    freq_h = 131000;
	    freq_l = 115000;
	    idxh = 23;
	    idxl = 24;
	} else if (freq_ch >= 88000) {
	    freq_h = 112000;
	    freq_l = 88000;
	    idxh = 25;
	    idxl = 25;
	} else if (freq_ch >= 80000) {
	    freq_h = 88000;
	    freq_l = 80000;
	    idxh = 25;
	    idxl = 26;
	} else if (freq_ch >= 68500) {
	    freq_h = 80000;
	    freq_l = 68500;
	    idxh = 26;
	    idxl = 27;
	} else if (freq_ch >= 60500) {
	    freq_h = 68500;
	    freq_l = 60500;
	    idxh = 27;
	    idxl = 28;
	} else if (freq_ch >= 52500) {
	    freq_h = 60500;
	    freq_l = 52500;
	    idxh = 28;
	    idxl = 29;
...
S32 mt_fe_tn_download_fw_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	...
	if (data0 == data1) {
	    ...
	    MT_INFO_FRONTEND("TC6800: Download firmware 0x%02x OK!\n", data1);
	    //#endif
	} else {
...
void mt_fe_tn_init_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	...
	MT_INFO_FRONTEND("TC6800 initialize.\n");
	//#endif
	handle->tuner_mcu_status = 0;
	...
	handle->tuner_version = 112;
	handle->tuner_fw_version = 0x0b;
	handle->tuner_time = 16061415;
...
void mt_fe_tn_set_freq_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 freq_KHz)
{
    ...
    MT_INFO_FRONTEND("TC6800 set freq, tuner_app is %d\n", handle->tuner_application);
    //#endif
    ...
    MT_INFO_FRONTEND("TC6800 set finished.\n");
    //#endif
...
S32 mt_fe_tn_get_signal_strength_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	...
	if ((freq_KHz <= 474000) && (freq_KHz > 400000))
...
{
    _mt_fe_tn_set_reg_bit_tc6800(handle, 0x37, 0x01, 2, 0);
    return;
}
...
void mt_fe_tn_get_diagnose_info_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 *data1, U32 *data2)
{
    ...
    MT_INFO_FRONTEND("TC6800 -- cg: %d, ci: %d, if: %d\n", reg_data0, reg_data1, reg_data4);
    ...
    MT_INFO_FRONTEND("TC6800 -- tia: %d, bb: %d, vga: %d, dac: %d, fr: %d\n", reg_data0, reg_data1, reg_data2, reg_data3, reg_data4);
    ...
    MT_INFO_FRONTEND("TC6800 -- 0x01: 0x%x, 0xc5: 0x%x, 0xc6: 0x%x, 0xc8: 0x%x\n", reg_data0, reg_data1, reg_data2, reg_data3);
    ...
    MT_INFO_FRONTEND("TC6800 -- 0x6e: 0x%x, 0x6f: 0x%x, 0x70: 0x%x, 0x71: 0x%x, 0x72: 0x%x\n", reg_data0, reg_data1, reg_data2, reg_data3, reg_data4);
    ...
    MT_INFO_FRONTEND("TC6800 -- 0x73: 0x%x, 0x74: 0x%x, 0x75: 0x%x\n", reg_data0, reg_data1, reg_data2);
...
S32 mt_fe_tn_self_check_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
    ...
    if (ret != 0) {
#if TC6800_ENABLE_PRINT
	MT_INFO_FRONTEND(("\tTC6800: I2C failed! Chip addr = 0x%02X\n", handle->tuner_dev_addr));
#endif
	return -1;
    }

#if TC6800_ENABLE_PRINT
    MT_INFO_FRONTEND(("\tTC6800: chip_id[0x01] = 0x%02X, %s\n", tmp, ((tmp == 0xB1) || (tmp == 0xB2)) ? "OK" : "Fail"));
#endif
...
void mt_fe_tn_set_crystal_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 crystal_KHz)
{
    handle->tuner_crystal = crystal_KHz;
}
