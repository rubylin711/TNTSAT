...
S32 mt_fe_tn_self_check_MxL608(MT_FE_CS8000_CAB_Device_Handle dev_handle)
{
	...
	ret = MxLWare608_OEM_ReadRegister((UINT8)(dev_handle->tuner_settings.tuner_dev_addr >> 1), 0x18, &tmp);
	if(ret != 0)
	{
		mt_fe_print(("\tMxL608: I2C failed! Chip addr = 0x%02X\n", dev_handle->tuner_settings.tuner_dev_addr));
		return -1;
	}

	mt_fe_print(("\tMxL608: chip_id[0x18] = 0x%02X, %s\n", tmp, (tmp == 0x02) ? "OK" : "Fail"));
...
{
	U8 devId = MXL608_I2C_ADDR;
	S16 iRxPower = 0;

	MxLWare608_API_ReqTunerRxPower(devId, &iRxPower);

	*p_strength = iRxPower / 100;
