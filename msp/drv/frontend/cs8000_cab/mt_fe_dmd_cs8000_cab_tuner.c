...
MT_FE_CS8000_CAB_Device_Handle demod_handle;
...
MT_FE_RET mt_fe_cs8k_cab_tn_init_tc2800(void *dev_handle)
{
    ...
    handle->tuner_settings.tuner_handle = (U32)kmalloc(sizeof(MT_FE_TN_TC2800_SETTINGS), GFP_KERNEL);
...
MT_FE_RET mt_fe_cs8k_cab_tn_get_strength_tc2800(void *dev_handle, U32 *p_gain, U32 *p_strength)
{
    ...
    *p_strength = (U32)(mt_fe_tn_get_signal_strength_tc2800(handle) + 107); // dBm + 107 ->dBuV
...
extern MT_FE_RET mt_fe_tn_get_signal_strength_MxL203(void *dev_handle, U32 *p_gain, U32 *p_strength);
extern S32 mt_fe_tn_self_check_MxL203(MT_FE_CS8000_CAB_Device_Handle dev_handle);
...
MT_FE_RET mt_fe_cs8k_cab_tn_init_tc3800(void *dev_handle)
{
    ...
    handle->tuner_settings.tuner_handle = (U32)kmalloc(sizeof(MT_FE_TN_DEVICE_SETTINGS_TC3800), GFP_KERNEL);
...
MT_FE_RET mt_fe_cs8k_cab_tn_get_strength_tc3800(void *dev_handle, U32 *p_gain, U32 *p_strength)
{
    ...
    *p_strength = (U32)(mt_fe_tn_get_signal_strength_tc3800(tuner_handle) + 107); // dBm + 107 ->dBuV
...
MT_FE_CS8000_CAB_SUPPORTED_TUNER mt_fe_cs8k_cab_check_tuner_type(void *dev_handle)
{
	...
	|| (handle->tuner_settings.tuner_type == TN_MONTAGE_TC6800))
	{
		if(handle->tn_get_reg != NULL)
		{
			...
			else if((Reg01H == 0xB1) || (Reg01H == 0xB2))
			{
				tuner_type = TN_MONTAGE_TC6800;
			}
			else
			{
				return tuner_type;
    ...
    //U8 ucTunerAddr = handle->tuner_settings.tuner_dev_addr;

    if ((handle->tuner_settings.tuner_type != TN_MONTAGE_TC3800) && (handle->tuner_settings.tuner_type != TN_MONTAGE_TC6800)) {
	return tuner_type; //MtFeErr_Ok;
    }
	...
	case 1:
	    handle->tuner_settings.tuner_dev_addr = 0xc4;
	    break;
	...
	case 3:
	default:
	    handle->tuner_settings.tuner_dev_addr = 0xc8;
	    break;
	}
	...
	if (i < 2) {
	    if ((reg_0x01 == 0xB4) || (reg_0x01 == 0xB5)) {
		//handle->tuner_cfg.tuner_type = TN_MONTAGE_TC3800;
		tuner_type = TN_MONTAGE_TC3800;
		break;
	    }
	} else {
	    if ((reg_0x01 == 0xB1) || (reg_0x01 == 0xB2)) {
		//handle->tuner_cfg.tuner_type = TN_MONTAGE_TC6800;
		tuner_type = TN_MONTAGE_TC6800;
		break;
	    }
	}
...
MT_FE_RET mt_fe_cs8k_cab_tn_init_tc6800(void *dev_handle)
{
    ...
    //handle->tuner_settings.tuner_handle = (U32)kmalloc(sizeof(MT_FE_TN_DEVICE_SETTINGS_TC6800), GFP_KERNEL);
	handle->tuner_settings.tuner_handle = (ulong)kmalloc(sizeof(MT_FE_TN_DEVICE_SETTINGS_TC6800), GFP_KERNEL);
...
MT_FE_RET mt_fe_cs8k_cab_tn_set_crystal_tc6800(void *dev_handle, U32 crystal_KHz)
{
    ...
    mt_fe_tn_set_crystal_tc6800(tuner_handle, crystal_KHz);

    return MtFeErr_Ok;
}

MT_FE_RET mt_fe_dc2800_tn_get_diagnose_info_tc6800(void *dev_handle)
{
    MT_FE_CS8000_CAB_Device_Handle handle = (MT_FE_CS8000_CAB_Device_Handle)dev_handle;
    MT_FE_Tuner_Handle_TC6800 tuner_handle = (MT_FE_Tuner_Handle_TC6800)(handle->tuner_settings.tuner_handle);
    U32 data1 = 0, data2 = 0;

    mt_fe_tn_get_diagnose_info_tc6800(tuner_handle, &data1, &data2);
