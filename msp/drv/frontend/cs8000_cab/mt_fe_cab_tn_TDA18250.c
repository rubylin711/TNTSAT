...
MT_FE_CS8000_CAB_Device_Handle tda_handle = NULL;
...
//	UInt32 uIFMaster = 0;
	...
	tda_handle = (MT_FE_CS8000_CAB_Device_Handle)dev_handle;
...
MT_FE_RET mt_fe_tn_get_signal_strength_TDA18250(void *dev_handle, U32 *p_gain, U32 *p_strength)
{
	...
	//tmddTDA18250GetRSSI(0, &iStrength);

	*p_gain = 0;
	*p_strength = iStrength;
...
MT_FE_RET mt_fe_tn_get_signal_strength_TDA18250A(void *dev_handle, U32 *p_gain, U32 *p_strength)
{
	...
	tmbslTDA18250A_GetPowerLevel(0, (Int32 *)&iLevel);
	tmbslTDA18250A_GetRSSI(0, &iStrength);

	*p_gain = (iLevel + 50) / 100 + 32;
	*p_strength = iStrength / 2;

	return MtFeErr_Ok;
}

S32 mt_fe_tn_self_check_TDA18250(void *dev_handle)
{
	U8 tmp1 = 0, tmp2 = 0, addr1 = 0x00, addr2 = 0x01;
	S32 ret = 0;
	...
	ret = UserWrittenI2CRead(0, 1, &addr1, 1, &tmp1);
	if(ret != 0)
	{
		mt_fe_print(("\tTDA18250 1: I2C failed! Chip addr = 0x%02X\n", tda_handle->tuner_settings.tuner_dev_addr));
		return -1;
	}

	ret = UserWrittenI2CRead(0, 1, &addr2, 1, &tmp2);
	if(ret != 0)
	{
		mt_fe_print(("\tTDA18250 2: I2C failed! Chip addr = 0x%02X\n", tda_handle->tuner_settings.tuner_dev_addr));
		return -1;
	}

	iChipId = ((tmp1 & 0x7F) << 8) + tmp2;

	mt_fe_print(("\tTDA18250: chip_id[0x00] = 0x%04X, %s\n", iChipId, (iChipId == 18250) ? "OK" : "Fail"));

	return (iChipId == 18250) ? 1 : 0;
}
