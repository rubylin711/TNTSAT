...
void _mt_fe_tn_set_RF_front_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
...
void _mt_fe_tn_cali_PLL_tc2800(MT_FE_CS8000_CAB_Device_Handle handle, U32 freq_KHz, U32 cali_freq_thres_div2, U32 cali_freq_thres_div3r, U32 cali_freq_thres_div3)
{
	S32	N, F, MUL;
	U8 buf, tmp, tmp2;
	...
	if (tuner_handle->tuner_mtt == 0xD1)
	{
		_mt_cs8k_cab_sleep(5);
	}
	else if (tuner_handle->tuner_mtt == 0xE1)
	{
		_mt_cs8k_cab_sleep(2);
	}
	else
	{
		return;
...
void _mt_fe_tn_set_PLL_freq_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	U8 buf, buf1;
...
void _mt_fe_tn_set_BB_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	MT_FE_TN_TC2800_Handle tuner_handle = (MT_FE_TN_TC2800_Handle)(handle->tuner_settings.tuner_handle);
...
void _mt_fe_tn_set_DAC_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
...
void _mt_fe_tn_preset_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	MT_FE_TN_TC2800_Handle tuner_handle = (MT_FE_TN_TC2800_Handle)(handle->tuner_settings.tuner_handle);
...
void _mt_fe_tn_set_appendix_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	U8 buf;
...
void mt_fe_tn_wakeup_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	U8 buf;
...
void mt_fe_tn_sleep_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	U8 buf;
...
void mt_fe_tn_init_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	MT_FE_TN_TC2800_Handle tuner_handle = (MT_FE_TN_TC2800_Handle)(handle->tuner_settings.tuner_handle);
	...
	if (tuner_handle->tuner_init_OK != 1)
	{
		...
		tuner_handle->tuner_mode			 = 0;		// 0: DVB-C, 1: CTTB, 2: MMDS

		tuner_handle->tuner_bandwidth		 = 8;

		tuner_handle->tuner_loopthrough		 = 0;		// 0:LP off, 1:LP on only in wakeup mode, 2:LP on both in wakeup and sleep modes
...
void mt_fe_tn_set_freq_tc2800(MT_FE_CS8000_CAB_Device_Handle handle, U32 freq_KHz)
{
	U8 buf;
	U8 buf1;
	U8 buf2;
...
void mt_fe_tn_set_BB_filter_band_tc2800(MT_FE_CS8000_CAB_Device_Handle handle, U8 bandwidth)
{
	U8 buf, tmp;
...
int mt_fe_tn_get_signal_strength_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	int level = -107;
...
void mt_fe_tn_get_diagnose_info(MT_FE_CS8000_CAB_Device_Handle handle, U32* data1, U32* data2)
{
	U8	tmp = 0;
...
S32 mt_fe_tn_self_check_tc2800(MT_FE_CS8000_CAB_Device_Handle handle)
{
	U8 tmp = 0;
	S32 ret = 0;

	ret = _mt_fe_tn_get_reg(handle, 0x01, &tmp);
	if(ret != 0)
	{
		mt_fe_print(("\tTC2800: I2C failed! Chip addr = 0x%02X\n", handle->tuner_settings.tuner_dev_addr));
		return -1;
	}

	mt_fe_print(("\tTC2800: chip_id[0x01] = 0x%02X, %s\n", tmp, ((tmp == 0x0D) || (tmp == 0x8E)) ? "OK" : "Fail"));

	return ((tmp == 0x0D) || (tmp == 0x8E)) ? 1 : 0;
}
