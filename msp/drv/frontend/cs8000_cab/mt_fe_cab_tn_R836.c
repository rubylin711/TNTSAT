...
MT_FE_CS8000_CAB_Device_Handle R836_handle = NULL;
...
int mt_fe_cab_r836_I2C_Write_Len(I2C_LEN_TYPE *I2C_Info)
{
...
int mt_fe_cab_r836_I2C_Read_Len(I2C_LEN_TYPE *I2C_Info)
{
	//implement your I2C code here//
	/*
	int DataCunt = 0;
...
int mt_fe_cab_r836_I2C_Write(I2C_TYPE *I2C_Info)
{
	//implement your I2C code here//
	UINT8 p_buf[2];
...
Sys_Info_Type R840_Sys_Sel(R840_Standard_Type R840_Standard)
{
	...
	case R840_DVB_C_8M:
		...
		*/
		R840_Sys_Info.IF_KHz=7200;
		R840_Sys_Info.BW=0x00;                           //BW=8M
		R840_Sys_Info.FILT_CAL_IF=11500;           //CAL IF
		R840_Sys_Info.HPF_COR=0x01;	             //R11[3:0]=01
...
R840_ErrCode R840_InitReg(void)
{
	...
	if(mt_fe_cab_r836_I2C_Write_Len(&R840_I2C_Len) != RT_Success)
		return RT_Fail;
...
R840_ErrCode R840_TF_Check(void)
{
	...
	R840_I2C_Len.RegAddr = 0x00;
	R840_I2C_Len.Len = 4;
	if(mt_fe_cab_r836_I2C_Read_Len(&R840_I2C_Len) != RT_Success)
	{
	    mt_fe_cab_r836_I2C_Read_Len(&R840_I2C_Len);
	}
	...
	else
	{
	 ...
	 if(mt_fe_cab_r836_I2C_Write(&R840_I2C) != RT_Success)
		return RT_Fail;
	 ...
	 else //DTV
	 {
         ...
         switch(R840_INFO.R840_Standard)
		 {
			...
			case R840_DVB_T2_1_7M_IF_5M:
				...
				//filter +3dB off
				R840_I2C.RegAddr = 0x1E;
				R840_Array[30] = (R840_Array[30] & 0xF7);   //R30[3]=0
				R840_I2C.Data = R840_Array[30];
				if(mt_fe_cab_r836_I2C_Write(&R840_I2C) != RT_Success)
					return RT_Fail;
...
R840_ErrCode R840_GetIfRssi(INT32 *VgaGain)
{
	...
	if(mt_fe_cab_r836_I2C_Write(&Dlg_I2C) != RT_Success)
		return RT_Fail;
	...
	Dlg_I2C_Len.Len = 2;
	if(mt_fe_cab_r836_I2C_Read_Len(&Dlg_I2C_Len) != RT_Success) // read data length
	{
		mt_fe_cab_r836_I2C_Read_Len(&Dlg_I2C_Len);
	}
...
{
	R836_handle = (MT_FE_CS8000_CAB_Device_Handle)dev_handle;
...
MT_FE_RET mt_fe_tn_set_freq_R836(void *dev_handle, U32 freq_KHz, U16 symbol_rate_KSs)
{
	...
	R840_Info.R840_Standard = R840_DVB_C_8M;
...
MT_FE_RET mt_fe_tn_get_signal_strength_R836(void *dev_handle, U32 *p_gain, U32 *p_strength)
{
	...
	R840_GetTotalRssi(R836_handle->channel_param.freq_KHz, R840_DVB_C_8M, &R836_RSSI);

	*p_gain = 0;
	*p_strength = R836_RSSI;

	return MtFeErr_Ok;
}

S32 mt_fe_tn_self_check_R836(MT_FE_CS8000_CAB_Device_Handle dev_handle)
{
	int ret = 0;
	...
	ret = mt_fe_cab_r836_I2C_Read_Len(&R840_I2C_Len);
	if(ret != RT_Success)
	{
		ret = mt_fe_cab_r836_I2C_Read_Len(&R840_I2C_Len);
	}
	...
	if(ret != RT_Success)
	{
		mt_fe_print(("\tR836: I2C failed! Chip addr = 0x%02X\n", dev_handle->tuner_settings.tuner_dev_addr));
		return -1;
	}

	mt_fe_print(("\tR836: chip_id[0x00] = 0x%02X, %s\n", R840_I2C_Len.Data[0], (R840_I2C_Len.Data[0] == 0x96) ? "OK" : "Fail"));

	return (R840_I2C_Len.Data[0] == 0x96) ? 1 : 0;
}
