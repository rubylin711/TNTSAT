...
MxL_ERR_MSG MxL_RFSynth_Lock_Status(MxL203RF_TunerConfigS* myTuner, MT_BOOL* isLock)
{	
	UINT8 Data;
	*isLock = FALSE; 
...
MxL_ERR_MSG MxL_REFSynth_Lock_Status(MxL203RF_TunerConfigS* myTuner, MT_BOOL* isLock)
{
	UINT8 Data;
	*isLock = FALSE; 
	if(MxL_I2C_Read((UINT8)myTuner->I2C_Addr, 0x14, &Data))
...
MxL_ERR_MSG MxL_Check_RF_Input_Power(MxL203RF_TunerConfigS* myTuner, UINT32* RF_Input_Level)
{	
	UINT32 temp;
	...
	temp = (UINT32)(whole) + (UINT32)(dec*125/1000);
