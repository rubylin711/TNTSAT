...
void mt_fe_tn_init_tc3800(MT_FE_Tuner_Handle_TC3800 handle)
{
	if (handle->tuner_init_OK != 1)
	{
		...
		handle->tuner_version			 = 126;
		handle->tuner_time				 = 16030816;
...
S32 mt_fe_tn_self_check_tc3800(MT_FE_Tuner_Handle_TC3800 handle)
{
	U8 tmp = 0;
	S32 ret = 0;

	ret = _mt_fe_tn_get_reg_tc3800(handle, 0x01, &tmp);
	if(ret != 0)
	{
	#if TC3800_ENABLE_PRINT
		tc3800_dbg_print(("\tTC3800: I2C failed! Chip addr = 0x%02X\n", handle->tuner_dev_addr));
	#endif
		return -1;
	}

#if TC3800_ENABLE_PRINT
	tc3800_dbg_print(("\tTC3800: chip_id[0x01] = 0x%02X, %s\n", tmp, ((tmp == 0xB4) || (tmp == 0xB5)) ? "OK" : "Fail"));
#endif

	return ((tmp == 0xB4) || (tmp == 0xB5)) ? 1 : 0;
}
