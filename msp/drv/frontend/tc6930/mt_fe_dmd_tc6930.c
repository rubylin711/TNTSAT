 ...
 *  Modify          0.04.05     2025.06.09  Ziv
 *  Modify          0.04.06     2025.07.08  Ziv
 *****************************************************************************/
...
MT_FE_RET mt_fe_dmd_tc6930_reset_all(MT_FE_TC6930_Device_Handle handle)
{
	...
	_mt_fe_tn_set_reg_16bit_tc6930_bits(handle,0x4000, 0x01, 0, 0);    //reset all i2c register

	_mt_fe_tn_set_reg_16bit_tc6930_bits(handle,0x4747, 0x00, 14, 14);  //reset fw flag
	_mt_fe_tn_set_reg_16bit_tc6930_bits(handle,0x4747, 0x00, 15, 15);  //reset poweron set flag
...
MT_FE_RET mt_fe_dmd_tc6930_check_4_reset_all(MT_FE_TC6930_Device_Handle handle, U8 demod_index)
{
	...
	// Check global reset
	_mt_fe_tc6930_get_reg_32bit(handle,(0x3F008 - 0x20000) / 4, &ulData);
	if(ulData != 0xFFFFFFFF)
	{
		_mt_fe_tc6930_set_reg_32bit(handle,(0x3F008 - 0x20000) / 4, 0x00000000);
		_mt_fe_tc6930_set_reg_32bit(handle,(0x3F008 - 0x20000) / 4, 0xFFFFFFFF);
	...
	_mt_fe_tc6930_get_reg_32bit(handle,(0x3F03C - 0x20000) / 4, &ulData);
	...
	_mt_fe_tc6930_get_reg_32bit(handle,0x4608, &ulData);	// Firmware version number
	if((ulData & 0xFF) == 0)
	{
		mt_fe_dmd_tc6930_reset_all(handle);
...
{
	U8 tmp1, tmp2, tmp3, i;/* data; Clean Warning*/

	//printk("%s[%d] -- demod_index = %d, demod_type = %d, max %d times\n", __FUNCTION__, __LINE__, demod_index, handle->demod_device[demod_index].demod_type, iMaxTimes);

	if((handle->demod_device[demod_index].dev_index != 2) ||
	   ((handle->demod_device[demod_index].demod_type != MT_FE_DEMOD_CS8000_CAB) &&
		(handle->demod_device[demod_index].demod_type != MT_FE_DEMOD_CT8000) && 
		(handle->demod_device[demod_index].demod_type != MT_FE_DEMOD_CS8800)))
	{
		return MtFeErr_Ok;
	}

	//printk("%s[%d] -- demod_index = %d, demod_type = %d\n", __FUNCTION__, __LINE__, demod_index, handle->demod_device[demod_index].demod_type);

	if (handle->demod_device[demod_index].demod_type == MT_FE_DEMOD_CS8800)
	{
		_mt_fe_sym4_sar_get_reg(0x32, &tmp1);
		tmp1 |= 0x53;//53//5f
		_mt_fe_sym4_sar_set_reg(0x32, tmp1); //set clear lms_lock log

		_mt_fe_sym4_sar_get_reg(0x33, &tmp1);
		tmp1 |= 0x43;//42-div2//43-div4//4b-div8
		_mt_fe_sym4_sar_set_reg(0x33, tmp1); //enable ext control & clock div4

		_mt_fe_sym4_sar_get_reg(0x34, &tmp1);
		tmp1 |= 0x40;//43//40
		_mt_fe_sym4_sar_set_reg(0x34, tmp1); //enable ext control

		_mt_fe_sym4_sar_set_reg(0x40, 0x71); //enable ext control

		_mt_fe_sym4_sar_get_reg(0x31, &tmp1);
		tmp1 |= 0x0F;
		_mt_fe_sym4_sar_set_reg(0x31, tmp1); //enable calibration & ext control

		handle->mt_sleep(2); //wait 2ms

		for(i = 0; i < iMaxTimes; i ++)
		{
			_mt_fe_sym4_sar_get_reg(0x30, &tmp2);
			tmp2 |= 0x08;
			_mt_fe_sym4_sar_set_reg(0x30, tmp2); //reset calibration

			handle->mt_sleep(5);//wait 5ms

			_mt_fe_sym4_sar_get_reg(0x32, &tmp1);
			tmp1 &= 0x3F;
			_mt_fe_sym4_sar_set_reg(0x32, tmp1);//release clear lms_lock log

			tmp2 &= 0xF0;
			_mt_fe_sym4_sar_set_reg(0x30, tmp2);//release calibration

			handle->mt_sleep(10); //wait 10ms

			_mt_fe_sym4_sar_get_reg(0x39, &tmp3);  //judge if calibration is success
		...
		{
			mt_fe_print(("\tSym4 C&B calibration OK!\n"));
			printk("%s[%d] -- Sym4 calibration OK! Result = 0x%02x\n", __FUNCTION__, __LINE__, tmp3);
		}
		else
		{
			mt_fe_print(("\tSym4 C&B calibration failed!\n"));
			printk("%s[%d] -- Sym4 calibration failed! Result = 0x%02x\n", __FUNCTION__, __LINE__, tmp3);
		}

		_mt_fe_sym4_sar_get_reg(0x31, &tmp1);
		tmp1 &= 0x00;
		_mt_fe_sym4_sar_set_reg(0x31, tmp1); //disable calibration
	}
	...
	if (handle->demod_device[demod_index].on_board_settings.bReadCali)
	{
		if (handle->demod_device[demod_index].demod_type == MT_FE_DEMOD_CS8800)
		{
			...
			for(i = 0x39; i <= 0x3F; i ++)
			{
				_mt_fe_sym4_sar_get_reg(i, &tmp1);
			}

			_mt_fe_sym4_sar_get_reg(0x36, &tmp2);
			tmp2 |= 0x02;
			_mt_fe_sym4_sar_set_reg(0x36, tmp2);

			for(i = 0; i < 28; i ++)
			{
				_mt_fe_sym4_sar_get_reg(0x35, &tmp1);
...
MT_FE_RET mt_fe_dmd_tc6930_init(MT_FE_TC6930_Device_Handle handle, U8 demod_index)
{
	...
	if (handle->demod_device[demod_index].dev_index == 2)
	{
		if(handle->demod_device[demod_index].demod_type == MT_FE_DEMOD_CS8800)
		{
			_mt_fe_sym4_sar_get_reg(0x03, &data);
			data |= 0x80;
			_mt_fe_sym4_sar_set_reg(0x03, data);
			handle->mt_sleep(1);
			data &= 0x7F;
			_mt_fe_sym4_sar_set_reg(0x03, data);
			...
			_mt_fe_sym4_sar_set_reg(0x04, 0x00);
			_mt_fe_sym4_sar_set_reg(0x05, 0x00);
			_mt_fe_sym4_sar_set_reg(0x06, 0x00);
			_mt_fe_sym4_sar_set_reg(0x07, 0x00);
			_mt_fe_sym4_sar_set_reg(0x08, 0x00);


			_mt_fe_sym4_sar_get_reg(0x04, &tmp1);
			_mt_fe_sym4_sar_set_reg(0x04, 0x05);
			...
			_mt_fe_sym4_sar_get_reg(0x0A, &data);
			data &= 0xFB;		// 2V
			//if(handle->m_device_c_b.board_cfg.iVppSelC == 1)		// 1V
			if (handle->demod_device[demod_index].on_board_settings.iVppSel == 1)	// 1V
			{
				data |= 0x04;
			}
			_mt_fe_sym4_sar_set_reg(0x0A, data);  //Set 1V or 2V Vdpp
		}
	...
	if(handle->demod_device[demod_index].demod_type == MT_FE_DEMOD_CS8800)
	{
		...
		_mt_fe_sym4_sar_set_reg(0x04, tmp1);
	
		handle->Get32Bits(0xBF5D0094, &reg);
		reg &= 0xFFFFFFF0;
		//ulTmp |= 0x00000006;
		handle->Set32Bits(0xBF5D0094, reg);		//+++ ANA: ANA_TOP_SW_REG1

		handle->Get32Bits(0xBF138020, &reg);
		reg |= 0x01;	// 0xbf138020[0] = 1
		handle->Set32Bits(0xBF138020, reg);

		handle->Get32Bits(0xBF138008, &reg);
		reg &= ~0x200;	// 0xbf138008[9] = 0
		handle->Set32Bits(0xBF138008, reg);

		handle->Get32Bits(0xBF13C048, &reg);
		reg &= 0xFFFFDFF0;
		reg |= 0x04;
		handle->Set32Bits(0xBF13C048, reg);

		handle->Get32Bits(0xBF5D0094, &reg);
		reg &= 0xFFFFFFF7;
		// BF5D0094[0]=1  //IQ PADÉèÖÃÎªdigital input
		reg |= 0x01;			// 20210104 for TS1
		handle->Set32Bits(0xBF5D0094, reg);	//+++ ANA: ANA_TOP_SW_REG1

		handle->Get32Bits(0xBF5D00D0, &reg);
		//BF5D00D0H[28 : 27] = _00
		//BF5D00D0H[22] = 0
		reg &= 0xE7BFFFFF;
		handle->Set32Bits(0xBF5D00D0, reg);		//+++ ANA: CADC_REG0
		...
		_mt_fe_sym4_sar_set_reg(0x04, 0x01);
		_mt_fe_sym4_sar_set_reg(0x07, 0x08);
		_mt_fe_sym4_sar_set_reg(0x06, 0x00);
		...
		reg |= (1 << 31);
		handle->Set32Bits(0xBF157004, reg);			//+++ ANA: ANA_AO_REG1
	}
	...
	if (demod_index == 0)
	{
		...
		_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F030 - 0x20000) / 4, 0x01, 18, 16);
		_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F02C - 0x20000) / 4, 0x01, 10,  8);
		//_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F030 - 0x20000) / 4, 0x01, 26, 24);

		_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F02C - 0x20000) / 4, 0x01, 26, 24);
		_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F02C - 0x20000) / 4, 0x01, 18, 16);

		_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F03C - 0x20000) / 4, 0x01,  6,  6);		// TS 0, pinmux
	}
	else if(demod_index == 1)
	{
		...
		{	// not TC6920 and RC6800
			...
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F028 - 0x20000) / 4, 0x01, 10,  8);		// For Symphony: 0, others: 1
#else
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F028 - 0x20000) / 4, 0x00, 10,  8);		// For Symphony: 0, others: 1
#endif
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F030 - 0x20000) / 4, 0x01,  2,  0);
			//_mt_fe_tc6930_set_reg_32bit_bits((0x3F028 - 0x20000) / 4, 0x01, 26, 24);

			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F028 - 0x20000) / 4, 0x01, 18, 16);
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F028 - 0x20000) / 4, 0x01,  2,  0);

			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F03C - 0x20000) / 4, 0x01,  7,  7);		// TS 1, pinmux
		}
	}
...
MT_FE_RET mt_fe_dmd_tc6930_auto_QAM_debug(MT_FE_TC6930_Device_Handle handle, U8 demod_index, U8 auto_mode, U16 *auto_qam, U32 *auto_symbol, S32 *auto_offset)
{
	...
	switch (auto_mode)
	{
		case 1:	handle->dmd_set_reg(handle, demod_index, 0x0c, 0x80); break;
		case 2:	handle->dmd_set_reg(handle, demod_index, 0x0c, 0xc0); break;
		case 3:	handle->dmd_set_reg(handle, demod_index, 0x0c, 0xe0); break;
		default:
				handle->dmd_set_reg(handle, demod_index, 0x0c, 0x80); break;
	}
...
MT_FE_RET mt_fe_dmd_tc6930_connect(MT_FE_TC6930_Device_Handle handle, U8 demod_index, U32 freq_KHz, U16 symbol_rate_KSs, U16 qam, U8 inverted)
{
	...
	if(handle->demod_device[demod_index].demod_type == MT_FE_DEMOD_CS8800)
	{
		_mt_fe_sym4_sar_get_reg(0x07, &tmp1);
		tmp1 |= 0x40;
		_mt_fe_sym4_sar_set_reg(0x07, tmp1);
		handle->mt_sleep(1);
		tmp1 &= 0xBF;
		_mt_fe_sym4_sar_set_reg(0x07, tmp1);
	}
...
#if (MT_PLATFORM_SEL == 0)
	{
		if ((demod_index == 0) && (handle->demod_device[demod_index].tuner_settings.tuner_custom_cfg == 1))
		{
			...
			{
				_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F030 - 0x20000) / 4, 0x00, 18, 16);
				_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F02C - 0x20000) / 4, 0x00, 10,  8);
			}
			else
			{
				_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F030 - 0x20000) / 4, 0x01, 18, 16);
	...
	if(handle->demod_device[demod_index].dev_index < 2)
	{
		_mt_fe_dsp_get_reg_tc6930(handle,demod_index, 0xF5, &tmp1);
		...
		_mt_fe_dsp_set_reg_tc6930(handle,demod_index, 0xF5, tmp1);
	}
	...
	else
	{
		...
		if(demod_index == 0)
		{
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F008 - 0x20000) / 4, 0x00, 27, 27);
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F008 - 0x20000) / 4, 0x01, 27, 27);
		}
		else if(demod_index == 1)
		{
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F008 - 0x20000) / 4, 0x00, 28, 28);
			_mt_fe_tc6930_set_reg_32bit_bits(handle,(0x3F008 - 0x20000) / 4, 0x01, 28, 28);
		}
	}
	...
	if(bAutoMode)
	{
		...
		if(handle->demod_device[demod_index].dev_index == 2)
		{
			if(handle->demod_device[demod_index].demod_type == MT_FE_DEMOD_CS8800)
			{
				handle->dmd_set_reg(handle, demod_index, 0x16, 0x00);//0x13 : 0xEC);
				handle->dmd_set_reg(handle, demod_index, 0x17, 0x00);//0x8E : 0x72);
			}
		}
...
************************************************/
MT_FE_RET mt_fe_dmd_tc6930_get_per(MT_FE_TC6930_Device_Handle handle, U8 demod_index, U32 *error_packages, U32 *total_packages)
{
	MT_FE_RET ret = MtFeErr_Ok;
	...
	handle->dmd_get_reg(handle, demod_index, 0xD1, &regD1H);
	if (regD1H <= 0x07)
	{
		*error_packages = 0;
		*total_packages = 65535;
	...
	handle->dmd_get_reg(handle, demod_index, 0xDF, &regDFH);
	regDFH |= 0x02;
	handle->dmd_set_reg(handle, demod_index, 0xDF, regDFH);

	handle->dmd_get_reg(handle, demod_index, 0xD1, &regD1H);
	handle->dmd_get_reg(handle, demod_index, 0xD0, &regD0H);
	handle->dmd_get_reg(handle, demod_index, 0xD5, &regD5H);
	handle->dmd_get_reg(handle, demod_index, 0xD4, &regD4H);
...
MT_FE_RET mt_fe_demod_tc6930_get_temperature(MT_FE_TC6930_Device_Handle handle, S16 *temp_degree)
{
	...
	_mt_fe_tn_get_reg_16bit_tc6930(handle,0x4008, &uiVal);
	uiVal &= ~(1 << 5);
	_mt_fe_tn_set_reg_16bit_tc6930(handle,0x4008, uiVal);

	_mt_fe_tn_get_reg_16bit_tc6930(handle,0x400d, &uiVal);
	uiVal &= ~(1 << 5);
	_mt_fe_tn_set_reg_16bit_tc6930(handle,0x400d, uiVal);

	_mt_fe_tn_get_reg_16bit_tc6930(handle,0x4012, &uiVal);
	uiVal &= ~(1 << 5);
	_mt_fe_tn_set_reg_16bit_tc6930(handle,0x4012, uiVal);

	_mt_fe_tn_get_reg_16bit_tc6930(handle,0x42c4, &uiVal);
	uiVal >>= 4;
	uiVal &= 0x0F;
