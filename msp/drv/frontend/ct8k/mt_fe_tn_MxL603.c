...
static U8 MXL603_Address_ct8k = 0;
/* MxLWare Driver version for MxL603 */
const UINT8 MxLWare603DrvVersion_ct8k[5] = {2, 1, 3, 2, 0};

/* OEM Data pointer array */
static void * MxL603_OEM_DataPtr[MXL603_MAX_NUM_DEVICES];

#define MXL603_XTAL_FREQ_SEL_CT8K MXL603_XTAL_16MHz
/*----------------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_OEM_WriteRegister_ct8k
--| 
--| AUTHOR        : Brenndon Lee
--| 
...
MXL_STATUS MxLWare603_OEM_WriteRegister_ct8k(UINT8 devId, UINT8 RegAddr, UINT8 RegData)
{
...
*/
  ...
  /* If OEM data is not required, customer should treat devId as I2C slave Address */
	...
	if ((mt_fe_tn_write_ct8k(MXL603_Address_ct8k, buf, 2)) != MtFeErr_Ok)
	{
		status = MXL_FALSE;
	}
	else
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_OEM_ReadRegister_ct8k
--| 
--| AUTHOR        : Brenndon Lee
--| 
--| DATE CREATED  : 7/30/2009
...
MXL_STATUS MxLWare603_OEM_ReadRegister_ct8k(UINT8 devId, UINT8 RegAddr, UINT8 *DataPtr)
{
...
*/
  ...
  /* If OEM data is not required, customer should treat devId as I2C slave Address */
	...
	tmp[0] = 0xfb;
	tmp[1] = RegAddr;
	if ((mt_fe_tn_read_ct8k(MXL603_Address_ct8k, tmp, 2, DataPtr, 1)) != MtFeErr_Ok)
	{
		status = MXL_FALSE;
	}
	else
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_OEM_Sleep_ct8k
--| 
--| AUTHOR        : Dong Liu
--| 
...
static void MxLWare603_OEM_Sleep_ct8k(UINT16 DelayTimeInMs)
{
	// OEM should implement sleep operation
	_mt_sleep_ct8k(DelayTimeInMs);
}

/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgDrvInit_ct8k
--| 
...
MXL_STATUS MxLWare603_API_CfgDrvInit_ct8k(UINT8 devId, void* oemDataPtr)
{
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgDevSoftReset_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_CfgDevSoftReset_ct8k(UINT8 devId)
{
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  status = MxLWare603_OEM_WriteRegister_ct8k(devId, AIC_RESET_REG, 0x00);

  return (MXL_STATUS)status;
}

/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgDevOverwriteDefaults_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_CfgDevOverwriteDefaults_ct8k(UINT8 devId, 
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  status |= MxL603_Ctrl_ProgramRegisters_ct8k(devId, MxL603_OverwriteDefaults_ct8k);

  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x00, 0x01);
  MxLWare603_OEM_ReadRegister_ct8k(devId, 0x00, &readData);

  status |= MxLWare603_OEM_ReadRegister_ct8k(devId, 0x31, &readData);

  readData &= 0x2F;
  readData |= 0xD0;
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x31, readData);
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x00, 0x00);


  /* If Single supply 3.3v is used */
  if (MXL_ENABLE == singleSupply_3_3V)
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, MAIN_REG_AMP, 0x04);

  return (MXL_STATUS)status;
}

/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgDevXtal_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--|                 Dong Liu
--| 
...
MXL_STATUS MxLWare603_API_CfgDevXtal_ct8k(UINT8 devId, MXL603_XTAL_SET_CFG_T xtalCfg)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 control = 0;
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  if ((xtalCfg.xtalFreqSel == MXL603_XTAL_16MHz) || (xtalCfg.xtalFreqSel == MXL603_XTAL_24MHz))
  {
    ...
    status = MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_CAP_CTRL_REG, control);
    ...
    if (xtalCfg.XtalSharingMode == MXL_ENABLE)
    {
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_ENABLE_DIV_REG, control);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_EXT_BIAS_REG, 0x80);
    }
    else
    {
      control &= 0x01;
      // program Clock out div & Xtal sharing
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_ENABLE_DIV_REG, control);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_EXT_BIAS_REG, 0x0A);
    }

    // Main regulator re-program
    if (MXL_ENABLE == xtalCfg.singleSupply_3_3V)
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, MAIN_REG_AMP, 0x14);
  }
  else
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgDevPowerMode_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--|                 Dong Liu
--| 
...
MXL_STATUS MxLWare603_API_CfgDevPowerMode_ct8k(UINT8 devId,
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  switch(powerMode)
  {
    ...
    case MXL603_PWR_MODE_ACTIVE:
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, TUNER_ENABLE_REG, MXL_ENABLE);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, START_TUNE_REG, MXL_ENABLE);

      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x01);
      if (enableLoopthrough == MXL_ENABLE)
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_TUNE_RF1_BO_REG, 0x0E);
      else
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_TUNE_RF1_BO_REG, 0x37);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x00);
      break;

    case MXL603_PWR_MODE_STANDBY:
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, START_TUNE_REG, MXL_DISABLE);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, TUNER_ENABLE_REG, MXL_DISABLE);
      if ((standbyLt != 0) && (enableLoopthrough == MXL_ENABLE))
      {
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x01);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_TUNE_RF1_BO_REG, (UINT8)(standbyLt & 0x3F));
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x00);
      }
      break;
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgDevGPO_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_CfgDevGPO_ct8k(UINT8 devId, MXL603_GPO_STATE_E gpoState)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 regData = 0;
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  switch(gpoState)
  {
    ...
    case MXL603_GPO_LOW:
      status = MxLWare603_OEM_ReadRegister_ct8k(devId, GPO_SETTING_REG, &regData);
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, GPO_SETTING_REG, regData);
      break;
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_ReqDevVersionInfo_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_ReqDevVersionInfo_ct8k(UINT8 devId,
...
//  MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  if (mxlDevVerInfoPtr)
  {
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, CHIP_ID_REQ_REG, &readBack);
    mxlDevVerInfoPtr->chipId = (readBack & 0xFF);

    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, CHIP_VERSION_REQ_REG, &readBack);
    ...
    for (k = 0; k < MXL603_VERSION_SIZE; k++)
      mxlDevVerInfoPtr->mxlwareVer[k] = MxLWare603DrvVersion_ct8k[k];
  }
  else
    status = MXL_INVALID_PARAMETER;
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_ReqDevGPOStatus_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_ReqDevGPOStatus_ct8k(UINT8 devId, 
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_ReqDevPllState_ct8k
--| 
--| AUTHOR        : Dong Liu
--| 
...
MXL_STATUS MxLWare603_API_ReqDevPllState_ct8k(UINT8 devId, MXL603_PLL_STATE_E* PllStatePtr)
{
  ...
  *PllStatePtr = MXL603_PLL_STATE_NA;
  ...
  for (k = 0; k < 7; k ++)
  {
    ...
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, regAddr[k], &regData[k]);
  }
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgTunerMode_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_CfgTunerMode_ct8k(UINT8 devId, 
...
*/
  switch(tunerModeCfg.signalMode)
  {
    ...
    case MXL603_DIG_J83B:
      tmpRegTable = MxL603_DigitalDvbc_ct8k;
      status = MxL603_Ctrl_ProgramRegisters_ct8k(devId, tmpRegTable);

      if (tunerModeCfg.ifOutFreqinKHz < HIGH_IF_35250_KHZ)
      {
        // Low power
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_0, 0xFE);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_1, 0x10);
      }
      else
      {
        // High power
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_0, 0xD9);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_1, 0x16);
      }
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_CSF_SS_SEL, dfeRegData);

      break;

    case MXL603_DIG_ISDBT_ATSC:
      tmpRegTable = MxL603_DigitalIsdbtAtsc_ct8k;
      status = MxL603_Ctrl_ProgramRegisters_ct8k(devId, tmpRegTable);

      if (tunerModeCfg.ifOutFreqinKHz < HIGH_IF_35250_KHZ)
      {
        // Low power
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_0, 0xF9);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_1, 0x18);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_PWR, 0xF1);
      }
      else
      {
        // High power
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_0, 0xD9);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_CFG_1, 0x16);
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_IF_PWR, 0xB1);
      }
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_DACIF_GAIN, dfeRegData);

      break;

    case MXL603_DIG_DVB_T_DTMB:
      tmpRegTable = MxL603_DigitalDvbt_ct8k;
      status = MxL603_Ctrl_ProgramRegisters_ct8k(devId, tmpRegTable);
      if (tunerModeCfg.ifOutFreqinKHz < HIGH_IF_35250_KHZ)
      {
        // Low power
  ...
  {
    // XTAL calibration
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_CALI_SET_REG, 0x00);
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, XTAL_CALI_SET_REG, 0x01);

    // 50 ms sleep after XTAL calibration
    MxLWare603_OEM_Sleep_ct8k(50);
  }
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgTunerAGC_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_CfgTunerAGC_ct8k(UINT8 devId, MXL603_AGC_CFG_T agcCfg)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 regData = 0;
...
*/
      ...
      (agcCfg.agcType <= MXL603_AGC_EXTERNAL))
  {
    ...
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, AGC_CONFIG_REG, &regData);
    regData &= 0xF2; // Clear bits <3:2> & <0>
    regData = (UINT8) (regData | (agcCfg.agcType << 2) | 0x01);
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, AGC_CONFIG_REG, regData);

    // AGC set point <6:0>
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, AGC_SET_POINT_REG, &regData);
    regData &= 0x80; // Clear bit <6:0>
    regData |= agcCfg.setPoint;
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, AGC_SET_POINT_REG, regData);

    // AGC Polarity <4>
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, AGC_FLIP_REG, &regData);
    regData &= 0xEF; // Clear bit <4>
    regData |= (agcCfg.agcPolarityInverstion << 4);
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, AGC_FLIP_REG, regData);
  }
  else
    status = MXL_INVALID_PARAMETER;
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgTunerLoopThrough_ct8k
--| 
...
MXL_STATUS MxLWare603_API_CfgTunerLoopThrough_ct8k(UINT8 devId, MXL_BOOL loopThroughCtrl, UINT8* gainArray, UINT8* attArray)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 regData;
 ...
 // MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  if (loopThroughCtrl <= MXL_ENABLE)
  {
    ...
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, DIG_ANA_GINJO_LT_REG, &regData);

    if (loopThroughCtrl == MXL_ENABLE)
      regData |= 0x10;  // Bit<4> = 1
    else
      regData &= 0xEF;  // Bit<4> = 0
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DIG_ANA_GINJO_LT_REG, regData);

    if ((loopThroughCtrl == MXL_ENABLE) && (gainArray != NULL) && (attArray != NULL))
    {
      regData = (gainArray[3]<<6 ) | (gainArray[2] <<4 ) | (gainArray[1] <<2 ) | gainArray[0] ;
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_DIGANA_LT_GAIN_REG, regData);
      regData = (attArray[3]<<6 ) | (attArray[2] <<4 ) | (attArray[1] <<2 ) | attArray[0] ;
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_DIGANA_LT_ATTN_REG, regData);
    }
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x00);
  }
  else
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgTunerSouthAfricaLT_ct8k
--| 
--| AUTHOR        : Dong Liu
--| 
...
MXL_STATUS MxLWare603_API_CfgTunerSouthAfricaLT_ct8k(UINT8 devId)
{
  UINT8 status = MXL_SUCCESS;
 ...
 // MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x01);
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_DIGANA_LT_GAIN_REG, 0x95);
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, DFE_SEQ_DIGANA_LT_ATTN_REG, 0x00);
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x00);
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgTunerChanTune_ct8k
--| 
--| AUTHOR        : Mahendra Kondur, Dong Liu
--| 
...
MXL_STATUS MxLWare603_API_CfgTunerChanTune_ct8k(UINT8 devId, 
...
*/
  // Abort Tune
  status |= MxLWare603_OEM_WriteRegister_ct8k(devId, START_TUNE_REG, 0x00);
  ...
  if (chanTuneCfg.startTune == MXL_ENABLE)
  {
    if (chanTuneCfg.signalMode <= MXL603_DIG_J83B)
    {
      ...
      {
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x7C, 0x1F);
      ...
      {
        status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x7C, 0x9F);
        if ((chanTuneCfg.signalMode == MXL603_DIG_DVB_C) || (chanTuneCfg.signalMode == MXL603_DIG_J83B))
          regData = 0xD1;
        else
          regData = 0x91;
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x00, 0x01);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x31, regData);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x00, 0x00);
      ...
      switch (chanTuneCfg.signalMode)
      {
        case MXL603_DIG_DVB_C:
        case MXL603_DIG_J83B:
          freqLutPtr = MXL603_DIG_CABLE_FREQ_LUT_ct8k;
          break;

        case MXL603_DIG_ISDBT_ATSC:
        case MXL603_DIG_DVB_T_DTMB:
          freqLutPtr = MXL603_DIG_TERR_FREQ_LUT_ct8k;
          break;
      }

      if (freqLutPtr)
        status |= Ctrl_SetRfFreqLutTblReg_ct8k(devId, chanTuneCfg.freqInHz, freqLutPtr);
      ...
      switch(chanTuneCfg.bandWidth)
      {
        ...
        case MXL603_TERR_BW_8MHz:
            status |= MxLWare603_OEM_WriteRegister_ct8k(devId, CHAN_TUNE_BW_REG, (UINT8)chanTuneCfg.bandWidth);
            ...
            // Set RF
            status |= MxLWare603_OEM_WriteRegister_ct8k(devId, CHAN_TUNE_LOW_REG, (UINT8)(freq & 0xFF));
            status |= MxLWare603_OEM_WriteRegister_ct8k(devId, CHAN_TUNE_HI_REG, (UINT8)((freq >> 8 ) & 0xFF));
            break;
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, TUNER_ENABLE_REG, 0x01);
      ...
      status |= MxLWare603_OEM_ReadRegister_ct8k(devId, 0xB6, &agcData);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x01);
      status |= MxLWare603_OEM_ReadRegister_ct8k(devId, 0x60, &dfeTuneData);
      status |= MxLWare603_OEM_ReadRegister_ct8k(devId, 0x5F, &dfeCdcData);
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x60, dfeTuneData);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0x5F, dfeCdcData);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, PAGE_CHANGE_REG, 0x00);
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, 0xB6, agcData);

      // Bit <0> 1 : start , 0 : abort calibrations
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, START_TUNE_REG, 0x01);

      // Sleep 15 ms
      MxLWare603_OEM_Sleep_ct8k(15);
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_CfgTunerIFOutParam_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_CfgTunerIFOutParam_ct8k(UINT8 devId, MXL603_IF_OUT_CFG_T ifOutCfg)
{
  UINT16 ifFcw;
  UINT8 status = MXL_SUCCESS;
  UINT8 readData = 0;
  ...
  status = MxLWare603_OEM_ReadRegister_ct8k(devId, IF_FREQ_SEL_REG, &readData);
  ...
  if (ifOutCfg.manualFreqSet == MXL_ENABLE)
  {
   // MxL_DLL_DEBUG0("%s, IF Freq = %d \n", __FUNCTION__, ifOutCfg.manualIFOutFreqInKHz);
    ...
    readData |= 0x20;
    status = MxLWare603_OEM_WriteRegister_ct8k(devId, IF_FREQ_SEL_REG, readData);

    // Manual IF freq set
    ifFcw = (UINT16)(ifOutCfg.manualIFOutFreqInKHz * 8192 / 216000);
    control = (ifFcw & 0xFF); // Get low 8 bit
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, IF_FCW_LOW_REG, control);

    control = ((ifFcw >> 8) & 0x0F); // Get high 4 bit
    status |= MxLWare603_OEM_WriteRegister_ct8k(devId, IF_FCW_HIGH_REG, control);
  }
  ...
  if (status == MXL_SUCCESS)
  {
    ...
    if (ifOutCfg.ifInversion <= MXL_ENABLE)
    {
      ...
      status |= MxLWare603_OEM_WriteRegister_ct8k(devId, IF_PATH_GAIN_REG, control);
    }
    else
      status |= MXL_INVALID_PARAMETER;
  }
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_ReqTunerAGCLock_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_ReqTunerAGCLock_ct8k(UINT8 devId, MXL_BOOL* agcLockStatusPtr)
{
  MXL_STATUS status = MXL_SUCCESS;
  UINT8 regData = 0;
 ...
 // MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  if (agcLockStatusPtr)
  {
    status = MxLWare603_OEM_ReadRegister_ct8k(devId, AGC_SAGCLOCK_STATUS_REG, &regData);
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_ReqTunerLockStatus_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--| 
--| DATE CREATED  : 12/10/2011
...
MXL_STATUS MxLWare603_API_ReqTunerLockStatus_ct8k(UINT8 devId, MXL_BOOL* rfLockPtr,
                                                          MXL_BOOL* refLockPtr)
{
  MXL_STATUS status = MXL_SUCCESS;
  UINT8 regData = 0;
 ...
 // MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  {
    status = MxLWare603_OEM_ReadRegister_ct8k(devId, RF_REF_STATUS_REG, &regData);
...
/*------------------------------------------------------------------------------
--| FUNCTION NAME : MxLWare603_API_ReqTunerRxPower_ct8k
--| 
--| AUTHOR        : Mahendra Kondur
--|                 Dong Liu
--| 
...
MXL_STATUS MxLWare603_API_ReqTunerRxPower_ct8k(UINT8 devId, SINT16* rxPwrPtr)
{
  UINT8 status = MXL_SUCCESS;
  UINT8 regData = 0;
  UINT16 tmpData = 0;
 ...
 // MxL_DLL_DEBUG0("%s", __FUNCTION__);
  ...
  if (rxPwrPtr)
  {
    ...
    status = MxLWare603_OEM_ReadRegister_ct8k(devId, RFPIN_RB_LOW_REG, &regData);
    tmpData = regData;

    // RF input power high <1:0>
    status |= MxLWare603_OEM_ReadRegister_ct8k(devId, RFPIN_RB_HIGH_REG, &regData);
...
MT_FE_RET mt_fe_tn_init_MxL603_ct8k(MT_FE_CT8K_Device_Handle handle)
{
	...
	for (devId = 0; devId < EXAMPLE_DEV_MAX; devId++)
	{
		...
		status = MxLWare603_API_CfgDrvInit_ct8k(devId, (void *) &device_context[devId]);

		// if you don’t want to pass any oem data, just use NULL as a parameter:
		// status = MxLWare603_API_CfgDrvInit_ct8k(devId, NULL);
	}
	...
	*/
	...
	MXL603_Address_ct8k = handle->m_device_ctt2.tuner_cfg.tuner_dev_addr;
	//Step 1 : Soft Reset MxL603
	status = MxLWare603_API_CfgDevSoftReset_ct8k(devId);
	if (status != MXL_SUCCESS)
	{
		return MtFeErr_Fail;
	}
	...
	status = MxLWare603_API_CfgDevOverwriteDefaults_ct8k(devId, singleSupply_3_3V);
	if (status != MXL_SUCCESS)
	{
		return MtFeErr_Fail;
	}
	...
	//Step 3 : XTAL Setting
	xtalCfg.xtalFreqSel = MXL603_XTAL_FREQ_SEL_CT8K;
	if (MXL603_XTAL_24MHz == xtalCfg.xtalFreqSel)
	{
		xtalCfg.xtalCap = 16;
	}
	else
	...
	status = MxLWare603_API_CfgDevXtal_ct8k(devId, xtalCfg);
	if (status != MXL_SUCCESS)
	{
		return MtFeErr_Fail;
		// printf("Error! MxLWare603_API_CfgDevXtal_ct8k\n");
	}
	...
	status = MxLWare603_API_CfgTunerIFOutParam_ct8k(devId, ifOutCfg);
	if (status != MXL_SUCCESS)
	{
		return MtFeErr_Fail;
		//printf("Error! MxLWare603_API_CfgTunerIFOutParam_ct8k\n");
	}
	...
	status = MxLWare603_API_CfgTunerAGC_ct8k(devId, agcCfg);
	if (status != MXL_SUCCESS)
	{
		return MtFeErr_Fail;
		//printf("Error! MxLWare603_API_CfgTunerAGC_ct8k\n");
	}
	...
	tunerModeCfg.xtalFreqSel = MXL603_XTAL_FREQ_SEL_CT8K;
	tunerModeCfg.ifOutGainLevel = 11;
	status = MxLWare603_API_CfgTunerMode_ct8k(devId, tunerModeCfg);
	if (status != MXL_SUCCESS)
	{
		return MtFeErr_Fail;
		//printf("Error! MxLWare603_API_CfgTunerMode_ct8k\n");
	}

	handle->m_device_ctt2.tuner_cfg.tuner_init_ok = 1;
	handle->m_device_ctt2.tuner_cfg.tuner_open = 1;
...
MT_FE_RET mt_fe_tn_set_freq_MxL603_ct8k(MT_FE_CT8K_Device_Handle handle, U32 freq, U32 sym_rate, S16 lpf_offset)
{
	...
	mt_fe_tn_init_MxL603_ct8k(handle);
	...
	if(handle->m_device_ctt2.demod_type == MtFeType_DVBC)
	{
		...
		if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_8M)
		...
		else if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_6M)
	...
	chanTuneCfg.xtalFreqSel = MXL603_XTAL_FREQ_SEL_CT8K;
	chanTuneCfg.startTune = MXL_START_TUNE;
	status = MxLWare603_API_CfgTunerChanTune_ct8k(devId, chanTuneCfg);

	if (status != MXL_SUCCESS)
	{
		return  MtFeErr_Undef;
		// printf("Error! MxLWare603_API_CfgTunerChanTune_ct8k\n");
	}
	else
	{
		// Wait 15 ms
...
/*	FUNCTIN:
**		mt_fe_tn_get_strength_MxL603_ct8k
**
**	DESCRIPTION:
...
MT_FE_RET mt_fe_tn_get_strength_MxL603_ct8k(MT_FE_CT8K_Device_Handle handle, S8 *p_strength)
{
	...
	*p_strength = 0;
	...
	MxLWare603_API_ReqTunerLockStatus_ct8k(devId, &rfLockPtr, &refLockPtr);
	...
	if (handle->m_device_ctt2.demod_type != MtFeType_DVBC)
	{
		MxLWare603_API_ReqTunerRxPower_ct8k(devId, &PwrPtr);
...
MT_FE_RET mt_fe_tn_sleep_MxL603_ct8k(MT_FE_CT8K_Device_Handle handle)
{
	...
	devId = handle->m_device_ctt2.tuner_cfg.tuner_dev_addr;

	mt_fe_i2c_repeat_enable_ct8k(handle);
	status = MxLWare603_API_CfgDevPowerMode_ct8k(devId, MXL603_PWR_MODE_STANDBY, MXL_DISABLE, 0);
	mt_fe_i2c_repeat_disable_ct8k(handle);
...
MT_FE_RET mt_fe_tn_wake_up_MxL603_ct8k(MT_FE_CT8K_Device_Handle handle)
{
	...
	mt_fe_i2c_repeat_enable_ct8k(handle);
	status = MxLWare603_API_CfgDevPowerMode_ct8k(devId, MXL603_PWR_MODE_ACTIVE, MXL_DISABLE, 0);
	mt_fe_i2c_repeat_disable_ct8k(handle);
...
void mt_fe_ct8k_tn_attach_MxL603(void)
{
	...
	handle->m_device_ctt2.tuner_cfg.tuner_wakeup = (MT_FE_RET(*)(void *))mt_fe_tn_wake_up_MxL603_ct8k;

	handle->m_device_ctt2.tuner_cfg.tuner_get_offset = NULL;
	handle->m_device_ctt2.tuner_cfg.tuner_get_gain = NULL;
	handle->m_device_ctt2.tuner_cfg.tuner_set_application = NULL;
	handle->m_device_ctt2.tuner_cfg.tuner_set_loop_through = NULL;
	handle->m_device_ctt2.tuner_cfg.tuner_set_xtal = NULL;
	handle->m_device_ctt2.tuner_cfg.tuner_set_clkout = NULL;
	handle->m_device_ctt2.tuner_cfg.tuner_get_diagnose_info = NULL;
