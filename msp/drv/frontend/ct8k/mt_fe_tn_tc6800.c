...
static MT_FE_TN_DEVICE_SETTINGS_TC6800 TC6800_tuner_cfg_ct8k;
static U32 tc6800_ct8k_dbg_level = 0;
...
static S32 _mt_fe_tn_write_fw_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 reg_addr, U8 *reg_data, U8 data_len)
{
	...
	mt_fe_tn_write_ct8k(handle->tuner_dev_addr, p_buf, (U8)(data_len + 1));
...
/*****************************************************************************/
static void _mt_fe_tn_freq_fine_tune_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U8 Delta_sign, U8 Num_step)
{
...
#if 0
static void _mt_fe_tn_set_BB_Cal_tc6800(MT_FE_Tuner_Handle_TC6800 handle, U32 bandwidth)
{
	return;
}
#endif
...
static void _mt_fe_tn_set_sysmon_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
...
#if 0
static void _mt_fe_tn_set_appendix_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	return;
}
#endif
...
static void mt_fe_tn_init_tc6800(MT_FE_Tuner_Handle_TC6800 handle)
{
	if (handle->tuner_init_OK != 1)
	{
		...
		handle->tuner_version			 = 127;
		handle->tuner_fw_version		 = 0x0c;
		handle->tuner_time				 = 20122215;
...
{
	TC6800_tuner_cfg_ct8k.tuner_dev_addr = handle->m_device_ctt2.tuner_cfg.tuner_dev_addr;

	if (handle->m_device_ctt2.demod_type == MtFeType_DVBC)
	{
		TC6800_tuner_cfg_ct8k.tuner_init_OK = handle->m_device_ctt2.tuner_cfg.tuner_init_ok;
		TC6800_tuner_cfg_ct8k.tuner_application = 0;
		if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_8M)
			TC6800_tuner_cfg_ct8k.tuner_bandwidth = 8000;
		else if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_7M)
			TC6800_tuner_cfg_ct8k.tuner_bandwidth = 7000;
		else if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_6M)
			TC6800_tuner_cfg_ct8k.tuner_bandwidth = 6000;

		TC6800_tuner_cfg_ct8k.tuner_dac = 7200;

		if (handle->m_device_ctt2.board_cfg.iVppSelC == 1)
			TC6800_tuner_cfg_ct8k.tuner_dac_gain = 2;
		else
			TC6800_tuner_cfg_ct8k.tuner_dac_gain = 4;
	}
	...
	else
	{
		...
		{
			TC6800_tuner_cfg_ct8k.tuner_application = 3;//4;
			TC6800_tuner_cfg_ct8k.tuner_dac_gain = 2;//1;
		}
		else if (handle->m_device_ctt2.demod_type == MtFeType_DVBT2)
		{
			TC6800_tuner_cfg_ct8k.tuner_application = 3;
			TC6800_tuner_cfg_ct8k.tuner_dac_gain = 2;
		}
		else
#endif
		...
		if (handle->m_device_ctt2.board_cfg.iVppSel == 1)
			TC6800_tuner_cfg_ct8k.tuner_dac_gain = 2;
		else
			TC6800_tuner_cfg_ct8k.tuner_dac_gain = 4;
		...
		if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_8M)
			...
			TC6800_tuner_cfg_ct8k.tuner_bandwidth = 6000;
		else if (handle->m_device_ctt2.input_params.demod_bandwidth == MtFeBandwidth_5M)
			TC6800_tuner_cfg_ct8k.tuner_bandwidth = 5000;
		else
			TC6800_tuner_cfg_ct8k.tuner_bandwidth = 8000;

		TC6800_tuner_cfg_ct8k.tuner_dac = 5000;
	}

	mt_fe_tn_init_tc6800(&TC6800_tuner_cfg_ct8k);

	handle->m_device_ctt2.tuner_cfg.tuner_init_ok = TC6800_tuner_cfg_ct8k.tuner_init_OK;
	handle->m_device_ctt2.tuner_cfg.tuner_open = TC6800_tuner_cfg_ct8k.tuner_init_OK;
...
MT_FE_RET mt_fe_tn_set_freq_tc6800_tc_ct8k(MT_FE_CT8K_Device_Handle handle, U32 Freq_KHz, U32 sym_rate, S16 lpf_offset)
{
	...
	{
		mt_fe_tn_init_tc6800_tc_ct8k(handle);
	}
	...
	mt_fe_tn_set_freq_tc6800(&TC6800_tuner_cfg_ct8k, Freq_KHz);
...
#if 0
	{
		...
		for (reg_addr = 0x00; reg_addr <= 0xff; reg_addr++)
		{
			_mt_fe_tn_get_reg_tc6800(&TC6800_tuner_cfg_ct8k, (unsigned char)reg_addr, &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x100; reg_addr <= 0x105; reg_addr++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_ct8k, 0x1a, (unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x200; reg_addr <= 0x27c; reg_addr++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_ct8k, 0x39,(unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x300; reg_addr <= 0x332; reg_addr++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_ct8k, 0x4e, (unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}

		for (reg_addr = 0x400; reg_addr <= 0x418; reg_addr++)
		{
			_mt_fe_tn_get_reg_expand_tc6800(&TC6800_tuner_cfg_ct8k, 0x53, (unsigned char)(reg_addr & 0x0ff), &reg_value);
			printk("%x\t%x\n", reg_addr, reg_value);
		}
	}
#endif
...
MT_FE_RET mt_fe_tn_get_strength_tc6800_tc_ct8k(MT_FE_CT8K_Device_Handle handle, S8 *p_strength)
{
	...
	if (handle->m_device_ctt2.tuner_cfg.tuner_lna_type == 1)
	{
		if (handle->m_device_ctt2.input_params.input_freq_kHz < 474000)
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_ct8k)) - 9);
		else if (handle->m_device_ctt2.input_params.input_freq_kHz <= 666000)
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_ct8k)) - 14);
		else if (handle->m_device_ctt2.input_params.input_freq_kHz <= 794000)
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_ct8k)) - 12);
		else
			strength = (S8)((mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_ct8k)) - 9);
	}
	else
		strength = (S8)(mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_ct8k));
...
MT_FE_RET mt_fe_tn_get_gain_tc6800_ct8k(MT_FE_CT8K_Device_Handle handle, U32 *p_gain)
{
	...
	strength = mt_fe_tn_get_signal_strength_tc6800(&TC6800_tuner_cfg_ct8k);
...
{
	mt_fe_tn_sleep_tc6800(&TC6800_tuner_cfg_ct8k);
...
MT_FE_RET mt_fe_tn_wake_up_tc6800_tc_ct8k(MT_FE_CT8K_Device_Handle handle)
{
	...
	mt_fe_tn_set_freq_tc6800_tc_ct8k(handle, handle->m_device_ctt2.input_params.input_freq_kHz, 0, 0);
...
{
	TC6800_tuner_cfg_ct8k.tuner_int_im = tc6800_im;
...
{
	TC6800_tuner_cfg_ct8k.tuner_clock_out = tc6800_clock;
...
void mt_fe_tn_get_diagnose_info_tc6800_tc_ct8k(MT_FE_CT8K_Device_Handle handle, U32* data1, U32* data2)
{
	...
	if(*data1 > 2)
	{
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_ct8k, 1);
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_ct8k, 2);
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_ct8k, 3);
	}
	else
	{
		mt_fe_tn_get_diagnose_info_tc6800(&TC6800_tuner_cfg_ct8k, 1);
...
void mt_fe_ct8k_tn_attach_tc6800(void)
{
	...
	handle->m_device_ctt2.tuner_cfg.tuner_get_gain = (MT_FE_RET(*)(void *, U32 *))mt_fe_tn_get_gain_tc6800_ct8k;
