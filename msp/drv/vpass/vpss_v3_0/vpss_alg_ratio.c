...
{
    mt_u32 v_w, v_h;
    mt_u32 ar_w = AspectRatioW, ar_h = AspectRatioH;
...
mt_s32 ALG_RATIO_RatioProcess(ALG_RATIO_DRV_PARA_S *pstDrvPara,ALG_RATIO_OUT_PARA_S *pstOutPara)
{
    ...
    mt_s32 sr_w, sr_h, ar_w = 0, ar_h = 0, f_w, f_h;
    // output video w h
    mt_u32 i_w, i_h;
    ...
    if ((MT_DRV_ASP_RAT_MODE_BUTT == pstDrvPara->eAspMode))
    {
        return MT_SUCCESS;
    }

    //In customer mode ,process pOutInfo first
    if (MT_TRUE == pstDrvPara->stUsrAsp.bUserDefAspectRatio)
    {
    ...
    if ((0 == ar_w) || (0 == ar_h))
    {
        ...
        //MT_UNF_DISP_ASPECT_RATIO_AUTO&Square 0/0
        if ((0 == ar_w) || (0 == ar_h))
        {
    ...
    if (MT_DRV_ASP_RAT_MODE_LETTERBOX == pstDrvPara->eAspMode)
    {
        ALG_RATIO_LetterBox(ar_w, ar_h, &(pstDrvPara->stOutWnd), u32pixr1_out);
    }
    else //  MT_UNF_VO_ASPECT_CVRS_PAN_SCAN == enCvrsMode  ||  MT_UNF_VO_ASPECT_CVRS_COMBINED == enCvrsMode
    {
        #if 0
        mt_u32 u32ZmeH;
        mt_u32 u32ZmeW;
        ...
        if (u32ExpH > pstDrvPara->stOutWnd.s32Height * ALG_RATIO_PIX_RATIO1 )
        {
            ...
            if (MT_DRV_ASP_RAT_MODE_COMBINED == pstDrvPara->eAspMode)
            {
        ...
        #else
        mt_rect_s InWnd = pstDrvPara->stInWnd;
        ...
        if (MT_DRV_ASP_RAT_MODE_COMBINED == pstDrvPara->eAspMode)
        {
            //mt_u32 NewAspectWidth = *AspectWidth, mt_u32 NewAspectHeight = *AspectHeight;
                        ...
                        ALG_RATIO_OUT_PARA_S *pstAspCfg)
{
    ...
    if (pstAspDrvPara->eAspMode == MT_DRV_ASP_RAT_MODE_LETTERBOX)
    {
        pstAspCfg->bEnCrop = MT_FALSE;
    ...
    else if (pstAspDrvPara->eAspMode == MT_DRV_ASP_RAT_MODE_PANANDSCAN)
    {
