...
mt_s32 VPSS_MT_Init(VPSS_MT_INFO_S *pstMtInfo)
{
    ...
    pstMBuf = &(pstMtInfo->stMadMtInfo.stMBuf);
    nRet = mt_drv_mmz_alloc_and_map("VPSS_MADMotionInfoBuf", MT_NULL, u32InfoSize * 3, 0, pstMBuf);
    if (nRet != MT_SUCCESS)
    {
    ...
    VPSS_MT_FLUSHDATA(pstMBuf, 0x007f007f);

    for (ii = 0; ii < 3; ii++)
    {
        pstMtInfo->stMadMtInfo.u32MadMvAddr[ii]
