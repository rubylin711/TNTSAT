...
#ifndef  __MT_UNF_VO_H__
#define  __MT_UNF_VO_H__
...
/**CNcomment:定义视频旋转角度*/
typedef enum mtUNF_VO_ROTATION_E
{
    MT_UNF_VO_ROTATION_0, /**< 0 degree rotation*//**<CNcomment: 0度旋转*/
    MT_UNF_VO_ROTATION_90, /**< 90 degree rotation*//**<CNcomment: 90度旋转*/
    MT_UNF_VO_ROTATION_180, /**< 180 degree rotation*//**<CNcomment: 180度旋转*/
    MT_UNF_VO_ROTATION_270, /**< 270 degree rotation*//**<CNcomment: 270度旋转*/
    MT_UNF_VO_ROTATION_BUTT
}MT_UNF_VO_ROTATION_E;


/**conversion mode of AspectRatio.*/
/**CNcomment:宽高比转换模式*/
typedef enum mtUNF_VO_ASPECT_CVRS_E
{
    MT_UNF_VO_ASPECT_CVRS_IGNORE = 0x0,
    MT_UNF_VO_ASPECT_CVRS_LETTERBOX,      /**<Add black borders*//**<CNcomment: 加黑边*/
    MT_UNF_VO_ASPECT_CVRS_PAN_SCAN,      /**<Clip the picture*//**<CNcomment: 裁减*/
    MT_UNF_VO_ASPECT_CVRS_COMBINED,       /**<Add black borders and clip the picture*//**<CNcomment: 加黑边和裁减混合*/

    MT_UNF_VO_ASPECT_CVRS_HORIZONTAL_FULL,  /**<keep picture's HORIZONTAL be full*//**<CNcomment: 保持水平方向铺满*/
    MT_UNF_VO_ASPECT_CVRS_VERTICAL_FULL,    /**<keep picture's VERTICAL be full*//**<CNcomment: 保持垂直方向铺满*/
    MT_UNF_VO_ASPECT_CVRS_BUTT,
} MT_UNF_VO_ASPECT_CVRS_E;

/** AspectRatio attribute.*/
/**CNcomment:宽高比属性*/
typedef struct mtUNF_WINDOW_ASPECT_ATTR_S
{
    MT_UNF_VO_ASPECT_CVRS_E   enAspectCvrs;  /**<the method to the aspect ratio transfer.\n*//**<CNcomment: 窗口的宽高转换方式 */
    MT_BOOL                   bUserDefAspectRatio;  /**<use or not user define aspectratio. \n
    ...
    mt_u32                    u32UserAspectWidth; /**<aspect width of user \n
                                                    this value should be in 0~3840 \n
                                                    zero indicate using video resolution.*/
                                                  /**<CNcomment: 用户期望显示的视频宽度 \n
                                                   设置值范围在0~3840之间 \n
                                                   0代表使用视频源分辨率*/
    mt_u32                    u32UserAspectHeight; /**<aspect height of user \n
                                                    this value should be in 0~3840 \n
                                                    zero indicate using video resolution.*/
                                                    /**<CNcomment: 用户期望显示的视频高度 \n
                                                    设置值范围需要在0~3840的范围内 \n
                                                    0代表使用视频源分辨率*/
} MT_UNF_WINDOW_ASPECT_ATTR_S;

/**define the attribution of window.*/
/**CNcomment:定义WINDOW属性*/
typedef struct mtUNF_WINDOW_ATTR_S
{
    MT_UNF_DISP_E             enDisp;          /**<the display channel where window locate in. can not be modified.\n*//**<CNcomment: 窗口所在的显示通道, 创建后不可修改 */
    MT_BOOL                   bUseSubLayer;      /**< whether show video on still_layer or not. can not be modified.\n*//**<CNcomment: 是否在still层显示视频，创建后不可修改 */
    MT_BOOL                   bSetVideoBot;      /**< whether set video layer at the bottom or not，only works when bUseSubLayer is set. can not be modified.\n*//**<CNcomment: 是否设置video层为底层显示，只有still层启用时有效。创建后不可修改 */
    MT_BOOL                   bVirtual;      /**< whether is virtual window or not. can not be modified.\n*//**<CNcomment: 是否为虚拟窗口，创建后不可修改 */
    MT_UNF_VIDEO_FORMAT_E     enVideoFormat; /**<the data format of virtual window.\n*//**<CNcomment: 虚拟窗口数据格式 */
    MT_UNF_WINDOW_ASPECT_ATTR_S     stWinAspectAttr;  /**<the AspectRatio of window.\n*//**<CNcomment:  窗口的宽高比属性*/
    MT_BOOL                   bUseCropRect;     /**<cut video frame as stCropRect struct or as stInputRect struct \n
    ...
    MT_UNF_CROP_RECT_S        stCropRect;   /**<clip window of user define ,clip pix num of top,buttom,left,right.\n*/
                                            /**<CNcomment: 用户定义的视频裁剪结构体类型, 上下左右裁剪的像素个数*/

    mt_rect_s                 stInputRect;       /**<the crop area .\n
    ...
    mt_rect_s                 stOutputRect;  /**<the window display area on the virtual screen. \n
...
} MT_UNF_WINDOW_ATTR_S;


/**define user buffer management attribute.*/
/**CNcomment:定义用户缓冲区管理属性*/
typedef struct mtUNF_BUFFER_ATTR_S
{
    mt_s32              s32Stride;          /**<the stride of the buffer.*//**<CNcomment: 缓冲区步长*/
    mt_u32              u32PhyAddr[16];     /**<start physics addrss of the buffer.*//**<CNcomment: 缓冲区物理起始地址*/
    mt_u32              u32BuffIndex[16];   /**<Stride of the buffer.*//**<CNcomment: 缓冲区索引编号*/
    mt_s32              s32BuffNum;         /**<the number of the buffer.*//**<CNcomment: 缓冲区个数*/
} MT_UNF_BUFFER_ATTR_S;

/** the running mode of vo*/
/**CNcomment:VO设备的运行模式 */
typedef enum mtUNF_VO_DEV_MODE_E
{
    MT_UNF_VO_DEV_MODE_NORMAL,  /**<normal mode:
    ...
    MT_UNF_VO_DEV_MODE_MOSAIC,  /**<mosaic mode: can create more than 2 windows.
                                 3712 v300: not support*/
                                /**<CNcomment: 马赛克模式，可以创建多个WINDOW
                                 3712 v300: 不支持*/
    MT_UNF_VO_DEV_MODE_STANDALONE,  /**<seperate mode,only 1 window can be created,and  the aspect ratio set seperately.
                                        3716 v300: not support*/
                                    /**<CNcomment: standalone模式，高标清显示宽高比转换单独处理模式
                                     3716 v300 : 不支持*/
    MT_UNF_VO_DEV_MODE_BUTT
} MT_UNF_VO_DEV_MODE_E;

/**define the freeze mode enumeration.*/
/**CNcomment:定义Freeze模式枚举*/
typedef enum mtUNF_WINDOW_FREEZE_MODE_E
{
    MT_UNF_WINDOW_FREEZE_MODE_LAST = 0,   /**<display the last frame when freeze.*//**<CNcomment: freeze时显示最后一帧*/
    MT_UNF_WINDOW_FREEZE_MODE_BLACK = 1,  /**<display a black frame when freeze.*//**<CNcomment: freeze时显示后黑屏*/
    MT_UNF_WINDOW_FREEZE_MODE_BUTT
} MT_UNF_WINDOW_FREEZE_MODE_E;
...
\retval ::MT_SUCCESS successs.CNcomment:成功 CNend
\retval ::MT_ERR_VO_DEV_NOT_EXIST  vo device not exist. CNcomment:VO设备不存在 CNend
\retval ::MT_ERR_VO_NOT_DEV_FILE   not vo device. CNcomment:VO非设备 CNend
\retval ::MT_ERR_VO_DEV_OPEN_ERR   open vo failed. CNcomment:VO打开失败 CNend
\retval ::MT_ERR_VO_INVALID_PARA   illegal param. CNcomment:输入参数非法 CNend
\see \n
CNcomment:无 CNend
*/
...
\attention \n
before calling this func ,you should call MT_UNF_VO_Close to close all the windows first.
CNcomment:在调用::MT_UNF_VO_Close接口关闭所有打开的VO后调用本接口 CNend
\param CNcomment:无 CNend
\retval ::MT_SUCCESS  success.CNcomment:成功 CNend
\retval ::MT_ERR_VO_DEV_CLOSE_ERR  vo deinited failed. CNcomment:VO关闭失败 CNend
\see \n
CNcomment:无 CNend
*/
...
\attention \n
CNcomment:无 CNend
\param[in] pWinAttr   a pointer typeof MT_UNF_WINDOW_ATTR_S.CNcomment:WINDO属性结构指针 CNend
\param[in] phWindow   the window handler to be created.CNcomment:创建的WINDOW句柄 CNend
\retval ::MT_SUCCESS CNcomment:成功 CNend
\retval ::MT_ERR_VO_NO_INIT    VO not inited.CNcomment:VO未初始化 CNend
\retval ::MT_ERR_VO_INVALID_PARA   illegal params.CNcomment:输入参数非法 CNend
\retval ::MT_ERR_VO_NULL_PTR       null pointer.CNcomment:输入指针为空 CNend
\retval ::MT_ERR_VO_INVALID_OPT    illegal operation.CNcomment:操作非法 CNend
\retval ::MT_ERR_VO_CREATE_ERR     CNcomment:创建失败 CNend
\see \n
CNcomment:无 CNend
*/
...
\retval ::MT_ERR_VO_NULL_PTR       null pointer.CNcomment:输入指针为空 CNend
\retval ::MT_ERR_VO_INVALID_OPT    illegal operation.CNcomment:操作非法 CNend
\retval ::MT_ERR_VO_WIN_NOT_EXIST  windows not exist.CNcomment:窗口不存在 CNend
\see \n
CNcomment:无 CNend
*/
...
Each value as MT_UNF_WINDOW_ATTR_S Definition
CNcomment:enDisp和bVirtual参数一旦创建后就不能再进行修改。参数修改参照MT_UNF_WINDOW_ATTR_S的参数说明。 CNend
\param[in] hWindow    the input window handler.CNcomment:WINDOW句柄 CNend
...
CNcomment:无 CNend
*/
mt_s32 MT_UNF_VO_QFrame(mt_handle hWindow, void *pFrame);
...
func is forbidden when normal playing. It should be called after MT_UNF_VO_AttachWindow.
CNcomment:通过设置此接口使能，保证快退不会出现场图像回退的现象，在正常播放时，\n
请设置为禁止，默认为禁止。MT_UNF_VO_AttachWindow调用之后使用有效。 CNend
\param[in] hWindow           the input window handler.CNcomment:WINDOW句柄 CNend
...
\retval ::MT_ERR_VO_INVALID_OPT    illegal operation.CNcomment:操作非法 CNend
\retval ::MT_ERR_VO_WIN_NOT_EXIST  windows not exist.CNcomment:窗口不存在 CNend
\retval ::MT_ERR_VO_WIN_UNSUPPORT  unsupported operation.CNcomment:操作不支持 CNend
\see \n
CNcomment:无 CNend
*/
...
\retval ::MT_SUCCESS openned success.CNcomment:成功 CNend
\retval ::MT_ERR_VO_NO_INIT  VO not inited. CNcomment:VO未初始化 CNend
\retval ::MT_ERR_VO_INVALID_PARA  illegal params.CNcomment:输入参数非法 CNend
\retval ::MT_ERR_VO_NULL_PTR       null pointer.CNcomment:输入指针为空 CNend
\see \n
...
\retval ::MT_SUCCESS  success.CNcomment:成功 CNend
\retval ::MT_ERR_VO_NO_INIT    VO uninitialization.CNcomment:VO 未初始化 CNend
\retval ::MT_ERR_VO_INVALID_PARA   invalid input parameter.CNcomment:输入参数非法 CNend
\see \n
...
/**
\brief get current queued frame info.CNcomment: 获最后进送入显示队列的帧信息 CNend
\attention \n
\param[in] hWindow    window handle .CNcomment:窗口句柄 CNend
\param[out] pFrame    frame info mem pointer.CNcomment:存放frame info的内存指针. CNend
\retval ::MT_SUCCESS  success.CNcomment:成功 CNend
\retval ::MT_ERR_VO_NO_INIT    VO uninitialization.CNcomment:VO 未初始化 CNend
\retval ::MT_ERR_VO_INVALID_PARA   invalid input parameter.CNcomment:输入参数非法 CNend
\see \n
