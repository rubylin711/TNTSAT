...
ifeq ($(CONFIG_MT_INSTRUMENT_FUNCTIONS),y)
	...
	@cp --remove-destination $(SHARED_LIB_DIR_STRIPED)/mt/libnagra_dal.so $(USR_FS_DIR)/lib -af
	@cp --remove-destination $(SHARED_LIB_DIR_STRIPED)/mt/libnagra_icc.so $(USR_FS_DIR)/lib -af
#
# copy mali ddk lib
#
ifeq ($(CONFIG_MT_MALI_SUPPORT), y)
...
ifneq ($(wildcard $(BIN_DIR_STRIPED)/bin/mt/dalts),)
	cp -af $(BIN_DIR_STRIPED)/bin/mt/dalts $(USR_FS_DIR)/stb
endif
...
ifeq ($(CONFIG_MT_FPGA),y)
	...
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/0303.pk ${SDK_DIR}/image/$(MFRS)/update
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/csc.dat ${SDK_DIR}/image/$(MFRS)/update
endif
...
update_fpga:
	...
	cp -f ${SDK_DIR}/out/general/kernel/linux-x.y.z/arch/arm64/boot/uImage ${SDK_DIR}/image/$(MFRS)
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/arm64/tee-os/512/teeos.bin ${SDK_DIR}/image/$(MFRS)
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/arm64/tee-os/512/tee.dtb.bin ${SDK_DIR}/image/$(MFRS)	
	
	cd $(SDK_DIR)/image/$(MFRS)/

#spinand:
	./tools/dtb_merge.sh ./symphony6-rawnand-gmac.dtb ./uImage ./kernel
#emmc:
#	./tools/dtb_merge.sh ./symphony6-emmc-gmac.dtb ./uImage ./kernel
	./tools/align16.sh ./kernel ./kernel.temp
	./tools/header.sh ./kernel.temp ./kernel.nasc

	./tools/dtb_merge.sh ./tee.dtb.bin ./teeos.bin ./tee
	./tools/align16.sh ./tee ./tee.temp
	./tools/header.sh ./tee.temp ./tee.nasc

	./tools/align16.sh ./logo.jpg ./logo.jpg.temp
	./tools/header.sh ./logo.jpg.temp ./logo.jpg.nasc


	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/all.img ${SDK_DIR}/image/$(MFRS)/update/
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/bl3.img ${SDK_DIR}/image/$(MFRS)/update/
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/0303.pk ${SDK_DIR}/image/$(MFRS)/update
	cp --remove-destination -arf ${SDK_DIR}/tee_bsp/csc.dat ${SDK_DIR}/image/$(MFRS)/update

	cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/kernel.nasc ${SDK_DIR}/image/$(MFRS)/update/
	cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/logo.jpg.nasc ${SDK_DIR}/image/$(MFRS)/update/
	cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/tee.nasc ${SDK_DIR}/image/$(MFRS)/update/
	cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/nandubi.img ${SDK_DIR}/image/$(MFRS)/update/
	cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/update_spinand_nagra_dmdemo.vbs ${SDK_DIR}/image/$(MFRS)/update/
	-cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/rootfs_enc.img ${SDK_DIR}/image/$(MFRS)/update/
	-cp --remove-destination -f ${SDK_DIR}/image/$(MFRS)/usrfs_enc.img ${SDK_DIR}/image/$(MFRS)/update/
