...
int main(void)
{
	CgUpnpControlPoint *dmc = cg_upnp_controlpoint_new();

	if (cg_upnp_controlpoint_start(dmc) == FALSE) {
		printf("\nCouldn't start this control point !!\n");
		exit(1);
	}

	cg_upnp_controlpoint_search(dmc, CG_UPNP_ST_ROOT_DEVICE);
	cg_sleep(cg_upnp_controlpoint_getssdpsearchmx(dmc) * 1000);
