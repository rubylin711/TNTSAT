...
static void cg_http_server_clientthread(CgThread *thread)
{
	...
	if (cg_thread_getuserdata(thread) != NULL) 
	{
...
static void cg_http_stop_clients(CgHttpServer *httpServer)
{
	CgThreadList *thread;

	cg_http_server_lock(httpServer);
	thread = cg_threadlist_gets(httpServer->clientThreads);
	for (; thread != NULL; thread = cg_thread_next(thread))
	{
		if (cg_thread_getuserdata(thread) != NULL)
		{
			cg_thread_setuserdata(thread, NULL);
...
MT_BOOL cg_http_server_stop(CgHttpServer *httpServer)
{
	...
	//	cg_threadlist_stop(httpServer->clientThreads);
		cg_http_stop_clients(httpServer);
		cg_threadlist_delete(httpServer->clientThreads);
		httpServer->clientThreads = NULL;
	}
