...
MT_BOOL cg_file_exists(CgFile *file)
{
#if !defined(BTRON) && !defined(ITRON) && !defined(TENGINE) 
	FILE *fp;
#else
...
MT_BOOL cg_file_remove(CgFile *file)
{
	char *fileName;
	int removeSuccess = 0;
...
MT_BOOL cg_file_load(CgFile *file)
{
...
MT_BOOL cg_file_save(CgFile *file)
{
#if defined(WIN32) || defined(HAVE_FOPEN)
	FILE *fp;
	size_t nWrite;
...
MT_BOOL cg_file_open(CgFile *file, int mode)
{
	char *filename;
	char *stdioMode;
...
MT_BOOL cg_file_close(CgFile *file)
{
	if (!file->fp)
		return FALSE;
...
MT_BOOL cg_file_write(CgFile *file, CgByte *buf, int bufLen)
{
	if (!file->fp)
		return FALSE;
...
MT_BOOL cg_file_read(CgFile *file, CgByte *buf, int bufLen)
{
	if (!file->fp)
		return FALSE;
...
MT_BOOL cg_file_seek(CgFile *file, CgInt64 offset, int whence)
{
	int stdioWhence;
