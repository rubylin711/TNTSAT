...
#if !defined(ITRON)
MT_BOOL cg_socket_tosockaddrin(char *addr, int port, struct sockaddr_in *sockaddr, MT_BOOL isBindAddr);
#endif

#if !defined(BTRON) && !defined(ITRON) && !defined(TENGINE)
MT_BOOL cg_socket_tosockaddrinfo(int sockType, char *addr, int port, struct addrinfo **addrInfo, MT_BOOL isBindAddr);
#endif
...
#if defined(TENGINE) && defined(CG_TENGINE_NET_KASAGO)
MT_BOOL cg_socket_setmulticastinterface(CgSocket *sock, char *ifaddr);
#endif
...
#if defined(ITRON)
MT_BOOL cg_socket_initwindowbuffer(CgSocket *sock);
MT_BOOL cg_socket_freewindowbuffer(CgSocket *sock);
static ER cg_socket_udp_callback(ID cepid, FN fncd, VP parblk);
static ER cg_socket_tcp_callback(ID cepid, FN fncd, VP parblk);
static MT_BOOL cg_socket_getavailablelocaladdress(T_IPV4EP *localAddr);
#endif
...
MT_BOOL cg_socket_isbound(CgSocket *sock)
{
	cg_log_debug_l4("Entering...\n");
...
MT_BOOL cg_socket_close(CgSocket *sock)
{
	cg_log_debug_l4("Entering...\n");
...
MT_BOOL cg_socket_listen(CgSocket *sock)
{
#if defined(BTRON) || (defined(TENGINE) && !defined(CG_TENGINE_NET_KASAGO))
...
MT_BOOL cg_socket_bind(CgSocket *sock, int bindPort, char *bindAddr,  MT_BOOL bindFlag, MT_BOOL reuseFlag)
{
...
MT_BOOL cg_socket_accept(CgSocket *serverSock, CgSocket *clientSock)
{
	struct sockaddr_in sockaddr;
	socklen_t socklen;
...
MT_BOOL cg_socket_connect(CgSocket *sock, char *addr, int port)
{
#if defined(BTRON) || (defined(TENGINE) && !defined(CG_TENGINE_NET_KASAGO))
	ERR ret;
	struct sockaddr_in sockaddr;
...
MT_BOOL cg_socket_setreuseaddress(CgSocket *sock, MT_BOOL flag)
{
	int sockOptRet;
#if defined(BTRON) || (defined(TENGINE) && !defined(CG_TENGINE_NET_KASAGO))
	B optval
...
MT_BOOL cg_socket_setmulticastttl(CgSocket *sock, int ttl)
{
	int sockOptRet;
	int ttl_;
	unsigned int len=0;
...
MT_BOOL cg_socket_settimeout(CgSocket *sock, int sec)
{
	int sockOptRet;
#if defined(BTRON) || (defined(TENGINE) && !defined(CG_TENGINE_NET_KASAGO))
...
MT_BOOL cg_socket_joingroup(CgSocket *sock, char *mcastAddr, char *ifAddr)
{
	struct ip_mreq ipmr;
