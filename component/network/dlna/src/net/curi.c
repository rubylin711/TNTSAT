...
MT_BOOL cg_net_uri_isequivalent(char *url, char *relative_url)
{
	CgNetURI *u;
	char *path;
	MT_BOOL ret;
...
MT_BOOL cg_net_uri_isescapedstring(char *buf, int bufSize)
{
	int idx;
...
MT_BOOL cg_net_uri_isreservedchar(char c)
{
...
MT_BOOL cg_net_uri_isalphanumchar(char c)
{
	if ('a' <= c && c <= 'z')
		return TRUE;
	if ('A' <= c && c <= 'Z')
...
MT_BOOL cg_net_uri_isunreservedchar(char c)
{
	if (cg_net_uri_isalphanumchar(c))
		return TRUE;
...
MT_BOOL cg_net_uri_isescapechar(char c)
{
