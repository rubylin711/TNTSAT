...
MT_BOOL IsInterfaceAddressInitialized = FALSE;
#endif
...
static char g_intf_opstate_path[128];
static char g_tmpbuf[128];
#define	INTF_OPSTATE_PATH		"/sys/class/net/%s/operstate"
#define	INTF_LINKSTATE_PATH		"/sys/class/net/%s/phys_link_state"
static int __intf_is_up(char* path)
{
	int ret, fd;
	...
	if (fd < 0)
		return 0;
	ret = read(fd, g_tmpbuf, sizeof(g_tmpbuf) - 1);
	if (ret <= 0) {
		close(fd);
		return 0;
	}
	g_tmpbuf[ret] = 0;
	if (strncmp(g_tmpbuf, "up", 2) == 0)
		ret = 1;
	else
		ret = 0;
	close(fd);
...
static int __intf_is_oper_up(char* ifname)
{
	sprintf(g_intf_opstate_path, INTF_LINKSTATE_PATH, ifname);
	if (__intf_is_up(g_intf_opstate_path))
		return 1;
	sprintf(g_intf_opstate_path, INTF_OPSTATE_PATH, ifname);
	return __intf_is_up(g_intf_opstate_path);
}
#define	GET_DEFAULT_ROUTE	"route -n | grep 'UG' | head -n 1 | awk {'print $8'}"
static int __is_default_intf(char* ifname)
{
	FILE *fp;
	int ret = 0;
	if (access("/usr/bin/head", F_OK))
		return 1;
	fp = popen(GET_DEFAULT_ROUTE, "r");
	if (!fp)
		return 0;
	if (fgets(g_tmpbuf, sizeof(g_tmpbuf) - 1, fp) && strstr(g_tmpbuf, ifname))
		ret = 1;
	pclose(fp);
	return ret;
}
...
int cg_net_gethostinterfaces(CgNetworkInterfaceList *netIfList)
{
	...
	while (!feof(fd)) {
		...
		if (req.ifr_flags & IFF_LOOPBACK)
			continue;
		if (!__intf_is_oper_up(ifname))
			continue;
		if (!__is_default_intf(ifname))
			continue;
