...
extern int dlnac_setvolume(CgUpnpArgument *arg);
extern int dlnac_getvolume(CgUpnpArgument *arg);
extern int dlnac_setmute(CgUpnpArgument *arg);
extern int dlnac_getmute(CgUpnpArgument *arg);
...
MT_BOOL cg_upnpav_dmr_renderingctrl_actionreceived(CgUpnpAction *action)
{
	...
	if (cg_streq(actionName, CG_UPNPAV_DMR_RENDERINGCONTROL_SETMUTE)) {
		...
		stateVar = cg_upnp_service_getstatevariablebyname(service, 
			CG_UPNPAV_DMR_RENDERINGCONTROL_MUTE);
		cg_upnp_statevariable_setvalue(stateVar, cg_upnp_argument_getvalue(arg));
		dlnac_command(dmr, arg, DLNAC_SETMUTE);
	...
	if (cg_streq(actionName, CG_UPNPAV_DMR_RENDERINGCONTROL_GETVOLUME)) {
		...
		stateVar = cg_upnp_service_getstatevariablebyname(service, 
			CG_UPNPAV_DMR_RENDERINGCONTROL_VOLUME);
		
		if (!dlnac_command(dmr, arg, DLNAC_GETVOLUME))
			cg_upnp_statevariable_setvalue(stateVar, cg_upnp_argument_getvalue(arg));
		else
			cg_upnp_argument_setvalue(arg, cg_upnp_statevariable_getvalue(stateVar));
 	...
 	if (cg_streq(actionName, CG_UPNPAV_DMR_RENDERINGCONTROL_SETVOLUME)) {
		...
		cg_upnp_statevariable_setvalue(stateVar, cg_upnp_argument_getvalue(arg));
		dlnac_command(dmr, arg, DLNAC_SETVOLUME);
