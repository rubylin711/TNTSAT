...
MT_BOOL cg_xml_attribute_isname(CgXmlAttribute *attr, char *name);
...
#define cg_xml_node_setname(node, nodename) cg_string_setvalue(node->name, (char*)nodename)
...
CgXmlNode *cg_xml_node_getchildnodewithnamespace(CgXmlNode *node, char *name, char *ns, MT_BOOL ignoreNs);
...
char *cg_xml_node_tostring(CgXmlNode *node, MT_BOOL withChildNode, CgString *str);
...
MT_BOOL cg_xml_parse(CgXmlParser *parser, CgXmlNodeList *nodeList, char *data, int len);
