...
static int tcp_open(URLContext *h, const char *uri, int flags)
{
    ...
    if (cur_ai && cur_ai->ai_family == AF_INET6){
        ...
        if (sockaddr_v6 && !sockaddr_v6->sin6_port){
    ...
    if (s->listen == 2) {
        // multi-client
        if (cur_ai && (ret = ff_listen(fd, cur_ai->ai_addr, cur_ai->ai_addrlen)) < 0)
            goto fail1;
    } else if (s->listen == 1) {
        // single client
        if (cur_ai && (ret = ff_listen_bind(fd, cur_ai->ai_addr, cur_ai->ai_addrlen,
