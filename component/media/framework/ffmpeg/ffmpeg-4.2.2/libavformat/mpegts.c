...
#define SIZE_STAT_THRESHOLD 10
    ...
    int current_pid;
    TransportDesDecCallback desdec_cb;
};
                                  ...
                                  uint32_t stream_type, uint32_t prog_reg_desc)
{
    ...
    {
        //st->request_probe = 50;  //removed by zhouxiang, dont need probe codec!!!!!
    }
...
typedef struct ChinaDrmInfo {
    unsigned int  descriptor_length;
    unsigned char *descriptor_data;
    ChinaDrmDescr descr;
} ChinaDrmInfo;

static void china_drm_parse_descriptor(
    AVFormatContext *s, const uint8_t *buf, unsigned int size, ChinaDrmInfo *cdrm_info)
{
    int ret = ff_transport_parse_china_drm_descriptor(s, buf, size, &(cdrm_info->descr));
    if (ret < 0) {
        return;
    }
    if (cdrm_info->descriptor_data) {
        av_freep(&(cdrm_info->descriptor_data));
    }

    cdrm_info->descriptor_data = av_memdup(buf, size);
    if (!cdrm_info->descriptor_data) {
        return;
    }
    cdrm_info->descriptor_length = size;
}

static void china_drm_export_encryption_info(AVStream *st, ChinaDrmInfo *cdrm_info)
{
    ChinaDrmDescr *descr = &(cdrm_info->descr);

    if (ENCRYPTION_SCHEME_NONE == descr->audio_encryption_method &&
        ENCRYPTION_SCHEME_NONE == descr->video_encryption_method) {
        return;
    }
    if (0 == descr->nb_drm_data_bytes) {
        return;
    }

    enum AVMediaType codec_type = st->codecpar->codec_type;
    int need_inject_encryption_info =
        (AVMEDIA_TYPE_VIDEO == codec_type && ENCRYPTION_SCHEME_NONE != descr->video_encryption_method) ||
        (AVMEDIA_TYPE_AUDIO == codec_type && ENCRYPTION_SCHEME_NONE != descr->audio_encryption_method);

    if (!need_inject_encryption_info) {
        return;
    }

    size_t side_data_size = cdrm_info->descriptor_length;
    uint8_t *side_data = av_memdup(cdrm_info->descriptor_data, side_data_size);

    if (!side_data) {
        return;
    }
    int ret = av_stream_add_side_data(st,
        AV_PKT_DATA_MPEGTS_CHINA_DRM_INFO, side_data, side_data_size);
    if (ret < 0) {
        av_free(side_data);
    }
}
...
/**
 * MTS_DESC_CA (0x09) parse_mpeg2_descriptor_ca:
 ...
 */
static void parse_mpeg2_descriptor_ca(MpegTSContext *ts,
    AVStream *st, const uint8_t *desc_list_start, const uint8_t *desc_list_end)
{
    const uint8_t *data = NULL;
    int data_length = (int) (desc_list_end - desc_list_start);
    /* Has CA function */
    if (ts && ts->desdec_cb) {
        return;
    }
    ...
    data = desc_list_start + 2;
    data_length = FFMIN(data_length, get16(&data, desc_list_end));
    if (data_length < 4) { /* CA_system_ID:16bits CA_PID : 16bits */
        return;
    }
    /* Cannot handle Conditional Access stream  */
    st->request_probe     = 0;
    st->stream_identifier = 0;
    st->disposition      |= AV_DISPOSITION_DESCRIPTIONS;
    st->need_parsing      = AVSTREAM_PARSE_NONE;
    if (st->codecpar) {
        st->codecpar->codec_id   = (enum AVCodecID) (-1);
        st->codecpar->codec_tag  = (enum AVCodecID) (-1);
        st->codecpar->codec_type = AVMEDIA_TYPE_UNKNOWN;
    }
    if (st->codec) {
        st->codec->codec_id   = (enum AVCodecID) (-1);
        st->codec->codec_tag  = (enum AVCodecID) (-1);
                              ...
                              MpegTSContext *ts)
{
    ...
    case 0xfd: /* ARIB data coding type descriptor */
        ...
        break;
    case 0x09: /* Conditional Access information from descriptor */ {
        parse_mpeg2_descriptor_ca(ts, st, *pp, desc_list_end);
        break;
    }
    default:
        break;
...
static void pmt_cb(MpegTSFilter *filter, const uint8_t *section, int section_len)
{
    ...
    int mp4_descr_count = 0;
    ChinaDrmInfo china_drm_info = { 0, {0} };
    Mp4Descr mp4_descr[MAX_MP4_DESCR_COUNT] = { { 0 } };
    int i;
    ...
    while (program_info_length >= 2) {
        ...
        if (tag == 0x1d) { // IOD descriptor
            ...
            len -= 4;
        } else if (tag == 0xC0 && len >= 4) { // ChinaDrm descriptor
            (void) china_drm_parse_descriptor(ts->stream, p, len, &(china_drm_info));
        }
        p += len;
    }
    ...
    for (i = 0; ; i++) {
        ...
        china_drm_export_encryption_info(st, &china_drm_info);
        p = desc_list_end;
    }
...
out:
    if (china_drm_info.descriptor_data) {
        av_free(china_drm_info.descriptor_data);
    }
    for (i = 0; i < mp4_descr_count; i++)
        av_free(mp4_descr[i].dec_config_descr);
}
...
static int handle_packet(MpegTSContext *ts, const uint8_t *packet)
{
    ...
    uint8_t desdec_packet[TS_FEC_PACKET_SIZE] = {0};
    if (packet[3] & 0xc0) {
        av_log(ts->stream, AV_LOG_TRACE, "[0x%x] transport_scrambling_control 2\n", (int) pid);
        if (ff_transport_desdec(
            ts->stream, NULL, ts->desdec_cb,
            packet, desdec_packet, ts->raw_packet_size) < 0) {
            return 0;
        }
        packet = desdec_packet;
    }
...
static int handle_packets(MpegTSContext *ts, int64_t nb_packets)
{
    ...
    for (;;) {
        ...
        if (ret != 0 || ff_check_interrupt(&s->interrupt_callback))
            break;
    }
...
static char *process_pid_get_value(char *str, int *id)
{
    int idx = 0;
    char value[4096] = {'\0'};
    while (idx < 4095 && str && *str != '\0') {
        if (*str == ',') {
            *str++ = '\0';
            break;
        }
...
static void process_pid_key_value(char *line,
    PESContext *apes, PESContext *vpes,  PESContext *spes)
{
    char *p = line;
    apes->pid = vpes->pid = spes->pid = -1;
    while (p && *p != '\0') {
        while (*p != '\0' && *p != ':') {
            if (*p == '{' || *p == '}' || *p == '[' || *p == ']') {
                *p = ' ';
        ...
        *p++ = '\0';
        ...
        if (strstr(tag, "pcr")) {
            p = process_pid_get_value(p, &vpes->pcr_pid);
        } else if (strstr(tag, "apid")) {
            p = process_pid_get_value(p, &apes->pid);
        } else if (strstr(tag, "vpid")) {
            p = process_pid_get_value(p, &vpes->pid);
        } else if (strstr(tag, "spid")) {
            p = process_pid_get_value(p, &spes->pid);
        } else if (strstr(tag, "atype")) {
            p = process_pid_get_value(p, &apes->stream_type);
        } else if (strstr(tag, "vtype")) {
            p = process_pid_get_value(p, &vpes->stream_type);
        } else if (strstr(tag, "stype")) {
            p = process_pid_get_value(p, &spes->stream_type);
        } else if (strstr(tag, "alang")) {
            p = process_pid_get_value(p, NULL);
        }
        line = p;
    }
    apes->pcr_pid = spes->pcr_pid = vpes->pcr_pid;
}

static int new_http_server_pid_stream(
    MpegTSContext *ts, AVFormatContext *s, int progid, PESContext *in)
{
    PESContext *pes = NULL;
    if (in->pid <= 0) {
        return 0;
    ...
    pes = add_pes_stream(ts, in->pid, in->pcr_pid);
    if (!pes) {
        av_log(s, AV_LOG_ERROR, "Failed to allocate memory for pes ctx\n");
        return AVERROR(ENOMEM);
    }
    pes->st =
        avformat_new_stream(ts->stream, NULL);
    if (!pes->st) {
        av_log(s, AV_LOG_ERROR, "Failed to allocate memory for pes stream\n");
        return AVERROR(ENOMEM);
    }

    pes->st->id = pes->pid;
    mpegts_set_stream_info(pes->st, pes, in->stream_type, 0);
    av_program_add_stream_index(ts->stream, progid, pes->st->index);

    return 0;
}

static int read_header_parse_http_server_pid_info(
    MpegTSContext *ts, AVFormatContext *s, const char *pid_info)
{
    unsigned int str_len = strlen(pid_info);
    if (!str_len) {
        return AVERROR_INVALIDDATA;
    }

    av_log(s, AV_LOG_INFO, "Http pid inforamtion:%p\n", pid_info);
    char *str = av_mallocz(str_len + 1);
    if (!str) {
        av_log(s, AV_LOG_ERROR, "Failed to allocate buffers for http server pid info\n");
        return AVERROR_INVALIDDATA;
    }
    memcpy(str, pid_info, strlen(pid_info));
    PESContext apes = {0};
    PESContext vpes = {0};
    PESContext spes = {0};
    process_pid_key_value(str, &apes, &vpes, &spes);
    av_free(str);

    clear_programs(ts);
    int program_id = 0x01;
    AVProgram *program =
        av_new_program(ts->stream, program_id);
    if (!program) {
        av_log(s, AV_LOG_ERROR, "Failed to allocate memory for program\n");
        return AVERROR(ENOMEM);
    }
    int ret = 0;
    ret = new_http_server_pid_stream(ts, s, program_id, &apes);
    if (ret < 0) {
        return ret;
    }
    ret = new_http_server_pid_stream(ts, s, program_id, &vpes);
    if (ret < 0) {
        return ret;
    }
    ret = new_http_server_pid_stream(ts, s, program_id, &spes);
    if (ret < 0) {
        return ret;
    }
    return 0;
...
static int mpegts_read_header(AVFormatContext *s)
{
    ...
    (void) ff_transport_get_desdec_callback(s, &ts->desdec_cb);
    ...
    ts->stream     = s;
    ts->auto_guess = 0;
    uint8_t *http_private_info = NULL;
    if (av_opt_get(pb, "server_private_info", AV_OPT_SEARCH_CHILDREN, &http_private_info) >= 0) {
        if (read_header_parse_http_server_pid_info(ts, s, http_private_info) >= 0) {
            seek_back(s, pb, pos);
            av_log(ts->stream, AV_LOG_INFO, "Use HTTP server private information\n");
            return 0;
        }
    }
