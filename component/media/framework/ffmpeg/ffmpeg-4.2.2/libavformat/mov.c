...
static int update_frag_index(MOVContext *c, int64_t offset)
{
    ...
    if (index < c->frag_index.nb_items) {
        ...
        /* current handle index frag */
        c->frag_index.item[index + 1].headers_read = 0;
    }
...
static int mov_read_mvhd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->time_scale > 0 && !c->trex_data) {
        if (MOV_DUR_CACAL_TYPE_MEHD != c->duration_type) {
            c->fc->duration = av_rescale(c->duration, AV_TIME_BASE, c->time_scale);
            c->duration_type = MOV_DUR_CACAL_TYPE_MVHD;
        }
    }
...
static int mov_read_stco(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->fc->nb_streams < 1)
        return 0;
    st = c->fc->streams[c->fc->nb_streams-1];
    if (ff_mov_opti_stream_enable(c, st)) {
        return ff_mov_opti_read_stco(c, st, pb, atom);
    }
    sc = st->priv_data;
...
static int mov_read_stss(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    st = c->fc->streams[c->fc->nb_streams-1];
    if (ff_mov_opti_stream_enable(c, st)) {
        return ff_mov_opti_read_stss(c, st, pb, atom);
    }
    sc = st->priv_data;
...
static int mov_read_stsz(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc = st->priv_data;
    if (ff_mov_opti_stream_enable(c, st)) {
        return ff_mov_opti_read_stsz(c, st, pb, atom);
    }
...
static void set_encryption_info(AVStream *st, AVEncryptionInitInfo *default_info)
{
    if (!st || !default_info) {
        return;
    }
    ...
    side_data = av_stream_get_side_data(
        st, AV_PKT_DATA_ENCRYPTION_INIT_INFO, &side_data_size);
    if (side_data && side_data_size) {
        return;
    }

    AVEncryptionInitInfo *info = NULL;
    if (ff_dash_clone_content_protection(&info, default_info) < 0) {
        return;
    }

    MOVStreamContext  *sc = st->priv_data;
    AVEncryptionInfo *des = sc->cenc.default_encrypted_sample;
    if (des && des->key_id && !(info->num_key_ids) && !info->key_ids) {
        unsigned int alloc_size = 0;
        info->key_ids = av_mallocz_array(1, sizeof(*info->key_ids));
        if (!info->key_ids) {
            return;
        }
        info->key_ids[0] = av_mallocz(des->key_id_size);
        if (!info->key_ids[0]) {
            return;
        }
        info->num_key_ids = 1;
        memcpy(info->key_ids[0], des->key_id, des->key_id_size);
    }
    (void) ff_decrypt_comp_merge_init_info(st, info);
    ff_dash_free_content_protection(&info);
}
...
static int mov_read_trak(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (ff_mov_opti_stream_create_ctx(c, st) < 0) {
        av_log(c->fc, AV_LOG_WARNING, "stream %d alloc opti ctx fail\n", st->index);
    } else {
        MOVOptiContext *moc = sc->opti_ctx;
        if (moc) {
            moc->add_ctts_entry = add_ctts_entry;
        }
    }
    ...
    /* mov opti read part of the table */
    if (sc->stsc_count && (!ff_mov_opti_stream_enable(c, st) &&
        sc->stsc_data[ sc->stsc_count - 1 ].first > sc->chunk_count)) {
    ...
    if (ff_mov_opti_stream_enable(c, st)) {
        ret = ff_mov_opti_build_index(c, st);
    ...
    /* mov opti use low memory, keep it until read close */
    if (!ff_mov_opti_stream_enable(c, st)) {
    ...
    set_encryption_info(st, c->default_encryption_info);
    return 0;
}

static mov_read_mehd(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    return ff_mov_read_mehd(c, pb, atom);
}
...
static int mov_read_trex(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    if (c->duration_type != MOV_DUR_CACAL_TYPE_MEHD) {
        c->fc->duration = AV_NOPTS_VALUE; // the duration from mvhd is not representing the whole file when fragments are used.
        c->duration_type = MOV_DUR_CACAL_TYPE_CACAL;
    }
...
static int mov_read_trun(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    flags = avio_rb24(pb);
    entries = avio_rb32(pb);
    av_log(c->fc, AV_LOG_DEBUG, "flags 0x%x entries %u trun pos:0x%llx\n", flags, entries, avio_tell(pb) - 16);
    ...
    for (i = 0; i < entries && !pb->eof_reached; i++) {
        ...
        if (0 == i && frag_stream_info && AV_NOPTS_VALUE == frag_stream_info->sidx_pts) {
            frag_stream_info->sidx_pts = dts;
        }
        index_entry_pos++;
...
static int mov_read_sidx(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    sc->has_sidx = 1;
    c->frag_index.reference_id = track_id;
...
static int mov_read_pssh(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    /* Some clip with pssh for a/v share, before new stream avformat_new_stream */
    if (c->fc->nb_streams >= 1) {
        st = c->fc->streams[c->fc->nb_streams-1];
    }
    ...
    int64_t pssh_offset = avio_tell(pb) - 12;
    if (ff_mov_parse_pssh_header(c, pb, pssh_offset, atom.size + 8, info) < 0) {
    ...
    /* a/v share pssh */
    if (!st) {
        return ff_mov_append_default_encryption_info(c, info);
    } else {
        if (c->default_encryption_info) {
            ff_dash_free_content_protection(&c->default_encryption_info);
            (void) ff_dash_clone_content_protection(&c->default_encryption_info, info);
        }
        (void) ff_mov_check_pssh_encryption_info(c, st, info);
    }
    ...
    if (encryption_index) {
        ...
        if (!encryption_index->nb_encrypted_samples) {
            ...
            av_log(mov->fc, AV_LOG_ERROR,
                "[%d]Incorrect number of samples in encryption info[%d %d]\n",
                    st->id, encryption_index->nb_encrypted_samples, encrypted_index);
            return AVERROR_INVALIDDATA;
        }
        ...
        if (mov->decryption_key) {
            ...
            return ff_mov_inject_encryption_info(mov, st, encrypted_sample, pkt);
        }
    }
...
/*
 * we should exit as soon as possible when
 * play url from network no matter whether it can seek or not,
 * or it will consume more time,
 * some fragment mp4 has sidx with moof, but some mp4 has no sidx with moof.
 * 1:has sidx/mfra, parse all moofs in one sidx/mfra table.
 * 2:no sidx/mfra, parse one moof
*/
static int mov_read_default(MOVContext *c, AVIOContext *pb, MOVAtom atom)
{
    ...
    while (total_size <= atom.size - 8 && !avio_feof(pb)) {
        ...
        if (atom.size >= 8) {
            ...
            if (ff_mov_more_moof_found(c, pb, &a)) {
                int64_t start_pos = avio_tell(pb);
                c->next_root_atom = start_pos - 8;
                c->atom_depth --;
                return 0;
            }
        ...
        if (!parse) { /* skip leaf atoms data */
            ...
            int sidx_frag_finsish =
                c->frag_index.complete && (!c->next_root_atom ||
                (c->next_root_atom && (start_pos + a.size) >= c->next_root_atom));
            if (c->found_moov && c->found_mdat &&
                ((!(pb->seekable & AVIO_SEEKABLE_NORMAL) || c->fc->flags & AVFMT_FLAG_IGNIDX || sidx_frag_finsish) ||
                 start_pos + a.size == avio_size(pb))) {
                if (!(pb->seekable & AVIO_SEEKABLE_NORMAL) || c->fc->flags & AVFMT_FLAG_IGNIDX || sidx_frag_finsish) {
...
static int probe_encrypted(const AVProbeData *p, int moov_offset)
{
    /* skip size bytes */
    int64_t offset = moov_offset + 4;
    uint32_t tag;
...
static int mov_probe(const AVProbeData *p)
{
    ...
    if(score > AVPROBE_SCORE_MAX - 50 && moov_offset != -1) {
        ...
        if (!has_pssh) {
            has_pssh = probe_encrypted(p, moov_offset);
        }
    }
#ifdef VMX_OTT_SVP
    /* FIXME, remove future */
    extern int g_dash_playmode;
    if (!g_dash_playmode && !has_pssh) {
       return score;
    }
    return 0;
#else
...
static int mov_read_close(AVFormatContext *s)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        av_freep(&sc->coll);
        (void) ff_mov_opti_stream_destroy_ctx(st);
    }
    ...
    av_encryption_init_info_free(mov->default_encryption_info);
    mov->default_encryption_info = NULL;
...
static int mov_read_header(AVFormatContext *s)
{
    ...
    mov->trak_index = -1;
    mov->default_encryption_info = NULL;
    mov->duration_type = MOV_DUR_CACAL_TYPE_NONE;
    s->is_network_stream = ff_mov_is_network_stream(mov);
...
static AVIndexEntry *mov_find_next_sample(AVFormatContext *s, AVStream **st)
{
    ...
    for (i = 0; i < s->nb_streams; i++) {
        ...
        AVIndexEntry *current_sample =
            ff_mov_opti_get_next_sample(s->priv_data, avst);
...
static int mov_switch_root(AVFormatContext *s, int64_t target, int index)
{
    ...
    mov->found_mdat = 0;
    mov->found_moof = 0;
    int seekable = s->pb->seekable;
    /* not allow seek when switch root, it need more time to seek network */
    if ((seekable & AVIO_SEEKABLE_NORMAL) && s->is_network_stream) {
        s->pb->seekable = 0;
    }
	...
	int updated = 0;
    while (index < mov->frag_index.nb_items &&
        mov->frag_index.item[index].moof_offset <= mov->fragment.moof_offset) {
        mov->frag_index.item[index].headers_read = 1;
        index++;
		updated = 1;
    }

    if (avio_feof(s->pb) && !updated)
        return AVERROR_EOF;
...
static int mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
 ...
 retry:
    ...
    ret = cenc_filter(mov, st, sc, pkt, (int) current_index);
    if (ret < 0)
        return ret;
...
static int mov_seek_fragment(AVFormatContext *s, AVStream *st, int64_t timestamp)
{
    ...
    if (!mov->frag_index.complete && !mov->stream_seek_type)
        return 0;

    /* seek sidx belongs to the stream */
    if (st->id != mov->frag_index.reference_id) {
        return 0;
    }
    ...
    int64_t next_root_atom = -1;
    int64_t next_moof_offset =
        (index + 1) < mov->frag_index.nb_items ?
        mov->frag_index.item[index + 1].moof_offset : 0;
    do {
        if (index >= 0 && mov->frag_index.item[index].headers_read) {
            break;
        }
        int ret = mov_switch_root(s, next_root_atom, index);
        if (ret < 0) {
            return ret;
        }
         /* Need parse all moof of the old sidx item when seek */
        if (mov->next_root_atom >= next_moof_offset) {
            return 0;
        }
        /* switch by offset */
        index = -1;
        next_root_atom = mov->next_root_atom;
    } while (mov->frag_index.current + 1 < mov->frag_index.nb_items);
    if (index + 1 < mov->frag_index.nb_items)
        mov->next_root_atom = mov->frag_index.item[index + 1].moof_offset;
...
static int mov_seek_stream(AVFormatContext *s, AVStream *st, int64_t timestamp, int flags)
{
    ...
    ret = ff_mov_opti_read_seek(s->priv_data, st, timestamp, flags);
    av_log(s, AV_LOG_TRACE, "[%d] timestamp %"PRId64", ret:%d\n", st->index, timestamp, ret);
    int base_sample = ff_mov_opti_curr_base_sample_id(s->priv_data, st);
    if (ret >= 0) {
        sample = ret;
    } else {
        return ret;
    ...
    mov_current_sample_set(sc, sample + base_sample);
    ...
    if (mc->seek_individually) {
        ...
        while (1) {
            ...
            if (!entry)
                return AVERROR_INVALIDDATA;
            sc = st->priv_data;
            int base_sample = ff_mov_opti_curr_base_sample_id(mc, st);
            if (sc->ffindex == stream_index && sc->current_sample == (sample + base_sample))
                break;
            mov_current_sample_inc(sc);
        }
    }
...
static const AVOption mov_options[] = {
    ...
    { "enable_drefs", "Enable external track support.", OFFSET(enable_drefs), AV_OPT_TYPE_BOOL,
        {.i64 = 0}, 0, 1, FLAGS },
    { "max_opti_index_entries", "Set optimum low memory case index entry number",
        OFFSET(max_opti_index_entries), AV_OPT_TYPE_INT, { .i64 = INT_MAX }, 256, INT_MAX, .flags = FLAGS},
...
#ifndef CONFIG_MT_FILEPLAY_NONETWORK

#ifdef VMX_OTT_SVP
#include "bento4api/Bento4Mp4DemuxApi.h"

typedef struct _tagMovDrmContext {
...
static int dash_mov_probe(const AVProbeData *p)
{
    int64_t offset;
    uint32_t tag;
    int score = 0;
    ...
    if(score > AVPROBE_SCORE_MAX - 50 && moov_offset != -1 && !has_pssh) {
        has_pssh = probe_encrypted(p, moov_offset);
    }
    if (!has_pssh && !g_dash_playmode) {
        return 0;
    }
...
static int dash_mov_read_header(AVFormatContext *s)
{
    MOVContext *mov = s->priv_data;
    AVIOContext *pb = s->pb;

    s->bneto4_priv_data = Bento4Mp4DemuxApi_ReadHeader(s);
    if (s->bneto4_priv_data == NULL) {
        return AVERROR(EINVAL);
    }
    printf("zx Bento4Mp4DemuxApi_ReadHeader end\n");
    s->dash_select_track = -1;
    mov->fc = s;
    return 0;
}

static int64_t choose_best_seek_pos(AVFormatContext *s, int64_t seek_timestamp)
{
    Bento4StreamContext *sc = NULL;
    AVStream *st = NULL;
    int64_t best_pos = -1, tmp_pos;
    int  j = 0, s_i = 0;

    printf("[%s] seek_timestamp: %lld\n", __FUNCTION__, seek_timestamp);
    for (s_i = 0; s_i < s->nb_streams; s_i++) {
        st = s->streams[s_i];
        sc = (Bento4StreamContext *)st->priv_data;
        if (sc->sidx) {
            //has fragment mp4 segments
            if (sc->sidx->entries_count == 0 || sc->sidx->entries == NULL) {
                printf("Error: Fragment mp4 dont has sidx!!!!!\n");
                return AVERROR_BUG;
            }
            for (j = 0; j < sc->sidx->entries_count; j++) {
                if (seek_timestamp <=  sc->sidx->entries[j].pts) {
                    printf("[%s] seeked time:%lld will break!\n", __FUNCTION__, sc->sidx->entries[j].pts);
                    tmp_pos = sc->sidx->sidx_base_offset + sc->sidx->entries[j].offset;
                    break;
                }
            }
        } else if (sc->p_trfaEntry) {
            for (j = 0; j < sc->trfaEntries_Count; j++) {
                if (seek_timestamp <=  sc->p_trfaEntry[j].pts) {
                    printf("[%s] seeked time:%lld will break!\n", __FUNCTION__, sc->p_trfaEntry[j].pts);
                    tmp_pos = sc->p_trfaEntry[j].offset;
                    break;
                }
            }
        }
...
static int dash_mov_read_packet(AVFormatContext *s, AVPacket *pkt)
{
    MOVContext *mov = s->priv_data;
    int i = 0;
    Bento4StreamContext *sc = NULL;
...
restart:
    //for dash, get the next sample
    if (s->dash_select_track == -1) {
        if (s->nb_streams > 1) {
            int track_id = Bento4Mp4DemuxApi_GetNextStForMultiTracks(s->bneto4_priv_data);
            if (track_id >= 0) {
                for (i = 0; i < s->nb_streams; i++) {
                    st = s->streams[i];
                    sc = st->priv_data;
                    if (st->index_entries == NULL && sc->track_id == track_id) {
                        //printf("BuildIndexEntry i %d\n", i);
                        s->dash_select_track = i;
                        Bento4Mp4DemuxApi_BuildIndexEntry(s->bneto4_priv_data, st);
                    }
                }
            }
        } else if (s->nb_streams == 1) {
            if (s->streams[0]->index_entries == NULL) {
                //printf("BuildIndexEntry 0\n");
                Bento4Mp4DemuxApi_BuildIndexEntry(s->bneto4_priv_data, s->streams[0]);
            }
            s->dash_select_track = 0;
        }
    }

    if (s->dash_select_track == -1) {
        //printf("%s: return AVERROR_EOF;!\n", __func__);
        return AVERROR_EOF;
    }

    st = s->streams[s->dash_select_track];
    sc = st->priv_data;

    sample = &st->index_entries[sc->current_sample];
    if (!sample) {
        printf("%s: AVERROR_EOF s->is_change_next %d!\n", __func__, s->is_change_next);
        if (s->is_change_next == 1) {
            if (Bento4Mp4DemuxApi_ResetHeader(s->bneto4_priv_data, s) != 0) {
                printf("%s: ERROR Bento4Mp4DemuxApi_ResetHeader!\n", __func__);
                return AVERROR_BUG;
            }
            s->is_change_next = 0;
            //Bento4Mp4DemuxApi_BuildIndexEntry(s->bneto4_priv_data, s->streams[0]);
    ...
    sc = st->priv_data;
    if (Bento4Mp4DemuxApi_ReadPacket(s->bneto4_priv_data, st, sample, pkt) != 0) {
        printf("%s: ERROR Bento4Mp4DemuxApi_ReadPacket!\n", __func__);
        return AVERROR(ENOMEM);
    }
    sc->current_sample++;
    if (st->nb_index_entries == sc->current_sample) {
        av_freep(&st->index_entries);
        st->index_entries = NULL;
        st->nb_index_entries = 0;
        sc->current_sample = 0;
        s->dash_select_track = -1;
        if (s->profile_isoff_ondemand) {
            if (Bento4Mp4DemuxApi_Check_Switch_Streams(s->bneto4_priv_data, st) == 1) {
                int64_t seek_timestamp = av_rescale_rnd(pkt->pts, AV_TIME_BASE,
                                                        st->time_base.den,
                                                        AV_ROUND_UP);
                if (Bento4Mp4DemuxApi_ResetHeader(s->bneto4_priv_data, s) != 0) {
                    printf("%s: ERROR Bento4Mp4DemuxApi_ResetHeader!\n", __func__);
                ...
                int64_t best_pos = choose_best_seek_pos(s, seek_timestamp);
                if (best_pos != -1) {
                    Bento4Mp4DemuxApi_Seek(s->bneto4_priv_data, best_pos, BENTO4_SEEK_BYTE);
                }

                s->is_change_next = 0;
                printf("Bento4Mp4DemuxApi_Check_Switch_Streams\n");
            }
        }
    }
    return 0;//zx
...
static int dash_mov_read_seek(AVFormatContext *s, int stream_index, int64_t sample_time, int flags)
{
    Bento4StreamContext *sc = NULL;
    AVStream *st = NULL;
    int j = 0;
    printf("%s %d sample_time %lld\n", __func__, __LINE__, sample_time);

    int64_t seek_timestamp = stream_index < 0 ? sample_time :
                             av_rescale_rnd(sample_time, AV_TIME_BASE,
                                            s->streams[stream_index]->time_base.den,
                                            flags & AVSEEK_FLAG_BACKWARD ?
                                            AV_ROUND_DOWN : AV_ROUND_UP);
    //printf("%s %d seek_timestamp %lld\n", __func__, __LINE__, seek_timestamp);
    if (Bento4Mp4DemuxApi_ResetHeader(s->bneto4_priv_data, s) != 0) {
        printf("%s: ERROR Bento4Mp4DemuxApi_ResetHeader!\n", __func__);
        return AVERROR_BUG;
    }
    ...
    int64_t best_pos = -1;
    if (s->nb_streams > 1) {
        //multi tracks, choose the best pos
        best_pos = choose_best_seek_pos(s, seek_timestamp);
        if (best_pos != -1) {
            Bento4Mp4DemuxApi_Seek(s->bneto4_priv_data, best_pos, BENTO4_SEEK_BYTE);
        }
    } else {
        ...
        if (sc->sidx || sc->p_trfaEntry) {
            best_pos = choose_best_seek_pos(s, seek_timestamp);
            if (best_pos != -1) {
                Bento4Mp4DemuxApi_Seek(s->bneto4_priv_data, best_pos, BENTO4_SEEK_BYTE);
            }
        } else {
            //small segment files case
            //Bento4Mp4DemuxApi_BuildIndexEntry(s->bneto4_priv_data, st);
        }
    }
...
static int dash_mov_read_close(AVFormatContext *s)
{
    printf("%s %d\n", __func__, __LINE__);
    for (int i = 0; i < s->nb_streams; i++) {
        Bento4StreamContext *sc = (Bento4StreamContext *)s->streams[i]->priv_data;
        Bento4Mp4DemuxApi_CloseStream(s->bneto4_priv_data, s->streams[i]);
    }
    Bento4Mp4DemuxApi_Destroy(s->bneto4_priv_data);
    s->bneto4_priv_data = NULL;

    return 0;
}

static const AVOption dash_mov_options[] = {
    { NULL },
};

static const AVClass dash_mov_class = {
    .class_name = "dashmov",
    .item_name  = av_default_item_name,
    .option     = dash_mov_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

AVInputFormat ff_dash_mov_demuxer = {
    .name           = "dashmov",
    .long_name      = NULL_IF_CONFIG_SMALL("DASH / MOV"),
    .priv_class     = &dash_mov_class,
    .priv_data_size = sizeof(MovDrmContext),
    .extensions     = "dashmov",
    .read_probe     = dash_mov_probe,
    .read_header    = dash_mov_read_header,
    .read_packet    = dash_mov_read_packet,
    .read_close     = dash_mov_read_close,
    .read_seek      = dash_mov_read_seek,
