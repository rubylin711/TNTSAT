    ...
    if ((delay == 0 || (delay == 1 && pc)) &&
        ...
        if (presentation_delayed) {
            ...
            if (pkt->pts == AV_NOPTS_VALUE &&
                ...
                st->parser && st->parser->pict_type == AV_PICTURE_TYPE_I) {
                pkt->pts = next_dts;
            }
                         ...
                         int64_t target_ts, int flags)
{
    ...
    if (st->index_entries) {
        ...
        } else { /* Not read enough index table */
            int64_t file_size = avio_size(s->pb);
            if (file_size > 0) {
                pos_max = file_size - 1;
                pos_limit = pos_max;
                av_log(s, AV_LOG_TRACE, "set pos_max=0x%"PRIx64" pos_limit=0x%"PRIx64
                        " dts_max=%s\n", pos_max, pos_limit, av_ts2str(ts_max));
            }
        }
    }
                               ...
                               int64_t timestamp, int flags)
{
    ...
    if (stream_index < 0) {
        /* Demux by external demux, no stream info here */
...
int avformat_find_stream_info(AVFormatContext *ic, AVDictionary **options)
{
    ...
    if (ic->probesize <= 0) {
        if(strstr(ic->iformat->name,"flv")){
            ic->probesize = 5000000;
        } else if(io_isnetworkstream == 0){
    	...
    	if(ic->nb_streams > 3) {
            av_log(ic, AV_LOG_WARNING, "nb_streams %d: not enough time to estimate rate; ", ic->nb_streams);
            ic->probesize = 0;
        }
    }
    ...
    if (!max_analyze_duration) {
        ...
        if (!strcmp(ic->iformat->name, "flv")){
            if(io_isnetworkstream == 0) {
                ...
                max_stream_analyze_duration = 10*AV_TIME_BASE;  //xingwei @20230919 for bug20506
            }
        }
        ...
        if (!strcmp(ic->iformat->name, "mpeg") || !strcmp(ic->iformat->name, "mpegts")) {
            ...
            max_stream_analyze_duration = 1*AV_TIME_BASE;   // xingwei @20230919 for bug19021
        }
    }
...
#if 0// del by zhouxiang ,because we do not open codec , so we can not get codec parameters from codec open.
...
/* ++++++Montage LZ player private function start:++++++*/
static PLAYER_FFMPEG_CTRL_T *ff_get_ctrl_data(void)
{
    static PLAYER_FFMPEG_CTRL_T s_player_ffmpeg_ctrl = {0};
    return &s_player_ffmpeg_ctrl;
}

void av_init_priv_data_mem(void)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();

    if (data->fake_mpd_url) {
        av_free(data->fake_mpd_url);
    }
    if (data->fake_mpd_str) {
        av_free(data->fake_mpd_str);
    }
    memset(data, 0, sizeof(PLAYER_FFMPEG_CTRL_T));
}

void av_deinit_priv_data_mem(int valid)
{
    av_init_priv_data_mem();
}

void *av_get_priv_data_mem_ptr(void)
{
    return (void *)ff_get_ctrl_data();
}

int av_player_is_exit(void)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();

    return data->player_exit;
}

int av_get_fileplay_speed_index(void)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();
    /* 0 is normal, 1 is fast play, -1 is rev fast */
    return data->speed_index;
}

void av_set_mov_decryption_key(char *key)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();

    data->mov_decryption_key = key;
}

char* av_get_mov_decryption_key(void)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();

    return data->mov_decryption_key;
}

void av_set_mov_decryption_key_len(int len)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();

    data->mov_decryption_key_len = len;
}

int av_get_mov_decryption_key_len(void)
{
    PLAYER_FFMPEG_CTRL_T *data =
        ff_get_ctrl_data();

    return data->mov_decryption_key_len;
}

int av_dash_set_fake_mpd(const char* mpd_url, const char* mpd_str)
{
    PLAYER_FFMPEG_CTRL_T *data = ff_get_ctrl_data();
    if (!mpd_url || !mpd_str || !data) {
        return -1;
    }
    if (data->fake_mpd_url) {
        av_free(data->fake_mpd_url);
    }
    size_t len = strlen(mpd_url);
    data->fake_mpd_url = av_malloc(len + 1);
    strcpy(data->fake_mpd_url, mpd_url);
    data->fake_mpd_url[len] = '\0';

    if (data->fake_mpd_str) {
        av_free(data->fake_mpd_str);
    }
    len = strlen(mpd_str);
    data->fake_mpd_str = av_malloc(len + 1);
    strcpy(data->fake_mpd_str, mpd_str);
    data->fake_mpd_str[len] = '\0';

    return 0;
}

int av_dash_get_fake_mpd(char** mpd_url, char** mpd_str)
{
    PLAYER_FFMPEG_CTRL_T *data = ff_get_ctrl_data();
    if (!data || !data->fake_mpd_str || !data->fake_mpd_url) {
        return -1;
    }
    if (mpd_url) {
        *mpd_url = data->fake_mpd_url;
    }
    if (mpd_str) {
        *mpd_str = data->fake_mpd_str;
    }

    return 0;
}

void av_dash_reset_fake_mpd(void)
{
    PLAYER_FFMPEG_CTRL_T *data = ff_get_ctrl_data();
    if (!data) {
        return;
    ...
    if (data->fake_mpd_str) {
        av_freep(&data->fake_mpd_str);
    }
    if (data->fake_mpd_url) {
        av_freep(&data->fake_mpd_url);
    }
}

void av_drm_set_index_info(int index1, int index2)
{
    PLAYER_FFMPEG_CTRL_T *data = ff_get_ctrl_data();
    if (!data) {
        return;
    }
    printf("zx av_drm_set_index_info\n");
    data->drm_eDrmIndex1 = index1;
    data->drm_eDrmIndex2 = index2;
}


/* --------Montage LZ player private function end:  --------*/
