...
#define WHITESPACES " \n\t\r"
/* 5 minits */
#define HTTP_FORCE_RECONNECT_INTERVAL   (5 * 60 * AV_TIME_BASE)
...
typedef struct HTTPContext {
    ...
    /* server defined some private inforamtion */
    char *server_private_info;
    int64_t qos_data_size;
    int64_t qos_start_time;
    int64_t qos_net_bitrate;
    /* last http read time, if user pause too long time,
     * different driver may return various error code, including 0,
     * thus, it's better to reconnect after long time pause.
     */
...
static const AVOption options[] = {
    { "qos_net_bitrate", "bitrate of network for qos", OFFSET(qos_net_bitrate), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, D },
    ...
    { "server_private_info", "Server defined some private inforamtion", OFFSET(server_private_info), AV_OPT_TYPE_STRING, { .str = NULL }, 0, 0, D | E },
    { NULL }
};
...
redo:
    ...
    if (ret < 0) {
        ...
        av_log(h, AV_LOG_WARNING, "Will reconnect at %"PRIu64" in %d Millisecond(s).\n", off, reconnect_delay);
        ret = ff_network_sleep_interruptible(1000U * reconnect_delay, &h->interrupt_callback);
...
int ff_http_do_new_request2(URLContext *h, const char *uri, AVDictionary **opts)
{
    ...
    av_log(s, AV_LOG_DEBUG, "Opening \'%s\' for %s\n", uri, h->flags & AVIO_FLAG_WRITE ? "writing" : "reading");
...
static int process_line(URLContext *h, char *line, int line_count)
{
    ...
    if (line_count == 0) {
        ...
        *p  = '\0';
        ...
        } else if (!av_strcasecmp(tag, "Pid-Info")) {
            if (s->server_private_info) {
                av_freep(&s->server_private_info);
            }
            s->server_private_info = av_strdup(p);
        }
    }
    return 1;
}
...
static int http_read_stream(URLContext *h, uint8_t *buf, int size)
{
    ...
    while (read_ret < 0) {
        ...
        if (read_ret == AVERROR_EXIT || read_ret == AVERROR_EOF)
            break;
        ...
        av_log(h, AV_LOG_WARNING, "Will reconnect at %"PRIu64" in %d Millisecond(s), error=%s.\n", s->off, reconnect_delay, av_err2str(read_ret));
        err = ff_network_sleep_interruptible(1000U*reconnect_delay, &h->interrupt_callback);
...
static void read_cacal_qos(URLContext *h,
    HTTPContext *s, int size, int64_t current_time_us, int update)
{
    int64_t time_diff;
    s->qos_data_size += size;
    if (s->qos_data_size <= 0) {
        s->qos_data_size = 0;
    }

    time_diff = s->last_read_time - s->qos_start_time;
    if (update || time_diff >= AV_TIME_BASE) {
        if (s->qos_data_size > 0) {
            s->qos_net_bitrate = s->qos_data_size * 8 * AV_TIME_BASE / time_diff;
        }
        s->qos_start_time  = s->last_read_time;
        s->qos_data_size   = 0;
    }
}
...
static int http_read(URLContext *h, uint8_t *buf, int size)
{
    ...
    if (-1 != s->last_read_time &&
        current_time_us >= s->last_read_time + HTTP_FORCE_RECONNECT_INTERVAL) {
        uint64_t target = h->is_streamed ? 0 : s->off;
        av_log(h, AV_LOG_WARNING, "Reconnect at %"PRIu64" for timedout\n", s->off);
        (void) http_seek_internal(h, target, SEEK_SET, 1);
    }
    int read_size = http_read_stream(h, buf, size);
    s->last_read_time = av_gettime();
    if (read_size < 0) {
        read_cacal_qos(h, s, 0, current_time_us, 1);
        return read_size;
    }

    s->icy_data_read += read_size;
    read_cacal_qos(h, s, read_size, current_time_us, (int) !(s->qos_net_bitrate));
    return read_size;
}
