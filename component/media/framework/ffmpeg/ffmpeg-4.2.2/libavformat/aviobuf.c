...
int64_t aviobuf_reset(AVIOContext *s)
{
    if(s == NULL)
        return -1LL;
    s->pos = 0;
    //printf("%s %d enter\n", __func__, __LINE__);
    s->buf_end = s->buf_ptr = s->buffer;//reset
    s->eof_reached = 0;
    return 0;
}
...
int ffio_set_buf_size(AVIOContext *s, int buf_size)
{
    ...
    unsigned char *old_buffer = s->buffer;
    int old_buffer_size = s->buffer_size;
    ...
    if ((!s->write_flag) && buf_size > old_buffer_size) {
        memcpy(buffer, old_buffer, old_buffer_size);
        s->buf_end = (unsigned char *)((intptr_t) s->buffer + old_buffer_size);
    }
    av_free(old_buffer);
    return 0;
}
...
int avio_open2(AVIOContext **s, const char *filename, int flags,
               const AVIOInterruptCB *int_cb, AVDictionary **options)
{
    if (!strncmp(filename, SEPERATE_URL_PREFIX, strlen(SEPERATE_URL_PREFIX))) {
        return ff_adaptive_open_seperate_media(s, filename);
    }
