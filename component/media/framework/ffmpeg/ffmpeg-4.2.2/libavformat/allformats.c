...
#ifdef VMX_OTT_SVP
extern AVInputFormat  ff_mont_dash_demuxer;
extern AVInputFormat  ff_dash_mov_demuxer;
#endif
extern AVInputFormat  ff_hls_mont_demuxer;
...
extern AVInputFormat  ff_hls_simple_demuxer;
extern AVInputFormat  ff_hls_mont_demuxer;
...
extern AVInputFormat  ff_mtsurl_demuxer;
...
static const AVInputFormat * const mont_demuxer_list[] = {
#ifdef VMX_OTT_SVP
    &ff_mont_dash_demuxer,
    &ff_dash_mov_demuxer,
#endif
    &ff_hls_mont_demuxer,
    NULL };
...
const AVInputFormat *av_demuxer_iterate(void **opaque)
{
    ...
    if (i < size) {
        ...
        f = indev_list[i - size];
    } else if (mont_demuxer_list) {
        int indev_list_size = 0;
        if(indev_list)
            indev_list_size = sizeof(indev_list)/sizeof(indev_list[0]) - 1;
        f = mont_demuxer_list[i - size - indev_list_size];
    }
...
static void av_format_init_next(void)
{
    ...
    //add zhouxiang mont dash and dashmov
    {
#ifdef VMX_OTT_SVP
        if (previn)
            previn->next = &ff_mont_dash_demuxer;
        previn = &ff_mont_dash_demuxer;
        printf("previn name %s\n", previn->name);
        if (previn)
            previn->next = &ff_dash_mov_demuxer;
        previn = &ff_dash_mov_demuxer;
        printf("previn name %s\n", previn->name);
#endif
    }
