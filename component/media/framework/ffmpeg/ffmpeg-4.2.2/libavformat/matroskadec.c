...
typedef struct MatroskaTrackEncryption {
     ...
     * Predefined values: 1 - DES; 2 - 3DES; 3 - Twofish; 4 - Blowfish; 5 - AES. WebM only supports a value of 5 (AES).
     */
    uint64_t algo;
    EbmlBin  key_id;
...
typedef struct MatroskaTrackEncoding {
     ...
     * Values (big endian) can be OR'ed. Possible values:
     ...
     * Possible values: 0 - compression; 1 - encryption.
     */
    uint64_t type;
                      ...
                      EbmlSyntax *syntax, void *data)
{
    ...
    {
        //printf("useless MATROSKA_ID_ATTACHMENTS, skip %lld, syntax->type %d\n", length, syntax->type);
        goto skip;
    }
...
static int matroska_parse_tracks(AVFormatContext *s)
{
    ...
    for (i = 0; i < matroska->tracks.nb_elem; i++) {
        ...
        if (encodings_list->nb_elem > 1) {
            ...
            if (encodings[0].type) {
                if (encodings[0].encryption.key_id.size > 0) {
                    ...
                    track->protection =
                        ff_matroska_create_protection(0,
                            encodings[0].encryption.key_id.size, IV_SIZE_WEBM_DEFAULT);
                    if (track->protection) {
                        AVEncryptionInfo *info = track->protection->info;
                        memcpy(info->key_id,
                            encodings[0].encryption.key_id.data,
                            encodings[0].encryption.key_id.size);
                    }
        ...
        if (track->type == MATROSKA_TRACK_TYPE_VIDEO) {
            ...
            st->codecpar->codec_tag   = fourcc;
			//for sbr aac out-samplerate will 2times samplerate
			//fileplay will use samplerate to reconstruct aac head
            //st->codecpar->sample_rate = track->audio.out_samplerate;
            st->codecpar->sample_rate = track->audio.samplerate;
            //printf("\n[%s_%d]srate=%d\n",__func__,__LINE__,st->codecpar->sample_rate);
                                ...
                                int64_t discard_padding)
{
    ...
    /* encryption and 5 (AES) */
    int encrypted = 0;
    if (encodings && 1 == encodings->type &&
        WEBM_ENCRYPT_ALGO_AES == encodings->encryption.algo) {
        res = ff_matroska_parse_protection_meta(
            track->protection, &pkt_data, &pkt_size, &encrypted);
        if (res < 0) {
            return res;
        }
    }
    ...
    if (encrypted) {
        ...
        side_data = av_encryption_info_add_side_data(track->protection->info, &size);
        if (!side_data) {
            av_packet_unref(pkt);
            return AVERROR(ENOMEM);
        }
        res = av_packet_add_side_data(pkt, AV_PKT_DATA_ENCRYPTION_INFO, side_data, size);
        if (res < 0) {
            av_free(side_data);
            av_packet_unref(pkt);
            return res;
...
static int matroska_read_close(AVFormatContext *s)
{
    ...
    for (n = 0; n < matroska->tracks.nb_elem; n++) {
        ...
        if (tracks[n].protection) {
            ff_matroska_release_protection(&tracks[n].protection);
        }
    }
