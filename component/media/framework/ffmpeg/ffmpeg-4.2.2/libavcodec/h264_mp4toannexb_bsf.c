...
static int h264_mp4toannexb_filter(AVBSFContext *ctx, AVPacket *out)
{
    ...
    do {
        ...
        } else if (H264_NAL_UNSPECIFIED == unit_type && 0 == nal_size) {
            goto next_nal;
        }
        ...
        /* if only SPS has been seen, also insert PPS */
            ...
            if(s->new_idr && unit_type == H264_NAL_IDR_SLICE && s->idr_sps_seen && s->idr_pps_seen) {
                //above check if and else if, here is special case, mark sps and pps, but length is 0
                //printf("zx sps_offset %d, pps_offset %d\n",s->sps_offset, s->pps_offset);
                if ((ret=alloc_and_copy(out,
                               ctx->par_out->extradata, ctx->par_out->extradata_size,
                               buf, nal_size, 1)) < 0)
                goto fail;
