...
static void buffer_replace(AVBufferRef **dst, AVBufferRef **src)
{
    ...
    if (atomic_fetch_add_explicit(&b->refcount, -1, memory_order_acq_rel) == 1) {
        //add by zhouxiang!!!!
        //see av_buffer_create AVBufferRef->data equal to AVBufferRef->buffer->data
        //demux_lavf.c will set pkt.buf->data = NULL, therefore the momory will release by mplayer!
        if(data)
