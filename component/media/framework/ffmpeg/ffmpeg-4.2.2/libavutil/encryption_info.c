...
AVEncryptionInfo *av_encryption_info_clone(const AVEncryptionInfo *info)
{
    ...
    if (ret->subsamples && info->subsamples) {
...
#define FF_ENCRYPTION_INIT_INFO_EXTRA 20
    ...
    uint32_t system_id_size, uint32_t num_key_ids, uint32_t key_id_size, uint32_t data_size)
{
    ...
    if (num_key_ids) {
        info->key_ids = key_id_size ?
            av_mallocz_array(num_key_ids, sizeof(*info->key_ids)) : NULL;
    }
    ...
    const uint8_t *side_data, size_t side_data_size)
{
    ...
    uint64_t system_id_size, num_key_ids, key_id_size, data_size, header_data_size, i, j;
    ...
    for (i = 0; i < init_info_count; i++) {
        ...
        header_data_size = AV_RB32(side_data + 16);

        // UINT32_MAX + UINT32_MAX + UINT32_MAX * UINT32_MAX == UINT64_MAX
        if (side_data_size - FF_ENCRYPTION_INIT_INFO_EXTRA <
            system_id_size + data_size + num_key_ids * key_id_size + header_data_size) {
            av_encryption_init_info_free(ret);
            return NULL;
        }
        ...
        header_data = NULL;
        if (header_data_size) {
            header_data = av_mallocz(header_data_size);
        }
        ...
        if (header_data) {
            temp_info->header_data = header_data;
            temp_info->header_data_size = header_data_size;
            memcpy(info->header_data, side_data, header_data_size);
        }
        side_data += header_data_size;
        side_data_size -= header_data_size;
    }
...
uint8_t *av_encryption_init_info_add_side_data(const AVEncryptionInitInfo *info, size_t *side_data_size)
{
    ...
    for (cur_info = info; cur_info; cur_info = cur_info->next) {
        temp_side_data_size += (uint64_t)FF_ENCRYPTION_INIT_INFO_EXTRA +
            cur_info->system_id_size + cur_info->data_size + cur_info->header_data_size;
    ...
    *side_data_size = temp_side_data_size;
    ...
    for (cur_info = info; cur_info; cur_info = cur_info->next) {
        ...
        AV_WB32(cur_buffer + 16, cur_info->header_data_size);
        cur_buffer += FF_ENCRYPTION_INIT_INFO_EXTRA;
        ...
        memcpy(cur_buffer, cur_info->header_data, cur_info->header_data_size);
        cur_buffer += cur_info->header_data_size;
    }
