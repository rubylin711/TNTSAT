...
static SiiLibSeqTimeMs_t sSequencerHandler( void );
...
SiiInst_t SiiLibSeqTimerCreate( const char* pNameStr, SiiLibSeqCbFunc_t cbFunc, SiiInst_t parentInst, SiiLibSeqPriority_t priority )
{
	Obj_t* pObj = NULL;
...
void SiiLibSeqTimerStart( SiiInst_t inst, SiiLibSeqTimeMs_t timeMs, SiiLibSeqTimeMs_t periodMs )
{
	...
	pObj->timePrv = SiiLibTimeMilliGet();
	pObj->timeRun = timeMs;
	pObj->timeSet = periodMs;
	//printf("\nTstart:%p,%u,%u,%u\n",pObj,pObj->timePrv,pObj->timeRun,pObj->timeSet);
}
...
void SiiLibSeqAbort( void )
{
	...
	bool_t sequencerLoop;
#else
	bool_t sequencerLoop = false;
#endif
...
static void sPriorityUpdate( Obj_t* pObj )
{
	...
	if ( (pTmpObj) && pTmpObj->priority > pObj->priority ) {
		...
		do {
			...
			pTmpObj = (Obj_t*)SII_LIB_OBJ_NEXT(pTmpObj);
...
static SiiLibSeqTimeMs_t sSequencerHandler( void )
{
	...
	SiiLibSeqTimeMs_t  timeNew    = SiiLibTimeMilliGet();
	SiiLibSeqTimeMs_t  timeWakeUp = STOP_VALUE;

	/* Search timer list for timeouts */
	while ( pObj ) {
		timeNew = SiiLibTimeMilliGet(); //Required to get the new time every time.
		spCurObj = pObj;
		...
		if ( STOP_VALUE > pObj->timeRun ) {
			...
			//if ( pObj->priority == 254 )
			//printf("Rckt: %p,%u,%u,%u,%u,%u\n",pObj,pObj->timeRun,timeDiff,pObj->timeSet,pObj->timePrv,timeNew);
			if ( timeNew < pObj->timePrv ) {
				timeDiff = 0;
				//printf("[pri:%u]Skip one time...  Rckt: %p,%u,%u,%u,%u,%u\n",pObj->priority,pObj,pObj->timeRun,timeDiff,pObj->timeSet,pObj->timePrv,timeNew);
			}
				...
				if ( pObj->bRealTime ) {
					if ( (STOP_VALUE != pObj->timeRun) && (timeTmp == pObj->timeRun) ) {
						if ( pObj->timeRun < sOverRunTime ) {
							SII_LIB_LOG_PRINT1(pObj, ("Real-Time violation by: %d msec!", sOverRunTime - pObj->timeRun));
