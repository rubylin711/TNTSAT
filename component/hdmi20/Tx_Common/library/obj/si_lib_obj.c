...
void* SiiLibObjCreate( SiiLibObjList_t* pInstList, const char* pClassStr, const char* pInstStr, SiiLibObjSize_t size )
{
	...
	if ( *pInstList ) {
		...
		if ( pLst == NULL ) {
			//SII_LIB_LOG_PRINT2(("SiiLibObjCreate Malloc List fail\n"));
			return NULL;
		}
	...
	if ( pHdr ) {
		...
		//SI_HDMI20_PRINT("testobj:%p,name:%s\n",(((uint8_t*)pHdr) + sizeof(Hdr_t)),pHdr->pInstStr);

		return (((uint8_t*)pHdr) + sizeof(Hdr_t));
	} else {
		//SII_LIB_LOG_PRINT2(("SiiLibObjCreate Malloc Hdr fail\n"));
	}
	return NULL;
}
...
void* SiiLibObjPointerGetByName( SiiLibObjList_t instList, const char* pClassStr )
{
	if ( instList ) {
		...
		if ( pLst->pLstHdr ) {
			...
			do {
				if ( !SII_STRCMP(pHdr->pInstStr, pClassStr) ) {
...
static Hdr_t* sUfdHdrPtrByStrGet( const char* pClassStr, const char* pInstStr )
{
	if ( spLstFirst ) {
		...
		while ( pLst->pNxtLst ) {
			...
			if ( !SII_STRCMP(pLst->pClassStr, pClassStr) ) {
				do {
					if ( !SII_STRCMP(pHdr->pInstStr, pInstStr) ) {
						return pHdr;
					}
