...
typedef  struct {
	...
	uint8_t			HPol;			// true on negative polarity for horizontal pulses
	uint8_t			VPol;			// true on negative polarity for vertical pulses
	uint8_t			NtscPal;		// 60/120/240Hz (1) or 50/100/200Hz (2) or unknown (0) TVs
...
void SiiLibVideoTimingDetailGet(SiiLibVideoVM_t vM, SiiLibVideoMod3D_t mode3D, bool_t perField, SiiLibVideoTiming_t* pTiming)
{
	...
	SI_HDMI20_PRINT("[%s_%d]ivM=%d\n tbl==>vM=%d,freq=%d\npicar=%d,4x3=%d,16x9=%d,vic=%d,[%d,%d] [%d,%d] ,[%d,%d] [%d,%d][%d %d %d %d %d %d]\n", __func__, __LINE__, (uint32_t)vM, (uint32_t)sVideoModeTable[vM].vM, \
					(uint32_t)sVideoModeTable[vM].pixelFrq, \
					(uint32_t)sVideoModeTable[vM].pictAr, \
					(uint32_t)sVideoModeTable[vM].Vic4x3, \
					(uint32_t)sVideoModeTable[vM].Vic16x9, \
					(uint32_t)sVideoModeTable[vM].HdmiVic, \
					(uint32_t)sVideoModeTable[vM].Active.H, (uint32_t)sVideoModeTable[vM].Active.V, \
					(uint32_t)sVideoModeTable[vM].Total.H, (uint32_t)sVideoModeTable[vM].Total.V, \
					(uint32_t)sVideoModeTable[vM].SyncOffset.H, (uint32_t)sVideoModeTable[vM].SyncOffset.V, \
					(uint32_t)sVideoModeTable[vM].SyncWidth.H, (uint32_t)sVideoModeTable[vM].SyncWidth.V, \
					(uint32_t)sVideoModeTable[vM].Interlaced, \
					(uint32_t)sVideoModeTable[vM].HPol, (uint32_t)sVideoModeTable[vM].VPol, \
					(uint32_t)sVideoModeTable[vM].NtscPal, \
					(uint32_t)sVideoModeTable[vM].Repetition, \
					(uint32_t)sVideoModeTable[vM].MaxAudioSR8Ch);
	if ( sIsCorrectVm(vM) ) {
