...
#define SII_LIB_SEQ_PRIOR__DRV_SCALER 1
	...
	pObj->seqInst = SII_LIB_SEQ_TIMER_CREATE("Scaler", sUpdateHandler, SII_LIB_OBJ_INST(pObj), SII_LIB_SEQ_PRIOR__DRV_SCALER);
...
#define VERTICAL_FRAC_BITS    17
	...
	#else
	//vtg settings
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_HORIZONTAL_SYNC_END, (uint16_t)out_horizontal_sync_end);	            // vtg end of hs pulse-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_HORIZONTAL_ACTIVE_VIDEO_START, (uint16_t)out_horizontal_active_video_start);	// vtg horizontal active line start-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_HALFLINE, (uint16_t)out_halfline);	                        // vtg half line-2
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_HORIZONTAL_ACTIVE_VIDEO_END, (uint16_t)out_horizontal_active_video_end);	    // vtg horizontal active line end-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_END_OF_LINE, (uint16_t)out_horizontal_end_of_line);	        // vtg end of horizontal line-2
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_VERTICAL_SYNC_END, (uint16_t)out_vertical_sync_end);	            // vtg end of vs pulse (in half lines)-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_TRIGGER_START, (uint16_t)out_trigger_start);	                // vtg sc trigger point (in half lines)-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_VERTICAL_ACTIVE_VIDEO_START, (uint16_t)out_vertical_active_video_start);	    // vtg vertical active start (in half lines)-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_VERTICAL_ACTIVE_VIDEO_END, (uint16_t)out_vertical_active_video_end);	    // vtg vertical active end (in half lines)-1
	SiiDrvCraWrReg16(pObj->instTxCra, REG_ADDR__VG__VIDGEN_VERTICAL_END_OF_FRAME, (uint16_t)out_vertical_end_of_frame);	        // vtg end of frame (in half lines)-1

	SiiDrvCraWrReg8(pObj->instTxCra, REG_ADDR__VG__VIDGEN_UPDATE_REQUEST, 0x01);	// vtg update request
	//SiiDrvCraWrReg8(pObj->instTxCra, 0x09fc, 0x10);	// force vtg unlock mode, need to force unlock to use Pll
	//Checked register map, there's one unlock in 0x3B7A [2], so comments out first
	//SiiDrvCraSetBit8(pObj->instTxCra, REG_ADDR__VP__VTG_CONFIG, BIT_MSK__VP__VTG_CONFIG__UNLOCK);

	//===== enable tpg =====
	SiiDrvCraWrReg8(pObj->instTxCra,  REG_ADDR__VG__VIDGEN_CONFIG, 0x01);		//enable vidgen using VTG timing
	#endif
}
