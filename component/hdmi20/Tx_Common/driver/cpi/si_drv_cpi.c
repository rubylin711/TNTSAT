...
//
// Copyright 2002-2012, Deco, Inc.  All rights reserved.
//***************************************************************************/
...
//------------------------------------------------------------------------------
SiiInst_t SiiDrvCpiCreate (SiiInst_t inst, uint16_t baseAddr, CecCallBackPtr_t cecCbFunc )
{
...
{
	CpiInstanceData_t *pCpiInstance = (CpiInstanceData_t *)SII_LIB_OBJ_PNTR(inst);
	SiiLibSeqTimerStop(pCpiInstance->cpiInstTimer);//cec maybe send/recevie msg,so must stop the timer and wait firstly.
	SiiLibTimeMilliDelay(200);//make sure cec out of SiiDrvCecInterruptHandler and SiiDrvCecInterruptHandler->cecCbFunc
...
void SiiDrvCpiDisable ( SiiInst_t inst )
{
	...
	//   SiiDrvCraPutBit8(pCpiInstance->instTxCra,  pCpiInstance->baseAddr | REG_ADDR__AON_SRST2, BIT_MASK__AON_SRST2__CECSWRST, SET_BITS );     // Disable RX CEC
	SiiDrvCraWrReg8(pCpiInstance->instTxCra,  pCpiInstance->baseAddr | REG_ADDR__CEC_CONFIG_CPI, CLEAR_BITS );
	...
	SiiDrvCraPutBit8(pCpiInstance->instTxCra,  pCpiInstance->baseAddr | REG_ADDR__CEC_INT_ENABLE_0, BIT_MSK__CEC_INT_STATUS_0__INTRP_RX_FIFO_NEMPTY | BIT_MSK__CEC_INT_STATUS_0__INTRP_TX_FIFO_EMPTY | BIT_MSK__CEC_INT_STATUS_0__INTRP_TX_FF_CSTATE, CLEAR_BITS );
	SiiDrvCraPutBit8(pCpiInstance->instTxCra,  pCpiInstance->baseAddr | REG_ADDR__CEC_INT_ENABLE_1, BIT_MSK__CEC_INT_STATUS_1__INTRP_FRME_RETX_CNT | BIT_MSK__CEC_INT_STATUS_1__INTRP_SHRT_PULSE_DT | BIT_MSK__CEC_INT_STATUS_1__INTRP_START_ODD_BIT | BIT_MSK__CEC_INT_STATUS_1__INTRP_RX_FIFO_OVRUN, CLEAR_BITS );
	pCpiInstance->statusFlags   |= SiiCPI_CEC_DISABLED;
	SiiLibSeqTimerStop(pCpiInstance->cpiInstTimer);//cec maybe send/recevie msg,so must stop the timer and wait firstly.
}
...
bool_t SiiDrvCpiStandby ( SiiInst_t inst )
{
	...
	for ( i = 0; i < 32; i++ ) {
		SiiDrvCraWrReg8(pCpiInstance->instTxCra, (uint16_t)(pCpiInstance->baseAddr |  (REG_ADDR__CEC_OP_ABORT_0 + i)), 0xFF );
	}

	SiiDrvCraWrReg8(pCpiInstance->instTxCra, (uint16_t)(pCpiInstance->baseAddr |  REG_ADDR__CEC_OP_ABORT_0),    (uint8_t)~0x10 );    // CECOP_IMAGE_VIEW_ON
	SiiDrvCraWrReg8(pCpiInstance->instTxCra, (uint16_t)(pCpiInstance->baseAddr |  REG_ADDR__CEC_OP_ABORT_1),    (uint8_t)~0x20 );    // CECOP_TEXT_VIEW_ON

	// Everybody must respond to GIVE_DEVICE_POWER_STATUS
	SiiDrvCraWrReg8(pCpiInstance->instTxCra, (uint16_t)(pCpiInstance->baseAddr |  REG_ADDR__CEC_OP_ABORT_17), (uint8_t)~0x80 );              // CECOP_GIVE_DEVICE_POWER_STATUS
...
bool_t SiiDrvCpiResume ( SiiInst_t inst )
{
	...
	for ( i = 0; i < 32; i++ ) {
		SiiDrvCraWrReg8(pCpiInstance->instTxCra, (uint16_t)(pCpiInstance->baseAddr |  (REG_ADDR__CEC_OP_ABORT_0 + i)), 0x00 );
	}
	...
	do {
		...
		if (pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].txState == SiiTX_SENDING) {
			...
			if (( mSec - pCpiInstance->msgQueueOut.msStart ) >= pCpiInstance->msgQueueOut.msTimeout ) {
				SII_LIB_LOG_DEBUG2( ("CEC Message time [end:%02X][start:%02X][timeout:%02X]\n", mSec, pCpiInstance->msgQueueOut.msStart, pCpiInstance->msgQueueOut.msTimeout));
				...
				if (!DrvIsOpcodeSendPing(pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex].opcode) ) {
					SII_LIB_LOG_DEBUG2( ("%s:CEC Message [W%02X][%02X] send timeout!\n",
										 "RX",
		...
		if (pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].txState != SiiTX_WAITCMD) {
			#if 0
			SII_LIB_LOG_DEBUG2( ("CEC: Wait CMD Sent... In[%d 0x%x 0x%x 0x%x %d] Out[%d 0x%x 0x%x 0x%x %d]\n",pCpiInstance->msgQueueOut.inIndex,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].opcode,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].args[0],\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].srcDestAddr,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].txState,\
				pCpiInstance->msgQueueOut.outIndex,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].opcode,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].args[0],\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].srcDestAddr,\
		...
		if ( DrvIsOpcodeSendPing(pOutMsg->opcode) ) {
			HDMI_DEBUG_TRACE("nSiiCpiWrite:: cec.ping 0x%x\n",pOutMsg->srcDestAddr);
			...
			#if 0
			SII_LIB_LOG_DEBUG2( ("CEC: Send Poll Msg... In[%d 0x%x 0x%x %d] Out[%d 0x%x 0x%x %d]\n",pCpiInstance->msgQueueOut.inIndex,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].opcode,pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].srcDestAddr,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].txState,\
				pCpiInstance->msgQueueOut.outIndex,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].opcode,pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].srcDestAddr,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.outIndex ].txState));
			#endif
		} else {
			#if 0
			SII_LIB_LOG_DEBUG2( ("CEC: Send Common Msg... In[%d 0x%x 0x%x 0x%x %d] Out[%d 0x%x 0x%x 0x%x %d]\n",
			    pCpiInstance->msgQueueOut.inIndex,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].opcode,\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].args[0],\
				pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].srcDestAddr,\
			...
			SII_LIB_LOG_DEBUG1(pCpiInstance, ("\nSiiCpiWrite:: Out[0x%x 0x%x %d 0x%x\n",pOutMsg->opcode,pOutMsg->srcDestAddr,pOutMsg->argCount,pOutMsg->args[0]) );
			HDMI_DEBUG_TRACE("nSiiCpiWrite:: cec.Out[0x%x 0x%x %d 0x%x,0x%x,0x%x,0x%x\n",
				pOutMsg->opcode,pOutMsg->srcDestAddr,
				pOutMsg->argCount,pOutMsg->args[0],pOutMsg->args[1],pOutMsg->args[2],pOutMsg->args[3]);
...
uint16_t SiiDrvCpiWrite( SiiInst_t inst, SiiCpiData_t *pMsg )
{
	...
	if (pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].txState == SiiTX_IDLE ) {
		...
		#if 0
		SII_LIB_LOG_DEBUG1( pCpiInstance, ("CEC: SiiDrvCpiWrite -> add msg to queue: In[%d 0x%x 0x%x %d] Out[%d 0x%x 0x%x %d]\n",pCpiInstance->msgQueueOut.inIndex,\
			pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].opcode,\
			pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].srcDestAddr,\
			pCpiInstance->msgQueueOut.queue[pCpiInstance->msgQueueOut.inIndex ].txState,\
			pCpiInstance->msgQueueOut.outIndex,\
...
void SiiDrvCecInterruptHandler( SiiInst_t inst )
{
	...
	if ( (cecStatus[0] & 0x7F) || cecStatus[1] ) {
		...
		//SII_LIB_LOG_DEBUG2( ("\n[%s_%d] intr0 3fa6=0x%x,intr1 3fa7 =0x%x\n", __func__, __LINE__, (uint32_t)(cecStatus[0] & 0x7F), (uint32_t)(cecStatus[1])) );
	...
	pCpiInstance->cecCbFunc(SII_LIB_OBJ_INST(pCpiInstance));  //call -> SiiCecCallBack
}
