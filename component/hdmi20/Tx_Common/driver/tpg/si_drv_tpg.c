...
SiiInst_t SiiDrvTpgCreate(SiiDrvTpgConfig_t* pConfig)
{
	...
	pObj->fAlphaBS      = 0.0;
	#endif
	pObj->seqInst       = SII_LIB_SEQ_TIMER_CREATE("BlueScrnTimeOut", sBlack2BlueScreenHandler, 0, 2);
...
void SiiDrvTpgTimingSet( SiiInst_t inst, SiiDrvTpgVM_t vm )
{
	...
	switch ( pObj->timing.pictAr ) {
		case SII_LIB_VIDEO_PICTAR__4X3  :
			pObj->fPictAspRat = SII_LIB_VIDEO_ASPRAT__133;
			break;
		case SII_LIB_VIDEO_PICTAR__16X9 :
			pObj->fPictAspRat = SII_LIB_VIDEO_ASPRAT__178;
			break;
		case SII_LIB_VIDEO_PICTAR__SQP  :
			pObj->fPictAspRat = (float)pObj->timing.hActive / (float)pObj->timing.vActive;
		default :
			break;
	}
...
static void sUpdatePattern( TpgObj_t* pObj )
{
	...
	if ( SII_DRV_TPG_MODE__PATTERN == pObj->mode ) {
		...
		switch ( pObj->pattern ) {
			...
			case SII_DRV_TPG_PATTERN__FRMGEOM         :
				sSetFrmGeomPattern(pObj);
				break;
			case SII_DRV_TPG_PATTERN__CHECKERBOARD	  :
				sSetCheckerBoard(pObj);
				break;
			case SII_DRV_TPG_PATTERN__WINDOWIRE0      :
				sSetFlexWindowPattern(pObj, 0.00, 0.00, 0.00, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__256GRAYRAMP     :
				sSet256GrayRamp(pObj);
				break;
			case SII_DRV_TPG_PATTERN__FULLWHITE75     :
				sSetFlexWindowPattern(pObj, 0.75, 0.75, 0.75, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__FULLRED75       :
				sSetFlexWindowPattern(pObj, 0.75, 0.00, 0.00, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__FULLGREEN75     :
				sSetFlexWindowPattern(pObj, 0.00, 0.75, 0.00, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__FULLBLUE75      :
				sSetFlexWindowPattern(pObj, 0.00, 0.00, 0.75, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__FULLCYAN75      :
				sSetFlexWindowPattern(pObj, 0.00, 0.75, 0.75, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__FULLMAGENTA75   :
				sSetFlexWindowPattern(pObj, 0.75, 0.00, 0.75, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__FULLYELLOW75    :
				sSetFlexWindowPattern(pObj, 0.75, 0.75, 0.00, 0.5);
				break;
			case SII_DRV_TPG_PATTERN__SIMP92    	  :
				sSetSimp92(pObj);
				break;
			case SII_DRV_TPG_PATTERN__CLR8BARS100     :
				sSetColor8Bars100(pObj);
				break;
...
#define    L16   ((uint8_t)(16*L1))
	...
	uint16_t  Lv = (Y % B) ? (3) : (2); /* For Odd number ver. resolutions */
	uint16_t i;
	...
	for ( i = 1; i <= X; i++ ) {
		...
		/* Generate pixels (line 0)*/
		if     ( i > (A - L16 + L12 + L3 + Lh + L3 + L12 + A - L5 - L16)     ) {
			Data |= (1UL << (0 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (A - L16 + L12 + L3 + Lh + L3 + L12)              ) {
			Data |= (2UL << (0 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (A - L16 + L12 + L3 + Lh + L3)                  ) {
			Data |= (0UL << (0 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (A - L16 + L12 + L3 + Lh)                     ) {
			Data |= (2UL << (0 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (A - L16 + L12 + L3)                        ) {
			Data |= (1UL << (0 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (A - L16 + L12)                           ) {
			Data |= (2UL << (0 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (A - L16)                               ) {
		...
		if     ( i > (L1 + L4 + A - L5 - L7 + L6 + Lh + L6 + A - L5 - L7 + L4)   ) {
			Data |= (2UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L1 + L4 + A - L5 - L7 + L6 + Lh + L6 + A - L5 - L7)      ) {
			Data |= (1UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L1 + L4 + A - L5 - L7 + L6 + Lh + L6)              ) {
			Data |= (0UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L1 + L4 + A - L5 - L7 + L6 + Lh)                 ) {
			Data |= (2UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L1 + L4 + A - L5 - L7 + L6)                    ) {
			Data |= (1UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L1 + L4 + A - L5 - L7)                       ) {
			Data |= (2UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L1 + L4)                               ) {
			Data |= (0UL << (1 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > L1                                    ) {
		...
		if     ( i > (L2 + L3 + A - L5 - L10 + L9 + Lh + L9 + A - L5 - L10 + L3) ) {
			Data |= (2UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L3 + A - L5 - L10 + L9 + Lh + L9 + A - L5 - L10)    ) {
			Data |= (3UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L3 + A - L5 - L10 + L9 + Lh + L9)             ) {
			Data |= (0UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L3 + A - L5 - L10 + L9 + Lh)                ) {
			Data |= (2UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L3 + A - L5 - L10 + L9)                   ) {
			Data |= (1UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L3 + A - L5 - L10)                      ) {
			Data |= (2UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L3)                               ) {
			Data |= (0UL << (2 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > L2                                    ) {
		...
		if     ( i > (L2 + L1 + L2 + A - L5 - L13 + L12 + Lh + L12 + A - L5 - L13 + L2 + L1)    ) {
			Data |= (2UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2 + A - L5 - L13 + L12 + Lh + L12 + A - L5 - L13 + L2)       ) {
			Data |= (0UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2 + A - L5 - L13 + L12 + Lh + L12 + A - L5 - L13)          ) {
			Data |= (3UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2 + A - L5 - L13 + L12 + Lh + L12)                   ) {
			Data |= (0UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2 + A - L5 - L13 + L12 + Lh)                       ) {
			Data |= (2UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2 + A - L5 - L13 + L12)                          ) {
			Data |= (1UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2 + A - L5 - L13)                              ) {
			Data |= (2UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1 + L2)                                       ) {
			Data |= (0UL << (3 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L1)                                          ) {
		...
		if     ( i > (L2 + L2 + L1 + A - L5 - L16 + L15 + Lh + L15 + A - L5 - L16 + L1 + L2)   ) {
			Data |= (2UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1 + A - L5 - L16 + L15 + Lh + L15 + A - L5 - L16 + L1)      ) {
			Data |= (0UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1 + A - L5 - L16 + L15 + Lh + L15 + A - L5 - L16)         ) {
			Data |= (3UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1 + A - L5 - L16 + L15 + Lh + L15)                  ) {
			Data |= (0UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1 + A - L5 - L16 + L15 + Lh)                      ) {
			Data |= (2UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1 + A - L5 - L16 + L15)                         ) {
			Data |= (1UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1 + A - L5 - L16)                             ) {
			Data |= (2UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2 + L1)                                      ) {
			Data |= (0UL << (4 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (L2 + L2)                                         ) {
		...
		if     ( i > (T + L1 + G + L1 + A - G - T - L3 + Lh + A - G - T - L3 + L1 + G + T)         ) {
			Data |= (1UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1 + A - G - T - L3 + Lh + A - G - T - L3 + L1 + G + L1)        ) {
			Data |= (0UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1 + A - G - T - L3 + Lh + A - G - T - L3 + L1 + G)           ) {
			Data |= (1UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1 + A - G - T - L3 + Lh + A - G - T - L3 + L1)             ) {
			Data |= (4UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1 + A - G - T - L3 + Lh + A - G - T - L3)                ) {
			Data |= (1UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1 + A - G - T - L3 + Lh)                         ) {
			Data |= (0UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1 + A - G - T - L3)                            ) {
			Data |= (1UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G + L1)                                     ) {
			Data |= (0UL << (5 * TPG_INDEX_NUM_OF_BITS));
		} else if ( i > (T + L1 + G)                                        ) {
...
static void sSet256GrayRamp( TpgObj_t* pObj )
{
	...
	uint16_t step = (256 << TPG_INDEX_NUM_OF_BITS) / H;
	/*uint16_t count=0;*/
	uint16_t gray = 0;
	uint16_t i;
...
static void sSetSimp92( TpgObj_t* pObj )
{
	...
	for (i = 0; i < (uint16_t)(1 << tpg_index_bits); i++) {
		...
		sInt2Float(&Rgb);
		sSetColorLut(pObj, (uint8_t)(bank_start + i), &Rgb);
	}
	...
	Rgb.fB = (float)(255 << tpg_bit_shift);
	sInt2Float(&Rgb);
	sSetColorLut(pObj, (uint8_t)(bank_start + 11), &Rgb);

	Rgb.fR = (float)(209 << tpg_bit_shift);
	Rgb.fG = (float)(0   << tpg_bit_shift);
	Rgb.fB = (float)(255 << tpg_bit_shift);
	sInt2Float(&Rgb);
	sSetColorLut(pObj, (uint8_t)(bank_start + 12), &Rgb);

	Rgb.fR = (float)(255 << tpg_bit_shift);
	Rgb.fG = (float)(0   << tpg_bit_shift);
	Rgb.fB = (float)(255 << tpg_bit_shift);
	sInt2Float(&Rgb);
	sSetColorLut(pObj, (uint8_t)(bank_start + 13), &Rgb);

	Rgb.fR = (float)(255 << tpg_bit_shift);
	Rgb.fG = (float)(0   << tpg_bit_shift);
	Rgb.fB = (float)(229 << tpg_bit_shift);
	sInt2Float(&Rgb);
	sSetColorLut(pObj, (uint8_t)(bank_start + 14), &Rgb);

	Rgb.fR = (float)(255 << tpg_bit_shift);
	Rgb.fG = (float)(0 << tpg_bit_shift);
	Rgb.fB = (float)(204 << tpg_bit_shift);
	sInt2Float(&Rgb);
	sSetColorLut(pObj, (uint8_t)(bank_start + 15), &Rgb);
	...
	for ( i = 1; i <= X; i++ ) {
		...
		// on/off gray #0
		if     ( i > ((104UL * X) / 128UL)) {
			Data |= (0 + (i - 1) % 2) << (1 * tpg_index_bits);    // white/black alternating
		} else if ( i > (( 24UL * X) / 128UL)) {
			Data |=  2            << (1 * tpg_index_bits);    // gray
		} else {
			Data |=  0            << (1 * tpg_index_bits);    // white
		}

		/* Generate pixels (pattern line 2)*/
		// on/off gray #1
		if   ( i >   ((104UL * X) / 128UL)) {
			Data |= (0 + (i - 1) % 2)   << (2 * tpg_index_bits);    // white/black alternating
		} else if ( i > (( 24UL * X) / 128UL)) {
			Data |= 2             << (2 * tpg_index_bits);    // gray
		} else {
			Data |= 1             << (2 * tpg_index_bits);    // black
		}

		/* Generate pixels (pattern line 3)*/
		// CGM
		if     ( i > (104UL * X / 128)) {
			Data |= 2 << (3 * tpg_index_bits);    // M
		} else if ( i > ( 24UL * X / 128)) {
			Data |= 1 << (3 * tpg_index_bits);    // G
		} else {
			Data |= 0 << (3 * tpg_index_bits);    // C
		}
