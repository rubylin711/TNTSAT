...
#ifdef VMX_OTT_SVP
extern "C" {
void *MT_VMX_OTT_AllocateBuffer(unsigned int xBufferSize, unsigned int *outphyaddr);
int MT_VMX_OTT_FreeBuffer(unsigned char *pxBuffer);
int MT_VMX_Sym4CryptoOTTConfig(unsigned int *pcrypto_handle, unsigned char *p_iv);
int MT_VMX_Sym4CryptoOTTdestroy(unsigned int crypto_handle);

unsigned int MT_VMX_OTT_Start( unsigned char bR2REngineIndex,
			unsigned char bR2RMode,
			unsigned char *pabDataIn,
			unsigned char *pabDataOut,
			unsigned int lDataLen);
unsigned int MT_VMX_OTT_FetchKey( unsigned char bR2REngineIndex,
			unsigned int iKeySequence,
			char * pabKeyfileURL,
			unsigned char * pabKeyIV,
			unsigned int iKeyIVSize );
unsigned int MT_VMX_OTT_InstallKey( unsigned char bR2REngineIndex,
			unsigned int iKeySequence );
unsigned short mt_catac_data_buf_register(unsigned char *addr1, unsigned int size1, unsigned char *addr2,
		unsigned int size2);
signed int mt_unf_cipher_crypto_async_start(unsigned int crypto);
signed int mt_unf_cipher_crypto_async_wait(unsigned int crypto);
signed int mt_unf_cipher_crypto_async_request(unsigned int crypto, const unsigned char *src, unsigned char *dst,
		unsigned int count,  unsigned int clear_length,  unsigned int protected_length);
int MT_VMX_OTT_SettedFlag();
void *MT_VMX_OTT_Malloc(unsigned int num);

unsigned int MT_VMXGetTAMallocPhyAddr(unsigned int vir_addr);
//unsigned int sleep(unsigned int seconds);
int MT_VMX_DRM_AddAAHeader(int in_size, int aud_sample_rate, int aud_channels, unsigned char * out_buf);
}
#endif
/*----------------------------------------------------------------------
|   constants
                                      ...
                                      AP4_CencSingleSampleDecrypter*& decrypter)
{
    ...
    if (crypt_byte_block && skip_byte_block) {
        stream_cipher = new AP4_PatternStreamCipher(stream_cipher, crypt_byte_block, skip_byte_block);
        stream_cipher->m_isPatternMode = 1;
    }
                                                 ...
                                                 int             eDrmIndex)
{
    ...
    if(encrypt_type)
    {
        data_out.SetBufferSize(data_in.GetDataSize());

        DMTRM_BUFFER_IN  dataIn;
        DMTRM_BUFFER_OUT dataOut;

        memset(&dataIn, 0, sizeof(dataIn));
        memset(&dataOut, 0, sizeof(dataOut));

        if (m_Cipher->m_isPatternMode)
        {
          AP4_PatternStreamCipher *pCipher = (AP4_PatternStreamCipher *)m_Cipher;
          dataIn.patternEncrypt  = pCipher->m_CryptByteBlock;
          dataIn.patternClear    = pCipher->m_SkipByteBlock;
        }

        dataIn.data = (mt_u8*)data_in.GetData();
        dataIn.data_length = data_in.GetDataSize();
        dataIn.iv = (mt_u8*)iv;
        dataIn.key_index = ex_param;

        if (subsample_count)
        {

          dataIn.region_count = subsample_count;
          dataIn.clear   = (mt_u16*)bytes_of_cleartext_data;
          dataIn.encrypt = (mt_u32*)bytes_of_encrypted_data;
        ...
        {
          cleartext[0] = 0;
          encrypted[0] = (uint32_t)data_in.GetDataSize();
          //printf("encrypted[0] %d\n", encrypted[0]);
          dataIn.region_count = 1;
          dataIn.clear = cleartext;
          dataIn.encrypt = encrypted;
        }
        #endif

#ifdef DRM_SMP_ENABLE
        DMTRM_BUFFER_OUT newbuf = {0,};
        if(MTDrm_MallocSecureMemory(&newbuf, data_in.GetDataSize()) != MTDRM_SUCCESS)
        {
            printf("%s malloc secure memory failed!\n",__func__);
            return AP4_ERROR_OUT_OF_MEMORY;
        }
#if 0
        printf("wayne set buffer,&newbuf.data = 0x%08x,  &newbuf.secure = 0x%08x!\n", &newbuf.data, &newbuf.is_secure);
        printf("wayne set buffer,newbuf.data = 0x%08x,  newbuf.secure = 0x%08x!\n", newbuf.data, newbuf.is_secure);
        data_out.SetBuffer(newbuf.data, (data_in.GetDataSize()));
        MTDrm_DecryptSampleData(&dataIn, &newbuf, eDrmIndex);
#endif
        data_out.SetBuffer(newbuf.data, data_in.GetDataSize());
        MTDrm_DecryptSampleData(&dataIn, &newbuf, eDrmIndex);//sl3000 no mips
#elif MT_DRM_SUPPORT
        dataOut.data = (mt_u8*)data_out.GetData();
        data_out.SetDataSize(data_in.GetDataSize());
        MTDrm_DecryptSampleData(&dataIn, &dataOut, eDrmIndex);
#endif
	...
	// setup direct pointers to the buffers
	const AP4_UI08* in  = data_in.GetData();
	const AP4_UI08* in_bk = data_in.GetData();;
#if 0
	AP4_UI08*       out = data_out.UseData();
...
#else
        decrypt_buf_addr = malloc(data_in.GetDataSize());
        data_out.SetBuffer((AP4_Byte*)decrypt_buf_addr, data_in.GetDataSize());
        out = (AP4_UI08*       )decrypt_buf_addr;
#endif

#endif
#ifdef VMX_OTT_SVP
	 static unsigned int align_len = 0x160000;
	 static unsigned char *tmp_buf_addr_in = NULL;
	 static unsigned int tmp_utphyaddr_in = 0;
	 static unsigned int testFlag = 0;
	 //static unsigned char *decrypt_buf_addr = NULL;
	static unsigned int tmp_utphyaddr_dst = 0;
	//static unsigned int smp_start = 0;
	//static unsigned int smp_len = 0x600000;
	 unsigned j = 0;
	 unsigned int i=0;
	 unsigned int crypto_handle = 0;
	 unsigned int count = 0, count2 = 0;

	unsigned int clear_len = 0;
	int frame_data_size = 0;
	unsigned char* frame_buffer = NULL;
	unsigned char *decrypt_buf_addr22 = NULL;
	unsigned int tmp_utphyaddr_dst22 = 0;
	 //unsigned char *new_decrypt_buf_addr = NULL;
	 int pkt_size = 0;
	int pkt_size2 = 0;
	unsigned int iKeySequence = 1; //start from 1
	if (codec_type == 0) {
		pkt_size = data_in.GetDataSize() + 4096 +64;
	} else {
		pkt_size = data_in.GetDataSize() + 7 +64;
	}
	pkt_size2 = 0;
	if(tmp_buf_addr_in == NULL) {
		tmp_buf_addr_in = (unsigned char*)MT_VMX_OTT_AllocateBuffer(align_len, &tmp_utphyaddr_in);
		printf("%s, %d test tmp_utphyaddr_in = 0x%x \n", __FUNCTION__, __LINE__, tmp_utphyaddr_in);
	}
	if (pkt_size >= align_len) {
		printf("%s, %d tmp_buf_addr_in = 0x%x \n", __FUNCTION__, __LINE__, pkt_size);
		while(1);
	}
	if(tmp_buf_addr_in == NULL)
	{
	    printf("%s malloc secure memory failed!%d \n",__func__, __LINE__);
	    return AP4_ERROR_OUT_OF_MEMORY;
	}
#if 0
	 if (decrypt_buf_addr == NULL) {
    		decrypt_buf_addr = (unsigned char *)MT_VMX_OTT_AllocateBuffer(0x10000, &tmp_utphyaddr_dst);
		//printf("%s, %d tmp_utphyaddr_dst = 0x%x \n", __FUNCTION__, __LINE__, tmp_utphyaddr_dst);
		/*
		decrypt_buf_addr22 = (unsigned char *)MT_VMX_OTT_AllocateBuffer(smp_len, &tmp_utphyaddr_dst22);
		smp_start = (unsigned int)tmp_utphyaddr_dst22;
		printf("%s, %d test decrypt_buf_addr22 = 0x%x \n", __FUNCTION__, __LINE__, smp_start);
		MT_VMX_OTT_FreeBuffer(decrypt_buf_addr22);
		decrypt_buf_addr22 = NULL;
		tmp_utphyaddr_dst22 = 0;
		*/
  	}
        if(decrypt_buf_addr == NULL)
        {
	...
	#if 1
	decrypt_buf_addr22 = (unsigned char *)MT_VMX_OTT_Malloc(pkt_size);
	tmp_utphyaddr_dst22 = MT_VMXGetTAMallocPhyAddr((unsigned int)decrypt_buf_addr22);
	#else
		decrypt_buf_addr22 = (unsigned char *)MT_VMX_OTT_AllocateBuffer(pkt_size, &tmp_utphyaddr_dst22);
	#endif
	//printf("%s %d, dst22 =0x%x, 0x%x, 0x%x \n",__func__, __LINE__, decrypt_buf_addr22, pkt_size, tmp_utphyaddr_dst22);
	if(decrypt_buf_addr22 == NULL || tmp_utphyaddr_dst22 == 0)
	{
	    printf("%s malloc secure memory failed!%d \n",__func__, __LINE__);
	    return AP4_ERROR_OUT_OF_MEMORY;
	}
	/*
	if (((unsigned int)tmp_utphyaddr_dst22 < smp_start) || ((unsigned int)(tmp_utphyaddr_dst22 + pkt_size) > (smp_start + smp_len))) {
		printf("%s, %d smp overflow addr22 = 0x%x \n", __FUNCTION__, __LINE__, tmp_utphyaddr_dst22);
		while(1);
	}
	*/
        data_out.SetBuffer((AP4_Byte*)decrypt_buf_addr22, data_in.GetDataSize());
        out = (AP4_UI08*       )decrypt_buf_addr22;
	//printf("decrypt_buf_addr22= 0x%x \n", decrypt_buf_addr22);
	 if (subsample_count) {
		// process the sample data, one sub-sample at a time
		const AP4_UI08* in_end = data_in.GetData()+data_in.GetDataSize();
		/*
		printf("iv = %x %x %x %x %x %x %x %x\n",
		iv[0], iv[1], iv[2], iv[3], iv[4], iv[5], iv[6], iv[7]);
		printf("     %x %x %x %x %x %x %x %x\n",
		iv[8], iv[9], iv[10], iv[11], iv[12], iv[13], iv[14], iv[15]);
		*/

		//MT_VMX_OTT_Start_Pro(tmp_utphyaddr_dst, align_len);
		//printf("%s, %d tmp_utphyaddr_dst22 = 0x%x \n", __FUNCTION__, __LINE__, tmp_utphyaddr_dst22);


		if (MT_VMX_OTT_SettedFlag() == 0) {
			//printf("--%s, %d-ret = 0x%x\n", __FUNCTION__, __LINE__, ret2);
			mt_catac_data_buf_register(decrypt_buf_addr22, 0x800, tmp_buf_addr_in, 0x800);
			 ret2 = MT_VMX_OTT_Start(0x40, 2, tmp_buf_addr_in, decrypt_buf_addr22, 0x800);
			//printf("--%s, %d-ret = 0x%x\n", __FUNCTION__, __LINE__, ret2);
			iKeySequence = 1; //start from 1
			ret2 = MT_VMX_OTT_FetchKey( 0x40, iKeySequence, vmx_str, (unsigned char *)iv, 16);
			//printf("--%s, %d-ret = 0x%x\n", __FUNCTION__, __LINE__, ret2);
			ret2 |= MT_VMX_OTT_InstallKey(0x40, iKeySequence);
			//printf("--%s, %d-ret = 0x%x\n", __FUNCTION__, __LINE__, ret2);
			testFlag = 0;
			ret2 = MT_VMX_Sym4CryptoOTTConfig(&crypto_handle, NULL);
			//sleep(4); //Play before SMP openning, there are wrong data in paly buffer
			//VMXTest_PlayVedioStart();
		} else {
			//printf("--%s, %d-ret = 0x%x\n", __FUNCTION__, __LINE__, ret2);
			ret2 = MT_VMX_Sym4CryptoOTTConfig(&crypto_handle, (unsigned char *)iv);
			testFlag++;
		}
		...
		for (i=0; i<subsample_count; i++) {
			...
			// check bounds
			if ((unsigned int)(in_end-in_bk) < cleartext_size+encrypted_size) {
				return AP4_ERROR_INVALID_FORMAT;
			}
			frame_data_size = 0;
			//printf("--%s, %d size = 0x%x, 0x%x\n", __FUNCTION__,__LINE__, data_in.GetDataSize(), in_bk[nalu_length_size]);
			if (codec_type == 0) {
				bool have_access_unit_delimiter = (data_in.GetDataSize()  > nalu_length_size) && ((in_bk[nalu_length_size] & 0x1F) == 9);//AP4_AVC_NAL_UNIT_TYPE_ACCESS_UNIT_DELIMITER is 9
				//printf("%s, %d nalu_length_size = 0x%x\n", __FUNCTION__, __LINE__,nalu_length_size);
				//printf("%x, %x,  %x,  %x,  %x,  %x \n", in_bk[0], in_bk[1], in_bk[2], in_bk[3], in_bk[4], in_bk[5]);
				if ((i == 0) && !have_access_unit_delimiter) {
					frame_buffer = tmp_buf_addr_in + count + frame_data_size;
					...
					frame_data_size += 6;
					//printf("%s, %d, frame_buffer = 0x%x\n", __FUNCTION__, __LINE__, frame_buffer);
				}
				if ((i == 0) && prefix_len && pPrefix && !have_access_unit_delimiter) {
			            frame_buffer = tmp_buf_addr_in + count + frame_data_size;
			            memcpy(frame_buffer, pPrefix, prefix_len);
			            frame_data_size += prefix_len;
					//printf("%s, %d frame_buffer = %x\n", __FUNCTION__, __LINE__, frame_buffer);
					//printf("%x, %x,  %x,  %x,  %x,  %x \n",
				  	//pPrefix[0], pPrefix[1], pPrefix[2], pPrefix[3], pPrefix[4], pPrefix[5]);
			        }
				frame_buffer = tmp_buf_addr_in + count + frame_data_size;
				clear_len = frame_data_size + cleartext_size -1; //skip first one byte

				memcpy(frame_buffer, in_bk + count2 + 1, cleartext_size + encrypted_size -1);  //skip first one byte
				frame_data_size += cleartext_size + encrypted_size - 1;
				count2 += cleartext_size + encrypted_size;
				//skip first one byte
				int nalu_size = ( ((int)*(frame_buffer+0))<<16 ) | ( ((int)*(frame_buffer+1))<<8 ) | ( ((int)*(frame_buffer+2)) );
				if (nalu_size != encrypted_size + 1) {
					printf("--%s, %d- nalu_size = 0x%x,  0x%x,  0x%x \n", __FUNCTION__,__LINE__, nalu_size, cleartext_size, encrypted_size);
					while(1);
				}
				frame_buffer[0] = 0;
				frame_buffer[1] = 0;
				...
				*/
				...
				frame_buffer = tmp_buf_addr_in + 7;
				clear_len = cleartext_size + 7;
				memcpy(frame_buffer, in, cleartext_size+encrypted_size);
				// move the pointers and udate counters
				frame_data_size = MT_VMX_DRM_AddAAHeader(cleartext_size+encrypted_size, sample_rate, channels, tmp_buf_addr_in);
				if (subsample_count > 1) {
					printf("--%s, %d-Audio error =\n", __FUNCTION__,__LINE__);
					while(1);
				}
				/*
				printf("--%s, %d-ret = 0x%x, input count = 0x%x\n", __FUNCTION__,__LINE__, ret2, frame_data_size);
				out = (AP4_UI08*)tmp_buf_addr_in;
				for (j = 0; j < 23; j++) {
					printf("0x%x ", out[j]);
				}
				printf("\n444 tail: \n");
				for (j = 0; j < 16; j++) {
					printf("0x%x ", out[j + frame_data_size - 16]);
				}
				printf("\n");

				*/
			} else {
				printf("--%s, %d-0x%x, 0x%x codec_type = 0x%x ????\n", __FUNCTION__, __LINE__, codec_type);
				while(1);
			}

			ret2 =  mt_unf_cipher_crypto_async_request(crypto_handle,
							(unsigned char*)(tmp_utphyaddr_in + count),
							(unsigned char*)(tmp_utphyaddr_dst22 + count),
							1,
							clear_len,
							encrypted_size);

			count += clear_len + encrypted_size; //skip first one byte
			//printf("--%s, %d-ret = 0x%x, count = 0x%x\n", __FUNCTION__,__LINE__, ret2, count);

        	}
		ret2 = mt_unf_cipher_crypto_async_start(crypto_handle);
		ret2 |= mt_unf_cipher_crypto_async_wait(crypto_handle);
		ret2 |= MT_VMX_Sym4CryptoOTTdestroy(crypto_handle);

		data_out.SetBuffer((AP4_Byte*)decrypt_buf_addr22, pkt_size);
		data_out.SetDataSize(count);//
		//MT_VMX_OTT_FreeBuffer((unsigned char*)tmp_buf_addr_in);
		/*
		printf("--%s, %d-addr = 0x%x, 0x%x, 0x%x\n", __FUNCTION__, __LINE__,
		data_out.GetData(), data_out.GetDataSize(), data_out.GetBufferSize());
		*/
	/*
		if (codec_type) {
			printf("--%s, %d-ret = 0x%x, clear count = 0x%x\n", __FUNCTION__,__LINE__, ret2, count);
			out = (AP4_UI08*)decrypt_buf_addr22;
			for (j = 0; j < 23; j++) {
				printf("0x%x ", out[j]);
			}
	...
	*/
		...
		for (i = 0; i<subsample_count; i++) {
			AP4_UI16 cleartext_size = bytes_of_cleartext_data[i];
			AP4_UI32 encrypted_size = bytes_of_encrypted_data[i];
			printf("333 decrypted data: \n");
			for (j = 0; j < 64; j++) {
				printf("0x%x ", out[count2 + j]);
			}

			printf("\n444 decrypted data: \n");
			for (j = 0; j < 64; j++) {
				printf("0x%x ", new_decrypt_buf_addr[count2 + j]);
			}
			if (i == 0)
				count2 += 6 + 39 + cleartext_size+encrypted_size - 1;
			else
				count2 += cleartext_size+encrypted_size - 1;

			printf("\n33tail 16\n");
			for (j = 0; j < 16; j++) {
				printf("0x%x ", out[count2 + j - 16]);
			}
			printf("\n");
			printf("\n44tail 16\n");
			for (j = 0; j < 16; j++) {
				printf("0x%x ", new_decrypt_buf_addr[count2 + j - 16]);
			}
			printf("\n");
		...
		#endif
		//out += count;
            //MT_BC_DVRDecrypt(1, (unsigned char*)tmp_buf_addr_dst, (unsigned char*)in, cleartext_size+encrypted_size);//iv_block

            //MT_VMX_OTT_FreeBuffer((unsigned char*)tmp_buf_addr_dst);
	...
	printf("grant %s, %d, subsample_count = %d \n", __FUNCTION__, __LINE__, subsample_count);

	printf("grant vmx_str = %s \n", vmx_str);
	printf("iv = %x %x %x %x %x %x %x %x\n",
		iv[0], iv[1], iv[2], iv[3], iv[4], iv[5], iv[6], iv[7]);
	printf("     %x %x %x %x %x %x %x %x\n",
		iv[8], iv[9], iv[10], iv[11], iv[12], iv[13], iv[14], iv[15]);
	unsigned j = 0;
        ...
        for (unsigned int i=0; i<subsample_count; i++) {
            AP4_UI16 cleartext_size = bytes_of_cleartext_data[i];
            AP4_UI32 encrypted_size = bytes_of_encrypted_data[i];
            printf("%s %d cleartext_size %d, encrypted_size %d %d/%d\n",
                __func__, __LINE__, cleartext_size, encrypted_size, i,subsample_count);
		printf("cleardata: \n");
		for (j = 0; j < cleartext_size; j++) {
			printf("0x%x ", in[j]);
		}
		printf("\n encrypteddata: \n");
		for (j = 0; j < 16; j++) {
			printf("0x%x ", in[j + cleartext_size]);
		}
		printf("\n");
            // check bounds
            if ((unsigned int)(in_end-in) < cleartext_size+encrypted_size) {
            ...
            memcpy(out, in, cleartext_size+encrypted_size);
            // move the pointers and udate counters
                                           ...
                                           const AP4_UI08* iv)
{
    ...
    /*FILE *fp = fopen("d:\dp111.dat", "wb+");
    fwrite(data_in.GetData(), data_in.GetDataSize(), 1, fp);
    fclose(fp);*/
    m_SingleSampleDecrypter->DecryptSampleData(data_in, data_out, iv_block, subsample_count, bytes_of_cleartext_data, bytes_of_encrypted_data, 0, 0, NULL,
    0, NULL, 0, 0, 0, 0, 0);
                                           ...
                                           int             eDrmIndex)
{
    ...
    fwrite(data_in.GetData(), data_in.GetDataSize(), 1, fp);
    fclose(fp);*/
     m_SingleSampleDecrypter->DecryptSampleData(data_in, data_out, iv_block, subsample_count, bytes_of_cleartext_data, bytes_of_encrypted_data,encrypt_type, ex_param, vmx_str,
            codec_type, pPrefix, prefix_len, nalu_length_size, sample_rate, channels, eDrmIndex);
