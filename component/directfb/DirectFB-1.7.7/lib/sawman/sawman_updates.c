     ...
     if (i >= 0) {
          ...
          if (bin->size < 1) {
               ...
               if (!trans || (premult && (bin->size == bin->curr))) {
                    ...
                    /* single blend */
                    //	printf("<%s> : <%d> bin->curr : %d\n", __FUNCTION__, __LINE__, bin->curr );
                    if(bin->curr == 1)
                    {
                        state->enable_cmdfifo_sync_mode = true;
                    }
					...
					state->enable_cmdfifo_sync_mode = false;
               }
          }
     }
}
          ...
          case DLBM_BACKVIDEO:
               if (tier->region->config.options & DLOP_STEREO) {
                    ...
                    if (!dfb_config->wm_fullscreen_updates) {
                         /* Copy back the updated region. */
		//				 printf("----------------><%s> : <%d>\n", __FUNCTION__, __LINE__);
		                 CoreGraphicsStateClient *client =  &wmdata->client;
		                 client->state->enable_cmdfifo_sync_mode = true;
						 ...
						 client->state->enable_cmdfifo_sync_mode = false;
                    }
               }
               break;
