                   ...
                   CoreDFB *core)
{
    ...
    funcs->Blit2 = TDEBlit2;
#ifdef CONFIG_MT_TDE_CMD_FIFO_SUPPORT
    funcs->BatchBlit = TDEBatchBlit;
    funcs->BatchFill = TDEBatchFill;
#endif
    ...
    ret = MT_TDE2_Open();
    if (ret != MT_SUCCESS) {
        D_ERROR("[%s][%d]Error MT_TDE2_Open, ret:0x%x\n", __FUNCTION__, __LINE__, ret);
        return DFB_INIT;
    }
    // TODO: get buffer size via screen size.
    ret = MT_TDE_Create_Mid_Mem(1920 * 1080 * 4);
    if (ret != MT_SUCCESS) {
        D_ERROR("[%s][%d]Error MT_TDE2_Open, ret:0x%x\n", __FUNCTION__, __LINE__, ret);
        return DFB_INIT;
    }
...
driver_init_device(CoreGraphicsDevice *device,
                   ...
                   void *device_data)
{
    TDEDeviceData *tdev = (TDEDeviceData *)device_data;
    ...
    /* device limitations */
    device_info->limits.surface_byteoffset_alignment = 16 * 4;
    ...
    // ARGB * entries = 4 * 256
    tdev->lut_offset = dfb_gfxcard_reserve_memory(device, 4 * 256);
