...
#ifdef CONFIG_MT_MTGO_JPEG_SUPPORT
static MT_CHAR* pLeaveBuf = NULL;
static MT_S32 gs_s32LeaveByte = 0;
static MT_S32 gs_s32Readbytes = 0;
...
* others:	  : nothing
*****************************************************************************/
static MT_VOID duplicate_stream_info(j_decompress_ptr cinfo)
{
...
* others:	  : nothing
*****************************************************************************/
static MT_VOID resume_stream(j_decompress_ptr cinfo)
{
...
static MT_VOID record_readbytes_before_stardecompress(
	                                       const unsigned int bytes,\
	                                       MT_BOOL read_write,
	                                       MT_BOOL cleanzero)
{     
  
	  if(MT_FALSE == read_write)
	  {
		     if(MT_TRUE==cleanzero)
		     {
		        gs_s32Readbytes = 0;
		     }
			 else
...
#ifdef CONFIG_MT_MTGO_JPEG_SUPPORT		  
		  record_readbytes_before_stardecompress(0,MT_FALSE,MT_TRUE);
#endif		  
     }
     else 
	 {
...
#ifdef CONFIG_MT_MTGO_JPEG_SUPPORT		  
		  record_readbytes_before_stardecompress(nbytes,MT_FALSE,MT_FALSE);
#endif		  
     }
                                      ...
                                      const DFBRectangle     *dest_rect )
{
     ...
     if (!data->image) {
		  ...
		  if (setjmp( jerr.setjmp_buffer ))
		  {
		   ...
		   memset(&stJpegInfo,0,sizeof(MT_JPEG_INFO_S));
	...
	//                   		 cinfo.out_color_space = JCS_ABGR_8888;
		  ...
		  {
		  	 stSurfaceDesc.stOutSurface.pOutVir[0] = (MT_CHAR*)data->image;
			 stSurfaceDesc.stOutSurface.u32OutStride[0] = row_stride;
			 stSurfaceDesc.stOutSurface.bUserPhyMem = MT_FALSE;
			 MT_JPEG_SetOutDesc(&cinfo, &stSurfaceDesc);
		  }
		  row_ptr = data->image;

		  pLeaveBuf = (MT_CHAR*)calloc(1,JPEG_PROG_BUF_SIZE * sizeof (JOCTET));
		  if(NULL==pLeaveBuf)
		  {
			  return DFB_FAILURE;
		  }
		  ...
		  record_readbytes_before_stardecompress(0,MT_TRUE,MT_FALSE);
		  ...
		  if(jpeg_if_softhuff(&cinfo))
			  jpeg_start_soft_huff_decompress(&cinfo);
...
//			   dfb_scale_linear_32( data->image, row_stride/4, data->image_height,lock.addr, lock.pitch, &rect, dst_surface, &clip );

               if (data->base.render_callback) 
			   {
                    DFBRectangle r = { 0, 0, data->image_width, data->image_height };
//					DFBRectangle r = { 0, 0, row_stride/4, data->image_height };
                    cb_result = data->base.render_callback( &r,data->base.render_callback_context );
               }
          }
