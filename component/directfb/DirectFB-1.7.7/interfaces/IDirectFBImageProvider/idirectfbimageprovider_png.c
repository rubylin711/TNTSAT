...
#include "config.h"
#ifdef CONFIG_MT_HW_PNG_SUPPORT
#include "mt_common.h"
...
#ifdef CONFIG_MT_HW_PNG_SUPPORT
    mt_handle hwHandle;
    bool isUsingSurfaceBuffer;
    unsigned long image_phys;
#endif
...
#ifdef CONFIG_MT_HW_PNG_SUPPORT
static void png_read(png_structp png_ptr, png_bytep data, png_size_t size);
static void streamReset(void *userdata);
static mt_u32 streamGetLength(void *userdata);
static mt_s32 streamGetData(void *userdata, mt_u32 length, void *ret_data, mt_u32 *ret_read, MT_BOOL *eof);
static int load_png_info(IDirectFBImageProvider_PNG_data *data);
#endif
...
#ifdef CONFIG_MT_HW_PNG_SUPPORT
    MT_PNG_Close(data->hwHandle);
#endif
    png_destroy_read_struct(&data->png_ptr, &data->info_ptr, NULL);

#ifdef CONFIG_MT_HW_PNG_SUPPORT
    if (!data->isUsingSurfaceBuffer && data->image_phys != 0) {
        mt_mmz_unmap(data->image);
        mt_mmz_delete(data->image_phys);
    }
#else
...
#ifdef CONFIG_MT_HW_PNG_SUPPORT
    if (MT_PNG_Open(&data->hwHandle) != MT_SUCCESS) {
        goto error;
    }

    MT_PNG_SetDecInfo(data->hwHandle, data->png_ptr, data->info_ptr);
    static MT_PNG_STREAM_CB streamFuncs;
    streamFuncs.Reset = streamReset;
    streamFuncs.GetData = streamGetData;
    streamFuncs.GetLength = streamGetLength;

    MT_PNG_SetStreamCallBack(data->hwHandle, &streamFuncs, data->base.buffer);
    png_set_read_fn(data->png_ptr, data, png_read);
    load_png_info(data);
#else
...
static pix_fmt_t dfbFormat2DisplayFormat(DFBSurfacePixelFormat dfbPixelFormat)
{
    switch (dfbPixelFormat) {
    case DSPF_LUT8:
        return PIX_FMT_RGBPALETTE8;

    case DSPF_RGB16:
        return PIX_FMT_RGB565;

    case DSPF_ARGB4444:
        return PIX_FMT_ARGB4444;

    case DSPF_ARGB1555:
        return PIX_FMT_ARGB1555;
...
static TDE2_COLOR_FMT_E dfbFormat2TDEFormat(DFBSurfacePixelFormat dfbPixelFormat)
{
    switch (dfbPixelFormat) {
    case DSPF_LUT8:
...
int tdeQuickResize(unsigned long phySrc, unsigned long phyDst, int pitchSrc, int pitchDst, DFBRectangle *srcRect, DFBRectangle *dstRect, TDE2_COLOR_FMT_E fmt)
{
    TDE2_SURFACE_S srcSurface;
    TDE2_SURFACE_S dstSurface;
    TDE2_RECT_S _srcRect = {srcRect->x, srcRect->y, srcRect->w, srcRect->h};
    TDE2_RECT_S _dstRect = {dstRect->x, dstRect->y, dstRect->w, dstRect->h};
    int ret;

    memset(&srcSurface, 0, sizeof(srcSurface));
    srcSurface.u32PhyAddr = phySrc;
    srcSurface.enColorFmt = fmt;
    srcSurface.u32Width = srcRect->w;
    srcSurface.u32Height = srcRect->h;
    srcSurface.u32Stride = pitchSrc;
    srcSurface.bAlphaMax255 = MT_TRUE;
    srcSurface.bAlphaExt1555 = MT_TRUE;
    srcSurface.u8Alpha0 = 0;
    srcSurface.u8Alpha1 = 255;

    memset(&dstSurface, 0, sizeof(dstSurface));
    dstSurface.u32PhyAddr = phyDst;
    dstSurface.enColorFmt = fmt;
    dstSurface.u32Width = dstRect->w;
    dstSurface.u32Height = dstRect->h;
    dstSurface.u32Stride = pitchDst;
    ...
    ret = MT_TDE2_QuickResize(h, &srcSurface, &_srcRect, &dstSurface, &_dstRect);
    if (ret != MT_SUCCESS) {
        D_ERROR("Error MT_TDE2_QuickResize, ret:%x\n", ret);
        MT_TDE2_EndJob(h, MT_FALSE, MT_FALSE, 10);
        return ret;
    }
    ret = MT_TDE2_EndJob(h, MT_FALSE, MT_TRUE, 10);
                                    ...
                                    const DFBRectangle *dest_rect)
{
    ...
    ret = MT_PNG_GetRowBytes(data->hwHandle, dfbFormat2DisplayFormat(dst_surface->config.format), &data->pitch);
    if (ret != MT_SUCCESS) {
        D_ERROR("%s(%d) Error MT_PNG_GetRowBytes\n", __FUNCTION__, __LINE__);
        return DFB_FAILURE;
    }
    ...
    D_ASSERT(lock.addr != 0);

    MT_PNG_IMAGE_INFO_S outImageInfo = {0};

    data->isUsingSurfaceBuffer = data->pitch == lock.pitch ? TRUE : FALSE;
    if (!data->isUsingSurfaceBuffer) {
        size_t size = data->pitch * data->height + 4;
        data->image_phys = (unsigned long)mt_mmz_new(size, 64, NULL, "dfb");
        data->image = mt_mmz_map(data->image_phys, MT_TRUE);
        outImageInfo.phyAddr = data->image_phys;
        outImageInfo.virAddr = data->image;
    } else {
        data->image_phys = 0;
        data->image = lock.addr;
        outImageInfo.phyAddr = lock.phys;
        outImageInfo.virAddr = data->image;
    }

    outImageInfo.pixFormat = dfbFormat2DisplayFormat(dst_surface->config.format);
    MT_PNG_SetOutImgInfo(data->hwHandle, &outImageInfo);

    int row_bytes = png_get_rowbytes(data->png_ptr, data->info_ptr);
    D_DEBUG_AT(imageProviderPNG, "%s(%d) w:%d,h:%d, rw:%d,rh:%d phys:%x, addr:%p, data_pitch:%d, lock_pitch:%d, dfmt:%d, sfmt:%s, tfmt:%d, row_bytes:%d,  colorType:%d\n", __FUNCTION__, __LINE__,
               data->width, data->height, rect.w, rect.h, lock.phys, lock.addr, data->pitch, lock.pitch, outImageInfo.pixFormat,
               dfb_pixelformat_name(dst_surface->config.format), dfbFormat2TDEFormat(dst_surface->config.format), row_bytes, data->color_type);

    ret = MT_PNG_Decode(data->hwHandle);
    if (ret != MT_SUCCESS) {
        D_ERROR("MT_PNG_Decode Error, ret:0x%x\n", ret);
        dfb_surface_unlock_buffer(dst_surface, &lock);
        return DFB_FAILURE;
    }
    if (!data->isUsingSurfaceBuffer) {
        DFBRectangle srcRect = {0, 0, data->width, data->height};
        DFBRectangle dstRect = {0, 0, rect.w, rect.h};

        ret = tdeQuickResize(data->image_phys, lock.phys, data->pitch, lock.pitch, &srcRect, &dstRect, dfbFormat2TDEFormat(dst_surface->config.format));
        if (ret != MT_SUCCESS) {
            D_ERROR("Error tdeQuickResize, ret:%x\n", ret);
        }
    }
...
{
    DFBResult ret;
    IDirectFBDataBuffer *buffer = (IDirectFBDataBuffer *)userdata;
...
static mt_u32 streamGetLength(void *userdata)
{
    ...
    ret = buffer->GetLength(buffer, &ret_length);
    if (ret != DFB_OK) {
        D_DEBUG_AT(imageProviderPNG, "%s (%d) Error!\n", __FUNCTION__, __LINE__);
    }
    return ret_length;
}
...
static mt_s32 streamGetData(void *userdata, mt_u32 length, void *ret_data, mt_u32 *ret_read, MT_BOOL *eof)
{
    ...
    if (buffer->HasData(buffer) == DFB_OK) {
        ret = buffer->GetData(buffer, length, ret_data, ret_read);
        if (ret != DFB_OK) {
            return MT_FAILURE;
        }
    }
    if (*ret_read < length) {
        *eof = MT_TRUE;
    } else if (*ret_read == length) {
        ret = buffer->WaitForData(buffer, 1);
        *eof = ret == DFB_EOF ? MT_TRUE : MT_FALSE;
    } else {
        *eof = MT_FALSE;
    }
...
static void png_read(png_structp png_ptr, png_bytep raw_data, png_size_t size)
{
    DFBResult ret;
    IDirectFBImageProvider_PNG_data *data = (IDirectFBImageProvider_PNG_data *)png_get_io_ptr(png_ptr);
    IDirectFBDataBuffer *buffer = data->base.buffer;

    if (buffer->HasData(buffer) == DFB_OK) {
        ret = buffer->GetData(buffer, size, raw_data, &size);
        if (ret) {
            D_ERROR("[%s %d] Error, size:%d\n", __FUNCTION__, __LINE__, size);
        }
    }
}
...
static int load_png_info(IDirectFBImageProvider_PNG_data *data)
{
    ...
    D_DEBUG_AT(imageProviderPNG, "%s(%d) w:%d, h:%d, bpp:%d, colortype:%d, colorkey:0x%x\n", __FUNCTION__, __LINE__, data->width, data->height, data->bpp, data->color_type, data->color_key);
    return 0;
}
