...
static MT_BOOL g_DFBSynFlag = MT_FALSE;

#define  INFLEXTION    (130*130)                                  
static MT_U32 s_u32SrcVirAddr = 0;				
static MT_U32 s_u32SrcPhysAddr = 0;				
static MT_U32 s_u32DstVirAddr = 0;				
static MT_U32 s_u32DstPhysAddr = 0;				

#define TDE_FLAG g_DFBSynFlag
...
tde_validate_DESTINATION( TDEDeviceData *tdev,
                             CardState        *state )
{
	s_u32DstVirAddr = (MT_U32)state->dst.addr;
	/* Remember destination parameters for usage in rendering functions. */
...
tde_validate_SOURCE( TDEDeviceData *tdev,
                        CardState        *state )
{
	s_u32SrcVirAddr = (MT_U32)state->src.addr;
...
static MT_S32 ADP_ColorExpand(TDE2_COLOR_FMT_E PFmt, unsigned long Color, MT_U32 *pFillData)
{
    switch (PFmt)
    {
...
bool      TDEFillRectangle( void *drv, void *dev, DFBRectangle *rect)
{
	...
	if((Rect.u32Height*Rect.u32Width  <= INFLEXTION)&&(pDev->drawingflags == DSDRAW_NOFX))
	{
		...
		s32Ret = ADP_DFBGenerateTDEFillOpt(pDev, &TDEOpt, MT_FALSE);
	...
	if ((pDev->drawingflags == DSDRAW_NOFX) && !(IS_HAVE_CLIPRECT(pDev, &Rect)))
	{
		...
		s32Ret = MT_TDE2_QuickFill(s32Handle, &pDev->dst_surface, &Rect, u32FillData);
		if (s32Ret != MT_SUCCESS)
		{
	...
	else
	{
		...
		s32Ret = MT_TDE2_SolidDraw(s32Handle, &pDev->dst_surface, &Rect, &pDev->dst_surface, &Rect, &FillColor, &TDEOpt);
		if (s32Ret != MT_SUCCESS)
		{
...
bool      TDEBlit         ( void *drv, void *dev, DFBRectangle *srect, int dx, int dy )
{
	...
	TDE2_OPT_S stOpt;
	TDE2_OPT_S tmpstOpt;
	MT_BOOL bWithOpt = MT_FALSE, bHaveClip = MT_FALSE;
	...
	/** prepare alpha,key,rop opt */ 
	s32Ret = ADP_DFBGenerateTDEOpt(pDev, &stOpt, MT_FALSE);
	if (MT_SUCCESS != s32Ret)
	{
		return MT_FALSE;
	}
	...
	{
		s32Ret = MT_TDE2_QuickCopy(s32Handle, &pDev->src_surface, &SrcRect, &pDev->dst_surface,&DstRect); // do memcpy        
	}
	else if (bWithOpt) //if with operation (alpha or ckey) or clip
	{
		s32Ret = MT_TDE2_Bitblit(s32Handle, &pDev->dst_surface, &DstRect, &pDev->src_surface, &SrcRect, &pDev->dst_surface,
									&DstRect, &stOpt);            
	}
	else // without clip and operation
	{
		s32Ret = MT_TDE2_Bitblit(s32Handle, NULL, NULL, &pDev->src_surface, &SrcRect, &pDev->dst_surface, //do simple blit
									&DstRect, &stOpt);
	}
...
bool      TDEStretchBlit  ( void *drv, void *dev, DFBRectangle *srect, DFBRectangle  *drect )
{
	...
	pDev = (TDEDeviceData *)dev;
	/** prepare alpha,key,rop opt */ 
	s32Ret = ADP_DFBGenerateTDEOpt(dev, &stOpt, MT_TRUE);
	if (MT_SUCCESS != s32Ret)
	{
		return MT_FALSE;
	}
	...
	{
		s32Ret = MT_TDE2_Bitblit(s32Handle, NULL, NULL, &pDev->src_surface, &SrcRect, &pDev->dst_surface, &DstRect, &stOpt);
	}
