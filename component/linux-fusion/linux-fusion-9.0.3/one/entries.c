...
static void *one_entries_seq_start(struct seq_file *f, loff_t * pos)
{
     ...
     ktime_get_real_ts64(&entries->now);
...
int one_entries_show(struct seq_file *p, void *v)
{
     ...
     if (entry->last_lock.tv_sec) {
          int diff = ((entry->entries->now.tv_sec - entry->last_lock.tv_sec) * 1000 +
                      (entry->entries->now.tv_nsec - entry->last_lock.tv_nsec) / 1000000);
          ...
          if (diff < 1000) {
               ...
               diff = (entry->entries->now.tv_sec - entry->last_lock.tv_sec +
                       (entry->entries->now.tv_nsec -
                        entry->last_lock.tv_nsec) / 1000000000);
...
static const struct proc_ops proc_one_entries_operations = {
     .proc_open    = one_entries_open,
     .proc_read    = seq_read,
