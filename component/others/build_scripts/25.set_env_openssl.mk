...
ifeq ($(COPY_SOURCECODE),y)
	...
	cd $(MT_BUILD_DIR)/$(SRC_PATH); ./config no-asm -shared --prefix=/usr --openssldir=$(BUILDROOT_SYSROOT_USR_DIR)/ssl;
else
	cd $(MT_BUILD_DIR)/$(SRC_PATH); $(MT_SRC_DIR)/$(SRC_PATH)/config no-asm -shared --prefix=/usr --openssldir=$(BUILDROOT_SYSROOT_USR_DIR)/ssl;
endif
