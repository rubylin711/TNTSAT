...
CFLAGS += $(CPPFLAGS)
CFLAGS += -Wno-stringop-overflow -Wno-stringop-truncation -Wno-packed-not-aligned -Wno-format-truncation -Wno-restrict -Wno-format-overflow -Wno-missing-attributes -Wno-implicit-fallthrough
LDFLAGS = -L$(BUILDROOT_SYSROOT_USR_LIB_DIR) -Wl,-rpath-link=$(BUILDROOT_SYSROOT_USR_LIB_DIR)
...
ifeq ($(COPY_SOURCECODE),y)
	...
	cd $(MT_BUILD_DIR)/$(SRC_PATH); ./configure --host=${HOST} --target=${HOST} --prefix=/usr;
else
	cd $(MT_BUILD_DIR)/$(SRC_PATH); $(MT_SRC_DIR)/$(SRC_PATH)/configure --host=${HOST} --target=${HOST} --prefix=/usr;
endif
