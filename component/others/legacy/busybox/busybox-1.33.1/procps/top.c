...
#define MT_TOP_FLUSH_TIMES 5
#define MT_TOP_ALL_CPU_INIT 0
#define MT_TOP_CPU_INIT (-1)
#define MT_TOP_TID_INIT (-1)
#define MT_TOP_THRES_INIT (500.0)
static int mt_top_all_cpu = MT_TOP_ALL_CPU_INIT;
static int mt_top_cpu = MT_TOP_CPU_INIT;
static long long mt_top_tid = MT_TOP_TID_INIT;
static float mt_top_thres = MT_TOP_THRES_INIT;
static volatile int mt_stop_ftrace = 0;
static int mt_top_flush_times = 0;
...
#if 0
			printf("\n\n	busy: %f, thres: %f\n\n", (atof(SHOW_STAT(usr)) + atof(SHOW_STAT(sys)) +
					atof(SHOW_STAT(nic)) + atof(SHOW_STAT(irq)) +
					atof(SHOW_STAT(softirq)) + atof(SHOW_STAT(steal))), mt_top_thres);
#endif
			if ((mt_top_flush_times > MT_TOP_FLUSH_TIMES) && (mt_top_all_cpu == 1 || mt_top_cpu == i)) {
				if (atof(SHOW_STAT(usr)) + atof(SHOW_STAT(sys)) +
					atof(SHOW_STAT(nic)) + atof(SHOW_STAT(irq)) +
					atof(SHOW_STAT(softirq)) + atof(SHOW_STAT(steal)) > mt_top_thres) {
					mt_stop_ftrace = 1;
				}
...
#if 0
		printf("\n\n	pcpu: %f, thres: %f\n\n", ((float)pcpu.quot + ((float)(pcpu.rem) / 10)), mt_top_thres);
#endif
		if ((mt_top_flush_times > MT_TOP_FLUSH_TIMES) && (mt_top_tid == s->pid)) {
			if (((float)pcpu.quot + ((float)(pcpu.rem) / 10)) > mt_top_thres) {
				mt_stop_ftrace = 1;
			}
		}
...
int top_main(int argc UNUSED_PARAM, char **argv)
{
	...
	mt_env = getenv("MT_TOP_ALL_CPU");
	if (mt_env) {
		mt_top_all_cpu = atoi(mt_env);
	}
	mt_env = getenv("MT_TOP_CPU");
	if (mt_env) {
		mt_top_cpu = atoi(mt_env);
	}
	mt_env = getenv("MT_TOP_TID");
	if (mt_env) {
		mt_top_tid = atoll(mt_env);
	}
	mt_env = getenv("MT_TOP_THRES");
	if (mt_env) {
		mt_top_thres = atof(mt_env);
	}
	...
	while (scan_mask != EXIT_MASK) {
		...
		if (mt_top_all_cpu != MT_TOP_ALL_CPU_INIT) {
			printf("\33[32m MT_TOP_ALL_CPU: %d \33[0m \n", mt_top_all_cpu);
		}
		if (mt_top_cpu != MT_TOP_CPU_INIT) {
			printf("\33[32m MT_TOP_CPU: %d \33[0m \n", mt_top_cpu);
		}
		if (mt_top_tid != MT_TOP_TID_INIT) {
			printf("\33[32m MT_TOP_TID: %lld \33[0m \n", mt_top_tid);
		}
		if (mt_top_thres < (MT_TOP_THRES_INIT / 2)) {
			printf("\33[32m MT_TOP_THRES: %f \33[0m \n", mt_top_thres);
		}
 ...
 IF_FEATURE_TOP_INTERACTIVE(display:)
		...
		if (((mt_top_all_cpu != MT_TOP_ALL_CPU_INIT) ||
			(mt_top_cpu != MT_TOP_CPU_INIT) ||
			(mt_top_tid != MT_TOP_TID_INIT) ||
			(mt_top_thres < (MT_TOP_THRES_INIT / 2))) &&
			(mt_top_flush_times > MT_TOP_FLUSH_TIMES) &&
			(fd_trace_marker != -1)) {
			lseek(fd_trace_marker, 0, SEEK_SET);
			write(fd_trace_marker, "mt_top_monitor", strlen("mt_top_monitor"));
		}
		if (mt_stop_ftrace == 1) {
			if (fd_tracing_on != -1) {
