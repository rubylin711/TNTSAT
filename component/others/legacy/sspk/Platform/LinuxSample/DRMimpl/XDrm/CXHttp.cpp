...
bool CXHttp::HttpRequestResponse(const std::string& server, const std::string& httpRequest, const std::string& body, std::string& response)
{
    ...
    {
        int httpResult;
        if(!ParseResponse(response, httpResult))
        {
            goto errorExit;
        }

        if(httpResult > HTTP_STATUS_AMBIGUOUS && httpResult <= HTTP_STATUS_REDIRECT_KEEP_VERB)
        {
            PALPRINTMSG( PALPRINT_CXHTTP_ERROR, ("HttpRequestResponse FAILED: Re-direct not implemented. HTTP status code %d\n", httpResult));
            goto errorExit;
        }
        if(httpResult != HTTP_STATUS_OK)
        {
            PALPRINTMSG( PALPRINT_CXHTTP_ERROR, ("HttpRequestResponse FAILED: HTTP status code %d\n", httpResult));
            goto errorExit;
        }
    }
