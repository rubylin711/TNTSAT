...
static int bt_en_gpio = 0;

static struct hci_uart_proto *hup[HCI_UART_MAX_PROTO];
static int hci_uart_flush(struct hci_dev *hdev);

#ifdef CONFIG_BT_MCU_SUSPEND_SUPPORT
static int montage_bt_standby(void *para);
extern int register_mt_standby(struct mt_standby_component *comp);
#endif
...
int hci_uart_tx_wakeup(struct hci_uart *hu)
{
	...
	if (in_interrupt() || in_atomic()) {
		 ...
		 */
		  /* no need to use  sem to protect tx_state ,because 
                       it accqured spin lock and disable irq in uart_write.
                */
        /*
...
#ifdef CONFIG_BT_MCU_SUSPEND_SUPPORT
	hu->mt_standby = (struct mt_standby_component*)kmalloc(sizeof(struct mt_standby_component), GFP_KERNEL);
	if (hu->mt_standby) {
		hu->mt_standby->enter_standby = montage_bt_standby;
		hu->mt_standby->data = hu;
		strncpy(hu->mt_standby->name,"bluetooth",MAX_MT_STANDBY_BLOCK_NAME);
		register_mt_standby(hu->mt_standby);
		printk(KERN_INFO "register_mt_standby OK\n");
	}
#endif
	return 0;
}
...
static int montage_bt_standby(void *para)
{
	...
	struct hci_uart *pbt_hci_uart = (struct hci_uart*)para;

	if (pbt_hci_uart == NULL) {
		return -1;
	}
	printk(KERN_INFO "montage_bt_standby,send cmd to controller\n");
#if WOBT_NOTIFY_BG_SCAN_LE_WHITELIST_ONLY
	/* Send set back ground scan parameters to Controller for power-on mode */
	result = hci_uart_async_send(pbt_hci_uart, 0xfc7a, 5, params_bg_scan);
	if (result)
		BT_ERR("Realtek bg-scan h5-bt failed");
	/* FIXME: Ensure the above vendor command is sent to Controller
	* and we received the h5 ack from Controller
...
#endif
#if RTKBT_TV_POWERON_WHITELIST
	result = rtkbt_lookup_le_device_poweron_whitelist(pbt_hci_uart->hdev);
	if (result < 0) {
		BT_ERR("rtkbt_lookup_le_device_poweron_whitelist error: %d", result);
	}
#endif

	printk(KERN_ERR "notify controller suspend\n");
	result = hci_uart_async_send(pbt_hci_uart, 0xfc28, 1, params_suspend_notify);
	if (result)
		BT_ERR("Realtek suspend h5-bt failed");
...
static int __init hci_uart_init(void)
{
	...
	BT_INFO("*HCI UART driver ver %s,bt_en_gpio:%d", VERSION,bt_en_gpio);
	/*BT_EN pin control*/
	
	if (bt_en_gpio > 0) {
		drv_gpio_io_enable(bt_en_gpio,2);
		drv_gpio_set_dir(bt_en_gpio,0);
		drv_gpio_set_value(bt_en_gpio,0);
		msleep(200);
		drv_gpio_set_value(bt_en_gpio,1);
		msleep(200);
	}
...
module_param(bt_en_gpio, int, 0644);
