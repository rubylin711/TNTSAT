...
CC := $(subst -D_FILE_OFFSET_BITS=64,,$(CC))
CC_NOSAN := $(subst -D_FILE_OFFSET_BITS=64,,$(CC_NOSAN))
CXX := $(subst -D_FILE_OFFSET_BITS=64,,$(CXX))
CXX_NOSAN := $(subst -D_FILE_OFFSET_BITS=64,,$(CXX_NOSAN))
CPPFLAGS = -I$(BUILDROOT_SYSROOT_USR_INC_DIR)
CFLAGS += $(CPPFLAGS)
ifeq ($(CONFIG_MT_GPERFTOOLS_HEAP_PROF),y)
CFLAGS += -DTCMALLOC_SMALL_BUT_SLOW
endif
LDFLAGS = -L$(BUILDROOT_SYSROOT_USR_LIB_DIR) -Wl,-rpath-link=$(BUILDROOT_SYSROOT_USR_LIB_DIR) -lpthread
LIBS = -L$(BUILDROOT_SYSROOT_USR_LIB_DIR) -lpthread
...
ifeq ($(COPY_SOURCECODE),y)
	...
	cd $(MT_BUILD_DIR)/$(SRC_PATH); ./autogen.sh; ./configure --host=${HOST} --prefix=/usr $(CONFIGURE_ARGS_EXTRA);
else
	cd $(MT_BUILD_DIR)/$(SRC_PATH); $(MT_SRC_DIR)/$(SRC_PATH)/autogen.sh; $(MT_SRC_DIR)/$(SRC_PATH)/configure --host=${HOST} --prefix=/usr $(CONFIGURE_ARGS_EXTRA);
endif
