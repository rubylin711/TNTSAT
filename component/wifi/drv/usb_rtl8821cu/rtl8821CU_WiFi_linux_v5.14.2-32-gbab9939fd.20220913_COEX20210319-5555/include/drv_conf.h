...
#endif
/* Configuration descriptions,Montage Wliao*/
/*
RTW_BUSY_DENY_SCAN: Driver will refuse scan request while busy. <enable>
CONFIG_RTW_REPEATER_SON: self-origanizing netowrk,it allow repeater to origanize network <disable>
CONFIG_RTW_SCAN_RAND: scan network in random channel. <enable>
CONFIG_TX_AC_LIFETIME : control TX Access Category life time.eg AC_BE,AC_VI,AC_VO <enable>
CONFIG_RTW_HIQ_FILTER : allow driver/wireless to transmit some high qualify packets <enable>
CONFIG_RTW_ADAPTIVITY_EN : allow driver to adjust tx power,trans rate and channel <disable>
CONFIG_RTW_EXCL_CHS : driver will exclude some channels while scanning. <disable>
CONFIG_RTW_EXCL_CHS_6G : driver will exclude channels in 6G band while scanning. <disable>
CONFIG_RTW_COUNTRY_IE_SLAVE_EN_ROLE : when work as STA or Group Client, the Count Info follow host <default enable>
CONFIG_IEEE80211_BAND_5GHZ: support 5G <default enable>
CONFIG_DFS: dynamic frequency selection,it will chose best channel <default enable>
CONFIG_TXPWR_BY_RATE_EN: define the TX Power setting from.0-;1-;2-by efuse <default 0>
CONFIG_RTW_CHPLAN : Channel plan, Wireless will select best channel <defualt enable>
CONFIG_CALIBRATE_TX_POWER_BY_REGULATORY : decide tx power by regulatory <default disabled>
RTW_DEF_MODULE_REGULATORY_CERT: driver will load regulatory certification <default disable>
CONFIG_RTW_REGD_SRC: wireless regulatory database source define,0-rtk_priv;1-os <default 1>
CONFIG_RTW_RX_AMPDU_SZ_LIMIT_1SS : define the max rx aggregation packet number for 1 antenna chip <default>
CONFIG_RTW_TARGET_TX_PWR_2G_A : define tx power 2G  <default>
CONFIG_CONCURRENT_MODE: concurrent mode, support sta/gc and ap/go at the same time. <disable>
CONFIG_IFACE_NUMBER : defines how many virtual interface, when CONFIG_CONCURRENT_MODE,it should > 2
  <default 1>
MACID_NUM_SW_LIMIT: max MAC number the driver can generate <default>
CONFIG_WOWLAN : wake-on-wirless LAN, WiFi wakeup function <disable>
RTW_IPS_MODE : Idle power save,depend on CONFIG_POWER_SAVING <disable>
CONFIG_POWER_SAVING : support suspen/resume, <disable>
CONFIG_OFFLOAD_MDNS_V4  : whether support IPv4 mDNS(Multicast DNS) uninstall <default disabled>
CONFIG_RTW_TPT_MODE : Traffic priority tagging, something like QOS <enable>
CONFIG_RTW_MGMT_QUEUE : use independent queue to storage and process management frame <default enable>
CONFIG_PCI_MSI : for PCI <default disabled>
CONFIG_BT_COEXIST : BT & WiFi co-exist <disable> question: if not support BT should this enable?




CONFIG_RTW_MULTI_AP  implements multi host spot  <disable>
  CONFIG_RTW_WDS: Realtek wirelss Distribution System, allow multiple AP associate by wireless
	CONFIG_RTW_WNM: Realtek wireless network management, control power saving;fast roaming and network optimization
	CONFIG_RTW_NLRTW: allow user space access WiFi driver over libnl/netlink without wpa_supplicant
	CONFIG_RTW_80211K: allow sta trans from one AP to another AP with less network broken

CONFIG_AP_MODE:
  CONFIG_RTW_AP_DATA_BMC_TO_UC: allow forward AP's broadcost\multicast to a device by unicast <disable>
	CONFIG_ACTIVE_TPC_REPORT: transmit power collection <disable>
Note: RTW SW Crypto is usless
