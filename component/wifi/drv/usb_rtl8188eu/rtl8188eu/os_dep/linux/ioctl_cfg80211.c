...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0) && PLATFORM_MONTAGE)
  struct wireless_dev *pwdev = adapter->pnetdev->ieee80211_ptr;
#endif
	u8 ret = _SUCCESS;
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(3, 19, 0))
	if (started) {
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 11, 0) && PLATFORM_MONTAGE)
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6, 6, 0) && PLATFORM_MONTAGE)
    mutex_lock(&pwdev->mtx);
		cfg80211_ch_switch_started_notify(adapter->pnetdev, &chdef,0, 0, false,0);
    mutex_unlock(&pwdev->mtx);
#else
