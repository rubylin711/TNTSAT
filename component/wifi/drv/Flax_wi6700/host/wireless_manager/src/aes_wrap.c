...
#define os_malloc(x)	os_api_alloc(x, GFP_KERNEL)
#define os_memcpy	memcpy
#define os_memset	memset
#define os_free		os_api_free
...
int aes_wrap(const u8 *kek, int n, const u8 *plain, u8 *cipher)
{
	...
	ctx = (void *) aes_encrypt_init(kek, 16);
	if (ctx == NULL)
		return -1;
...
int aes_unwrap(const u8 *kek, int n, const u8 *cipher, u8 *plain)
{
	...
	ctx = (void *) aes_decrypt_init(kek, 16);
	if (ctx == NULL)
		return -1;
			 ...
			 const u8 *addr[], const size_t *len, u8 *mac)
{
	...
	ctx = (void *) aes_encrypt_init(key, 16);
	if (ctx == NULL)
		return -1;
...
int aes_128_cbc_decrypt(const u8 *key, const u8 *iv, u8 *data, size_t data_len)
{
	...
	ctx = (void *) aes_decrypt_init(key, 16);
	if (ctx == NULL)
		return -1;
	os_memcpy(cbc, iv, BLOCK_SIZE);
