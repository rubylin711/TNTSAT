...
static void ath9k_init_channels_rates(struct ath9k_htc_priv *priv)
{
	if (priv->ah->caps.hw_caps & ATH9K_HW_CAP_2GHZ) {
		priv->sbands[NL80211_BAND_2GHZ].channels =
			ath9k_2ghz_channels;
		priv->sbands[NL80211_BAND_2GHZ].band = NL80211_BAND_2GHZ;
		priv->sbands[NL80211_BAND_2GHZ].n_channels =
			ARRAY_SIZE(ath9k_2ghz_channels);
		priv->sbands[NL80211_BAND_2GHZ].bitrates = ath9k_legacy_rates;
		priv->sbands[NL80211_BAND_2GHZ].n_bitrates =
			       ...
			       struct ieee80211_hw *hw)
{
	...
	if (priv->ah->caps.hw_caps & ATH9K_HW_CAP_HT) {
		if (priv->ah->caps.hw_caps & ATH9K_HW_CAP_2GHZ)
			setup_ht_cap(priv,
				     &priv->sbands[NL80211_BAND_2GHZ].ht_cap);
		if (priv->ah->caps.hw_caps & ATH9K_HW_CAP_5GHZ)
			setup_ht_cap(priv,
