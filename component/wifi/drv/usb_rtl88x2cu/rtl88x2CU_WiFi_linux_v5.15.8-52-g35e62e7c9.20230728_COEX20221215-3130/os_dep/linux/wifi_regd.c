...
static void rtw_regd_overide_flags(struct wiphy *wiphy, struct get_chplan_resp *chplan, bool rtnl_lock_needed)
{
	...
	for (i = 0; i < chs_len; i++) {
      ...
      /*
        Montage added:
        if you want enable all channel for P2P try to comment ch->flags |= IEEE80211_CHAN_NO_IR;
      */
			#endif
		}
	}
	...
	, const char *caller)
{
	...
	for (i = 0; i < dvobj->iface_nums; i++) {
		...
		if (!iface->rtw_wdev)
			continue;
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5, 19, 2)) || defined(CONFIG_MLD_KERNEL_PATCH)  /* Montage added */
