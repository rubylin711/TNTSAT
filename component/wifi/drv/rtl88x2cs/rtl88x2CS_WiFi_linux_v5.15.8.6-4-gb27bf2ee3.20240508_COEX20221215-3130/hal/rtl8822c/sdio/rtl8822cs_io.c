...
static struct recv_buf *sd_recv_rxfifo(PADAPTER adapter, u32 size)
{
  ...
  /* 
    Montage: Workaround for Montage Symphony4 Platform in SDMA mode
    Montage SYM4 use SDIO SDMA,it accept 64K data only
  */
//#ifdef PLATFORM_MONTAGE_SYMPHONY4
  while(bufsz > 65536){
	  ret = rtl8822cs_read_port(d, 65536, rbuf);
    bufsz -= 65536;
    rbuf += 65536;
  }
//#endif
