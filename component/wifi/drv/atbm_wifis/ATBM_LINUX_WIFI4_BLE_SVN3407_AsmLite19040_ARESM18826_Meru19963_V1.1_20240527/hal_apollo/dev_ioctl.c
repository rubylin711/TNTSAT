...
static int atbm_dev_get_temparature(struct net_device *dev, struct altm_wext_msg *msg);

#ifdef ATBM_EVIZE_CUSTOMER_FUNC
static int atbm_dev_get_evize_wifi_stat(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_evize_reason_code(struct net_device *dev, struct altm_wext_msg *msg);
#endif
static int atbm_dev_get_ap_list_new(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_set_scan_channel_limit(struct net_device *dev, struct altm_wext_msg *msg);
static int atbm_dev_get_ap_list_arol(struct net_device *dev, struct altm_wext_msg *msg);
...
static int atbm_dev_set_snd_probe_req_mutil_ie(struct net_device *dev, struct altm_wext_msg *msg);
...
//	{ATBM_DEV_IO_SET_UPERR_PROCESS_PID,,},
	...
	{ATBM_DEV_IO_GET_CHIP_TEMPERATURE,atbm_dev_get_temparature,"ATBM_DEV_IO_GET_CHIP_TEMPERATURE"},
#ifdef ATBM_EVIZE_CUSTOMER_FUNC
	{ATBM_DEV_IO_GET_EVIZE_WIFI_STATE,atbm_dev_get_evize_wifi_stat,"ATBM_DEV_IO_GET_EVIZE_WIFI_STATE = 51"},
	{ATBM_DEV_IO_GET_EVIZE_REASON_CODE,atbm_dev_get_evize_reason_code,"ATBM_DEV_IO_GET_EVIZE_WIFI_STATE = 51"},
		
#endif
	{ATBM_DEV_IO_GET_AP_LIST_NEW,atbm_dev_get_ap_list_new,"ATBM_DEV_IO_GET_EVIZE_WIFI_STATE = 54"},
	{ATBM_DEV_IO_SET_SCAN_CHANNEL_LIMIT,atbm_dev_set_scan_channel_limit,"ATBM_DEV_IO_GET_EVIZE_WIFI_STATE = 54"},
	{ATBM_DEV_IO_SET_SND_PROBE_REQ_MUTIL_IE,atbm_dev_set_snd_probe_req_mutil_ie,"ATBM_DEV_IO_SET_SND_PROBE_REQ_MUTIL_IE=68"},
	{ATBM_DEV_IO_GET_AP_AROL,atbm_dev_get_ap_list_arol,"ATBM_DEV_IO_GET_AP_AROL=69"},
	{-1,NULL,NULL},
...
static int atbm_dev_set_snd_probe_req_mutil_ie(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	void __user *user_pos = NULL;
	struct altm_snd_mutil_ie *snd_mutil_ie = NULL;
	int ret = 0,i = 0;
	int sta_conn_state = 0,work_channel = 0;
//	u8  atbm_add_ie[800] = {0};
	u8  *atbm_add_ie = NULL;
	int atbm_add_ie_len = 0,pos = 0;
	u8 scan_channel[14],scan_n_channel = 0;
	/*
		Check the current network port connection status
	*/
extern int atbm_get_sta_wifi_connect_status(struct ieee80211_sub_if_data *sdata);
	sta_conn_state = atbm_get_sta_wifi_connect_status(sdata);	
	
	atbm_printk_err("%s :sta_conn_state = %d\n",__func__,sta_conn_state);
	if(sta_conn_state){
		work_channel = get_work_channel(sdata,0);
		atbm_printk_err("%s :work_channel = %d\n",__func__,work_channel);
	}
	
	/*
		Process the data passed down by the application
	*/
	snd_mutil_ie = atbm_kzalloc(sizeof(struct altm_snd_mutil_ie), GFP_KERNEL);
	if(!snd_mutil_ie){
		atbm_printk_err("%s : Failed to allocate snd_mutil_ie memory!!!\n",__func__);
		ret = -ENOMEM;
		goto err;
	}
	
	memset(snd_mutil_ie,0,sizeof(struct altm_snd_mutil_ie));
	memcpy(&user_pos, &msg->externData[0], sizeof(void*));
	atbm_printk_err("user_pos=%x \n",user_pos);
	
	if(copy_from_user(snd_mutil_ie, user_pos, sizeof(struct altm_snd_mutil_ie)) != 0){
		atbm_printk_err("%s : copy form user err!!!\n",__func__);
		ret = -EINVAL;
		goto err;
	}

	atbm_printk_err("n_channel[%d] n_ie[%d] time[%d] \n",
			snd_mutil_ie->n_channels,snd_mutil_ie->n_ies,snd_mutil_ie->max_scan_time);
	for(i = 0;i<snd_mutil_ie->n_channels;i++){
		atbm_printk_err("channel = %d \n ",snd_mutil_ie->channels[i]);
	}
	atbm_printk_err("ie:\n");
	for(i = 0;i <snd_mutil_ie->n_ies;i++ ){
		atbm_printk_err("ie id : %d \n",snd_mutil_ie->ie[i][0]);
		atbm_printk_err("ie len : %d \n",snd_mutil_ie->ie[i][1]);
		atbm_printk_err("%s \n",&snd_mutil_ie->ie[i][2]);

		atbm_add_ie_len += (snd_mutil_ie->ie[i][1] + 2);
		
	}
	if(atbm_add_ie_len > 800){
		atbm_printk_err("%s:atbm_add_ie_len > 800 byte \n");
		ret = -1;
		goto err;
	}
	...
	*/
	if(atbm_add_ie_len > 0){
		atbm_add_ie =  atbm_kzalloc(atbm_add_ie_len * sizeof(u8), GFP_KERNEL);
		if(!atbm_add_ie){
			atbm_printk_err("%s : Failed to allocate atbm_add_ie memory!!! atbm_add_ie_len = %d\n",__func__,atbm_add_ie_len);
			ret = -ENOMEM;
			goto err;
		}
		...
		pos = 0;
		for(i = 0;i < snd_mutil_ie->n_ies;i++){
			memcpy(atbm_add_ie + pos,snd_mutil_ie->ie[i],snd_mutil_ie->ie[i][1] + 2);
			pos += (snd_mutil_ie->ie[i][1] + 2);
		}
	}else{
		atbm_printk_err("%s : not found vendor special ie\n",__func__);
		atbm_add_ie = NULL;
		atbm_add_ie_len = 0;
	}
	...
	*/
	...
	if(sta_conn_state){
		for(i = 0;i < 14;i++)
			scan_channel[i] = work_channel;
			
	}else{
		if(snd_mutil_ie->channels[0] == 0){
			for(i = 0;i < 14;i++)
				scan_channel[i] = i+1;
		}else if(snd_mutil_ie->n_channels == 1){
			for(i = 0;i < 14;i++)
				scan_channel[i] = snd_mutil_ie->channels[0];
		}else{
			
			for(i = 0;i < snd_mutil_ie->n_channels;i++){
				scan_channel[i] = snd_mutil_ie->channels[i];
				atbm_printk_err("scan channel  = %d \n",scan_channel[i]);
			}
			scan_n_channel = snd_mutil_ie->n_channels;
			
		}

	}
	atbm_printk_err("scan_n_channel  = %d \n",scan_n_channel);
	/*
		set scan time

	*/
	hw_priv->private_scan_flag = true;
	hw_priv->private_min_scan_time = 60;
	hw_priv->private_max_scan_time = snd_mutil_ie->max_scan_time;
...
#if 1	
	ieee80211_sta_triger_positive_scan(sdata,scan_channel,scan_n_channel,NULL,0,atbm_add_ie,atbm_add_ie_len,NULL);
	

#else	
	/*

		bool ieee80211_send_special_probe_req2(struct ieee80211_sub_if_data *sdata, u8 *dst,
				const u8 *ssid, size_t ssid_len,
				const u8 **special_ie,int special_ie_len, size_t special_ie_num)
	*/
	ieee80211_send_special_probe_req2(sdata,NULL,
				NULL,0,
				snd_mutil_ie->ie,atbm_add_ie_len,snd_mutil_ie->n_ies);
#endif	
err:
	hw_priv->private_scan_flag = false;
	if(snd_mutil_ie)
		atbm_kfree(snd_mutil_ie);
	if(atbm_add_ie)
		atbm_kfree(atbm_add_ie);
	return ret;
}
...
static int atbm_dev_ioctl_set_ratePower(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	if(power_req.power > 128)
		power_req.power -= 256;
	atbm_printk_err("power=%d %d\n",msg->externData[1],power_req.power);
...
static int atbm_dev_get_sta_info_new(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	ret = atbm_internal_get_ground_noise(hw_priv,&noise_level);

	noise_level_dBm = noise_level.noise_level_dBm;
	if(noise_level_dBm > 127)
		noise_level_dBm -= 256;
	if(noise_level.initial_gain >= 50)
		p_info->noise_level_dBm = noise_level_dBm;
	else
		p_info->noise_level_dBm = 0;
    ...
    dev_printk("    channel idle %ld\n", p_info->channel_idle);
	dev_printk("    ground noise %d\n", p_info->noise_level_dBm);
...
static int atbm_dev_get_temparature(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	msg->value = temp;
//	memcpy(msg->externData, &temp, sizeof(temp));
...
static int atbm_dev_get_evize_wifi_stat(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	char *pdata = NULL;
	int ret = 0;
	memcpy(&pdata, &msg->externData[0], sizeof(int)); 
	
	mutex_lock(&hw_priv->evize_thread_mutex);
	
	if(pdata != NULL){
        if (copy_to_user(pdata, hw_priv->wifi_rate_state, sizeof(hw_priv->wifi_rate_state))){
            ret = -1;
        }
    }
	
	mutex_unlock(&hw_priv->evize_thread_mutex);
	return ret;
}
static int atbm_dev_get_evize_reason_code(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	msg->value = hw_priv->evize_reason_code;
...
static int atbm_dev_set_txpwr_dcxo(struct net_device *dev, struct altm_wext_msg *msg)
{
    ...
    memset(cmd, 0, sizeof(cmd));
    sprintf(cmd, "set_txpwr_and_dcxo,%d,%d,%d,%d,0,0,0,0,0,0 ", txpwr_L, txpwr_M, txpwr_H, dcxo);
											   ...
											   struct ieee80211_internal_scan_sta *sta)
{
	...
	int i = 0,j = 0;
	//atbm_printk_err("atbm_dev_handle_ap_list:req->n_channels=%d,sta->channel=%d \n",req->n_channels,sta->channel);
	if(req->n_stas >= req->n_channels*AP_SCAN_NUM_MAX){
		return false;
	}
...
int atbm_dev_get_ap_list(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	memset(&internal_scan,0,sizeof(struct ieee80211_internal_scan_request));
	memset(&req,0,sizeof(struct atbm_internal_scan_results_req));
	...
	atbm_printk_err("atbm_dev_get_ap_list,req.n_channels=%d \n",req.n_channels);
	recv_info = atbm_kzalloc(sizeof(BEST_CHANNEL_INFO)*req.n_channels*AP_SCAN_NUM_MAX, GFP_KERNEL);
	if(recv_info == NULL){
		ret =  -ENOMEM;
		goto exit;
	}
	...
	if(copy_to_user(user_pos, recv_info, sizeof(BEST_CHANNEL_INFO)*req.n_channels*AP_SCAN_NUM_MAX) != 0){
		ret = -EINVAL;
	}
exit:
...
static int  atbm_dev_set_cail_auto_ppm(struct net_device *dev, struct altm_wext_msg *msg)
{
	...
	open_auto_cfo(hw_priv,msg->value,msg->externData[0]);
...
//	struct cfg_txpower_t configured_txpower;
	...
	sprintf(writebuf, "set_txpwr_and_dcxo,%d,%d,%d,%d,0,0,0,0,0,0 ", efuse_data.delta_gain1,efuse_data.delta_gain2,
...
static u8 channel_id[50]={0};
#define ATBM_6132_SUPPORT_CHANNEL_AP_NUM 32
static bool atbm_dev_handle_ap_list_new(struct ieee80211_hw *hw,struct atbm_internal_scan_results_req *req,
											   struct ieee80211_internal_scan_sta *sta)
{
	BEST_CHANNEL_INFO_NEW *info     = (BEST_CHANNEL_INFO_NEW *)req->priv;
	BEST_CHANNEL_INFO_NEW *pos_info = NULL;
	int i = 0,j = 0;
	
	if(req->n_stas >= req->n_channels*ATBM_6132_SUPPORT_CHANNEL_AP_NUM){
		atbm_printk_err("atbm_dev_handle_ap_list_new,req->n_stas=%d \n",req->n_stas);
		return false;
	}
	...
	//	return false;
	//}
	for(i = 0;channel_id[i] != 0;i++){
		if(channel_id[i] == sta->channel)
			break;
	}
	if(channel_id[i] == 0){
		atbm_printk_err("channel[%d],ssid:[%s]ssid_len:[%d] not support channel! \n",sta->channel,sta->ssid,sta->ssid_len);
		return true;
	}
		
	
	pos_info = info+(i)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM;
	
	for(i = 0;i<ATBM_6132_SUPPORT_CHANNEL_AP_NUM;i++){
		/*
			If it is already marked, shift it back
		*/
		if(pos_info->flag == 2){
			pos_info++;
			continue;
		...
		atbm_printk_debug("channel[%d],ssid:[%s]ssid_len:[%d] \n",sta->channel,pos_info->ssid,sta->ssid_len);
		/*
			Save a new bss to make a mark
		*/
		pos_info->flag = 2;
...
int atbm_dev_get_ap_list_new(struct net_device *dev, struct altm_wext_msg *msg)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	 struct ieee80211_local *local = sdata->local;
	int ret = 0,i = 0,j = 0,scan_n_channels = 0;
	struct ieee80211_internal_scan_request internal_scan;
	struct atbm_internal_scan_results_req req;
	...
	for (i = 0; i < IEEE80211_NUM_BANDS; i++){
			if (wiphy->bands[i]){
				scan_n_channels += wiphy->bands[i]->n_channels;
				for (j = 0; j < wiphy->bands[i]->n_channels; j++) {
					
					channel_id[(i*14) + j] = ieee80211_frequency_to_channel(wiphy->bands[i]->channels[j].center_freq);
					atbm_printk_err("channel:%d \n",channel_id[(i*14) + j]);
				}
			}
	}
#else
	scan_n_channels = ieee80211_get_support_channel(local,channel_id,scan_n_chan);
#endif
	//ieee80211_frequency_to_channel(chandef->chan->center_freq);


	req.n_channels = scan_n_channels;
	atbm_printk_err("atbm_dev_get_ap_list_new:scan channels = %d req.n_channels=%d , len = %d\n",scan_n_channels,req.n_channels,sizeof(BEST_CHANNEL_INFO_NEW)*(req.n_channels)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM);

	recv_info = atbm_kzalloc(sizeof(BEST_CHANNEL_INFO_NEW)*(req.n_channels)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM, GFP_KERNEL);
	if(recv_info == NULL){
		ret =  -ENOMEM;
		goto exit;
	}
	...
	memcpy(&user_pos, &msg->externData[0], 4);
	atbm_printk_err("atbm_dev_get_ap_list_new:&user_pos=%x , user_pos = %x\n",&user_pos,user_pos);
	req.flush = true;
	req.n_stas = 0;
	req.priv = recv_info;
	req.result_handle = atbm_dev_handle_ap_list_new;

	ieee80211_scan_internal_req_results(sdata->local,&req);
	atbm_printk_err("atbm_dev_get_ap_list_new:scan bss num = %d\n",req.n_stas);

#if 1	
	int all_len,need_copy_len = 10000,cnt = 0;
	all_len = sizeof(BEST_CHANNEL_INFO_NEW)*(req.n_channels)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM;
	while (1){
		if((all_len - need_copy_len) > 0){
			if((ret = copy_to_user(user_pos+cnt*need_copy_len, recv_info+need_copy_len*cnt,need_copy_len)!= 0)){
				atbm_printk_err("atbm_dev_get_ap_list_new : copy_to_user err ! ret = %d\n",ret);
				ret = -EINVAL;
			}
			all_len -= need_copy_len;
			if(all_len <= 0){
				atbm_printk_err("cnt = %d,all_len=%d \n",cnt,all_len);
				break;
			}
		}else{
			if((ret = copy_to_user(user_pos+cnt*need_copy_len, recv_info+need_copy_len*cnt,all_len)!= 0)){
				atbm_printk_err("atbm_dev_get_ap_list_new : copy_to_user err ! ret = %d\n",ret);
				ret = -EINVAL;
			}
			break;
...
#else	
	//One copy failed because the length was too long
	if((ret = copy_to_user(user_pos, recv_info, sizeof(BEST_CHANNEL_INFO_NEW)*(req.n_channels)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM) )!= 0){
		atbm_printk_err("atbm_dev_get_ap_list_new : copy_to_user err ! ret = %d\n",ret);
		ret = -EINVAL;
	}
#endif	
...
int atbm_dev_get_ap_list_arol(struct net_device *dev, struct altm_wext_msg *msg)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	...
	u8 *recv_info = NULL;
	u8  scan_n_chan[5] = {0};
	u8 every_channel_scan_cnt = 0;
	u8 resume_value = 0;
	u8 *customer_channel = NULL;
	int ap_list_arol_len = 0;
	AP_LIST_AROL *scan_param_set = NULL;
	u8 fast_scan_channel[]={1,6,11,36,40,44,48,149,153,157,161,165};


	
	list_for_each_entry(sdata_tmp, &local->interfaces, list){
			if (sdata_tmp->vif.type == NL80211_IFTYPE_AP){
				atbm_printk_err("%s:AP is running,not allow scan\n",__func__);
				return -1;
			}
	};
	memset(&internal_scan,0,sizeof(struct ieee80211_internal_scan_request));
	...
	scan_param_set = msg->externData;
	atbm_printk_err("mode[%d] positive[%d] probe_num[%d] scan_time[%d] ssid[%s] ssid[%s] \n",
					scan_param_set->scan_mode,scan_param_set->positive_scan,scan_param_set->n_probe_req,
					scan_param_set->scan_time,scan_param_set->scan_ssid[0],scan_param_set->scan_ssid[1]);
	scan_n_channels = ieee80211_get_support_channel(local,channel_id,scan_n_chan);
	
	every_channel_scan_cnt = scan_param_set->scan_time/100;
	resume_value = scan_param_set->scan_time%100;
	if(resume_value){
		atbm_printk_err("%s:The scan time is not exactly 100(%d) \n",__func__,scan_param_set->scan_time);
	}
	scan_param_set->scan_time = 100;
	if(every_channel_scan_cnt < 1){
		every_channel_scan_cnt = 1;
	}
	/*
		Allocate full channel space 
	*/
	scan_param_set->scan_channel = atbm_kmalloc(scan_n_channels * every_channel_scan_cnt ,GFP_KERNEL);
	if(!scan_param_set->scan_channel){
		atbm_printk_err("%s: scan_channel malloc fail!\n",__func__);
		return -ENOMEM;
	}
	switch(scan_param_set->scan_mode){
		case SCAN_ALL_CHANNEL:{
			for(i = 0;i < scan_n_channels; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = channel_id[i];
				}
			}
			scan_param_set->scan_n_channel = scan_n_channels* every_channel_scan_cnt;
			}break;
		case SCAN_2_4G_CHANNEL:{
			for(i = 0;i < scan_n_chan[0]; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = channel_id[i];
				}
			}
			
			scan_param_set->scan_n_channel = scan_n_chan[0] * every_channel_scan_cnt;
			}break;
		case SCAN_5G_CHANNEL:{
			for(i = 0;i < scan_n_chan[1]; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = channel_id[scan_n_chan[0] + i];
				}
			}
			scan_param_set->scan_n_channel = scan_n_chan[1] * every_channel_scan_cnt ;
			}break;
		case SCAN_FAST_ALL_CHANNEL:{
			for(i = 0;i < 12; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = fast_scan_channel[i];
				}
			}
			
			scan_param_set->scan_n_channel = 12 * every_channel_scan_cnt;
			}break;
		case SCAN_FAST_2_4G_CHANNEL:{
			for(i = 0;i < 3; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = fast_scan_channel[i];
				}
			...
			//memcpy(scan_param_set->scan_channel,fast_scan_channel,3);
			scan_param_set->scan_n_channel = 3 * every_channel_scan_cnt;
			}break;
		case SCAN_FAST_5G_CHANNEL:{
			for(i = 0;i < 9; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = fast_scan_channel[3+i];
				}
			}
		//	memcpy(scan_param_set->scan_channel,&fast_scan_channel[3],9);
			scan_param_set->scan_n_channel = 9 * every_channel_scan_cnt;
			}break;
		case SCAN_CUSTOMER_CHANNEL:{
			ap_list_arol_len = sizeof(AP_LIST_AROL);
			customer_channel = &msg->externData[ap_list_arol_len];
			for(i = 0;i < scan_param_set->scan_n_channel; i++ ){
				for(j = 0;j<every_channel_scan_cnt;j++ ){
					scan_param_set->scan_channel[ i*every_channel_scan_cnt + j ] = customer_channel[i];
				}
			}
			scan_param_set->scan_n_channel = (every_channel_scan_cnt * scan_param_set->scan_n_channel);
			atbm_printk_err("set customer scan :arol_len[%d] n_chan[%d] chan[%d]\n",
							ap_list_arol_len,scan_param_set->scan_n_channel,scan_param_set->scan_channel[0]);
			}break;
		default:{
			atbm_printk_err("scan mode err : %d \n",scan_param_set->scan_mode);
			return -1;
			}
	}
	for(i = 0;i <scan_param_set->scan_n_channel;i++ )
		atbm_printk_err("scan num[%d] \n",scan_param_set->scan_channel[i]);
	/*
		channel set
	*/
	internal_scan.channels = scan_param_set->scan_channel;
	internal_scan.n_channels = scan_param_set->scan_n_channel;
	req.n_channels = scan_n_channels;
	/*
		set scan time

	*/
	hw_priv->private_scan_flag = 1;
	hw_priv->private_min_scan_time = scan_param_set->scan_time - 20;
	hw_priv->private_max_scan_time = scan_param_set->scan_time;


	/*
		set passtive or positive scan
	*/
	if(scan_param_set->positive_scan){
		
		/*
			ssid set
		*/

		ssids = (struct cfg80211_ssid *)atbm_kmalloc(sizeof(struct cfg80211_ssid)*scan_param_set->scan_n_ssid,GFP_KERNEL);
		if(!ssids){
			atbm_printk_err("%s: scan ssids malloc fail!\n",__func__);
			return -ENOMEM;
		}
		
		for(i = 0;i <scan_param_set->scan_n_ssid;i++){
			ssids[i].ssid_len = strlen(scan_param_set->scan_ssid[i]);
			memcpy(ssids[i].ssid,scan_param_set->scan_ssid[i],ssids[i].ssid_len);
			atbm_printk_err("scan ssid:%s ,ssid_len:%d\n",scan_param_set->scan_ssid[i],ssids[i].ssid_len);
		}
		internal_scan.ssids = ssids;
		internal_scan.n_ssids = scan_param_set->scan_n_ssid;
		
		hw_priv->numOfProbeFlag = 1;
		hw_priv->numOfProbeRequests = scan_param_set->n_probe_req;

		
	}else{
		atbm_printk_err("%s: passv scan,flag[%x]\n",__func__,internal_scan.req_flags);
		internal_scan.req_flags |= IEEE80211_INTERNAL_SCAN_FLAGS__PASSAVI_SCAN;
	}
//	hw_priv.scan
	...
	/* set support channel,save return ap info */
	req.n_channels = scan_n_channels;
	atbm_printk_err("atbm_dev_get_ap_list_new:scan channels = %d req.n_channels=%d , len = %d\n",scan_n_channels,req.n_channels,sizeof(BEST_CHANNEL_INFO_NEW)*(req.n_channels)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM);
	recv_info = atbm_kzalloc(sizeof(BEST_CHANNEL_INFO_NEW)*(req.n_channels)*ATBM_6132_SUPPORT_CHANNEL_AP_NUM, GFP_KERNEL);
	if(recv_info == NULL){
	...
	memcpy(&user_pos, &scan_param_set->user_addr , 4);
	atbm_printk_err("atbm_dev_get_ap_list_new:&user_pos=%x , user_pos = %x\n",&user_pos,user_pos);
	req.flush = true;
	req.n_stas = 0;
	req.priv = recv_info;
...
#endif	
exit:
	hw_priv->numOfProbeFlag = 0;
	hw_priv->private_scan_flag = 0;
	
	if(scan_param_set->scan_channel)
		atbm_kfree(scan_param_set->scan_channel);
