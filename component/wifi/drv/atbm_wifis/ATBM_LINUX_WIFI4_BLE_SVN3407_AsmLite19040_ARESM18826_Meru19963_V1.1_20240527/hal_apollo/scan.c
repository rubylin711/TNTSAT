...
{
	int ret, i;
	int tmo = 9000;//reserve more time budget for extreme case, and some customer require scan retry mechanism
	struct atbm_common *hw_priv = ABwifi_vifpriv_to_hwpriv(priv);
...
//		wsm_vif_lock_tx(priv);
		...
		*/
		...
		for (i = 0; i < req_wrap->req->n_ssids; ++i) {
			if(i > 1){
				atbm_printk_err("max ssid is 2,break \n");
				break;
			}
			struct wsm_ssid *dst =
				&hw_priv->scan.ssids[i/*hw_priv->scan.n_ssids*/];
...
void atbm_scan_work(struct atbm_work_struct *work)
{
	...
	if (!hw_priv->scan.req || (hw_priv->scan.curr == hw_priv->scan.end)) {
		...
		*passive scan
		*/
		if(hw_priv->numOfProbeFlag == 1){
			scan.numOfProbeRequests = hw_priv->numOfProbeRequests;
			atbm_printk_err("\n%s:probe req num[%d] \n",__func__,scan.numOfProbeRequests);
		...
		for (i = 0; i < scan.numOfChannels; ++i) {
			...
			if (hw_priv->enable_advance_scan) {
				...
				if(hw_priv->private_scan_flag == true){
					scan.ch[i].minChannelTime = hw_priv->private_min_scan_time;
					scan.ch[i].maxChannelTime = hw_priv->private_max_scan_time;
					//hw_priv->private_scan_flag = false;
					atbm_printk_scan("ch[%d] min[%d] max[%d] \n",
							scan.ch[i].number,scan.ch[i].minChannelTime,scan.ch[i].maxChannelTime);
				}
#ifdef CONFIG_ATBM_APOLLO_TESTMODE
			}
#endif
...
//#endif  //CONFIG_WIRELESS_EXT
	...
	if (likely(atomic_xchg(&hw_priv->scan.in_progress, 0))) {
		if (hw_priv->scan.status > 0)
			...
			hw_priv->scan.curr = hw_priv->scan.end;
#if 0 //Montage added, as vendor suggestion, comment WARN_ON      
			if(WARN_ON(wsm_stop_scan(hw_priv,
						hw_priv->scan.if_id ? 1 : 0)))
#else
			if(wsm_stop_scan(hw_priv,
						hw_priv->scan.if_id ? 1 : 0))
#endif            
			{
				hw_priv->scan.wait_complete= 0;
