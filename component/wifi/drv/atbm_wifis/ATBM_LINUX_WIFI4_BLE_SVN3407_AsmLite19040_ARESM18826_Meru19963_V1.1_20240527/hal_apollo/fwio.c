...
#else
		{"ATBM_asm_fw_sdio.bin"},
#endif
};
char firmware_lite_bin[][40]={
#ifdef USB_BUS
		{"ATBM_lite_fw_usb.bin"},
		{"ATBM_lite_blecomb_usb.bin"},
		{"ATBM_lite_blecomb_auth_usb.bin"},
#else
		{"ATBM_lite_fw_sdio.bin"},
		{"ATBM_lite_blecomb_sdio.bin"},
		{"ATBM_lite_blecomb_auth_sdio.bin"},
#endif
};

char firmware_mercu_bin[][40]={
#ifdef USB_BUS
		{"ATBM_mercu_fw_usb.bin"},
		{"ATBM_mercu_blecomb_usb.bin"},
		{"ATBM_mercu_blecomb_auth_usb.bin"},

#else
		{"ATBM_mercu_fw_sdio.bin"},
		{"ATBM_mercu_blecomb_sdio.bin"},
		{"ATBM_mercu_blecomb_auth_sdio.bin"},
#endif
};
...
char * atbm_HwGetChipFw(struct atbm_common *priv)
{
	...
	switch(priv->chip_version){
		...
		case ARES_LITE:{
			fw=firmware_lite_bin[0];
			if((priv->loader_ble == 1) && (atbm_wifi_bt_comb_get() == 1)){
				fw=firmware_lite_bin[1];
			}else if((priv->loader_ble == 1) && (atbm_wifi_bt_comb_get() == 2)){
				fw=firmware_lite_bin[2];
			}
		}break;
		case Mercurius:{
			fw=firmware_mercu_bin[0];
			if((priv->loader_ble == 1) && (atbm_wifi_bt_comb_get() == 1)){
				fw=firmware_mercu_bin[1];
			}else if((priv->loader_ble == 1) && (atbm_wifi_bt_comb_get() == 2)){
				fw=firmware_mercu_bin[2];
			}
		}break;
		default:{
			fw=firmware_bin[0];
			...
			case Mercurius:{
				...
				if(priv->loader_ble == 1){
					if(atbm_wifi_bt_comb_get() != 2){
						...
						ret = load_usb_mercu_bt_firmware_auth(&fw_altobeam);
					}
#endif

				}else{
					atbm_printk_err("\n======>>> current chip not support BLE <<<======\n\n");
					ret = 0;
					goto error;
				}
#else
...
#ifdef ATBM_ADD_CRERT_FIRMWARE
					else if(atbm_wifi_bt_comb_get() == 2){
						atbm_printk_err("\n======>>>load WIFI BLE COMB CERT firmware <<<======\n\n");
						ret = load_usb_mercu_bt_firmware_auth(&fw_altobeam);
					}
#endif
			...
			case ARES_LITE:{
				...
				if(priv->loader_ble == 1){
					if(atbm_wifi_bt_comb_get() != 2){
						...
						atbm_printk_err("\n======>>> load WIFI BLE COMB CERT firmware <<<======\n\n");
						ret = load_usb_lite_bt_firmware_auth(&fw_altobeam);
					}
#endif
