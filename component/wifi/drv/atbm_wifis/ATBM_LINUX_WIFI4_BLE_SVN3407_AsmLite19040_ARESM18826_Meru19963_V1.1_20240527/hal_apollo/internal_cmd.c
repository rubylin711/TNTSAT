...
bool atbm_internal_cmd_scan_triger(struct ieee80211_sub_if_data *sdata,struct ieee80211_internal_scan_request *req)
{
	...
	if(req->n_channels == 0){
		//if(hw_priv->chip_version == Mercurius)
		{
			...
			if (wiphy->bands[0])
					n_channels += wiphy->bands[0]->n_channels;
		}*/
	}else {
	...
	*/
	if(req->n_channels){
		...
		//if(hw_priv->chip_version == Mercurius)
		{
			...
			/*else{
			for (band = 0; band < IEEE80211_BAND_2GHZ + 1; band++) {
	...
	*/
	...
	if(hw_priv->chip_version == Mercurius){
		...
		for (i = 0; i < IEEE80211_BAND_2GHZ + 1; i++)
			if (wiphy->bands[i])
				scan_req->rates[i] =
					(1 << wiphy->bands[i]->n_bitrates) - 1;
	}	
...
bool atbm_internal_wsm_txpwr_dcxo(struct atbm_common *hw_priv,struct ieee80211_internal_wsm_txpwr_dcxo *txpwr_dcxo)
{
	...
	len = snprintf(cmd, ATBM_WSM_CMD_LEN,"set_txpwr_and_dcxo,%d,%d,%d,%d,0,0,0,0,0,0 ",
...
int atbm_internal_get_ground_noise(struct atbm_common *hw_priv,struct RX_QA_STATUS_S *buf)
{
	int agc_flag,cmd_len = 0,ret = 0;
	char cmd[32] = {0};
	...
	if(agc_flag == 1){
		memcpy(cmd,"AgcStop",7);
		cmd_len = 7;	
	}else{
		memcpy(cmd,"agc 56",6);
		cmd_len = 6;
	}
	wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,cmd, cmd_len, 0);

	msleep(300);
	ret = wsm_get_noise_level(hw_priv,buf,sizeof(struct RX_QA_STATUS_S));
	memset(cmd,0,32);
	if(agc_flag == 1){
		memcpy(cmd,"AgcStart",8);
		cmd_len = 8;	
	}else{
		memcpy(cmd,"agc 0",5);
		cmd_len = 5;
...
int open_auto_cfo(struct atbm_common *hw_priv,int open,int value)
{
	char ppm_buf[16]="cfo 1 0";
//	char ppm_buf_close[16]="cfo 0 0";
	int err = 0,buf_len = 0;

	
	buf_len = snprintf(ppm_buf,16,"cfo %d %d",open,value);
	atbm_printk_err("open_auto_cfo:ppm_buf:%s , buf_len:%d \n",ppm_buf,buf_len+1);
	//if(open){
	err = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, ppm_buf, buf_len, 0);
...
//	char threshold_param[100] = {0};
	...
	u32 chanMin = 0;
	u32 chanMax = 14;
	char certification_cmd[20] = "set_cert ";
	struct ieee80211_local *local = sdata->local;
	...
	if((rate <= WSM_TRANSMIT_RATE_11)&&(len == 0))
	{
		...
		//			atbm_scan_timeout(&hw_priv->scan.timeout.work);
				...
				if((chipversion == 0x49) || (chipversion == 0x4A) ||
					((chipversion >= 0xA0) && (chipversion <= 0xA3)))
				{
					if(((tx_param->precomp_sel == 1) ||  (tx_param->precomp_sel == 2))/* && (channel == 13)*/){
						...
						if(chipversion == 0x49){
							sprintf(certification_cmd,"set_cert,%d ",tx_param->precomp_sel);
							WARN_ON(wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,
								certification_cmd, sizeof(certification_cmd), vif->if_id));
						}
						else{
		...
		atbm_for_each_vif(hw_priv,vif,i){
			if((vif != NULL)){
				...
				else
				{
					...
					((chipversion >= 0xA0) && (chipversion <= 0xA3)))
					{
						if(((tx_param->precomp_sel == 1) ||  (tx_param->precomp_sel == 2)) && (rate <= WSM_TRANSMIT_RATE_11)){
							atbm_printk_always("----SRRC--DSSS--\n");								
						}
						else if(((tx_param->precomp_sel == 1) || (tx_param->precomp_sel == 2)) && (rate >= WSM_TRANSMIT_RATE_6)){
							...
							if(rate > WSM_TRANSMIT_RATE_11){//ofdm
								if((is_40M == 0)/* && ((channel == 1) || (channel == 13))*/){
									if((channel < 7) || ((channel >= 36) && (channel < 116)))
										precomp_sel = 3;
									else 
										precomp_sel = 4;
								}
								else if((is_40M >= 1)/* && ((channel == 5) || (channel == 13))*/){
									if((channel < 9) || ((channel >= 36) && (channel < 118)))
										precomp_sel = 3;
									else 
										precomp_sel = 4;
								}
						...
						if(chipversion == 0x49){
							sprintf(certification_cmd,"set_cert,%d ",precomp_sel);
							WARN_ON(wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD,
								certification_cmd, sizeof(certification_cmd), vif->if_id));
						}
						else{
...
extern void atbm_Mercurius_ECO_set(int status);
int atbm_Default_driver_configuration(struct atbm_common *hw_priv)
{
	...
	if((hw_priv->chip_version == Mercurius) && (hw_priv->efuse.version == 17)){
		atbm_Mercurius_ECO_set(1);
	}
		...
		if(strfilename && gpio4){
			...
			if(access_file(strfilename,readbuf,1024,1) > 0)
			{
	...
	if(hw_priv->chip_version == Mercurius)
			...
			wsm_set_rate_power_5g(hw_priv,1);
#else
...
int atbm_evize_get_deauth_reason_code(struct atbm_common *hw_priv)
{
	
	return hw_priv->evize_reason_code;

}



int atbm_rate_to_string(int rate,char *rate_str)
{

	if(rate_str == NULL){
		atbm_printk_err("atbm_rate_to_string, rate_str = NULL\n");
		return -1;
	}

	if(rate > 585 && rate <= 650)
		memcpy(rate_str,"MCS7",4);
	else if (rate > 520 && rate <= 585)
		memcpy(rate_str,"MCS6",4);
	else if (rate > 390 && rate <= 520)
		memcpy(rate_str,"MCS5",4);
	else if (rate > 260 && rate <= 390)
		memcpy(rate_str,"MCS4",4);
	else if (rate > 195 && rate <= 260)
		memcpy(rate_str,"MCS3",4);
	else if (rate > 130 && rate <= 195)
		memcpy(rate_str,"MCS2",4);
	else if (rate > 110 && rate <= 130)
		memcpy(rate_str,"MCS1",4);
	else if (rate > 65 && rate <= 110)
		memcpy(rate_str,"MCS0",4);
	else if (rate > 2 && rate < 65)
		memcpy(rate_str,"5.5M",4);
	else if (rate > 1 && rate <= 2)
		memcpy(rate_str,"2M",2);
	else
		memcpy(rate_str,"1M",2);
	

	return 0;
}

int atbm_evize_thread_start(void * arg)
{
	static int cnt=0;
	struct atbm_common *hw_priv = (struct atbm_common *)arg;
	struct TX_RX_Statistics_S txrx_stat;
	unsigned int rate_val = 0,ret = 0 ,wifi_status = 0;
	struct sta_info *sta = NULL;
	struct ieee80211_local *local = hw_to_local(hw_priv->hw);
	...
	while(1){
		...
		sdata = NULL;
		//msleep(1000);
		wait_event_interruptible_timeout(hw_priv->evize_wq,hw_priv->evize_thread_stop == 1,1*HZ);
		if(hw_priv->evize_thread_stop == 1){
			atbm_printk_err("atbm_evize_thread  stop \n");
			break;
		}
		rcu_read_lock();
	    list_for_each_entry_rcu(sta, &local->sta_list, list) {	
			if(!sta)
				continue;
			wifi_status = test_sta_flag(sta, WLAN_STA_AUTHORIZED);
	        if ((sta->sdata->vif.type == NL80211_IFTYPE_STATION) && wifi_status){	
				sdata = sta->sdata;
				atbm_printk_err("ifname[%s],wifi_status=%d\n",sdata->name,wifi_status);
				break;
			}
	    }
		rcu_read_unlock();
		
		if(wifi_status && sdata){
		
			ret = atbm_get_txrx_status(hw_priv,(char *)&txrx_stat,sizeof(struct TX_RX_Statistics_S));
			
			tx_rate = atbm_internal_get_current_txrate(sdata,NULL);
			
			wsm_read_mib(hw_priv, WSM_MIB_ID_GET_RATE, &rate_val, sizeof(unsigned int), 0);

			mutex_lock(&hw_priv->evize_thread_mutex);
			memset(&hw_priv->wifi_rate_state[cnt],0,sizeof(atbm_evize_link_state));
			atbm_rate_to_string(txrx_stat.rx_rate_avg ,hw_priv->wifi_rate_state[cnt].rx_link_rate);
			atbm_rate_to_string(tx_rate ,hw_priv->wifi_rate_state[cnt].tx_link_rate);
			hw_priv->wifi_rate_state[cnt].tx_throughput = rate_val/2048*65/100;
			hw_priv->wifi_rate_state[cnt].rx_throughput = 0;
			mutex_unlock(&hw_priv->evize_thread_mutex);
			
			atbm_printk_err("[%d]tx_link_rate  = %s\n",cnt, hw_priv->wifi_rate_state[cnt].tx_link_rate);
			atbm_printk_err("[%d]rx_link_rate  = %s\n",cnt, hw_priv->wifi_rate_state[cnt].rx_link_rate);
			atbm_printk_err("[%d]tx_throughput = %d\n",cnt, hw_priv->wifi_rate_state[cnt].tx_throughput);
			atbm_printk_err("[%d]rx_throughput = %d\n",cnt, hw_priv->wifi_rate_state[cnt].rx_throughput);
			...
			cnt = 0;
			memset(hw_priv->wifi_rate_state,0,sizeof(hw_priv->wifi_rate_state));
			msleep(1000);
		}
...
int atbm_evize_thread_init(struct atbm_common *hw_priv)
{
	if(!hw_priv){
		atbm_printk_err("atbm_evize_thread_init : hw_priv is NULL\n");
		return -1;
	}
	mutex_init(&hw_priv->evize_thread_mutex);
	mutex_init(&hw_priv->evize_reason_mutex);
	init_waitqueue_head(&hw_priv->evize_wq);
	
	hw_priv->evize_thread_stop = 0;
	hw_priv->evize_thread = NULL;
	hw_priv->evize_thread = kthread_create(&atbm_evize_thread_start, hw_priv, ieee80211_alloc_name(hw_priv->hw,"atbm_evize"));

	
	if (!hw_priv->evize_thread){
		atbm_printk_err("atbm_evize_thread_init :error!! \n");
		hw_priv->evize_thread = NULL;
		return -1;
	}else{
		wake_up_process(hw_priv->evize_thread);
	}
	
	return 0;
}

int atbm_evize_thread_exit(struct atbm_common *hw_priv)
{

	struct task_struct *thread = hw_priv->evize_thread;
	if (WARN_ON(!thread))
		return -1;
	hw_priv->evize_thread_stop = 1;
	wake_up(&hw_priv->send_prvmgmt_wq);
	kthread_stop(thread);
	mutex_destroy(&hw_priv->evize_thread_mutex);
	mutex_destroy(&hw_priv->evize_reason_mutex);
	return 0;
