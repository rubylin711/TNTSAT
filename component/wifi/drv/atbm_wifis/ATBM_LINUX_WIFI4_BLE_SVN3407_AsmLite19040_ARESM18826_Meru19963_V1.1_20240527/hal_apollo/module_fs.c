...
extern atbm_caps_display extcaps_display[];
static void atbm_module_firmware_caps_show(struct atbm_module_show *show_buff,struct atbm_common *hw_priv)
{
	...
	atbm_module_show_put(show_buff,"EX_CAPABILITIES_DRIVER_PROCESS_BA	[%d]\n",!!(EX_CAPABILITIES_DRIVER_PROCESS_BA));
...
static void atbm_module_driver_caps_show(struct atbm_module_show *show_buff,struct atbm_common *hw_priv)
{
	...
	atbm_module_show_put(show_buff,"HW_CHW      2.4G[%s]\n","20M");
#else
...
#ifdef ATBM_NOT_SUPPORT_5G_40M_CHW
	atbm_module_show_put(show_buff,"HW_CHW      5G[%s]\n","20M");
#else
	atbm_module_show_put(show_buff,"HW_CHW      5G[%s]\n","40M");
#endif
	}
			     ...
			     struct kobj_attribute *attr, char *buf)
{
	...
	atbm_module_show_put(&sys_show,LIGHT"example: echo LOG_MGMT=ON > /sys/module/xxx/atbmfs/atbm_printk_mask"NORMAL ENTER);
