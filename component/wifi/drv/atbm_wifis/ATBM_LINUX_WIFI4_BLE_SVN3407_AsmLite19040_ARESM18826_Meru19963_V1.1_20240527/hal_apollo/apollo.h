...
typedef struct{
	...
	u32 rx_throughput;//kbps
}atbm_evize_link_state; 
#endif
struct atbm_common {
...
#endif
#ifdef ATBM_EVIZE_CUSTOMER_FUNC
	struct task_struct		*evize_thread;
	struct mutex			evize_thread_mutex;
	wait_queue_head_t		evize_wq;
	int evize_thread_stop;
	atbm_evize_link_state wifi_rate_state[20];
	struct mutex			evize_reason_mutex;
	int evize_reason_code;
#endif
...
*/
	...
	bool private_scan_flag;
	int private_min_scan_time;
	int private_max_scan_time;
	u8 numOfProbeFlag;
...
int mdns_offload_chr_init(const char *name);
