...
static int ieee80211_check_limit_scan_chan(struct ieee80211_local *local,struct cfg80211_scan_request *req)
{
#if 0
	struct atbm_common *hw_priv=local->hw.priv;
	struct wiphy *wiphy = local->hw.wiphy;
	short chan_num_id[100] = {0};
	char scan_n_channels[5] = {0};
	int support_all_channels = 0;
#endif
	int scan_n_chans = 0,scan_chan = 0,freq = 0;
	int channel_2_start,channel_2_end,channel_5_start,channel_5_end;
	int j = 0,k = 0;
	void *pptr[100];
#if 0	
	support_all_channels = ieee80211_get_support_channel(local,chan_num_id,scan_n_channels);
	if(support_all_channels == 0){
		atbm_printk_err("%s : not find support channel\n",__func__);
		return 0;
	}
#endif
	 ...
	 channel_2_start = local->channel_2_4G_start;
	 channel_2_end  = local->channel_2_4G_end;
	 channel_5_start = local->channel_5G_start;
	 channel_5_end = local->channel_5G_end;
	if(channel_2_start < 0){
		return 0;
	}
	atbm_printk_err("2.4G start[%d] end[%d]  5G start[%d] end[%d] \n",
		local->channel_2_4G_start,local->channel_2_4G_end,local->channel_5G_start,local->channel_5G_end);
	scan_n_chans = req->n_channels;
	atbm_printk_err("%s:init scan channels[%d] \n",__func__,scan_n_chans);
	//atbm_printk_err("support channel : ");
	for(j = 0; j < req->n_channels ; j++){
		...
		*/
		if(scan_chan <= 14){
			if((scan_chan != 0) && ((scan_chan >= channel_2_start) && (scan_chan <= channel_2_end))){
			...
			if((scan_chan != 0) && ((scan_chan >= channel_5_start) && (scan_chan <= channel_5_end))){
				pptr[k++] = req->channels[j];
			//		printk("%d ",scan_chan);
				continue;
			}else{
	...
	//printk("\n");
	atbm_printk_err("%s:change scan channels[%d] \n",__func__,scan_n_chans);
	if(scan_n_chans <= 0){
...
static int ieee80211_scan_results_limit(struct ieee80211_local *local,
														struct sk_buff *skb,
														struct ieee80211_channel *channel)
{
	int freq,curr_chan,fc;
	...
	//atbm_printk_err("2.4G start[%d] end[%d] \n 5G start[%d] end[%d] \n",
	//		local->channel_2_4G_start,local->channel_2_4G_end,local->channel_5G_start,local->channel_5G_end);
	...
	if(skb){
		...
		if (!presp && !beacon){
			atbm_printk_err("%s :not beacon & probe resp\n",__func__);
			return 0;
		}
	...
	if(curr_chan <= 14){
			if((curr_chan < channel_2_start) || (curr_chan > channel_2_end)){
				atbm_printk_err("packet reject!curr_chan=%d \n",curr_chan);
				return -1;
			}
	}else{
		if((curr_chan < channel_5_start) || (curr_chan > channel_5_end)){
			atbm_printk_err("packet reject!curr_chan=%d \n",curr_chan);
			return -1;
		}
	}
...
static int ieee80211_check_country_limit_scan_chan(struct ieee80211_local *local,struct cfg80211_scan_request *req)
{
	...
	if((country_chan > 0) && (req != NULL)){
		atbm_printk_debug("%s : scan_n_channals = %d ++++++++ \n",__func__,req->n_channels);
		...
		for(j = 0; j < req->n_channels ; j++){
			...
			if(((scan_chan >= 1) && (scan_chan <= 14)) && (scan_chan <= country_chan)){
				pptr[k++] = req->channels[j];
				atbm_printk_debug("%s :country[%c%c] support chan [%d] ++++++++ \n",__func__,
...
#ifdef ATBM_ARP_ANDROID
	struct ieee80211_if_managed *ifmgd=NULL;
	sdata = local->scan_sdata;
	ifmgd = &sdata->u.mgd;
	if(ifmgd && ifmgd->associated){
		local->scan_channel_space = 1;
		local->scan_idle_time     = 100;
	}
#endif
			  ...
			  bool beacon)
{
	...
	/* ========== ???????? BSS ?? SSID ??????? ========== */
	const char *ssid_str = "[unknown]";
	int ssid_len = 0;

	if (elems && elems->ssid && elems->ssid_len > 0 && elems->ssid_len <= 32) {
		ssid_str = (const char *)elems->ssid;
		ssid_len = elems->ssid_len;
	}

	__le16 capab = ieee80211_is_probe_resp(mgmt->frame_control) ?
		mgmt->u.probe_resp.capab_info : mgmt->u.beacon.capab_info;

//	atbm_printk_always("BSS: %pM ch%d '%.*s' Privacy=%s\n",
//		mgmt->bssid,
//		ieee80211_frequency_to_channel(channel->center_freq),
//		ssid_len, ssid_str,
//		(le16_to_cpu(capab) & WLAN_CAPABILITY_PRIVACY) ? "YES" : "NO");
	/* ====================================================== */
	...
	*/
...
#else // android  CtsWifiTestCases
	struct timespec ts;
	ts = ktime_to_timespec(ktime_get_boottime());
	#define ATBM_BOOT_TIME  (u64)ts.tv_sec * 1000000 + div_u64(ts.tv_nsec, 1000)
#endif
	#endif
	...
	if(ieee80211_scan_results_limit(local,NULL,channel) < 0)
		return NULL;
	...
	if (!cbss) {
		atbm_printk_err("BSS_UPDATE_FAIL: BSSID=%pM not added!\n", mgmt->bssid);
		return NULL;
	}
...
ieee80211_scan_rx_internal(struct ieee80211_sub_if_data *sdata, struct sk_buff *skb)
{
	...
	if(ieee80211_scan_results_limit(sdata->local,skb,NULL) < 0)
		return handle;
				  ...
				  struct cfg80211_scan_request *req)
{
	...
	if(ieee80211_check_limit_scan_chan(local,req) < 0){
		atbm_printk_scan("%s:%d\n",__func__,__LINE__);
		return -EBUSY;
	}	
