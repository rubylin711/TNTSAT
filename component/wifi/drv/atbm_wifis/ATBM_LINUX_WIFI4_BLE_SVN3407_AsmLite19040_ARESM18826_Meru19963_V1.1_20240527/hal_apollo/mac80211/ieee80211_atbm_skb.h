...
#if (LINUX_VERSION_CODE <= KERNEL_VERSION(4,10,0)) //(4, 9, 129))
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(6, 3, 0)) /* Montage, added */
#define atbm_amsdu_to_8023s(_skb, _list,_addr,_iftype,_extra_headroom)		\
		do {																\
			struct ethhdr _eth;												\
...
#endif
#else  /* Montage, Wliao added */
#define atbm_amsdu_to_8023s(_skb, _list,_addr,_iftype,_extra_headroom)		\
		do {																\
			struct ethhdr _eth;												\
			if(atbm_ieee80211_data_to_8023_exthdr(_skb,&_eth,_addr,_iftype) == 0)	\
				ieee80211_amsdu_to_8023s(_skb, _list,_addr,_iftype,_extra_headroom,NULL,NULL,false);	\
			else	\
				atbm_dev_kfree_skb(_skb);	\
		}while(0)
#endif
