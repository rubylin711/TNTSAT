					   ...
					   void *cookie, bool send_frame)
{
	...
	mgmt->u.deauth.reason_code = cpu_to_le16(reason);
		...
		hw_priv->evize_reason_code = mgmt->u.deauth.reason_code;
			 ...
			 struct atbm_ieee80211_mgmt *mgmt, size_t len)
{
	...
	mutex_lock(&hw_priv->evize_reason_mutex);
	hw_priv->evize_reason_code = reason_code;
	mutex_unlock(&hw_priv->evize_reason_mutex);
#endif
...
static const u64 care_about_ies =
	...
	(1ULL << ATBM_WLAN_EID_EXT_CHANSWITCH_ANN) |
	(1ULL << ATBM_WLAN_EID_SECONDARY_CH_OFFSET);
				  ...
				  struct sk_buff *skb)
{
	...
	if (!ieee80211_is_mgmt(mgmt->frame_control)){
		atbm_printk_mgmt("%s:recv(%x) drop\n",sdata->name,mgmt->frame_control);
		return;
	}
	...
	case WLAN_AUTH_SAE:
		...
		if (status_code != WLAN_STATUS_SUCCESS) {
			if (auth_alg == WLAN_AUTH_SAE &&
				...
				ieee80211_free_authen_bss(sdata);
				sta_info_destroy_addr(sdata,wk->probe_auth.bss->bssid);
				ieee80211_auth_deliver_sae(sdata, mgmt, len);
