...
static int atbm_ioctl_set_Tjroom(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext);
static int atbm_ioctl_get_assoc_info(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra);
...
static int atbm_set_ap_sleep_alive_time(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext);
static int atbm_ioctl_set_chan_scan_limit(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext);
...
static int atbm_ioctl_set_rate_txpwr_by_file_5g(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra);
...
iwripv_common_cmd_t common_cmd[]={
...
#endif
	{"set_tjroom",10,atbm_ioctl_set_Tjroom,"set chip tjroom"},
	{"get_tjroom",10,atbm_ioctl_get_Tjroom,"get chip tjroom"},
	{"get_assoc_info",14,atbm_ioctl_get_assoc_info,"get_assoc_info"},
	...
	{"set_whole_efuse",15,atbm_ioctl_set_all_efuse_V2,"set all efuse,including:dcxo,deltagain,tjroom,macaddr"},
#ifdef ATBM_PRIVATE_IE
...
#endif
#ifdef CONFIG_RATE_TXPOWER_5G
	{"set_rate_5gpower",16,atbm_ioctl_set_rate_txpwr_by_file_5g,"read "CONFIG_RATE_TXPOWER_5G" to set Power at different rates"},
#else
	{"set_rate_5gpower",16,atbm_ioctl_set_rate_txpwr_by_file_5g,"read /tmp/set_rate_power_5g.txt to set Power at different rates"},
#endif
...
//	{"send_deauth",11,atbm_ioctl_send_deauth,"send brodcast deauth frame"},
	...
	{"ap_alive_time",13,atbm_set_ap_sleep_alive_time,"0:normal,1:set rf low power"},
	{"chan_limit",10,atbm_ioctl_set_chan_scan_limit,"4 param : iwpriv wlan0 common chan_limit,srart1,end1,start2,end2 ,start1&end1 is 2.4G channel ,start2&end2 is 5G channel"},
	{NULL,0,NULL,NULL}
};
...
static int atbm_ioctl_set_SIGMSTAR_256BITSEFUSE(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	if ((ret = wsm_set_SIGMSTAR_256BITSEFUSE(hw_priv, &efuseBuff[0], efuse_len)) == 0)
	{
...
//	int index = 0;
	...
	if(writeEfuseFlag)
	{
		...
		if(set_deltagain_flag){
			atbm_printk_err("set deltagain effect immediately && SET TO EFUSE\n");
			memset(writebuf, 0, sizeof(writebuf));
			sprintf(writebuf, "set_txpwr_and_dcxo,%d,%d,%d,%d,0,0,0,0,0,0 ", efuse_temp.delta_gain1,
		...
		//5G
		if(set_deltagain_5g){
			atbm_printk_err("set 5g deltagain effect immediately && SET TO EFUSE\n");
			memset(writebuf, 0, sizeof(writebuf));
	...
	else
	{
		if(set_dcxo_flag){
			...
			atbm_printk_err("set 5g deltagain effect immediately \n");
			memset(writebuf, 0, sizeof(writebuf));
			sprintf(writebuf, "set_txpwr_5g,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",
			efuse_temp.delta_gain1_5g,efuse_temp.delta_gain2_5g,efuse_temp.delta_gain3_5g,efuse_temp.delta_gain4_5g,efuse_temp.delta_gain5_5g,
			efuse_temp.delta_gain6_5g,efuse_temp.delta_gain7_5g,efuse_temp.delta_gain8_5g,efuse_temp.delta_gain9_5g,efuse_temp.delta_gain10_5g);
...
static int atbm_ioctl_set_Tjroom(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	if(memcmp(cmd, "set_tjroom", 10) == 0)
	{
		CmdLine_GetSignInteger(&pRxData, &rxData);
		if(rxData < 0 || rxData > 127){
			atbm_printk_err("ERR!!!  tj_room range is 0~127,now input value is [%d] \n",rxData);
			atbm_kfree(extra);
			return -EINVAL;
		}
		memset(writebuf, 0, sizeof(writebuf));
		sprintf(writebuf, "set_tjroom,%d ", rxData);
...
static int atbm_ioctl_get_assoc_info(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra){
	...
	if(sdata->vif.type == NL80211_IFTYPE_STATION){
		...
		if(ifmgd->associated){
			...
			if (sta) {
				sta_ptk = rcu_dereference(sta->ptk);
				if(sta_ptk){
					cipher = sta_ptk->conf.cipher;
					switch (cipher) {
				...
				is_40M = test_bit(WLAN_STA_40M_CH, &sta->_flags);
				atbm_printk_err("cipher:%s, is_40M:%d",enc_type,is_40M);
				sprintf(pbuff, "cipher:%s, is_40M:%d", enc_type,is_40M);
				if(extra){	
					memcpy(extra, (char *)pbuff, strlen(pbuff));	
					wrqu->data.length = strlen(pbuff);
				}
	...
	*/
	...
	list_for_each_entry_rcu(sta, &local->sta_list, list) {
		...
		*/
		...
		copy_len = scnprintf(results + total_len,512-total_len,"mac[%pM],rssi[%d],bg[%x],11n[%x],bandwidth[%s]\n",
		sta->sta.addr,(s8)-atbm_ewma_read(&sta->avg_signal2),sta->sta.supp_rates[chan_state->oper_channel->band],
		sta->sta.ht_cap.mcs.rx_mask[0],test_sta_flag(sta,WLAN_STA_40M_CH)==1?"HT40":"HT20");
...
static int atbm_ioctl_set_txpwr_by_file(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	if(access_file(STR_FILE,readbuf,1024,1) > 0)
	{
		atbm_printk_always("param:\n%s\n",readbuf);		
		if(hw_priv->chip_version == Mercurius){
			...
			acture_len = snprintf(readbuf, 1024,"set_txpwr_and_dcxo,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",deltagain[0], deltagain[1], deltagain[2], deltagain[13],
				bgain[0], bgain[1], bgain[2],gngain[0], gngain[1], gngain[2]);
			atbm_printk_always("2.4G acture_len:%d , cmd: %s\n",acture_len, readbuf);
			ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, readbuf, acture_len, if_id);
			//5G deltagain
			memset(readbuf, 0, 1024);
			acture_len = snprintf(readbuf,1024,"set_txpwr_5g,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",
			...
			atbm_printk_always("5G acture_len:%d , cmd: %s\n",acture_len, readbuf);
		ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, readbuf, acture_len, if_id);
		}
		else{
			...
			//memset(readbuf, 0, sizeof(readbuf));
			acture_len = snprintf(readbuf,1024, "set_txpwr_and_dcxo,%d,%d,%d,%d,%d,%d,%d,%d,%d,%d ",deltagain[0], deltagain[1], deltagain[2], deltagain[3],
				bgain[0], bgain[1], bgain[2],gngain[0], gngain[1], gngain[2]);
			atbm_printk_always("acture_len:%d , cmd: %s\n",acture_len, readbuf);
			ret = wsm_write_mib(hw_priv, WSM_MIB_ID_FW_CMD, readbuf, acture_len, if_id);
		}
...
extern int get_rate_delta_gain_5g(s8 *dst);
static int atbm_ioctl_set_rate_txpwr_by_file_5g(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	struct ieee80211_sub_if_data *sdata = IEEE80211_DEV_TO_SUB_IF(dev);
	struct ieee80211_local *local = sdata->local;
	...
	CmdLine_GetSignInteger(&pRxData, &useFlag);
#ifdef CONFIG_RATE_TXPOWER_5G
	wsm_set_rate_power_5g(hw_priv,useFlag);
#else
	atbm_printk_err("undefine CONFIG_RATE_TXPOWER_5G,so Not support\n");
#endif
	if(pbuffer)
		atbm_kfree(pbuffer);
...
static int atbm_set_ap_sleep_alive_time(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	struct atbm_common *hw_priv=local->hw.priv;
	struct atbm_vif *priv = (struct atbm_vif *)sdata->vif.drv_priv;
	unsigned int ap_alive_time = 0,ptr_len = 0;
	unsigned char ptr[64] = {0};
	if (!ieee80211_sdata_running(sdata)){
		atbm_printk_err("%s:net interface not runing! \n",__func__);
		return  -EINVAL;
	}
	
	if(sdata->vif.type != NL80211_IFTYPE_AP){
		atbm_printk_err("%s:interface not run AP mode! \n",__func__);
		return  -EINVAL;
	}	
	...
	sscanf(ptr,"ap_alive_time,%d",&ap_alive_time);
	atbm_printk_err("ap_alive_time:%d ,\n",ap_alive_time,priv->vif->bss_conf.beacon_int);

	if(ap_alive_time > priv->vif->bss_conf.beacon_int){
		atbm_printk_err("%s() ap_alive_time > beacon_int(%d)!!\n",__func__,priv->vif->bss_conf.beacon_int);
		return -EINVAL;
	}
	priv->p2p_ps_modeinfo.oppPsCTWindow = ap_alive_time;
	if(ap_alive_time > 0){
...
static int atbm_ioctl_set_chan_scan_limit(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *ext)
{
	...
	atbm_printk_err("ptr %s\n",ptr);
	sscanf(ptr,"chan_limit,%d,%d,%d,%d",
		&ch_ilmit.channel_2_4G_start,&ch_ilmit.channel_2_4G_end,
		&ch_ilmit.channel_5G_start,&ch_ilmit.channel_5G_end);
	
	atbm_printk_err("chan_limit,%d,%d,%d,%d\n",ch_ilmit.channel_2_4G_start,ch_ilmit.channel_2_4G_end,
		ch_ilmit.channel_5G_start,ch_ilmit.channel_5G_end);
...
static int atbm_ioctl_get_cfg_txpower_by_file(struct net_device *dev, struct iw_request_info *info, union iwreq_data *wrqu, char *extra)
{
	...
	struct cfg_txpower_v2_t configured_txpower_v2;
	...
	memset(&configured_txpower_v2,0, sizeof(configured_txpower_v2));
	
#if defined CONFIG_TXPOWER_DCXO_VALUE || defined CONFIG_RATE_TXPOWER || defined CONFIG_RATE_TXPOWER_5G

	if(hw_priv->chip_version == Mercurius)
	{
		if ((ret = wsm_get_cfg_txpower(hw_priv, (void *)&configured_txpower_v2, sizeof(configured_txpower_v2))) == 0){	
			for(i=0;i<sizeof(configured_txpower_v2.set_txpwr_delta_gain);i++)
				atbm_printk_err("gain%d:%d\n",i+1,configured_txpower_v2.set_txpwr_delta_gain[i]);
			for(i=0;i<sizeof(configured_txpower_v2.set_b_txpwr_delta_gain);i++)
				atbm_printk_err("bgain%d:%d\n",i+1,configured_txpower_v2.set_b_txpwr_delta_gain[i]);
			for(i=0;i<sizeof(configured_txpower_v2.set_gn_txpwr_delta_gain);i++)
				atbm_printk_err("gngain%d:%d\n",i+1,configured_txpower_v2.set_gn_txpwr_delta_gain[i]);  
			for(i=0;i<sizeof(configured_txpower_v2.different_rate_txpower_mode);i++)
				atbm_printk_err("%s:%d\n",ratebuf[i],configured_txpower_v2.different_rate_txpower_mode[i]);
			for(i=0;i<sizeof(configured_txpower_v2.different_rate_txpower_mode_40M);i++)
				atbm_printk_err("%s_40M:%d\n",ratebuf[i],configured_txpower_v2.different_rate_txpower_mode_40M[i]);
			for(i=0;i<sizeof(configured_txpower_v2.different_rate_txpower_mode);i++)
				atbm_printk_err("%s_5g:%d\n",ratebuf[i],configured_txpower_v2.different_rate_txpower_mode_5g[i]);
			for(i=0;i<sizeof(configured_txpower_v2.different_rate_txpower_mode_40M);i++)
				atbm_printk_err("%s_40M_5g:%d\n",ratebuf[i],configured_txpower_v2.different_rate_txpower_mode_40M_5g[i]);
		}
		else{
...
#else
		atbm_printk_err("%s:ieee80211_set_sta_channel not support!! \n",__func__);
#endif
	}else{
#ifdef CONFIG_IEEE80211_SPECIAL_FILTER
