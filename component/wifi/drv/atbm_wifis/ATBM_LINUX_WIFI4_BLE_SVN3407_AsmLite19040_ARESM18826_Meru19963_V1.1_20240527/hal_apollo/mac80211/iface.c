...
static void ieee80211_sdata_gro_setup(struct ieee80211_sub_if_data *sdata)
{
	...
	if(sdata->vif.type == NL80211_IFTYPE_AP || sdata->vif.type == NL80211_IFTYPE_STATION){
		...
		netif_napi_add_weight(ndev,&sdata->napi_rx,ieee80211_rx_napi_poll,64); //Montage ,wliao modified
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,16,0)) /* Montage,Added */
		dev_addr_mod(dev,0,local->hw.wiphy->perm_addr, ETH_ALEN);
#else
		memcpy(dev->dev_addr,
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,6,0))  /* Montage, added */
	  mutex_lock(&local->mtx);
#endif
...
#endif
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(5,15,0))
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,6,0))  /* Montage, Wliao added */
	wiphy_lock(sdata->wdev.wiphy);
#endif
	ret = cfg80211_register_netdevice(ndev);
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(6,6,0))  /* Montage, Wliao added */
	wiphy_unlock(sdata->wdev.wiphy);
#endif
#else
