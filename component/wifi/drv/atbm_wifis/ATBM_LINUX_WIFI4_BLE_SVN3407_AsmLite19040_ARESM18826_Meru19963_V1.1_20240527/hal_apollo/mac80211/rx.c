...
#endif
        }else if((rx->sdata->vif.type == NL80211_IFTYPE_STATION) && rx->sta){
#ifdef CONFIG_ATBM_SUPPORT_DETECT_REAUTH
            if(!is_multicast_ether_addr(hdr->addr1)){
                rx->sta->reauthen_detect ++;
            }
            if(rx->sta->reauthen_detect > STA_REAUTH_LIMIT){
                atbm_printk_err("%s:reauthen_detect[%d]\n",__func__,rx->sta->reauthen_detect);
                ieee80211_connection_loss(&rx->sdata->vif);
            }
#endif
        }
		return RX_DROP_MONITOR;
...
#ifdef CONFIG_ATBM_SUPPORT_DETECT_REAUTH
    if(rx->sta && !is_multicast_ether_addr(ehdr->h_dest)){
        if(rx->sta->reauthen_detect){
            atbm_printk_err("%s:reauthen_detect clear[%d]",__func__,rx->sta->reauthen_detect);
        }
        rx->sta->reauthen_detect = 0;
    }
#endif
	skb = rx->skb;
	xmit_skb = NULL;
...
bool ieee80211_rx_sta_cook_deauthen(struct sta_info *sta)
{
	...
	status->band = IEEE80211_BAND_2GHZ;
		...
		mutex_lock(&hw_priv->evize_reason_mutex);
		hw_priv->evize_reason_code = deauth->u.deauth.reason_code;
		mutex_unlock(&hw_priv->evize_reason_mutex);
#endif
					 ...
					 ieee80211_rx_result res)
{
	...
	if(res != RX_QUEUED){
		ieee80211_special_check_package(rx->sdata->local,rx->skb);
	}
#endif	
	switch (res) {
