				...
				struct ieee80211_csa_work *csa_req)
{
	...
	if(csa_req->block){
		...
		drv_flush(sdata->local,sdata, false);
	}
...
static void ieee80211_csa_beacon_update_after(struct ieee80211_sub_if_data *sdata)
{
	...
	if(beacon_after){
		...
		ieee802_11_parse_elems(mgmt->u.beacon.variable,old->head_len - (mgmt->u.beacon.variable - old->head) ,&elems);
	...
	if(proberesp_after){
		...
		ieee802_11_parse_elems(mgmt->u.probe_resp.variable,old->head_len - (mgmt->u.probe_resp.variable - old->head) ,&elems);
...
static void ieee80211_chan_pre_complete_csa(struct ieee80211_local *local,struct ieee80211_csa_status *status)
{
	...
	list_for_each_entry(csa_req, &local->csa_req, list){
		...
		if(ieee80211_chan_complete_csa(csa_req) == true){
			status->n_suces ++;
			csa_req->success = true;
		}else {
			csa_req->success = false;
		}
	}
	/*
	*has fail
