...
#ifdef CONFIG_ATBM_AUTH_WITH_DEAUTH
			  ...
			  struct cfg80211_auth_request *req)
{
	...
	if(trans == 1){
		atbm_printk_err("before auth ,send deauth channel = %d\n",channel_hw_value(req->bss->channel));
		u8 da_broadcast[6]={0xff,0xff,0xff,0xff,0xff,0xff};
		local->listen_sdata = sdata;
		local->listen_channel = req->bss->channel;
		local->ops->sta_triger_listen(&local->hw,&local->listen_sdata->vif,local->listen_channel);
		local->listen_started = true;
		ieee80211_send_deauth(sdata,da_broadcast);
		ieee80211_send_deauth(sdata,req->bss->bssid);
		local->ops->sta_stop_listen(&local->hw,&local->listen_sdata->vif);
		local->listen_started = false;
		local->listen_sdata = NULL;
		local->listen_channel =0;
	}
#endif
