...
static int Mercurius_ECO = 0;

void atbm_Mercurius_ECO_set(int status)
{
	Mercurius_ECO = status;
}

int atbm_Mercurius_ECO_get(void)
{
	return Mercurius_ECO;
}
...
// Montage added macro ATBM_BLE_LOAD
static int wifi_bt_comb = ATBM_BLE_LOAD;
...
struct ieee80211_channel atbm_5ghz_pretend_2g_chantable[] = {
...
static struct ieee80211_supported_band atbm_band_5ghz = {
	...
	.ht_cap = {
		.cap = IEEE80211_HT_CAP_GRN_FLD |
#ifndef ATBM_NOT_SUPPORT_5G_40M_CHW
			IEEE80211_HT_CAP_SUP_WIDTH_20_40|
			IEEE80211_HT_CAP_DSSSCCK40|
#endif
#ifdef CONFIG_ATBM_APOLLO_SUPPORT_SGI
...
struct ieee80211_hw *atbm_init_common(size_t hw_priv_data_len)
{
	...
	hw->ba_tid_rx_mask = ampdu;
	hw->ba_tid_tx_mask = ampdu;
...
void atbm_free_common(struct ieee80211_hw *dev)
{
	...
	atbm_evize_thread_exit(hw_priv);
...
#ifdef	CONFIG_ATBM_APOLLO_5GHZ_SUPPORT
#if ((LINUX_VERSION_CODE > KERNEL_VERSION(3, 9, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(3, 10, 0)))||(LINUX_VERSION_CODE >= KERNEL_VERSION(3, 11, 0))
...
int get_rate_delta_gain_eco(s8 *dst)
{
	int i=0,count = 0,readnum=0,val = 0;
	char *file = NULL;
	char readbuf[512] = {0};
	char *pdata[24] = {NULL};
	char *pstr = NULL;
#ifdef CONFIG_RATE_TXPOWER_ECO
	file = CONFIG_RATE_TXPOWER_ECO;
#else
	file = "/tmp/set_rate_power_eco.txt";
#endif
...
int get_rate_delta_gain_5g(s8 *dst)
{
	int i=0,count = 0,readnum=0,val = 0;
	char *file = NULL;
	char readbuf[512] = {0};
	...
	file = CONFIG_RATE_TXPOWER_5G;
#else
	file = "/tmp/set_rate_power_5g.txt";
#endif
	if(dst == NULL)
	{
		atbm_printk_err("Invalid parameter,dst == NULL\n");
...
int get_rate_delta_gain_5g_eco(s8 *dst)
{
	int i=0,count = 0,readnum=0,val = 0;
	char *file = NULL;
	char readbuf[512] = {0};
	char *pdata[24] = {NULL};
	char *pstr = NULL;
#ifdef CONFIG_RATE_TXPOWER_5G_ECO
	file = CONFIG_RATE_TXPOWER_5G_ECO;
#else
	file = "/tmp/set_rate_power_5g_eco.txt";
#endif
	if(dst == NULL)
	{
		atbm_printk_err("Invalid parameter,dst == NULL\n");
...
#if defined (CONFIG_ATBM_APOLLO_5GHZ_SUPPORT) || defined (CONFIG_ATBM_5G_PRETEND_2G)
	if(hw_priv->chip_version != Mercurius){	
		atbm_printk_init("chip is  not Mercurius ,not support  5G!5G_PRETEND_2G \n");
		hw_priv->hw->wiphy->bands[IEEE80211_BAND_5GHZ]->channels = atbm_5ghz_pretend_2g_chantable;
		hw_priv->hw->wiphy->bands[IEEE80211_BAND_5GHZ]->n_channels = ARRAY_SIZE(atbm_5ghz_pretend_2g_chantable);
	}
#endif /* CONFIG_ATBM_APOLLO_5GHZ_SUPPORT */
...
#endif
#ifdef ATBM_EVIZE_CUSTOMER_FUNC
	atbm_evize_thread_init(hw_priv);

#endif
#ifdef CONFIG_SUPPORT_MDNS_OFFLOAD
	mdns_offload_chr_init(hw_priv);
#endif
