...
int atbm_save_efuse(struct atbm_common *hw_priv,struct efuse_headr *efuse_save)
{
	...
	int iResult=0;
	//struct atbm_vif *vif;
	struct efuse_headr efuse_bak  = {0};
	int efuseLen = 0;
	...
	*/
	...
	if (iResult == 0)
	{
		...
		wsm_get_efuse_data(hw_priv,(void *)&efuse_bak, efuseLen);
		...
		{
			//frame_hexdump("efuse_write", (u8 *)&efuse_save, efuseLen);
			//frame_hexdump("efuse_read", (u8 *)&efuse_bak, efuseLen);
