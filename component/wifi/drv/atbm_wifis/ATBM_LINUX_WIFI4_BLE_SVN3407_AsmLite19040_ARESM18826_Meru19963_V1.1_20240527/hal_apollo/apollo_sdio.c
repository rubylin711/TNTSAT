...
enum atbm_bus_pm_state
{
	ATBM_BUS_PM_STATE_DOZE,
};
struct atbm_bus_pm
{
	unsigned long state;
...
{
#ifdef CONFIG_SUPPORT_SDIO_PS
	struct atbm_bus_pm  *pm = &self->pm;
	int ret = 0;
	u16 ctrl_reg;
	int retries = 0;
	
	if(!test_bit(ATBM_BUS_PM_STATE_DOZE,&pm->state)){
		goto active;
	}
	
	ret = atbm_reg_write_16(self->core, ATBM_HIFREG_CONTROL_REG_ID,
		ATBM_HIFREG_CONT_WUP_BIT);
	if (WARN_ON(ret)){
		goto exit;
	}
	do {
		ret = atbm_bh_read_ctrl_reg(self->core, &ctrl_reg);
		if (WARN_ON(ret)){
			goto exit;
		}
		...
		if((retries % 1000)  == 0){
			...
			atbm_printk_err("[BH] Device awake Err[%d][%x]\n",retries,ctrl_reg);
		}
	}while(1);
	atbm_printk_debug("__atbm_sdio_pm_active(%x)\n",ctrl_reg);
	clear_bit(ATBM_BUS_PM_STATE_DOZE,&pm->state);
	goto active;
...
static int __atbm_sdio_pm_doze(struct sbus_priv *self)
{
	...
	ret = atbm_reg_write_16(self->core, ATBM_HIFREG_CONTROL_REG_ID,0);
	if (WARN_ON(ret)){
		goto exit;
	}
	
	set_bit(ATBM_BUS_PM_STATE_DOZE,&pm->state);
exit:
	return ret;
#else
	return 0;
...
static void atbm_sdio_pm_init(struct sbus_priv *self)
{
	self->pm.count = 0;
	self->pm.state = 0;
	mutex_init(&self->pm.mutex);
}

static void atbm_sdio_pm_exit(struct sbus_priv *self)
{
	mutex_destroy(&self->pm.mutex);
}
