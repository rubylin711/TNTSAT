...
static int wsm_set_mdns_offload_confirm(struct atbm_common *hw_priv,
			     struct wsm_buf *buf)
{
	u32 status = WSM_GET32(buf);
	...
	/*Hit & Miss count*/
	hw_priv->tmp_count = WSM_GET32(buf);
	return 0;
...
extern int get_rate_delta_gain_5g_eco(s8 *dst);
int wsm_set_rate_power_5g(struct atbm_common *hw_priv,int use_flag)
{
	s8 rate_txpower[23] = {0};//validfalg,data
	int i,err;
	int (*func)(s8 *dst);
	func = get_rate_delta_gain_5g;
	if((hw_priv->chip_version == Mercurius) && (hw_priv->efuse.version == 17)){
		if(atbm_Mercurius_ECO_get() == 1){
			atbm_printk_err("eco chip!!");
			func = get_rate_delta_gain_5g_eco;
		}
	}	
	
	if(func(&rate_txpower[1]) ==  0){
		
		rate_txpower[0] = use_flag;
		
		err = wsm_write_mib(hw_priv, WSM_MIB_ID_SET_RATE_TX_POWER_5G, rate_txpower, sizeof(rate_txpower), 0);
		if(err < 0){
			atbm_printk_err("write mib failed(%d). \n", err);
		}
...
atbm_caps_display caps_display[]={
	...
	{"CAPABILITIES_SDIO_PATCH",CAPABILITIES_SDIO_PATCH},
	{"CAPABILITIES_NO_BACKOFF",CAPABILITIES_NO_BACKOFF},
	{"CAPABILITIES_CFO",CAPABILITIES_CFO},
	{"CAPABILITIES_AGC",CAPABILITIES_AGC},
	{"CAPABILITIES_TXCAL",CAPABILITIES_TXCAL},
	{"CAPABILITIES_MONITOR",CAPABILITIES_MONITOR},
	{"CAPABILITIES_CUSTOM",CAPABILITIES_CUSTOM},
	{"CAPABILITIES_SMARTCONFIG",CAPABILITIES_SMARTCONFIG},
	{"CAPABILITIES_ETF",CAPABILITIES_ETF},
	{"CAPABILITIES_LMAC_RATECTL",CAPABILITIES_LMAC_RATECTL},
	{"CAPABILITIES_LMAC_TPC",CAPABILITIES_LMAC_TPC},
	{"CAPABILITIES_LMAC_TEMPC",CAPABILITIES_LMAC_TEMPC},
	{"CAPABILITIES_CTS_BUG",CAPABILITIES_CTS_BUG},
	{"CAPABILITIES_USE_IPC",CAPABILITIES_USE_IPC},
	{"CAPABILITIES_OUTER_PA",CAPABILITIES_OUTER_PA},
	{"CAPABILITIES_POWER_CONSUMPTION",CAPABILITIES_POWER_CONSUMPTION},
	{"CAPABILITIES_RSSI_DECIDE_TXPOWER",CAPABILITIES_RSSI_DECIDE_TXPOWER},
	...
	{"CAPABILITIES_NOISE_SET_DCXO",CAPABILITIES_NOISE_SET_DCXO},
	{"CAPABILITIES_CFO_DCXO_CORRECTION",CAPABILITIES_CFO_DCXO_CORRECTION},
	{"CAPABILITIES_HW_CHECKSUM",CAPABILITIES_HW_CHECKSUM},
	{"CAPABILITIES_SINGLE_CHANNEL_MULTI_RX",CAPABILITIES_SINGLE_CHANNEL_MULTI_RX},
	{NULL,0},
};
atbm_caps_display extcaps_display[]={
	{"EX_CAPABILITIES_TWO_CHIP_ONE_SOC",EX_CAPABILITIES_TWO_CHIP_ONE_SOC},
	{"EX_CAPABILITIES_MANUAL_SET_AC",EX_CAPABILITIES_MANUAL_SET_AC},
	{"EX_CAPABILITIES_LMAC_BW_CONTROL",EX_CAPABILITIES_LMAC_BW_CONTROL},
	{"EX_CAPABILITIES_SUPPORT_TWO_ANTENNA",EX_CAPABILITIES_SUPPORT_TWO_ANTENNA},
	{"EX_CAPABILITIES_ENABLE_STA_REMAIN_ON_CHANNEL",EX_CAPABILITIES_ENABLE_STA_REMAIN_ON_CHANNEL},
	{"EX_CAPABILITIES_ENABLE_PS",EX_CAPABILITIES_ENABLE_PS},
	{"EX_CAPABILITIES_TX_REQUEST_FIFO_LINK",EX_CAPABILITIES_TX_REQUEST_FIFO_LINK},
	{"EX_CAPABILITIES_DRIVER_PHY_REG_INIT",EX_CAPABILITIES_DRIVER_PHY_REG_INIT},
	{"EX_CAPABILITIES_DRIVER_PROCESS_BA",EX_CAPABILITIES_DRIVER_PROCESS_BA},
	{"EX_CAPABILITIES_DRIVER_PHY_REG_INIT",EX_CAPABILITIES_DRIVER_PHY_REG_INIT},
	{NULL,0},
};
...
*/
	...
	caps_ptr = extcaps_display;
	while(caps_ptr->name != NULL){
		atbm_printk_init("%s  [%d]\n" ,caps_ptr->name,!!(hw_priv->wsm_caps.firmeareExCap & caps_ptr->wsm_cap_flag)); 
		caps_ptr++;
	}
...
#ifdef CONFIG_SUPPORT_MDNS_OFFLOAD
		case WSM_SET_MDNS_OFFLOAD_RESP:
			{
				wsm_set_mdns_offload_confirm(hw_priv, &wsm_buf);
				break;
			}
#endif
		...
		int if_id)
{
	...
	{
		WSM_PUT8(buf, arg->delta_gain1_5g);
		WSM_PUT8(buf, arg->delta_gain2_5g);
		WSM_PUT8(buf, arg->delta_gain3_5g);
		WSM_PUT8(buf, arg->delta_gain4_5g);
		WSM_PUT8(buf, arg->delta_gain5_5g);
		WSM_PUT8(buf, arg->delta_gain6_5g);
		WSM_PUT8(buf, arg->delta_gain7_5g);
		WSM_PUT8(buf, arg->delta_gain8_5g);
		WSM_PUT8(buf, arg->delta_gain9_5g);
		WSM_PUT8(buf, arg->delta_gain10_5g);
	}
...
#ifdef CONFIG_SUPPORT_MDNS_OFFLOAD
int wsm_set_mdns_offload(struct atbm_common *hw_priv, u8 * data, int len, int if_id)
{
	int ret;
        struct wsm_buf *buf = &hw_priv->wsm_cmd_buf;

	wsm_cmd_lock(hw_priv);
	WSM_PUT(buf, data, len);
	ret = wsm_cmd_send(hw_priv, buf, NULL, WSM_SET_MDNS_OFFLOAD, WSM_CMD_TIMEOUT, if_id);
