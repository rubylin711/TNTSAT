...
/*			MAP_CHANNEL_ID_TO_KHZ(ch, m); */
		...
		/*================================ */
	{
		...
		memcpy(custom, pIoctlScan->MainSharedKey[0], 16);
...
/*	MLME_DEAUTH_REQ_STRUCT      DeAuthReq; */
	...
	if (copy_from_user(&mlme,wrqu->data.pointer,sizeof(struct iw_mlme))) {
		return -EFAULT;
	}
	pMlme = &mlme;
...
/*    if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	...
	if (copy_from_user(&pmksa,wrqu->data.pointer,sizeof(struct iw_pmksa))) {
			pr_err("rt_ioctl_siwpmksa:copy from user space failed\n");
			return -EFAULT;
	}
