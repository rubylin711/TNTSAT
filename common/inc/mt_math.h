...
#define MT_SYS_GET_STRIDE(w)   ((((w)%256)==0)?(w):(((((w)/256)%2)==0)?((((w)/256)+1)*256):((((w)/256)+2)*256)))

/******************************************************************************
** Get the span between two unsinged number, such as
** SPAN(mt_u32, 100, 200) is 200 - 100 = 100
** SPAN(mt_u32, 200, 100) is 0xFFFFFFFF - 200 + 100
** SPAN(mt_u64, 200, 100) is 0xFFFFFFFFFFFFFFFF - 200 + 100
******************************************************************************/
...
__inline static MT_BOOL IS_LITTLE_END(void)
{
    union unEND_TEST_U
    {
        mt_char cTest[4];
    ...
    return (unEndTest.u32Test > 0x01020304) ? (MT_TRUE) : (MT_FALSE);
}
...
******************************************************************************/
__inline static mt_void Rgb2Yc(mt_u8 r, mt_u8 g, mt_u8 b, mt_u8 * py, mt_u8 * pcb, mt_u8 * pcr)
{
    /* Y */
    *py = (mt_u8)(((r*66+g*129+b*25) >> 8) + 16);

    /* Cb */
    *pcb = (mt_u8)((((b*112-r*38)-g*74) >> 8) + 128);

    /* Cr */
    *pcr = (mt_u8)((((r*112-g*94)-b*18) >> 8) + 128);
}

__inline static mt_u32 Rgb2Yuv(mt_u32 u32Rgb)
{
    mt_u8 y,u,v;

    Rgb2Yc((mt_u8)RGB_R(u32Rgb), (mt_u8)RGB_G(u32Rgb), (mt_u8)RGB_B(u32Rgb), &y, &u, &v);

    return (mt_u32)YUV(y,u,v);
}

/******************************************************************************
** GetYCFromRGB(rgb, *y, *cbcr)    convert rgb to yyyy, uvuv,
******************************************************************************/
__inline static mt_void GetYCFromRGB(mt_u32 rgb, mt_u32 * pY, mt_u32 * pC)
{
    mt_u8 y, cb, cr;
    mt_u32 color_y, color_c, tmp;

    Rgb2Yc((mt_u8)RGB_R(rgb), (mt_u8)RGB_G(rgb), (mt_u8)RGB_B(rgb), &y, &cb, &cr);
...
{
    mt_u32 u32Ffps;    /* Full frame rate    */
    mt_u32 u32Tfps;    /* Target frame rate  */
    mt_u32 u32FrmKey;  /* update key frame   */
} FPS_CTRL_S;

__inline static mt_void InitFps(FPS_CTRL_S *pFrmCtrl, mt_u32 u32FullFps,
                                mt_u32 u32TagFps)
{
...
__inline static MT_BOOL FpsControl(FPS_CTRL_S *pFrmCtrl)
{
    MT_BOOL   bReturn       = MT_FALSE;
...
*******************************************************************************/
    ...
    #include <sys/time.h>
#endif
__inline static mt_u32 GetSysTimeBySec(void)
{
    ...
    #endif
    return (mt_u32)stTime.tv_sec;
}

__inline static mt_u64 GetSysTimeByUsec(void)
{
    struct timeval stTime;
    #ifdef __KERNEL__
        do_gettimeofday(&stTime);
        ...
        gettimeofday(&stTime, NULL);
    #endif
    return  (mt_u64)(((unsigned long long)stTime.tv_sec * 1000000LLU) + (mt_u64)stTime.tv_usec);
}
