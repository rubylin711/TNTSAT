...
#define CCDISP_MUTEX_LOCK(x)  \
    do{\
        ...
        if(lock_ret != 0){\
            MT_ERR_CC("CC call pthread_mutex_lock(DISP) failure,lock_ret = 0x%x\n",lock_ret);\
        }\
    }while(0)
...
#define CCDISP_MUTEX_UNLOCK(x)  \
    do{\
        ...
        if(unlock_ret != 0){\
            MT_ERR_CC("CC call pthread_mutex_unlock(DISP) failure, unlock_ret = 0x%x\n",unlock_ret);\
        }\
    }while(0)
...
static MT_U32 s_u32Opacity[5]={0x00000000,0xff000000,0xff000000,0x7f000000,0x00000000};

static CCDisp_Screen_S s_astCCDispScreen[1];
static MT_UNF_CC_RECT_S s_astWindowRect[8];
...
{
    MT_U32 u32WinID = (MT_U32)enWinID;
...
static MT_S32 CCDISP_CheckRect(MT_UNF_CC_RECT_S stMasterRect, MT_UNF_CC_RECT_S stSlaveRect)
{
...
*******************************************************************************/
MT_S32 _DtvCC_CharFlash(CCDISP_FlashText_S * text,MT_U32 flag);
...
MT_S32 CCDISP_Screen_SetSize(MT_UNF_CC_RECT_S stScreenRect)
{
    ...
    {
        MT_ERR_CC("stScreenRect(%d %d %d %d) is invalid !\n", stScreenRect.x, stScreenRect.y,
                  stScreenRect.width, stScreenRect.height);
        return MT_FAILURE;
    }

    pstScreen->stScreenRect = stScreenRect;
    pstScreen->u32ScreenColor = MT_UNF_CC_COLOR_BLACK;
    pstScreen->enScreenOpacity = MT_UNF_CC_OPACITY_TRANSPARENT;
...
MT_S32 CCDISP_Screen_SetClipArea(MT_UNF_CC_RECT_S stClipArea)
{
    ...
    if (MT_FAILURE == CCDISP_CheckRect(pstScreen->stScreenRect, stClipArea))
    {
        MT_ERR_CC("stClipArea(%d %d %d %d) is invalid !\n", stClipArea.x, stClipArea.y,
...
{
    CCDisp_Screen_S *pstScreen = &s_astCCDispScreen[0];
    MT_U32 u32RGB = pstScreen->u32ScreenColor;
    MT_UNF_CC_OPACITY_E enOpacity = pstScreen->enScreenOpacity;
    MT_U32 u32Color = 0;
    MT_UNF_CC_DISPLAY_PARAM_S stDispParam;

    DISP_TRACE("color:0x%x,opa:%s\n",u32RGB,DBG_OPA_OUT[enOpacity]);

    stDispParam.enOpt = MT_UNF_CC_OPT_FILLRECT;
    ...
    if (pstScreen->stOpt.pfnDisplay)
    {
        ...
        (MT_VOID)pstScreen->stOpt.pfnDisplay((ulong)pstScreen->stOpt.pUserData, &stDispParam);
...
MT_S32 CCDISP_Window_Create(CCDISP_WINDOW_ID_E enWinID, MT_UNF_CC_RECT_S stWinRect)
{
    ...
    if(MT_NULL == pstWindow )
    {
        MT_ERR_CC("enWinID : %d is invalid !\n", enWinID);
        return MT_FAILURE;
    }

    if (MT_FAILURE == CCDISP_CheckRect(pstScreen->stClipArea, stWinRect))
    {
        MT_ERR_CC("stWinRect(%d %d %d %d) is invalid ! screen(%d %d %d %d)\n", stWinRect.x, stWinRect.y,
    ...
    pstWindow->bVisible           = MT_TRUE;
    pstWindow->stWinRect          = stWinRect;
    pstWindow->u32WinColor         = MT_UNF_CC_COLOR_BLACK;
    pstWindow->enWinOpacity       = MT_UNF_CC_OPACITY_TRANSPARENT;
    pstWindow->stText.u32FgColor   = MT_UNF_CC_COLOR_WHITE;
    pstWindow->stText.enFgOpacity = MT_UNF_CC_OPACITY_SOLID;
    pstWindow->stText.u32BgColor   = MT_UNF_CC_COLOR_BLACK;
    pstWindow->stText.enBgOpacity = MT_UNF_CC_OPACITY_SOLID;
    pstWindow->stText.enFontName  = MT_UNF_CC_FN_DEFAULT;
    pstWindow->stText.enFontStyle = MT_UNF_CC_FONTSTYLE_NORMAL;
    pstWindow->stText.enFontSize  = MT_UNF_CC_FONTSIZE_STANDARD;
...
MT_S32 CCDISP_Window_SetSize(CCDISP_WINDOW_ID_E enWinID, MT_UNF_CC_RECT_S stWinRect)
{
    ...
    if (MT_FAILURE == CCDISP_CheckRect(pstScreen->stClipArea, stWinRect))
    {
        MT_ERR_CC("stWinRect(%d %d %d %d) is invalid !\n", stWinRect.x, stWinRect.y,
...
MT_S32 CCDISP_Window_GetColor(CCDISP_WINDOW_ID_E enWinID, MT_U32*pu32WinColor, MT_UNF_CC_OPACITY_E *penWinOpacity)
{
    ...
    if (MT_NULL != pu32WinColor)
    {
        *pu32WinColor = pstWindow->u32WinColor;
    }

    if (MT_NULL != penWinOpacity)
    {
...
MT_S32 CCDISP_Window_Clear(CCDISP_WINDOW_ID_E enWinID)
{
    CCDisp_Screen_S *pstScreen = &s_astCCDispScreen[0];
    CCDisp_Window_S *pstWindow = CCDISP_GetWindowByID(enWinID);
    MT_U32 u32RGB = MT_UNF_CC_COLOR_DEFAULT;
    ...
    if( MT_FALSE == pstWindow->bVisible )
    {
        return MT_SUCCESS;
    }
...
* Return value:  0 success, other value that indicate the error when failed.                    *
*************************************************************************************************/
MT_S32 CCDISP_Window_FillRect(CCDISP_WINDOW_ID_E enWinID, MT_U32 u32RGB,
                              MT_UNF_CC_OPACITY_E enOpacity, MT_UNF_CC_RECT_S stRect,MT_U32 flag)
{
    MT_U32 u32Color = 0;
    MT_UNF_CC_COLOR_S stColor;
    MT_S32 s32Ret = MT_SUCCESS;
    MT_UNF_CC_RECT_S stFillRect = stRect ;
...
MT_S32 CCDISP_Window_FillRectByRGB(CCDISP_WINDOW_ID_E enWinID, MT_UNF_CC_COLOR_S stColor, MT_UNF_CC_RECT_S stRect,MT_U32 flag)
{
    ...
    */
    ...
    if (MT_FAILURE == CCDISP_CheckRect(stWinRectTmp, stRect))
    {
        MT_ERR_CC("window rect is (%d %d %d %d)\n", stWinRectTmp.x, stWinRectTmp.y, stWinRectTmp.width, stWinRectTmp.height);
        MT_ERR_CC("stRect(%d %d %d %d) is invalid !\n", stRect.x, stRect.y, stRect.width, stRect.height);
    }
                               ...
                               MT_S32 s32Width, MT_S32 s32Height, MT_S32 s32SrcX, MT_S32 s32SrcY)
{
    CCDisp_Screen_S *pstScreen = &s_astCCDispScreen[0];
    CCDisp_Window_S *pstWindow = CCDISP_GetWindowByID(enWinID);
    MT_UNF_CC_RECT_S stSrcRect, stDstRect;
    ...
    {
        (MT_VOID)pstScreen->stOpt.pfnBlit((ulong)pstScreen->stOpt.pUserData, &stSrcRect, &stDstRect);
    }
                            ...
                            MT_U16 u16StartY, MT_U16 u16EndY, MT_U16 u16Offset, CCDISP_ROLLSPEED_E enSpeed)
{
    ...
    MT_S32 s32RollupStep = 0,s32RollupOffset = 0,s32ScrollStep = 0,s32ScrollHeight= 0;
    MT_U16 u16DstY = 0,u16SrcY= 0;
    MT_UNF_CC_RECT_S stSrcRect, stDstRect;
    MT_U32 u32Color = 0;
    MT_UNF_CC_DISPLAY_PARAM_S stDispParam;
    ...
    while(s32RollupOffset > 0)
    {
        ...
        if(pstScreen->stOpt.pfnBlit)
        {
            s32Ret = pstScreen->stOpt.pfnBlit((ulong)pstScreen->stOpt.pUserData, &stSrcRect, &stDstRect);
        }
        ...
        if(pstScreen->stOpt.pfnDisplay)
        {
            ...
            CCDISP_MUTEX_LOCK(&drawMutex);
            s32Ret |= pstScreen->stOpt.pfnDisplay((ulong)pstScreen->stOpt.pUserData, &stDispParam);
            CCDISP_MUTEX_UNLOCK(&drawMutex);
        }
        //u16SrcY = u16DstY;
    }
...
*************************************************************************************************/
MT_S32 CCDISP_Text_GetSize(CCDISP_WINDOW_ID_E enWinID, MT_U16 *pu16Str, MT_S32 s32Length,
                           MT_S32 *ps32Width, MT_S32 *ps32Height)
{
    MT_S32 s32Ret = MT_SUCCESS;
    CCDisp_Screen_S *pstScreen = &s_astCCDispScreen[0];
    CCDisp_Window_S *pstWindow = CCDISP_GetWindowByID(enWinID);
    ...
    {
        s32Ret = pstScreen->stOpt.pfnGetTextSize((ulong)pstScreen->stOpt.pUserData, pu16Str, s32Length, &s32Width, &s32Height);
    }
    if (MT_NULL != ps32Width)
    {
        *ps32Width = s32Width;
    }
    if (MT_NULL != ps32Height)
    {
                        ...
                        MT_U16* pu16Str, MT_S32 s32StrLen, MT_S32* ps32DrawWidth, MT_U32 flag)
{
    ...
    MT_U32 u32FgColor = 0, u32BgColor = 0,u32EdgeColor=0;
    MT_UNF_CC_DISPLAY_PARAM_S stDispParam;
    ...
    if ((MT_NULL == pu16Str) || (s32StrLen <= 0))
    {
        MT_ERR_CC("parameter is error\n");
        return MT_FAILURE;
    }
    ...
    if(pstScreen->stOpt.pfnGetTextSize && pstScreen->stOpt.pfnDisplay)
    {
        ...
        do
        {
            s32Ret = pstScreen->stOpt.pfnGetTextSize((ulong)pstScreen->stOpt.pUserData, pu16Str, s32StrLen, &s32Width, &s32Height);
            if (MT_FAILURE == s32Ret)
            {
                MT_ERR_CC("failed to get text size !\n");
                return MT_FAILURE;
            }
            ...
            if (2 == flag)
            {
                ...
                if (MT_UNF_CC_FONTSIZE_SMALL == pstWindow->stText.enFontSize )
                {
                    scale = 1.0 * MAX_CC_COLUMNS_4B3 / MAX_CC_COLUMNS_16B9;
                }
                else if (MT_UNF_CC_FONTSIZE_LARGE == pstWindow->stText.enFontSize )
                {
            ...
            if (MT_FAILURE == s32Ret)
            {
                MT_WARN_CC("text(%d %d %d %d) is over range (%d %d %d %d) !\n", stRect.x, stRect.y, stRect.width, stRect.height,
                          pstWindow->stWinRect.x,pstWindow->stWinRect.y,pstWindow->stWinRect.width,pstWindow->stWinRect.height);

                s32StrLen--;
            }

        }while (MT_FAILURE == s32Ret && s32StrLen);

        if (s32StrLen <= 0)
        {
            MT_WARN_CC("text length is less than or equal to 0!\n");
            return MT_FAILURE;
        }

        stDispParam.enOpt = MT_UNF_CC_OPT_DRAWTEXT;
        stDispParam.stRect = stRect;
        stDispParam.u32DisplayWidth = pstScreen->stScreenRect.width;
        stDispParam.u32DisplayHeight = pstScreen->stScreenRect.height;
        ...
        if (3 == flag)
        {
            if (MT_UNF_CC_FONTSIZE_LARGE == pstWindow->stText.enFontSize )
            {
                stDispParam.unDispParam.stText.enFontSize = MT_UNF_CC_FONTSIZE_STANDARD;
            }
        }
        stDispParam.unDispParam.stText.enEdgetype = pstWindow->stText.enEdgeType;

        //record flash char,just support 708
        if (((2 == flag) && (pstWindow->stText.enFgOpacity == MT_UNF_CC_OPACITY_FLASH || pstWindow->stText.enBgOpacity == MT_UNF_CC_OPACITY_FLASH))
           || ((flag ==1) && (pstWindow->stText.enFgOpacity == MT_UNF_CC_OPACITY_FLASH)))
        {
            CCDISP_FlashText_S* temp = MT_NULL;
            CCDISP_FlashText_S* text = (CCDISP_FlashText_S*)MT_MEMCC_MALLOC(sizeof(CCDISP_FlashText_S));
            MT_U16 * pu16buf = (MT_U16*)MT_MEMCC_MALLOC(2*s32StrLen);
            if (MT_NULL == pu16buf)
            {
                MT_ERR_CC("malloc pu16buf failed\n");
                if (MT_NULL != text)
                {
                    MT_MEMCC_FREE(text);
                    text = MT_NULL;
            ...
            else
            {
                if (MT_NULL == text)
                {
                    MT_MEMCC_FREE(pu16buf);
                    pu16buf = MT_NULL;
                    MT_ERR_CC("malloc text failed\n");
                }
                else
                {
                    ...
                    if (pstWindow->stText.enFgOpacity == MT_UNF_CC_OPACITY_FLASH)
                    {
                        text->bFgFlash = MT_TRUE;
                    }
                    else
                    {
                        text->bFgFlash = MT_FALSE;
                    }
                    text->next = MT_NULL;
        ...
        s32Ret = pstScreen->stOpt.pfnDisplay((ulong)pstScreen->stOpt.pUserData, &stDispParam);
        CCDISP_MUTEX_UNLOCK(&drawMutex);
        if(MT_SUCCESS == s32Ret && ps32DrawWidth != MT_NULL)
        {
...
MT_S32 CCDISP_Text_DeleteCharFlash(CCDISP_WINDOW_ID_E enWinID)
{
    ...
    CCDISP_FlashText_S * flashText_B = MT_NULL;
    s_astCCDispFlashText[enWinID]  = MT_NULL;
    while(flashText_A != MT_NULL)
    {
        flashText_B = flashText_A;
        flashText_A = flashText_B->next;
        MT_MEMCC_FREE(flashText_B->pu16Text);
        MT_MEMCC_FREE(flashText_B);
        flashText_B = MT_NULL;
    }
    CCDISP_MUTEX_UNLOCK(&flashMutex);
    return MT_SUCCESS;
...
MT_S32 _DtvCC_CharFlash(CCDISP_FlashText_S * text,MT_U32 flag)
{
    ...
    if( (flag == 0) && (text->bFgFlash))
    {
         ...
         stDispParam.enOpt = MT_UNF_CC_OPT_FILLRECT;
         CCDISP_MUTEX_LOCK(&drawMutex);
         pstScreen->stOpt.pfnDisplay((ulong)pstScreen->stOpt.pUserData, &stDispParam);
         CCDISP_MUTEX_UNLOCK(&drawMutex);
         return MT_SUCCESS;
    }
    else
...
{
    MT_U32 winCount;
    static MT_U32 count=0;
    CCDISP_MUTEX_LOCK(&flashMutex);
    for( winCount = 0 ; winCount < 8 ; winCount++ )
    {
        if(MT_NULL==s_astCCDispFlashText[winCount])
        {
             continue;
        }
        else
...
MT_S32 CCDISP_Text_SetUnderLine(CCDISP_WINDOW_ID_E enWinID, MT_BOOL bUnderline)
{
    ...
    if (bUnderline)
    {
        if ((MT_UNF_CC_FONTSTYLE_ITALIC == pstWindow->stText.enFontStyle)||(MT_UNF_CC_FONTSTYLE_ITALIC_UNDERLINE == pstWindow->stText.enFontStyle))
        {
            pstWindow->stText.enFontStyle = MT_UNF_CC_FONTSTYLE_ITALIC_UNDERLINE;
        }
        else
        {
            pstWindow->stText.enFontStyle = MT_UNF_CC_FONTSTYLE_UNDERLINE;
        }
    }
    else
    {
        if ((MT_UNF_CC_FONTSTYLE_ITALIC == pstWindow->stText.enFontStyle)||(MT_UNF_CC_FONTSTYLE_ITALIC_UNDERLINE == pstWindow->stText.enFontStyle))
        {
            pstWindow->stText.enFontStyle = MT_UNF_CC_FONTSTYLE_ITALIC;
        }
        else
        {
            pstWindow->stText.enFontStyle = MT_UNF_CC_FONTSTYLE_NORMAL;
...
MT_S32 CCDISP_Text_SetItalic(CCDISP_WINDOW_ID_E enWinID, MT_BOOL bItalic)
{
    ...
    if (bItalic)
    {
        if ((MT_UNF_CC_FONTSTYLE_UNDERLINE == pstWindow->stText.enFontStyle)||(MT_UNF_CC_FONTSTYLE_ITALIC_UNDERLINE == pstWindow->stText.enFontStyle))
        {
            pstWindow->stText.enFontStyle = MT_UNF_CC_FONTSTYLE_ITALIC_UNDERLINE;
        }
        else
...
MT_S32 CCDISP_Text_SetFlash(CCDISP_WINDOW_ID_E enWinID, MT_BOOL bFlash)
{
    ...
    if (MT_TRUE == bFlash)
    {
        pstWindow->stText.enFgOpacity = MT_UNF_CC_OPACITY_FLASH;
    }
...
MT_S32 CCDISP_Text_GetFGColor(CCDISP_WINDOW_ID_E enWinID, MT_U32 *pu32FgColor, MT_UNF_CC_OPACITY_E *penFgOpacity)
{
    ...
    if (MT_NULL != pu32FgColor)
    {
        *pu32FgColor = pstWindow->stText.u32FgColor;
    }
    if (MT_NULL != penFgOpacity)
    {
...
MT_S32 CCDISP_Text_GetBGColor(CCDISP_WINDOW_ID_E enWinID, MT_U32 *pu32BgColor, MT_UNF_CC_OPACITY_E *penBgOpacity)
{
    ...
    if (MT_NULL != pu32BgColor)
    {
        *pu32BgColor = pstWindow->stText.u32BgColor;
    }
    if (MT_NULL != penBgOpacity)
    {
...
{
    CCDisp_Screen_S *pstScreen = &s_astCCDispScreen[0];
    MT_UNF_CC_DISPLAY_PARAM_S stCCdispalyParm;

    if (MT_NULL == pstBitmap)
    {
        MT_ERR_CC("bitmap data is error  !\n");
        return MT_FAILURE;
    }

    if (pstScreen->stOpt.pfnDisplay)
    {
        stCCdispalyParm.enOpt = MT_UNF_CC_OPT_DRAWBITMAP;
        ...
        (MT_VOID)pstScreen->stOpt.pfnDisplay((ulong)pstScreen->stOpt.pUserData, &stCCdispalyParm);
    }
...
MT_S32 CCDISP_SendVbiData(MT_UNF_CC_VBI_DADA_S *pstVbiDataField1,MT_UNF_CC_VBI_DADA_S *pstVbiDataField2)
{
    ...
    if (MT_NULL == pstVbiDataField1 ||MT_NULL == pstVbiDataField2 )
    {
        MT_ERR_CC("vbi data is error  !\n");
        return MT_FAILURE;
    }

    if (pstScreen->stOpt.pfnVBIOutput)
    {
        //(MT_VOID)pstScreen->stOpt.pfnVBIOutput(pstScreen->stOpt.pUserData, pstVbiDataField1,pstVbiDataField2); // 708
        (MT_VOID)pstScreen->stOpt.pfnVBIOutput((ulong)pstScreen->stOpt.pUserData, pstVbiDataField1);
        (MT_VOID)pstScreen->stOpt.pfnVBIOutput((ulong)pstScreen->stOpt.pUserData, pstVbiDataField2);
...
MT_S32 CCDISP_GetCurPts(S64 *ps64CurPts)
{
    ...
    {
       (MT_VOID)pstScreen->stOpt.pfnGetPts((ulong)pstScreen->stOpt.pUserData, ps64CurPts);
    }
    return MT_SUCCESS;
}
...
MT_S32 CCDISP_OutputXDSData(MT_U8 u8XDSClass, MT_U8 u8XDSPacketType, MT_U8 *pu8Data, MT_U8 u8Length)
{
    ...
    if (MT_NULL == pu8Data || 0 == u8Length)
    {
        MT_ERR_CC("XDS data is error  !\n");
        return MT_FAILURE;
    }

    if (pstScreen->stOpt.pfnXDSOutput)
    {
        (MT_VOID)pstScreen->stOpt.pfnXDSOutput(u8XDSClass,u8XDSPacketType,pu8Data,u8Length);
    }
