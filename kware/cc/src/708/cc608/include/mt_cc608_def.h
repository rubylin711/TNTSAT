...
#define EnterCriticalSection(x)  \
    do{ \
        ...
        if(ret != 0){ \
            MT_ERR_CC("CC call pthread_mutex_lock failure,ret = 0x%x\n",ret); \
        } \
    }while(0)
...
#define LeaveCriticalSection(x)  \
    do{ \
        ...
        if(ret != 0){ \
            MT_ERR_CC("CC call pthread_mutex_unlock failure,ret = 0x%x\n",ret); \
        } \
    }while(0)
...
#define InitializeCriticalSection(x)  \
    do{ \
        ...
        if(ret != 0){ \
            MT_ERR_CC("CC call pthread_mutex_init failure,ret = 0x%x\n",ret); \
        } \
    }while(0)
...
#define DeleteCriticalSection(x)  \
    do{ \
        ...
        if(ret != 0){ \
            MT_ERR_CC("CC call pthread_mutex_destroy failure,ret = 0x%x\n",ret); \
        } \
    }while(0)
...
typedef struct tagCC608_PENATTR_S
{
    ...
    MT_BOOL         bIsItalic;  /*is cc italic*/
    MT_BOOL         bIsUnderline;   /*is cc underline*/
    MT_BOOL         bIsFlash;       /*is cc flash on*/
    void *          reserved;       /*reserved for extend use*/
} CC608_PENATTR_S;

typedef struct tagCC608_CAPTION_S
{
    MT_U16      u16Text;            /*unicode*/
    MT_U8       u8IsChar;         /*is filled with a caption?*/
    MT_U8       u8IsDisplay;      /*is the caption displayed*/
    MT_U16      u16PacCode;        /*pac code of the screen*/
} CC608_CAPTION_S;

typedef struct tagCC608_BUFFER_S
{
    CC608_CAPTION_S   stCaption[CC608ROWS][CC608ROWCHARS]; /*unicode*/
    MT_U8             u8CharNum[CC608ROWS];                /*The char numbers of the row*/
    MT_U32            u32SeqNum[CC608ROWS];                /*seq num indicate the draw sequence*/
    MT_U32            u32CurSeq;                           /*current sequence number*/
    MT_U8             u8PacCode[CC608ROWS];                /*PAC CODE of every row*/
    MT_U8             u8IsPacOverwrite;                    /*is there any two pacs of the same row*/
    CC608_PENATTR_S   stLastPenAttr[CC608ROWS];            /*The last character's attributes of this row*/
    MT_U8             u8CurRow;                            /*current row number*/
    MT_U8             u8CurColumn;                         /*current column number*/
} CC608_BUFFER_S;

typedef struct tagCC608_SCREEN_S
{
    MT_UNF_CC_RECT_S  stPhyScreen;   /*panel or crt size*/
    MT_UNF_CC_RECT_S  stStaScreen;   /*safe title area*/
    MT_U8       u8RowHeight;         /*row height The value = sta height/CC608ROWS*/
    MT_U8       u8CharWidth;         /*The width of each character it's value = sta width/CC608COLUMNS*/
    MT_U16      u16CurTextWidth;      /*current cursor of 608 caption. The next text will be drawn at this positoin*/
    MT_U8       u8CurRow;            /*current row of caption*/
    MT_U8       u8CurColumn;         /*current column of caption*/

    MT_U8       u8BaseRow;           /*the base row of rollup type*/
    MT_U8       u8TopRow;            /*the top row of rollup type*/
} CC608_SCREEN_S;
...
typedef struct tagCC608_OBJECT_S
{
    ...
    MT_U8               u8IsModeModified; /*is mode modified, when switch mode.set it to 1*/
    MT_U8               u8IsCaptureText;  /*to indicate whether to capture text*/
    MT_U8               u8IsXDSStarted;  /*is vbi data xds packet*/
    MT_U8               u8IsTextShown;   /*when show text set this variable. When EDM reset it*/
    ...
    MT_U8               au8CharWidth[2];  /*each character width*/

    CCDISP_WINDOW_ID_E  enWindowId;

    CRITICAL_SECTION   stCriticalSection; /*mutual exclusive critical section*/
    MT_UNF_CC_608_DATATYPE_E    enCCNum;              /*cc number cc1..cc4,tc1..tc4 */
    MT_U8              u8IsDecStart;            /*is decoder start*/
    TRI_QUEUE          stCCDataQueue;
    MT_U16             au16CCData[CC608DATA_QUEUE_SIZE][CC608DATA_BUFFER_SIZE];
    MT_U8              u8CmdCount;
    MT_S32             s32VChipTimerID;
    MT_S32             s32TimeoutEraseTimerID;
    MT_BOOL            bIgnoreBACWithoutSpace;
    MT_BOOL            bIgnoreExtCharWithoutStdChar;
    MT_BOOL            bOnlyUseSolidBlackBGIn608;
    MT_BOOL            bUnderline;
    MT_BOOL            bPACOverwrite;

    MT_BOOL            bLeadingTailingSpace;
} CC608_OBJECT_S;
