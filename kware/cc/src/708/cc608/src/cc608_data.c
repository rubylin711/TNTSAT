...
*******************************************************************************/
static void _CC608_DATA_ClrCell(MT_U8 module_id, MT_U8 row,MT_U8 column, MT_U8 len);
static inline void _CC608_DATA_CalcStaPosition(MT_U8 module_id);
static inline void _CC608_DATA_ResetBuffer(MT_U8 module_id,CC608_BUFFER_TYPE_E buf_type);
static MT_U16 _CC608_DATA_CalcCurrentCursor(MT_U8 module_id,MT_U8 row,MT_U8 column,CC608_BUFFER_TYPE_E buf_type);
static MT_S32 _CC608_DATA_PaintCaption(MT_U8 module_id, MT_U16 *buf, MT_U8 len,
                         MT_U8 is_overflow, MT_BOOL bClear, MT_BOOL bHasLeadingSpace, MT_BOOL bTailingSpace);
static MT_S32 _CC608_DATA_PopCaption(MT_U8 module_id);
static MT_S32 _CC608_DATA_ClearRows(MT_U8 module_id, MT_U8 top, MT_U8 bottom);
static void _CC608_DATA_ResetText(MT_U8 module_id);
static void _CC608_DATA_EnterTextMode(MT_U8 module_id);
static void _CC608_DATA_EnterCaptionMode(MT_U8 module_id);
static inline MT_S32 _CC608_DATA_StoreCaptionToBuffer(MT_U8 module_id,MT_U16 *caption,MT_U16 len,CC608_BUFFER_TYPE_E buf_type);
static inline void _CC608_DATA_MoveToNewBaserow(MT_U8 module_id,MT_U8 old_baserow,MT_U8 old_rows);
static void _CC608_DATA_RedrawRowFromColumn(MT_U8 module_id,MT_U8 row,MT_U8 fromcolumn);
static MT_BOOL _CC608_DATA_IsCharDisplayable(MT_U16 caption);
static void _CC608_DATA_PaintLeadingTailingSpace(MT_U8 module_id, MT_U16 x, MT_U16 y,MT_U16 width, MT_U16 height);
static void _CC608_DATA_SetUnderline(MT_U8 module_id, MT_BOOL bUnderline);
/*******************************************************************************
*                       Export Function Definition
*******************************************************************************/
/*****************************************************************************
* function name: CC608_DATA_ClearSTA(MT_U8 module_id);                   *
...
MT_S32 CC608_DATA_ClearSTA(MT_U8 module_id)
{
    ...
    {
       (MT_VOID)CCDISP_Screen_SetColor(MT_UNF_CC_COLOR_BLACK,MT_UNF_CC_OPACITY_TRANSPARENT);
    }

    (MT_VOID)CCDISP_Window_FillRect(pstObject->enWindowId,MT_UNF_CC_COLOR_BLACK,MT_UNF_CC_OPACITY_TRANSPARENT,rect,1);
...
/*****************************************************************************
* function name: CC608_DATA_Reset(MT_U8 module_id);                      *
...
void CC608_DATA_Reset(MT_U8 module_id)
{
     ...
     pstObject->stPenAttr.bIsFlash = MT_FALSE;
     pstObject->stPenAttr.bIsItalic = MT_FALSE;
     pstObject->stPenAttr.bIsUnderline = MT_FALSE;
     pstObject->stPenAttr.u32FgColor        = MT_UNF_CC_COLOR_WHITE;
     pstObject->stPenAttr.enFgOpacity      = MT_UNF_CC_OPACITY_SOLID;
     pstObject->stPenAttr.u32BgColor        = MT_UNF_CC_COLOR_BLACK;
     pstObject->stPenAttr.enBgOpacity      = MT_UNF_CC_OPACITY_SOLID;

     (MT_VOID)CCDISP_Text_SetBGColor(pstObject->enWindowId,pstObject->stPenAttr.u32BgColor,pstObject->stPenAttr.enBgOpacity);
     (MT_VOID)CCDISP_Text_SetFGColor(pstObject->enWindowId,pstObject->stPenAttr.u32FgColor,pstObject->stPenAttr.enFgOpacity);

     (MT_VOID)CCDISP_Screen_SetColor(MT_UNF_CC_COLOR_BLACK,MT_UNF_CC_OPACITY_TRANSPARENT);
     (MT_VOID)CCDISP_Window_SetColor(pstObject->enWindowId,MT_UNF_CC_COLOR_BLACK,MT_UNF_CC_OPACITY_SOLID);

     (MT_VOID)CCDISP_Text_SetFontName(pstObject->enWindowId,MT_UNF_CC_FN_MONOSPACED);
     (MT_VOID)CCDISP_Text_SetFontSize(pstObject->enWindowId,MT_UNF_CC_FONTSIZE_STANDARD);

     (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,pstObject->stPenAttr.bIsUnderline);
     (MT_VOID)CCDISP_Text_SetFlash(pstObject->enWindowId,pstObject->stPenAttr.bIsFlash);
     ...
     (MT_VOID)CC608_DATA_ClearSTA(module_id);
...
/*****************************************************************************
* function name: CC608_DATA_SetBgColor(MT_U8 module_id, MT_U8 bg_color, MT_UNF_CC_OPACITY_E bg_opct)*
...
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    CC608_BUFFER_S *pstBuffer = MT_NULL;
    MT_U8 u8Row;
    MT_U32 u32Color;
    ...
    if( pstObject->stUserPenAttr.u32BgColor == MT_UNF_CC_COLOR_DEFAULT )
    {
    ...
    if( pstObject->stUserPenAttr.enBgOpacity == MT_UNF_CC_OPACITY_DEFAULT )
    {
    ...
    (MT_VOID)CCDISP_Text_SetBGColor(pstObject->enWindowId,u32Color,enOpacity);
    ...
    if(pstObject->enCurMode == CC608_MODE_CAPTION)
    {
        ...
        u8Row = (MT_U8)pstBuffer->u8CurRow;
...
/*****************************************************************************
* function name: void CC608_DATA_SetTextAttr(MT_U8 module_id,  *
*                    MT_U8 txt_color, MT_U8 is_under) *
...
void CC608_DATA_SetTextAttr(MT_U8 module_id, MT_U32 u32TxtColor, MT_U8 u8IsUnderline)
{
    ...
    if( pstObject->stUserPenAttr.u32FgColor == MT_UNF_CC_COLOR_DEFAULT )
    {
        if(u32TxtColor != 0)
        {
    ...
    if( pstObject->stUserPenAttr.bIsUnderline == MT_FALSE )
    {
        pstObject->stPenAttr.bIsUnderline = u8IsUnderline ? MT_TRUE : MT_FALSE;
    }
    else if( pstObject->stUserPenAttr.bIsUnderline == MT_TRUE )
    {
        pstObject->stPenAttr.bIsUnderline = MT_TRUE;
    }
    else
    {
        pstObject->stPenAttr.bIsUnderline = MT_FALSE;
    ...
    if( pstObject->stUserPenAttr.enFgOpacity == MT_UNF_CC_OPACITY_DEFAULT)
    {
        pstObject->stPenAttr.enFgOpacity = MT_UNF_CC_OPACITY_SOLID;
    }
    else
    ...
    (MT_VOID)CCDISP_Text_SetFGColor(pstObject->enWindowId,u32Color,enOpacity);
    (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,pstObject->stPenAttr.bIsUnderline);
...
void CC608_DATA_GetTextAttr(MT_U8 module_id, MT_U32 *pu32TxtColor, MT_U8 *pu8IsUnderline)
{
    ...
    if( pu32TxtColor != MT_NULL )
    {
        *pu32TxtColor = pstObject->stPenAttr.u32FgColor;
    }
    if( pu8IsUnderline != MT_NULL )
    {
...
void CC608_DATA_SetTextItalic(MT_U8 module_id, MT_U8 u8IsItalic)
{
    ...
    if( pstObject->stUserPenAttr.bIsItalic == MT_FALSE )
    {
        pstObject->stPenAttr.bIsItalic = u8IsItalic ? MT_TRUE : MT_FALSE;
    }
    else if( pstObject->stUserPenAttr.bIsItalic == MT_TRUE )
    {
        pstObject->stPenAttr.bIsItalic = MT_TRUE;
    }
    else
    {
        pstObject->stPenAttr.bIsItalic = MT_FALSE;
    ...
    (MT_VOID)CCDISP_Text_SetItalic(pstObject->enWindowId,pstObject->stPenAttr.bIsItalic);
...
/**************************************************************************
* function name: CC608_DATA_SetDefaultAttr(MT_U8 module_id)           *
...
void CC608_DATA_SetDefaultAttr(MT_U8 module_id)
{
    ...
    (MT_VOID)CCDISP_Text_SetBGColor(pstObject->enWindowId, pstObject->stPenAttr.u32BgColor, pstObject->stPenAttr.enBgOpacity);
    (MT_VOID)CCDISP_Text_SetFGColor(pstObject->enWindowId, pstObject->stPenAttr.u32FgColor, pstObject->stPenAttr.enFgOpacity);
    (MT_VOID)CCDISP_Text_SetFontName(pstObject->enWindowId, MT_UNF_CC_FN_DEFAULT);
...
/*source monitor*/
static void CC608_DATA_Caption_ModeText(MT_U8 module_id, MT_U16 *pu16CaptionData, MT_U8 u8Len, CC608_OBJECT_S *pstObject)
{
    MT_U8 row,column,overflow_num;
    MT_U8 ret_val = 0;
    MT_BOOL bLeadingSpace = MT_FALSE;
    MT_BOOL bClear = MT_TRUE;
    ...
    MT_BOOL bTailing=MT_TRUE;
    MT_S32 tailing_pos=column+u8Len;
    /**
    *   check if tailing pos has MT_U8
    ...
    */
    ...
    (MT_VOID)_CC608_DATA_PaintCaption(module_id, pu16CaptionData, u8Len,!ret_val,bClear,MT_FALSE,bTailing);
...
void CC608_DATA_Caption(MT_U8 module_id, MT_U16 *pu16CaptionData, MT_U8 u8Len)
{
    ...
    MT_BOOL bLeadingSpace = MT_FALSE;
    MT_BOOL bClear = MT_TRUE;
    MT_BOOL bTailingSpace = MT_TRUE;

    if( MT_NULL == pstObject || pu16CaptionData == MT_NULL || u8Len == 0 )
    {
        MT_ERR_CC("Invalid module id:%d\n",module_id);
        return;
    }
    ...
    if( pstObject->enCurMode == CC608_MODE_CAPTION )
    {
        ...
        {
            (MT_VOID)CC608_TimeoutErase_Start(module_id,CC608_AUTOERASE_TIMEOUT);
            (MT_VOID)_CC608_DATA_StoreCaptionToBuffer(module_id,pu16CaptionData,u8Len,CC608_BUFFER_OFF_SCREEN);
        }
        else
        {
            (MT_VOID)CC608_TimeoutErase_Start(module_id,CC608_AUTOERASE_TIMEOUT);
            row = (MT_U8)pstObject->stOnBuffer.u8CurRow;
            column = (MT_U8)pstObject->stOnBuffer.u8CurColumn;
            ...
            (MT_VOID)_CC608_DATA_PaintCaption(module_id, pu16CaptionData, u8Len,!ret_val,bClear,bLeadingSpace,bTailingSpace);
            if( ret_val == 2 && pstObject->enDispStyle == CC608_DISP_PAINTON)
            {
                ...
                {
                    CC608_DEC_MidRow(module_id,0x11,(MT_U8)((MT_U32)ccdata&0xff));
                }
            }
            ...
            if(((ret_val == 4)||(ret_val == 5)) && pstObject->enDispStyle == CC608_DISP_PAINTON)
            {
                MT_U32 pac;
                MT_U32 rowchar;
                MT_U8 is_underline = 0,is_italic = 0;
                MT_U8 txt_color = 0;
...
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_S32 rollup_rows,valid_rows;
    MT_U8 u8OldBaseRow;

    if( MT_NULL == pstObject || u8Row >= CC608ROWS )
    {
        MT_ERR_CC("Invalid module id:%d\n",module_id);
        return;
    }
...
/************************************************************************************
* function name:  CC608_DATA_SetColumn(MT_U8 module_id, MT_U8 column                     *
...
void CC608_DATA_SetColumn(MT_U8 module_id, MT_U8 u8Column)
{
    ...
    if( MT_NULL==pstObject || u8Column >= CC608COLUMNS - 1)
    {
        MT_ERR_CC("Invalid module id:%d\n",module_id);
        return;
    }
    ...
    if( u8Column == 0 )
    {
        CC608_DATA_SetBgColor(module_id,MT_UNF_CC_COLOR_BLACK,MT_UNF_CC_OPACITY_SOLID);
    }
...
/************************************************************************************
* function name:  void CC608_DATA_SetDefaultRowColumn(MT_U8 module_id,          *
...
/**********************************************************************************
* function name:  CC608_DATA_SetPacCode(MT_U8 module_id, MT_U8 pac    *
...
void CC608_DATA_SetPacCode(MT_U8 module_id, MT_U8 u8Pac)
{
    ...
    if(pstObject->enCurMode == CC608_MODE_CAPTION)
    {
        ...
        {
            pstObject->bPACOverwrite = MT_TRUE;
        }
...
* arguments    : none                                                       *
****************************************************************************/
MT_S32 CC608_DATA_ResumeCL(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
...
****************************************************************************/
MT_S32 CC608_DATA_BackSpace(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_U8 row,column,cap_len;
    MT_UNF_CC_RECT_S rect;
    ...
    if( pstObject->enCurMode == CC608_MODE_TEXT )
    {
        ...
        MT_U32 u32Color = 0;
        MT_UNF_CC_OPACITY_E enOpacity = MT_UNF_CC_OPACITY_DEFAULT;
        (MT_VOID)CCDISP_Window_GetColor(pstObject->enWindowId, &u32Color, &enOpacity);
        (MT_VOID)CCDISP_Window_FillRect(pstObject->enWindowId,u32Color,enOpacity,rect,1);
    ...
    if( pstObject->enDispStyle == CC608_DISP_POPON )
    {
        ...
        else
        {
            ...
            MT_INFO_CC("current column = 0, no backspace \n");
        }
        return MT_SUCCESS;
    }
...
*                the substitute character of the received extend MT_U8    *
* arguments    : none                                                       *
    ...
    if(pstObject->enCurMode == CC608_MODE_CAPTION)
    {
        ...
        if(pstObject->stScreen.u8CurColumn > 0 )
        {
            ...
            //for non-monospaced font, the char width should reacquire.
            {
                MT_S32 s32Width = 0;
                MT_S32 s32Height = 0;
                MT_U16 *pu16Str = &(pstObject->stOnBuffer.stCaption[row][column].u16Text);
                (MT_VOID)CCDISP_Text_GetSize(pstObject->enWindowId, pu16Str, 1, &s32Width, &s32Height);
                cap_len = s32Width;
                pstObject->stOnBuffer.stCaption[row][column].u16Text = 0;
            }
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_AlarmOff(MT_U8 module_id)
{
    //TODU
    return MT_SUCCESS;
}
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_AlarmOn(MT_U8 module_id)
{
    //TODU
    return MT_SUCCESS;
}
...
* arguments    : none                                                       *
****************************************************************************/
MT_S32 CC608_DATA_DeleteToER(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_S32 i,row;
    CC608_BUFFER_S *pstBuffer = MT_NULL;
...
/*****************************************************************************
* function name: CC608_DATA_Rollup(MT_U8 rows);                                  *
...
****************************************************************************/
MT_S32 CC608_DATA_Rollup(MT_U8 module_id, MT_U8 u8RollupRows)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_U8 u8OldRollupRows,u8OldTopRow;
    ...
    if( pstObject->enDispStyle == CC608_DISP_ROLLUP )
    {
        ...
        if ( u8OldRollupRows != u8RollupRows )
        {
            ...
            if ((u8RollupRows < u8OldRollupRows) && (pstObject->stScreen.u8TopRow > 0))
            {
                ...
                (MT_VOID)_CC608_DATA_ClearRows(module_id,u8OldTopRow, pstObject->stScreen.u8TopRow - 1);
            }
        }
        return MT_SUCCESS;
    }
...
/*****************************************************************************
* function name: CC608_DATA_FlashOn(MT_U8 is_flash);                     *
* descripton   : caption flash on(off)                                      *
* arguments    :                                                            *
*                is_flash   1:on  0:off                                     *
****************************************************************************/
MT_S32 CC608_DATA_FlashOn(MT_U8 module_id, MT_U8 u8IsFlash)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    CC608_BUFFER_S *pstBuffer = MT_NULL;
    MT_U8 row;
    MT_BOOL bIsFlash = u8IsFlash ? MT_TRUE : MT_FALSE;
    if( MT_NULL==pstObject )
    {
        MT_ERR_CC("Invalid module id:%d\n",module_id);
        return MT_FAILURE;
    ...
    (MT_VOID)CCDISP_Text_SetFlash(pstObject->enWindowId,bIsFlash);
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_ResumeDC(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_TextRestart(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_TextDisplay(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
...
* arguments    : none                                                       *
****************************************************************************/
MT_S32 CC608_DATA_EraseDM(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
...
* arguments    : none                                                       *
****************************************************************************/
MT_S32 CC608_DATA_CarriageReturn(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_S32 x,top_y, btm_y;
    CC608_BUFFER_S *pstBuffer = MT_NULL;
    MT_U8 row;
    ...
    (MT_VOID)CCDISP_Window_Rollup(pstObject->enWindowId,x,pstObject->stScreen.stStaScreen.width,top_y,
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_EraseNM(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    if( MT_NULL==pstObject )
    {
 ...
 * arguments    : none                                                       *
 ****************************************************************************/
MT_S32 CC608_DATA_FlipMemory(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    ...
    (MT_VOID)_CC608_DATA_PopCaption(module_id);
...
/*****************************************************************************
 * function name: CC608_DATA_Tab(MT_U8 columns);                          *
 ...
 ******************************************************************/
void CC608_DATA_ResumeLastAttr(MT_U8 module_id)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_U8 row;
    MT_U32 color;
    MT_UNF_CC_OPACITY_E opac;
    ...
    (MT_VOID)CCDISP_Text_SetItalic(pstObject->enWindowId,pstObject->stOnBuffer.stLastPenAttr[row].bIsItalic);
    pstObject->stPenAttr.bIsItalic = pstObject->stOnBuffer.stLastPenAttr[row].bIsItalic;

    _CC608_DATA_SetUnderline(module_id,(MT_BOOL)pstObject->stOnBuffer.stLastPenAttr[row].bIsUnderline);
    ...
    (MT_VOID)CCDISP_Text_SetFGColor(pstObject->enWindowId,color,opac);
    (MT_VOID)CCDISP_Text_SetFlash(pstObject->enWindowId,pstObject->stOnBuffer.stLastPenAttr[row].bIsFlash);
}

static void CC608_TimeoutErase_Callback(MT_U32 u32Param)
{
    MT_U32 ucModuleID = u32Param;
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(ucModuleID);

    if(pstObject == MT_NULL)
    {
        return;
    }
...
{
    MT_U32 u32ModuleID = module_id;

    (MT_VOID)CCTimer_Open(CCTIMER_ID_608ERASE, CC608_TimeoutErase_Callback, u32ModuleID);

    return (MT_S32)CCTIMER_ID_608ERASE;
}

void CC608_TimeoutErase_TimerStop(MT_U8 module_id)
{
    (MT_VOID)CCTimer_Stop(CCTIMER_ID_608ERASE);
}

MT_S32 CC608_TimeoutErase_Start(MT_U8 module_id, MT_U32 u32Seconds)
{
    MT_U32 u32Msec = u32Seconds * 1000;

    (MT_VOID)CCTimer_Start(CCTIMER_ID_608ERASE, u32Msec, TIMER_MODE_ONE_SHOOT);
...
static MT_BOOL _CC608_DATA_IsCharDisplayable(MT_U16 caption)
{
    MT_BOOL bDisplayable = MT_FALSE;

    if(caption != 0x1111)
    {
        bDisplayable = MT_TRUE;
    }
    return bDisplayable;
}
...
static inline void _CC608_DATA_ResetBuffer(MT_U8 module_id,CC608_BUFFER_TYPE_E buf_type)
{
    ...
    {
        MT_ERR_CC("Invalid buf_type:0x%x\n",buf_type);
        return;
    }
...
/**************************************************************************
* function name: _CC608_DATA_CalcCurrentCursor(MT_U8 module_id,                    *
*                                        MT_U8 row,              *
*                                        MT_U8 column            *
                     ...
                     MT_U8 is_overflow, MT_BOOL bClear, MT_BOOL bHasLeadingSpace, MT_BOOL bTailingSpace)
{
    ...
    else if( buf[0] >= 0x1120 && buf[0] <= 0x112f )
    {
        ...
        buf[0] = 0x20;
        //(MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId, x, y, buf, 1, &s32DrawWidth);
    ...
    else if( buf[0] == 0xbbbb )
    {
        ...
        buf[0] = 0x20;
        (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId, x, y, buf, 1, &s32DrawWidth,1);

        (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,pstObject->stPenAttr.bIsUnderline);

        (MT_VOID)CC608_DATA_FlashOn(module_id,1);
    }
             ...
             ((buf[0]&0xf7ff) >= 0x172d && (buf[0]&0xf7ff) <= 0x172f))
    {
        ...
        (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,MT_FALSE);
    ...
    else   /*regular character*/
    {
        if( pstObject->stScreen.u8CurColumn == 32 && len == 2 &&is_overflow )
        {
            ...
            (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,(MT_BOOL)pstObject->stPenAttr.bIsUnderline);

            (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId, x, y, buf, 1, &s32DrawWidth,1);
            (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId, x, y, buf+1, 1, &s32DrawWidth,1);
        }
        else
        {
            OSD_TRACE("paint_on2:%c%c,cur_width:%d\n",buf[0],buf[1],pstObject->stScreen.u16CurTextWidth);
            (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,(MT_BOOL)pstObject->stPenAttr.bIsUnderline);
            (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId, x, y, buf, len, &s32DrawWidth,1);
        }
    }
...
/*****************************************************************************
* function name: _CC608_DATA_PaintLeadingTailingSpace(MT_U8 module_id, MT_U16 x, MT_U16 y) *
...
static void _CC608_DATA_PaintLeadingTailingSpace(MT_U8 module_id, MT_U16 x, MT_U16 y,MT_U16 width, MT_U16 height)
{
    ...
    if(MT_TRUE == pstObject->bLeadingTailingSpace)
    {
        rect.x = x;
        rect.y = y;
        rect.width = width;
        ...
        MT_U32 u32Color = 0;
        MT_UNF_CC_OPACITY_E enOpacity = MT_UNF_CC_OPACITY_DEFAULT;
        (MT_VOID)CCDISP_Text_GetBGColor(pstObject->enWindowId, &u32Color, &enOpacity);
        (MT_VOID)CCDISP_Window_FillRect(pstObject->enWindowId,u32Color,enOpacity,rect,1);
    }
...
/*****************************************************************************
* function name: _CC608_DATA_PopCaption(MT_U8 module_id);                      *
...
static MT_S32 _CC608_DATA_PopCaption(MT_U8 module_id)
{
    ...
    MT_U16 u16CaptionData;
    MT_U16 au16Buf[64];
    MT_U8 u8CharCount,u8RightCharNumInRow = 0;
    MT_S32 s32FirstCaptionColumn = -1;
    MT_S32 s32DrawX = 0, s32DrawY = 0, s32StaX = 0, s32DrawWidth = 0;
    ...
    for( s32Row = 0; s32Row < CC608ROWS; s32Row++ )
    {
        ...
        for( s32Column = 0; s32Column < CC608COLUMNS; s32Column++)
        {
            ...
            if( pstObject->stOffBuffer.stCaption[s32Row][s32Column].u8IsChar == 1)
            {
                ...
                {
                    (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,(MT_BOOL)pstObject->stPenAttr.bIsUnderline);
                    (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId,s32DrawX,s32DrawY,au16Buf,u8CharCount,&s32DrawWidth,1);
                    u8CharCount = 0;
                    s32FirstCaptionColumn = -1;
                }
        ...
        (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,(MT_BOOL)pstObject->stPenAttr.bIsUnderline);
        (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId,s32StaX,s32DrawY,au16Buf,u8CharCount,&s32DrawWidth,1);
        if(s32DrawWidth > 0)
        {
            pstObject->stScreen.u16CurTextWidth += s32DrawWidth;
        }
...
/*****************************************************************************
* function name: _CC608_DATA_ClearRows(MT_U8 module_id);                       *
* descripton   : clear safe title area with window-color(transparent)       *
* arguments    :                                                            *
*                                                                           *
****************************************************************************/
static MT_S32 _CC608_DATA_ClearRows(MT_U8 module_id, MT_U8 top, MT_U8 bottom)
{
    ...
    if( top > bottom )
    {
        MT_ERR_CC("Invalide row parameter top = %d, bottom = %d for _CC608_DATA_ClearRows!\n",top, bottom);
        return MT_FAILURE;
    }
...
/********************************************************************************
* function name: _CC608_DATA_ResetText(MT_U8 module_id)                           *
...
static void _CC608_DATA_ResetText(MT_U8 module_id)
{
    ...
    (MT_VOID)CCDISP_Window_SetColor(pstObject->enWindowId,MT_UNF_CC_COLOR_BLACK,MT_UNF_CC_OPACITY_TRANSPARENT);

    /*Draw text background window*/
    (MT_VOID)_CC608_DATA_ClearRows(module_id, 7, CC608ROWS - 1);
    return;
}

/**************************************************************************
* function name: _CC608_DATA_EnterTextMode(MT_U8 module_id)                *
...
/**************************************************************************
* function name: _CC608_DATA_EnterCaptionMode(MT_U8 module_id)             *
...
/**************************************************************************
* function name: store_caption_to_buffe(MT_U8 module_id,         *
*                                       MT_U16 *caption,         *
*                                       MT_U16 len,              *
*                                       CC608_BUFFER_TYPE_E buf_type)        *
...
/**************************************************************************
* function name: _CC608_DATA_MoveToNewBaserow(MT_U8 ,MT_U8 ,MT_U8 )               *
*                                                                        *
...
*                to new base row                                         *
*************************************************************************/
static inline void _CC608_DATA_MoveToNewBaserow(MT_U8 module_id,MT_U8 old_baserow,MT_U8 old_rollrows)
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_U8 u8RollupRows,u8NewBaseRow;
    //MT_U16 x,srcy,dsty,height,width;
    MT_S32 i;
    ...
    //(MT_VOID)CCDISP_Window_BlockMove(pstObject->enWindowId,x,dsty,width,height,x,srcy);
#endif
    ...
    {
        /*didn't clip*/
        (MT_VOID)_CC608_DATA_ClearRows(module_id,old_baserow + 1 - u8RollupRows, old_baserow);
    }
    else if( old_baserow > pstObject->stScreen.u8BaseRow )
    {
        /*move up*/
        (MT_VOID)_CC608_DATA_ClearRows( module_id,pstObject->stScreen.u8BaseRow + 1, old_baserow );
    }
    else if( old_baserow < pstObject->stScreen.u8BaseRow )
    {
        /*move down*/
        (MT_VOID)_CC608_DATA_ClearRows( module_id, old_baserow + 1 - u8RollupRows, pstObject->stScreen.u8BaseRow - u8RollupRows);
    }
}
...
{
    CC608_OBJECT_S *pstObject = CC608_OBJ_Get(module_id);
    MT_S32 s32Column , s32DrawWidth = 0;
    MT_U16 x,y,ccdata;
    MT_U16 buf[64];
    MT_U8 u8CharCount = 0;
    MT_U8 u8FirstCaptionColumn = 0xff;
    MT_S32 s32CaptionLen = 0;
    MT_UNF_CC_RECT_S rect;
    MT_S32 width = 0,height = 0;
    MT_U8 u8MidrowCode;
    ...
    if(row >= CC608ROWS)
    {
 	  MT_ERR_CC("Invalid row num:%d\n",row);
 	  return;
    }

    /*if there is no MT_U8 on this row, do nothing*/
    if( pstObject->stOnBuffer.u8CharNum[row] == 0 )
    {
        return;
    }
    ...
    for( s32Column = fromcolumn; s32Column < CC608ROWCHARS; s32Column++)
    {
            ...
            pstObject->stOnBuffer.stCaption[row][s32Column].u8IsDisplay )
        {
            ...
            if( ccdata == 0x1111 )
            {
                ...
                (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,(MT_BOOL)pstObject->stPenAttr.bIsUnderline);
                (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId,x+s32CaptionLen,y,buf,u8CharCount,&s32DrawWidth,1);
            ...
            else if( ccdata >= 0x1120 && ccdata <= 0x112f )
            {
                ...
                (MT_VOID)CCDISP_Text_GetSize(pstObject->enWindowId,buf,u8CharCount,&width,&height);

                buf[0] = 0x20;
                (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId,x+s32CaptionLen+width,y,buf,1,&s32DrawWidth,1);
    ...
    (MT_VOID)CCDISP_Text_SetUnderLine(pstObject->enWindowId,(MT_BOOL)pstObject->stPenAttr.bIsUnderline);
    (MT_VOID)CCDISP_Text_Draw(pstObject->enWindowId,x,y,buf,u8CharCount,&s32DrawWidth,1);
    if(s32DrawWidth > 0)
    {
        pstObject->stScreen.u16CurTextWidth += s32DrawWidth;
    }
