...
/*C0 Code Space*/
#define C0_NUL  ((MT_U8) 0x0)
#define C0_ETX  ((MT_U8) 0x3)
#define C0_CR   ((MT_U8) 0xD)        // carriage return (soft)
#define C0_HCR ((MT_U8) 0xE)
#define C0_FF ((MT_U8) 0xC)
#define C0_EXT1 ((MT_U8) 0x10)       // code space extension - 1 MT_U8

/*C1 Code Space */
#define C1_DLC      ((MT_U8) 0x8E)   // DelayCancel
#define C1_RST      ((MT_U8) 0x8F)   // Reset
...
{ \
        if( (argu) >= (maxval) ) \
        { \
                MT_INFO_CC("Parameter Error! %s validation(<%d) check failed(%d)!\n", \
                (args), \
                (maxval), \
                (argu)); \
                argu = (default_val); \
...
{ \
        if( (argu) >= (maxval) ) \
        { \
                MT_INFO_CC("Parameter Error! %s validation(param<%d) check failed(%d)!\n", \
                (args), \
                (maxval), \
                (argu)); \
                return ; \
...
{ \
        if((argu) == NULL) \
        { \
                MT_INFO_CC("Parameter Error! %s validation check failed!\n", \
                (args)); \
                return ; \
        } \
}
...
{
    MT_UNF_CC_JUSTIFY_E     enJustify;
    CC708_PrintDirection_E  enPrintDirection;
    CC708_ScrollDirection_E enScrollDirection;
    MT_UNF_CC_WORDWRAP_E    enWordWrap;
    CC708_DisplayEffect_E   enDisplayEffect;
    CC708_EffectDirection_E enEffectDirection;
    MT_U8                   u8EffectSpeed;     /*in 0.5 second units,*/
    MT_U32       u32FillColor;
    MT_UNF_CC_OPACITY_E     enFillOpacity;
    MT_UNF_CC_EdgeType_E    enBorderType;
    MT_U32       u32BorderColor;
}CC708_WindowAttr_S;
...
{
    MT_U16              u16Caption;     /* if equal to zero, there is no u16Caption */
    MT_U8               u8IsPenChanged; /* if the text attributes changed from this position */
...
typedef struct tagCC708_Window_S
{
    ...
    MT_U8          u8Priority;
    MT_U8          u8AnchorPoint;
    MT_U8          u8RelativePositioning;   /*0--absolute, 1-- relative*/
    MT_U8          u8AnchorVertical;
    MT_U8          u8AnchorHorizontal;
    MT_U8          u8RowCount;
    MT_U8          u8ColumnCount;
    MT_U8          u8IsRowLock;
    MT_U8          u8IsColumnLock;
    MT_U8          u8IsVisible;
    MT_U8          u8WindowStyleID;
    MT_U8          u8PenStyleID;

    /*follows are window attribution*/
    MT_UNF_CC_JUSTIFY_E        enTextJustify;
    ...
    /* row attributes */
    MT_S32                as32RowWidth[MAX_CC_ROWS];
    MT_S32                as32RowHeight[MAX_CC_ROWS];
    MT_BOOL               bAddedTailingSpaceWidth[MAX_CC_ROWS];

    CC708_Cursor_S        stCursor;
    CC708_PenDef_S        stCurPen;
    CC708_PenDef_S        stPrevPen;
    MT_BOOL               bPenColorChanged;
    ...
    MT_U8                 u8IsEmpty;
    MT_BOOL               bPendingRow; /*if not left justified and window is visible, set this flag and draw the caption until next control command.*/
    MT_S32                s32PendingRow;

    MT_U32                u32OverlapLowerWindowMap;// window map which has lower u8Priority and overlaps with current one
    MT_U32                u32OverlapHigherWindowMap;// window map which has higher u8Priority and overlaps with current one

    //MT_UNF_CC_RECT_S CCClipArea[32];
    //MT_U8                u8ClipAreaNum;

    /* window display order */
    /* 0 is the first displayed window on the screen, 7 is the last */
    /* -1 is the default value */
    MT_S32                s32DisplayOrder;
