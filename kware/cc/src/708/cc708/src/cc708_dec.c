...
typedef MT_VOID (*DTVCC_CmdDecode_FN)(MT_U8 *pu8Cmd);
...
{
    MT_U16 u16Buf[CAPTION_BUF_SIZE];
    MT_U8  u8BufLen;
}CC708_CaptionBuf_S;
...
typedef struct tagNTSC_BitStream_S
{
    ...
    MT_U8     *pu8Buf;
    MT_U8     *pu8End;
    MT_U8     u8LeftBits;    /* read_count number of available bits */
} NTSC_BitStream_S;

/*******************************************************************************
*                       Static Data Definition
*******************************************************************************/
static MT_VOID _DtvCC_NulCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_ETXCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_BSCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_FFCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_CRCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_HCRCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_EXT1Cmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_P16Cmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_SetCurrentWindowCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_ClearWindowsCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_DisplayWindowsCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_HideWindowsCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_ToggleWindowsCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_DeleteWindowsCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_DelayCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_ResetCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_SetPenAttributesCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_SetPenColorCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_SetPenLocationCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_SetWindowAttributesCmd(MT_U8 *pu8Cmd);
static MT_VOID _DtvCC_DefineWindowCmd(MT_U8 *pu8Cmd);

static MT_BOOL _DtvCC_OddParityCheck(MT_U8 data);
static MT_S32 _DtvCC_GetEXT1CmdLen(MT_U8 *pu8Cmd, MT_S32 avalaible_len);
...
static MT_U16 DTVCCG2Code2UnicodeMap[96] =
{
...
static MT_U16 DTVCCG3Code2UnicodeMap[96] =
{
...
static void _DtvCC_DelayTimerCallback(MT_U32 u32Param)
{
    MT_U8 u8ModuleId = (MT_U8)u32Param;
...
static MT_VOID _DtvCC_DelayTimerInit(MT_U8 u8ModuleID)
{
    MT_U32 u32ModuleID = (MT_U32)u8ModuleID;
    (MT_VOID)CCTimer_Open(CCTIMER_ID_708DELAY, _DtvCC_DelayTimerCallback, u32ModuleID);
}

static void _DtvCC_DelayTimerStart(MT_U8 u8ModuleID, MT_U8 u8TenthOfSecond)
{
    MT_U32 u32Msec = u8TenthOfSecond * 100;
   (MT_VOID)CCTimer_Start(CCTIMER_ID_708DELAY, u32Msec, TIMER_MODE_ONE_SHOOT);
}

static void _DtvCC_DelayTimerStop(MT_U8 u8ModuleID)
{
    (MT_VOID)CCTimer_Stop(CCTIMER_ID_708DELAY);
}

static MT_U32 _DtvCC_Translate2Color(MT_U8 u8Color)
{
    Color_S stRGB;

    MT_U32 u32Color = 0;

    static MT_U32 au8RGBMap[3][4] = {
...
*****************************************************************************************/
/************************************************************************
 * Function name: MT_S32 _DtvCC_ParseServiceBlock(MT_U8 *pbuf, MT_U8 len)
 ...
 * Todo:
 *************************************************************************/
static MT_S32 _DtvCC_ParseServiceBlock(MT_U8 *pu8Buf, MT_U8 u8BufLen)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);
    MT_U8 *pu8ServiceData = NULL;
    MT_U8 u8ServiceLen,u8Offset = 0;
    MT_U8 u8ServiceData;
    DTVCC_CMDBuffer_S *pstCMDBuffer = &pstCCObj->stCMDBuffer;

    if(NULL == pu8Buf || 0 == u8BufLen)
    {
        MT_ERR_CC("pu8Buf pointer NULL!\n");
        return MT_FAILURE;
    }
    ...
    if(pstCMDBuffer->u8CurSize < pstCMDBuffer->u8TotalSize)
    {
        ...
        if((u8ServiceLen - u8Offset) >= (pstCMDBuffer->u8TotalSize - pstCMDBuffer->u8CurSize))
        {
            ...
            MT_U8 u8CMD;
            MT_U8 CmdLen;
            DTVCC_CmdDecode_FN DTVCCCmdDecoder = NULL;
        ...
        if(u8ServiceData < 0x20 || ((0x7f < u8ServiceData) && (u8ServiceData < 0xa0)))/*CMD,C0,C1*/
        {
            ...
            if(((0x7f < u8ServiceData) && (u8ServiceData < 0xa0)) || u8ServiceData == C0_ETX || u8ServiceData== C0_CR || u8ServiceData== C0_HCR || u8ServiceData== C0_FF)
            {
               if((stCaptionBuf.u8BufLen > 0) && (stCaptionBuf.u8BufLen <= CAPTION_BUF_SIZE))
               {
                  ...
                  memset(stCaptionBuf.u16Buf, 0, sizeof(MT_U16)*stCaptionBuf.u8BufLen);
...
/*CEA-708 P24 */
static MT_S32 _DtvCC_GetEXT1CmdLen(MT_U8 *pu8Cmd, MT_S32 s32AvalaibleLen)
{
    MT_U8 u8ByteCode;
    MT_U8 u8Length;
...
/*************************************************************************************************
 * Function name: static MT_U8 _DtvCC_DetectRstDcCmd(MT_U8 *pbuf, MT_U8 len, MT_U8 *pu8Cmd)*
 ...
 * Todo:                                                                                         *
 *************************************************************************************************/
static MT_U8 _DtvCC_DetectRstDcCmd(MT_U8 *pu8buf, MT_U8 u8BufLen, MT_U8 *pu8Cmd)
{
    MT_U8 *pu8Service = NULL;
    MT_U8 u8ServiceLen,u8Offset = 0;
    MT_U8 u8ServiceData;
 ...
 *******************************************************************************************/
static MT_VOID _DtvCC_PutServiceDataToFIFO( MT_U8 *pu8Buf,MT_U8 u8BufLen)
{
    ...
    if (u8BufLen > 0)
    {
        ...
        {
            MT_INFO_CC("ServiceQueue full,reset delay command \n");
            pstCCObj->u8IsDelay = 0;
            _DtvCC_DelayTimerStop(0);
        }
    }
...
static MT_S32 _DtvCC_CheckSvcBlockDataIntegrality(MT_U8 *pu8buf,MT_U8 u8BufSize)
{
    MT_S32 i;
    MT_U8 u8Data;
...
/***************************************************************************
 * function name: _DtvCC_ParsePacketData(MT_U8 *pPacketBuf, MT_U8 u8BufLen)         *
 * description:   Service Block Parse Routine                              *
 *                Put the specific block_data to cc708 queue               *
 ***************************************************************************/
static MT_S32 _DtvCC_ParsePacketData(MT_U8 *pu8PacketBuf, MT_U8 u8BufLen)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);
    MT_U8 u8ServiceSize    = 0;
    MT_U8 u8HeaderSize,u8ServiceNo;
    MT_U8 u8Offset         = 0;
    ...
    {
        MT_ERR_CC("service block null!\n");
        return MT_FAILURE;
    }

    while( u8Offset < u8BufLen )
    {
        if(pstCCObj->bServiceBlockEnded == MT_TRUE)
        {
            ...
            {
                MT_INFO_CC("Block extends past the end of packet!Reset!\n");
                (MT_VOID)CC708_DEC_Reset(0);
                break;
            }
            ...
            pstCCObj->bServiceBlockEnded = MT_FALSE;
        ...
        if((pServiceBlock->u8ServiceNo == pstCCObj->u32ServiceType) && CC708_IsStart(0))
        {
            ...
            else              /*find a  servive block in a packet*/
            {
                ...
                pstCCObj->bServiceBlockEnded = MT_TRUE;
            }
        }
...
static MT_VOID _DtvCC_ParsePacket(MT_U8 *pu8PacketBuf, MT_U8 u8PacketLen)
{
    if( NULL == pu8PacketBuf || u8PacketLen == 0 )
    {
        MT_ERR_CC("packet null or lenght zero!\n");
        return ;
    }
...
static inline MT_VOID _DtvCC_EndPacket(MT_VOID)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);
    ...
    {
        memset(pstPacket,0,sizeof(DTVCC_Packet_S));
        pstPacket->bStart = MT_FALSE;
...
static MT_VOID _DtvCC_CheckPacketSequenceNo(MT_VOID)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);
    DTVCC_Packet_S *pstPacket = &pstCCObj->stPacket;
    ...
    {
        if( pstPacket->u8SeqenceNo != ((MT_U8)(pstCCObj->s32LastSeqenceNo + 1) & 0x03 ) )
        {
...
static MT_VOID _DtvCC_ParsePicUsrData_608 ( MT_BOOL bCheckATSC608, MT_U8 u8NtscCounter, MT_U8 *pau8NtscData )
{
    if( (u8NtscCounter > 0) && (u8NtscCounter < (MAX_VBI_DATA_COUNT - 1)) && bCheckATSC608 && CC608_IsStart(0))
    {
        ...
        {
            //copy to buff
            buff[0] = pau8NtscData[loop];   //field
            buff[1] = pau8NtscData[loop+1]; //ccdata1
            buff[2] = pau8NtscData[loop+2]; //ccdata2
 ...
 **********************************************************************/
static MT_S32 _DtvCC_ParsePicUsrData( MT_U8 *pu8CCPicData, MT_U8 u8CCdataLength, MT_BOOL bCheckATSC608)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);
    ...
    MT_U8 au8NtscData[MAX_VBI_DATA_COUNT] = {0};
    MT_U8 u8NtscCounter = 0;

    if( (NULL == pu8CCPicData) || (u8CCdataLength == 0) )
    {
        MT_ERR_CC("Param invalid!\n");
        return MT_FAILURE;
    }

    MT_UNF_CC_VBI_DADA_S stVBIDataField1, stVBIDataField2;
        ...
        switch(u8CCFlag)
        {
...
#if 0 //gavin.s,these code are exist some bug,cause exception.           
            ...
            case PACKET_CC608_FIELD2:
            {
                ...
                while(u8LoopTimes-- > 0)
                {
                    ...
                    {
                        MT_ERR_CC("u8offset:%d  >= u8CCdataLength:%d\n",u8offset,u8CCdataLength);
                        return MT_FAILURE;
                    }
    ...
    if( u8NtscCounter > 0)
    {
          (MT_VOID)CCDISP_SendVbiData(&stVBIDataField1,&stVBIDataField2);
    }

    /*one user data ,parse one time 608 data*/
    (MT_VOID)_DtvCC_ParsePicUsrData_608 ( bCheckATSC608, u8NtscCounter, &au8NtscData[0] );

    return MT_SUCCESS;
}
MT_S32 CC708_DtvCC_ParsePicUsrData( MT_U8 *pu8CCPicData, MT_U8 u8CCdataLength, MT_BOOL bCheckATSC608)
{
    return _DtvCC_ParsePicUsrData(pu8CCPicData,u8CCdataLength,bCheckATSC608);
}

#if CC608_FROM_CABLE
static MT_U8 ReverseChar(MT_U8 Data)
{
    MT_U8 b0,b1,b2,b3,b4,b5,b6,b7;
    MT_U8  Reversed;
...
{
    s->pu8Start = (MT_U8 *)p_data;
    s->pu8Buf       = (MT_U8 *)p_data;
    s->pu8End   = s->pu8Buf + i_data;
    s->u8LeftBits  = 8;
}


static inline MT_U8 _DtvCC_BitStreamRead( NTSC_BitStream_S *s, MT_U8 read_count )
{
    ...
    while( read_count > 0 )
    {
        ...
        else /* less in the buffer than requested */
        {
           ...
           i_result |= (unsigned int)((*s->pu8Buf&i_mask[s->u8LeftBits]) << (0-i_shr));
...
static MT_S32 _DtvCC_DecodeCABLE(MT_U8 *pBuf, MT_U8 BufLen, MT_BOOL bTopFieldFirst)
{
    MT_U8 cc_invalid = 1;
    MT_U8 vbi_data_flag = 0,field_number=0;
    MT_U8 cc_data1=0,cc_data2=0;
    MT_U8 ccdata1=0,ccdata2=0;
    MT_U8 ccCount = 0,i=0;
    MT_U8 pucCCData[3];
    NTSC_BitStream_S s;
    ...
    if(vbi_data_flag & 0x01)
    {
        ...
        for(i=0;i<ccCount;i++)
        {
            (MT_VOID)_DtvCC_BitStreamRead(&s,2);//priority: 2bits
            field_number = _DtvCC_BitStreamRead(&s,2);//field_number: 2bits
            (MT_VOID)_DtvCC_BitStreamRead(&s,5);//line_offset: 5bits
            cc_data1 = _DtvCC_BitStreamRead(&s,8);//cc1: 8bits
            cc_data2 = _DtvCC_BitStreamRead(&s,8);//cc2: 8bits
            (MT_VOID)_DtvCC_BitStreamRead(&s,1);//marker_bit: 1bits
...
static MT_S32 _DtvCC_DecodeSAUD(MT_U8 *pBuf, MT_U8 BufLen, MT_BOOL bCheckATSC608)
{
    MT_U8 saud_type = 0,offset = 0;
    MT_U8 cc_count = 0;
    MT_U8 i,field_num,ccdata1,ccdata2;
    MT_U8 au8608Data[3];
    ...
    {
        MT_ERR_CC("invalide cc_count:%d for saud data BufLen:%d,underflow\n",cc_count, BufLen);
        return MT_FAILURE;
    }
    for(i=0; i<cc_count; i++)
    {
        ...
        {
            (MT_VOID)CC608_VBIParse(0, au8608Data, 3);
        }
    }
    return MT_SUCCESS;
}
...
*********************************************************************************/
static MT_S32 _DtvCC_DecodeATSC(MT_U8 *pu8Buf, MT_U8 u8BufLen, MT_BOOL bCheckATSC608)
{
    MT_U8 u8Offset = 0;
    MT_U8 u8ProcessFlag = 0;
    MT_U8 u8DataType = 0;
    MT_U8 u8ccCount = 0;
    ...
    case USER_DATA_TYPE_CODE:    /*CC*/
        ...
        {
            MT_ERR_CC("Data len is less than cc data in stream!");
            return MT_FAILURE;
        }
        ...
        {
            MT_ERR_CC("Invalid userdata mask value:%#x !\n",pu8Buf[u8Offset + u8ccCount*3 ]);
        }

        (MT_VOID)_DtvCC_ParsePicUsrData(&pu8Buf[u8Offset], u8ccCount*3, bCheckATSC608);
    ...
    case 0x04:
        ...
        break;
    default:
        MT_ERR_CC("User Data type not match: 0x%x\n",u8DataType);
        return MT_FAILURE;
    }
    return MT_SUCCESS;
}
...
*                    export Function
*****************************************************************************/
MT_S32 CC708_DEC_Start(MT_U8 u8ModuleID)
{
    stCaptionBuf.u8BufLen = 0;
...
MT_S32 CC708_DEC_Stop(MT_U8 u8ModuleID)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(u8ModuleID);
    if( pstCCObj == NULL )
    {
        MT_ERR_CC("Invalid module id!\n");
        return MT_FAILURE;
    }
...
MT_S32 CC708_DEC_Reset(MT_U8 u8ModuleID)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(u8ModuleID);
    if( pstCCObj == NULL )
    {
...
MT_S32 CC708_DEC_UserDataParse(MT_U8 *pu8UserData, MT_U32 u32DataLen, MT_BOOL bTopFieldFirst)
{
    MT_S32 s32Ret = 0;
    MT_U32 u32offset = 0;
    MT_U32 u32DataType = 0;
    MT_U8 *pu8Buf = pu8UserData;
    MT_BOOL bCheckATSC608 = MT_TRUE;

    if(MT_NULL == pu8UserData || 0 == u32DataLen )
    {
        MT_ERR_CC("\nParam invalid\n");
        return MT_FAILURE;
    }
    ...
    while (u32offset < u32DataLen)
    {
        ...
        {
            u32offset++;
            s32Ret = _DtvCC_DecodeCABLE( &pu8UserData[u32offset],(MT_U8)(u32DataLen-u32offset), bTopFieldFirst);
            if(1 == s32Ret)
            {
                bCheckATSC608 = MT_FALSE;
            }
        ...
        {
            s32Ret = _DtvCC_DecodeATSC( &pu8UserData[u32offset],(MT_U8)(u32DataLen-u32offset),bCheckATSC608);
        ...
        {
            s32Ret = _DtvCC_DecodeSAUD(&pu8UserData[u32offset],(MT_U8)(u32DataLen-u32offset),bCheckATSC608);
...
MT_S32 CC708_DEC_ProcessServicefifo(MT_VOID)
{
    ...
    while(!CCQueue_IsEmpty(pstServiceQueue) && !pstCCObj->u8IsDelay)
    {
        if (CCQueue_Get(pstServiceQueue, &stSvsElement))
        {
            ...
            if(MT_FAILURE == s32Ret)
            {
                MT_ERR_CC("_DtvCC_ParseServiceBlock failed!\n");
            }
...
static void _DtvCC_NulCmd(MT_U8 *pu8Cmd)
{

}

static void _DtvCC_ETXCmd(MT_U8 *pu8Cmd)
{
...
static void _DtvCC_BSCmd(MT_U8 *pu8Cmd)
{
    DEC_TRACE("C0_BS command! %#x\n",*pu8Cmd);
    CC708_OSD_BSText();
}

static void _DtvCC_FFCmd(MT_U8 *pu8Cmd)
{
    DEC_TRACE("C0_ff command!%#x\n",*pu8Cmd);
    CC708_OSD_FFText();
}

static void _DtvCC_CRCmd(MT_U8 *pu8Cmd)
{
    DEC_TRACE("C0_CR  %#x\n",*pu8Cmd);
    CC708_OSD_CRText();
}

static void _DtvCC_HCRCmd(MT_U8 *pu8Cmd)
{
    DEC_TRACE("CO_HCR command %#x\n",*pu8Cmd);
    CC708_OSD_HCRText();
}

static void _DtvCC_EXT1Cmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8Byte;
    MT_U16 u16Caption = 0;
...
static const MT_U16 ksc5601_2uni_page21[1115] = {
  /* 0x21 */
...
static const MT_U16 ksc5601_2uni_page30[2350] = {
  /* 0x30 */
...
static const MT_U16 ksc5601_2uni_page4a[4888] = {
  /* 0x4a */
...
/*******************************************/
static void _DtvCC_P16Cmd(MT_U8 *pu8Cmd)
{
    MT_U16 u16Unicode;
    MT_U8 u8Char1,u8Char2;
    ...
    if(pu8Cmd[1] >= 0xa1)
    {
            ...
            || (u8Char1 >= 0x4a && u8Char1 <= 0x7d))
        {
            ...
            {
                MT_S32 i = 94 * (u8Char1 - 0x21) + (u8Char2 - 0x21);
                if (i < 1410)
                {
                    if (i < 1115)
                    {
    ...
    {
        MT_ERR_CC("p16 code data : %#x not support\n",pu8Cmd[1]);
    }
    DEC_TRACE("--->%#x\n",u16Unicode);
...
static void _DtvCC_SetCurrentWindowCmd(MT_U8 *pu8Cmd)
{
    MT_U8 windowID;
    if(NULL == pu8Cmd)
    {
        MT_ERR_CC("CMD is null!\n");
        return ;
    }
...
static void _DtvCC_ClearWindowsCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8WindowMap;
...
static void _DtvCC_DisplayWindowsCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8WindowMap;
...
static void _DtvCC_HideWindowsCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8WindowMap;
...
static void _DtvCC_ToggleWindowsCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8WindowMap;
...
static void _DtvCC_DeleteWindowsCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8WindowMap;
...
static void _DtvCC_DelayCmd(MT_U8 *pu8Cmd)
{
    MT_U8 tenthOfSecond;
    CC708_OBJECT_S *pstCCObj= CC708_OBJ_GetHandle(0);
...
static void _DtvCC_ResetCmd(MT_U8 *pu8Cmd)
{
...
static void _DtvCC_SetPenAttributesCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8PenSize,u8FontName,u8TextTag,u8TextOffset,u8IsItalics,u8IsUnderline,u8EdgeType;
    MT_U8 u8Offset = 1;
    ...
    u8PenSize = GET2(pu8Cmd,u8Offset,0) + MT_UNF_CC_FONTSIZE_SMALL;
    CC708DECPARAM_UPPER_CHECK(u8PenSize,MT_UNF_CC_FONTSIZE_BUTT,"Pen Size",MT_UNF_CC_FONTSIZE_STANDARD);
    ...
    u8FontName = GET3(pu8Cmd,u8Offset,0) + MT_UNF_CC_FN_DEFAULT;
    u8EdgeType = GET3(pu8Cmd,u8Offset,3) + MT_UNF_CC_EDGETYPE_NONE;
    CC708DECPARAM_UPPER_CHECK(u8EdgeType,MT_UNF_CC_EDGETYPE_BUTT,"Edge Type",MT_UNF_CC_EDGETYPE_NONE);
...
static void _DtvCC_SetPenColorCmd(MT_U8 *pu8Cmd)
{
    MT_U8 u8FGColor,u8FGOpacity,u8BGColor,u8BGOpacity,u8EdgeColor;
    ...
    u8FGOpacity = GET2(pu8Cmd,u8Offset,6) + MT_UNF_CC_OPACITY_SOLID;//according with mt_unf_cc.h/MT_UNF_CC_OPACITY_E
    u8Offset++;

    u8BGColor = GET6(pu8Cmd,u8Offset,0);
    u32BGColor = _DtvCC_Translate2Color(u8BGColor);// get r,g,b value
    u8BGOpacity = GET2(pu8Cmd,u8Offset,6) + MT_UNF_CC_OPACITY_SOLID;
    u8Offset++;
...
static void _DtvCC_SetPenLocationCmd(MT_U8 *pu8Cmd)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);
    MT_U8 u8Row,u8Column,u8Offset = 1;
...
static void _DtvCC_SetWindowAttributesCmd(MT_U8 *pu8Cmd)
{

    MT_U8 u8Justify,u8PrintDirection,u8ScrollDirection,u8Temp;
    MT_U8 u8WordWrap,u8DisplayEffect,u8EffectDirection,u8EffectSpeed;
    MT_U8 u8FillColor,u8FillOpacity,u8BorderType,u8BorderColor,u8Offset = 1;
    MT_U32 u32FillColor,u32BorderColor;
    ...
    u8FillOpacity += MT_UNF_CC_OPACITY_SOLID;/*here we map the opacity in ts stream to unf opacity*/
    u8Offset++;
    ...
    u8Justify += MT_UNF_CC_JUSTIFY_LEFT;
    ...
    u8WordWrap += MT_UNF_CC_WW_DISABLE;

    u8Temp = GET1(pu8Cmd,u8Offset,7);
    u8BorderType = (u8Temp << 2) | u8BorderType;
    u8BorderType += MT_UNF_CC_EDGETYPE_NONE;
    u8Offset++;
...
static void _DtvCC_DefineWindowCmd(MT_U8 *pu8Cmd)
{
    CC708_OBJECT_S *pstCCObj = CC708_OBJ_GetHandle(0);

    MT_U8 u8WindowID,u8Priority,u8AnchorPoint,u8RelativePositioning,u8AnchorVertical;
    MT_U8 u8AnchorHorizontal,u8RowCount,u8ColumnCount,u8RowLock,u8ColumnLock;
    MT_U8 u8IsVisible,u8WindowStyleID,u8PenStyleID;
    MT_U8 u8Offset = 0;
    ...
    #if 0
    if(u8ColumnCount > MAX_CC_COLUMNS_4B3) //gavin.s debug 
    {
        u8ColumnCount = MAX_CC_COLUMNS_4B3;
    }
    #endif
...
static MT_BOOL _DtvCC_OddParityCheck(MT_U8 data)
{
    ...
    {
        return MT_FALSE;   //faied the odd parity check
    }

    return MT_TRUE; //odd parity check ok.
}
