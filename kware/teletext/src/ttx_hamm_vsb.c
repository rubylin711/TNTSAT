/********************************************************************************************/
/* Montage Technology (Shanghai) Co., Ltd.                                                  */
/* Montage Proprietary and Confidential                                                     */
/* Copyright (c) 2014 Montage Technology Group Limited and its affiliated companies         */
/********************************************************************************************/
#include "mt_type.h"
#include "sys_define.h"

#include "ttx_hamm_vsb.h"

const u8 _vbi_bit_reverse_vsb[256] =
{
    
};

const u8 _vbi_hamm8_fwd_vsb[16] =
{
    0x15, 0x02, 0x49, 0x5e, 0x64, 0x73, 0x38, 0x2f,
    0xd0, 0xc7, 0x8c, 0x9b, 0xa1, 0xb6, 0xfd, 0xea
};

const s8 _vbi_hamm8_inv_vsb[256] =
{
    0x01, 0xff, 0x01, 0x01, 0xff, 0x00, 0x01, 0xff,
    0xff, 0x02, 0x01, 0xff, 0x0a, 0xff, 0xff, 0x07,
    0xff, 0x00, 0x01, 0xff, 0x00, 0x00, 0xff, 0x00,    /*    1    */
    0x06, 0xff, 0xff, 0x0b, 0xff, 0x00, 0x03, 0xff,
    0xff, 0x0c, 0x01, 0xff, 0x04, 0xff, 0xff, 0x07, /*    2    */
    0x06, 0xff, 0xff, 0x07, 0xff, 0x07, 0x07, 0x07,
    0x06, 0xff, 0xff, 0x05, 0xff, 0x00, 0x0d, 0xff,    /*    3    */
    0x06, 0x06, 0x06, 0xff, 0x06, 0xff, 0xff, 0x07,
    0xff, 0x02, 0x01, 0xff, 0x04, 0xff, 0xff, 0x09,    /*    4    */
    0x02, 0x02, 0xff, 0x02, 0xff, 0x02, 0x03, 0xff,
    0x08, 0xff, 0xff, 0x05, 0xff, 0x00, 0x03, 0xff,    /*    5    */
    0xff, 0x02, 0x03, 0xff, 0x03, 0xff, 0x03, 0x03,
    0x04, 0xff, 0xff, 0x05, 0x04, 0x04, 0x04, 0xff, /*    6    */
    0xff, 0x02, 0x0f, 0xff, 0x04, 0xff, 0xff, 0x07,
    0xff, 0x05, 0x05, 0x05, 0x04, 0xff, 0xff, 0x05, /*    7    */
    0x06, 0xff, 0xff, 0x05, 0xff, 0x0e, 0x03, 0xff,
    0xff, 0x0c, 0x01, 0xff, 0x0a, 0xff, 0xff, 0x09,    /*    8    */
    0x0a, 0xff, 0xff, 0x0b, 0x0a, 0x0a, 0x0a, 0xff,
    0x08, 0xff, 0xff, 0x0b, 0xff, 0x00, 0x0d, 0xff, /*    9    */
    0xff, 0x0b, 0x0b, 0x0b, 0x0a, 0xff, 0xff, 0x0b,
    0x0c, 0x0c, 0xff, 0x0c, 0xff, 0x0c, 0x0d, 0xff, /*    A    */
    0xff, 0x0c, 0x0f, 0xff, 0x0a, 0xff, 0xff, 0x07,
    0xff, 0x0c, 0x0d, 0xff, 0x0d, 0xff, 0x0d, 0x0d, /*    B    */
    0x06, 0xff, 0xff, 0x0b, 0xff, 0x0e, 0x0d, 0xff,
    0x08, 0xff, 0xff, 0x09, 0xff, 0x09, 0x09, 0x09, /*    C    */
    0xff, 0x02, 0x0f, 0xff, 0x0a, 0xff, 0xff, 0x09,
    0x08, 0x08, 0x08, 0xff, 0x08, 0xff, 0xff, 0x09, /*    D    */
    0x08, 0xff, 0xff, 0x0b, 0xff, 0x0e, 0x03, 0xff,
    0xff, 0x0c, 0x0f, 0xff, 0x04, 0xff, 0xff, 0x09, /*    E    */
    0x0f, 0xff, 0x0f, 0x0f, 0xff, 0x0e, 0x0f, 0xff,
    0x08, 0xff, 0xff, 0x05, 0xff, 0x0e, 0x0d, 0xff, /*    F    */
    0xff, 0x0e, 0x0f, 0xff, 0x0e, 0x0e, 0xff, 0x0e
};

const u8 _vbi_hamm24_fwd_0_vsb[256] =
{
    
};

const u8 _vbi_hamm24_fwd_1_vsb[256] =
{
    
};

const u8 _vbi_hamm24_fwd_2_vsb[4] =
{
    0x00, 0x0a, 0x0b, 0x01
};

const s8 _vbi_hamm24_inv_par_vsb[3][256] =
{
    {
        
    },
    {
        
    },
    {
        
    }
};

const u8 _vbi_hamm24_inv_d1_d4_vsb[64] =
{
    0x00, 0x01, 0x00, 0x01, 0x02, 0x03, 0x02, 0x03,
    0x04, 0x05, 0x04, 0x05, 0x06, 0x07, 0x06, 0x07,
    0x08, 0x09, 0x08, 0x09, 0x0a, 0x0b, 0x0a, 0x0b,
    0x0c, 0x0d, 0x0c, 0x0d, 0x0e, 0x0f, 0x0e, 0x0f,
    0x00, 0x01, 0x00, 0x01, 0x02, 0x03, 0x02, 0x03,
    0x04, 0x05, 0x04, 0x05, 0x06, 0x07, 0x06, 0x07,
    0x08, 0x09, 0x08, 0x09, 0x0a, 0x0b, 0x0a, 0x0b,
    0x0c, 0x0d, 0x0c, 0x0d, 0x0e, 0x0f, 0x0e, 0x0f
};

const s32 _vbi_hamm24_inv_err_vsb[64] =
{
    0x00000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x00000000, 0x00000000, 0x00000000, 0x00000001,
    0x00000000, 0x00000002, 0x00000004, 0x00000008,
    0x00000000, 0x00000010, 0x00000020, 0x00000040,
    0x00000080, 0x00000100, 0x00000200, 0x00000400,
    0x00000000, 0x00000800, 0x00001000, 0x00002000,
    0x00004000, 0x00008000, 0x00010000, 0x00020000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000,
    0x80000000, 0x80000000, 0x80000000, 0x80000000
};

void vbi_par_vsb(u8 *p_p, u32 n)
{
    while(n-- > 0)
    {
        u8 c = *p_p;

        /* if 0 == (inv_par[] & 32) change msb of *p. */
        *p_p ++ = c ^ (128 & ~(_vbi_hamm24_inv_par_vsb[0][c] << 2));
    }
}

s32 vbi_unpar_vsb(u8 *p_p, u32 n)
{
    s32 r = 0;

    while(n -- > 0)
    {
        u8 c = *p_p;

        /* if 0 == (inv_par[] & 32) set msb of r. */
        r |= ~ _vbi_hamm24_inv_par_vsb[0][c] << (sizeof (s32) * 8 - 1 - 5);

        *p_p ++ = c & 127;
    }

    return r;
}

void vbi_ham24p_vsb(u8 *p_p, u32 c)
{
    u32 d5_d11  = 0;
    u32 d12_d18 = 0;
    u32 p5 = 0, p6 = 0;
    u32 byte_0  = 0;

    byte_0 = (_vbi_hamm24_fwd_0_vsb[(c >> 0) & 0xFF]
              ^ _vbi_hamm24_fwd_1_vsb[(c >> 8) & 0xFF]
              ^ _vbi_hamm24_fwd_2_vsb[(c >> 16) & 0x03]);
    p_p[0] = (u8)byte_0;

    d5_d11  = (c >> 4) & 0x7F;
    d12_d18 = (c >> 11) & 0x7F;

    p5   = 0x80 & ~(_vbi_hamm24_inv_par_vsb[0][d12_d18] << 2);
    p_p[1] = (u8)(d5_d11 | p5);
    p6   = 0x80 & ((_vbi_hamm24_inv_par_vsb[0][byte_0]
                    ^ _vbi_hamm24_inv_par_vsb[0][d5_d11]) << 2);
    p_p[2] = (u8)(d12_d18 | p6);
}

s32 vbi_unham24p_vsb(const u8 *p_p)
{
    u32 d1_d4 = 0;
    u32 d5_d11 = 0;
    u32 d12_d18 = 0;
    u32 ABCDEF = 0;
    s32 d = 0;

    d1_d4   = _vbi_hamm24_inv_d1_d4_vsb[p_p[0] >> 2];
    d5_d11  = p_p[1] & 0x7F;
    d12_d18 = p_p[2] & 0x7F;

    d = d1_d4 | (d5_d11 << 4) | (d12_d18 << 11);

    ABCDEF = (_vbi_hamm24_inv_par_vsb[0][p_p[0]]
              ^ _vbi_hamm24_inv_par_vsb[1][p_p[1]]
              ^ _vbi_hamm24_inv_par_vsb[2][p_p[2]]);

    /* Correct single bit error, set MSB on double bit error. */
    return d ^ (s32) _vbi_hamm24_inv_err_vsb[ABCDEF];
}