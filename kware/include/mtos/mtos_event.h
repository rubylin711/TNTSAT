...
/*!
  The value to indicate event flag waiting will return  immediately if no flag set
  */
#define MTOS_WAIT_NO_WAIT  (0)

/*!
  Define the event type
  */
typedef unsigned long mtos_evt_t;
...
/*!
  clear specified bit(s) in Event Set.

  \param[in] p_event The pointer to the caller provide space, used to store handle of Event Set.
  \param[in] mask This is the bit(s) pattern want to be clear.

  \return TRUE if OK, Others if fail
  */
MT_BOOL mtos_event_clear(mtos_evt_t *p_event, unsigned int mask);
