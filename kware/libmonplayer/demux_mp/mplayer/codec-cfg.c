...
static int add_comment(char *s, char **d)
{
    ...
    if (!(*d = realloc33(*d, pos + strlen(s) + 1))) {
...
int parse_codec_cfg(const char *cfgfile)
{
    ...
    if ((line = malloc33(MAX_LINE_LEN + 1)) == NULL) {
    ...
    while ((tmp = get_token(1, 1)) != RET_EOF) {
        ...
        loop_enter:
            ...
            if (!(codec->name = strdup33(token[0]))) {
            ...
            if (codec->info || get_token(1, 1) < 0)
                goto err_out_parse_error;
            if (!(codec->info = strdup33(token[0]))) {
...
static void codecs_free33(codecs_t* codecs,int count) {
    int i;
    for ( i = 0; i < count; i++)
        if ( codecs[i].name ) {
            free33(codecs[i].name);
            free33(codecs[i].info);
            free33(codecs[i].comment);
            free33(codecs[i].dll);
            free33(codecs[i].drv);
        }
    free33(codecs);
...
void codecs_uninit_free33(void) {
    if (video_codecs)
    codecs_free33(video_codecs,nr_vcodecs);
    video_codecs=NULL;
    if (audio_codecs)
    codecs_free33(audio_codecs,nr_acodecs);
    audio_codecs=NULL;
}
...
void stringset_init(stringset_t *set) {
    *set = calloc33(1, sizeof(char *));
}

void stringset_free33(stringset_t *set) {
    int count = 0;
    while ((*set)[count]) free33((*set)[count++]);
    free33(*set);
    *set = NULL;
}
...
void stringset_add(stringset_t *set, const char *str) {
    ...
    count++;
    *set = realloc33(*set, sizeof(char *) * (count + 1));if(*set == NULL) return;
    (*set)[count - 1] = strdup33(str);
    (*set)[count] = NULL;
}
