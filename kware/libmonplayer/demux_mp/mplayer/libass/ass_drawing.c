                                     ...
                                     FT_Vector *point)
{
    ...
    if (ol->n_points >= drawing->max_points) {
        ...
        ol->points = (FT_Vector*)realloc33(ol->points, sizeof(FT_Vector) *
                             drawing->max_points);
        ol->tags = (char*)realloc33(ol->tags, drawing->max_points);
    }
    ...
    while (*p) {
        if (*p == 'c' && spline_start) {
            ...
            if (token_check_values(spline_start->next, 2, TOKEN_B_SPLINE)) {
                for (i = 0; i < 3; i++) {
                    tail->next = (struct ass_drawing_token *)calloc33(1, sizeof(ASS_DrawingToken));
                    tail->next->prev = tail;
                    tail = tail->next;
        ...
        if (type != -1 && is_set == 2) {
            if (root) {
                ...
                root = tail = (struct ass_drawing_token *)calloc33(1, sizeof(ASS_DrawingToken));
            tail->type = type;
            tail->point = point;
            is_set = 0;
...
ASS_Drawing *ass_drawing_new(ASS_Library *lib, FT_Library ftlib)
{
    ...
    drawing = (ASS_Drawing *)calloc33(1, sizeof(*drawing));
    drawing->text = (char *)calloc33(1, DRAWING_INITIAL_SIZE);
 ...
 */
void ass_drawing_free33(ASS_Drawing* drawing)
{
    if (drawing) {
        free33(drawing->text);
...
void ass_drawing_add_char(ASS_Drawing* drawing, char symbol)
{
    ...
    if (drawing->i + 1 >= drawing->size) {
        drawing->size *= 2;
        drawing->text = (char *)realloc33(drawing->text, drawing->size);
    }
}
