...
#ifdef CONFIG_FRIBIDI
        shaper->event_text = realloc33(shaper->event_text, sizeof(FriBidiChar) * new_size);
        shaper->ctypes     = realloc33(shaper->ctypes, sizeof(FriBidiCharType) * new_size);
        shaper->emblevels  = realloc33(shaper->emblevels, sizeof(FriBidiLevel) * new_size);
#endif
        shaper->cmap       = (FriBidiStrIndex *)realloc33(shaper->cmap, sizeof(FriBidiStrIndex) * new_size);
    }
}
 ...
 */
void ass_shaper_free33(ASS_Shaper *shaper)
{
#ifdef CONFIG_HARFBUZZ
    ass_cache_done(shaper->metrics_cache);
    free33(shaper->features);
#endif
#ifdef CONFIG_FRIBIDI
    free33(shaper->event_text);
    free33(shaper->ctypes);
    free33(shaper->emblevels);
#endif
    free33(shaper->cmap);
    free33(shaper);
}

void ass_shaper_font_data_free33(ASS_ShaperFontData *priv)
{
#ifdef CONFIG_HARFBUZZ
    int i;
    for (i = 0; i < ASS_FONT_MAX_FACES; i++)
        if (priv->fonts[i]) {
            free33(priv->metrics_data[i]);
...
{
    shaper->features = calloc33(sizeof(hb_feature_t), NUM_FEATURES);
...
static hb_font_t *get_hb_font(ASS_Shaper *shaper, GlyphInfo *info)
{
    ...
    if (!font->shaper_priv)
        font->shaper_priv = calloc33(sizeof(ASS_ShaperFontData), 1);
    ...
    if (!hb_fonts[info->face_index]) {
        ...
        font->shaper_priv->metrics_data[info->face_index] =
            calloc33(sizeof(struct ass_shaper_metrics_data), 1);
    ...
    for (i = 0; i < run; i++) {
        ...
        for (j = 0; j < num_glyphs; j++) {
            ...
            if (info->skip == 0) {
                while (info->next)
                    info = info->next;
                info->next = malloc33(sizeof(GlyphInfo));
...
{
    int i;
    FriBidiJoiningType *joins = calloc33(sizeof(*joins), len);
...
{
    ASS_Shaper *shaper = (ASS_Shaper *)calloc33(sizeof(*shaper), 1);
