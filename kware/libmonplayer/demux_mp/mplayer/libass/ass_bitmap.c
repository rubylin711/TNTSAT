...
static int generate_tables(ASS_SynthPriv *priv, double radius)
{
    ...
    if (priv->g_r) {
        priv->g = realloc33(priv->g, priv->g_w * sizeof(unsigned));
        priv->gt2 = realloc33(priv->gt2, 256 * priv->g_w * sizeof(unsigned));
...
static void resize_tmp(ASS_SynthPriv *priv, int w, int h)
{
    ...
    while (priv->tmp_h < h)
        priv->tmp_h *= 2;
    free33(priv->tmp);
    priv->tmp = malloc33((priv->tmp_w + 1) * priv->tmp_h * sizeof(short));
}

ASS_SynthPriv *ass_synth_init(double radius)
{
    ASS_SynthPriv *priv = calloc33(1, sizeof(ASS_SynthPriv));
...
{
    Bitmap *bm;
    unsigned s = (unsigned)w; // XXX: alignment
    bm = malloc33(sizeof(Bitmap));
    bm->buffer = calloc33(s, h);
    bm->w = w;
    bm->h = h;
    bm->stride = s;
    bm->left = bm->top = 0;
