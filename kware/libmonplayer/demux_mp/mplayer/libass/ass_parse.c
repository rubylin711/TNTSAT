...
void update_font(ASS_Renderer *render_priv)
{
    ...
    if (render_priv->state.family[0] == '@') {
        desc.vertical = 1;
        desc.family = strdup33(render_priv->state.family + 1);
    } else {
        desc.vertical = 0;
        desc.family = strdup33(render_priv->state.family);
    }

    val = render_priv->state.bold;
    // 0 = normal, 1 = bold, >1 = exact weight
    if (val == 1 /*|| val == -1*/)  // for tscan
        val = 200;              // bold
    else if (/*val <= 0*/ val == 0)          // for tscan
        val = 80;               // normal
    desc.bold = val;
...
char *parse_tag(ASS_Renderer *render_priv, char *p, double pwr)
{
    ...
    if (mystrcmp(&p, "xbord")) {
        ...
        skip_to('\\');
        if (p > start) {
            family = malloc33(p - start + 1);
            strncpy(family, start, p - start);
            family[p - start] = '\0';
        } else
            family = strdup33(render_priv->state.style->FontName);
