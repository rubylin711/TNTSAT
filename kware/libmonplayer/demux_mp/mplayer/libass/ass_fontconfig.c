                          ...
                          uint32_t code)
{
    ...
    *index = r_index;
    ...
    if (result != FcResultMatch)
        goto error;
    retval = strdup33((const char *) r_file);
                        ...
                        uint32_t code)
{
    ...
    if (!priv->config) {
        *index = priv->index_default;
        res = priv->path_default ? strdup33(priv->path_default) : 0;
        return res;
    }
    if (family && *family)
        res =
    ...
    if (!res && priv->path_default) {
        res = strdup33(priv->path_default);
        *index = priv->index_default;
...
{
    int rc;
    FCInstance *priv = calloc33(1, sizeof(FCInstance));
    ...
    priv->family_default = family ? strdup33(family) : NULL;
exit:
    priv->path_default = path ? strdup33(path) : NULL;
...
{
    *index = priv->index_default;
    char* res = priv->path_default ? (char*)strdup33(priv->path_default) : 0;
    return res;
}
                            ...
                            int update)
{
    ...
    priv = (FCInstance *)calloc33(1, sizeof(FCInstance));

    priv->path_default = path ? (char *)strdup33(path) : 0;
    priv->index_default = 0;
    return priv;
}
...
void fontconfig_done(FCInstance *priv)
{
    ...
    if (priv) {
        ...
        if (priv->config)
            FcConfigDestroy(priv->config);
#endif
        free33(priv->path_default);
        free33(priv->family_default);
    }
    free33(priv);
}
