...
void ass_free_track(ASS_Track *track)
{
    ...
    if (track->parser_priv) {
        free33(track->parser_priv->fontname);
        free33(track->parser_priv->fontdata);
        free33(track->parser_priv);
    }
    free33(track->style_format);
    free33(track->event_format);
...
int ass_alloc_style(ASS_Track *track)
{
    ...
    if (track->n_styles == track->max_styles) {
        ...
        track->styles =
            (ASS_Style *) realloc33(track->styles,
                                  sizeof(ASS_Style) *
                                  track->max_styles);
    }
...
int ass_alloc_event(ASS_Track *track)
{
    ...
    if (track->n_events == track->max_events) {
        ...
        track->events =
            (ASS_Event *) realloc33(track->events,
                                  sizeof(ASS_Event) *
                                  track->max_events);
    }
...
{
    style->Name             = (char *)strdup33("Default");
    style->FontName         = (char *)strdup33("Arial");
    style->FontSize         = 18;
...
static long long string2timecode(ASS_Library *library, char *p)
{
    ...
    tm = (long long) (((h * 60 + m) * 60 + s) * 1000 + ms * 10);
    return tm;
}
...
#define STRVAL(name) \
		...
		if (target->name != NULL) free33(target->name); \
		target->name = (char *)strdup33(token); \
		ass_msg(track->library, MSGL_DBG2, "%s = %s", #name, token);
                              ...
                              char *str, int n_ignored)
{
    ...
    char *format = (char *)strdup33(track->event_format);
    ...
    while (1) {
        ...
        if (strcasecmp(tname, "Text") == 0) {
            char *last;
            event->Text = (char *)strdup33(p);
            if (event->Text && *event->Text != 0) {
                last = event->Text + strlen(event->Text) - 1;
                if (last >= event->Text && *last == '\r')
                    *last = 0;
            }
            if(event->Text) ass_msg(track->library, MSGL_DBG2, "Text = %s", event->Text);
...
static int process_style(ASS_Track *track, char *str)
{
    ...
    q = format = (char *)strdup33(track->style_format);
    ...
    while (1) {
        ...
        if (0) {                // cool ;)
            STRVAL(Name)
            if ((target->Name)&&((strcmp(target->Name, "Default") == 0)
...
static int process_styles_line(ASS_Track *track, char *str)
{
    if (!strncmp(str, "Format:", 7)) {
        char *p = str + 7;
        skip_spaces(&p);
        track->style_format = (char *)strdup33(p);
...
static int process_info_line(ASS_Track *track, char *str)
{
    if (!strncmp(str, "PlayResX:", 9)) {
        ...
        char *p = str + 9;
        while (*p && isspace(*p)) p++;
        track->Language = (char *)malloc33(3);
...
static void event_format_fallback(ASS_Track *track)
{
    ...
    if (track->track_type == TRACK_TYPE_SSA)
        track->event_format = (char *)strdup33("Format: Marked, Start, End, Style, "
            "Name, MarginL, MarginR, MarginV, Effect, Text");
    else
        track->event_format = (char *)strdup33("Format: Layer, Start, End, Style, "
...
static int process_events_line(ASS_Track *track, char *str)
{
    if (!strncmp(str, "Format:", 7)) {
        ...
        skip_spaces(&p);
        free33(track->event_format);
        track->event_format = (char *)strdup33(p);
...
static int decode_font(ASS_Track *track)
{
    ...
    buf = (unsigned char *)malloc33(size / 4 * 3 + 2);
    q = buf;
...
static int process_fonts_line(ASS_Track *track, char *str)
{
    ...
    if (!strncmp(str, "fontname:", 9)) {
        ...
        track->parser_priv->fontname = (char *)strdup33(p);
    ...
    if (track->parser_priv->fontdata_used + len >
        ...
        track->parser_priv->fontdata =
            (char *)realloc33(track->parser_priv->fontdata,
                    track->parser_priv->fontdata_size);
    }
        ...
        while (1) {
            ...
            if (rc == (size_t) (-1)) {
                if (errno == E2BIG) {
                    ...
                    outbuf = (char *) realloc33(outbuf, osize + size);
                    op = outbuf + offset;
                    osize += size;
                    oleft += size;
                } else {
                         ...
                         char *codepage)
{
    ...
    track->name = (char *)strdup33(fname);
...
{
    ASS_Track *track = (ASS_Track *)calloc33(1, sizeof(ASS_Track));
    track->library = library;
    track->ScaledBorderAndShadow = 1;
    track->parser_priv = (ASS_ParserPriv *)calloc33(1, sizeof(ASS_ParserPriv));
    return track;
}
