...
ASS_Renderer *ass_renderer_init(ASS_Library *library)
{
    ...
    priv = (ASS_Renderer *)calloc33(1, sizeof(ASS_Renderer));
    if (!priv) {
        FT_Done_FreeType(ft);
        goto ass_init_exit;
    }
    ...
    priv->text_info.glyphs = (GlyphInfo *)calloc33(MAX_GLYPHS_INITIAL, sizeof(GlyphInfo));
    priv->text_info.lines = (LineInfo *)calloc33(MAX_LINES_INITIAL, sizeof(LineInfo));
...
void ass_renderer_done(ASS_Renderer *render_priv)
{
    ...
    ass_shaper_free33(render_priv->shaper);
    free33(render_priv->eimg);
    free33(render_priv->text_info.glyphs);
    free33(render_priv->text_info.lines);

    free33(render_priv->settings.default_font);
    free33(render_priv->settings.default_family);
...
{
    ASS_Image *img = (ASS_Image *)malloc33(sizeof(ASS_Image));
...
static unsigned char *clone_bitmap_buffer(ASS_Image *img)
{
    ...
    img->bitmap = (unsigned char *)malloc33(size);
...
{
    if (!render_priv->free_head) {
        render_priv->free_head = (FreeList *)calloc33(1, sizeof(FreeList));
        render_priv->free_head->object = object;
        render_priv->free_tail = render_priv->free_head;
    } else {
        FreeList *l = (FreeList *)calloc33(1, sizeof(FreeList));
        l->object = object;
                              ...
                              ASS_Image *head)
{
    ...
    if (val) {
        ...
        // Add to cache
        memset(&v, 0, sizeof(v));
        key.u.clip.text = (char *)strdup33(drawing->text);
        v.bm = clip_bm;
...
blend_vector_error:
    ...
    for (cur = head; cur; cur = cur->next) {
        ...
        if (render_priv->state.clip_drawing_mode) {
            ...
            nbuffer = (unsigned char *)malloc33(as * ah);
            ...
            // Allocate new buffer and add to free list
            nbuffer = (unsigned char *)calloc33(as, ah);
            if (!nbuffer) goto blend_vector_exit;
            free_list_add(render_priv, nbuffer);
...
blend_vector_exit:
    ass_drawing_free33(render_priv->state.clip_drawing);
...
void reset_render_context(ASS_Renderer *render_priv, ASS_Style *style)
{
    ...
    free33(render_priv->state.family);
    render_priv->state.family = NULL;
    render_priv->state.family = (char *)strdup33(style->FontName);
...
init_render_context(ASS_Renderer *render_priv, ASS_Event *event)
{
    ...
    ass_drawing_free33(render_priv->state.drawing);
...
get_outline_glyph(ASS_Renderer *priv, GlyphInfo *info)
{
    ...
    if (!val) {
        ...
        if (info->drawing) {
            ...
            v.asc = drawing->asc;
            v.desc = drawing->desc;
            key.u.drawing.text = (char *)strdup33(drawing->text);
        } else {
        ...
        if (info->border_style == 3 &&
            ...
            v.border = (FT_Outline *)calloc33(1, sizeof(FT_Outline));
    ...
    ass_drawing_free33(info->drawing);
}
...
get_bitmap_glyph(ASS_Renderer *render_priv, GlyphInfo *info)
{
    ...
    if (!val) {
        ...
        outline_free33(render_priv->ftlibrary, outline);
        outline_free33(render_priv->ftlibrary, border);
    }
    ...
    for (i = 0; i < text_info->length; ++i) {
        ...
        if (break_at != -1) {
            ...
            if (text_info->n_lines >= text_info->max_lines) {
                ...
                text_info->lines = (LineInfo *)realloc33(text_info->lines,
                                           sizeof(LineInfo) *
                                           text_info->max_lines);
            }
    ...
    while (1) {
        ...
        if (text_info->length >= text_info->max_glyphs) {
            ...
            text_info->glyphs = glyphs =
                (GlyphInfo *)realloc33(text_info->glyphs,
                                       ...
                                       ASS_Event *event)
{
    if (!event->render_priv)
        event->render_priv = (ASS_RenderPriv *)calloc33(1, sizeof(ASS_RenderPriv));
...
{
    Segment *used = (Segment *)malloc33(cnt * sizeof(*used));
    int cnt_used = 0;
    int i, j;
    ...
    for (i = 0; i < track->n_events; ++i) {
        ...
        if ((event->Start <= now)
            ...
            if (cnt >= priv->eimg_size) {
                priv->eimg_size += 100;
                priv->eimg =
                    (EventImages *)realloc33(priv->eimg,
