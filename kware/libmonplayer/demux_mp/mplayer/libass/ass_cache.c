...
static void outline_destruct(void *key, void *value)
{
    ...
    OutlineHashKey *k = key;
    if (v->outline)
        outline_free33(v->lib, v->outline);
    if (v->border)
        outline_free33(v->lib, v->border);
    if (k->type == OUTLINE_DRAWING)
        free33(k->u.drawing.text);
    free33(key);
    free33(value);
}
...
{
    Cache *cache = calloc33(1, sizeof(*cache));
    ...
    cache->map = calloc33(cache->buckets, sizeof(CacheItem *));
...
void *ass_cache_put(Cache *cache, void *key, void *value)
{
    ...
    while (*item)
        item = &(*item)->next;
    (*item) = calloc33(1, sizeof(CacheItem));
    (*item)->key = malloc33(cache->key_size);
    (*item)->value = malloc33(cache->value_size);
