                       ...
                       ASS_FontDesc *desc)
{
    ...
    font.n_faces = 0;
    font.desc.family = (char *)strdup33(desc->family);
                                    ...
                                    FT_Glyph glyph, int under, int through)
{
    ...
    ol->points = (FT_Vector*)realloc33(ol->points, sizeof(FT_Vector) *
                         (ol->n_points + i));
    ol->tags = (char*)realloc33(ol->tags, ol->n_points + i); if(ol->tags == NULL) return 1;
    i = !!under + !!through;
    ol->contours = (short*)realloc33(ol->contours, sizeof(short) *
                           (ol->n_contours + i));if(ol->contours == NULL) return 1;
...
void outline_copy(FT_Library lib, FT_Outline *source, FT_Outline **dest)
{
    ...
    *dest = (FT_Outline *)calloc33(1, sizeof(**dest));

    FT_Outline_New(lib, source->n_points, source->n_contours, *dest);
    FT_Outline_Copy(source, *dest);
}

void outline_free33(FT_Library lib, FT_Outline *outline)
{
    if (outline)
 ...
 **/
void ass_font_free33(ASS_Font *font)
{
    int i;
    for (i = 0; i < font->n_faces; ++i)
        if (font->faces[i])
    ...
    if (font->shaper_priv)
        ass_shaper_font_data_free33(font->shaper_priv);
    free33(font->desc.family);
    free33(font);
}
...
void fix_freetype_stroker(FT_Outline *outline, int border_x, int border_y)
{
    ...
    int begin, stop;
    char modified = 0;
    char *valid_cont = (char *)malloc33(nc);
    int start = 0;
    int end = -1;
    FT_BBox *boxes = (FT_BBox *)malloc33(nc * sizeof(FT_BBox));
    int i, j;
