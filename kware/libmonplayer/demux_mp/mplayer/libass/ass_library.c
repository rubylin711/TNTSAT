...
{
    ASS_Library* lib = calloc33(1, sizeof(*lib));
...
{
    free33(priv->fonts_dir);

    priv->fonts_dir = fonts_dir ? strdup33(fonts_dir) : 0;
}
...
void ass_set_style_overrides(ASS_Library *priv, char **list)
{
    ...
    priv->style_overrides = malloc33((cnt + 1) * sizeof(char *));
...
void ass_add_font(ASS_Library *priv, char *name, char *data, int size)
{
    ...
    priv->fontdata[idx].name = strdup33(name);

    priv->fontdata[idx].data = malloc33(size);
...
void ass_clear_fonts(ASS_Library *priv)
{
    ...
    for (i = 0; i < priv->num_fontdata; ++i) {
        free33(priv->fontdata[i].name);
        free33(priv->fontdata[i].data);
    }
    free33(priv->fontdata);
    priv->fontdata = NULL;
    priv->num_fontdata = 0;
}
