...
void *xbuffer_init(int chunk_size) {
  uint8_t *data=calloc33(1,chunk_size+XBUFFER_HEADER_SIZE);
...
void *xbuffer_free33(void *buf) {
  if (!buf) {
    return NULL;
  }

  free33(((uint8_t*)buf)-XBUFFER_HEADER_SIZE);
...
void *xbuffer_ensure_size(void *buf, int size) {
  ...
  if (xbuf->size < size) {
    ...
    xbuf->size = new_size;
    buf = ((uint8_t*)realloc33(((uint8_t*)buf)-XBUFFER_HEADER_SIZE,
...
void *xbuffer_strcat(void *buf, char *data) {
  ...
  buf = xbuffer_ensure_size(buf, strlen(buf)+strlen(data)+1);if(buf == NULL) return NULL;
