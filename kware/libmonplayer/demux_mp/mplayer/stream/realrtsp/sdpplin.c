...
//        exit(1);
    ...
    out = xbuffer_ensure_size(out, k+4);if(out == NULL) return NULL;
    out[k++] = (b[0] << 2) | (b[1] >> 4);
    out[k++] = (b[1] << 4) | (b[2] >> 2);
    out[k++] = (b[2] << 6) | b[3];
    i = a[2] == '=' ? 1 : (a[3] == '=' ? 2 : 3);
...
static sdpplin_stream_t *sdpplin_parse_stream(char **data) {
  ...
  sdpplin_stream_t *desc=calloc33(1,sizeof(sdpplin_stream_t));
  ...
  if (filter(*data, "m=", &buf)) {
    desc->id = strdup33(buf);
  } else
  {
    printf("sdpplin: no m= found.\n");
    free33(desc);
    xbuffer_free33(buf);
    xbuffer_free33(decoded);
    return NULL;
  }
  *data=nl(*data);
  ...
  while (*data && **data && *data[0]!='m') {
    ...
    if(filter(*data,"a=StreamName:string;",&buf)) {
      desc->stream_name=strdup33(buf);
      if(desc->stream_name) desc->stream_name_size=strlen(desc->stream_name);
      handled=1;
      *data=nl(*data);
    }

    if(filter(*data,"a=mimetype:string;",&buf)) {
      desc->mime_type=strdup33(buf);
      if(desc->mime_type) desc->mime_type_size=strlen(desc->mime_type);
      handled=1;
      got_mimetype = 1;
      *data=nl(*data);
    }
    ...
    if(filter(*data,"a=OpaqueData:buffer;",&buf)) {
      ...
      desc->mlti_data=malloc33(desc->mlti_data_size);
      if(desc->mlti_data && decoded) memcpy(desc->mlti_data, decoded, desc->mlti_data_size);
      handled=1;
      *data=nl(*data);
#ifdef LOG
    ...
    if(filter(*data,"a=ASMRuleBook:string;",&buf)) {
      desc->asm_rule_book=strdup33(buf);
      handled=1;
      *data=nl(*data);
    }
  ...
  if (!got_mimetype) {
    ...
    desc->mime_type = strdup33("audio/x-pn-realaudio");
    if(desc->mime_type) desc->mime_type_size = strlen(desc->mime_type);
    if (desc->stream_id)
...
sdpplin_t *sdpplin_parse(char *data) {
  ...
  sdpplin_t        *desc=calloc33(1,sizeof(sdpplin_t));
  char             *buf=xbuffer_init(32);
  char             *decoded=xbuffer_init(32);
  int              handled;
  int              len;
  ...
  while (data && *data) {
    ...
    if (filter(data, "m=", &buf)) {
      ...
      free33(stream);
        ...
        if (stream->stream_id == 0) {
          desc->stream_count=1;
          desc->stream=malloc33(sizeof(sdpplin_stream_t*));
    ...
    if(filter(data,"a=Copyright:buffer;",&buf)) {
      decoded=b64_decode(buf, decoded, &len);
      desc->copyright=strdup33(decoded);
      handled=1;
      data=nl(data);
    }
    ...
    if(filter(data,"a=Abstract:buffer;",&buf)) {
      decoded=b64_decode(buf, decoded, &len);
      desc->abstract=strdup33(decoded);
      handled=1;
      data=nl(data);
    }
    ...
    if(filter(data,"a=StreamCount:integer;",&buf)) {
      ...
      desc->stream=calloc33(desc->stream_count, sizeof(sdpplin_stream_t*));
...
void sdpplin_free33(sdpplin_t *description) {
  ...
  for (i = 0; i < description->stream_count; i++) {
    if (description->stream[i]) {
      free33(description->stream[i]->stream_name);
      free33(description->stream[i]->mime_type);
      free33(description->stream[i]->mlti_data);
      free33(description->stream[i]->asm_rule_book);
      free33(description->stream[i]->id);
      free33(description->stream[i]);
    }
  }

  if(description->stream_count)
    free33(description->stream);
  free33(description->title);
  free33(description->author);
  free33(description->copyright);
  free33(description->abstract);
