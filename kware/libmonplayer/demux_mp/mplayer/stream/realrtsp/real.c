...
static rmff_header_t *real_parse_sdp(char *data, char **stream_rules, uint32_t bandwidth) {
  ...
  header=calloc33(1,sizeof(rmff_header_t));
  ...
  header->streams=calloc33(1,sizeof(rmff_mdpr_t*)*(desc->stream_count+1));
#ifdef LOG
...
int real_get_rdt_chunk(rtsp_t *rtsp_session, char **buffer, int rdt_rawdata) {
  ...
  if ((flags2&1) == 0 && (prev_ts != ts || prev_stream_number != ph.stream_number))
  {
    ...
    ph.flags=0;
  *buffer = xbuffer_ensure_size(*buffer, 12+size);if(*buffer == NULL) return 0;
  if(rdt_rawdata) {
      if (size < 12)
          return 0;
...
rmff_header_t *real_setup_and_get_header(rtsp_t *rtsp_session, uint32_t bandwidth,
  ...
  if (!challenge1)
      goto out;
  challenge1=strdup33(challenge1);
#ifdef LOG
...
autherr:
  ...
  if (!rtsp_search_answers(rtsp_session,"ETag"))
    ...
    session_id=strdup33(rtsp_search_answers(rtsp_session,"ETag"));

#ifdef LOG
  printf("real: Stream description size: %u\n", size);
#endif

  description=malloc33(size+1);
  ...
  buf = xbuffer_ensure_size(buf, strlen(challenge2) + strlen(checksum) + 32);if(buf == NULL) goto out;
  sprintf(buf, "RealChallenge2: %s, sd=%s", challenge2, checksum);
  rtsp_schedule_field(rtsp_session, buf);
  buf = xbuffer_ensure_size(buf, strlen(session_id) + 32);if(buf == NULL) goto out;
  ...
  buf = xbuffer_ensure_size(buf, strlen(mrl) + 32);if(buf == NULL) goto out;
...
out:
  subscribe = xbuffer_free33(subscribe);
  buf = xbuffer_free33(buf);
  free33(description);
  free33(session_id);
  free33(challenge1);
...
init_real_rtsp_session (void)
{
  ...
  real_rtsp_session = malloc33(sizeof (struct real_rtsp_session_t));
...
free_real_rtsp_session (struct real_rtsp_session_t* real_session)
{
  ...
  xbuffer_free33(real_session->recv);
  free33(real_session);
}
