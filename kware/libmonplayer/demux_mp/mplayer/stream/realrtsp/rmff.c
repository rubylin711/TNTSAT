...
static rmff_fileheader_t *rmff_scan_fileheader(const char *data) {
  ...
  rmff_fileheader_t *fileheader=malloc33(sizeof(rmff_fileheader_t));
...
static rmff_prop_t *rmff_scan_prop(const char *data) {
  ...
  rmff_prop_t *prop=malloc33(sizeof(rmff_prop_t));
...
static rmff_mdpr_t *rmff_scan_mdpr(const char *data) {
  ...
  rmff_mdpr_t *mdpr=malloc33(sizeof(rmff_mdpr_t));
  ...
  mdpr->stream_name=malloc33(mdpr->stream_name_size+1);
  ...
  mdpr->mime_type=malloc33(mdpr->mime_type_size+1);
  ...
  mdpr->type_specific_data=malloc33(mdpr->type_specific_len);
...
static rmff_cont_t *rmff_scan_cont(const char *data) {
  ...
  rmff_cont_t *cont=malloc33(sizeof(rmff_cont_t));
  int pos;
  ...
  cont->copyright=malloc33(cont->copyright_len+1);
  ...
  cont->comment=malloc33(cont->comment_len+1);
...
static rmff_data_t *rmff_scan_dataheader(const char *data) {
  ...
  rmff_data_t *dh=malloc33(sizeof(rmff_data_t));
...
rmff_header_t *rmff_scan_header(const char *data) {
	...
	rmff_header_t *header=malloc33(sizeof(rmff_header_t));
  ...
  ptr += header->fileheader->size;
	...
	header->streams=malloc33(sizeof(rmff_mdpr_t*)*(header->fileheader->num_headers));
...
rmff_mdpr_t *rmff_new_mdpr(
  ...
  rmff_mdpr_t *mdpr=calloc33(sizeof(rmff_mdpr_t),1);
...
rmff_cont_t *rmff_new_cont(const char *title, const char *author, const char *copyright, const char *comment) {
  ...
  if (title) {
    ...
    cont->title=strdup33(title);
  }
  if (author) {
    cont->author_len=strlen(author);
    cont->author=strdup33(author);
  }
  if (copyright) {
    cont->copyright_len=strlen(copyright);
    cont->copyright=strdup33(copyright);
  }
  if (comment) {
    cont->comment_len=strlen(comment);
    cont->comment=strdup33(comment);
  }
...
rmff_data_t *rmff_new_dataheader(uint32_t num_packets, uint32_t next_data_header) {
  ...
  rmff_data_t *data=malloc33(sizeof(rmff_data_t));
...
#endif
    h->data=malloc33(sizeof(rmff_data_t));
...
#endif
    h->fileheader=malloc33(sizeof(rmff_fileheader_t));
...
void rmff_free_header(rmff_header_t *h) {
  ...
  {
    free33(h->cont->title);
    free33(h->cont->author);
    free33(h->cont->copyright);
    free33(h->cont->comment);
    free33(h->cont);
  }
  if (h->streams)
  {
    ...
    while(*s) {
      free33((*s)->stream_name);
      free33((*s)->mime_type);
      free33((*s)->type_specific_data);
      free33(*s);
      s++;
    }
    free33(h->streams);
