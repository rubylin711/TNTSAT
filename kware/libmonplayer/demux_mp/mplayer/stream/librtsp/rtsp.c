...
static void rtsp_send_request(rtsp_t *s, const char *type, const char *what) {
  ...
  buf = malloc33(strlen(type)+strlen(what)+strlen(RTSP_PROTOCOL_VERSION)+3);
...
static void rtsp_schedule_standard(rtsp_t *s) {
  ...
  if (s->session) {
    char *buf;
    buf = malloc33(strlen(s->session)+15);
...
int rtsp_request_describe(rtsp_t *s, const char *what) {
  ...
  {
    buf=malloc33(strlen(s->host)+strlen(s->path)+16);
...
int rtsp_request_teardown(rtsp_t *s, const char *what) {
  ...
  {
    buf =
      malloc33(strlen (s->host) + strlen (s->path) + 16);
    sprintf (buf, "rtsp://%s:%i/%s", s->host, s->port, s->path);
  }
...
rtsp_t *rtsp_connect(int fd, char* mrl, char *path, char *host, int port, char *user_agent) {
  ...
  if (user_agent)
    s->user_agent=strdup33(user_agent);
  else
    s->user_agent=strdup33("User-Agent: RealMedia Player Version 6.0.9.1235 (linux-2.0-libc6-i386-gcc2.95)");

  s->mrl = strdup33(mrl);
  s->host = strdup33(host);
  s->port = port;
  s->path = strdup33(path);
  while (*path == '/')
    path++;
  if (s->path && (s->param = strchr(s->path, '?')) != NULL)
    s->param++;
