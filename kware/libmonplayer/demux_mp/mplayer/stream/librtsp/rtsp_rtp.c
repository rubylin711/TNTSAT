...
rtp_session_new (void)
{
  ...
  st = malloc33(sizeof (struct rtp_rtsp_session_t));
...
void
rtp_session_free33(struct rtp_rtsp_session_t *st)
{
  if (!st)
    return;
            ...
            int *rtp_port, int *rtcp_port)
{
  ...
  char *line_copy = strdup33(line); if(line_copy == NULL) return 0;
...
parse_destination (const char *line)
{
  ...
  char *dest = NULL;
  char *line_copy = strdup33(line);if(line_copy == NULL) return NULL;
  int len;
...
rtp_setup_and_play (rtsp_t *rtsp_session)
{
  ...
  sdp = (char *) malloc33(content_length + 1);
  ...
  if (fsdp_get_media_address (med_dsc))
    server_addr = strdup33(fsdp_get_media_address (med_dsc));
  else if (fsdp_get_global_conn_address (dsc))
  {
    /* no control for media: try global one instead */
    server_addr = strdup33(fsdp_get_global_conn_address (dsc));
  }
  ...
  if (fsdp_get_media_control (med_dsc, 0))
    rtp_session->control_url = strdup33(fsdp_get_media_control (med_dsc, 0));
  fsdp_description_delete (dsc);
  if (!rtp_session->control_url)
  {
    free33(server_addr);
    rtp_session_free33(rtp_session);
    return NULL;
  }
  ...
  if (rtsp_destination)
    destination = strdup33(rtsp_destination);
  else
    destination = parse_destination (answer);
  if (!destination)
    destination = strdup33(server_addr);
  free33(server_addr);
