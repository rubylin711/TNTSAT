...
rtsp_session_t *rtsp_session_start(int fd, char **mrl, char *path, char *host,
  ...
  rtsp_session = malloc33(sizeof (rtsp_session_t));
  rtsp_session->s = NULL;
...
//connect:
  *redir = 0;
  ...
  if (rtsp_search_answers(rtsp_session->s,RTSP_OPTIONS_SERVER))
    server=strdup33(rtsp_search_answers(rtsp_session->s,RTSP_OPTIONS_SERVER));
  else {
    if (rtsp_search_answers(rtsp_session->s,RTSP_OPTIONS_REAL))
      server=strdup33(RTSP_SERVER_TYPE_REAL);
    else
      server=strdup33(RTSP_SERVER_TYPE_UNKNOWN);
  }
  if (strstr(server,RTSP_SERVER_TYPE_REAL) || strstr(server,RTSP_SERVER_TYPE_HELIX))
  {
    ...
    if (!h || !h->streams[0]) {
      ...
      {
        free33(mrl_line);
	mrl_line=strdup33(rtsp_search_answers(rtsp_session->s, RTSP_OPTIONS_LOCATION));
...
void rtsp_session_end(rtsp_session_t *session) {
  ...
  if (session->rtp_session)
    rtp_session_free33(session->rtp_session);
  free33(session);
}
