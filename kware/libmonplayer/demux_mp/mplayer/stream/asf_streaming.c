...
static int asf_streaming_parse_header(int fd, streaming_ctrl_t* streaming_ctrl) {
  ...
  do {
	  ...
	  buffer = malloc33(size+buffer_size);
	  if(buffer == NULL) {
...
*/
  ...
  while ((pos = find_asf_guid(buffer, asf_stream_header_guid, pos, size)) >= 0)
  {
	...
	if(asf_ctrl->audio_streams == NULL){
	  asf_ctrl->audio_streams = malloc33(sizeof(int));
	  asf_ctrl->n_audio = 1;
	} else {
	  asf_ctrl->n_audio++;
	  asf_ctrl->audio_streams = realloc33(asf_ctrl->audio_streams,
	...
	if(asf_ctrl->video_streams == NULL){
	  asf_ctrl->video_streams = malloc33(sizeof(int));
	  asf_ctrl->n_video = 1;
	} else {
	  asf_ctrl->n_video++;
	  asf_ctrl->video_streams = realloc33(asf_ctrl->video_streams,
  ...
  v_rates = calloc33(asf_ctrl->n_video, sizeof(int));
  a_rates = calloc33(asf_ctrl->n_audio, sizeof(int));
...
static int asf_http_streaming_start( stream_t *stream, int *demuxer_type ) {
	...
	asf_http_ctrl = malloc33(sizeof(asf_http_streaming_ctrl_t));
	...
	do {
		...
		http_hdr = asf_http_request( stream->streaming_ctrl ); if(http_hdr == NULL) goto err_out;
