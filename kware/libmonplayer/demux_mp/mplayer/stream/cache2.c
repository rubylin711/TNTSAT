...
static void shared_free33(void *ptr, int64_t size) {
	return;
...
static cache_vars_t* cache_init(int64_t size,int sector){
	...
	if(s->buffer == NULL){
		shared_free33(s, sizeof(cache_vars_t));
		return NULL;
	}
...
void cache_uninit(stream_t *s) {
	...
	if(!c) return;
	shared_free33(c->buffer, c->buffer_size);
	c->buffer = NULL;
	c->stream = NULL;
	shared_free33(s->cache_data, sizeof(cache_vars_t));
	s->cache_data = NULL;
}
...
{
	int ss = stream->sector_size ? stream->sector_size : io_stream_buffer_size;
	int res = -1;
	cache_vars_t* s;
	...
	if((stream->cache_pid=fork())){
		...
		{
			OS_PRINTF("[%s] stream->cache_pid:%d\n",stream->cache_pid);
			stream->cache_pid = 0;
		}
#else
		{
			stream_t* stream2=malloc33(sizeof(stream_t));
