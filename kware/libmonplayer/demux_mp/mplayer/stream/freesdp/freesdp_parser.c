...
static char *mp_strdup(const char *s)
{
  ...
  new_string = malloc33(strlen(s) + 1);
...
fsdp_parse (const char *text_description, fsdp_description_t * dsc)
{
  ...
  if (!strncmp (p, "o=", 2))
  {
    ...
    dsc->o_announcement_version = mp_strdup (fsdp_buf[2]);
    ...
    dsc->o_address = mp_strdup (fsdp_buf[5]);
  }
  else
  {
  ...
  if (!strncmp (p, "s=", 2))
  {
    ...
    dsc->s_name = mp_strdup (longfsdp_buf);
  }
  else
  {
  ...
  if (!strncmp (p, "i=", 2)
      && sscanf (p, "i=%" MLFLENS "[^\r\n]", longfsdp_buf))
  {
    dsc->i_information = mp_strdup (longfsdp_buf);
    NEXT_LINE (p);
  }
  else
  {
      ...
      && sscanf (p, "u=%" MLFLENS "[^\r\n]", longfsdp_buf))
  {
    ...
    dsc->u_uri = mp_strdup (longfsdp_buf);
    NEXT_LINE (p);
  }
  else
  {
  ...
  if (dsc->emails_count > 0)
  {
    ...
    dsc->emails = calloc33(j, sizeof (const char *)); if(dsc->emails == NULL) return FSDPE_INVALID_TIME;
    for (j = 0; j < dsc->emails_count; j++)
    {
      sscanf (p, "e=%" MLFLENS "[^\r\n]", longfsdp_buf);
      /* TODO? check valid email-address. */
      dsc->emails[j] = mp_strdup (longfsdp_buf);
      NEXT_LINE (p);
    }
  }
  ...
  {
    dsc->phones = calloc33(j, sizeof (const char *));if(dsc->phones == NULL) return FSDPE_INVALID_TIME;
    for (j = 0; j < dsc->phones_count; j++)
    {
      sscanf (p, "p=%" MLFLENS "[^\r\n]", longfsdp_buf);
      /* TODO? check valid phone-number. */
      dsc->phones[j] = mp_strdup (longfsdp_buf);
      NEXT_LINE (p);
    }
  }
  ...
  dsc->time_periods = calloc33(dsc->time_periods_count,
			      sizeof (fsdp_time_period_t *));if(dsc->time_periods == NULL)return FSDPE_INVALID_TIME;
  for (j = 0; j < dsc->time_periods_count; j++)
  {
    ...
    dsc->time_periods[j] = calloc33(1, sizeof (fsdp_time_period_t));
    ...
    if (h > 0)
    {
      ...
      dsc->time_periods[j]->repeats =
        calloc33(h, sizeof (fsdp_repeat_t *));
      for (h = 0; h < dsc->time_periods[j]->repeats_count; h++)
      {
                    ...
                    fsdp_buf[0], fsdp_buf[1], longfsdp_buf) == 3)
        {
          ...
          dsc->time_periods[j]->repeats[h] =
            calloc33(1, sizeof (fsdp_repeat_t));
          ...
          if (result == FSDPE_OK)
          {
            ...
            if (result == FSDPE_OK)
            {
              ...
              repeat->offsets = calloc33(k, sizeof (time_t));if(repeat->offsets == NULL) return FSDPE_INVALID_REPEAT;
              i = longfsdp_buf;
              for (k = 0;
  ...
  if (!strncmp (p, "z=", 2))
  {
    if (sscanf (p, "z=%" MLFLENS "[^\r\n]", longfsdp_buf))
    {
      ...
      dsc->timezone_adj = mp_strdup (longfsdp_buf);
      NEXT_LINE (p);
    }
    else
    {
         ...
         fsdp_buf[1]) == 2)
    {
      ...
      else if (!strncmp (fsdp_buf[0], "sdplang", 7))
      {
        if (NULL == dsc->a_sdplangs)
        {
          ...
          dsc->a_sdplangs =
            calloc33(SDPLANGS_MAX_COUNT, sizeof (char *));
        }
      ...
      else if (!strncmp (fsdp_buf[0], "lang", 4))
      {
        ...
        {
          dsc->a_langs_count = 0;
          dsc->a_langs = calloc33(SDPLANGS_MAX_COUNT, sizeof (char *));
        }
      ...
      else if (!strncmp (fsdp_buf[0], "control", 7))
      {
        if (NULL == dsc->a_controls)
        {
          ...
          dsc->a_controls =
            calloc33(SDPCONTROLS_MAX_COUNT, sizeof (char *));
        }
        ...
        *longfsdp_buf = '\0';
        ...
        if (NULL == dsc->unidentified_attributes)
        {
          ...
          dsc->unidentified_attributes =
            calloc33(UNIDENTIFIED_ATTRIBUTES_MAX_COUNT,
                    sizeof (char *));
        }
  ...
  {				/* dsc->media_announcements_count > 0 */
    dsc->media_announcements =
      calloc33(j, sizeof (fsdp_media_announcement_t *));if(dsc->media_announcements == NULL) return FSDPE_INVALID_MEDIA;
    for (j = 0; j < dsc->media_announcements_count; j++)
    {
      ...
      else
      {
        dsc->media_announcements[j] =
          calloc33(1, sizeof (fsdp_media_announcement_t));
          ...
          return FSDPE_UNKNOWN_MEDIA_TRANSPORT;
        {
          ...
          media->formats = calloc33(k, sizeof (char *));
          s = longfsdp_buf;
	      ...
	      unsigned int *bw_modifiers_count)
{
  ...
  *bw_modifiers = calloc33(i, sizeof (fsdp_bw_modifier_t));if(*bw_modifiers == NULL) return FSDPE_INVALID_BANDWIDTH;
		   ...
		   const char *value)
{
  ...
  {
    *counter = 0;
    *rtpmap = calloc33(MEDIA_RTPMAPS_MAX_COUNT, sizeof (fsdp_rtpmap_t *));
  }
  if (*counter < MEDIA_RTPMAPS_MAX_COUNT)
  {
    ...
    char fsdp_buf[MAXSHORTFIELDLEN];
    char longfsdp_buf[MAXLONGFIELDLEN];
    map[c] = calloc33(1, sizeof (fsdp_rtpmap_t));
