...
int mthttp_url_is_protocol(const URL_t *url, const char *proto) {
  int proxy = is_proxy(url);
  if (proxy) {
    URL_t *tmp = mthttp_url_new(url->file + 1);if(tmp == NULL) return -1;
    int res = !strcasecmp(tmp->protocol, proto);
    mthttp_url_free33(tmp);
    return res;
  }
...
void mthttp_url_set_protocol(URL_t *url, const char *proto) {
  int proxy = is_proxy(url);
  if (proxy) {
    char *dst = url->file + 1;
  ...
  #ifndef __LINUX__
  mthttp_free33(url->protocol);
  url->protocol = mtstrdup33(proto);
  #else
  free33(url->protocol);
  url->protocol = strdup33(proto);
  #endif
...
URL_t *mthttp_url_redirect(URL_t **url, const char *redir) {
  URL_t *u = *url;
  ...
  if (!strchr(redir, '/') || *redir == '/') {
    char *tmp;
    #ifndef __LINUX__
    newurl = buffer = mthttp_malloc33(strlen(oldurl) + strlen(redir) + 1);
    #else
    newurl = buffer = malloc33(strlen(oldurl) + strlen(redir) + 1);
    #endif
  ...
  if (proxy) {
    char *tmp = mthttp_get_http_proxy_url(u, newurl);
    #ifndef __LINUX__
    mthttp_free33(buffer);
    #else
    free33(buffer);
    #endif
    newurl = buffer = tmp;
  }
  res = mthttp_url_new(newurl);
  #ifndef __LINUX__
  mthttp_free33(buffer);
  #else
  free33(buffer);
...
static char *mthttp_get_noauth_url(const URL_t *url)
{
    if (url->port)
        return mthttp_mp_asprintf("%s://%s:%d%s",
                           url->protocol, url->hostname, url->port, url->file);
    else
        return mthttp_mp_asprintf("%s://%s%s",
...
char *mthttp_get_http_proxy_url(const URL_t *proxy, const char *host_url)
{
    if (proxy->username)
...
URL_t* mthttp_url_new(const char* url) {
	...
	if( url==NULL ) return NULL;
        ...
        #ifndef __LINUX__
        escfilename=mthttp_malloc33(strlen(url)*3+1);
        #else
        escfilename=malloc33(strlen(url)*3+1);
        #endif
        if (!escfilename ) {
                mp_msg(MSGT_NETWORK,MSGL_FATAL,MSGTR_MemAllocFailed);
                goto err_out;
	...
	#ifndef __LINUX__
	Curl = mthttp_calloc33(1, sizeof(*Curl));
	#else
	Curl = calloc33(1, sizeof(*Curl));
	#endif
	if( Curl==NULL ) {
		mp_msg(MSGT_NETWORK,MSGL_FATAL,MSGTR_MemAllocFailed);
		goto err_out;
	...
	mthttp_url_escape_string(escfilename,url);

	// Copy the url in the URL container
	#ifndef __LINUX__
	Curl->url = mtstrdup33(escfilename);
	#else
	Curl->url = strdup33(escfilename);
	#endif
	if( Curl->url==NULL ) {
		mp_msg(MSGT_NETWORK,MSGL_FATAL,MSGTR_MemAllocFailed);
		goto err_out;
	...
	#ifndef __LINUX__
	Curl->protocol = mthttp_malloc33(pos1+1);
	#else
	Curl->protocol = malloc33(pos1+1);
	#endif
	...
	if( ptr2!=NULL ) {
		...
		int len = ptr2-ptr1;
		#ifndef __LINUX__
		Curl->username = mthttp_malloc33(len+1);
		#else
		Curl->username = malloc33(len+1);
		#endif
		...
		if( ptr3!=NULL && ptr3<ptr2 ) {
			...
			#ifndef __LINUX__
			Curl->password = mthttp_malloc33(len2+1);
			#else
			Curl->password = malloc33(len2+1);
			#endif
	...
	if( ptr2==NULL ) {
		...
		if( ptr3==NULL ) {
            ...
            //for special url like http://www.hostname.com?xxxxx
			ptr3 = strstr(ptr1, "\?");
			if(ptr3 != NULL)
	...
	#ifndef __LINUX__
	Curl->hostname = mthttp_malloc33(pos2-pos1+1);
	#else
	Curl->hostname = malloc33(pos2-pos1+1);
	#endif
	...
	if( ptr2!=NULL ) {
		...
		if( strlen(ptr2)>1 ) {
			...
			#ifndef __LINUX__
			Curl->file = mtstrdup33(ptr2);
			#else
			Curl->file = strdup33(ptr2);
			#endif
			if( Curl->file==NULL ) {
				mp_msg(MSGT_NETWORK,MSGL_FATAL,MSGTR_MemAllocFailed);
				goto err_out;
	    ...
	    ptr2 = strstr(ptr1, "\?");
            if( ptr2!=NULL ) {
            		...
            		if( strlen(ptr2)>1 ) {
                        ...
                        file_name_len = strlen(ptr2) + 16;

            			file_name = malloc33(file_name_len);
            			if(file_name)
            			{
						    //add / for special url
            			    snprintf(file_name, file_name_len, "/%s", ptr2);
            			    Curl->file = strdup33(file_name);
            			}
            			else
            			    Curl->file = strdup33(ptr2);
            			if(file_name)
	...
	if( Curl->file==NULL ) {
        #ifndef __LINUX__
		Curl->file = mthttp_malloc33(2);
		#else
		Curl->file = malloc33(2);
		#endif
		if( Curl->file==NULL ) {
			mp_msg(MSGT_NETWORK,MSGL_FATAL,MSGTR_MemAllocFailed);
			goto err_out;
	...
	Curl->noauth_url = mthttp_get_noauth_url(Curl);
		if (!Curl->noauth_url) {
			mp_msg(MSGT_NETWORK, MSGL_FATAL, MSGTR_MemAllocFailed);
			goto err_out;
		}
    #ifndef __LINUX__
	mthttp_free33(escfilename);
	#else
	free33(escfilename);
	#endif
	return Curl;
err_out:
	...
	free33(escfilename);
	#endif
	if (Curl) mthttp_url_free33(Curl);
	return NULL;
}
...
err_out:
	free33(escfilename);
	if (Curl) url_free33(Curl);
	return NULL;
}
void
mthttp_url_free33(URL_t* url) {
	if(!url) return;
	#ifndef __LINUX__
	mthttp_free33(url->url);
	mthttp_free33(url->protocol);
	mthttp_free33(url->hostname);
	mthttp_free33(url->file);
	mthttp_free33(url->username);
	mthttp_free33(url->password);
	mthttp_free33(url->noauth_url);
	mthttp_free33(url);
	#else
	free33(url->url);
	free33(url->protocol);
	...
	free33(url->username);
	free33(url->password);
	free33(url->noauth_url);
	free33(url);
	#endif
}

void
url_free33(URL_t* url) {
	if(!url) return;
	free33(url->url);
	free33(url->protocol);
	free33(url->hostname);
...
/* works like strcpy(), but without return argument */
void
mthttp_url_escape_string(char *outbuf, const char *inbuf) {
	unsigned char c;
	...
	while(i < len) {
		...
		if(j < len) {
		#ifndef __LINUX__
			if(!tmp) tmp = mthttp_malloc33(len+1);
		#else
		       if(!tmp) tmp = malloc33(len+1);
		 #endif
			...
			in = (char*)inbuf+i;
        #ifndef __LINUX__
		if(!unesc) unesc = mthttp_malloc33(len+1);
		#else
		if(!unesc) unesc = malloc33(len+1);
		#endif
