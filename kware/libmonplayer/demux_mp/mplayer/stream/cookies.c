...
static int parse_line(char **ptr, char *cols[7/*6*/])      // for tsscan
{
    int col;
    cols[0] = *ptr;
						  ...
						  *list)
{
	...
	if (parse_line(&ptr, cols)) {
	    ...
	    new = malloc33(sizeof(cookie_list_t));
...
static struct cookie_list_type *load_cookies(void)
{
    ...
    buf = malloc33(strlen(homedir) + sizeof("/.mozilla/default") + 1);
	...
	while ((ent = readdir(dir)) != NULL) {
	    if ((ent->d_name)[0] != '.') {
		const char *home = getenv("HOME");if(home == NULL) return NULL;
    ...
    buf = malloc33(strlen(homedir) + sizeof("/.netscape/cookies.txt") + 1);
...
cookies_set(HTTP_header_t * http_hdr, const char *domain, const char *url)
{
    ...
    buf = strdup33("Cookie:");if(buf == NULL) return;

    for (i = 0; i < found_cookies; i++) {
	char *nbuf;

	nbuf = malloc33(strlen(buf) + strlen(" ") + strlen(cookies[i]->name) +
