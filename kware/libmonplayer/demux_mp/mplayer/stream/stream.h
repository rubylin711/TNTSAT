...
#if __LINUX__
//revert for network stream
#define STREAM_BUFFER_SIZE 2048
#else
//revert to 64k for exp13_720p.mp4 MPEG2_MPEG2_1080P.avi
#define STREAM_BUFFER_SIZE /*2048*/(256 * 1024)
#endif
...
#define STREAM_SOURCE_TYPE_NORMAL_FILE      0 /* General file operation */
#define STREAM_SOURCE_TYPE_FFMPEG           1 /* ffmpeg not handle "rtp", "udp", "rtsp", "rtmp" */
#define STREAM_SOURCE_TYPE_FFMPEG_MISC      2 /* ffmpeg handle "rtp", "udp", "rtsp", "rtmp" */
#define STREAM_SOURCE_TYPE_EXTERNAL_IO      3 /* User defined i/o source */
...
#define STREAM_CTRL_GET_SOURCE_TYPE   15

extern int io_stream_buffer_size;
...
typedef struct streaming_control
{
    ...
    int64_t chunksize;
    void *i_stream;
    unsigned char *hc_buf;
    unsigned char *hc_buf_ptr;
    unsigned char *hc_buf_end;
    unsigned int hc_buf_size;
} streaming_ctrl_t;
...
typedef struct stream
{
    ...
    int is_chunked; //add by zhouxiang
    int eof;
    int mode; //STREAM_READ or STREAM_WRITE
    unsigned int cache_pid;
    void *cache_data;
    void *priv; // used for DVD, TV, RTSP etc
    char *url;  // strdup33() of filename/url
    ...
    void *transport_desdec_ctx;
    int (*transport_desdec_func) (void *h,
        const void *info, const unsigned char *in, unsigned char *out, int len);
} stream_t;

typedef struct stream_io_options {
    void *interrupt_cb;
    void **ff_dict_opts;
} stream_io_options_t;
...
{
    unsigned int y;
    y = (unsigned int)stream_read_char(s);
    y = (y << 8) | (unsigned int)stream_read_char(s);
    y = (y << 8) | (unsigned int)stream_read_char(s);
    y = (y << 8) | (unsigned int)stream_read_char(s);
    return y;
}
...
static inline unsigned int stream_read_dword_le(stream_t *s)
{
    unsigned int y;
    y = (unsigned int)stream_read_char(s);
    y |= ((unsigned int)stream_read_char(s)) << 8;
    y |= ((unsigned int)stream_read_char(s)) << 16;
    y |= ((unsigned int)stream_read_char(s)) << 24;
    return y;
}
...
{
    uint64_t y;
    y = (uint64_t)stream_read_char(s);
    y = (y << 8) | (uint64_t)stream_read_char(s);
    y = (y << 8) | (uint64_t)stream_read_char(s);
    y = (y << 8) | (uint64_t)stream_read_char(s);
    y = (y << 8) | (uint64_t)stream_read_char(s);
    y = (y << 8) | (uint64_t)stream_read_char(s);
...
{
    if (len < 0 || (len > 2 * io_stream_buffer_size && (s->flags & MP_STREAM_SEEK_FW))) {
	// negative or big skip!
	return stream_seek(s, stream_tell(s) + len);
    }
    while (len > 0) {
	int x = (int)(s->buf_len - s->buf_pos);
	if (x == 0) {
	    if (!cache_stream_fill_buffer(s))
		return 0; // EOF
	    x = (int)(s->buf_len - s->buf_pos);
	}
	if (x > len)
	    x = len;
	//memcpy(mem,&s->buf[s->buf_pos],x);
	s->buf_pos += (unsigned int)x;
	len -= x;
    }
    return 1;
}

static inline mt_u32 stream_is_rtp_type(const char *fname)
{
    return (0 == strncmp(fname, "rtp:", 4));
}

static inline mt_u32 stream_is_udp_type(const char *fname)
{
    return (0 == strncmp(fname, "udp:", 4));
}

static inline mt_u32 stream_is_http_type(const char *fname)
{
    return ((0 == strncmp(fname, "http:", 5)) ||
            (0 == strncmp(fname, "https:", 6)));
}

static inline mt_u32 stream_is_rtsp_type(const char *fname)
{
    return (0 == strncmp(fname, "rtsp:", 5));
}

static inline mt_u32 stream_is_rtmp_type(const char *fname)
{
    return (0 == strncmp(fname, "rtmp:", 5));
}

static inline mt_u32 stream_is_srt_type(const char *fname)
{
    return (0 == strncmp(fname, "srt:", 4));
}
