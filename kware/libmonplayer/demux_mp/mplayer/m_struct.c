...
void
m_struct_free33(const m_struct_t* st, void* obj) {
  int i;

  for(i = 0 ; st->fields[i].name ; i++)
    m_option_free33(&st->fields[i],M_ST_MB_P(obj,st->fields[i].p));
  free33(obj);
}

void*
m_struct_copy(const m_struct_t* st, void* obj) {
  void* r = (void *)malloc33(st->size);
  int i;
