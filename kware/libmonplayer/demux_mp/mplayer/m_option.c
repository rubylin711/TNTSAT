...
static char* dup_printf(const char *fmt, ...) {
  ...
  while (1) {
    ...
    if ((p = (char *)realloc33(p, size)) == NULL)
      return NULL;
  }
}
...
static char* print_flag(const m_option_t* opt,  const void* val) {
  if(VAL(val) == opt->min)
    return (char *)strdup33("no");
  else
    return (char *)strdup33("yes");
}
...
static char* print_str(const m_option_t* opt,  const void* val) {
  return (val && VAL(val) && strlen(VAL(val)) > 0) ? strdup33(VAL(val)) : NULL;
}

static void copy_str(const m_option_t* opt,void* dst, const void* src) {
  if(dst && src) {
#ifndef NO_FREE
    free33(VAL(dst)); //FIXME!!!
#endif
    VAL(dst) = VAL(src) ? strdup33(VAL(src)) : NULL;
  }
}
...
static int str_list_add(char** add, int n,void* dst,int pre) {
  ...
  lst = (char**)realloc33(lst,(unsigned int)(n+ln+1)*sizeof(char*));if(lst == NULL) return M_OPT_UNKNOWN;

  if(pre) {
    memmove(&lst[n],lst,(unsigned int)ln*sizeof(char*));
    memcpy(lst,add,(unsigned int)n*sizeof(char*));
  } else
    memcpy(&lst[ln],add,(unsigned int)n*sizeof(char*));
...
static int str_list_del(char** del, int n,void* dst) {
  ...
  d = (char **)calloc33(s+1,sizeof(char*));
  for(i = 0, n = 0 ; i < ln ; i++) {
...
static int parse_str_list(const m_option_t* opt,const char *name, const char *param, void* dst, int src) {
  int n = 0,len = (int)strlen(opt->name);
  char *str;
  ...
  res = (char **)malloc33(((int)n+2)*sizeof(char*));
  ptr = str = (char *)strdup33(param);
  n = 0;
  ...
  while(1) {
    ...
    if(!ptr) {
      res[n] = (char *)strdup33(last_ptr);
      n++;
      break;
    }
    len = ptr - last_ptr;
    res[n] = (char *)malloc33(len + 1);
    if(len) strncpy(res[n],last_ptr,(unsigned int)len);
    res[n][len] = '\0';
    ptr++;
    n++;
  }
...
static void copy_str_list(const m_option_t* opt,void* dst, const void* src) {
  ...
  for(n = 0 ; s[n] != NULL ; n++)
    /* NOTHING */;
  d = (char **)malloc33((n+1)*sizeof(char*));
  for( ; n >= 0 ; n--)
    d[n] = s[n] ? strdup33(s[n]) : NULL;
...
static char* print_str_list(const m_option_t* opt, const void* src) {
  ...
  for(i = 0 ; lst[i] ; i++) {
    if(last) {
      ...
      ret = (char *)strdup33(lst[i]);
    last = ret;
  }
  if(last && last != ret) free33(last);
  return ret;
}
...
static int parse_func_pf(const m_option_t* opt,const char *name, const char *param, void* dst, int src) {
  ...
  s = (m_func_save_t *)calloc33(1,sizeof(m_func_save_t));
  s->name = (char*)strdup33(name);
  s->param = param ? strdup33(param) : NULL;
...
static void copy_func_pf(const m_option_t* opt,void* dst, const void* src) {
  ...
  while(s) {
    d = (m_func_save_t *)calloc33(1,sizeof(m_func_save_t));
    d->name = (char*)strdup33(s->name);
    d->param = s->param ? strdup33(s->param) : NULL;
    if(last)
      last->next = d;
    else
      VAL(dst) = d;
...
static int parse_subconf(const m_option_t* opt,const char *name, const char *param, void* dst, int src) {
  ...
  subparam = (char *)malloc33(strlen(param)+1);
  subopt = (char *)malloc33(strlen(param)+1);
  p = param;
  ...
  while(p[0])
	...
	case 2:
	  ...
	  if(r < 0) goto out;
	  if(dst) {
	    lst = (char**)realloc33(lst,2 * ((unsigned int)nr+2) * sizeof(char*));if(lst == NULL) goto out;
	    lst[2*nr] = (char*)strdup33(subopt);
	    lst[2*nr+1] = subparam[0] == 0 ? NULL : strdup33(subparam);
...
static int get_obj_param(const char* opt_name,const char* obj_name, const m_struct_t* desc,
  ...
  if(eq) {
    ...
    if(dst) {
      dst[0] = (char*)strdup33(str);
      dst[1] = p ? strdup33(p) : NULL;
    }
    eq[0] = '=';
  } else {
    if((*nold) >= oldmax) {
    ...
    if(dst) {
      dst[0] = (char*)strdup33(opt->name);
      dst[1] = (char*)strdup33(str);
    }
    (*nold)++;
  }
  return 1;
...
static int get_obj_params(const char* opt_name, const char* name,char* params,
  ...
  ret = (char**)malloc33((n+2)*2*sizeof(char*));
  n = nold = 0;
  last_ptr = params;
...
static int parse_obj_settings(const char* opt,char* str,const m_obj_list_t* list,
  ...
  if(param) {
    if(!desc && _ret) {
      ...
      plist = (char**)calloc33(4,sizeof(char*));
      plist[0] = (char*)strdup33("_oldargs_");
      plist[1] = (char*)strdup33(param);
    } else if(desc) {
  ...
  ret = (m_obj_settings_t *)realloc33(ret,(ret_n+2)*sizeof(m_obj_settings_t));if(ret == NULL) return M_OPT_INVALID;
  memset(&ret[ret_n],0,2*sizeof(m_obj_settings_t));
  ret[ret_n].name = (char*)strdup33(str);
...
static int obj_settings_list_del(const char *opt_name,const char *param,void* dst, int src) {
  ...
  for(r = 0 ; str_list[r] ; r++) {
    ...
    if(endptr == str_list[r]) {
      ...
      m_option_free33(&list_opt,&str_list);
      return M_OPT_INVALID;
    }
  ...
  obj_list = (m_obj_settings_t*)realloc33(obj_list,sizeof(m_obj_settings_t)*(idx_max+1));
  VAL(dst) = obj_list;
...
static int parse_obj_settings_list(const m_option_t* opt,const char *name,
  ...
  if(dst) {
	...
	/* NOP */;
      res = (m_obj_settings_t *)realloc33(res,(qsize+n+1)*sizeof(m_obj_settings_t));if(res == NULL) return M_OPT_UNKNOWN;
	...
	/* NOP */;
      head = (m_obj_settings_t *)realloc33(head,(hsize+n+1)*sizeof(m_obj_settings_t));if(head == NULL) return M_OPT_UNKNOWN;
...
static void copy_obj_settings_list(const m_option_t* opt,void* dst, const void* src) {
  ...
  for(n = 0 ; s[n].name ; n++)
    /* NOP */;
  d = (m_obj_settings_t *)malloc33((n+1)*sizeof(m_obj_settings_t));
  for(n = 0 ; s[n].name ; n++) {
    d[n].name = (char*)strdup33(s[n].name);
    d[n].attribs = NULL;
