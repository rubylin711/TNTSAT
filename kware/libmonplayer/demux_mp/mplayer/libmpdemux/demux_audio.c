    ...
    if (((*list)->next_frame_pos < st_pos) || ((*list)->mp3_chans != mp3_chans)
      ...
      *list = tmp;
      ...
      if ((*list)->cons_hdrs >= MIN_MP3_HDRS) {
        ...
        tmp = (mp3_hdr_t *)malloc33(sizeof(mp3_hdr_t));
...
#define MAX_DETECT_LEN (5*1024*1024)
static void detect_mp3_bps(demuxer_t* demuxer)
{
  uint8_t hdr[4];
  int len;
  ...
  //printf("%s demuxer->movi_start %lld, movi_end %lld\n", __func__, demuxer->movi_start, demuxer->movi_end);
  if(demuxer->movi_end <= demuxer->movi_start)
    return;
  stream_t *s = demuxer->stream;
  off_t filesize = (demuxer->movi_end - demuxer->movi_start);
  off_t start_pos = stream_tell(s);
  off_t detect_len = MAX_DETECT_LEN;
  if(filesize < detect_len)
    detect_len = filesize;

  off_t detect_end_pos = start_pos + detect_len;
  if (demuxer->movi_end) {
    /* ignore last header without data */
    detect_end_pos = MAX(MIN(detect_end_pos, demuxer->movi_end) - HDR_SIZE + 1, 0);
  }

  //printf("%s %d, filesize %lld, start_pos %lld\n",__func__, __LINE__, filesize, start_pos);
  while(stream_tell(s) < detect_end_pos && is_file_seq_exit()==FALSE) {
    //printf("%s pos %lld\n",__func__, __LINE__, stream_tell(s));
    ...
    next_pts += sh_audio->audio.dwScale/(double)sh_audio->samplerate;
    if (demuxer->movi_end && stream_tell(s) >= demuxer->movi_end)
	  break;
  }
  int s32_detect_len = (int)detect_len;

  sh_audio->i_bps = (int)((double)s32_detect_len / next_pts);
  sh_audio->wf->nAvgBytesPerSec = sh_audio->i_bps;
  //printf("%s priv->next_pts %d, s32_detect_len %d calc sh_audio->i_bps %d\n",__func__, (int)(next_pts*1000), s32_detect_len, sh_audio->i_bps);
...
static int demux_audio_open(demuxer_t* demuxer) {
  ...
  if(sh_audio == NULL)
  {
    return DEMUXER_TYPE_UNKNOWN;        // for tsscan
  }

  switch(frmt) {
  case MP3:
    if(mp3_found){      // for tsscan
    ...
    sh_audio->wf = (WAVEFORMATEX*)malloc33(sizeof(*sh_audio->wf));
    ...
    mp3_found = NULL;
    OS_PRINTF("[%s] -----demuxer->movi_end=%d   s->flags=%x\n",__func__,demuxer->movi_end,s->flags);
  ...
  case WAV: {
    ...
    sh_audio->wf = w = (WAVEFORMATEX*)malloc33(l > sizeof(*w) ? l : sizeof(*w));
...
//	    get_flac_metadata (demuxer);
  ...
  priv = (da_priv_t*)malloc33(sizeof(da_priv_t));
  priv->frmt = frmt;
  priv->next_pts = 0;
  demuxer->priv = priv;
  demuxer->audio->id = 0;
  ...
  if(frmt == MP3  && duration < 0.1 && demuxer->movi_end > demuxer->movi_start)//
  {
    detect_mp3_bps(demuxer);
  }
...
static int demux_audio_fill_buffer(demuxer_t *demux, demux_stream_t *ds) {
  ...
  case WAV : {
    ...
    dp = new_demux_packet(l); if(dp == NULL) return 0;
...
static void demux_audio_seek(demuxer_t *demuxer,float rel_seek_secs,float audio_delay,int flags){
  ...
  if(priv->frmt == MP3 && demuxer->enbale_hr_mp3_seek == TRUE
     && !(flags & SEEK_FACTOR)) {
