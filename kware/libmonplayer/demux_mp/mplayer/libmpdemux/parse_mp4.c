...
#define freereturn(a,b) free33(a); return b
...
int mp4_parse_esds(unsigned char *data, int datalen, esds_t *esds) {
  ...
  stream_t *s = new_memory_stream(data, datalen);if(s == NULL) return 1;
  uint16_t len;
#ifdef MP4_DUMPATOM
  {int i;
  ...
  esds->decoderConfig = malloc33(esds->decoderConfigLen);
  ...
  esds->SLConfig = malloc33(esds->SLConfigLen);
  if (esds->SLConfig) {
    stream_read(s, esds->SLConfig, esds->SLConfigLen);
  } else {
    esds->SLConfigLen = 0;
  }
  if(esds->SLConfig)        // for tsscan
  mp_msg(MSGT_DEMUX, MP4_DL,
...
void mp4_free_esds(esds_t *esds) {
  if(esds->decoderConfigLen)
    free33(esds->decoderConfig);
  if(esds->SLConfigLen)
    free33(esds->SLConfig);
}
