...
//  printf( "[demux_mf] frame: %d (%s,%d)\n",mf->curr_frame,mf->names[mf->curr_frame],fs.st_size );
  ...
  {
   sh_video_t     * sh_video = demuxer->video->sh;
   demux_packet_t * dp = new_demux_packet( fs.st_size );if(dp == NULL){fclose( f ); return 0;}
   if ( !fread( dp->buffer,fs.st_size,1,f ) )           // for tsscan
   {
        free33(dp->buffer);
        fclose( f );
        return 0;
...
static demuxer_t* demux_open_mf(demuxer_t* demuxer){
  ...
  if(!mf_type){
    ...
    if(!p){
      ...
      free33( mf ); return NULL;
    }
    mf_type=strdup33(p+1);
