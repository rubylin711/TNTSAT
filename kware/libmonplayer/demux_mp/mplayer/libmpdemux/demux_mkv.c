...
static void av_noinline grow_array(void *arrayp, int nelem, size_t elsize)
{
    ...
    if (nelem > UINT_MAX / elsize - 32)
        ...
        *array = realloc33(*array, (nelem + 32) * elsize);
    if (!*array)
        free33(oldp);
}
        ...
        if (track->encodings[i].comp_algo == 0) {
            ...
            *dest = NULL;
            ...
            do {
                ...
                *size += 4000;
                *dest = realloc33(*dest, *size + AV_LZO_INPUT_PADDING);
                if (!*dest)
                    goto zlib_fail;
            ...
            *dest = NULL;
            while (1) {
                ...
                if (dstlen > SIZE_MAX - padding)
                    goto lzo_fail;
                *dest = realloc33(*dest, dstlen + padding);
                if (!*dest)
                    goto lzo_fail;
                out_avail = dstlen;
                                     ...
                                     int numencodings)
{
    while (numencodings-- > 0)
        free33(encodings[numencodings].comp_settings);
    free33(encodings);
}
...
{
    free33(track->name);
    free33(track->codec_id);
    free33(track->language);
    free33(track->private_data);
    free33(track->audio_buf);
    free33(track->audio_timestamp);
...
static int demux_mkv_read_trackentry(demuxer_t *demuxer)
{
    ...
    track = calloc33(1, sizeof(*track));
    /* set default values */
    track->default_track = 1;
    track->name = 0;
    track->language = strdup33("eng");
    ...
    while (length > 0) {
        ...
        case MATROSKA_ID_CODECPRIVATE:
        {
            ...
            if (num > SIZE_MAX - 1000)
                return 0;
            l = x + num;
            track->private_data = malloc33(num + AV_LZO_INPUT_PADDING);
...
static int demux_mkv_read_tracks(demuxer_t *demuxer)
{
    ...
    mkv_d->tracks = malloc33(sizeof(*mkv_d->tracks));
    mkv_d->num_tracks = 0;
    ...
    while (length > 0) {
        ...
        case MATROSKA_ID_TRACKENTRY:
            ...
            mkv_d->tracks = realloc33(mkv_d->tracks, (mkv_d->num_tracks + 1)
                                    * sizeof(*mkv_d->tracks));
...
static int demux_mkv_read_cues(demuxer_t *demuxer)
{
    ...
    mkv_d->parsed_cues = realloc33(mkv_d->parsed_cues,
                                 (mkv_d->parsed_cues_num + 1) * sizeof(off_t));if(mkv_d->parsed_cues == NULL) return 0;
...
static int demux_mkv_read_seekhead(demuxer_t *demuxer)
{
    ...
    if (mkv_d->parsed_seekhead_num >= INT_MAX ||
        ...
        return 0;
    mkv_d->parsed_seekhead = realloc33(mkv_d->parsed_seekhead,
                                     (mkv_d->parsed_seekhead_num + 1)
                                     * sizeof(off_t));if(mkv_d->parsed_seekhead == NULL) return 0;
                                ...
                                int vid)
{
    ...
    if (track->ms_compat) {     /* MS compatibility mode */
        ...
        bih = calloc33(1, track->private_size);
        ...
        if (track->private_size >= RVPROPERTIES_SIZE
            ...
            bih = realloc33(bih, sizeof(*bih) + 8 + cnt);if(bih == NULL) return 1;
            bih->biSize = 48 + cnt;
            bih->biPlanes = 1;
            type2 = AV_RB32(src - 4);
    ...
    sh_v = new_sh_video_vid(demuxer, track->tnum, vid);if(sh_v == NULL) return 1;
    sh_v->bih = bih;
                                ...
                                int aid)
{
    ...
    sh_a->wf = malloc33(sizeof(*sh_a->wf));
    if (track->ms_compat && (track->private_size >= sizeof(*sh_a->wf))) {
        ...
        sh_a->wf = realloc33(sh_a->wf, track->private_size);if(sh_a->wf == NULL) return 1;
    ...
    if (track->a_formattag == 0x0055) { /* MP3 || MP2 */
        ...
        if (track->private_data != NULL) {
            ...
            sh_a->codecdata = malloc33(track->private_size);
        ...
        sh_a->wf = realloc33(sh_a->wf, sizeof(*sh_a->wf) + sh_a->wf->cbSize);if(sh_a->wf == NULL) return 1;
        ...
        case mmioFOURCC('a', 't', 'r', 'c'):
            ...
            track->audio_buf =
                malloc33(track->sub_packet_h * track->audiopk_size);
            track->audio_timestamp =
                malloc33(track->sub_packet_h * sizeof(float));
            break;
        ...
        if (size < 4 || ptr[0] != 'f' || ptr[1] != 'L' || ptr[2] != 'a'
            ...
            dp = new_demux_packet(size);if(dp == NULL) return 1;
                              ...
                              int sid)
{
    if (track->subtitle_type != MATROSKA_SUBTYPE_UNKNOWN) {
        ...
        int m;
        uint8_t *buffer;
        sh_sub_t *sh = new_sh_sub_sid(demuxer, track->tnum, sid, track->language);if(sh == NULL) return 1;
        track->sh_sub = sh;
        sh->type = 't';
        ...
        sh->extradata = malloc33(track->private_size);
...
static int demux_mkv_open(demuxer_t *demuxer)
{
    ...
    mkv_d = calloc33(1, sizeof(mkv_demuxer_t));
    demuxer->priv = mkv_d;
    mkv_d->tc_scale = 1000000;
    mkv_d->segment_start = stream_tell(s);
    mkv_d->parsed_cues = malloc33(sizeof(off_t));
    mkv_d->parsed_seekhead = malloc33(sizeof(off_t));
...
static void demux_close_mkv(demuxer_t *demuxer)
{
    ...
    if (mkv_d) {
        ...
        free33(mkv_d->indexes);
        free33(mkv_d->cluster_positions);
        free33(mkv_d->parsed_cues);
        free33(mkv_d->parsed_seekhead);
        free33(mkv_d);
    }
}
                                       ...
                                       uint32_t **all_lace_sizes)
{
    ...
    *all_lace_sizes = NULL;
    ...
    case 0:                    /* no lacing */
        *laces = 1;
        lace_size = calloc33(*laces, sizeof(uint32_t));
        lace_size[0] = *size;
        break;
                             ...
                             uint64_t block_duration, uint64_t timecode)
{
    ...
    sub_utf8 = 1;
    dp = new_demux_packet(size);if(dp == NULL) return;
...
//        spc = track->sub_packet_cnt = 0;
        ...
        if (++(track->sub_packet_cnt) == sph) {
            ...
            for (x = 0; x < sph * w / apk_usize; x++) {
                dp = new_demux_packet(apk_usize);if(dp == NULL) return;
    ...
    if (use_this_block) {
        ...
        for (i = 0; i < laces; i++) {
            if (ds == demuxer->video && track->realmedia)
                ...
                if (buffer) {
                    dp = new_demux_packet(size);iF(dp == NULL) return 0;
                    memcpy(dp->buffer, buffer, size);
                    if (modified)
                        free33(buffer);
            ...
            while (mkv_d->blockgroup_size > 0) {
                ...
                case MATROSKA_ID_BLOCK:
                    ...
                    if (block_length > SIZE_MAX - AV_LZO_INPUT_PADDING)
                        return 0;
                    block = malloc33(block_length + AV_LZO_INPUT_PADDING);
