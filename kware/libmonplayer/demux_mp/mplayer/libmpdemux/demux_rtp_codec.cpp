		...
		unsigned int& configSize )
{
	...
	if( configStr == NULL || *configStr == '\0' )
		return NULL;
	psz = dup = strdup33( configStr );if(dup == NULL) return NULL;
		...
		unsigned size)
{
	...
	if (!size || size > INT_MAX - sizeof(BITMAPINFOHEADER))
		return bih;
	bih = (BITMAPINFOHEADER*)realloc33(bih, sizeof(BITMAPINFOHEADER) + size);
	if (!bih)
		return original;
	bih->biSize = sizeof(BITMAPINFOHEADER) + size;
#if 1	
	OS_PRINTF("[%s]---------extradata_size=%d, extradata: %x %x %x %x %x %x %x %x\n",\
			__func__,size,extraData[0],extraData[1],extraData[2],\
...
void rtpCodecInitialize_video(demuxer_t* demuxer,
	...
	BITMAPINFOHEADER* bih
		= (BITMAPINFOHEADER*)calloc33(1,sizeof(BITMAPINFOHEADER));
	...
	if (strcmp(subsession->codecName(), "MPV") == 0) {
		...
		if(configData) sh_video->bih = bih = insertVideoExtradata(bih, configData, configLen);
#ifdef CONFIG_FFMPEG_MP
		int fooLen;
...
void rtpCodecInitialize_audio(demuxer_t* demuxer,
	...
	WAVEFORMATEX* wf = (WAVEFORMATEX*)calloc33(1,sizeof(WAVEFORMATEX));
	sh_audio->wf = wf;
	...
	if (strcmp(subsession->codecName(), "MPA") == 0 ||
		...
		while (pos+8 < endpos) {
			...
			if (!memcmp(pos+4, "wave", 4) && fourcc==mmioFOURCC('Q','D','M','2') &&
					atomLength > 8 &&
					atomLength <= INT_MAX) {
				sh_audio->codecdata = (unsigned char*) malloc33(atomLength-8);
...
#else
	OS_PRINTF("[%s] -- delete code of computing the fps!\n",__func__);
#endif
}
