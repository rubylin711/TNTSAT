...
static demuxer_t* demux_open_roq(demuxer_t* demuxer)
{
  ...
  roq_data_t *roq_data = malloc33(sizeof(roq_data_t));
  int chunk_id;
  int chunk_size;
  int chunk_arg;
  int last_chunk_id = 0;
  ...
  while (!stream_eof(demuxer->stream))
  {
      ...
      (chunk_id == RoQ_SOUND_STEREO))
    {
      ...
      // index the chunk
      roq_data->chunks = realloc33(roq_data->chunks,
        (roq_data->total_chunks + 1) * sizeof (roq_chunk_t));   if(roq_data->chunks == NULL) return NULL;
