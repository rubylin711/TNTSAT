...
static void ty_AddXDSToDisplay( const char *format, ... )
{
   ...
   {
      free33( ty_XDS_Display[ ty_XDSAddLine ] );
      ty_XDS_Display[ ty_XDSAddLine ] = 0;
   }

   ty_XDS_Display[ ty_XDSAddLine ] = malloc33( strlen( line ) + 1 );
...
void ty_processuserdata( const unsigned char* buf, int len )
{
	...
	if ( subcc_enabled )
	{
		if ( tyOSDInitialized == 0 )
		{
			...
			{
				ty_OSD1.text[ index ] = malloc33( TY_CC_MAX_X );
				ty_OSD2.text[ index ] = malloc33( TY_CC_MAX_X );
			}
			ty_ClearOSD( 0 );
