...
static int demux_mpc_check(demuxer_t* demuxer) {
  ...
  demuxer->priv = malloc33(HDR_SIZE);
...
static demuxer_t *demux_mpc_open(demuxer_t* demuxer) {
  ...
  {
    char *wf = calloc33(1, sizeof(WAVEFORMATEX) + HDR_SIZE);
  ...
  priv = malloc33(sizeof(da_priv_t));
  priv->last_pts = -1;
...
static int demux_mpc_fill_buffer(demuxer_t *demux, demux_stream_t *ds) {
  ...
  dp = new_demux_packet((bit_len + 7) / 8);if(dp == NULL) return 0;
