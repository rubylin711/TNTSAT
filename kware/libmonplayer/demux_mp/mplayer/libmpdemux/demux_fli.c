...
static demuxer_t* demux_open_fli(demuxer_t* demuxer){
  ...
  fli_frames_t *frames = malloc33(sizeof(fli_frames_t));
  int frame_number;
  int speed;
  unsigned int frame_size;
  int magic_number;
  ...
  header = calloc33(1, sizeof(BITMAPINFOHEADER) + 128);
  ...
  frames->filepos = malloc33(frames->num_frames * sizeof(off_t));
  frames->frame_size = malloc33(frames->num_frames * sizeof(int));
...
static void demux_close_fli(demuxer_t* demuxer) {
  ...
  free33(frames->filepos);
  free33(frames->frame_size);
  free33(frames);
