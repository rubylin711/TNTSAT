...
while(1){
  ...
  if(0==size2)
  {
        //OS_PRINTF("***********    Error syntax, quit!   **********\n");
        break;
  }
  /*
  if(mmioFOURCC('R','I','F','F') == id)
  {
        break;
  }
  */
  ...
  switch(id){
    ...
    case mmioFOURCC('i', 'n', 'd', 'x'): {
      ...
      if(priv->suidx) s = &priv->suidx[priv->suidx_size-1]; if(s == NULL) break;
      ...
      *(uint32_t *)s->dwChunkId = stream_read_dword_le(demuxer->stream);
      ...
      s->aIndex = (avisuperindex_entry *)calloc33(s->nEntriesInUse, sizeof (avisuperindex_entry)); if(s->aIndex == NULL) break;
      s->stdidx = (avistdindex_chunk *)calloc33(s->nEntriesInUse, sizeof (avistdindex_chunk));
    ...
    case ckidSTREAMFORMAT: {      // read 'strf'
      if(last_fccType==streamtypeVIDEO){
        ...
        sh_video->bih=(BITMAPINFOHEADER*)calloc33(sh_video->bih_size, 1);
//        sh_video->bih=malloc33(chunksize); memset(sh_video->bih,0,chunksize);
        mp_msg(MSGT_HEADER, MSGL_V, "Found 'bih', %u bytes of %zu\n",
               chunksize, sizeof(*sh_video->bih));
        stream_read(demuxer->stream,(char*) sh_video->bih,chunksize);
        esize=sh_video->bih->biSize;
        size=sh_video->bih_size;
        if(size > 10*4 && size<(1<<30) && size <= chunksize)
        {
            if(esize == size-1 && (esize&1)) sh_video->sh.extradata_size= esize - 10*4;
            else                             sh_video->sh.extradata_size=  size - 10*4;
            sh_video->sh.extra_data= (unsigned char *)calloc33(sh_video->sh.extradata_size+16, 1);

            if(sh_video->sh.extra_data == NULL)
            {
                return -1;              // for tsscan
            }
            memcpy(sh_video->sh.extra_data,sh_video->bih+1,sh_video->sh.extradata_size);
        }
        if(!(!sh_video->sh.extra_data || sh_video->sh.extradata_size < 6))
        {
            sh_video->sh.needfilter = 1;
        }
...
//        if(demuxer->video->id==-1) demuxer->video->id=stream_id;
	...
	case mmioFOURCC('D', 'I', 'V', '2'):
	  ...
	  break;
        ...
        if(idxfix_divx)
            sh_video->sh.needfilter = 2;
      } else
      if(last_fccType==streamtypeAUDIO){
	unsigned wf_size = chunksize<sizeof(*sh_audio->wf)?sizeof(*sh_audio->wf):chunksize;
        sh_audio->wf=(WAVEFORMATEX*)calloc33(wf_size,1);
//        sh_audio->wf=malloc33(chunksize); memset(sh_audio->wf,0,chunksize);
	...
	if (sh_audio->wf->cbSize != 0 &&
	    ...
	    sh_audio->wf=(WAVEFORMATEX*)realloc33(sh_audio->wf, sizeof(*sh_audio->wf)+sh_audio->wf->cbSize);
	}
...
//        if(demuxer->audio->id==-1) demuxer->audio->id=stream_id;
	...
	VideoPropHeader *vprp = (VideoPropHeader *)malloc33(chunksize);
	unsigned int i;
    ...
    if(index_mode && !priv->isodml){
      ...
      priv->idx=(void*)malloc33(priv->idx_size<<4);
      if(NULL == priv->idx)
      {
          printf("\n malloc for idx fail !!! \n");
          return -1;
...
if (priv->isodml && (index_mode==-1 || index_mode==0 || index_mode==1)) {
    ...
    if (priv->idx_size) free33(priv->idx);
    priv->idx_size = 0;
    priv->idx_offset = 0;
    priv->idx = NULL;
	...
	for (j=0; j<cx->nEntriesInUse; j++) {
	    ...
	    cx->stdidx[j].aIndex = (avistdindex_entry *)malloc33(cx->stdidx[j].nEntriesInUse*sizeof(avistdindex_entry));
    ...
    idx = priv->idx = (void*)malloc33(priv->idx_size * sizeof (AVIINDEXENTRY));
    //yliu:malloc fail  return NULL;
    if(NULL == priv->idx)
    {
        // free unneeded stuff
        cx = &priv->suidx[0];
        do {
    	for (j=0;j<cx->nEntriesInUse;j++)
    	    if (cx->stdidx[j].nEntriesInUse) free33(cx->stdidx[j].aIndex);
        free33(cx->aIndex);
    	free33(cx->stdidx);
...
if (index_file_load) {
  ...
  priv->idx=(void*)malloc33(priv->idx_size*sizeof(AVIINDEXENTRY));
  if (!priv->idx) {
...
//      priv->idx_size+=32;
      ...
      priv->idx=(void*)realloc33(priv->idx,priv->idx_size*sizeof(AVIINDEXENTRY));
