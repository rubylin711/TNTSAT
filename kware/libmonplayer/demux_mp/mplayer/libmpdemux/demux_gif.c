...
static int demux_gif_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds)
{
  ...
  dp = new_demux_packet(priv->w * priv->h); if(dp == NULL) return 0;
  buf = calloc33(gif->Image.Width, gif->Image.Height);if(buf == NULL) return 0;
  if (priv->useref)
...
{
  gif_priv_t *priv = calloc33(1, sizeof(gif_priv_t));
  ...
  sh_video->bih = malloc33(sizeof(*sh_video->bih) + (256 * 4));
  ...
  priv->refimg = malloc33(priv->w * priv->h);
