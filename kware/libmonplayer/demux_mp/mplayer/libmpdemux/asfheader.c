...
{
  char* outbuf = (char*)calloc33(inlen, 2);
  char* q;
  int i;
...
int asf_check_header(demuxer_t *demuxer){
  ...
  struct asf_priv* asf = (struct asf_priv*)calloc33(1,sizeof(*asf));
...
//      avg_ft = AV_RL64(buffer); // provided in 100ns units
      ...
      if (is_video) {
        ...
        asf->vid_repdata_sizes = (int *)malloc33(payct*sizeof(int));
      } else {
        asf->aud_repdata_count = payct;
        asf->aud_repdata_sizes = (int *)malloc33(payct*sizeof(int));
      }
...
static int asf_init_audio_stream(demuxer_t *demuxer,struct asf_priv* asf, sh_audio_t* sh_audio, ASF_stream_header_t *streamh, int *ppos, uint8_t** buf, char *hdr, unsigned int hdr_len)
{
  ...
  sh_audio->wf=(WAVEFORMATEX*)calloc33(FFMAX(streamh->type_size, sizeof(*sh_audio->wf)), 1);
...
int read_asf_header(demuxer_t *demuxer,struct asf_priv* asf){
  ...
  if ((pos = find_asf_guid(hdr, asf_ext_stream_audio, 0, hdr_len)) >= 0)
  {
    ...
    if (sh_pos > 0) {
      ...
      if(sh_audio) sh_audio->needs_parsing = 1;
  ...
  while ((pos = find_asf_guid(hdr, asf_stream_header_guid, pos, hdr_len)) >= 0)
  {
	...
	//if(demuxer->audio->id==-1) demuxer->audio->id=streamh.stream_no & 0x7F;
      ...
      case ASF_GUID_PREFIX_video_stream: {
        unsigned int len;
        float asp_ratio;
        sh_video_t* sh_video=new_sh_video(demuxer,streamh->stream_no & 0x7F); if(sh_video == NULL) break;
...
//        sh_video->bih=malloc33(chunksize); memset(sh_video->bih,0,chunksize);
        sh_video->bih=(BITMAPINFOHEADER*)calloc33((len<sizeof(*sh_video->bih))?sizeof(*sh_video->bih):len,1);
  ...
  if (pos >= 0) {
      ...
      asf->packet=(unsigned char*)malloc33(asf->packetsize); // !!!
  ...
  if (pos >= 0) {
        ...
        ptr += sizeof(uint16_t);
        if(stream_count > max_streams) stream_count = (uint16_t)max_streams;
        if(stream_count > 0)
              streams = (uint32_t*)malloc33(2*stream_count*sizeof(uint32_t));
