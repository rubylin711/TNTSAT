...
static void asf_descrambling(unsigned char **src,unsigned len, struct asf_priv* asf){
  ...
  if (len > UINT_MAX - MP_INPUT_BUFFER_PADDING_SIZE)
	return;
  dst = (unsigned char *)malloc33(len + MP_INPUT_BUFFER_PADDING_SIZE);
...
static void demux_asf_append_to_packet(demux_packet_t* dp,unsigned char *data,int len,int offs)
{
  ...
  dp->buffer=(unsigned char *)realloc33(dp->buffer,dp->len+len+MP_INPUT_BUFFER_PADDING_SIZE);
...
//              p_str  = (char *)(&biComp);
              if (strstr(((char *) &biComp), "h264"))
              {
                    extraSize = sh_video->bih->biSize - sizeof(BITMAPINFOHEADER);

                    if(extraSize>0)
                    {
                        demux_packet_t* dp_new;
                        dp_new=new_demux_packet(len + extraSize);if(dp_new == NULL) return 0;
                        fast_memcpy(dp->buffer,data,len);
                        memcpy(dp_new->buffer, extraData, extraSize);
                        memcpy(dp_new->buffer+extraSize, dp->buffer, len);
                        ds->asf_packet=dp_new;
...
static void get_payload_extension_data(demuxer_t *demux, unsigned char** pp, unsigned char id, unsigned int seq, int *keyframe, uint64_t *seg_time){
    struct asf_priv* asf = demux->priv;
    uint64_t payload_time = 0; //100ns units
...
static void demux_close_asf(demuxer_t *demuxer) {
    ...
    free33(asf->aud_repdata_sizes);
    free33(asf->vid_repdata_sizes);
    free33(asf->packet);
    free33(asf);
}
