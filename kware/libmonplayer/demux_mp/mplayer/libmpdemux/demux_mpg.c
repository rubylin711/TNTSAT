...
#define MAX_LPCM_CODEC_DATA_LEN    4
...
static float read_first_mpeg_pts_at_position(demuxer_t *demuxer, off_t stream_pos)
{
    ...
    while (found < 3 && !s->eof && (fabsf(found_pts2 - found_pts1) < MAX_PTS_DIFF_FOR_CONSECUTIVE) && (fabsf(found_pts3 - found_pts2) < MAX_PTS_DIFF_FOR_CONSECUTIVE) && (stream_tell(s) < stream_pos + TIMESTAMP_PROBE_LEN) && ds_fill_buffer(demuxer->video)) {
...
static demuxer_t *demux_mpg_open(demuxer_t *demuxer)
{
    ...
    mpg_d = calloc33(1, sizeof(mpg_demuxer_t));
    ...
    if (mpg_d) {
        ...
        if (demuxer->seekable && (demuxer->stream->type == STREAMTYPE_FILE || demuxer->stream->type == STREAMTYPE_VCD || demuxer->stream->type == STREAMTYPE_STREAM) && demuxer->movi_start != demuxer->movi_end) {
            ...
            if (first_pts != -1.0) {
                ...
                if (middle_pts != -1.0) {
                    ...
                    // Bug 100234:for get the last pts
                    while (!demuxer->video->eof && (is_file_seq_exit() == FALSE)) {
                        ds_fill_buffer(demuxer->video);
                        final_pts = (demuxer->video)->pts;
                    }
                    if (final_pts != -1.0) {
            ...
            if (!mpg_d->has_valid_timestamps) {
                if (mpg_d->first_pts == -1.0) {
                    mpg_d->last_pts = -1.0;
                    mpg_d->first_pts = -1.0;
                    demuxer->video->eof = 0;
                    stream_seek(s, demuxer->movi_start);
                    while (!demuxer->video->eof && get_pts_times++ < 20 && (is_file_seq_exit() == FALSE)) {
                        ds_fill_buffer(demuxer->video);
                        if (mpg_d->last_pts != -1.0) {
                            mpg_d->first_pts = mpg_d->last_pts;
                            break;
                        }
                    }
                }
                // mtos_printk("[%s] line[%d],mpg_d->last_pts[%lld]pos[%lld]\n", __func__,__LINE__,(int64_t)(mpg_d->last_pts*1000),demuxer->movi_start);
                while (mpg_d->last_pts != -1.0 && loop_size_step < 3 && (is_file_seq_exit() == FALSE)) {
                    demux_flush(demuxer);
                    mpg_d->last_pts = -1.0;
                    demuxer->video->eof = 0;
                    stream_seek(s, demuxer->movi_end - 50000 * (1 + 5 * loop_size_step++));
                    get_pts_times = 0;
                    while (!demuxer->video->eof && get_pts_times++ < 20 && (is_file_seq_exit() == FALSE)) {
                        ds_fill_buffer(demuxer->video);
                        if (mpg_d->last_pts != -1.0) {
                            mpg_d->first_to_final_pts_len = mpg_d->last_pts - mpg_d->first_pts;
                            mpg_d->has_valid_timestamps = 1;
                            // mtos_printk("[%s] line[%d],mpg_d->first_to_final_pts_len[%lld]\n", __func__,__LINE__,(int64_t)(mpg_d->first_to_final_pts_len*1000));
                            break;
                        }
                    }
                    if (mpg_d->last_pts != -1.0)
...
/* 20 bits may get wrong bytes */
const static unsigned char lpcm_quantization_map[] = {
    16, 20, 24, 0
};

const static unsigned char lpcm_channel_map[] = {
    1, 2, 2, 2, 5, 6, 6, 8
};

const static int lpcm_samplerate_map[] = {
    44100, 48000, 96000, 0
};

static int mpg_read_lpcm_header(demuxer_t *demux)
{
    int idx;
    sh_audio_t *sh_audio = (sh_audio_t *) demux->audio->sh;
    unsigned char  hdr[MAX_LPCM_CODEC_DATA_LEN] = {0};

    // save audio header as codec_extradata!
    if (!(sh_audio->codec_extradata_size)) {
        sh_audio->codec_extradata = (unsigned char*) malloc33(MAX_LPCM_CODEC_DATA_LEN);
        if (NULL == sh_audio->codec_extradata) {
            sh_audio->codec_extradata_size = 0;
            mp_dbg(MSGT_DEMUX, MSGL_ERR, "mpg codec data malloc fail\n");
        } else {
            sh_audio->codec_extradata_size = MAX_LPCM_CODEC_DATA_LEN;
        }
    }

    // read LPCM header:
    // bits:emphasis[1], mute[1], rvd[1], frame number[5]:
    ...
    if (NULL == sh_audio->wf) {
        sh_audio->wf = (WAVEFORMATEX *) calloc33 (sizeof(WAVEFORMATEX), 1);
    }

    if (NULL != sh_audio->wf) {
        sh_audio->wf->nChannels      = lpcm_channel_map[(hdr[1]) & 0x07];
        sh_audio->wf->nSamplesPerSec = lpcm_samplerate_map[(hdr[1] >> 4) & 0x03];
        sh_audio->wf->wBitsPerSample = lpcm_quantization_map[(hdr[1] >> 6) & 0x03];
    }

    if (NULL != sh_audio->codec_extradata) {
        for (idx = 0; idx < MAX_LPCM_CODEC_DATA_LEN; idx++) {
            sh_audio->codec_extradata[idx] = hdr[idx];
        }
    }

    return MAX_LPCM_CODEC_DATA_LEN;
}

static int pcm_dvd_audio_parse(demuxer_t *demux)
{
    ...
    sh_audio_t *sh_audio = (sh_audio_t *) demux->audio->sh;
    unsigned char  header[MAX_PCM_DVD_CODEC_DATA_LEN] = {0};

    header[0] = stream_read_char(demux->stream);
    header[1] = stream_read_char(demux->stream);
    header[2] = stream_read_char(demux->stream);
    MLOGD("pcm_dvd_parse_header: header = %02x%02x%02x\n", header[0], header[1], header[2]);
    /*
    ...
    unsigned int bits_per_coded_sample = 16 + (header[1] >> 6 & 3) * 4;
    if (bits_per_coded_sample == 28) {
        MLOGE("PCM DVD unsupported sample depth %i\n", bits_per_coded_sample);
        return 0;
    }
    // unsigned int sample_fmt = bits_per_coded_sample == 16 ? AV_SAMPLE_FMT_S16 : AV_SAMPLE_FMT_S32;
    unsigned int bits_per_raw_sample = bits_per_coded_sample;
    /* get the sample rate */
    unsigned int sample_rate = frequencies[header[1] >> 4 & 3];
    /* get the number of channels */
    unsigned int channels = 1 + (header[1] & 7);
    /* calculate the bitrate */
    unsigned int bit_rate = channels * sample_rate * bits_per_coded_sample;
    if (NULL == sh_audio->wf) {
        MLOGI("PCM DVD sr:%d ch:%d bits:%d br:%d\n",
            sample_rate, channels, bits_per_raw_sample, bit_rate);
        sh_audio->wf = (WAVEFORMATEX *) calloc33 (sizeof(WAVEFORMATEX), 1);
    }

    if (NULL != sh_audio->wf) {
        sh_audio->wf->nChannels      = channels;
        sh_audio->wf->nSamplesPerSec = sample_rate;
        sh_audio->wf->wBitsPerSample = bits_per_raw_sample;
    }
    return MAX_PCM_DVD_CODEC_DATA_LEN;
}
...
static int demux_mpg_read_packet(demuxer_t *demux, int id)
{
    ...
    if ((c >> 6) == 1) { // Read (skip) STD scale & size value
        len -= 2;
        (void)stream_read_char(demux->stream);
        c = stream_read_char(demux->stream);
    }
    ...
    if ((c >> 4) == 2) {
            ...
            if ((aid & 0xE0) == 0x20) {
                ...
                if (demux->audio->id == aid) {
                    ...
                    if ((aid & 0xE0) == 0xA0 && len >= 3) {
                        ...
                        // save audio header as codec_extradata!
                        if (!((sh_audio_t *)(ds->sh))->codec_extradata_size) {
                            ((sh_audio_t *)(ds->sh))->codec_extradata = malloc33(3);
                            ((sh_audio_t *)(ds->sh))->codec_extradata_size = 3;
                        }

                        hdr = ((sh_audio_t *)(ds->sh))->codec_extradata;
                        // read LPCM header:
        ...
        (void)read_mpeg_timestamp(demux->stream, c);
        len -= 4 + 1 + 4;
    } else if ((c >> 6) == 2) {
        int pts_flags;
        int hdrlen;
    ...
    if (ds) {
...
#if 1 // linda zhu add, fix bug 49003, add sequence_header(0x1b3) before every group_of_picture_header(0x1b8) of mpg file
        if (id >= 0x1E0 && id <= 0x1EF) {
            int i;
            unsigned int state = -1;
            unsigned char *buf = dp->buffer;
            int sequence_header_flag = 0;
            ...
            if (ds->sh) {
                ...
                if (!sh->sh.extradata_parsed) {
                    ...
                    if (sh->sh.extradata_parsed == 1) {
                        state = -1;
                        for (i = start_i; i < dp->len; i++) {
                            state = (state << 8) | buf[i];
                            if (state == 0x1B7 || state == 0x1B8 || state == 0x100) {
                                end_i = i;
                                vol_len = end_i - start_i;
                                sh->codec_extradata_size = vol_len;
                                sh->codec_extradata = (unsigned char *)malloc33(vol_len + 1);
                                if (sh->codec_extradata) {
                                    memset(sh->codec_extradata, 0x00, vol_len + 1);
                                    memcpy(sh->codec_extradata, &(buf[start_i - 3]), vol_len);
                                }
                                break;
                            }
                        }
                    }
                } else if (sh->codec_extradata_size) {

                    state = -1;
                    for (i = 0; i < dp->len; i++) {
                        state = (state << 8) | buf[i];
                        if (state == 0x1B3) {
                            sequence_header_flag = 1;
                            break;
                        }
                        if (state == 0x1B8) {
                            break;
                    ...
                    if (i < dp->len && (sequence_header_flag == 0)) {
                        demux_packet_t *new_pkt = new_demux_packet(sh->codec_extradata_size + dp->len);
                        unsigned char *p_tmp = dp->buffer;
                        if (new_pkt->buffer == NULL) {
                            // free_demux_packet(new_pkt);
                            new_pkt = NULL;
                            return -1;
                        }
                        memcpy(new_pkt->buffer, p_tmp, i - 3);
                        memcpy(new_pkt->buffer + i - 3, sh->codec_extradata, sh->codec_extradata_size);
                        memcpy(new_pkt->buffer + i - 3 + sh->codec_extradata_size, p_tmp + i - 3, dp->len - i + 3);
        ...
        */
        ...
        if (demux->priv && set_pts) {
            ((mpg_demuxer_t *)demux->priv)->last_pts = pts / 90000.0f;
        }
...
static inline void update_stats(int head)
{
    ...
    } else if (head == 0x1B3 || head == 0x1B8) {
        ++num_mpeg12_startcode;
    } else if (head == 0x100) {
...
static int demux_mpg_probe(demuxer_t *demuxer)
{
    ...
    if (demux_mpg_open(demuxer)) {
        ...
        if (num_mp3audio_packets > 50 && num_mp3audio_packets > 2 * num_elementary_packets100 && abs(num_elementary_packets100 - num_elementary_packets101) > 2) {
            return file_format;
        }
...
static int demux_mpg_es_fill_buffer(demuxer_t *demux, demux_stream_t *ds)
{
    ...
    ds_read_packet(demux->video, demux->stream, io_stream_buffer_size, 0, demux->filepos, 0);
    return 1;
}
...
static int demux_mpg_gxf_fill_buffer(demuxer_t *demux, demux_stream_t *ds)
{
    ...
    int len;
    demux->filepos = stream_tell(demux->stream);
    pack = new_demux_packet(io_stream_buffer_size);
    len = stream_read(demux->stream, pack->buffer, io_stream_buffer_size);

    if (len <= 0 || pack->buffer == NULL) {
    ...
    {
        register uint64_t state = (uint64_t)((ulong)demux->priv);
        ...
        demux->priv = (void *)((ulong)state);
        len = buf - pack->buffer;
    }

    if (len < io_stream_buffer_size) {
        int resiez_ret = resize_demux_packet(pack, len);
        if (resiez_ret == -1) {
            free_demux_packet(pack);
            pack = NULL;
            // mtos_printk("\n%s %d\n",__func__,__LINE__);
    ...
    do {
        ...
        if ((head & 0xFFFFFF00) != 0x100) {
            ...
            while (1) {
                ...
                // OS_PRINTF("%s __222 c %d__\n",__func__,c);
                if (c < 0) {
                    demux->stream->eof = 1; // EOF
                    break;
                }
                           ...
                           float audio_delay, int flags)
{
    ...
    float checkpts = 0;
    const static unsigned int MAX_SEEK_TRY_COUNT = 50;
    const static unsigned int MAX_SYNC_TRY_COUNT = 10000;
    unsigned int seek_try_cnt = 0;
    unsigned int sync_try_cnt = 0;

    MLOGD("[%s %d] flag:%d, rel_seek_secs:%f\n", __FUNCTION__, __LINE__, flags, rel_seek_secs);
    if (mpg_d) {
...
again_calc:
    ...
    if (flags && ps_trick_back) {
        newpts = newpts - 2; // seek before the rel_seek_secs
    }
    ...
    if (flags && ps_trick_back) {
        if (newpts < oldpts && newpos > oldpos) {
            newpos = oldpos - 2324 * 75 * 1; // 174.3 kbyte/se
        }
    }

    while (seek_try_cnt++ < MAX_SEEK_TRY_COUNT) {
        ...
        int start_len;
        while (sync_try_cnt++ < MAX_SYNC_TRY_COUNT) {
            int i;
            ...
            } else if (sh_video->format == 0x10000004) { // mpeg4
                if (i == 0x1B0 || i == 0x1B3 || i == 0x1B5 || i == 0x1B6) {
                    if (flags && ps_trick_back) {

                        if (d_video->pts - rel_seek_secs > -0.001 && d_video->pts - rel_seek_secs < 0.001) {
                            if (!d_video->eof)
                                d_video->buffer_pos = d_video->buffer_pos - start_len;
                            break;

                        } else {
                            if (d_video->pts > rel_seek_secs) {
                                break;
                            }
                        }
                    } else {
        ...
        if (flags && ps_trick_back) {
            if (rel_seek_secs < mpg_d->first_pts && abs(d_video->pts - mpg_d->first_pts) <= 0.01) {
                // reached the head of stream.
                break;
            }
            if (d_video->pts > rel_seek_secs) {
                newpos = newpos - 2 * (d_video->pts - rel_seek_secs) * 2324 * 75;
                if (mpg_d) {
            ...
            checkpts = ((newpts - mpg_d->last_pts) < 0) ? (mpg_d->last_pts - newpts) : (newpts - mpg_d->last_pts); // linda zhu add, fix bug 58905, abs(x) is not applied to float
            if (!precision || checkpts < 0.5 || (mpg_d->last_pts == oldpts)) {
                if (!d_video->eof)
                    d_video->buffer_pos = d_video->buffer_pos - start_len;
                break;
            }
    ...
    if((rel_seek_secs < 0) && (oldpts <= newpts)) {
        rel_seek_secs -= 1.0;
        goto again_calc;
    }

    MLOGD("[%s %d] flag:%d, sync_try_cnt:%d, seek_try_cnt:%d\n", __FUNCTION__, __LINE__, flags, sync_try_cnt, seek_try_cnt);
}
...
static int demux_mpg_control(demuxer_t *demuxer, int cmd, void *arg)
{
    ...
    case DEMUXER_CTRL_SWITCH_AUDIO:
        if (!(mpg_d && mpg_d->num_a_streams > 1 && demuxer->audio && demuxer->audio->sh)) {
            ...
            if (*((int *)arg) < 0 || *((int *)arg) >= p_file_seq->audio_track_num) {
                ...
                audio_track_id = p_file_seq->audio_lang_array[*((int *)arg)].track_id;
                for (i = 0; i < mpg_d->num_a_streams; i++) {
                    if (audio_track_id == mpg_d->a_stream_ids[i]) {
                        break;
                    }
                }

                if (i < mpg_d->num_a_streams) {
                    sh_a = (sh_audio_t *)demuxer->a_streams[audio_track_id];
                }
            }
...
static demuxer_t *demux_mpg_ps_open(demuxer_t *demuxer)
{
    ...
    if (!sh_video->format && ps_probe > 0) {
        ...
        do {
            head = sync_video_packet(demuxer->video,NULL);
...

    {
        ...
        // OS_PRINTF("movie end %lld \n",demuxer->movi_end);
        // OS_PRINTF("movie start %lld \n",demuxer->movi_start);
        // mtos_printk_f("all pts %f \n",mpg_d->first_to_final_pts_len);
        if (mpg_d->first_to_final_pts_len > 0) {
            sh_video->i_bps = (demuxer->movi_end - demuxer->movi_start) / mpg_d->first_to_final_pts_len;
        }
    }
