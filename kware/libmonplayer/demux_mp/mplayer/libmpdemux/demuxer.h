...
#define MAX_64M_PACK_BYTES (0x400000)
#define MAX_PACK_BYTES (0x500000) //peacer modify yliu modify to 2M
                                  //peacer modify 7M //ybc modify 5M
...
#define DEMUXER_CTRL_GET_SEEK_MARKPOS    17
#define DEMUXER_CTRL_SET_SEEK_ENDPOS     18
#define DEMUXER_CTRL_GET_SEEK_STOPFLAG   19
#define DEMUXER_CTRL_SET_SEEK_STARTPOS   20
#define DEMUXER_CTRL_DISABLE_TRACK       21
#define DEMUXER_CTRL_SWITCH_PLAYLIST     22
#define DEMUXER_CTRL_GET_DL_BITRATE      23
#define DEMUXER_CTRL_GET_PLAYLIST        24

#define DEMUXER_BITRATE_CHANGE_NONE        0
#define DEMUXER_BITRATE_CHANGE_NOR_UP      1
#define DEMUXER_BITRATE_CHANGE_NOR_DOWN    2
#define DEMUXER_BITRATE_CHANGE_STEP_UP     3
#define DEMUXER_BITRATE_CHANGE_STEP_DOWN   4

#define DEMUXER_ADAPTIVE_BITRATE_UPDATED    0
#define DEMUXER_ADAPTIVE_QUERY_IDLE_TIME    1
#define DEMUXER_ADAPTIVE_ASYNC_OPEN         2
#define DEMUXER_ADAPTIVE_ASYNC_DONE         3
#define DEMUXER_ADAPTIVE_LIVE_MEDIA_UPDATED 4
#define DEMUXER_ADAPTIVE_QUERY_PLAY_STATE   5
#define DEMUXER_ADAPTIVE_SERVER_ERROR       6
...
enum {
    TYPE_ADAPTIVE_STREAM_NONE   = -1,
    TYPE_ADAPTIVE_STREAM_HLS    = 0,
    TYPE_ADAPTIVE_STREAM_DASH   = 1,
    TYPE_ADAPTIVE_STREAM_MTSURL = 2,
};
...
typedef struct demux_packet
{
    ...
    //int refcount;              // refcounter for the master packet, if 0, buffer can be free33()d
    //struct demux_packet *master; //pointer to the master packet if this one is a cloned one
    long long dts;               /* unit:microsecond, in arithmetic, long long run faster than double */
    int sample_size;
    struct demux_packet *next;
    void *ff_av_pkt;
} demux_packet_t;

#define BIG_PACK_LEN (614400) //600k
#define MAX_BIG_PACK_NUM (3)
typedef struct
{
    int buffer_pos;             // current buffer position
    int buffer_size;            // current buffer size
    int gop_seg_num;  //ts packet seg num
    unsigned int pts31bit;      //3//higher 31bit pts value
    unsigned int pts_valide;
    unsigned char *buffer;      // current buffer, never free33() it, always use free_demux_packet(buffer_ref);
    ...
    // ---- stream header ----
    void *sh;
    unsigned char *trickbuf;
    int trickbuf_pos;
    int trickbuf_size;
    int trickbuf_alloc_len;

    int big_packs;
    unsigned char is_secure;
    int sample_size;
    /* when track is disabled, we need discard the packet */
    int discard;
} demux_stream_t;
...
#define DEMUX_SUBTITLE_LEN 20
#define DEMUX_SUBTITLE_CNT 40 //tizhang@20180830 for 104443. Modified from 20 to 40

typedef struct
{
    char lang[DEMUX_SUBTITLE_LEN];
    char title[DEMUX_SUBTITLE_LEN];
    char code[DEMUX_SUBTITLE_LEN];
    int id;
} demux_subtitle;
...
{
    int cnt;
    demux_subtitle subtitle[DEMUX_SUBTITLE_CNT];
} demux_video_subtitle;

typedef struct demuxer_trick {
    mlzp_thread_t tid;
    int    speed;
    int    trick_cnt;
    int    trick_finish;
    mt_s64 start_pts;  /* unit:ms */
    mt_s64 start_time; /* unit:ms */

    mt_s64 last_start_pts; /* unit:ms */
} demuxer_trick_t;

typedef enum {
    ADAPTIVE_PLAYLIST_FLAGS_NONE   = 0x00,
    ADAPTIVE_PLAYLIST_FLAGS_ENABLE = 0x01,  /**< Is it enabled, 1:enable */
} ADAPTIVE_PLAYLIST_FLAGS_E;
...
typedef struct {
    ...
    ADAPTIVE_PLAYLIST_FLAGS_E flags;
} demuxer_adaptive_playlist_t;
...
typedef struct demuxer
{
    ...
    demux_video_subtitle subt_info;
    ...
    int type_adaptive_stream; /* 0 : dash, 1: hls 2 : mturl */
    demuxer_adaptive_playlist_t *adaptive_playlist;
    int idx_adaptive_playlist;
    int num_adaptive_playlist;

    void *priv; // fileformat-dependent data
    char **info;
    void *callback_ctx;
    int (*interrupt_callback)(void*);
    int (*app_message_callback) (void *s, int type, void *data, size_t data_size);

    struct demuxer_trick trick;

    MT_BOOL enbale_hr_mp3_seek;    // TRUE -- Enable high res seeking
} demuxer_t;
...
#define DEMUXER_LAVF_INVALID_VALUE 0x4444
#define DEMUXER_INVALID_TRACK_ID   0xFFFFffff

#define SAFEFREE(p) do{ if(p){free33(p);  (p)=NULL;} }while(0)
