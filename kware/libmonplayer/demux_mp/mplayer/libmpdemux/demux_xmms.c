...
static int disk_free33(void) { // vqf plugin sends more than it should
...
static int disk_open(AFormat fmt, int rate, int nch) {
    ...
    xmms_samplerate=(uint32_t)rate;
    xmms_channels=(uint32_t)nch;
    return 1;
}
...
static int demux_xmms_open(demuxer_t* demuxer) {
    ...
    xmms_priv=priv=malloc33(sizeof(xmms_priv_t));
    ...
    sh_audio->wf = w = malloc33(sizeof(*w));
...
static void demux_close_xmms(demuxer_t* demuxer) {
    ...
    if (priv != NULL) {
        if (priv->ip != NULL)
            priv->ip->stop();
        free33(priv); xmms_priv=demuxer->priv=NULL;
    }
    cleanup_plugins();
}
