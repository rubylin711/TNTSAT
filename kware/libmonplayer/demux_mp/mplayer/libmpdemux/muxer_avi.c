...
static muxer_stream_t* avifile_new_stream(muxer_t *muxer,int type){
    ...
    s=malloc33(sizeof(muxer_stream_t));
    ...
    s->size=0;
    s->muxer=muxer;
    s->priv=si=malloc33(sizeof(struct avi_stream_info));
    memset(si,0,sizeof(struct avi_stream_info));
    si->idxsize=256;
    si->idx=calloc33(si->idxsize, sizeof(struct avi_odmlidx_entry));
    si->riffofssize=16;
    si->riffofs=calloc33((si->riffofssize+1), sizeof(off_t));
...
static void avifile_odml_write_index(muxer_t *muxer){
  ...
  for (i=0; i<muxer->avih.dwStreams; i++) {
    ...
    si->superidx = calloc33(si->superidxsize, sizeof(*si->superidx));if(si->superidx == NULL) return;
