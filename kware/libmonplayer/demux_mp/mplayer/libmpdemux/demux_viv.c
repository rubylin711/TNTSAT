...
static void vivo_parse_text_header(demuxer_t *demux, int header_len)
{
    ...
    if (!demux->priv)
    {
	priv = malloc33(sizeof(vivo_priv_t));
	memset(priv, 0, sizeof(vivo_priv_t));
	demux->priv = priv;
	priv->supported = 0;
    }

    buf = malloc33(header_len);
    opt = malloc33(header_len);
    param = malloc33(header_len);
...
//	printf("packet!\n");
	    ...
	    return 0;
        dp->buffer=realloc33(dp->buffer,dp->len+len+MP_INPUT_BUFFER_PADDING_SIZE);
...
{		sh_video_t* sh=new_sh_video(demuxer,0);
		...
		sh->bih=calloc33(1, sizeof(*sh->bih));
		sh->bih->biSize=40;
		if (priv->width)
...
static void demux_close_vivo(demuxer_t *demuxer)
{
	...
	free33(priv->title);
	free33(priv->author);
	free33(priv->copyright);
	free33(priv->producer);
	free33(priv);
    }
    return;
}
