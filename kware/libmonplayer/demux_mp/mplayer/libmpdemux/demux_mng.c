 ...
 * \param[in] size memory block size
 */
static void demux_mng_free33(mng_ptr ptr, mng_size_t size)
{
    free33(ptr);
}
                                        ...
                                        mng_uint32 height)
{
    ...
    // allocate canvas
    mng_priv->canvas = malloc33(height * width * 4);
    if (!mng_priv->canvas) {
                                 ...
                                 demux_stream_t * ds)
{
    ...
    dp = new_demux_packet(mng_priv->height * mng_priv->width * 4);if(dp == NULL) return 0;
...
static demuxer_t * demux_mng_open(demuxer_t * demuxer)
{
    ...
    mng_priv = calloc33(1, sizeof(mng_priv_t));
    ...
    sh_video->bih                = malloc33(sizeof(*sh_video->bih));
