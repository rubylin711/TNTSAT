...
static mpeg_frame_t *init_frames(uint16_t num, size_t size)
{
	...
	tmp = calloc33(num, sizeof(mpeg_frame_t));
	if(tmp == NULL)
		return NULL;

	for(i=0; i < num; i++)
	{
		tmp[i].buffer = calloc33(1, size);
...
static muxer_stream_t* mpegfile_new_stream(muxer_t *muxer,int type){
  ...
  s = calloc33(1, sizeof(muxer_stream_t));
  if(!s) return NULL; // no mem!?
  if (!(s->b_buffer = malloc33(priv->packet_size)))
    goto init_fail;
  s->b_buffer_size = priv->packet_size;
  s->b_buffer_ptr = 0;
  s->b_buffer_len = 0;
  s->priv = calloc33(1, sizeof(muxer_headers_t));
  if(s->priv == NULL)
    goto init_fail;
  spriv = (muxer_headers_t *) s->priv;
  spriv->pack = malloc33(priv->packet_size);
  if(! spriv->pack)
    goto init_fail;
  spriv->buffer_track = calloc33(1, 4096*sizeof(buffer_track_t));
  ...
  if (type == MUXER_TYPE_VIDEO) {
    spriv->type = 1;
    spriv->last_pts = (uint64_t) (conf_init_vpts * 90 * 300) ;
    if(conf_init_vdelay) {
    ...
    spriv->last_pts = (uint64_t)(conf_init_apts * 90 * 300) ;
...
static void update_demux_bufsize(muxer_headers_t *spriv, uint64_t dts, int framelen, int type)
{
	...
	{
		buffer_track_t *tmp = realloc33(spriv->buffer_track, dim);
		if(!tmp)
		{
...
static int fill_last_frame(muxer_headers_t *spriv,  uint8_t *ptr, int len)
{
	...
	if(spriv->framebuf[idx].alloc_size < spriv->framebuf[idx].size + len)
	{
		if(spriv->framebuf[idx].size > SIZE_MAX - (size_t)len)
			return 0;
		spriv->framebuf[idx].buffer = realloc33(spriv->framebuf[idx].buffer, spriv->framebuf[idx].size + len);
...
static int add_frame(muxer_headers_t *spriv, uint64_t idur, uint8_t *ptr, int len, uint8_t pt, uint64_t dts, uint64_t pts)
{
	...
	if(idx >= spriv->framebuf_cnt)
	{
		...
		spriv->framebuf[spriv->framebuf_cnt].buffer = malloc33(len);
...
static void mpegfile_write_chunk(muxer_stream_t *s,size_t len,unsigned int flags, double dts_arg, double pts_arg)
{
	...
	{ // MUXER_TYPE_AUDIO
		...
		if(s->b_buffer_size - s->b_buffer_len < len)
		{
			...
			tmp = realloc33(s->b_buffer, len  + s->b_buffer_len);
			if(!tmp)
			{
...
{
	muxer_priv_t *priv;
	priv = calloc33(1, sizeof(muxer_priv_t));
	if(priv == NULL)
	return 0;
	priv->update_system_header = 1;

	//calloc33() already zero-ed all flags, so we assign only the ones we need
	...
	priv->buff = malloc33(priv->packet_size);
	if((priv->buff == NULL))
	{
