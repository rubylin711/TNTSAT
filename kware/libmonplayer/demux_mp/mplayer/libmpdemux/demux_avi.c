...
static int demux_avi_read_packet(demuxer_t *demux,demux_stream_t *ds,unsigned int id,unsigned int len,int idxpos,int flags){
  ...
  if(ds==demux->audio){
      if(priv->pts_corrected==0){
	      ...
	      float delay=0;
	      if(ds && ((sh_audio_t*)(ds->sh))->wf->nAvgBytesPerSec)   // for tsscan
...
static demuxer_t* demux_open_avi(demuxer_t* demuxer){
    ...
    avi_priv_t* priv=(avi_priv_t*)calloc33(1, sizeof(avi_priv_t));
   int header_res=0;
  demuxer->priv=(void*)priv;

  //---- AVI header:
  header_res = read_avi_header(demuxer,(demuxer->stream->flags & MP_STREAM_SEEK_BW)?index_mode:-2);
  //yiu: if read NULL, open fail
  if(-1 == header_res)
    return NULL;
