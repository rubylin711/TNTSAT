    ...
    while (*cmt) {
        ...
        if (!strncasecmp(*cmt, "LANGUAGE=", 9)) {
            ...
            if (index >= 0) {
                ...
                free33(ogg_d->text_langs[index]);
                ogg_d->text_langs[index] = strdup33(val);
                sh = d->s_streams[index];
                if (sh)
                    free33(sh->lang);
                if (sh)
                    sh->lang = strdup33(val);
            }
                                ...
                                int id, ogg_packet *pack)
{
    ...
    /// Send the packet
    dp = new_demux_packet(pack->bytes - (data - pack->packet));if(dp == NULL) return 0;
        ...
        while (ogg_stream_packetout(oss, &op) == 1) {
            ...
            if (index_mode == 2 && (flags || (os->vorbis && op.granulepos >= 0))) {
                if (ogg_d->num_syncpoint > SIZE_MAX / sizeof(ogg_syncpoint_t) - 1)
                    break;
                ogg_d->syncpoints = realloc_struct(ogg_d->syncpoints, (ogg_d->num_syncpoint + 1), sizeof(ogg_syncpoint_t));if(ogg_d->syncpoints == NULL) return;
...
static void fixup_vorbis_wf(sh_audio_t *sh, ogg_demuxer_t *od)
{
    ...
    sh->wf = calloc33(1, sizeof(*sh->wf) + len + len / 255 + 64);
    ...
    sh->wf = realloc33(sh->wf, sizeof(*sh->wf) + sh->wf->cbSize);if(sh->wf == NULL) goto release_buf;
...
int demux_ogg_open(demuxer_t *demuxer)
{
    ...
    demuxer->priv = ogg_d = calloc33(1, sizeof(*ogg_d));
    sync = &ogg_d->sync;
    page = &ogg_d->page;
    ...
    while (1) {
        ...
        ogg_d->subs = realloc_struct(ogg_d->subs, ogg_d->num_sub+1,
                                     sizeof(ogg_stream_t));if(ogg_d->subs == NULL) goto err_out;
        ...
        if (pack.bytes >= 7 && !strncmp(&pack.packet[1], "vorbis", 6)) {
            ...
            sh_a->wf         = calloc33(1, sizeof(*sh_a->wf) + pack.bytes);
            ...
            if (errorCode < 0) {
                ...
                sh_v->bih = calloc33(1, sizeof(*sh_v->bih));
            ...
            sh_a->wf = calloc33(1, sizeof(*sh_a->wf) + 34);
            sh_a->wf->wFormatTag = sh_a->format;
            sh_a->wf->cbSize     = 34;
            ...
            if (AV_RL32(pack.packet + 96) == 0x05589f80 && pack.bytes >= 184) {
                ...
                sh_a->wf         = calloc33(1, sizeof(*sh_a->wf) + extra_size);
            ...
            if (strncmp(st->streamtype, "video", 5) == 0) {
                ...
                ogg_d->n_text++;
                ogg_d->text_ids = realloc_struct(ogg_d->text_ids, ogg_d->n_text, sizeof(*ogg_d->text_ids));if(ogg_d->text_ids == NULL) goto err_out;
                ogg_d->text_ids[ogg_d->n_text - 1] = ogg_d->num_sub;
                ogg_d->text_langs = realloc_struct(ogg_d->text_langs, ogg_d->n_text, sizeof(*ogg_d->text_langs));if(ogg_d->text_langs == NULL) goto err_out;
...
demuxer_t *init_avi_with_ogg(demuxer_t *demuxer)
{
    ...
    ogg_d = calloc33(1, sizeof(*ogg_d));
    ogg_d->num_sub = 1;
    ogg_d->subs    = malloc33(sizeof(*ogg_d->subs));
    ...
    for (i = 0; i < 3; i++) {
        dp = new_demux_packet(hdrsizes[i]);if(dp == NULL) goto fallback;
...
static void demux_close_ogg(demuxer_t *demuxer)
{
    ...
    free33(ogg_d->syncpoints);
    free33(ogg_d->text_ids);
    if (ogg_d->text_langs) {
        for (i = 0; i < ogg_d->n_text; i++)
            free33(ogg_d->text_langs[i]);
        free33(ogg_d->text_langs);
    }
    free33(ogg_d);
}
