...
switch(video_codec){
 ...
 case VIDEO_MPEG4: {
   ...
   while(1){
      int i=sync_video_packet(d_video,NULL);
...
mpeg_header_parser:
   ...
   if(sync_video_packet(d_video ,NULL)==0x1B5){ // next packet is seq. ext.
 ...
 case VIDEO_VC1: {
   ...
   if(mp_vc1_decode_sequence_header(&picture, &videobuffer[4], videobuf_len-4)) {
     sh_video->bih = calloc33(1, sizeof(*sh_video->bih) + videobuf_len);
        ...
        *start=videobuffer; in_size=videobuf_len;
        ...
        while(videobuf_len<VIDEOBUFFER_SIZE-MAX_VIDEO_PACKET_SIZE){
          ...
          if((i&~0x60) == 0x107 && i != 0x107) {
            ...
            i=sync_video_packet(d_video ,NULL);
            if(!i) return -1;
            if(!read_video_packet(d_video)) return -1; // EOF
          }
          ...
          if((i&~0x60) == 0x101 || (i&~0x60) == 0x102 || (i&~0x60) == 0x105)
            ...
            in_picture = 1;
          if (in_picture) {
            i = sync_video_packet(d_video ,NULL) & ~0x60; // code of next packet
