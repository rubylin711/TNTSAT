     ...
     while ( fgets( fname,PATH_MAX,lst_f ) )
      {
       ...
       if ( stat( fname,&fs ) )
        ...
        {
         mf->names=realloc33( mf->names,( mf->nr_of_files + 1 ) * sizeof( char* ) );
         mf->names[mf->nr_of_files]=strdup33( fname );
         mf->nr_of_files++;
        }
      }
      fclose( lst_f );
...
//       mp_msg( MSGT_STREAM,MSGL_V,"[mf] added file %d.: %s\n",mf->nr_of_files,mf->names[mf->nr_of_files] );
 ...
 fname=malloc33( strlen( filename ) + 32 );
 ...
 if ( !strchr( filename,'%' ) )
  {
   ...
   if ( glob( fname,0,NULL,&gg ) )
    { free33( mf ); free33( fname ); return NULL; }

   mf->nr_of_files=gg.gl_pathc;
   mf->names=calloc33( gg.gl_pathc, sizeof( char* ) );
   ...
   for( i=0;i < gg.gl_pathc;i++ )
    {
     ...
     mf->names[i]=strdup33( gg.gl_pathv[i] );
