...
static int demux_film_fill_buffer(demuxer_t *demuxer, demux_stream_t *ds)
{
  ...
  if (film_chunk.syncinfo1 == 0xFFFFFFFF)
  {
   if(demuxer->audio->id>=-1){   // audio not disabled
    dp = new_demux_packet(film_chunk.chunk_size);if(dp == NULL) return 0;
  ...
  else
  {
    ...
    if (sh_video->format == mmioFOURCC('c', 'v', 'i', 'd'))
    {
      ...
      dp = new_demux_packet(film_chunk.chunk_size - length_fix_bytes);if(dp == NULL) return 0;
...
static demuxer_t* demux_open_film(demuxer_t* demuxer)
{
  ...
  film_data = malloc33(sizeof(film_data_t));
  ...
  while (header_size > 0)
  {
	...
	break; // audio disabled (or no soundcard)
      ...
      if (film_data->film_version)
      {
        ...
        if (audio_channels > 0)
        {
          ...
          sh_audio->wf = malloc33(sizeof(*sh_audio->wf));
    ...
    case CHUNK_STAB:
      ...
      film_data->chunks =
        calloc33(film_data->total_chunks, sizeof(film_chunk_t));if(film_data->chunks == NULL) return NULL;
      ...
      for (i = 0; i < film_data->total_chunks; i++)
      {
        ...
        if (counting_chunks)
        {
          ...
          else if ( sh_video && ((film_chunk.syncinfo1 & 0x7FFFFFFF) >= sh_video->fps))
            counting_chunks = 0;
          else
            film_data->chunks_per_second++;
        }
        ...
        if (film_chunk.syncinfo1 == 0xFFFFFFFF)
        {
	  if(demuxer->audio->id>=-1 && sh_audio)
          film_chunk.pts =
