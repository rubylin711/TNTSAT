...
static int y4m_check_file(demuxer_t* demuxer){
    ...
    demuxer->priv = malloc33(sizeof(y4m_priv_t));
    priv = demuxer->priv;
...
static int demux_y4m_fill_buffer(demuxer_t *demux, demux_stream_t *dsds) {
  ...
  dp = new_demux_packet(3*size/2);if(dp == NULL) return 0;
...
static demuxer_t* demux_open_y4m(demuxer_t* demuxer){
    ...
    priv->framenum = 0;
    priv->si = malloc33(sizeof(y4m_stream_info_t));
...
static void demux_close_y4m(demuxer_t *demuxer)
{
    ...
    free33(((y4m_priv_t*)demuxer->priv)->si);
    free33(demuxer->priv);
    return;
}
