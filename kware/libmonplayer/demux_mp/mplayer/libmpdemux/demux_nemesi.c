...
static demuxer_t* demux_open_rtp(demuxer_t* demuxer)
{
    ...
    RTSP_Error reply;
    rtsp_medium * media;
    Nemesi_DemuxerStreamData * ndsd = calloc33(1, sizeof(Nemesi_DemuxerStreamData));
    ...
    for (; media; media=media->next) {
        ...
        if (ptinfo->type == AU) {
            if (ndsd->session[NEMESI_SESSION_AUDIO] == NULL) {
                ...
                wf = calloc33(1,sizeof(*wf)+buff.len);
                wf->cbSize = buff.len;
            ...
            if (ndsd->session[NEMESI_SESSION_VIDEO] == NULL) {
                ...
                bih = calloc33(1,sizeof(*bih)+buff.len);
...
{
    demux_packet_t* dp = new_demux_packet(fr->len); if(dp == NULL) return;
