...
char *get_path(const char *filename){
	...
	len = (int)strlen(homedir) + (int)strlen(config_dir) + 1;
	if (filename == NULL) {
		if ((buff = (char *)malloc33(len)) == NULL)
			return NULL;
...
void set_codec_path(const char *path)
{
    ...
    codec_path = malloc33(strlen(path) + 1);
...
#ifdef __LINUX__
    if (len == 0)
        return (char *)strdup33("./");
#else
       if (len == 0)
        return strdup33("");
#endif
    dirname = (char *)malloc33(len + 1);
    if (!dirname)
        return NULL;
...
char *mp_path_join(const char *base, const char *path)
{
    ...
    if (path[0] == '/')
#endif
        return (char *)strdup33(path);

    ret = mp_dirname(base);
    if (!ret)
        return NULL;
    tmp = (char *)realloc33(ret, strlen(ret) + strlen(path) + 1);
    if (!tmp) {
        free33(ret);
        return NULL;
    }
