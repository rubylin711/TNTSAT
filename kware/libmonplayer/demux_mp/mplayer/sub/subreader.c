 ...
 * Some code cleanup & realloc33() by A'rpi/ESP-team
 *
...
static void sami_add_line(subtitle *current, char *buffer, char **pos) {
    ...
    *p = 0;
    ...
    if (*buffer && current->lines < SUB_MAX_TEXT)
        current->text[current->lines++] = strdup33(buffer);
    *pos = buffer;
}
...
static subtitle *sub_read_line_sami(stream_t* st, subtitle *current, int utf16) {
	...
	case 1: /* find (optional) "<P", skip other TAGs */
	    for  (; s && (*s == ' ' || *s == '\t') ; s++); /* strip blanks, if any */
	    if (s && *s == '\0') break;
	    if (s && *s != '<') { state = 3; p = text; continue; } /* not a TAG */
	    s++;
	    if ( s && (*s == 'P' || *s == 'p') ) { s++; state = 2; continue; } /* found '<P' */
	    for (; s && (*s != '>' && *s != '\0') ; s++); /* skip remains of non-<P> TAG */
	    if (s && *s == '\0')
	      break;
	    s++;
	    continue;
	...
	case 3: /* get all text until '<' appears */
	    ...
	    if (s && *s == '\0') break;
	    else if ( s && !strncasecmp (s, "<br>", 4)) {
                sami_add_line(current, text, &p);
		s += 4;
	    }
	    else if (s && (*s == '{') && !sub_no_text_pp) { state = 5; ++s; continue; }
	    else if (s && *s == '<') { state = 4; }
	    else if (s && !strncasecmp (s, "&nbsp;", 6)) { *p++ = ' '; s += 6; }
	    else if (s && *s == '\t') { *p++ = ' '; s++; }
	    else if (s && (*s == '\r' || *s == '\n')) { s++; }
	    else if(s) *p++ = *s++;
	...
	case 4: /* get current->end or skip <TAG> */
		...
		*p = '\0'; trail_space (text);
		if (text[0] != '\0')
		    current->text[current->lines++] = strdup33(text);
...
static subtitle *sub_read_line_subrip(stream_t* st, subtitle *current, int utf16) {
	...
	current->start = (unsigned long) (a1*360000+a2*6000+a3*100+a4) ;
	current->end   = (unsigned long) (b1*360000+b2*6000+b3*100+b4) ;
	...
	for (current->lines=1; current->lines < SUB_MAX_TEXT; current->lines++) {
	    ...
	    current->text[current->lines-1]=malloc33(len+1);
...
static subtitle *sub_ass_read_line_subviewer(stream_t *st, subtitle *current, int utf16)
{
    ...
    while (!current->text[0]) {
        ...
        current->start = (unsigned long) (h1 * 360000 + m1 * 6000 + s1 * 100 + ms1 / 10) ;
        current->end   = (unsigned long) (h2 * 360000 + m2 * 6000 + s2 * 100 + ms2 / 10) ;
        ...
        if (full_line[0]) {
            ...
            current->text[0] = strdup33(converted_line);
            current->lines = 1;
        }
    }
    return current;
...
static subtitle *sub_read_line_subviewer(stream_t *st,subtitle *current, int utf16) {
	...
	if ((len=sscanf (line, "%d:%d:%d%[,.:]%d --> %d:%d:%d%[,.:]%d",&a1,&a2,&a3,(char *)&i,&a4,&b1,&b2,&b3,(char *)&i,&b4)) < 10)
	    continue;
	current->start = (unsigned long) (a1*360000+a2*6000+a3*100+a4/10) ;
	current->end   = (unsigned long) (b1*360000+b2*6000+b3*100+b4/10) ;
	for (i=0; i<SUB_MAX_TEXT;) {
	    ...
	    if (len && !blank) {
                int j=0,skip=0;
		char *curptr=current->text[i]=malloc33(len+1);
...
static subtitle *sub_read_line_vplayer(stream_t *st,subtitle *current, int utf16) {
	...
	while (!current->text[0]) {
		...
		if (!(current->start = (unsigned long) (a1*360000+a2*6000+a3*100) ))
			continue;
                /* removed by wodzu
		p=line;
 ...
 */
	...
	current->end   = 360000*hour2 + 6000*min2 + 100*sec2 + hunsec2;
		...
		current->text[num]=malloc33(tmp-line2+1);
	...
	current->text[num]=strdup33(line2);
	current->lines++;
...
retry:
    ...
    current->start = (unsigned long) (a1*360000+a2*6000+a3*100);
...
static subtitle *sub_read_line_jacosub(stream_t* st, subtitle * current, int utf16)
{
	...
	if (sscanf
		...
		if (line1[0] == '#') {
			...
			1;
		    unsigned units = (unsigned)jacoShift;
	...
	for (q = line1; (!eol(*p)) && (current->lines < SUB_MAX_TEXT); ++p) {
		...
		if (*(p + 1) == 'n') {
		    *q = '\0';
		    q = line1;
		    current->text[current->lines++] = strdup33(line1);
		    ++p;
		    break;
		}
		if ((toupper(*(p + 1)) == 'C')
...
static subtitle* sub_fribidi (subtitle *sub, int sub_utf8, int from)
{
  ...
  while (l > from) {
    ...
    if(len > 0) {
      if((op = malloc33((FFMAX(2*orig_len,2*len) + 1))) == NULL) {
	mp_msg(MSGT_SUBREADER,MSGL_WARN,"SUB: error allocating mem.\n");
	l++;
	break;
      }
...
static void adjust_subs_time(subtitle* sub, float subtime, float fps, int block,
	...
	unsigned long overlap = (unsigned long) ((sub_uses_time ? 100 : fps) / 5) ; // 0.2s
...
const char* guess_cp(stream_t *st, const char *preferred_language, const char *fallback)
{
    ...
    buffer = malloc33(MAX_GUESS_BUFFER_SIZE);
...
//    printf("%s     %s  %d\n",__func__,filename,__LINE__);
    ...
    sub_num=0;n_max=32;
    first=malloc33(n_max*sizeof(subtitle));
    if(!first){
#if CONFIG_ICONV
	  subcp_close();
    ...
    while(1){
        if(sub_num>=n_max){
            n_max+=16;
            first=realloc33(first,n_max*sizeof(subtitle));
        }
#ifndef CONFIG_SORTSUB
	sub = &first[sub_num];
#endif
...
// here we manage overlapping subtitles
	...
	placeholder = malloc33(sizeof(int *) * counter);
	for (i = 0; i < counter; ++i) {
	    placeholder[i] = malloc33(sizeof(int) * lines_to_add);
 ...
 //   printf("%s     %s  %d\n",__func__,filename,__LINE__);
	...
	do {
		...
		// subtitles
		second = realloc33(second, (sub_num + sub_to_add + 1) * sizeof(subtitle));if(second == NULL) return NULL;
		for (j = 0; j <= sub_to_add; ++j) {
		    int ls;
			...
			second[sub_num + j].text[ls] = strdup33(first[sub_first + j].text[ls]);
		    }
		}
	    ...
	    // subs.
	    second = realloc33(second, (sub_num + 1) * sizeof(subtitle));if(second == NULL) return NULL;
		...
		if (placeholder[counter][j] != -1) {
			...
			second[sub_num].text[i++] = strdup33(first[placeholder[counter][j]].text[ls]);
		    }
		    j += lines - 1;
		} else {
		    second[sub_num].text[i++] = strdup33(" ");
		}
	    }
	    ++sub_num;
	    ++counter;
   ...
   // printf("%s     %s  %d\n",__func__,filename,__LINE__);
    ...
    subt_data = malloc33(sizeof(sub_data));
    subt_data->filename = strdup33(filename);
                                 ...
                                 const char *fname, int limit_fuzziness)
{
    ...
    FILE *f;
    ufs_dir_t d;
    ufs_fileinfo_t file_info_t;
    ufs_file_t *f_handle = NULL;
    // f_handle = mtos_malloc(sizeof(ufs_file_t));
   f_handle = malloc33(sizeof(ufs_file_t)); //peacer add 20130525
    memset(f_handle,0,sizeof(ufs_file_t));

    len = (strlen(fname) > 256 ? strlen(fname) : 256)
         + (strlen(path) > 256 ? strlen(path) : 256) + 2;

    f_fname       = strdup33(mp_basename(fname));
    f_fname_noext = malloc33(len);
    f_fname_trim  = malloc33(len);

    tmp_fname_noext = malloc33(len);
    tmp_fname_trim  = malloc33(len);
    tmp_fname_ext   = malloc33(len);

    tmpresult = malloc33(len);
    ...
    if (dvdsub_lang && !whiteonly(dvdsub_lang)) {
        tmp_sub_id = malloc33(strlen(dvdsub_lang) + 1);
    ...
    ret = ufs_opendir(&d,path);
    if (flag) {
        mp_msg(MSGT_SUBREADER, MSGL_INFO, "Load subtitles in %s\n", path);
        while ((ufs_readdir(&d,&file_info_t) == FR_OK)) {
           //   templfname = file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname;
              //ufs_readdir(&d,&File_info_t);
            // retrieve various parts of the filename
          //  ufs_readdir(&d,&file_info_t) ;
            strcpy_strip_ext_lower(tmp_fname_noext, file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname);
            strcpy_get_ext(tmp_fname_ext, file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname);
            strcpy_trim(tmp_fname_trim, tmp_fname_noext);
//  printf("%s    %d    %s    %s\n",__func__,__LINE__,d.name,file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname);
  //printf("%s    %d    %s    %s\n",__func__,__LINE__, file_info_t.fileinfo.lfname, file_info_t.fileinfo.fname);
            ...
            if (strcasecmp(tmp_fname_ext, "sub") == 0) {
                char *idx, *idxname = strdup33(file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname);

                strcpy(idxname + strlen(file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname) - sizeof("idx") + 1, "idx");
                idx = mp_dir_join(path, idxname);
                free33(idxname);
                ret = ufs_open(f_handle, idx,UFS_OPEN | UFS_READ);
...
#endif
        subpath = mp_dir_join(path, file_info_t.fileinfo.lfname[0] ? file_info_t.fileinfo.lfname : file_info_t.fileinfo.fname);
                unsigned char *subpath_temp = (unsigned char *)subpath;
                     ret = ufs_open(f_handle, subpath_temp,UFS_OPEN | UFS_READ);
    ...
    free33(f_handle);
    free33(f_fname);
    free33(f_fname_noext);
    free33(f_fname_trim);

    free33(tmp_fname_noext);
    free33(tmp_fname_trim);
    free33(tmp_fname_ext);
                                 ...
                                 const char *fname, int limit_fuzziness)
{
    ...
    if (d) {
        ...
        while ((de = readdir(d))) {
            ...
            // there is one, it's certainly a vobsub so we skip it.
            if (strcasecmp(tmp_fname_ext, "sub") == 0) {
                char *idx, *idxname = strdup33(de->d_name);
...
void load_subtitles(const char *fname, float fps, open_sub_func add_f)
{
    ...
    slist.sid  = 0;
    slist.subs = calloc33(MAX_SUBTITLE_FILES, sizeof(*slist.subs));
    if (!slist.subs)
        return;
...
void sub_free33( sub_data * subd )
{
    ...
    for (i = 0; i < subd->sub_num; i++)
        for (j = 0; j < subd->subtitles[i].lines; j++)
            free33( subd->subtitles[i].text[j] );
    free33( subd->subtitles );
    free33( subd->filename );
    free33( subd );
}
