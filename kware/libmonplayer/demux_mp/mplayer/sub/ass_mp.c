...
static void init_style(ASS_Style *style, const char *name, double playres)
{
	double fs;
	uint32_t c1, c2;
	style->Name = strdup33(name);
	style->FontName = (font_fontconfig >= 0 && sub_font_name) ? strdup33(sub_font_name) : (font_fontconfig >= 0 && font_name) ? strdup33(font_name) : strdup33("Sans");
...
ASS_Track* ass_read_subdata(ASS_Library* library, sub_data* subdata, double fps) {
	...
	track->name = subdata->filename ? strdup33(subdata->filename) : 0;
...
ASS_Track* ass_read_stream(ASS_Library* library, const char *fname, char *charset) {
	...
	if (fd->end_pos > io_stream_buffer_size)
	...
	for (;;) {
		...
		if (buf_alloc < sz + io_stream_buffer_size)
			buf_alloc += io_stream_buffer_size;
		buf = realloc33(buf, buf_alloc + 1);if(buf == NULL) return NULL;
	...
	buf[sz] = 0;
	buf = realloc33(buf, sz + 1);
	track = ass_read_memory(library, buf, sz, charset);
	if (track) {
		free33(track->name);
		track->name = strdup33(fname);
	}
	free33(buf);
	return track;
}
...
static void ass_configure_fonts(ASS_Renderer* priv) {
	...
	dir = get_path("fonts");
	if (font_fontconfig < 0 && sub_font_name) path = strdup33(sub_font_name);
	else if (font_fontconfig < 0 && font_name) path = strdup33(font_name);
	else path = get_path("subfont.ttf");
	if (font_fontconfig >= 0 && sub_font_name) family = strdup33(sub_font_name);
	else if (font_fontconfig >= 0 && font_name) family = strdup33(font_name);
	else family = 0;
...
//	long long ts_ms = (ts + sub_delay) * 1000 + .5;
    ...
    #ifdef CONFIG_ASS
	aimg = sub_visibility && ts != MP_NOPTS_VALUE ?
